...
function addPageBinding(bindingName) {
  ...
  window[bindingName] = async(...args) => {
    ...
    return promise;
        ...
        await asyncio.wait(
            [asyncio.create_task(_evaluate(frame, expression)) for frame in self.frames]
        )

    async def authenticate(self, credentials: dict[str, str] | None) -> None:
    ...
    async def goto(
        self, url: str, options: dict[str, str] | None = None, **kwargs: Any
    ) -> Response | None:
        """Go to the ``url``.
