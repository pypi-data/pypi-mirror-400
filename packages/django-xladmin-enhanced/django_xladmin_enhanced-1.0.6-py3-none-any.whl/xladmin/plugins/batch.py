...
from xladmin.layout import FormHelper, Layout, Fieldset, Container, Col
from xladmin.plugins.actions import BaseActionView, ACTION_CHECKBOX_NAME
from xladmin.util import model_ngettext, vendor
from xladmin.views.base import filter_hook
from xladmin.views.edit import ModelFormAdminView
...
class ChangeFieldWidgetWrapper(forms.Widget):
    ...
    @property
    def media(self):
        media = self.widget.media + vendor('xladmin.plugin.batch.js')
        return media
...
class BatchChangeAction(BaseActionView):
    ...
    def get_media(self):
        ...
        media = media + self.form_obj.media + self.vendor(
            'xladmin.page.form.js', 'xladmin.form.css')
        return media
