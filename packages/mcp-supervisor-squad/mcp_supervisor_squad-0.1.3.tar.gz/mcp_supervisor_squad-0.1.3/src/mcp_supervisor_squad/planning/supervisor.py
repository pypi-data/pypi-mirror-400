...
from ..persistence.storage import StorageLayout, atomic_write_json, find_repo_root, prune_repo_storage, utc_now_iso
...
def write_requirements_single(*, layout: StorageLayout, run_id: str, query: str, primary_root: Path) -> Path:
    ...
    text = "\n".join(
        [
            ...
            "",
            "## Next Actions",
            "- Include a short summary (<=10 lines) in the patch footer only if needed (touched files, risks, follow-ups).",
            "",
        ]
    )
    ...
    *,
    ...
    primary_text = "\n".join(
        [
            ...
            "",
            "## Next Actions",
            "- If cross-project sync is needed, update `contract.json` fields only (keep <= 4 KB).",
            "",
        ]
    )
    secondary_text = "\n".join(
    ...
    contract = {
        ...
        "sync_rules": [
            "Only share minimal API contract (routes/schema/field names/status codes).",
            "Keep this file <= 4KB. Do not paste logs or full files.",
        ],
        "notes": "Fill only what's needed for cross-project coordination.",
        "request": query.strip(),
        "primary_root": primary_rel,
        "secondary_root": secondary_rel,
        "api_contract": {},
    }
    raw = json.dumps(contract, ensure_ascii=False, indent=2)
    if len(raw.encode("utf-8")) > 4096:
        contract = {k: contract[k] for k in ("version", "generated_at", "primary_root", "secondary_root", "api_contract")}
    atomic_write_json(paths.contract_path, contract)
    return paths.requirements_primary_path, paths.requirements_secondary_path, paths.contract_path


def prune_on_start(*, repo_root: Path, retention_days: int) -> None:
    layout = StorageLayout.for_repo(repo_root)
    prune_repo_storage(layout=layout, retention_days=retention_days)
