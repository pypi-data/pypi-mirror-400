...
def _format_shell_output(
    ...
    # Summary mode: for allowlisted commands with lots of output, just summarize
    stdout_lines = stdout.count("\n") + 1 if stdout else 0
    stderr_lines = stderr.count("\n") + 1 if stderr else 0
    use_summary = allowlisted and (stdout_lines > 20 or stderr_lines > 10)

    if use_summary:
        # Just show a brief summary since output was already printed live
        if timed_out:
            return f"Command timed out (after {timeout_value}s)\n"
        elif interrupted:
            return f"Command interrupted\n"
        elif returncode and returncode != 0:
            return f"Command failed (exit {returncode})\n"
        else:
            return f"Command succeeded ({stdout_lines} lines)\n"

    # Apply aggressive shortening for non-summary output
    stdout = _shorten_stdout(
        stdout, pre_tokens=500, post_tokens=1000, logdir=logdir, cmd=cmd
    )
    stderr = _shorten_stdout(
        stderr, pre_tokens=500, post_tokens=500, logdir=logdir, cmd=f"{cmd} (stderr)"
    )
