...
#define GLOO_LOG_MSG(level, ...)                                                                                       \
    std::cerr << ::gloo::MakeString("[", __FILE__, ":", __LINE__, "] ", level, " ", __VA_ARGS__, "\n")
...
#define GLOO_ENFORCE(condition, ...)                                                                                   \
    do {                                                                                                               \
        if (!(condition)) {                                                                                            \
            throw ::gloo::EnforceNotMet(__FILE__, __LINE__, #condition, ::gloo::MakeString(__VA_ARGS__));              \
        }                                                                                                              \
    } while (false)
...
#define GLOO_ENFORCE_THAT_IMPL(condition, expr, ...)                                                                   \
    do {                                                                                                               \
        ...
        if (r.bad()) {                                                                                                 \
            throw EnforceNotMet(__FILE__, __LINE__, expr, r.get_message_and_free(MakeString(__VA_ARGS__)));            \
        }                                                                                                              \
    } while (false)
}  // namespace enforce_detail

#define GLOO_ENFORCE_THAT(condition, ...) GLOO_ENFORCE_THAT_IMPL((condition), #condition, __VA_ARGS__)

#define GLOO_ENFORCE_EQ(x, y, ...) GLOO_ENFORCE_THAT_IMPL(Equals((x), (y)), #x " == " #y, __VA_ARGS__)
#define GLOO_ENFORCE_NE(x, y, ...) GLOO_ENFORCE_THAT_IMPL(NotEquals((x), (y)), #x " != " #y, __VA_ARGS__)
#define GLOO_ENFORCE_LE(x, y, ...) GLOO_ENFORCE_THAT_IMPL(LessEquals((x), (y)), #x " <= " #y, __VA_ARGS__)
#define GLOO_ENFORCE_LT(x, y, ...) GLOO_ENFORCE_THAT_IMPL(Less((x), (y)), #x " < " #y, __VA_ARGS__)
#define GLOO_ENFORCE_GE(x, y, ...) GLOO_ENFORCE_THAT_IMPL(GreaterEquals((x), (y)), #x " >= " #y, __VA_ARGS__)
#define GLOO_ENFORCE_GT(x, y, ...) GLOO_ENFORCE_THAT_IMPL(Greater((x), (y)), #x " > " #y, __VA_ARGS__)
