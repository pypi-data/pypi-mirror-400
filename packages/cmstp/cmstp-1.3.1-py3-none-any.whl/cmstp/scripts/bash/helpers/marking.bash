_cmstp_marker() {
	: '
	Generate a CMSTP marker string.
	...
	if [[ "$type" == "START" ]]; then
		...
		log_step "(_cmstp_marker) Invalid marker type '$type'" true
		return 1
	fi

	local hashes=$(printf '#%.0s' $(seq 1 $n_hashes))
	echo -e "${hashes} CMSTP ${type} ${hashes}"
}
...
markers_exist() {
	...
	Args:
	  - file:           Path to the file.
	  - start_marker:   Start marker string (default: CMSTP start marker)
	  - end_marker:     End marker string (default: CMSTP end marker)
	Outputs:
	  None
	Returns:
	...
	'
	local file="$1"
	local start_marker="${2:-$(_cmstp_marker 'START')}"
	local end_marker="${3:-$(_cmstp_marker 'END')}"
...
write_marked() {
	: '
	Log a message or file content to a file, wrapped with CMSTP start and end markers.
	...
	# Get markers
	local start_marker="$(_cmstp_marker 'START')"
	local end_marker="$(_cmstp_marker 'END')"
...
remove_markers() {
	: '
	Remove all sections wrapped by the CMSTP markers from a file.
	...
	local start_marker="${3:-$(_cmstp_marker 'START')}"
	local end_marker="${4:-$(_cmstp_marker 'END')}"
