...
#
# Usage: cat drhook.* | perl -w drhook_merge_walltime_max.pl
#
...
#  printf ("Total time : %.3f secs (sum over all MPI-tasks; only max OpenMP-time accounted for)\n", $tottim);
  ...
  open(PIPE,"|sort -k5,6 -nr|sed 's/ %/%/'"); # sorts w.r.t. max column# (column number starts from 0, not 1 in unix sort)
