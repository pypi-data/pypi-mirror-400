    ...
    def __init__(
        ...
        if (self.pipeName == "TrainPipeline"):
            ...
            if self.dim == 3:
                ...
                shape.append(image_num_channel)
                shape.append(self.batch_size)
                self.img_reshape_dflip = fn.Reshape(size=shape, tensorDim=len(
                    shape), layout='', dtype=dt.FLOAT32, device='hpu')  # img [W*H,D,C,N]
                ...
                shape.append(shape_patch[2] * image_num_channel)
                shape.append(self.batch_size)
                self.img_reshape_noise = fn.Reshape(size=shape, tensorDim=len(
                    shape), layout='', dtype=dt.FLOAT32, device='hpu')  # img [W,H,D*C,N]
            ...
            shape.append(
                ...
                self.patch_size[1] *
                self.patch_size[2] *
                image_num_channel)  # W*H*D*C
            shape.append(self.batch_size)  # W*H*D*C, N
            self.img_reshape_before_mul = fn.Reshape(size=shape,
                                                     tensorDim=2,
                                                     layout='',
                                                     dtype=dt.FLOAT32, device='hpu')  # [W*H*D*C,N]
