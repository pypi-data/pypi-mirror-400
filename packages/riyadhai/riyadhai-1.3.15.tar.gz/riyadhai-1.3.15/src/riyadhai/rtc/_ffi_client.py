...
def get_ffi_lib():
    ...
    res = importlib.resources.files("riyadhai.rtc.resources") / "bin" / libname
...
def _ensure_ffi_symbol(expected_name: str, fallback_name_b64: bytes) -> None:
    try:
        getattr(ffi_lib, expected_name)
        return
    except AttributeError:
        pass

    fallback_name = _base64.b64decode(fallback_name_b64).decode("ascii")
    setattr(ffi_lib, expected_name, getattr(ffi_lib, fallback_name))


_ensure_ffi_symbol("riyadhai_ffi_initialize", b"bGl2ZWtpdF9mZmlfaW5pdGlhbGl6ZQ==")
_ensure_ffi_symbol("riyadhai_ffi_request", b"bGl2ZWtpdF9mZmlfcmVxdWVzdA==")
_ensure_ffi_symbol("riyadhai_ffi_drop_handle", b"bGl2ZWtpdF9mZmlfZHJvcF9oYW5kbGU=")
_ensure_ffi_symbol("riyadhai_ffi_dispose", b"bGl2ZWtpdF9mZmlfZGlzcG9zZQ==")

# C function types
ffi_lib.riyadhai_ffi_initialize.argtypes = [
    ffi_cb_fnc,
    ctypes.c_bool,
    ctypes.c_char_p,
    ctypes.c_char_p,
...
ffi_lib.riyadhai_ffi_request.argtypes = [
    ctypes.POINTER(ctypes.c_ubyte),
    ctypes.c_size_t,
    ctypes.POINTER(ctypes.POINTER(ctypes.c_ubyte)),
    ctypes.POINTER(ctypes.c_size_t),
]
ffi_lib.riyadhai_ffi_request.restype = ctypes.c_uint64

ffi_lib.riyadhai_ffi_drop_handle.argtypes = [ctypes.c_uint64]
ffi_lib.riyadhai_ffi_drop_handle.restype = ctypes.c_bool


ffi_lib.riyadhai_ffi_dispose.argtypes = []
ffi_lib.riyadhai_ffi_dispose.restype = None
...
class FfiHandle:
    ...
    def dispose(self) -> None:
        if self.handle != INVALID_HANDLE and not self._disposed:
            self._disposed = True
            assert ffi_lib.riyadhai_ffi_drop_handle(ctypes.c_uint64(self.handle))
...
class FfiClient:
    ...
    def __init__(self) -> None:
        ...
        ffi_lib.riyadhai_ffi_initialize(
            ffi_event_callback, True, b"python", __version__.encode("ascii")
        )

        @atexit.register
        def _dispose_lk_ffi():
            ffi_lib.riyadhai_ffi_dispose()
    ...
    def request(self, req: proto_ffi.FfiRequest) -> proto_ffi.FfiResponse:
        ...
        handle = ffi_lib.riyadhai_ffi_request(
