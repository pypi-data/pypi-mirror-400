...
from ... import ProcessToolCallResultPy, FriskSession
...
class FriskToolMiddleware(AgentMiddleware):
    ...
    async def awrap_tool_call(
        ...
        try:
            frisk_session = self.get_frisk_session_from_runtime(request.runtime)
    ...
    def get_frisk_session_from_runtime(self, runtime: ToolRuntime) -> FriskSession:
        runtime_context = runtime.context
        if runtime_context is None:
            raise MissingFriskSessionContextError()

        frisk_session = runtime_context.get("frisk_session", None)
        if frisk_session is None:
            raise MissingFriskSessionContextError()

        if not isinstance(frisk_session, FriskSession):
            raise InvalidFriskSessionError()

        return frisk_session
    ...
    async def block_or_execute_tool_call(
        ...
        if process_tool_call_result.decision == "deny":
            return ToolMessage(
                content=f"Call to {tool_call_info.name} denied. Reason: {process_tool_call_result.reason}",
