...
def _upload(
  ...
  if 0 <= args.wait < 10:
    raise le.EEException('Wait time should be at least 10 seconds.')
  request_id = le.data.newTaskId()[0]
...
def _decode_property_flags(args: argparse.Namespace) -> Dict[str, Any]:
  ...
  if duplicates:
    raise le.EEException('Duplicate property name(s): %s.' % duplicates)
...
def _check_valid_files(filenames: Sequence[str]) -> None:
  ...
  for filename in filenames:
    if not filename.startswith('gs://'):
      raise le.EEException('Invalid Cloud Storage URL: ' + filename)
...
class SetProjectCommand:
  ...
  def run(
    ...
    le.oauth.write_private_json(config_path, config)
...
class AclChCommand:
  ...
  def run(
    ...
    acl = le.data.getAssetAcl(args.asset_id)
    self._apply_permissions(acl, permissions)
    le.data.setAssetAcl(args.asset_id, json.dumps(acl))
  ...
  def _set_permission(
    ...
    if len(parts) != 2 or parts[1] not in ['R', 'W']:
      raise le.EEException('Invalid permission "%s".' % grant)
    user, role = parts
    prefixed_user = user
    if not self._is_all_users(user):
      prefixed_user = prefix + user
    if prefixed_user in permissions:
      raise le.EEException('Multiple permission settings for "%s".' % user)
    if self._is_all_users(user) and role == 'W':
      raise le.EEException('Cannot grant write permissions to all users.')
...
class AssetInfoCommand:
  ...
  def run(
    ...
    config.ee_init()
    info = le.data.getInfo(args.asset_id)
    if info:
      _pretty_print_json(info)
    else:
      raise le.EEException(
...
class AssetSetCommand:
  ...
  def run(
    ...
    if not properties and args.time_start is None and args.time_end is None:
      raise le.EEException('No properties specified.')
    update_mask = [
    ...
    le.data.updateAsset(args.asset_id, asset, update_mask)
    return
...
class CreateCommandBase:
  ...
  def run(
    ...
    config.ee_init()
    le.data.create_assets(args.asset_id, self.asset_type, args.parents)
...
class CreateCollectionCommand(CreateCommandBase):
  ...
  def __init__(self, parser: argparse.ArgumentParser):
    super().__init__(
        parser, 'an image collection', le.data.ASSET_TYPE_IMAGE_COLL)
...
class CreateFolderCommand(CreateCommandBase):
  ...
  def __init__(self, parser: argparse.ArgumentParser):
    super().__init__(parser, 'a folder', le.data.ASSET_TYPE_FOLDER)
...
class ListCommand:
  ...
  def _print_assets(self, assets, max_items, indent, long_format, recursive):
    ...
    if recursive:
      ...
      max_type_length = le.data.MAX_TYPE_LENGTH
    ...
    for asset in assets:
      ...
      if recursive and asset['type'] in (le.data.ASSET_TYPE_FOLDER,
                                         le.data.ASSET_TYPE_FOLDER_CLOUD):
        list_req = {'id': asset['id']}
        children = le.data.getList(list_req)
...
class SizeCommand:
  ...
  def run(
    ...
    if not args.asset_id:
      assets = le.data.getAssetRoots()
    else:
      assets = [le.data.getInfo(asset) for asset in args.asset_id]
    ...
    for index, asset in enumerate(assets):
      ...
      is_parent = asset['type'] in (
          le.data.ASSET_TYPE_FOLDER,
          le.data.ASSET_TYPE_IMAGE_COLL,
          le.data.ASSET_TYPE_FOLDER_CLOUD,
          le.data.ASSET_TYPE_IMAGE_COLL_CLOUD,
      )
      if not is_parent or args.summarize:
        self._print_size(asset)
      else:
        children = le.data.getList({'id': asset['id']})
        if not children:
          # A leaf asset
          children = [asset]
  ...
  def _get_size_image_collection(self, asset):
    images = le.ImageCollection(asset['id'])
...
class MoveCommand:
  ...
  def run(
    ...
    config.ee_init()
    le.data.renameAsset(args.source, args.destination)
...
class RmCommand:
  ...
  def _safe_get_info(self, asset_id, verbose):
    try:
      return le.data.getInfo(asset_id)
    except le.EEException as e:
      if verbose:
  ...
  def _delete_asset(self, asset_id, recursive, verbose, dry_run):
    ...
    if recursive:
      ...
      children = le.data.getList({'id': asset_id})
      for child in children:
...
class TaskCancelCommand:
  ...
  def run(
    ...
    if cancel_all:
      statuses = le.data.getTaskList()
    else:
      statuses = le.data.getTaskStatus(args.task_ids)
    for status in statuses:
      state = status['state']
      task_id = status['id']
      if state == 'UNKNOWN':
        raise le.EEException('Unknown task id "%s"' % task_id)
      elif state == 'READY' or state == 'RUNNING':
        print('Canceling task "%s"' % task_id)
        le.data.cancelTask(task_id)
      elif not cancel_all:
...
class TaskInfoCommand:
  ...
  def run(
    ...
    config.ee_init()
    for i, status in enumerate(le.data.getTaskStatus(args.task_id)):
      if i:
        print()
      print('%s:' % status['id'])
...
def _using_v1alpha(func):
  ...
  def inner(
    ...
    original = le._cloud_api_utils.VERSION
    le._cloud_api_utils.VERSION = 'v1alpha'
    func(self, args, config)
    le._cloud_api_utils.VERSION = original
...
class UploadImageCommand:
  ...
  def run(
    ...
    _upload(args, manifest, le.data.startIngestion)
  ...
  def manifest_from_args(self, args: argparse.Namespace) -> Dict[str, Any]:
    ...
    args.asset_id = le.data.convert_asset_id_to_asset_name(args.asset_id)
...
class UploadExternalImageCommand:
  ...
  def run(
    ...
    config.ee_init()
    manifest = self.manifest_from_args(args)
    name = le.data.startExternalImageIngestion(manifest, args.force)['name']
...
class UploadTableCommand:
  ...
  def run(
    ...
    config.ee_init()
    manifest = self.manifest_from_args(args)
    _upload(args, manifest, le.data.startTableIngestion)
...
class UploadImageManifestCommand(_UploadManifestBase):
  ...
  def run(
    ...
    super().run(args, config, le.data.startIngestion)
...
class UploadTableManifestCommand(_UploadManifestBase):
  ...
  def run(
    ...
    super().run(args, config, le.data.startTableIngestion)
...
class ProjectConfigGetCommand:
  ...
  def run(
    ...
    del args  # Unused.
    config.ee_init()
    print(le.data.getProjectConfig())
