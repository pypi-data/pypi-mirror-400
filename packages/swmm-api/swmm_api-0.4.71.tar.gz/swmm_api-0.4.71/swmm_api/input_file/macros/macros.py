...
from .tags import get_subcatchment_tags, get_node_tags, get_link_tags
from .. import SEC
from ..inp import SwmmInput
from ..section_labels import *
from ..section_lists import LINK_SECTIONS, NODE_SECTIONS, SUBCATCHMENT_SECTIONS
...
def increase_max_node_depth(inp, node_label):
    """
    Raises maximum node depth to surrounding xsection height.

    Args:
        inp (SwmmInput): SWMM input-file data.
...
def subcatchments_data_frame(inp: SwmmInput, label_sep='.', add_section_label=False):
    """
    Combine all information of the subcatchments from the input file into a single data-frame.

    Includes SUBCATCHMENTS, SUBAREAS, INFILTRATION, LOADINGS, COVERAGES, TAGS section.
    ...
    Args:
        ...
        add_section_label (bool): add section label to the column names.

    Returns:
        pandas.DataFrame: combined subcatchment data-frame.
    """
    res = []
    for sec in inp.iter_avail_section_labels(SUBCATCHMENT_SECTIONS):
        if sec == POLYGONS:
            continue
        df = inp[sec].frame
        if add_section_label:
            df = df.rename(columns=lambda c: f'{sec}{label_sep}{c}')
        res.append(df)
    res.append(get_subcatchment_tags(inp))
    return pd.concat(res, axis=1)


def nodes_data_frame(inp: SwmmInput, label_sep='.', add_section_label=False, add_coordinates=True):
    """
    Combine all information of the nodes from the input file into a single data-frame.

    Includes JUNCTIONS, OUTFALLS, DIVIDERS, STORAGE, DWF, INFLOWS, COORDINATES, TAGS section.
    ...
    Args:
        ...
        label_sep (str): separator for attribute label between section header and object attribute.
            I.e. "JUNCTIONS.elevation" with label_sep='.'
        add_section_label (bool): add section label to the column names.
        add_coordinates (bool): add coordinates to the final data frame.

    Returns:
        pd.DataFrame: nodes data frame with all available parameter columns.
    """
    _col_storage_data = 'data'
    if add_section_label:
        _col_storage_data = f'{STORAGE}{label_sep}{_col_storage_data}'

    keep_cols = {'elevation'}

    res = []
    for sec in inp.iter_avail_section_labels(NODE_SECTIONS):
        df = inp[sec].frame
        if add_section_label:
            df = df.rename(columns=lambda c: f'{sec}{label_sep}{c}' if c not in keep_cols else c)

        if sec == STORAGE:
            df[_col_storage_data] = df[_col_storage_data].astype(str)

        for sub_sec in inp.iter_avail_section_labels([DWF, INFLOWS]):
    ...
    data_list = [pd.concat(res, axis=0), get_node_tags(inp)]
    if add_coordinates:
        data_list.append(inp[COORDINATES].frame)
    return pd.concat(data_list, axis=1)


def links_data_frame(inp: SwmmInput, label_sep='.', add_section_label=False, add_vertices=False):
    """
    Combine all information of the links from the input file into a single data-frame.

    Includes CONDUITS, ORIFICES, OUTLETS, PUMPS, WEIRS, LOSSES, XSECTIONS, VERTICES, TAGS section.
    ...
    Args:
        ...
        label_sep (str): separator for attribute label between section header and object attribute.
            I.e. "JUNCTIONS.elevation" with label_sep='.'
        add_section_label (bool): add section label to the column names.
        add_vertices (bool): add vertices to the final data frame.

    Returns:
        pd.DataFrame: nodes data frame with all available parameter columns.
    """
    _col_outlet_curve = 'curve_description'
    if add_section_label:
        _col_outlet_curve = f'{OUTLETS}{label_sep}{_col_outlet_curve}'

    keep_cols = {'from_node', 'to_node'}

    res = []
    for sec in inp.iter_avail_section_labels(LINK_SECTIONS):
        df = inp[sec].frame
        if add_section_label:
            df = df.rename(columns=lambda c: f'{sec}{label_sep}{c}' if c not in keep_cols else c)

        if sec == OUTLETS:
            df[_col_outlet_curve] = df[_col_outlet_curve].astype(str)

        res.append(df)

    data_list = [pd.concat(res, axis=0), get_link_tags(inp)]

    for sec in inp.iter_avail_section_labels([XSECTIONS, LOSSES]):
        df = inp[sec].frame
        if add_section_label:
            df = df.rename(columns=lambda c: f'{sec}{label_sep}{c}')
        data_list.append(df)
    ...
    if add_vertices:
        data_list.append(inp[VERTICES].frame)
    return pd.concat(data_list, axis=1)


def iter_sections(inp: SwmmInput, section_list: (list, tuple, set)):
    return inp.iter_avail_sections(section_list)
...
def _convert_cms_to_lps(inp: SwmmInput):
    # TODO Testing
...
def get_link_offset(link: _Link) -> float:
    """
    Get the upstream/inlet offset of a link independent of link type.
    Returns None for pump link object since it has no offset height.

    Args:
        link (_Link or Conduit or Weir or Orifice or Outlet): any link object

    Returns:
        float: offset height.
    """
    if isinstance(link, Conduit):
        return link.offset_upstream
    elif isinstance(link, Weir):
        return link.height_crest
    elif isinstance(link, (Orifice, Outlet)):
        return link.offset
    return
