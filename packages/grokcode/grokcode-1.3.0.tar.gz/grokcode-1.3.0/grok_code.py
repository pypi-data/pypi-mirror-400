...
PROVIDERS = {
    ...
    "openrouter": {
        ...
        "models": {
            ...
            "x-ai/grok-3-beta": "Grok 3 Beta (xAI via OpenRouter)",
        },
    },
}
...
XAI_BASE_URL = PROVIDERS["xai"]["base_url"]
AVAILABLE_MODELS = PROVIDERS["xai"]["models"]
...
MODELS_CACHE_DIR = Path.home() / ".grokcode" / "cache"
MODELS_CACHE_EXPIRY_HOURS = 24


def fetch_models_from_api(provider: str, api_key: str = None) -> list[dict] | None:
    ...
    if not api_key:
        ...
        if not api_key:
            # Try keyring (imported lazily)
            try:
                import keyring
                api_key = keyring.get_password("grokcode", provider_config["keyring_username"])
            except Exception:
                pass
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=15) as response:
            ...
            for m in models:
                ...
                if "embed" in model_id.lower() or "whisper" in model_id.lower():
                    continue
        ...
        # API error - return None to fall back to defaults
        return None
    except Exception:
        return None


def get_models_cache_path(provider: str) -> Path:
    """Get the cache file path for a provider's models."""
    return MODELS_CACHE_DIR / f"models_{provider}.json"


def get_cached_models(provider: str) -> list[dict] | None:
    """Get models from local cache if valid."""
    cache_path = get_models_cache_path(provider)
    ...
    try:
        ...
        expiry_seconds = MODELS_CACHE_EXPIRY_HOURS * 3600
        if time.time() - cached_at > expiry_seconds:
            return None
...
def save_models_to_cache(provider: str, models: list[dict]) -> bool:
...
def get_available_models(provider: str, force_refresh: bool = False, api_key: str = None) -> list[dict]:
    """Get available models for a provider (from cache or API).
    ...
    fetched = fetch_models_from_api(provider, api_key)
    if fetched:
        save_models_to_cache(provider, fetched)
        return fetched

    # Fall back to hardcoded defaults
    provider_config = PROVIDERS.get(provider, PROVIDERS[DEFAULT_PROVIDER])
    default_models = provider_config.get("models", {})
    return [
        {"id": model_id, "name": desc, "context_length": 0}
...
def run_doctor() -> dict[str, Any]:
    ...
    for provider_id, provider_config in PROVIDERS.items():
        ...
        check_key = f"api_key_{provider_id}"

        if api_key:
            # Check source (env var or keyring)
            if os.environ.get(env_var):
                checks[check_key] = {"status": "ok", "message": f"{provider_name}: {env_var} is set"}
            else:
                checks[check_key] = {"status": "ok", "message": f"{provider_name}: stored in keyring"}
        else:
            checks[check_key] = {"status": "info", "message": f"{provider_name}: not configured"}
...
def get_api_key_from_keyring(provider: str = DEFAULT_PROVIDER) -> str | None:
    """Get API key from secure keyring storage for a provider."""
    try:
        import keyring
        provider_config = PROVIDERS.get(provider, PROVIDERS[DEFAULT_PROVIDER])
        keyring_username = provider_config["keyring_username"]
...
def set_api_key_in_keyring(api_key: str, provider: str = DEFAULT_PROVIDER) -> dict[str, Any]:
    """Store API key in secure keyring storage for a provider."""
    try:
        import keyring
        provider_config = PROVIDERS.get(provider, PROVIDERS[DEFAULT_PROVIDER])
        keyring_username = provider_config["keyring_username"]
        keyring.set_password(KEYRING_SERVICE, keyring_username, api_key)
        return {"success": True, "message": f"API key stored securely for {provider_config['name']}"}
    except ImportError:
...
def delete_api_key_from_keyring(provider: str = DEFAULT_PROVIDER) -> dict[str, Any]:
    """Delete API key from secure keyring storage for a provider."""
    try:
        import keyring
        provider_config = PROVIDERS.get(provider, PROVIDERS[DEFAULT_PROVIDER])
        keyring_username = provider_config["keyring_username"]
        keyring.delete_password(KEYRING_SERVICE, keyring_username)
        return {"success": True, "message": f"API key removed for {provider_config['name']}"}
    except ImportError:
        return {"success": False, "error": "keyring package not installed"}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
def get_api_key(provider: str = DEFAULT_PROVIDER) -> str | None:
    """Get API key from environment or secure storage for a provider.

    Priority:
    1. Provider-specific environment variable (XAI_API_KEY or OPENROUTER_API_KEY)
    ...
    # Try keyring
    return get_api_key_from_keyring(provider)
...
class GrokCode:
    ...
    def __init__(self, skip_permissions: bool = False, model: str = None, provider: str = None):
        ...
        if self.provider not in PROVIDERS:
            self.console.print(f"[error]Unknown provider: {self.provider}[/error]")
            self.console.print(f"[dim]Available providers: {', '.join(PROVIDERS.keys())}[/dim]")
            sys.exit(1)

        provider_config = PROVIDERS[self.provider]

        # Model: CLI arg > config > provider default
        self.model = model or self.config.get("model") or provider_config["default_model"]
        ...
        if not api_key:
            env_var = provider_config["env_var"]
            self.console.print(
                f"[error]Error: {env_var} not set[/error]\n"
                f"Set it with: export {env_var}='your-key'\n"
                f"Or store securely with: grokcode login --provider {self.provider}"
            )
            sys.exit(1)
    ...
    def print_welcome(self):
        ...
        cwd = get_cwd_display()
        provider_name = PROVIDERS[self.provider]["name"]
        self.console.print()
        self.console.print(f"[bold magenta]╭─ Grok Code v{VERSION} ─╮[/bold magenta]")
        self.console.print(f"[dim]│ Provider: {provider_name}[/dim]")
...
[bold]Commands:[/bold]
  ...
  /models      List available models or switch model
  /provider    Switch provider (xai, openrouter)
...
"""
    ...
    def show_models(self, refresh: bool = False):
        ...
        with Progress(
            ...
            if refresh:
                progress.add_task("Fetching models from API...", total=None)
            else:
                progress.add_task("Loading models...", total=None)

            models = get_available_models(self.provider, force_refresh=refresh)

        self.console.print(f"[bold]{provider_config['name']} Models:[/bold]")
        self.console.print(f"[dim]Current: {self.model}[/dim]\n")

        # Group by provider/org for OpenRouter
        if self.provider == "openrouter" and len(models) > 20:
            # Show top models grouped by org
            orgs = {}
            for m in models:
                model_id = m["id"]
            ...
            for org in sorted(orgs.keys()):
                org_models = orgs[org][:5]  # Show top 5 per org
                self.console.print(f"  [cyan]{org}[/cyan]")
                for m in org_models:
                    ctx = f" ({m['context_length']:,} ctx)" if m.get("context_length") else ""
                    marker = " [green](current)[/green]" if m["id"] == self.model else ""
                    self.console.print(f"    {m['id']}{ctx}{marker}")
                if len(orgs[org]) > 5:
                    self.console.print(f"    [dim]... and {len(orgs[org]) - 5} more[/dim]")
            self.console.print(f"\n[dim]Total: {len(models)} models. Use /models MODEL_ID to switch.[/dim]")
        else:
            for m in models[:30]:
                ctx = f" ({m['context_length']:,} ctx)" if m.get("context_length") else ""
                marker = " [green](current)[/green]" if m["id"] == self.model else ""
                self.console.print(f"  [cyan]{m['id']}[/cyan]{ctx}{marker}")
            if len(models) > 30:
                self.console.print(f"\n[dim]... and {len(models) - 30} more models[/dim]")
    ...
    def switch_model(self, model_id: str):
        ...
        models = get_available_models(self.provider)
        model_ids = [m["id"] for m in models]

        if model_id not in model_ids:
            # Try fuzzy match
            matches = [m for m in model_ids if model_id.lower() in m.lower()]
            if len(matches) == 1:
                model_id = matches[0]
            elif len(matches) > 1:
                self.console.print(f"[warning]Ambiguous model name. Did you mean one of:[/warning]")
                for m in matches[:10]:
                    self.console.print(f"  {m}")
                return
            else:
                self.console.print(f"[error]Model not found: {model_id}[/error]")
                self.console.print("[dim]Use /models to see available models[/dim]")
                return

        old_model = self.model
        self.model = model_id
        self.console.print(f"[success]Switched model: {old_model} -> {model_id}[/success]")
    ...
    def show_providers(self):
        ...
        for provider_id, provider_config in PROVIDERS.items():
            marker = " [green](current)[/green]" if provider_id == self.provider else ""
            api_key = get_api_key(provider_id)
            status = "[green]configured[/green]" if api_key else "[dim]not configured[/dim]"
            self.console.print(f"  [cyan]{provider_id}[/cyan]{marker}")
            self.console.print(f"    {provider_config['name']} - {status}")
        self.console.print()

    def switch_provider(self, provider_id: str):
        """Switch to a different provider."""
        if provider_id not in PROVIDERS:
            self.console.print(f"[error]Unknown provider: {provider_id}[/error]")
            self.console.print(f"[dim]Available: {', '.join(PROVIDERS.keys())}[/dim]")
            return
        ...
        if not api_key:
            ...
            self.console.print(f"[error]{provider_config['name']} is not configured[/error]")
            self.console.print(f"[dim]Set {provider_config['env_var']} or run: grokcode login --provider {provider_id}[/dim]")
            return
        ...
        # Update model to provider default if current model is invalid
        self.model = provider_config["default_model"]

        # Re-initialize client
        self.client = OpenAI(api_key=api_key, base_url=provider_config["base_url"])

        self.console.print(f"[success]Switched provider: {old_provider} -> {provider_id}[/success]")
        self.console.print(f"[dim]Model set to: {self.model}[/dim]")
            ...
            try:
                ...
                if cmd.startswith("/models"):
                    ...
                    if len(parts) < 2:
                        # Just show models
                        self.show_models()
                    elif parts[1].strip() == "--refresh":
                        # Refresh from API
                        self.show_models(refresh=True)
                    else:
                        # Switch to specified model
                        self.switch_model(parts[1].strip())
                    continue
                if cmd.startswith("/provider"):
                    ...
                    if len(parts) < 2:
                        ...
                        # Switch provider
                        self.switch_provider(parts[1].strip())
                    continue
                if cmd == "/vim":
...
"""
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="refresh_models",
        help="Refresh models from API (use with --list-models)"
    )
    parser.add_argument(
        "--list-providers",
        action="store_true",
...
def handle_login_command(provider: str = None):
    """Handle 'grokcode login' subcommand."""
    from prompt_toolkit import prompt as pt_prompt
    console = Console(theme=CUSTOM_THEME)

    # Default to xai if not specified
    ...
    console.print(f"\n[bold]Store {provider_name} API Key[/bold]")
    console.print("[dim]Your API key will be stored securely in your system keychain.[/dim]\n")

    # Check if key already exists
    existing_key = get_api_key_from_keyring(provider)
    if existing_key:
        console.print(f"[warning]An API key for {provider_name} is already stored.[/warning]")
        if not Confirm.ask("Replace it?"):
            console.print("[dim]Cancelled.[/dim]")
            return

    try:
        api_key = pt_prompt(f"Enter your {provider_name} API key: ", is_password=True)
        if not api_key or not api_key.strip():
            console.print("[error]No API key provided.[/error]")
            sys.exit(1)

        result = set_api_key_in_keyring(api_key.strip(), provider)
        if result["success"]:
            console.print(f"[success]{result['message']}[/success]")
        else:
...
def handle_logout_command(provider: str = None):
    ...
    existing_key = get_api_key_from_keyring(provider)
    if not existing_key:
        console.print(f"[dim]No stored API key found for {provider_name}.[/dim]")
        return

    if Confirm.ask(f"Remove stored API key for {provider_name}?"):
        result = delete_api_key_from_keyring(provider)
        if result["success"]:
            console.print(f"[success]{result['message']}[/success]")
        else:
            console.print(f"[error]{result['error']}[/error]")
...
def parse_provider_from_args() -> str | None:
    """Parse --provider flag from command line arguments."""
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if subcommand == "update":
            ...
            provider = parse_provider_from_args()
            handle_login_command(provider)
            return
        elif subcommand == "logout":
            provider = parse_provider_from_args()
            handle_logout_command(provider)
            return
    ...
    if args.list_providers:
        ...
        for provider_id, provider_config in PROVIDERS.items():
            marker = " [green](default)[/green]" if provider_id == DEFAULT_PROVIDER else ""
            console.print(f"  [cyan]{provider_id}[/cyan]{marker}")
            console.print(f"    {provider_config['name']}")
            console.print(f"    Base URL: {provider_config['base_url']}")
            console.print(f"    Env var: {provider_config['env_var']}\n")
        return

    # Handle --list-models
    if args.list_models:
        console = Console(theme=CUSTOM_THEME)
        force_refresh = getattr(args, 'refresh_models', False)

        # If provider specified, show only that provider's models
        providers_to_show = [args.provider] if args.provider else list(PROVIDERS.keys())

        for provider_id in providers_to_show:
            if provider_id not in PROVIDERS:
                console.print(f"[error]Unknown provider: {provider_id}[/error]")
                sys.exit(1)

            provider_config = PROVIDERS[provider_id]
            console.print(f"\n[bold]{provider_config['name']} Models:[/bold]")
            ...
            with Progress(
                ...
                msg = "Fetching from API..." if force_refresh else "Loading..."
                progress.add_task(msg, total=None)
                models = get_available_models(provider_id, force_refresh=force_refresh)

            default_model = provider_config["default_model"]

            if not models:
                console.print("[dim]  No models available (API key may not be configured)[/dim]\n")
                continue

            # Show models with context length
            for m in models[:50]:
                model_id = m["id"]
                ctx = f" ({m['context_length']:,} ctx)" if m.get("context_length") else ""
                marker = " [green](default)[/green]" if model_id == default_model else ""
                console.print(f"  [cyan]{model_id}[/cyan]{ctx}{marker}")
