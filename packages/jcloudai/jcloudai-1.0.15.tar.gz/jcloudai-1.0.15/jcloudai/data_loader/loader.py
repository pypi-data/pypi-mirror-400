...
from jcloudai.exceptions import MethodNotImplementedError
from jcloudai.helpers.path import (
...
class DatasetLoader(ABC):
    ...
    def create_loader_from_schema(
        ...
        if schema.source and schema.source.type in LOCAL_SOURCE_TYPES:
            from jcloudai.data_loader.local_loader import LocalDatasetLoader

            loader = LocalDatasetLoader(schema, dataset_path)
        elif schema.view:
            from jcloudai.data_loader.view_loader import ViewDatasetLoader

            loader = ViewDatasetLoader(schema, dataset_path)
        else:
            from jcloudai.data_loader.sql_loader import SQLDatasetLoader
