...
class Shape:
    ...
    def __init__(
        ...
        self.other_data = {}
        self.mask = mask
        self._actual_scale = 1.0  # 用于存储实际的缩放比例
    ...
    def paint(self, painter):
        ...
        # 根据 scale 调整线宽，保持视觉一致性
        # 当 scale = 1.0 时（painter 已应用变换），需要反向缩放线宽
        if self.scale == 1.0:
            # 获取当前 painter 的变换矩阵来计算实际缩放
            transform = painter.transform()
            actual_scale = transform.m11()  # 获取 x 方向的缩放因子
            # 线宽除以缩放因子，保持固定的视觉宽度
            pen.setWidthF(self.PEN_WIDTH / actual_scale if actual_scale > 0 else self.PEN_WIDTH)
            # 保存实际缩放比例，供 drawVertex 使用
    ...
    def drawVertex(self, path, i):
        ...
        # 根据实际缩放比例调整圆圈大小，保持固定的视觉大小
        if hasattr(self, '_actual_scale') and self._actual_scale > 0:
            d = d / self._actual_scale
    ...
    def makePath(self):
        if self.shape_type in ["rectangle", "mask"]:
            ...
            for p in self.points[1:]:
                path.lineTo(p)
            # 关闭路径，确保 containsPoint 能正确判断点是否在多边形内
            if self._closed:
                path.closeSubpath()
        return path
