...
Furthermore, long prompts are susceptible to the "lost in the middle" problem. The model pays less attention to instructions buried deep in a large context window, leading to parts of your request being ignored [[5]](https://arxiv.org/html/2410.23884v1). Models can also struggle with output consistency, hallucinating formats or labels that were not requested, which undermines any downstream automation [[1]](https://arxiv.org/pdf/2309.08181). In short, you trade control and reliability for a false sense of simplicity.
...
First, let's set up our environment. We will use the Google Gemini library throughout this lesson.```python
# Import key packages
import asyncio
...
Next, we will define our source documents. For brevity, we will just show the titles and a high-level description.```python
...
Now, let's build our single, complex prompt. We will ask the model to generate questions, provide answers, and cite the sources, all in one go. We will use Pydantic for structured output, a concept we covered in Lesson 4.```python
...
Here’s a sample of the output we might get:```json
{
...
This modular approach brings several key benefits, moving us from hopeful prompting to deliberate engineering [[25]](https://www.vellum.ai/blog/what-is-prompt-chaining). It transforms an unpredictable process into a structured, controllable pipeline.

First, it dramatically improves **modularity and debuggability**. Each link in the chain is an isolated component with a clear, single responsibility. If the final output is wrong, you do not have to guess what went wrong in a massive prompt. You can inspect the output of each intermediate step, pinpoint the exact point of failure, and fix it [[26]](https://dev.to/kapusto/enhancing-large-language-model-performance-with-prompt-chaining-2p84). This makes the system far easier to maintain and improve over time, as changes to one step are less likely to break others.

Second, accuracy often increases. A simpler, more focused prompt is easier for the LLM to understand and execute correctly. By asking the model to do one thing at a time, like generating questions first, then answering them, then finding sources, we reduce the cognitive load and minimize the chance of instructions being missed or misinterpreted [[27]](https://www.voiceflow.com/blog/prompt-chaining). This leads to more reliable and consistent outputs.
...
However, prompt chaining is not a silver bullet. It introduces its own set of trade-offs. The most obvious are increased **latency and cost**; you are making multiple API calls instead of one [[8]](https://blog.promptlayer.com/what-is-prompt-chaining/). Each call adds to the total execution time and the final bill, which can quickly accumulate for high-volume applications. There is also a risk of **information loss** between steps. If an early step in the chain produces a summary, for example, important details might be dropped, leading to a degraded final output that lacks the richness of the original information [[7]](https://www.humanfirst.ai/blog/prompt-chaining). This can also manifest as "cascading undesired responses" if an unpredictable output from one step propagates errors downstream [[7]](https://www.humanfirst.ai/blog/prompt-chaining). Finally, managing the "glue code" that connects the steps can add engineering complexity, especially as chains become longer and more intricate [[9]](https://ai.plainenglish.io/prompt-chaining-is-dead-long-live-prompt-stuffing-58a1c08820c5). This "glue code" requires careful design and maintenance to ensure smooth data flow and error handling between distinct LLM calls, increasing the overall management difficulty of the system [[6]](https://aisdr.com/blog/what-is-prompt-chaining/).
...
Here is the Mermaid diagram illustrating this flow:```mermaid
graph TD
...
First, we create a function dedicated solely to generating questions. The prompt is simple and focused: given the content, produce a list of questions.```python
...
Next, we build a function to answer one question at a time. Its only job is to generate a concise answer based on the provided content.```python
...
Finally, we create a function to identify the sources for a given question-answer pair. This function's prompt asks the model to act as a fact-checker, linking the generated answer back to the original documents.```python
...
With our three modular components ready, we can now chain them together in a single pipeline. We'll iterate through each generated question, answer it, find its sources, and then assemble the final FAQ object.```python
...
And here's one of the resulting FAQ objects:```json
{
...
Now, we can build our main parallel workflow. It starts by synchronously generating the list of questions. Then, it creates a list of asynchronous tasks—one `process_question_parallel` call for each question—and runs them all concurrently using `asyncio.gather()`.```python
...
To summarize the trade-offs, sequential processing offers a predictable execution order and is generally easier to debug, as each step completes before the next begins. However, this comes at the cost of higher total processing time, especially for tasks with many independent sub-operations. In contrast, parallel processing significantly reduces overall execution time and makes better use of computational resources by running multiple tasks simultaneously. The trade-off is increased complexity in managing concurrent operations and handling potential errors. You also need to carefully manage API rate limits.

⚠️ **A Note on Rate Limits:** This speed comes with new considerations and trade-offs. While parallel processing offers significant performance gains, it introduces complexity in error handling and resource management. You must also be mindful of the rate limits imposed by your Large Language Model (LLM) provider, such as requests per minute (RPM) or tokens per minute (TPM) [[11]](https://platform.openai.com/docs/guides/rate-limits), [[12]](https://cloud.google.com/vertex-ai/generative-ai/docs/quotas), [[13]](https://learn.microsoft.com/azure/ai-services/openai/quotas-limits), [[15]](https://docs.anthropic.com/en/docs/build-with-claude/rate-limits). Hitting these limits can lead to `429 Too Many Requests` errors [[14]](https://platform.openai.com/docs/guides/error-codes), which production systems must handle gracefully.

Best practices include implementing an exponential backoff strategy, which retries a failed request with an increasing delay. You should also add jitter, a small, random delay, to prevent all retries from happening at once [[11]](https://platform.openai.com/docs/guides/rate-limits). Your code should be prepared to catch these errors, monitor your quota usage, and manage a request queue to stay within your account's limits. You can potentially distribute the load across multiple deployments or models if available [[13]](https://learn.microsoft.com/azure/ai-services/openai/quotas-limits).
...
This approach keeps our components modular and specialized. When we optimize the prompt for handling billing inquiries, it will not accidentally degrade performance for technical questions [[35]](https://spring.io/blog/2025/01/21/spring-ai-agentic-patterns). Routing also allows us to direct inputs to different LLM models, leveraging their specific strengths for different tasks. For instance, a smaller, faster model might handle simple classifications, while a more powerful one tackles complex reasoning. We will explore model routing in more detail in future lessons.

Developers can develop, test, and maintain each workflow branch independently. This provides a structured way to manage complexity as your application's capabilities grow, forming a foundational pattern for building more sophisticated, multi-turn conversational agents [[34]](https://docs.aws.amazon.com/prescriptive-guidance/latest/agentic-ai-patterns/workflow-for-routing.html).
...
The workflow is illustrated in the diagram below:```mermaid
graph TD
...
First, we define the possible intents and create a classifier function. The prompt asks the Large Language Model (LLM) to categorize the user's query into one of our predefined intents, returning a structured output.```python
...
Next, we create a specific prompt for each intent. The technical support prompt asks for troubleshooting details, while the billing prompt asks for an account number. This specialization allows us to craft the perfect response for each scenario.```python
...
Finally, we create the main handler function. It takes the user query and the classified intent, and uses a simple `if/elif/else` block to select the correct prompt and generate the final response.```python
...
Let's test it with a few different queries:```python
# Define queries
...
The key difference from simple parallelization is its flexibility; subtasks are not predefined. The orchestrator determines them on the fly based on the specific input. This makes the pattern powerful for handling unpredictable, multi-faceted requests [[23]](https://fme.safe.com/guides/ai-agent-architecture/ai-agentic-workflows/).

Here is a diagram of the pattern:```mermaid
graph TD
...
The orchestrator's prompt instructs it to break down a query into a list of tasks, each with a type and the necessary parameters.```python
...
1.  The `BillingTask` class defines the structured output for a billing inquiry, including the invoice number, the user's specific concern, the action taken, and the estimated resolution time. The `prompt_billing_worker_extractor` is an LLM prompt designed to extract the user's concern from the original query.
...
2.  The `handle_billing_worker` function simulates the process of handling a billing inquiry. It uses the LLM to extract the user's concern and then generates a simulated investigation ID and resolution estimate.
...
3.  The `ReturnTask` class defines the structured output for a product return request, including the product name, reason for return, Return Merchandise Authorization (RMA) number, and shipping instructions. The `handle_return_worker` function simulates generating an RMA number and providing detailed shipping instructions.
...
4.  The `StatusTask` class defines the structured output for an order status update, including the order ID, current status, carrier, tracking number, and expected delivery. The `handle_status_worker` function simulates fetching order status details from a backend system and returns them in a structured format.
...
The synthesizer is another LLM call. Its prompt takes the structured outputs from all the workers and asks the model to combine them into a single, friendly, and well-formatted email to the customer.```python
prompt_synthesizer = """
...
Now, let's test the entire workflow with our complex query. The `process_user_query` function orchestrates the entire flow, from task decomposition to synthesizing the final response.```python
...
def process_user_query(user_query):
    ...
    # 2. Run workers based on the decomposed tasks
    worker_results = []
