"""Synchronous webpage extraction engine for netpull"""
...
from netpull.cookie_handler import handle_cookie_consent
...
def extract_webpage(
    ...
    """Extract webpage content synchronously
    ...
    try:
        with sync_playwright() as p:
            ...
            browser = browser_type.launch(headless=browser_config.headless)
            ...
            try:
                ...
                page.goto(url, timeout=browser_config.timeout)

                # Wait for network idle
                if extraction_config.wait_for_networkidle:
                    page.wait_for_load_state('networkidle', timeout=browser_config.timeout)

                # Handle cookie consent
                if extraction_config.handle_cookie_consent:
                    handle_cookie_consent(page, extraction_config.cookie_consent_timeout)

                # Scroll to bottom for lazy-loaded content
                if extraction_config.scroll_to_bottom:
                    page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
                    page.wait_for_timeout(extraction_config.wait_for_timeout)

                # Wait for specific selector if provided
                if extraction_config.wait_for_selector:
                    page.wait_for_selector(extraction_config.wait_for_selector, timeout=browser_config.timeout)
...
def extract_batch(
    ...
    """Extract multiple webpages with concurrent processing

    Uses ThreadPoolExecutor for parallel extraction. Each thread
    gets its own browser instance for thread safety.
    ...
    with ThreadPoolExecutor(max_workers=extraction_config.batch_concurrency) as executor:
        # Submit all tasks
        future_to_url = {
            executor.submit(extract_webpage, url, extraction_config, browser_config): url
            for url in urls
        }
        ...
        for future in as_completed(future_to_url):
            ...
            try:
                ...
                if extraction_config.batch_delay > 0 and completed < total:
                    time.sleep(extraction_config.batch_delay)

            except Exception as e:
                logger.error(f"Failed to process {url}: {e}")
                results.append(ExtractionResult(url=url, success=False, error=str(e)))
