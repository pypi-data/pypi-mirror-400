...
class NotionPage:
    ...
    def from_api_response(cls, data: dict) -> "NotionPage":
        ...
        # Extract clotion ID
        clotion_id = None
        clotion_id_prop = properties.get("Clotion ID", {})
        if clotion_id_prop.get("type") == "rich_text":
            rich_text = clotion_id_prop.get("rich_text", [])
            if rich_text:
                clotion_id = rich_text[0].get("plain_text")
...
class NotionClient:
    ...
    def __init__(self, api_key: str, database_id: str):
        ...
        Args:
            ...
            database_id: ID of the Notion database to work with
        """
    ...
    async def get_database(self) -> dict:
        ...
        Returns:
            Database metadata
        """
        return await self._request("GET", f"/databases/{self.database_id}")
    ...
    async def query_pages(
        ...
        for result in data.get("results", []):
            try:
                pages.append(NotionPage.from_api_response(result))
    ...
    async def update_page_properties(self, page_id: str, properties: dict) -> NotionPage:
        ...
        Returns:
            Updated NotionPage
        """
        data = await self._request("PATCH", f"/pages/{page_id}", json={"properties": properties})
        return NotionPage.from_api_response(data)
    ...
    async def set_current_status(self, page_id: str, text: Optional[str]) -> NotionPage:
    ...
    async def set_clotion_id(self, page_id: str, clotion_id: str) -> NotionPage:
        """Set the Clotion ID property.

        Args:
            page_id: Page ID
            clotion_id: Clotion task ID (e.g., "CLO-001")
        ...
        return await self.update_page_properties(
            page_id,
            {"Clotion ID": {"rich_text": [{"type": "text", "text": {"content": clotion_id}}]}},
        )
    ...
    async def get_comments(self, page_id: str) -> list[NotionComment]:
        ...
        Returns:
            List of comments
        """
        # Note: Notion's comment API has some limitations
        # We need to use the block_id parameter to get comments
