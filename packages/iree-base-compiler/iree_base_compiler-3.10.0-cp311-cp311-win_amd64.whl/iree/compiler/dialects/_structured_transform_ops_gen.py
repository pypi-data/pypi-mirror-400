...
class FuseOp(_ods_ir.OpView):
  ...
  def __init__(self, transformed, loops, target, tile_sizes, tile_interchange, *, static_tile_sizes=None, static_tile_interchange=None, apply_cleanup=None, use_forall=None, loc=None, ip=None):
    operands = []
    attributes = {}
    regions = None
    operands.append(target)
    operands.append(_get_op_results_or_values(tile_sizes))
    operands.append(_get_op_results_or_values(tile_interchange))
    _ods_context = _ods_get_default_loc_context(loc)
    ...
    if static_tile_interchange is not None: attributes["static_tile_interchange"] = (static_tile_interchange if (
        isinstance(static_tile_interchange, _ods_ir.Attribute) or
        not _ods_ir.AttrBuilder.contains('DenseI64ArrayAttr')) else
          _ods_ir.AttrBuilder.get('DenseI64ArrayAttr')(static_tile_interchange, context=_ods_context))
    if bool(apply_cleanup): attributes["apply_cleanup"] = _ods_ir.UnitAttr.get(
      _ods_get_default_loc_context(loc))
    if bool(use_forall): attributes["use_forall"] = _ods_ir.UnitAttr.get(
      _ods_get_default_loc_context(loc))
    results = []
    results.append(transformed)
  ...
  @builtins.property
  def tile_interchange(self) -> _ods_ir.OpOperandList:
    operand_range = _ods_segmented_accessor(
         self.operation.operands,
  ...
  @builtins.property
  def static_tile_interchange(self) -> _Optional[_ods_ir.DenseI64ArrayAttr]:
    if "static_tile_interchange" not in self.operation.attributes:
      return None
    return self.operation.attributes["static_tile_interchange"]

  @static_tile_interchange.setter
  def static_tile_interchange(self, value: _Optional[_ods_ir.DenseI64ArrayAttr]):
    if value is not None:
      self.operation.attributes["static_tile_interchange"] = value
    elif "static_tile_interchange" in self.operation.attributes:
      del self.operation.attributes["static_tile_interchange"]

  @static_tile_interchange.deleter
  def static_tile_interchange(self):
    del self.operation.attributes["static_tile_interchange"]

  @builtins.property
  def apply_cleanup(self) -> bool:
    return "apply_cleanup" in self.operation.attributes

  @apply_cleanup.setter
  def apply_cleanup(self, value):
    if bool(value):
      self.operation.attributes["apply_cleanup"] = _ods_ir.UnitAttr.get()
    elif "apply_cleanup" in self.operation.attributes:
      del self.operation.attributes["apply_cleanup"]

  @apply_cleanup.deleter
  def apply_cleanup(self):
    del self.operation.attributes["apply_cleanup"]

  @builtins.property
  def use_forall(self) -> bool:
    return "use_forall" in self.operation.attributes

  @use_forall.setter
  def use_forall(self, value):
    if bool(value):
      self.operation.attributes["use_forall"] = _ods_ir.UnitAttr.get()
    elif "use_forall" in self.operation.attributes:
      del self.operation.attributes["use_forall"]
...
def structured_fuse(transformed, loops, target, tile_sizes, tile_interchange, *, static_tile_sizes=None, static_tile_interchange=None, apply_cleanup=None, use_forall=None, loc=None, ip=None) -> _Union[_ods_ir.OpResult, _ods_ir.OpResultList, FuseOp]:
  op = FuseOp(transformed=transformed, loops=loops, target=target, tile_sizes=tile_sizes, tile_interchange=tile_interchange, static_tile_sizes=static_tile_sizes, static_tile_interchange=static_tile_interchange, apply_cleanup=apply_cleanup, use_forall=use_forall, loc=loc, ip=ip); results = op.results
  ...
  def __init__(self, padded, pad, copy, target, pad_to_multiple_of, *, padding_values=None, padding_dimensions=None, static_pad_to_multiple_of=None, nofold_flags=None, transpose_paddings=None, copy_back_op=None, use_prescribed_tensor_shapes=None, loc=None, ip=None):
    ...
    if transpose_paddings is not None: attributes["transpose_paddings"] = (transpose_paddings if (
        ...
        not _ods_ir.AttrBuilder.contains('anonymous_608')) else
          _ods_ir.AttrBuilder.get('anonymous_608')(transpose_paddings, context=_ods_context))
