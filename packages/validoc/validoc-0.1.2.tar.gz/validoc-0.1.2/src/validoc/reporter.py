...
# SPDX-FileCopyrightText: 2025-2026 Abilian SAS <https://abilian.com>
...
class ConsoleReporter:
    ...
    def block_result(self, result: BlockResult) -> None:
        ...
        # Show output: always on failure, or in verbose mode on success
    ...
    def _print_output(self, result: BlockResult) -> None:
        """Print stdout/stderr for a failed block."""
        has_output = result.stdout.strip() or result.stderr.strip()
        if not has_output:
            return

        print(self._c(self.DIM, "       Output:"))
        if result.stdout.strip():
            for line in result.stdout.strip().split("\n")[:10]:
                print(self._c(self.DIM, f"         {line}"))
        if result.stderr.strip():
            print(self._c(self.RED, "       Stderr:"))
            for line in result.stderr.strip().split("\n")[:10]:
                print(self._c(self.RED, f"         {line}"))
