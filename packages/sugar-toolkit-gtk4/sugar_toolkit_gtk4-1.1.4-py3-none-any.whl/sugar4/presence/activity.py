...
CONN_INTERFACE_ROOM_CONFIG = TelepathyGLib.IFACE_CHANNEL_INTERFACE_ROOM_CONFIG
...
class Activity(GObject.GObject):
    ...
    __gsignals__ = {
        ...
        "buddy-left": (GObject.SignalFlags.RUN_FIRST, None, ([GObject.TYPE_PYOBJECT])),
        "new-channel": (GObject.SignalFlags.RUN_FIRST, None, ([GObject.TYPE_PYOBJECT])),
        "joined": (
            GObject.SignalFlags.RUN_FIRST,
            None,
    ...
    def do_get_property(self, pspec):
        ...
        if self._get_properties_call is not None:
            _logger.debug(
                "%r: Blocking on GetProperties() because someone "
    ...
    def invite(self, buddy, message, response_cb):
        ...
        if not self._joined:
            raise RuntimeError(
                "Cannot invite a buddy to an activity that is" "not shared."
            )
    ...
    def _start_tracking_buddies(self):
        ...
        group.GetAllMembers(
            reply_handler=self.__get_all_members_cb,
            error_handler=self.__error_handler_cb,
        )

        group.connect_to_signal(
            "MembersChanged", self.__text_channel_members_changed_cb
        )
    ...
    def __get_all_members_cb(self, members, local_pending, remote_pending):
        _logger.debug(
            "__get_all_members_cb %r %r" % (members, self._text_channel_group_flags)
        )
    ...
    def _resolve_handles(self, input_handles, reply_cb):
        ...
        if self._text_channel_group_flags & CHANNEL_GROUP_FLAG_CHANNEL_SPECIFIC_HANDLES:

            group = self.telepathy_text_chan[CHANNEL_INTERFACE_GROUP]
            group.GetHandleOwners(
                input_handles,
    ...
    def __text_channel_members_changed_cb(
        self, message, added, removed, local_pending, remote_pending, actor, reason
    ):
        _logger.debug(
            "__text_channel_members_changed_cb %r"
            % [
                added,
                message,
                added,
    ...
    def join(self):
        ...
        self._join_command = _JoinCommand(self.telepathy_conn, self.room_handle)
    ...
    def share(self, share_activity_cb, share_activity_error_cb):
        ...
        self._share_command.connect(
            "finished",
            partial(self.__shared_cb, share_activity_cb, share_activity_error_cb),
        )
        self._share_command.run()

    def __shared_cb(
        self, share_activity_cb, share_activity_error_cb, share_command, error
    ):
    ...
    def get_channels(self):
        ...
        channels = [
            self.telepathy_text_chan.object_path,
            self.telepathy_tubes_chan.object_path,
        ]

        _logger.debug(
            "%r: bus name is %s, connection is %s, channels are %r"
...
class _JoinCommand(_BaseCommand):
    ...
    def __create_text_channel_cb(self, channel_path):
        ...
        self.text_channel[CHANNEL_TYPE_TEXT] = dbus.Interface(
            self.text_proxy, CHANNEL_TYPE_TEXT
        )
    ...
    def __text_get_interfaces_reply_cb(self, interfaces):
        for interface in interfaces:
            self.text_channel[interface] = dbus.Interface(self.text_proxy, interface)
    ...
    def __create_tubes_channel_cb(self, channel_path):
        ...
        self.tubes_channel[CHANNEL_TYPE_TUBES] = dbus.Interface(
            self.tubes_proxy, CHANNEL_TYPE_TUBES
        )
    ...
    def __tubes_get_interfaces_reply_cb(self, interfaces):
        for interface in interfaces:
            self.tubes_channel[interface] = dbus.Interface(self.tubes_proxy, interface)
        _logger.debug("%r: Tubes channel is ready" % (self))
        self._tubes_ready = True
        self._ready()

    def __tubes_error_handler_cb(self, error):
        if error.get_dbus_name() == "org.freedesktop.Telepathy.Error.NotImplemented":
    ...
    def _ready(self):
        if not self._text_ready or (self._tubes_supported and not self._tubes_ready):
            return
    ...
    def _add_self_to_channel(self):
        ...
        def got_all_members(members, local_pending, remote_pending):
            _logger.debug(
                "got_all_members members %r local_pending %r "
                "remote_pending %r" % (members, local_pending, remote_pending)
            )
            ...
            if members:
                self.__text_channel_members_changed_cb("", members, (), (), (), 0, 0)
        ...
        def got_group_flags(flags):
            ...
            group.GetAllMembers(
                reply_handler=got_all_members, error_handler=self.__error_handler_cb
            )

        def got_self_handle(channel_self_handle):
            self.channel_self_handle = channel_self_handle
            group.connect_to_signal(
                "GroupFlagsChanged", self.__text_channel_group_flags_changed_cb
            )
            group.GetGroupFlags(
                reply_handler=got_group_flags, error_handler=self.__error_handler_cb
            )

        group.GetSelfHandle(
            reply_handler=got_self_handle, error_handler=self.__error_handler_cb
        )
    ...
    def __text_channel_members_changed_cb(
        ...
        _logger.debug(
            "__text_channel_members_changed_cb added %r removed %r "
            "local_pending %r remote_pending %r channel_self_handle "
            "%r"
            % (added, removed, local_pending, remote_pending, self.channel_self_handle)
        )

        if self.text_channel_group_flags & CHANNEL_GROUP_FLAG_CHANNEL_SPECIFIC_HANDLES:
            self_handle = self.channel_self_handle
        else:
            self_handle = self._global_self_handle
    ...
    def __room_cfg_error_cb(self, error):
        ...
        if error.get_dbus_name() != "org.freedesktop.Telepathy.Error.PermissionDenied":
