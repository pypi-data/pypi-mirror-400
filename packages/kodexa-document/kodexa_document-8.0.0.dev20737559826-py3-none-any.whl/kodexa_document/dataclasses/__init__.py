...
import jinja2
from kodexa_document import ContentNode
from kodexa_document.model import Tag, Document
from kodexa_document.model.objects import ContentException, Taxon, Taxonomy, Assistant
from pydantic import BaseModel, ConfigDict

from kodexa_document.utils import snake_to_camel, to_snake, taxon_to_property_name, taxon_to_class_name, taxon_to_group_path
    ...
    def process_exceptions(self, document: "KodexaDocumentLLMWrapper"):
        ...
        if self.exceptions is not None:
            for exception in self.exceptions:
                ...
                # document
                from kodexa_document.model import ContentException as KodexaContentException
...
class LLMDataObject(BaseModel):
    ...
    def apply_labels(
        ...
        for field in self.model_fields:
            ...
            if isinstance(value, list):
                logger.debug(f"Processing as a list with {len(value)} items")
                for item in value:
                    self.process_child(item, document, parent_group_uuid, assistant)
            else:
                logger.debug(f"Processing as a single value")
    ...
    def process_child(self, value, document, parent_group_uuid, assistant):
        ...
        if isinstance(value, LLMDataAttribute):
            ...
            logger.debug(f"Value: {value.normalized_text}, node_uuid_list: {value.node_uuid_list}")
            if value.node_uuid_list is None:
                value.node_uuid_list = value.line_ids
            logger.debug(f"Applying label {tag} to node UUIDs {value.node_uuid_list}")
            ...
            nodes_to_label: list[ContentNode] = (
                [
                    document.doc.get_node_by_uuid(str(node_uuid))
            ...
            for node in nodes_to_label:
                if node:
                    ...
                    # Get confidence directly from node (already normalized 0-1)
                    if node.has_confidence():
                        confidence = node.get_confidence()
                    
                    logger.debug(f"Confidence: {confidence}")
                    
                    # Always use node.tag() - each call adds a new tag row (INSERT)
                    # This is efficient even for nodes that already have the tag
                    try:
                        node.tag(
                            tag_to_apply=tag,
                        ...
                        logger.error(f"Error tagging node {node.uuid} with tag {tag}: {e}")
            ...
            logger.debug(f"Applied label {tag} to {len(nodes_to_label)} nodes")
        ...
        if isinstance(value, LLMDataObject):
            ...
            logger.debug(f"Applied labels to data object {value.group_uuid}")


def find_nearby_word_to_tag(node, tag):
    logger.debug(f"find_nearby_word_to_tag: {tag}")
