..
   This documentation page is generated during the Sphinx build.
   The underlying code is manually maintained and not autogenerated.

eegdash.dataset.DS004388
========================

Somatosensory evoked potentials in the human spinal cord to mixed nerve stimulation (OpenNeuro ``ds004388``). Access recordings and metadata through EEGDash.

.. rst-class:: sd-badges

:bdg-light:`Modality: eeg` :bdg-light:`Tasks: 3` :bdg-light:`License: CC0` :bdg-light:`Subjects: 40` :bdg-light:`Recordings: 399` :bdg-light:`Source: openneuro`

Dataset Information
-------------------

.. list-table::
   :widths: 25 75
   :header-rows: 0

   * - Dataset ID
     - ``DS004388``
   * - Title
     - Somatosensory evoked potentials in the human spinal cord to mixed nerve stimulation
   * - Year
     - Unknown
   * - Authors
     - Birgit Nierula, Tilman Stephani, Merve Kaptan, André Moruaux, Burkhard Maess, Gabriel Curio, Vadim V. Nikulin, Falk Eippert
   * - License
     - CC0
   * - Citation / DOI
     - `doi:10.18112/openneuro.ds004388.v1.0.0 <https://doi.org/10.18112/openneuro.ds004388.v1.0.0>`__
   * - Source links
     - `OpenNeuro <https://openneuro.org/datasets/ds004388>`__ | `NeMAR <https://nemar.org/dataexplorer/detail?dataset_id=ds004388>`__

.. dropdown:: Copy-paste BibTeX
   :class-container: sd-shadow-sm
   :class-title: sd-bg-light

   .. code-block:: bibtex

      @dataset{ds004388,
        title = {Somatosensory evoked potentials in the human spinal cord to mixed nerve stimulation},
        author = {Birgit Nierula and Tilman Stephani and Merve Kaptan and André Moruaux and Burkhard Maess and Gabriel Curio and Vadim V. Nikulin and Falk Eippert},
        doi = {10.18112/openneuro.ds004388.v1.0.0},
        url = {https://doi.org/10.18112/openneuro.ds004388.v1.0.0},
      }

Highlights
----------

.. grid:: 1 2 3 3
   :gutter: 2

   .. grid-item-card:: Subjects & recordings
      :class-card: sd-border-1

      - Subjects: 40
      - Recordings: 399
      - Tasks: 3

   .. grid-item-card:: Channels & sampling rate
      :class-card: sd-border-1

      - Channels: 67
      - Sampling rate (Hz): 10000
      - Duration (hours): 43.327

   .. grid-item-card:: Tasks & conditions
      :class-card: sd-border-1

      - Tasks: 3
      - Experiment type: Unknown
      - Subject type: Other

   .. grid-item-card:: Files & format
      :class-card: sd-border-1

      - Size on disk: 682.54 GB
      - File count: Unknown
      - Format: BIDS

   .. grid-item-card:: License & citation
      :class-card: sd-border-1

      - License: CC0
      - DOI: doi:10.18112/openneuro.ds004388.v1.0.0

   .. grid-item-card:: Provenance
      :class-card: sd-border-1

      - Source: openneuro
      - OpenNeuro: `ds004388 <https://openneuro.org/datasets/ds004388>`__
      - NeMAR: `ds004388 <https://nemar.org/dataexplorer/detail?dataset_id=ds004388>`__

Quickstart
----------

**Install**

.. code-block:: bash

   pip install eegdash

**Load a recording**

.. code-block:: python

   from eegdash.dataset import DS004388

   dataset = DS004388(cache_dir="./data")
   recording = dataset[0]
   raw = recording.load()

**Filter/query**

.. tab-set::

   .. tab-item:: Basic

      .. code-block:: python

         dataset = DS004388(cache_dir="./data", subject="01")

   .. tab-item:: Advanced

      .. code-block:: python

         dataset = DS004388(
             cache_dir="./data",
             query={"subject": {"$in": ["01", "02"]}},
         )


Quality & caveats
-----------------

- No dataset-specific caveats are listed in the available metadata.

API
---

.. currentmodule:: eegdash.dataset

.. autoclass:: eegdash.dataset.DS004388
   :members: __init__, save
   :show-inheritance:
   :member-order: bysource


See Also
--------

* :class:`eegdash.dataset.EEGDashDataset`
* :mod:`eegdash.dataset`
* `OpenNeuro dataset page <https://openneuro.org/datasets/ds004388>`__
* `NeMAR dataset page <https://nemar.org/dataexplorer/detail?dataset_id=ds004388>`__

