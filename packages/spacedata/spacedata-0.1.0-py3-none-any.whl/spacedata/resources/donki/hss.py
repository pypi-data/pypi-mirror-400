...
from spacedata.models.donki import DONKIDateRangeInput, DONKIHss
...
async def _get_response(
    ...
    response = await resource.client.make_request(
        method="GET",
        url=f"{resource.client.settings.main_base_url}/DONKI/HSS",
        params=params,
    )
    return [DONKIHss.model_validate(item) for item in response]
...
async def hss(
    ...
    **kwargs,
) -> SpaceDataResult[DONKIHss]:
    self.client.logger.debug("Calling hss endpoint with input_data: %s", input_data)
