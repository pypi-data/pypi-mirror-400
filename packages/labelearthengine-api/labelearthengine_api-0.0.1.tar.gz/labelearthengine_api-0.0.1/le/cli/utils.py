...
TASK_FINISHED_STATES: Tuple[str, str, str] = (
    le.batch.Task.State.COMPLETED,
    le.batch.Task.State.FAILED,
    le.batch.Task.State.CANCELLED,
)
...
class CommandLineConfig:
  ...
  def _get_credentials(self):
    ...
    if self.service_account_file:
      return le.ServiceAccountCredentials(self.client_email,
                                          self.service_account_file)
    elif self.account and self.private_key:
      return le.ServiceAccountCredentials(self.account, self.private_key)
    else:
      return 'persistent'
  ...
  def create_gcs_helper(self) -> 'GcsHelper':
    ...
    if creds == 'persistent':
      creds = le.data.get_persistent_credentials()
    return GcsHelper(
...
def wait_for_task(
  ...
  while True:
    ...
    status = le.data.getTaskStatus(task_id)[0]
    state = status['state']
    if state in TASK_FINISHED_STATES:
      ...
      if error_message:
        raise le.ee_exception.EEException('Error: %s' % error_message)
      return
...
def wait_for_tasks(
  ...
  status_list = le.data.getTaskStatus(task_id_list)
...
def expand_gcs_wildcards(source_files: List[str]) -> Iterable[str]:
  ...
  for source in source_files:
    ...
    if bucket_match:
      ...
      raise le.ee_exception.EEException(
...
def _gcs_ls(bucket: str, prefix: str = '') -> Iterable[str]:
  ...
  method = 'GET'
  http = le.data.authorizeHttp(httplib2.Http(0))
  ...
  while True:
    ...
    try:
      ...
      raise le.ee_exception.EEException('Unexpected HTTP error: %s' % str(e))

    if response.status < 100 or response.status >= 300:
      raise le.ee_exception.EEException(('Error retrieving bucket %s;'
    ...
    if 'error' in json_content:
      ...
      raise le.ee_exception.EEException('Error retrieving bucket %s: %s' %
                                        (bucket, json_error))
