"""
Tests for Django logging handler.
"""
...
from error_explorer_django.logging import ErrorExplorerHandler
...
class TestErrorExplorerHandler:
    ...
    def test_handler_captures_error_as_event(
        ...
        # Should capture as message event
    ...
    def test_handler_captures_critical_as_event(
        ...
        mock_transport.send.assert_called_once()
        event = mock_transport.send.call_args[0][0]
        # Note: Python SDK maps CRITICAL to "error" level for capture_message
        assert event["severity"] in ("critical", "error")
    ...
    def test_handler_includes_logger_name_in_category(
        ...
        assert "test_logger" in breadcrumbs[-1].category
    ...
    def test_handler_with_capture_breadcrumbs_disabled(
        ...
        logger = logging.getLogger("test_no_breadcrumbs")
        logger.addHandler(handler)
        logger.setLevel(logging.DEBUG)
    ...
    def test_handler_does_nothing_when_not_initialized(self, handler):
        ...
        logger = logging.getLogger("test_uninit")
        logger.addHandler(handler)
        logger.setLevel(logging.DEBUG)
