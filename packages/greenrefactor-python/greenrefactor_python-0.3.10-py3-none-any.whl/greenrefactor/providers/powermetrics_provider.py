...
def _run_powermetrics(args: list[str], timeout: float = 1.5) -> str:
...
class PowerMetricsProvider(BaseProvider):
    ...
    def read_watts(self) -> float:
        # 'smc' sampler is not reliable on all Macs (especially M1/M2/M3 where it might be 'cpu_power')
        # Empty args usually gives default samplers including CPU power
