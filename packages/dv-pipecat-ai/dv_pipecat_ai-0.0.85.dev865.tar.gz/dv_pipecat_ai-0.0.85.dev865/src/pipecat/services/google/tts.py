...
class GoogleTTSService(TTSService):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._voice = None
        self._voice_config = kwargs.get("voice_config", dict())
        ...
        if self._voice_config.get("is_clone", False):
            voice_cloning_key = self._voice_config.get("voice_cloning_key")
            if not voice_cloning_key:
                raise ValueError("voice_cloning_key is required for voice cloning google TTS")
            self._voice_clone_params = texttospeech_v1.VoiceCloneParams(
