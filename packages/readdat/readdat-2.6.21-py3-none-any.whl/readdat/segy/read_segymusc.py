...
def is_segymusc(
    ...
    """
    determines whether a stream read from segy file is from MUSC or not
    :return answer: True or False
    """

    encoding = stream.stats['textual_file_header_encoding']
    if encoding.lower() == "ascii":
        textual_header = stream.stats['textual_file_header'].decode('ascii')

        if "MUSC" not in textual_header:
            return False

    weathering_velocities = np.asarray([trace.stats.segy['trace_header']['weathering_velocity'] for trace in stream])
...
def _convert_time_units_in_segymusc_header(
    ...
    """
    if verbose:
        warnings.warn('convert SAMPLE_INTERVAL from nanoseconds (SEGY-MUSC) to seconds (OBSPY)')
...
def _extract_receiver_location_from_segymusc_header(trace):
    try:
        # ========
        ...
        trace.stats.receiver_x = \
            trace.stats.segy['trace_header']['group_coordinate_x'] * k / 1000.

        trace.stats.receiver_y = \
            trace.stats.segy['trace_header']['group_coordinate_y'] * k / 1000.
        ...
        trace.stats.receiver_z = \
            trace.stats.segy['trace_header']['receiver_group_elevation'] * k / 1000.

    except KeyError as err:
        warnings.warn(str(err))


def _extract_source_location_from_segymusc_header(trace):
    try:
        # ========
        scalar_to_be_applied_to_all_coordinates = \
            trace.stats.segy['trace_header']['scalar_to_be_applied_to_all_coordinates']
        ...
        trace.stats.source_x = \
            trace.stats.segy['trace_header']['source_coordinate_x'] * k / 1000.
...
def _extract_delta_source_laser_from_header(trace):
    ...
    assert trace.stats.segy['trace_header']['weathering_velocity'] == MUSC_CODE  # (means MUSC data)
    return float(trace.stats.segy['trace_header']['subweathering_velocity']) / DELTA_SOURCE_LASER_FACTOR  #


def segymusc_to_obspy(stream: obspy.core.stream.Stream, verbose: bool=True) -> obspy.core.stream.Stream:
    """
    ...
    if not is_segymusc(stream):
        raise Exception(
            'This data file does not look like '
            'a segy file from MUSC, try debugging with acquisition_system=None to use obspy standards'
            'is it?')
    ...
    for trace in stream:
        ...
        # ==== header conversions
        _convert_time_units_in_segymusc_header(trace=trace, verbose=verbose)

        # ==== extract data from variable header
        _extract_receiver_location_from_segymusc_header(trace=trace)

        _extract_source_location_from_segymusc_header(trace=trace)
