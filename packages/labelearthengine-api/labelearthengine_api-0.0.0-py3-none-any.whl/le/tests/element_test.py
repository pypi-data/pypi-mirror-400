...
class ElementTestCase(apitestcase.ApiTestCase):
  ...
  def testSet(self):
    ...
    def AssertProperties(expected, image):
      properties = {}
      while image.func == le.ApiFunction.lookup('Element.set'):
        key = image.args['key']
        if not isinstance(key, str):
          key = key.encode()
        properties[key] = image.args['value']
        image = image.args['object']
      self.assertEqual(le.Image(1), image)
    ...
    # Computed dictionary.
    computed_arg = le.ComputedObject(None, None, 'foo')

    def CheckMultiProperties(result):
      self.assertEqual(le.ApiFunction.lookup('Element.setMulti'), result.func)
      self.assertEqual({
          'object': image,
          'properties': le.Dictionary(computed_arg)
      }, result.args)
  ...
  def testInitOptParams(self):
    result = le.Element(func=None, args=None, opt_varName='test').serialize()
