...
class BibleQueryResponse:
    ...
    v_name: str | None = None  # Version name (Chinese)
...
@dataclass
class Commentary:
    """Represents a Bible commentary entry."""

    title: str  # Commentary title (e.g., "創世記 1章9節 到 1章13節")
    book_name: str  # Commentary source name (e.g., "信望愛站註釋")
    com_text: str  # Commentary text content
...
class CommentaryResponse:
    """Response from commentary query."""

    status: str  # "success" or "error"
    record_count: int  # Number of commentary entries
    records: list[Commentary]  # List of commentary records
    prev_verse: BibleReference | None = None  # Previous verse with commentary
    next_verse: BibleReference | None = None  # Next verse with commentary
    error_message: str | None = None  # Error message if status is "error"
    raw_response: dict[str, Any] | None = None  # Raw API response
    ...
    def __str__(self) -> str:
        ...
        commentaries = "\n\n".join(str(comm) for comm in self.records)
        return f"Commentaries ({self.record_count}):\n{commentaries}"
