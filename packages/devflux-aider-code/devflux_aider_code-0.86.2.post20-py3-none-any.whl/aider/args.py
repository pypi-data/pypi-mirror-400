...
def default_env_file(git_root):
    return os.path.join(git_root, ".env") if git_root else os.path.join(".env")
...
def get_parser(default_config_files, git_root):
    ...
    default_input_history_file = (
        os.path.join(git_root, ".aider", ".aider.input.history")
        if git_root
        else os.path.join(".aider", ".aider.input.history")
    )
    default_chat_history_file = (
        os.path.join(git_root, ".aider", ".aider.chat.history.md")
        if git_root
        else os.path.join(".aider", ".aider.chat.history.md")
    )
    group.add_argument(
        "--input-history-file",
    ...
    default_aiderignore_file = (
        os.path.join(git_root, ".aider", ".aiderignore")
        if git_root
        else os.path.join(Path.home(), ".aider", ".aiderignore")
    )
    ...
    group.add_argument(
        ...
        action="append",
        metavar="FILE",
        help="specify a read rule file (can be used multiple times)",
    ).complete = shtab.FILE
    group.add_argument(
        "--vim",
        action="store_true",
    ...
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Enable/disable detection and Model Context Protocol (default: False)",
    )
    group.add_argument(
        "--output-format",
        choices=["text", "markdown", "html"],
        default="text",
        help="How aider outputs to the console(default: aider)",
    )
    # </code>
