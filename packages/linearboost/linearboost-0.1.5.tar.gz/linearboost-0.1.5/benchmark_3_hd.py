"""
High-Dimensional Dataset Benchmark with Dual LinearBoost Variants
=================================================================

This benchmark compares models on HIGH-DIMENSIONAL datasets (.mat files):
...
- LinearBoost-K-exact: Non-linear kernels without approximation
- XGBoost
- LightGBM  
- CatBoost
- LogisticRegression
...
HD-datasets are gene expression and other high-dimensional datasets stored
in .mat format in the hd-datasets/ folder.
...
# Suppress sklearn matrix multiplication warnings (can occur with kernel computations)
warnings.filterwarnings("ignore", category=RuntimeWarning, module="sklearn.utils.extmath")
warnings.filterwarnings("ignore", message=".*invalid value encountered in matmul.*")
warnings.filterwarnings("ignore", message=".*divide by zero encountered in matmul.*")
warnings.filterwarnings("ignore", message=".*overflow encountered in matmul.*")
# Suppress multiprocessing semaphore leak warnings (harmless, resources are cleaned up at shutdown)
warnings.filterwarnings("ignore", message=".*resource_tracker.*", module="multiprocessing.resource_tracker")
...
import numpy as np
# Configure numpy to suppress warnings about invalid values in matrix operations
# These can occur with kernel computations and are handled internally
...
class ModelBenchmark:
    ...
    def __init__(self, X, y, categorical_cols, n_trials=200, cv_folds=10, 
                 ...
                 hp_sample_threshold=100000, hp_sample_size=100000,
        ...
        self.optimization_times = {}  # Track optimization time per algorithm
    ...
    def _create_hp_sample(self):
        ...
        # Use more aggressive sampling threshold to prevent memory issues
        # Kernel approximation methods are memory-intensive, so sample earlier
        effective_threshold = min(self.hp_sample_threshold, 10000)
        
        # Cap sample size to prevent memory issues during kernel approximation
        safe_sample_size = min(self.hp_sample_size, 10000)  # Max 20k for HP tuning
        
        if n_samples > effective_threshold:
            print(f"\n{'='*60}")
            print(f"DATASET SIZE: {n_samples} records (> {effective_threshold} threshold)")
            print(f"Using stratified sample of {safe_sample_size} records for HP tuning")
            if safe_sample_size < self.hp_sample_size:
                print(f"(Reduced from {self.hp_sample_size} to prevent memory issues)")
            print(f"{'='*60}")
            
            X_hp, _, y_hp, _ = train_test_split(
                self.X, self.y,
                train_size=safe_sample_size,
                stratify=self.y,
    ...
    def _cross_validate_with_pruning(self, model, X, y, cv, trial):
        """Perform cross-validation with intermediate reporting for Optuna pruning.
        
        Works with both Pipeline objects and regular sklearn models.
        """
        fold_scores = []
        for fold_idx, (train_idx, val_idx) in enumerate(cv.split(X, y)):
            ...
            fold_score = f1_score(y_val_fold, y_pred, average='weighted', zero_division=0.0)
            ...
            # Clean up memory after each fold to prevent accumulation
    ...
    def create_linearboost_preprocessor(self, n_jobs=None):
        ...
        # Always use n_jobs=1 for ColumnTransformer to avoid multiprocessing semaphore leaks
        # Preprocessing is fast and not the bottleneck, so single-threaded is fine
        return ColumnTransformer(
            ...
            n_jobs=1,  # Fixed to avoid semaphore leaks
        )
    def create_logreg_preprocessor(self, n_jobs=None):
        ...
        # Include all numeric types (int8, uint8, int16, uint16, etc. for HD datasets)
    ...
    def optimize_hyperparameters(self):
        ...
        self.optimization_times['LinearBoost-L'] = time.time() - start_time
        self.best_params['LinearBoost-L'] = lb_linear_study.best_params
        print(f"  Best F1: {lb_linear_study.best_value:.4f} (Time: {self.optimization_times['LinearBoost-L']:.2f}s)")
        ...
        self.optimization_times['LinearBoost-K'] = time.time() - start_time
        self.best_params['LinearBoost-K'] = lb_kernel_study.best_params
        print(f"  Best F1: {lb_kernel_study.best_value:.4f} (Time: {self.optimization_times['LinearBoost-K']:.2f}s)")
        ...
        self.optimization_times['LinearBoost-K-exact'] = time.time() - start_time
        self.best_params['LinearBoost-K-exact'] = lb_kernel_exact_study.best_params
        print(f"  Best F1: {lb_kernel_exact_study.best_value:.4f} (Time: {self.optimization_times['LinearBoost-K-exact']:.2f}s)")
        ...
        self.optimization_times['XGBoost'] = time.time() - start_time
        self.best_params['XGBoost'] = xgboost_study.best_params
        print(f"  Best F1: {xgboost_study.best_value:.4f} (Time: {self.optimization_times['XGBoost']:.2f}s)")
        
        # LightGBM
        print("\n→ Optimizing LightGBM...")
        start_time = time.time()
        lightgbm_study = self._optimize_lightgbm(cv_opt)
        self.optimization_times['LightGBM'] = time.time() - start_time
        self.best_params['LightGBM'] = lightgbm_study.best_params
        print(f"  Best F1: {lightgbm_study.best_value:.4f} (Time: {self.optimization_times['LightGBM']:.2f}s)")
        
        # CatBoost
        print("\n→ Optimizing CatBoost...")
        start_time = time.time()
        catboost_study = self._optimize_catboost(cv_opt)
        self.optimization_times['CatBoost'] = time.time() - start_time
        self.best_params['CatBoost'] = catboost_study.best_params
        print(f"  Best F1: {catboost_study.best_value:.4f} (Time: {self.optimization_times['CatBoost']:.2f}s)")
        
        # Logistic Regression
        print("\n→ Optimizing LogisticRegression...")
        start_time = time.time()
        logreg_study = self._optimize_logistic_regression(cv_opt)
        self.optimization_times['LogisticRegression'] = time.time() - start_time
        self.best_params['LogisticRegression'] = logreg_study.best_params
        print(f"  Best F1: {logreg_study.best_value:.4f} (Time: {self.optimization_times['LogisticRegression']:.2f}s)")

        # Random Forest
        print("\n→ Optimizing RandomForest...")
        start_time = time.time()
        rf_study = self._optimize_random_forest(cv_opt)
        self.optimization_times['RandomForest'] = time.time() - start_time
        self.best_params['RandomForest'] = rf_study.best_params
        print(f"  Best F1: {rf_study.best_value:.4f} (Time: {self.optimization_times['RandomForest']:.2f}s)")

        # TabPFN (no tuning needed)
        if self.include_tabpfn:
            print("\n→ TabPFN: No hyperparameter tuning required")
            self.best_params['TabPFN'] = {}
            self.optimization_times['TabPFN'] = 0.0
        
        # Print optimization time summary
        print("\n" + "="*60)
        print("OPTIMIZATION TIME SUMMARY")
        print("="*60)
        linearboost_variants = ['LinearBoost-L', 'LinearBoost-K', 'LinearBoost-K-exact']
        other_algorithms = ['XGBoost', 'LightGBM', 'CatBoost', 'LogisticRegression', 'RandomForest']
        
        linearboost_total = sum(self.optimization_times[v] for v in linearboost_variants)
        other_total = sum(self.optimization_times.get(a, 0) for a in other_algorithms)
        
        print(f"\nLinearBoost Variants (3 total):")
        for variant in linearboost_variants:
            print(f"  {variant:<25}: {self.optimization_times[variant]:>8.2f}s")
        print(f"  {'TOTAL':<25}: {linearboost_total:>8.2f}s")
        
        print(f"\nOther Algorithms:")
        for algo in other_algorithms:
            if algo in self.optimization_times:
                print(f"  {algo:<25}: {self.optimization_times[algo]:>8.2f}s")
        print(f"  {'TOTAL':<25}: {other_total:>8.2f}s")
        
        print(f"\n{'='*60}")
        print(f"LinearBoost (all 3 variants): {linearboost_total:.2f}s")
        print(f"Other algorithms combined:    {other_total:.2f}s")
        if linearboost_total < other_total:
            speedup = other_total / linearboost_total
            print(f"✓ LinearBoost is {speedup:.2f}x FASTER than optimizing all other algorithms!")
        else:
            slowdown = linearboost_total / other_total
            print(f"⚠ LinearBoost takes {slowdown:.2f}x longer than other algorithms")
        print("="*60)
    ...
    def _optimize_linearboost_linear(self, cv):
        ...
        def objective(trial):
            ...
            try:
                result = self._cross_validate_with_pruning(pipe, self.X_hp, self.y_hp, cv, trial)
                # Clean up after trial to free memory
                del pipe, clf, preprocessor
                gc.collect()
                return result
            except optuna.TrialPruned:
                # Clean up even if pruned
                del pipe, clf, preprocessor
                gc.collect()
                raise
            except Exception as e:
    ...
    def _optimize_linearboost_kernel(self, cv):
        ...
        def objective(trial):
            ...
            params = {
                ...
                # Reduce n_components during HP tuning for memory efficiency with large datasets
                # Full evaluation will use best params which may have higher n_components
                "n_components": trial.suggest_categorical("n_components", [64, 128, 256]),  # Reduced max from 512 to 256
                "early_stopping": trial.suggest_categorical("early_stopping", [True, False]),
            }
    ...
    def _optimize_linearboost_kernel_exact(self, cv):
        ...
        def objective(trial):
            ...
            try:
                return self._cross_validate_with_pruning(pipe, self.X_hp, self.y_hp, cv, trial)
            except optuna.TrialPruned:
                raise
            except Exception:
                mean_score = -np.inf
    ...
    def _optimize_linearboost_auto(self, cv):
        """
        Optimize LinearBoost by searching across BOTH Linear and Non-Linear kernels.
        This makes 'LinearBoost' a single adaptive algorithm.
        """
        def objective(trial):
            # 1. First, select the kernel type as a top-level hyperparameter
            kernel_type = trial.suggest_categorical("kernel_type", ["linear", "non_linear"])

            params = {
                "n_estimators": trial.suggest_int("n_estimators", 10, 500),
                "learning_rate": trial.suggest_float("learning_rate", 0.01, 1.0, log=True),
                "algorithm": trial.suggest_categorical("algorithm", ["SAMME", "SAMME.R"]),
                "scaler": trial.suggest_categorical("scaler", ["minmax", "robust", "quantile-uniform", "standard"]),
                "early_stopping": trial.suggest_categorical("early_stopping", [True, False]),
                "subsample": trial.suggest_float("subsample", 0.5, 1.0)
            }
            
            # Common early stopping params
            if params["early_stopping"]:
                if params["subsample"] >= 1.0:
                    params["validation_fraction"] = trial.suggest_float("validation_fraction", 0.05, 0.2)
                params["n_iter_no_change"] = trial.suggest_int("n_iter_no_change", 3, 10)
            ...
            if kernel_type == "linear":
                ...
                # Select specific non-linear kernel
                params["kernel"] = trial.suggest_categorical("kernel_subtype", ["rbf", "poly", "sigmoid"])
                params["gamma"] = trial.suggest_float("gamma", 1e-3, 10.0, log=True)
                
                # Decide between Approximation vs Exact (Trade-off: Speed vs Accuracy)
                # This allows the model to choose "Exact" for small data and "Nystrom" for large data automatically
                use_approx = trial.suggest_categorical("use_approx", [True, False])
                
                if use_approx:
                    if params["kernel"] == "rbf":
                        params["kernel_approx"] = trial.suggest_categorical("approx_type", ["rff", "nystrom"])
                    else:
                        params["kernel_approx"] = "nystrom"
                    params["n_components"] = trial.suggest_categorical("n_components", [64, 128, 256, 512])
                else:
                    params["kernel_approx"] = None

                # Kernel specific degree/coef0
    ...
    def _optimize_xgboost(self, cv):
        ...
        def objective(trial):
            ...
            # XGBoost doesn't use Pipeline, so we need to adapt for pruning
            try:
                result = self._cross_validate_with_pruning(model, self.X_hp, self.y_hp, cv, trial)
                del model
                gc.collect()
                return result
            except optuna.TrialPruned:
...
# =============================================================================
# HD-DATASETS LOADING (.mat files)
# =============================================================================

def load_hd_dataset(mat_file_path):
    """
    Load a dataset from .mat file in hd-datasets folder.
    ...
    dataset_name : str
        Name of the dataset (derived from filename)
    """
    import scipy.io
    ...
    dataset_name = Path(mat_file_path).stem
...
if __name__ == "__main__":
    try:
        ...
        print("ERROR: scipy package required for HD-datasets benchmarking.")
        print("Install with: pip install scipy")
        raise
    
    # HD-datasets folder path
    hd_datasets_dir = Path("hd-datasets")
    
    if not hd_datasets_dir.exists():
        print(f"ERROR: HD-datasets folder not found at {hd_datasets_dir}")
        print("Please ensure the hd-datasets folder exists with .mat files.")
        raise FileNotFoundError(f"HD-datasets directory not found: {hd_datasets_dir}")
    
    # Find all .mat files in hd-datasets folder
    mat_files = sorted(list(hd_datasets_dir.glob("*.mat")))
    
    if not mat_files:
        print(f"ERROR: No .mat files found in {hd_datasets_dir}")
        raise FileNotFoundError(f"No .mat files found in: {hd_datasets_dir}")
    
    print("\n" + "="*70)
    print("  HD-DATASETS BENCHMARK SUITE")
    print(f"  Running benchmarks for {len(mat_files)} datasets")
    print("="*70)
    print(f"\nFound datasets:")
    ...
    for mat_file in mat_files:
        try:
            ...
            # Load dataset
            X, y, categorical_cols, imbalance_ratio, dataset_name = load_hd_dataset(mat_file)
            ...
            benchmark = ModelBenchmark(
                ...
                n_jobs=10,
                hp_sample_threshold=2000,  # Lower threshold for HD datasets
                hp_sample_size=1500,       # Smaller sample for HP tuning due to high dimensionality
                include_tabpfn=False       # TabPFN not suitable for HD datasets
            )
            ...
            for model_name in results:
                ...
                results[model_name]['dataset_id'] = mat_file.stem  # Use filename without extension as ID
            ...
            benchmark.save_results(results, stat_results, 
                                 filename_prefix=f"hd_{mat_file.stem}_{dataset_name}")
            ...
            aggregate_file = results_dir / "hd_aggregate_summary_latest.csv"
            ...
            print(f"\n  ✗ Error processing dataset {mat_file.name}: {e}")
            import traceback
            traceback.print_exc()
            continue
    ...
    if all_results:
        ...
        results_dir.mkdir(exist_ok=True)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        summary_file = results_dir / f"hd_aggregate_summary_{timestamp}.csv"
