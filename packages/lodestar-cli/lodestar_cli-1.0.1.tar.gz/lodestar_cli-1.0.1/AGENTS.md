...
## Task Planning (MCP Tools Available)

**Task creation is available via both MCP tools and CLI.** MCP tools enable planning agents to create and manage tasks programmatically.

### MCP Task Planning Workflow

```
1. DESIGN    (analyze requirements, apply INVEST principles)
2. CREATE    lodestar_task_create()      -> Create well-structured task
3. UPDATE    lodestar_task_update()      -> Refine as needed
4. DELETE    lodestar_task_delete()      -> Remove if invalid
```

### Task Creation Best Practices

- Apply INVEST principles (see below) for 15-minute completable tasks
- Use structured WHAT/WHERE/WHY/SCOPE/ACCEPT/REFS description format
- Reference PRD sections when applicable
- Set appropriate locks, dependencies, and labels
- Validate task is well-scoped before creating
...
| **Task Planning** | `lodestar_task_create` | Create new task with INVEST validation |
| | `lodestar_task_update` | Update task fields (title, description, etc.) |
| | `lodestar_task_delete` | Delete task (supports --cascade) |
...
Most functionality is available via MCP tools. CLI useful for:
