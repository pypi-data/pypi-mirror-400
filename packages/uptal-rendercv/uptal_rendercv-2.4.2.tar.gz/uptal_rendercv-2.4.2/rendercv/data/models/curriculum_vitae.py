    ...
    def url(self) -> str:
        ...
        if self.network == "Mastodon":
            ...
            if self.username.count("@") >= 2:
                _, username, domain = self.username.split("@", 2)
            ...
            base_url = url_dictionary.get(self.network)
            if base_url:
    ...
    def connections(self) -> list[dict[str, Optional[str]]]:
        ...
        if self.social_networks is not None:
            ...
            for social_network in self.social_networks:
                ...
                connection = {
                    "typst_icon": icon_dictionary.get(social_network.network, "link"),
                ...
                if social_network.network == "StackOverflow":
                    if "/" in social_network.username:
                        username = social_network.username.split("/", 1)[1]
