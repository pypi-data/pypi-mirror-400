...
- **Reproducible Script**: `benchmarks/benchmark_soma.py` with full methodology
...
*Note: Exact values vary by system. Run `benchmarks/benchmark_soma.py` to get results for your hardware.*
...
python benchmarks/benchmark_soma.py --iterations 100 --warmup 10 --corpus-size large --output my_results.csv
```
...
**Note**: While SOMA provides universal coverage at the character/byte level, higher-level algorithms (word, grammar, syllable) are optimized for specific language characteristics. For best results with non-Latin scripts, use character or byte tokenization, or ensure appropriate language detection is enabled.
