...
class TestLimits:
    ...
    def setup_class(self) -> None:
        ...
        create_star_data(self.test_dir + "self.limits", npix=21, pos_star=10.0)
    ...
    def test_read_data(self) -> None:
        ...
        module = FitsReadingModule(
            name_in="read", image_tag="read", input_dir=self.test_dir + "self.limits"
        )
        ...
        assert np.sum(data) == pytest.approx(
            108.43655133957289, rel=self.limit, abs=0.0
        )
    ...
    def test_angle_interpolation(self) -> None:
        ...
        assert np.sum(attr) == pytest.approx(900.0, rel=self.limit, abs=0.0)
    ...
    def test_contrast_curve(self) -> None:
        ...
        for item in proc:
            ...
            if item == "multi":
                with h5py.File(
                    self.test_dir + "PynPoint_database.hdf5", "a"
                ) as hdf_file:
                    hdf_file["config"].attrs["CPU"] = 4

            module = ContrastCurveModule(
                name_in="contrast_" + item,
                image_in_tag="read",
                psf_in_tag="read",
                contrast_out_tag="limits_" + item,
                separation=(0.2, 0.3, 0.2),
                angle=(0.0, 360.0, 180.0),
                threshold=("sigma", 5.0),
                psf_scaling=1.0,
                aperture=0.05,
                pca_number=2,
            ...
            assert data[0, 0] == pytest.approx(0.2, rel=self.limit, abs=0.0)
            assert data[0, 1] == pytest.approx(
                2.580878183791224, rel=self.limit, abs=0.0
            )
            assert data[0, 2] == pytest.approx(
                0.0007097688120261913, rel=self.limit, abs=0.0
            )
            assert data[0, 3] == pytest.approx(
                0.00020126490906225968, rel=self.limit, abs=0.0
            )
    ...
    def test_contrast_curve_fpf(self) -> None:
        ...
        assert data[0, 0] == pytest.approx(0.2, rel=self.limit, abs=0.0)
        assert data[0, 1] == pytest.approx(1.9339430843041776, rel=self.limit, abs=0.0)
        assert data[0, 2] == pytest.approx(
            0.000709768812026221, rel=self.limit, abs=0.0
        )
        assert data[0, 3] == pytest.approx(1e-06, rel=self.limit, abs=0.0)
        assert data.shape == (1, 4)
    ...
    def test_mass_limits(self) -> None:
        ...
        url = (
            "https://home.strw.leidenuniv.nl/~stolker/pynpoint/"
            "model.AMES-Cond-2000.M-0.0.NaCo.Vega"
        )

        filename = pooch.retrieve(
            url=url,
            fname="model.AMES-Cond-2000.M-0.0.NaCo.Vega",
            path=self.test_dir,
            known_hash=None,
        )

        module = MassLimitsModule(
            model_file=filename,
            star_prop={"magnitude": 10.0, "distance": 100.0, "age": 20.0},
            name_in="mass",
        ...
        assert np.mean(data[:, 0]) == pytest.approx(0.55, rel=self.limit, abs=0.0)
        assert np.mean(data[:, 1]) == pytest.approx(
            0.001891690765603738, rel=self.limit, abs=0.0
        )
        assert np.mean(data[:, 2]) == pytest.approx(
            0.000964309686441908, rel=self.limit, abs=0.0
        )
        assert np.mean(data[:, 3]) == pytest.approx(
            -0.000696402843279597, rel=self.limit, abs=0.0
        )
