...
class TestOpenAPISchemaGeneration:
    ...
    def test_attribute_comparison_proxy_callable_directly(self):
        """Test AttributeComparisonProxy callable behavior directly."""
        from vital_ai_vitalsigns.model.GraphObject import AttributeComparisonProxy
        
        # Create a proxy instance
        proxy = AttributeComparisonProxy(str, "test_attr")
        
        # This should fail with "object is not callable" if the bug exists
        try:
            result = proxy()
            print(f"✓ AttributeComparisonProxy is callable, returned: {result}")
        except TypeError as e:
            if "not callable" in str(e):
                print(f"✗ AttributeComparisonProxy is not callable: {e}")
                raise e
            else:
                raise e
