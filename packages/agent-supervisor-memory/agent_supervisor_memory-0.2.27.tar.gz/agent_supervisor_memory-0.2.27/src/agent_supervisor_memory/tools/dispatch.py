...
from ..utils.fs import find_ancestor_with_child_dir, project_codex_path
...
def _extract_readable_stdout(res: dict[str, Any], *, max_chars: int = 700) -> str | None:
    ...
    def _flatten_content(content: Any) -> str | None:
        ...
        if isinstance(content, dict):
            ...
            if isinstance(ctype, str) and (ctype in noise_types or ctype in noisy_item_types):
                return None
            if "text" in content:
    ...
    def _extract_from_obj(obj: Any) -> str | None:
        ...
        if "item" in obj and isinstance(obj.get("item"), dict):
            ...
            if isinstance(item_type, str) and item_type in noise_types:
                return None
...
def _compact_event(obj: dict[str, Any], *, max_text_chars: int) -> dict[str, Any] | None:
    noise_types = {
        "thread.started",
        "thread.created",
        "turn.started",
    ...
    etype = obj.get("type")
    if isinstance(etype, str) and etype in noise_types:
        return None
    ...
    def _flatten(v: Any) -> str | None:
        ...
        if isinstance(v, dict):
            vtype = v.get("type")
            if isinstance(vtype, str) and (vtype in noise_types or vtype in noisy_item_types):
                return None
...
def _format_dispatch_status_output(res: dict[str, Any], *, verbosity: str) -> dict[str, Any]:
    if verbosity == "full":
        payload = dict(res)
    ...
    return {"state": resolved_state, "message": message}
...
def _wait_for_job(
    ...
    *,
    ...
    while True:
        ...
        if not res.get("found"):
            return _format_wait_result(res, job_id, verbosity, state_override="not_found")

        state = (res.get("meta") or {}).get("status")
        now = time.time()
        if call_deadline and now >= call_deadline:
            return _format_wait_result(res, job_id, verbosity, state_override="running")
        if deadline and now >= deadline:
            return _format_wait_result(res, job_id, verbosity, state_override="timeout")
        ...
        if state and state != "running":
            ...
            return _format_wait_result(res, job_id, verbosity)
    ...
    *,
    ...
    def codex_dispatch_status(
        *,
        ...
        out = _format_dispatch_status_output(res, verbosity=verbosity)
        ...
        if raw:
            ...
            for ev in events:
                if not isinstance(ev, dict):
                    continue
                ce = _compact_event(ev, max_text_chars=max_text_chars)
                if ce:
                    compacted.append(ce)
    ...
    def codex_dispatch_wait_all(
        *,
        ...
        if summary_only:
            payload: dict[str, Any] = {"state": overall_state, "counts": counts}
        ...
        return {"state": overall_state, "jobs": results, "counts": counts}
