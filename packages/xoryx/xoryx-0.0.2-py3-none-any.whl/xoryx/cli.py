...
def run_menu() -> None:
    """
    Wrapper for interactive menu mode that handles TTY detection and exceptions.
    
    This ensures:
    - Menu only runs in interactive TTY environments
    - Clear error message when run in non-interactive contexts
    - Proper handling of EOFError and KeyboardInterrupt
    """
    if not sys.stdin.isatty():
        err("Interactive mode requires a TTY (terminal)")
        err("Use 'xoryx --help' to see available commands")
        sys.exit(1)
