...
class WSWrapper:
        ...
        async def _recv_impl():
            while True:
                ...
                if not rest.endswith("%"):
                    logger.debug("Message payload not terminated with '%', ignoring")
                    continue
    ...
    async def recv_any(self, tags: list[str], timeout: Optional[float] = None) -> Tuple[str, dict | str]:
        ...
        try:
            ...
            if not rest.endswith("%"):
                logger.debug("Message payload not terminated with '%', returning raw")
                return tag, raw
            ...
            while True:
                ...
                try:
                    ...
                    if raw.startswith("%xt%"):
                        ...
                        await tag_q.put(msg)
                        logger.debug(f"Dispatched message to tag queue '{short}'")
                except Exception:
