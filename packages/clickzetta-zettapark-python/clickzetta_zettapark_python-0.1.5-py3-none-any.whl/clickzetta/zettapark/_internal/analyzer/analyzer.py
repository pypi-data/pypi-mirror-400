...
class Analyzer:
        ...
        *,
        ...
        if isinstance(expr, (SubfieldString, SubfieldInt)):
            ...
            if parse_local_name and isinstance(field, str):
                field = field.lower()
            operand = self.analyze(
                expr.expr, df_aliased_col_name_to_real_col_name, parse_local_name
            )
            if parse_local_name:
                operand = unquote_if_safe(operand)
            return subfield_expression(operand, field)
        ...
        if isinstance(expr, UDF):
            ...
            if generate_udf_alias and expr.callable_name:
                func_name = expr.callable_name
            return function_expression(
                func_name,
                [
                    self.analyze(
    ...
    def unary_expression_extractor(
        ...
        if isinstance(expr, Alias):
            ...
            child = expr.child
            # A special case for UDFs; a better way is expected
            if isinstance(child, Alias) and isinstance(child.child, UDF):
                child = child.child
        ...
        if isinstance(expr, UnresolvedAlias):
            ...
            # TODO: fix me later
            # if parse_local_name and isinstance(expr.child, Attribute):
