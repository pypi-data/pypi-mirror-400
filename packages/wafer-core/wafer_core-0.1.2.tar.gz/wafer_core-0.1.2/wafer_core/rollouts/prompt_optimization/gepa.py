...
async def run_evolutionary_gepa(
    ...
    for gen in range(config.generations):
        ...
        async with trio.open_nursery() as nursery:
            ...
            # Intentionally captures `limiter` and `results` by reference - these are
            # shared across all nursery tasks (not loop variables despite B023 warning)
            async def eval_candidate(idx: int, template: PromptTemplate) -> None:
                async with limiter:  # noqa: B023
                    ...
                    results.append((idx, scored))  # noqa: B023
