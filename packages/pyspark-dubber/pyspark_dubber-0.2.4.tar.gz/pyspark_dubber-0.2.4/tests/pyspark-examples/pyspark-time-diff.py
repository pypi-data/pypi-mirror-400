...
df2 = (
    ...
    .withColumn(
        "DiffInSeconds",
        col("end_timestamp").cast("long") - col("from_timestamp").cast("long"),
    )
)
df2.show(truncate=False)

df.withColumn("from_timestamp", to_timestamp(col("from_timestamp"))).withColumn(
    "end_timestamp", current_timestamp()
).withColumn(
    "DiffInSeconds", unix_timestamp("end_timestamp") - unix_timestamp("from_timestamp")
).show(
    truncate=False
)
...
df3.withColumn(
    ...
    "DiffInSeconds",
    col("from_timestamp").cast("long") - col("to_timestamp").cast("long"),
).withColumn(
...
df3 = spark.createDataFrame(
    data=[("1", "07-01-2019 12:01:19.406")], schema=["id", "input_timestamp"]
)
df3.withColumn(
    "input_timestamp", to_timestamp(col("input_timestamp"), "MM-dd-yyyy HH:mm:ss.SSS")
).withColumn(
    "current_timestamp", current_timestamp().alias("current_timestamp")
).withColumn(
    "DiffInSeconds",
    current_timestamp().cast("long") - col("input_timestamp").cast("long"),
).withColumn(
    "DiffInMinutes", round(col("DiffInSeconds") / 60)
).withColumn(
    "DiffInHours", round(col("DiffInSeconds") / 3600)
