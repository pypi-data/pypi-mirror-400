...
2) Optional manual flow:
   ...
   - `code_squad_tick`: optional bounded status refresh (also enqueues dependency-ready queued tasks).
...
- Phase A runs automatically before execution (set `options.preflight=false` to skip):
  ...
  - Context budget check: flags very high `auto_context_pack.max_total_chars`, overly broad hints, or disabled context packs.
...
- Ordering: `options.order="auto"` (default) picks dependency topological order when `depends_on` exists, otherwise run order. Use `options.order="run"` to force run order.
- Conflict precheck: by default, dry-runs compute `conflicts` based on overlapping `*.files.json` from `collect` (`options.detect_conflicts=true` when `dry_run=true`). Set `options.fail_on_conflicts=true` to fail fast before attempting `git apply`.
...
code_squad_apply_many(
  ...
  options={"dry_run": true, "stop_on_error": true, "order": "topo", "fail_on_conflicts": true}
)
```
