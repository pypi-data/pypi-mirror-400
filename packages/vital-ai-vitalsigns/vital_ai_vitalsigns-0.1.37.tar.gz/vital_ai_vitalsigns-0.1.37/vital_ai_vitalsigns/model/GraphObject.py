...
class AttributeComparisonProxy:
    ...
    def __call__(self, *args, **kwargs):
        # Make proxy callable for Pydantic v2 compatibility
        # When Pydantic calls this as a JSON schema modification function,
        # it expects: js_modify_function(schema_or_field, current_handler)
        # We should delegate to the current_handler to process the schema properly
        if len(args) >= 2:
            schema_or_field, current_handler = args[0], args[1]
            # Delegate to the actual handler to process the schema
            try:
                return current_handler(schema_or_field)
            except Exception:
                # If handler fails, return the schema unchanged
                return schema_or_field
        elif len(args) == 1:
            # Single argument case - return it unchanged
            return args[0]
        return None
