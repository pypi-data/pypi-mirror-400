...
ALGORITHMVERSIONID_ANY_OF_SCHEMAS = ["int", "str"]


class AlgorithmVersionId(BaseModel):
    """
    AlgorithmVersionId
    """
    ...
    if TYPE_CHECKING:
        ...
        actual_instance: Any
    any_of_schemas: List[str] = Field(ALGORITHMVERSIONID_ANY_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        instance = AlgorithmVersionId.construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in AlgorithmVersionId with anyOf schemas: int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> AlgorithmVersionId:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> AlgorithmVersionId:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into AlgorithmVersionId with anyOf schemas: int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
