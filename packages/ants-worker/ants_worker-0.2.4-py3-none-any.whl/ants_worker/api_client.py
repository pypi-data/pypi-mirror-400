...
class GatewayClient:
    ...
    async def connect(self):
        ...
        if data.get("status") == "ok":
            ...
            print(f"[yellow]Gateway health check returned: {data}[/yellow]")
    ...
    async def sense_cold_regions(self, limit: int = 50) -> List[Region]:
        ...
        return [
            Region(
                id=r["id"],
                start=int(r["range_start"], 16) if isinstance(r["range_start"], str) else r["range_start"],
                end=int(r["range_end"], 16) if isinstance(r["range_end"], str) else r["range_end"],
                pheromone_trail=r.get("pheromone_trail", 0),
                pheromone_working=r.get("pheromone_working", 0),
            )
