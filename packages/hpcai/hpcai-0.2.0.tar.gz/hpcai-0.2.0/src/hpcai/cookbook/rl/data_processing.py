...
import hpcai
import torch
from hpcai import TensorData
from hpcai.cookbook.rl.types import Trajectory, TrajectoryGroup
from hpcai.cookbook.supervised.common import (
    create_rightshifted_model_input_and_leftshifted_targets,
)
from hpcai.cookbook.utils.misc_utils import all_same, safezip
...
FlatObElem = int | hpcai.ModelInputChunk
...
def _flat_ob_to_model_input(flat_ob: FlatOb) -> hpcai.ModelInput:
    out: list[hpcai.ModelInputChunk] = []
    current_text_chunk: list[int] = []

    def flush_text_chunk():
        if current_text_chunk:
            out.append(hpcai.EncodedTextChunk(tokens=current_text_chunk))
    ...
    for elem in flat_ob:
        if isinstance(elem, int):
            ...
            out.append(elem)
    flush_text_chunk()
    return hpcai.ModelInput(chunks=out)


def _flatten_chunks(chunks: list[hpcai.ModelInputChunk]) -> FlatOb:
    out: FlatOb = []
    for chunk in chunks:
...
def trajectory_to_data(traj: Trajectory, traj_advantage: float) -> list[hpcai.Datum]:
    """
...
def assemble_training_data(
    ...
    advantages_P: List[torch.Tensor],
) -> tuple[List[hpcai.Datum], List[dict[str, int]]]:
