...
if TYPE_CHECKING:
    ...
    from ..models.state_advancedfilter_4_neq import StateAdvancedfilter4Neq


T = TypeVar("T", bound="StateAdvancedfilter4")


@_attrs_define
class StateAdvancedfilter4:
    r"""Advanced IncidentStateEnum filter

    Attributes:
        eq (StateAdvancedfilter4Eq | Unset): Checks for equality with the provided value.
        neq (StateAdvancedfilter4Neq | Unset): Checks for inequality with the provided value.
        exists (bool | Unset): Checks if the current property exists.
        in_ (list[StateAdvancedfilter4InItem] | Unset): Checks if the property matches any of the provided values.
        not_in (list[StateAdvancedfilter4NotInItem] | Unset): Checks if the property does not match any of the provided
            values.
    ...
    eq: StateAdvancedfilter4Eq | Unset = UNSET
    neq: StateAdvancedfilter4Neq | Unset = UNSET
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        d = dict(src_dict)
        _eq = d.pop("$eq", UNSET)
        eq: StateAdvancedfilter4Eq | Unset
        if isinstance(_eq, Unset):
            eq = UNSET
        else:
            eq = StateAdvancedfilter4Eq.from_dict(_eq)

        _neq = d.pop("$neq", UNSET)
        neq: StateAdvancedfilter4Neq | Unset
        if isinstance(_neq, Unset):
            neq = UNSET
        else:
            neq = StateAdvancedfilter4Neq.from_dict(_neq)
        ...
        if _in_ is not UNSET:
            ...
            for in_item_data in _in_:
                in_item = StateAdvancedfilter4InItem(in_item_data)
        ...
        if _not_in is not UNSET:
            ...
            for not_in_item_data in _not_in:
                not_in_item = StateAdvancedfilter4NotInItem(not_in_item_data)

                not_in.append(not_in_item)

        like = d.pop("$like", UNSET)

        state_advancedfilter_4 = cls(
            eq=eq,
            neq=neq,
            exists=exists,
            in_=in_,
        ...
        state_advancedfilter_4.additional_properties = d
        return state_advancedfilter_4
