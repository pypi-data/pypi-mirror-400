...
class ProgressDisplay:
    ...
    def create_part_task(self, part_id: int, size: int):
        """Create a new progress bar for a part.

        Args:
            part_id: Part number (0-indexed)
            size: Total bytes for this part
        """
        if part_id not in self.part_tasks:
            task_id = self.progress.add_task(
                f"Part {part_id + 1}/{self.total_parts}",
                total=size,
                completed=0,
                visible=True,
                mbps=0.0,
    ...
    def update_from_callback(self, progress_data: dict):
        ...
        Args:
            progress_data: Progress data dict with:
                ...
                - speed_mbps: Aggregate speed in Mbps
                - cdn_server: Current CDN server hostname
                - total_parts: Total number of parts
                - parts: Array of part data (part_id, bytes, total, speed)
        """
        ...
        if self.overall_task_id is not None:
            self.progress.update(
                ...
                mbps=progress_data.get("speed_mbps"),
            )

        # Update all active parts (only update, don't create)
        for part_data in progress_data.get("parts", []):
            part_id = part_data["part_id"]
            part_bytes = part_data["bytes_downloaded"]
            part_speed = part_data["speed_mbps"]

            # Only update if task already exists (created by lifecycle callback)
