...
class PixelTypeTest(apitestcase.ApiTestCase):
  ...
  def test_int(self):
    ...
    max_value = 1
    dimensions = 2
    pixeltype = le.PixelType(precision, min_value, max_value, dimensions)
    self.assertEqual({'value': 'fakeValue'}, pixeltype.getInfo())

    pixeltype_func = le.ApiFunction.lookup('PixelType')
  ...
  def test_minimal_double(self):
    ...
    pixeltype = le.PixelType(precision)
    result = json.loads(pixeltype.serialize())
    expect = {
        'result': '0',
        'values': {
  ...
  def test_float_named_args(self):
    ...
    cast_result = json.loads(le.PixelType(pixeltype).serialize())
  ...
  def test_float_computed_object_args(self):
    ...
    dimensions = 5
    pixeltype = le.PixelType(
        precision=le.String(precision).cat(''),
        minValue=le.Number(min_value),
        maxValue=le.Number(max_value),
        dimensions=le.Number(dimensions),
    )
    result = json.loads(pixeltype.serialize())
    expected_precision = {
        'functionInvocationValue': {
  ...
  def test_float_no_dimensions(self):
    ...
    max_value = 0.2
    result = json.loads(
        le.PixelType(precision, min_value, max_value).serialize()
    )
    expect = {
        'result': '0',
        'values': {
  ...
  def test_dimensions(self):
    ...
    expression = le.PixelType(Type.FLOAT).dimensions()
  ...
  def test_maxValue(self):
    ...
    expression = le.PixelType(Type.FLOAT).maxValue()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_minValue(self):
    ...
    expression = le.PixelType(Type.FLOAT).minValue()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_precision(self):
    ...
    expression = le.PixelType(Type.FLOAT).precision()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

  def test_double(self):
    expect = pixeltype_noargs_expr('double')
    expression = le.PixelType.double()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

  def test_float(self):
    expect = pixeltype_noargs_expr('float')
    expression = le.PixelType.float()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

  def test_int16(self):
    expect = pixeltype_noargs_expr('int16')
    expression = le.PixelType.int16()
    result = json.loads(expression.serialize())
