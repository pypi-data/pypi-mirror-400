...
def uninstall(ctx, box, ip, user, keep_config, keep_docker_images, remove_all, yes):
    ...
    \b
    - /etc/lager directory (saved nets configuration)
    ...
    Examples:
        ...
        lager uninstall --ip <BOX_IP> --keep-config --yes
        lager uninstall --box JUL-10
    """
        ...
        try:
            ...
            if result.returncode == 0 or allow_fail:
                ...
                if not use_interactive_ssh and hasattr(result, 'stderr') and result.stderr:
                    click.secho(f"    Error: {result.stderr.strip()}", fg='red', err=True)
                return False
        except subprocess.TimeoutExpired:
