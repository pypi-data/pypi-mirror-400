...
class ExePacker(He):
    def __init__(self, u, mainpy:str, website_module:str=None, one_file:bool=False, one_dir:bool=False, show_console:bool=False, pack_overwrite:bool=False):
        ...
        self.configs.update({
            ...
            'root_modules': [],
            'dist_path': (pkgdir:=self.__si__.project_folder.child_folder('.packages').child_folder(exename)).child_folder('dist'),
            ...
            if fd.is_exists():
                ...
                """for dirpath, _, filenames in os.walk(fd):
                    for file in filenames:
                        ...
                        dst = os.path.relpath(src, self.__si__.project_folder)
                        # --add-data 也必须拆开
