...
@mcp.tool()
async def save_note_as_word(note: dict[str, Any]) -> str:
    """根据帖子数据生成word文档：note: 小红书帖子数据（包含帖子内容和评论）
    """
    ...
    note_id = info.get("note_id") or "note"
    filename = f"{note_id}_{datetime.now().strftime('%Y%m%d%H%M%S')}.docx"
