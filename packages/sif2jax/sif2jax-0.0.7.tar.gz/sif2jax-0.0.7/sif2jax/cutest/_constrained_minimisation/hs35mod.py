...
class HS35MOD(AbstractConstrainedMinimisation):
    """Hock and Schittkowski problem 35 (Modified version).
    ...
    def objective(self, y, args):
        ...
        # Constant from CONSTANTS section: 9.0 (positive)
        constant = 9.0
    ...
    @property
    def bounds(self):
        """Variable bounds - x2 is fixed at 0.5, others non-negative."""
        lower = jnp.array([0.0, 0.5, 0.0], dtype=jnp.float64)
        upper = jnp.array([jnp.inf, 0.5, jnp.inf], dtype=jnp.float64)
        return lower, upper
