...
def NotFoundError() -> labelearthapiclient.errors.HttpError:
  ...
  return labelearthapiclient.errors.HttpError(resp, content)
...
class DataTest(unittest.TestCase):
  ...
  def testIsInitialized(self):
    self.assertFalse(le.data.is_initialized())
    with apitestcase.UsingCloudApi():
      self.assertTrue(le.data.is_initialized())

  def testSetMaxRetries_badValues(self):
    with self.assertRaises(ValueError):
      le.data.setMaxRetries(-1)
    with self.assertRaises(ValueError):
      le.data.setMaxRetries(100)

  def testSetMaxRetries(self):
    mock_result = {'result': 5}
    original_max_retries = le.data._max_retries
    le.data.setMaxRetries(3)
    cloud_api_resource = mock.MagicMock()
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      cloud_api_resource.projects().value().compute().execute.return_value = (
          mock_result
      )
      self.assertEqual(5, le.data.computeValue(le.Number(1)))
      self.assertEqual(
          3,
    ...
    le.data._max_retries = original_max_retries
  ...
  def testListOperationsEmptyList(self):
    ...
    with apitestcase.UsingCloudApi(mock_http=mock_http):
      self.assertEqual([], le.data.listOperations())
  ...
  def testCreateAsset(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      ...
      le.data.createAsset({'type': 'FOLDER'}, 'users/foo/xyz123')
  ...
  def testCreateFolder(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      ...
      le.data.createFolder('users/foo/xyz123')
      mock_create_asset = cloud_api_resource.projects().assets().create
      mock_create_asset.assert_called_once()
      parent = mock_create_asset.call_args.kwargs['parent']
      self.assertEqual(parent, 'projects/earthengine-legacy')
  ...
  def testCreateAssets(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      ...
      le.data.create_assets([asset_name], 'FOLDER', False)
      mock_create_asset = cloud_api_resource.projects().assets().create
  ...
  def testCreateAssets_empty(self):
    cloud_api_resource = mock.MagicMock()
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      le.data.create_assets([], 'FOLDER', False)
      mock_create_asset = cloud_api_resource.projects().assets().create
  ...
  def testCreateAssets_withParents(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      ...
      le.data.create_assets([asset_name], 'FOLDER', True)
      mock_create_asset = cloud_api_resource.projects().assets().create
  ...
  def testSetAssetProperties(self):
    ...
    with apitestcase.UsingCloudApi(mock_http=mock_http), mock.patch.object(
        le.data, 'updateAsset', autospec=True) as mock_update_asset:
      le.data.setAssetProperties('foo', {
          'mYPropErTy': 'Value',
          'system:time_start': 1
      })
  ...
  def testListAssets(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      ...
      actual_result = le.data.listAssets('path/to/folder')
      cloud_api_resource.projects().assets().listAssets(
  ...
  def testListAssetsWithPageSize(self):
    ...
    with apitestcase.UsingCloudApi(mock_http=mock_http):
      actual_result = le.data.listAssets(
  ...
  def testListImages(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      ...
      cloud_api_resource.projects().assets().listAssets_next.return_value = None
      actual_result = le.data.listImages('path/to/folder')
      cloud_api_resource.projects().assets().listAssets(
      ).execute.assert_called_once()
  ...
  def testListImagesWithPageSize(self):
    ...
    mock_http.request.side_effect = [(ok_resp, page)]
    with apitestcase.UsingCloudApi(mock_http=mock_http):
      actual_result = le.data.listImages(
          {'parent': 'path/to/folder', 'pageSize': 3}
      )
      expected_result = {
  ...
  def testListBuckets(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      ...
      actual_result = le.data.listBuckets()
  ...
  def testSimpleGetListViaCloudApi(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      ...
      actual_result = le.data.getList({'id': 'glam', 'num': 3})
      expected_params = {
  ...
  def testGetMapId(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      ...
      actual_result = le.data.getMapId({
      ...
      self.assertIsInstance(actual_result['tile_fetcher'], le.data.TileFetcher)

  def testGetMapId_withWorkloadTag(self):
    with le.data.workloadTagContext('mapid-tag'):
      cloud_api_resource = mock.MagicMock()
      with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
        mock_result = {
            'name': 'projects/earthengine-legacy/maps/DOCID',
  ...
  def testGetDownloadId(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      ...
      actual_result = le.data.getDownloadId({
          'image': image.Image('my-image'),
          'name': 'dummy'
      })
      cloud_api_resource.projects().thumbnails().create(
  ...
  def testGetDownloadId_withWorkloadTag(self):
    with le.data.workloadTagContext('downloadid-tag'):
      cloud_api_resource = mock.MagicMock()
      with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
        mock_result = {'name': 'projects/earthengine-legacy/thumbnails/DOCID'}
        cloud_api_resource.projects().thumbnails().create(
        ).execute.return_value = mock_result
        le.data.getDownloadId({
            'image': image.Image('my-image'),
            'name': 'dummy'
        })
        self.assertEqual(
  ...
  def testGetDownloadId_withImageID(self):
    cloud_api_resource = mock.MagicMock()
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      with self.assertRaisesRegex(le.ee_exception.EEException,
                                  '^Image ID string is not supported.'):
        le.data.getDownloadId({'id': 'my-image', 'name': 'dummy'})
  ...
  def testGetThumbId(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      ...
      actual_result = le.data.getThumbId({
          'image': image.Image('my-image'),
          'name': 'dummy'
      })
      cloud_api_resource.projects().thumbnails().create(
  ...
  def testGetThumbId_withWorkloadTag(self):
    with le.data.workloadTagContext('thumbid-tag'):
      cloud_api_resource = mock.MagicMock()
      with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
        mock_result = {'name': 'projects/earthengine-legacy/thumbnails/DOCID'}
        cloud_api_resource.projects().thumbnails().create(
        ).execute.return_value = mock_result
        le.data.getThumbId({'image': image.Image('my-image'), 'name': 'dummy'})
        self.assertEqual(
            'thumbid-tag',
            cloud_api_resource.projects().thumbnails().create.call_args
            .kwargs['workloadTag'])
  ...
  def testGetTableDownloadId(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      ...
      actual_result = le.data.getTableDownloadId({
  ...
  def testGetTableDownloadId_withWorkloadTag(self):
    with le.data.workloadTagContext('tableid-tag'):
      cloud_api_resource = mock.MagicMock()
      with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
        mock_result = {'name': 'projects/earthengine-legacy/thumbnails/DOCID'}
        cloud_api_resource.projects().tables().create(
        ).execute.return_value = mock_result
        le.data.getTableDownloadId({
            'table': featurecollection.FeatureCollection('my-fc'),
            'filename': 'dummy'
        })
        self.assertEqual(
  ...
  def testCloudProfilingEnabled(self):
    ...
    with le.data.profiling(ProfileHook):
      with apitestcase.UsingCloudApi(), DoCloudProfileStubHttp(self, True):
        le.data.listImages({'parent': 'projects/earthengine-public/assets/q'})
  ...
  def testCloudErrorTranslation(self):
    ...
    with apitestcase.UsingCloudApi(mock_http=mock_http):
      with self.assertRaisesRegex(le.ee_exception.EEException, '^errorly$'):
        le.data.listImages({'parent': 'projects/earthengine-public/assets/q'})
  ...
  def testListFeatures(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      ...
      actual_result = le.data.listFeatures({
          'assetId':
  ...
  @mock.patch.object(le.data, '_tile_base_url', new='base_url')
  def testGetFeatureViewTilesKey(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      ...
      actual_result = le.data.getFeatureViewTilesKey({
  ...
  def testGetProjectConfig(self) -> None:
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      ...
      actual_result = le.data.getProjectConfig()
  ...
  def testUpdateProjectConfig(self) -> None:
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      ...
      actual_result = le.data.updateProjectConfig(
  ...
  def testWorkloadTag(self):
    self.assertEqual('', le.data.getWorkloadTag())
    le.data.setDefaultWorkloadTag(None)
    self.assertEqual('', le.data.getWorkloadTag())
    le.data.setDefaultWorkloadTag('')
    self.assertEqual('', le.data.getWorkloadTag())
    le.data.setDefaultWorkloadTag(0)
    self.assertEqual('0', le.data.getWorkloadTag())
    le.data.setDefaultWorkloadTag(123)
    self.assertEqual('123', le.data.getWorkloadTag())

    with self.assertRaisesRegex(ValueError, 'Invalid tag'):
      le.data.setDefaultWorkloadTag('inv@lid')

    with self.assertRaisesRegex(ValueError, 'Invalid tag'):
      le.data.setDefaultWorkloadTag('in.valid')

    with self.assertRaisesRegex(ValueError, 'Invalid tag'):
      le.data.setDefaultWorkloadTag('Invalid')

    with self.assertRaisesRegex(ValueError, 'Invalid tag'):
      le.data.setDefaultWorkloadTag('-invalid')

    with self.assertRaisesRegex(ValueError, 'Invalid tag'):
      le.data.setDefaultWorkloadTag('invalid_')

    with self.assertRaisesRegex(ValueError, 'Invalid tag'):
      le.data.setDefaultWorkloadTag('i' * 64)

    le.data.setDefaultWorkloadTag('default-tag')
    self.assertEqual('default-tag', le.data.getWorkloadTag())

    le.data.setWorkloadTag('exports-1')
    self.assertEqual('exports-1', le.data.getWorkloadTag())

    le.data.setWorkloadTag('exports-2')
    self.assertEqual('exports-2', le.data.getWorkloadTag())

    le.data.resetWorkloadTag()
    self.assertEqual('default-tag', le.data.getWorkloadTag())

    with le.data.workloadTagContext('in-context'):
      self.assertEqual('in-context', le.data.getWorkloadTag())

    self.assertEqual('default-tag', le.data.getWorkloadTag())

    le.data.setWorkloadTag('reset-me')
    self.assertEqual('reset-me', le.data.getWorkloadTag())

    le.data.setWorkloadTag('')
    self.assertEqual('', le.data.getWorkloadTag())

    le.data.setDefaultWorkloadTag('reset-me')
    self.assertEqual('reset-me', le.data.getWorkloadTag())

    le.data.resetWorkloadTag(True)
    self.assertEqual('', le.data.getWorkloadTag())

  def testResetWorkloadTagOptParams(self):
    le.data.setDefaultWorkloadTag('reset-me')
    self.assertEqual('reset-me', le.data.getWorkloadTag())
    le.data.resetWorkloadTag(opt_resetDefault=True)
    self.assertEqual('', le.data.getWorkloadTag())
  ...
  def testGetAssetRootQuota_V1Alpha(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      ...
      quota = le.data.getAssetRootQuota('projects/test-proj/assets')
      expected = {
...
def DoCloudProfileStubHttp(test, expect_profiling):
  ...
  def MockRequest(self, method, uri, data, headers, timeout):
    ...
    test.assertEqual(expect_profiling, le.data._PROFILE_REQUEST_HEADER
                     in headers)
    ...
    if expect_profiling:
      response.headers[
          le.data._PROFILE_RESPONSE_HEADER_LOWERCASE] = 'someProfileId'
    return response
