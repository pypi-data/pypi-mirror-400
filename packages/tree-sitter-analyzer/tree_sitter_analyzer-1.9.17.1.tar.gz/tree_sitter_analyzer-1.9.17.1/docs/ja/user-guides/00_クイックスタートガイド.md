...
以下の**7言語**が完全にサポートされています（実際の解析動作確認済み）：
...
**重要:** C、C++、Go、Rust、C#、PHP、JSONは`LanguageDetector`に定義されていますが、現時点ではプラグイン実装が存在しないため、実際の解析には使用できません。

**確認方法:**
```bash
# 実際に読み込まれたプラグイン（7言語）の拡張子を表示
uv run tree-sitter-analyzer --show-supported-extensions
```
