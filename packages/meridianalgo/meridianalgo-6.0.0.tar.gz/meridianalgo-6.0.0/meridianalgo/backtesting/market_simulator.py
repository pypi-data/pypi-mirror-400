...
from .events import MarketEvent, OrderEvent, FillEvent, OrderSide, OrderType, FillStatus
...
class SquareRootSlippageModel(SlippageModel):
    ...
    def calculate_slippage(self, order: OrderEvent, market_state: MarketState) -> float:
        ...
        # Square-root impact
        impact = self.impact_coefficient * np.sqrt(participation_rate) * market_state.volatility
...
class MarketSimulator:
    ...
    def execute_order(self, order: OrderEvent) -> List[FillEvent]:
        ...
        if order.order_type == OrderType.MARKET:
            ...
            return [self._create_rejection(order, f"Order type {order.order_type} not supported")]
    
    def _execute_market_order(self, order: OrderEvent, market_state: MarketState) -> List[FillEvent]:
        ...
        # Create fill event
        fill_status = FillStatus.FILLED if fill_quantity == order.quantity else FillStatus.PARTIAL_FILL
    ...
    def _execute_limit_order(self, order: OrderEvent, market_state: MarketState) -> List[FillEvent]:
    ...
    def _execute_stop_order(self, order: OrderEvent, market_state: MarketState) -> List[FillEvent]:
        ...
        if order.side == OrderSide.BUY and market_state.last_price >= order.stop_price:
            triggered = True
        elif order.side == OrderSide.SELL and market_state.last_price <= order.stop_price:
            triggered = True
    ...
    def _execute_stop_limit_order(self, order: OrderEvent, market_state: MarketState) -> List[FillEvent]:
        """Execute a stop-limit order."""
        if order.stop_price is None or order.price is None:
            return [self._create_rejection(order, "Stop-limit order requires both stop and limit price")]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_commission_per_trade': self.total_commission / max(1, self.total_trades),
