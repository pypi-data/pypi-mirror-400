...
if command -v tailscale &> /dev/null && tailscale status &> /dev/null; then
    ...
    echo -e "${GREEN}✓ Detected Tailscale interface: $TAILSCALE_IFACE${NC}"
else
...
if [ -n "$CORPORATE_VPN_IFACE" ]; then
    if ip link show "$CORPORATE_VPN_IFACE" &> /dev/null; then
        ...
        echo -e "${GREEN}✓ Detected corporate VPN interface: $CORPORATE_VPN_IFACE ($CORPORATE_IP)${NC}"
    else
        echo -e "${RED}✗ Corporate VPN interface $CORPORATE_VPN_IFACE not found${NC}"
...
echo ""
echo -e "${GREEN}✓ Default DENY policy for incoming traffic${NC}"
echo -e "${GREEN}✓ SSH (22) allowed from anywhere${NC}"
echo -e "${GREEN}✓ Lager services accessible from:${NC}"
echo -e "  - Localhost (lo)"
...
echo -e "${GREEN}✓ External access blocked for Lager services${NC}"
echo ""
