...
class GitHubClient:
    ...
    def format_pr_body(
        ...
        """Format a PR body with Notion task link.

        Args:
            task_identifier: Notion task identifier (e.g., "CLO-001")
    ...
    def format_pr_title(
        self, task_identifier: str, title: str, pr_type: str = "feat"
    ) -> str:
        """Format a PR title.

        Args:
            task_identifier: Notion task identifier
            title: Task title
        ...
        return f"{pr_type}({task_identifier}): {clean_title}"
    ...
    async def merge_pr(
        ...
        Args:
            repo_path: Path to the repository (main or worktree)
        ...
        try:
            await self._run_gh(*args, cwd=repo_path)
