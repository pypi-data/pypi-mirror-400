...
در صورتی که میخواهید از این حالت خارج شود و از ورودی های متود ها پیروی کند مقدار آن را در متود ست مین پارس مود برابر 'Unknown' کنید"""
    ...
    def on_message_updates(self, filters: Optional[Filter] = None, edited_messages: Literal[False, True, "both"] = False, deleted_messages: Literal[False, True, "both"] = False):
    ...
    async def _process_messages_webhook(self):
        while self._running:
            ...
            if json_response and json_response.get('status') is True and response.status_code == 200:
                results = json_response.get('updates', [])
                if results:
                    for result in results:
                        if result["type"] not in ["NewMessage", "UpdatedMessage"]:
                            continue
    ...
    async def _run_all(self):
        ...
        if self._fetch_messages_polling and self._message_handlers_:
    ...
    async def run(self):
        ...
        if (self._fetch_messages_webhook and not self._message_handlers) or (self._fetch_messages_polling and not self._message_handlers_):
