...
from .helpers import (section_to_string, CustomDict, convert_section, InpSection, InpSectionGeo,
                      InpSectionGeneric, check_order, head_to_str,
                      iter_section_lines, SwmmInputWarning, BaseSectionObject, )
from .section_lists import SECTIONS_ORDER_DEFAULT
from .._io_helpers import get_default_encoding, read_txt_file, CONFIG
...
class SwmmInput(CustomDict):
    ...
    def __init__(self, *args, custom_section_handler=None, encoding='', force_ignore_case=False, **kwargs):
        ...
        self._original_section_order = CONFIG.section_order
    ...
    def _init_from_str(self, txt, force_ignore_case=False):
        ...
        if check_order(self, SECTIONS_ORDER_DEFAULT):
            self._original_section_order = SECTIONS_ORDER_DEFAULT
    ...
    def _init_from_file(self, filename, force_ignore_case=False):
        ...
        Args:
            filename (str or pathlib.Path): path/filename to .inp file
        ...
        fn = Path(filename)
        if fn.is_file() or isinstance(filename, Path):
            if fn.suffix.lower() != '.inp':
                warnings.warn('Reading a file as SwmmInput which has has not the .inp extension.', SwmmInputWarning)
    ...
    def read_file(cls, filename, custom_converter=None, force_ignore_case=False, encoding=''):
        ...
        Args:
            filename (str | pathlib.Path): path/filename to .inp file
    ...
    def SYMBOLS(self):
        ...
        Returns:
            Mapping[str, Symbol] | InpSectionGeo: Symbol section
        """
        if SYMBOLS in self:
            return self[SYMBOLS]
