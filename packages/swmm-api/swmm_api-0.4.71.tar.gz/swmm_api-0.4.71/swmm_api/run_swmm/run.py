...
def swmm5_run(fn_inp, fn_rpt=None, fn_out=None, progress_size=None, swmm_lib_path=None, working_dir=None):
    ...
    If you want to use a custom swmm executable, use :func:`~swmm_api.run_swmm.run_epaswmm.swmm5_run_epa`(..., swmm_path='/path/to/custom_smm') instead.
    ...
    try:
        if progress_size:
            swmm5_run_progress(fn_inp, fn_rpt, fn_out, n_total=progress_size, working_dir=working_dir)
        else:
