...
class SerializerTest(apitestcase.ApiTestCase):
  ...
  def testSerialization(self):
    ...
    call = le.ComputedObject('String.cat', {'string1': 'x', 'string2': 'y'})
    body = lambda x, y: le.CustomFunction.variable(None, 'y')
    sig = {'returns': 'Object',
           'args': [
               {'name': 'x', 'type': 'Object'},
               {'name': 'y', 'type': 'Object'}]}
    custom_function = le.CustomFunction(sig, body)
    to_encode = [
        None,
        True,
        5,
        ...
        112233445566778899,
        'hello',
        le.Date(1234567890000),
        le.Geometry(le.Geometry.LineString(1, 2, 3, 4), 'SR-ORG:6974'),
        le.Geometry.Polygon([
  ...
  def testRepeats(self):
    ...
    test1 = le.Image(5).mask(le.Image(5))
    expected1 = {
        'type': 'CompoundValue',
        'scope': [
            ['0', {
  ...
  def testToJsonOptParams(self):
    self.assertIn('\n', serializer.toJSON(le.Image(0), opt_pretty=True))
    self.assertNotIn('\n', serializer.toJSON(le.Image(0), opt_pretty=False))
