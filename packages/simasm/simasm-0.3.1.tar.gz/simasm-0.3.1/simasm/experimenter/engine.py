...
class VerificationEngine:
    ...
    def _run_multi_seed_verification(
        ...
        model_names = list(seed_results[0]["model_stats"].keys())
        avg_stats = {}
        for name in model_names:
            raw_lengths = [r["model_stats"][name]["raw_length"] for r in seed_results]
            ns_lengths = [r["model_stats"][name]["ns_length"] for r in seed_results]
            avg_stats[name] = {
        ...
        # Add multi-seed specific attributes
        self._result.seeds = seeds
        self._result.num_seeds = len(seeds)
        self._result.equivalent_count = equivalent_count
        self._result.failed_seeds = failed_seeds
        self._result.seed_results = seed_results
    ...
    def _write_json_output(self, path: Path) -> None:
        ...
        if hasattr(self._result, 'num_seeds') and self._result.num_seeds > 1:
            data["seeds"] = self._result.seeds
            ...
            data["seed_results"] = self._result.seed_results
        else:
