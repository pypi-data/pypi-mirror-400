...
from ..utils.fs import atomic_write_json, codex_path_for, find_ancestor_with_child_dir, project_codex_path
...
def _extract_readable_stdout(res: dict[str, Any], *, max_chars: int = 700) -> str | None:
    ...
    for line in reversed(lines):
        ...
        if isinstance(obj, dict):
            etype = obj.get("type")
            if isinstance(etype, str) and (etype in NOISE_EVENT_TYPES or etype in NOISY_ITEM_TYPES):
                is_noise_obj = True
            item = obj.get("item")
            if isinstance(item, dict):
                itype = item.get("type")
                if isinstance(itype, str) and (itype in NOISE_EVENT_TYPES or itype in NOISY_ITEM_TYPES):
                    is_noise_obj = True
        if raw_fallback is None and not is_noise_obj:
            raw_fallback = candidate
...
def _format_dispatch_status_output(
    ...
    *,
    ...
    if cursor_mode:
        ...
        if state == "running" and next_poll_ms is not None:
            payload["next_poll_ms"] = int(next_poll_ms)
    ...
    if resolved_state == "running" and next_poll_ms is not None:
        payload["next_poll_ms"] = int(next_poll_ms)
    if include_summary_cursor:
        payload["summary_cursor"] = summary_cursor_value
    return payload
...
def _wait_for_job(
    ...
    *,
    ...
    summary_max_items: int = 5,
    include_summary_cursor: bool = False,
    refresh_summary: Callable[..., list[str]] | None = None,
    get_summary_cursor: Callable[[str], int] | None = None,
) -> dict[str, Any]:
    refresh_fn = refresh_summary or (lambda *_args, **_kwargs: [])
    cursor_fn = get_summary_cursor or (lambda *_args, **_kwargs: 0)
    ...
    while True:
        ...
        try:
            summary_lines = refresh_fn(job_id, dispatch, store_max_items=30)
        except Exception:
            summary_lines = []
        try:
            summary_cursor_value = int(cursor_fn(job_id) or 0)
        except Exception:
            summary_cursor_value = 0
        if not res.get("found"):
...
def _dispatch_jobs_dir_for_cwd(
    *,
    dispatch_base_dir: Path,
    cwd: str | None,
) -> Path:
    """
    Resolve the effective dispatch jobs dir.

    If server is configured with a global/base dispatch dir, use it.
    Otherwise, default to <repo>/.codex/codex-dispatch derived from the provided cwd.
    """
    base = Path(dispatch_base_dir).expanduser()
    # Heuristic: if user points dispatch_base_dir to ~/.codex/*, treat it as global.
    try:
    ...
    try:
        ...
        return base
    return codex_path_for(start, "codex-dispatch")
...
def register_dispatch_tools(
    ...
    *,
    ...
    job_summary: dict[str, list[str]] = {}
    job_summary_cursor: dict[str, int] = {}
    poll_backoff_lock = threading.Lock()
    poll_backoff_state: dict[str, dict[str, float]] = {}
    poll_backoff_schedule = [250, 500, 1000, 2000, 4000, 8000, 15000, 25000, 35000]
    ...
    def _compute_next_poll_ms(job_id: str | None, state: str | None, *, include_value: bool = True) -> int | None:
        if not job_id:
            return None
        now_ms = time.monotonic() * 1000.0
        with poll_backoff_lock:
            entry = poll_backoff_state.get(job_id, {"step": -1, "last_seen": 0.0})
            step = int(entry.get("step", -1))
            last_seen = float(entry.get("last_seen", 0.0) or 0.0)
            if state != "running" or (last_seen and (now_ms - last_seen) > 70000):
                step = -1
            if state == "running":
                step = min(step + 1, len(poll_backoff_schedule) - 1)
                entry["step"] = step
            else:
                entry["step"] = -1
            entry["last_seen"] = now_ms
            poll_backoff_state[job_id] = entry
            if not include_value or state != "running":
                return None
            idx = entry.get("step", -1)
            if not isinstance(idx, int) or idx < 0:
                idx = 0
            if idx >= len(poll_backoff_schedule):
                idx = len(poll_backoff_schedule) - 1
            return int(poll_backoff_schedule[idx])

    def _dispatch_for_job(job_id: str) -> CodexDispatch:
        bucket, _ = _split_bucketed_job_id(job_id)
        if bucket:
            return CodexDispatch(_config_with_prune(Path(dispatch_base_dir).expanduser() / bucket))
        try:
            base = Path(dispatch_base_dir).expanduser()
            for child in base.iterdir():
                ...
                if (child / job_id / "meta.json").exists():
                    return CodexDispatch(_config_with_prune(child))
        except FileNotFoundError:
            pass
        return CodexDispatch(_config_with_prune(Path(dispatch_base_dir).expanduser()))
    ...
    @instrument_tool("codex_dispatch_start")
    def codex_dispatch_start(
        *,
        ...
        fast_mode = True
        if options and "fast_mode" in options:
        ...
        def _has_effort_config(config: list[str] | None) -> bool:
            if not config:
                return False
            for item in config:
                if isinstance(item, str) and "model_reasoning_effort" in item:
                    return True
            return False
        ...
        max_effort = _normalize_effort(options.get("max_model_reasoning_effort") or options.get("max_reasoning_effort")) if options else None
        if max_effort:
            requested_effort = _normalize_effort(
                options.get("model_reasoning_effort")
                if options.get("model_reasoning_effort") is not None
                else options.get("reasoning_effort") if options else None
            )
            ...
            effective_extra_config = list(extra_config) if extra_config else []
            if not _has_effort_config(effective_extra_config):
                effective_extra_config.append('model_reasoning_effort="medium"')

        bucket = _dispatch_bucket_for_cwd(cwd)
        jobs_dir = _dispatch_jobs_dir_for_cwd(dispatch_base_dir=dispatch_base_dir, cwd=cwd)
        idempotency_key = None
        if options and isinstance(options.get("idempotency_key"), str):
            idempotency_key = str(options.get("idempotency_key") or "").strip()
            if not idempotency_key:
                idempotency_key = None
        job_id_override = f"{bucket}--idemp_{_sha256_short(idempotency_key, n=16)}" if idempotency_key else None
        ...
        if not wait:
            if verbosity == "full":
                ...
                return full_payload
            state_value = str(res.get("status") or "running")
            payload: dict[str, Any] = {"state": state_value, "job_id": job_id, "job_ref": job_ref}
            if res.get("existing"):
        ...
        result = _wait_for_job(
            ...
            summary_max_items=summary_max_items,
            include_summary_cursor=include_summary_cursor,
            refresh_summary=lambda job_id, d, store_max_items=30: _refresh_summary(job_id, d, store_max_items=store_max_items),
            get_summary_cursor=lambda job_id: _get_summary_cursor(job_id),
        )
        if isinstance(result, dict) and "job_ref" not in result:
            result["job_ref"] = job_ref
            if options and options.get("omit_job_id") is True:
                result.pop("job_id", None)
        if isinstance(result, dict) and res.get("existing"):
    ...
    @instrument_tool("codex_dispatch_status")
    def codex_dispatch_status(
        *,
        ...
        summary_max_items = int(options.get("summary_max_items", 5)) if options else 5
        include_summary_cursor = bool(options.get("include_summary_cursor")) if options else False
        include_next_poll_ms = True
        if options:
            if options.get("omit_next_poll_ms") is True:
                include_next_poll_ms = False
            elif "include_next_poll_ms" in options:
                include_next_poll_ms = bool(options.get("include_next_poll_ms"))
        with last_job_lock:
        ...
        state_value = (res.get("meta") or {}).get("status") or res.get("state")
        next_poll_ms = _compute_next_poll_ms(effective_job_id, state_value, include_value=include_next_poll_ms)
    ...
    @instrument_tool("codex_dispatch_cancel")
    ...
    @instrument_tool("codex_dispatch_events")
    ...
    @instrument_tool("codex_dispatch_wait_all")
    def codex_dispatch_wait_all(
        *,
        ...
        if job_ids is None and not effective_job_ids:
            return {
                "state": "error",
                "code": "E_JOB_IDS_REQUIRED",
                "message": "job_ids omitted and no started jobs tracked in this server process; pass job_ids (e.g. from project_status/task state)",
            }
