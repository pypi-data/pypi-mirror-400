...
def root(context, missing=missing):
    ...
    if t_5((undefined(name='management_security') if l_0_management_security is missing else l_0_management_security)):
        ...
        if t_5(environment.getattr(environment.getattr((undefined(name='management_security') if l_0_management_security is missing else l_0_management_security), 'password'), 'encryption_reversible')):
            ...
            yield '\n'
        for l_1_policy in t_3(environment.getattr(environment.getattr((undefined(name='management_security') if l_0_management_security is missing else l_0_management_security), 'password'), 'policies'), sort_key='name'):
            _loop_vars = {}
            pass
            yield '   !\n   password policy '
            ...
            if t_5(environment.getattr(l_1_policy, 'maximum')):
                ...
                if t_5(environment.getattr(environment.getattr(l_1_policy, 'maximum'), 'sequential')):
                    ...
                    yield '\n'
        l_1_policy = missing
        for l_1_profile in t_3(environment.getattr((undefined(name='management_security') if l_0_management_security is missing else l_0_management_security), 'shared_secret_profiles'), sort_key='profile'):
            _loop_vars = {}
            pass
            yield '   !\n   session shared-secret profile '
            yield str(environment.getattr(l_1_profile, 'profile'))
            yield '\n'
            for l_2_secret in t_3(environment.getattr(l_1_profile, 'secrets'), sort_key='name'):
        ...
        l_1_profile = missing
        for l_1_ssl_profile in t_3(environment.getattr((undefined(name='management_security') if l_0_management_security is missing else l_0_management_security), 'ssl_profiles'), sort_key='name'):
            _loop_vars = {}
            pass
            yield '   !\n   ssl profile '
