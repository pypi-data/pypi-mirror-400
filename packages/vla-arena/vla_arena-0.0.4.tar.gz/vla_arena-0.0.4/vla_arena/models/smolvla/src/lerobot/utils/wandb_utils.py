...
def get_wandb_run_id_from_filesystem(log_dir: Path) -> str:
    ...
    if len(paths) != 1:
        raise RuntimeError(
            "Couldn't get the previous WandB run ID for run resumption."
        )
...
class WandBLogger:
    ...
    def __init__(self, cfg: TrainPipelineConfig):
        ...
        wandb_run_id = (
            ...
            if cfg.wandb.run_id
                ...
                get_wandb_run_id_from_filesystem(self.log_dir)
                if cfg.resume
                else None
            )
        )
    ...
    def log_dict(
        ...
        if step is None and custom_step_key is None:
            raise ValueError(
                'Either step or custom_step_key must be provided.'
            )
    ...
    def log_video(self, video_path: str, step: int, mode: str = 'train'):
        ...
        wandb_video = self._wandb.Video(
            video_path, fps=self.env_fps, format='mp4'
        )
