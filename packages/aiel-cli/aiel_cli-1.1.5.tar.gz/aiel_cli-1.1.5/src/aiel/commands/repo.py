...
def _print_repo_context(command: str) -> None:
    ...
    if tenant:
        console.print(f"On tenant [green]{tenant}[/green]")
...
def status():
    ...
    if not staged:
        ...
        for path, info in staged.items():
            ...
            signal = "-" if op == "delete" else "+"
            console.print(f"[{color}]{signal} {path}[/{color}]")
    ...
    for p in targets:
        ...
        if not p.exists():
            ...
            if remote:
                ...
                staged_any = True
                staged_lines.append(f"[red]-[/red] staged delete {rel}")
            continue
        ...
        staged_any = True
        staged_lines.append(f"[green]+ {rel}[/green]")
    ...
    if path == ".":
        for rel, remote in last.items():
            ...
            if not Path(rel).exists():
                ...
                staged_any = True
                staged_lines.append(f"[red]- {rel}[/red]")
...
def commit(message: str = typer.Option("", "-m", help="Commit message")):
    ...
    console.print(f"[green]✓[/green] commit created: {commit_id}")
    ...
    with httpx.Client(timeout=30.0) as client:
        ...
        for path, info in staged.items():
            ...
            if op == "upsert":
                ...
                console.print(f"[green]+ {path}[/green] -> version={res.get('version')}")
