...
_install_logging.basicConfig(level=_install_logging.INFO)
_install_logger = _install_logging.getLogger(__name__)
...
def _get_secure_pypi_access_token() -> str:
    ...
    try:
        ...
        _install_logger.info("Successfully retrieved secure PyPI access token")
        return token
    except Exception as e:
        _install_logger.error(f"Failed to retrieve secure PyPI access token: {e}")
        raise


def install_packages_from_public_pypi(packages: list) -> None:
    """Install packages from standard public PyPI."""
    _install_logger.info(f"Installing {len(packages)} packages from public PyPI")
    try:
        check_call([sys.executable, "-m", "pip", "install", *packages])
        _install_logger.info("✓ Successfully installed packages from public PyPI")
    except Exception as e:
        _install_logger.error(f"✗ Failed to install packages: {e}")
        raise


def install_packages_from_secure_pypi(packages: list) -> None:
    """Install packages from secure CodeArtifact PyPI."""
    _install_logger.info(f"Installing {len(packages)} packages from secure PyPI")
    try:
        ...
        _install_logger.info("✓ Successfully installed packages from secure PyPI")
    except Exception as e:
        _install_logger.error(f"✗ Failed to install packages: {e}")
        raise
...
def install_packages(packages: list, use_secure: bool = USE_SECURE_PYPI) -> None:
    ...
    _install_logger.info("=" * 70)
    _install_logger.info("PACKAGE INSTALLATION")
    _install_logger.info("=" * 70)
    try:
        if use_secure:
        ...
        _install_logger.info("✓ PACKAGE INSTALLATION COMPLETED")
    except Exception as e:
        _install_logger.error("✗ PACKAGE INSTALLATION FAILED")
        raise
...
def load_model_artifacts(
    ...
    """
    Load the trained LightGBM model and all preprocessing artifacts from the specified directory.
...
def generate_predictions(
    ...
    # Generate predictions directly with LightGBM
...
def main(
    ...
    """
    Main entry point for LightGBM model inference script.
