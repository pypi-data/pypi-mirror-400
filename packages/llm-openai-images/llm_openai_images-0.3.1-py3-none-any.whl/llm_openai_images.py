...
class ImageOptions(Options):
    ...
    aspect_ratio: Optional[str] = Field(
        description=(
            "Aspect ratio for Google Nano Banana models (e.g., 1:1, 16:9)."
        ),
        default=None,
    )
...
class _BaseGoogleImageModel(KeyModel):
    ...
    def is_pro(self) -> bool:
        return "-pro" in self.model_id

    def _validate_aspect_ratio(self, aspect_ratio: str) -> str:
        valid_set = self.valid_aspects_pro if self.is_pro else self.valid_aspects_flash
        if aspect_ratio not in valid_set:
            supported = ", ".join(sorted(valid_set))
            raise ValueError(
                f"'{aspect_ratio}' is invalid. Supported aspect ratios: {supported}"
            )
        return aspect_ratio
    ...
    def _build_payload(self, prompt: Prompt) -> dict:
        ...
        aspect_ratio = prompt.options.aspect_ratio or "1:1"
