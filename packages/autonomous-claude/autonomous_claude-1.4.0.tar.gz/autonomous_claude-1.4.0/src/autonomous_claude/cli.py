...
from .config import get_config, AUTONOMOUS_CLAUDE_DIR
...
def has_actionable_issues(project_dir: Path) -> tuple[bool, int]:
    """Check if project has issues the agent can work on (excludes 'needs-info'). Returns (has_issues, count)."""
    result = subprocess.run(
    ...
    try:
        ...
        actionable = [i for i in issues if not any(l.get("name") == "needs-info" for l in i.get("labels", []))]
        return len(actionable) > 0, len(actionable)
