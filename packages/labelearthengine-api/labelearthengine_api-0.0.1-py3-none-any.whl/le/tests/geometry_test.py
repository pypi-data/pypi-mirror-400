...
class GeometryTest(apitestcase.ApiTestCase, parameterized.TestCase):
  ...
  def testBBox(self, expected, coords):
    self.assertEqual(
        ...
        le.Geometry.BBox(*coords),
    )

  def testBBox_computedArgs(self):
    ten = le.Number(5).add(5)
    box = le.Geometry.BBox(-10, -20, ten, 20)
    func = le.ApiFunction.lookup('GeometryConstructors.BBox')

    self.assertIsInstance(box, le.Geometry)
  ...
  def testLineString(self):
    ...
    proj = 'EPSG:4326'
    max_error = 1000
    func = le.ApiFunction.lookup('GeometryConstructors.LineString')

    geometry = le.Geometry.LineString(coords, proj, True, max_error)
    self.assertEqual(func, geometry.func)
    self.assertEqual(
        {
            'coordinates': le.List(coords),
            'crs': le.Projection(proj),
            'geodesic': True,
            'maxError': le.ErrorMargin(max_error),
        },
        geometry.args,
    )
  ...
  def testLineString_kwargs(self):
    ...
    geometry = le.Geometry.LineString(coords, geodesic=True)
    self.assertEqual(func, geometry.func)
    self.assertEqual(
        {'coordinates': le.List(coords), 'geodesic': True},
        geometry.args,
    )

  def testLineString_computedArgs(self):
    """Verifies that a LineString with computed inputs can be constructed."""
    p1 = le.Geometry.Point([1, 2])
    p2 = le.Geometry.Point([3, 4])
    line = le.Geometry.LineString([p1, p2])

    self.assertIsInstance(line, le.Geometry)
    self.assertEqual(
        le.ApiFunction.lookup('GeometryConstructors.LineString'), line.func
    )
    self.assertEqual({'coordinates': le.List([p1, p2])}, line.args)
  ...
  def testLinearRing(self):
    ...
    geodesic = True
    max_error = 1000
    func = le.ApiFunction.lookup('GeometryConstructors.LinearRing')

    geometry = le.Geometry.LinearRing(coords, proj, geodesic, max_error)
    self.assertEqual(func, geometry.func)
    self.assertEqual(
        {
            'coordinates': le.List(coords),
  ...
  def testLinearRing_kwargs(self):
    ...
    geometry = le.Geometry.LinearRing(coords, geodesic=geodesic)
    self.assertEqual(func, geometry.func)
    self.assertEqual(
        {'coordinates': le.List(coords), 'geodesic': geodesic},
        geometry.args,
    )
  ...
  def testLinearRing_computedArgs(self):
    ...
    p1 = le.Geometry.Point([1, 2])
    p2 = le.Geometry.Point([3, 4])
    ring = le.Geometry.LinearRing([p1, p2])

    self.assertIsInstance(ring, le.Geometry)
    self.assertEqual(
        le.ApiFunction.lookup('GeometryConstructors.LinearRing'), ring.func
    )
    self.assertEqual({'coordinates': le.List([p1, p2])}, ring.args)
  ...
  def testMultiLineString(self):
    ...
    geodesic = True
    max_error = 1000
    func = le.ApiFunction.lookup('GeometryConstructors.MultiLineString')

    geometry = le.Geometry.MultiLineString(coords, proj, geodesic, max_error)
    self.assertEqual(func, geometry.func)
    self.assertEqual(
        {
            'coordinates': le.List(coords),
  ...
  def testMultiLineString_kwargs(self):
    ...
    geometry = le.Geometry.MultiLineString(coords, geodesic=geodesic)
    self.assertEqual(func, geometry.func)
    self.assertEqual(
        {'coordinates': le.List(coords), 'geodesic': geodesic},
        geometry.args,
  ...
  def testMultiLineString_computedArgs(self):
    ...
    p1 = le.Geometry.Point([1, 2])
    p2 = le.Geometry.Point([3, 4])
    line = le.Geometry.MultiLineString([p1, p2])

    self.assertIsInstance(line, le.Geometry)
    self.assertEqual(
        le.ApiFunction.lookup('GeometryConstructors.MultiLineString'), line.func
    )
    self.assertEqual({'coordinates': le.List([p1, p2])}, line.args)
  ...
  def testMultiLineStringInvalid(self, coords, message):
    ...
    with self.assertRaisesRegex(le.EEException, message):
      le.Geometry.MultiLineString(*coords)

  def testMultiPoint(self):
    coords = [1, 2, 3, 4]
    proj = 'EPSG:4326'
    func = le.ApiFunction.lookup('GeometryConstructors.MultiPoint')

    geometry = le.Geometry.MultiPoint(coords, proj)
    self.assertEqual(func, geometry.func)
    self.assertEqual(
        {
            'coordinates': le.List(coords),
  ...
  def testMultiPoint_kwargs(self):
    ...
    geometry = le.Geometry.MultiPoint(coords, proj=proj)
    self.assertEqual(func, geometry.func)
    self.assertEqual(
        {'coordinates': le.List(coords), 'crs': le.Projection(proj)},
        geometry.args,
    )
  ...
  def testMultiPoint_computedArgs(self):
    ...
    p1 = le.Geometry.Point([1, 2])
    p2 = le.Geometry.Point([3, 4])
    point = le.Geometry.MultiPoint([p1, p2])

    self.assertIsInstance(point, le.Geometry)
    self.assertEqual(
        le.ApiFunction.lookup('GeometryConstructors.MultiPoint'), point.func
    )
    self.assertEqual({'coordinates': le.List([p1, p2])}, point.args)
  ...
  def testMultiPolygon(self):
    ...
    max_error = 1000
    even_odd = True
    func = le.ApiFunction.lookup('GeometryConstructors.MultiPolygon')

    geometry = le.Geometry.MultiPolygon(
  ...
  def testMultiPolygon_kwargs(self):
    ...
    geometry = le.Geometry.MultiPolygon(coords, geodesic=geodesic)
    self.assertEqual(func, geometry.func)
    self.assertEqual(
        {'coordinates': le.List(coords), 'geodesic': geodesic},
        geometry.args,
  ...
  def testMultiPolygon_computedArgs(self):
    ...
    p1 = le.Geometry.Point([1, 2])
    p2 = le.Geometry.Point([3, 4])
    line = le.Geometry.MultiPolygon([p1, p2])

    self.assertIsInstance(line, le.Geometry)
    self.assertEqual(
        le.ApiFunction.lookup('GeometryConstructors.MultiPolygon'), line.func
    )
    self.assertEqual({'coordinates': le.List([p1, p2])}, line.args)
  ...
  def testMultiPolygonInvalid(self, coords, message):
    ...
    with self.assertRaisesRegex(le.EEException, message):
      le.Geometry.MultiPolygon(*coords)

  def testPoint(self):
    coords = [1, 2, 3, 4]
    proj = 'EPSG:4326'
    func = le.ApiFunction.lookup('GeometryConstructors.Point')

    geometry = le.Geometry.Point(coords, proj)
    self.assertEqual(func, geometry.func)
    self.assertEqual(
        {
            'coordinates': le.List(coords),
  ...
  def testPoint_kwargs(self):
    ...
    geometry = le.Geometry.Point(coords, proj=proj)
    self.assertEqual(func, geometry.func)
    self.assertEqual(
        {'coordinates': le.List(coords), 'crs': le.Projection(proj)},
        geometry.args,
  ...
  def testPoint_oldKeywordArgs(self):
    ...
    self.assertEqual(le.Geometry.Point(1, 2), le.Geometry.Point(lon=1, lat=2))
    self.assertEqual(le.Geometry.Point(1, 2), le.Geometry.Point(1, lat=2))

  def testPoint_computedArgs(self):
    """Verifies that a Point with computed inputs can be constructed."""
    v1 = le.Number(1)
    v2 = le.Number(2)
    point = le.Geometry.Point([v1, v2])

    self.assertIsInstance(point, le.Geometry)
    self.assertEqual(
        le.ApiFunction.lookup('GeometryConstructors.Point'), point.func
    )
    self.assertEqual({'coordinates': le.List([v1, v2])}, point.args)
  ...
  def testPolygon(self):
    ...
    max_error = 1000
    even_odd = True
    func = le.ApiFunction.lookup('GeometryConstructors.Polygon')

    geometry = le.Geometry.Polygon(coords, proj, geodesic, max_error, even_odd)
    self.assertEqual(func, geometry.func)
    self.assertEqual(
        {
            'coordinates': le.List(coords),
  ...
  def testPolygon_kwargs(self):
    ...
    geometry = le.Geometry.Polygon(coords, geodesic=geodesic)
    self.assertEqual(func, geometry.func)
    self.assertEqual(
        {'coordinates': le.List(coords), 'geodesic': geodesic},
        geometry.args,
  ...
  def testPolygon_computedArgs(self):
    ...
    p1 = le.Geometry.Point([1, 2])
    p2 = le.Geometry.Point([3, 4])
    line = le.Geometry.Polygon([p1, p2])

    self.assertIsInstance(line, le.Geometry)
    self.assertEqual(
        le.ApiFunction.lookup('GeometryConstructors.Polygon'), line.func
    )
    self.assertEqual({'coordinates': le.List([p1, p2])}, line.args)

  def testPolygon_evenOdd(self):
    poly1 = le.Geometry.Polygon([0, 0, 0, 5, 5, 0])
    self.assertTrue(poly1.toGeoJSON()['evenOdd'])

    # TODO: Use kwargs instead of positional args for evenOdd.
    poly2 = le.Geometry.Polygon([0, 0, 0, 5, 5, 0], None, None, None, False)
  ...
  def testRectangle(self):
    ...
    geodesic = True
    even_odd = True
    func = le.ApiFunction.lookup('GeometryConstructors.Rectangle')

    geometry = le.Geometry.Rectangle(coords, proj, geodesic, even_odd)
    self.assertEqual(func, geometry.func)
    self.assertEqual(
        {
            'coordinates': le.List(coords),
  ...
  def testRectangle_kwargs(self):
    ...
    geometry = le.Geometry.Rectangle(coords, geodesic=geodesic)
    self.assertEqual(func, geometry.func)
    self.assertEqual(
        {'coordinates': le.List(coords), 'geodesic': geodesic},
        geometry.args,
  ...
  def testRectangle_oldKeywordArgs(self):
    ...
    self.assertEqual(
        le.Geometry.Rectangle(1, 2, 3, 4),
        le.Geometry.Rectangle(xlo=1, ylo=2, xhi=3, yhi=4),
    )
    self.assertEqual(
        le.Geometry.Rectangle(1, 2, 3, 4),
        le.Geometry.Rectangle(1, 2, xhi=3, yhi=4),
    )
  ...
  def testRectangle_computedArgs(self):
    ...
    p1 = le.Geometry.Point([1, 2])
    p2 = le.Geometry.Point([3, 4])
    line = le.Geometry.Rectangle([p1, p2])

    self.assertIsInstance(line, le.Geometry)
    self.assertEqual(
        le.ApiFunction.lookup('GeometryConstructors.Rectangle'), line.func
    )
    self.assertEqual({'coordinates': le.List([p1, p2])}, line.args)
  ...
  def testValid_GeometryCollection(self):
    ...
    self.assertIsInstance(geometry, le.Geometry)
  ...
  def testArrayConstructors(self):
    ...
    multipoint = le.Geometry.MultiPoint([[1, 2], [3, 4], [5, 6]])
    self.assertEqual(3, get_coordinates_count(multipoint))

    line = le.Geometry.LineString([[1, 2], [3, 4], [5, 6]])
    self.assertEqual(3, get_coordinates_count(line))

    ring = le.Geometry.LinearRing([[1, 2], [3, 4], [5, 6]])
    self.assertEqual(3, get_coordinates_count(ring))

    multiline = le.Geometry.MultiLineString(
        [[[1, 2], [3, 4]], [[5, 6], [7, 8]]]
    )
    ...
    mpolygon = le.Geometry.MultiPolygon(
  ...
  def testGeodesicFlag(self):
    ...
    geodesic = le.Geometry({
        'type': 'LineString',
        'coordinates': [[1, 2], [3, 4]],
        'geodesic': True,
    })
    projected = le.Geometry({
        'type': 'LineString',
        'coordinates': [[1, 2], [3, 4]],
        'geodesic': False,
    })
  ...
  def testConstructor(self):
    ...
    """
    line = le.Geometry.LineString(1, 2, 3, 4)

    # GeoJSON.
    from_json = le.Geometry(line.toGeoJSON())
    ...
    from_json_with_crs = le.Geometry(json_with_crs)
    ...
    self.assertEqual(le.Geometry(line), line)

    # A not-computed geometry with an override.
    with_override = le.Geometry(line, 'SR-ORG:6974')
    self.assertEqual(with_override._proj, 'SR-ORG:6974')

    # A computed geometry.
    self.assertEqual(le.Geometry(line.bounds()), line.bounds())

    # Something to cast to a geometry.
    computed = le.ComputedObject(le.Function(), {'a': 1})
    geom = le.Geometry(computed)
  ...
  def testComputedGeometries(self):
    ...
    self.assertIsInstance(bounds, le.Geometry)
    self.assertEqual(le.ApiFunction.lookup('Geometry.bounds'), bounds.func)
  ...
  def testComputedCoordinate(self):
    ...
    coords = [1, le.Number(1).add(1)]
    p = le.Geometry.Point(coords)

    self.assertIsInstance(p, le.Geometry)
    self.assertEqual(
        le.ApiFunction.lookup('GeometryConstructors.Point'), p.func
    )
    self.assertEqual({'coordinates': le.List(coords)}, p.args)

  def testComputedList(self):
    """Verifies that a computed coordinate produces a computed geometry."""
    lst = le.List([1, 2, 3, 4]).slice(0, 2)
    p = le.Geometry.Point(lst)
  ...
  def testComputedProjection(self):
    ...
    p = le.Geometry.Point([1, 2], 'epsg:4326')
    ...
    expected_args = {
        'coordinates': le.List([1, 2]),
        'crs': le.ApiFunction.lookup('Projection').call('epsg:4326'),
    }
    self.assertEqual(expected_args, p.args)

  def testInvalidCrs(self):
    geo_json = le.Geometry.LineString(1, 2, 3, 4).toGeoJSON()
    geo_json['crs'] = {'something': 'invalid-crs'}
    with self.assertRaisesRegex(le.EEException, 'Invalid CRS declaration'):
      le.Geometry(geo_json)

  def testInternals(self):
    """Test eq(), ne() and hash()."""
    a = le.Geometry.Point(1, 2)
    b = le.Geometry.Point(2, 1)
    c = le.Geometry.Point(1, 2)
  ...
  @parameterized.named_parameters(
      (
          'BBox',
          lambda: le.Geometry.BBox(1, 2, 3, 4),
          dict(
      ...
      (
          'LineString',
          lambda: le.Geometry.LineString(1, 2, 3, 4),
          dict(coordinates=[[1, 2], [3, 4]], type='LineString'),
      ),
      (
          'LinearRing',
          lambda: le.Geometry.LinearRing(1, 2, 3, 4),
          dict(coordinates=[[1, 2], [3, 4]], type='LinearRing'),
      ),
      (
          'MultiLineString',
          lambda: le.Geometry.MultiLineString(1, 2, 3, 4),
          dict(coordinates=[[[1, 2], [3, 4]]], type='MultiLineString'),
      ),
      (
          'MultiLineString_Empty',
          lambda: le.Geometry.MultiLineString(),  # pylint: disable=unnecessary-lambda
          dict(coordinates=[], type='MultiLineString'),
      ),
      (
          'MultiPoint',
          lambda: le.Geometry.MultiPoint(1, 2, 3, 4),
          dict(coordinates=[[1, 2], [3, 4]], type='MultiPoint'),
      ),
      (
          'MultiPoint_Empty',
          lambda: le.Geometry.MultiPoint(),  # pylint: disable=unnecessary-lambda
          dict(coordinates=[], type='MultiPoint'),
      ),
      (
          'MultiPolygon',
          lambda: le.Geometry.MultiPolygon(1, 2, 3, 4),
          dict(
              coordinates=[[[[1, 2], [3, 4]]]],
              evenOdd=True,
              type='MultiPolygon',
      ...
      (
          'MultiPolygon_Empty',
          lambda: le.Geometry.MultiPolygon(),  # pylint: disable=unnecessary-lambda
          dict(coordinates=[], evenOdd=True, type='MultiPolygon'),
      ),
      (
          'Point',
          lambda: le.Geometry.Point(coords=[1, 2]),
          dict(coordinates=[1, 2], type='Point'),
      ),
      (
          'Polygon',
          lambda: le.Geometry.Polygon(1, 2, 3, 4),
          dict(
              coordinates=[[[1, 2], [3, 4]]],
              evenOdd=True,
              type='Polygon',
      ...
      (
          'Rectangle',
          lambda: le.Geometry.Rectangle(1, 2, 3, 4),
          dict(
              coordinates=[[[1, 4], [1, 2], [3, 2], [3, 4]]],
              evenOdd=True,
              type='Polygon',
  ...
  @parameterized.named_parameters(
      ...
      ('LineString', lambda: le.Geometry.LineString([], 'EPSG:4326')),
      ('LinearRing', lambda: le.Geometry.LinearRing([], 'EPSG:4326')),
      ('MultiLineString', lambda: le.Geometry.MultiLineString([], 'EPSG:4326')),
      ('MultiPoint', lambda: le.Geometry.MultiPoint([], 'EPSG:4326')),
      ('MultiPolygon', lambda: le.Geometry.MultiPolygon([], 'EPSG:4326')),
      ('Point', lambda: le.Geometry.Point([], 'EPSG:4326')),
      ('Polygon', lambda: le.Geometry.Polygon([], 'EPSG:4326')),
      ('Rectangle', lambda: le.Geometry.Rectangle([], 'EPSG:4326')),
  )
