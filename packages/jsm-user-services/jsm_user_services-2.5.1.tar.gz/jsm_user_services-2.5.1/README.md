...
We have a set of permissions for Django REST Framework (DRF) that can be used to protect views. These permissions can be also protected by `jsm_user_services.drf.permissions.protected` decorator.

Example:

```python
from jsm_user_services.drf.authentications import LvJWTAuthentication
from jsm_user_services.drf.permissions import protected
from jsm_user_services.drf.permissions.lv import LVClientBuyerPermission

@authentication_classes((LvJWTAuthentication,))
@permission_classes((protected(LVClientBuyerPermission),))
def my_view(request: Request):
    pass
```

`protected` decorator can receive a couple of arguments:
- `keys`: set of keys to be protected, this will be injected to the request as `request.protected_keys` to the be used by the user. (Default: None)
- `should_return_forbidden`: if True, the permission will return a 403 Forbidden response if "fid" is present in the JWT token. (Default: False)
