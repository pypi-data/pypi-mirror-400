...
def _hide_thinking_blocks(content: str) -> str:
    """Collapse <think>...</think> blocks to a dimmed indicator."""
    return re.sub(
        r"<think>.*?</think>\s*",
...
def _hide_tool_invocations(content: str) -> str:
    ...
    def replace_file_tool(match):
        ...
        action = action_map.get(tool_name, "Processed")
        return f"\n> *{action} {path}*\n"
...
def _truncate_shell_output(content: str, max_lines: int = 20) -> str:
    """Truncate stdout/stderr blocks for display while model keeps full content."""
...
def format_msgs(
    ...
    for msg in msgs:
        ...
        # Get content, applying role-specific transformations for display
        content = msg.content
        if msg.role == "assistant":
            content = _hide_thinking_blocks(content)
        elif msg.role == "system":
            # Truncate shell output for display (model still has full content)
            content = _truncate_shell_output(content)

        if oneline:
            content_oneline = content.replace("\n", "\\n")
            if highlight:
                content_oneline = escape_markup(content_oneline)
            output += textwrap.shorten(
                content_oneline, width=max_len, placeholder="..."
            )
            if len(output) < 20:
                output = content_oneline[:max_len] + "..."
        else:
            # Use rich Markdown rendering for assistant messages
            if msg.role == "assistant" and highlight:
                output += "\n"
                try:
                    md = Markdown(content)
                    output += rich_to_str(md)
                except Exception:
                    # Fallback to plain text if markdown parsing fails
                    output += content
            else:
                multiline = len(content.split("\n")) > 1
                ...
                for i, block in enumerate(content.split("```")):
                    if i % 2 == 0:
                        ...
                        block = f"[{fmt}]{lang}\n[/{fmt}]" + rich_to_str(
                            Syntax(
                                block_content.rstrip().replace("[", r"\["),
                                lang,
                            )
                        )
...
def print_msg(
    ...
    for m in msgs:
        if m.hide and not show_hidden:
            ...
            continue
        try:
            # Use Panel for assistant messages to match streaming output
            if m.role == "assistant" and highlight and not oneline:
                content = _hide_thinking_blocks(m.content)

                # Check if this is a tool output (has call_id) vs LLM response
                if m.call_id:
                    # Tool output - blue box, indented
                    try:
                        md = Markdown(content)
                        panel = Panel(
                            md,
                        ...
                        console.print(f"[blue]Tool Output:[/blue] {content}")
                else:
                    # LLM response - green box, hide tool invocations
                    content = _hide_tool_invocations(content)
                    content = content.strip()
                    if not content:
                        # Skip empty messages (only had tool invocations)
                        continue
                    try:
                        md = Markdown(content)
                        panel = Panel(
                ...
                # Use standard formatting for other roles
                msgstrs = format_msgs([m], highlight=highlight, oneline=oneline)
                console.print(msgstrs[0])
        except Exception:
            # rich can throw errors, if so then print the raw message
            logger.exception("Error printing message")
            msgstrs = format_msgs([m], highlight=False, oneline=oneline)
            print(msgstrs[0])
    if skipped_hidden:
        console.print(
