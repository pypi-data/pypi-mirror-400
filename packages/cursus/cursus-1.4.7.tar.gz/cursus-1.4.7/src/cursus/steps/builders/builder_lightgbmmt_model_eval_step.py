...
from ..configs.config_lightgbmmt_model_eval_step import LightGBMMTModelEvalConfig
from ...core.base.builder_base import StepBuilderBase
from ...core.deps.registry_manager import RegistryManager
from ...core.deps.dependency_resolver import UnifiedDependencyResolver

# Import the multi-task model evaluation specification
try:
    from ..specs.lightgbmmt_model_eval_spec import LIGHTGBMMT_MODEL_EVAL_SPEC

    SPEC_AVAILABLE = True
except ImportError:
    LIGHTGBMMT_MODEL_EVAL_SPEC = None
    SPEC_AVAILABLE = False

logger = logging.getLogger(__name__)


class LightGBMMTModelEvalStepBuilder(StepBuilderBase):
    """
    Builder for a LightGBMMT Multi-Task Model Evaluation ProcessingStep.

    This implementation uses the specification-driven approach where dependencies, outputs,
    and script contract are defined in the multi-task model evaluation specification.
    Uses FrameworkProcessor with PyTorch container and LightGBM installed via pip.
    """

    def __init__(
        self,
        config: LightGBMMTModelEvalConfig,
        ...
        """
        Initializes the builder with a specific configuration for the multi-task model evaluation step.

        Args:
            config: A LightGBMMTModelEvalConfig instance containing all necessary settings.
            sagemaker_session: The SageMaker session object to manage interactions with AWS.
            role: The IAM role ARN to be used by the SageMaker Processing Job.
            registry_manager: Optional registry manager for dependency injection
            dependency_resolver: Optional dependency resolver for dependency injection
        """
        if not isinstance(config, LightGBMMTModelEvalConfig):
            raise ValueError(
                "LightGBMMTModelEvalStepBuilder requires a LightGBMMTModelEvalConfig instance."
            )

        # Use the multi-task model evaluation specification if available
        spec = LIGHTGBMMT_MODEL_EVAL_SPEC if SPEC_AVAILABLE else None
        ...
        self.config: LightGBMMTModelEvalConfig = config
    ...
    def validate_configuration(self) -> None:
        ...
        """
        self.log_info("Validating LightGBMMTModelEvalConfig...")
        ...
        required_attrs = [
            ...
            "job_type",
            "id_name",
            "task_label_names",  # Multi-task: list of labels
            "framework_version",
        ]
        ...
        for attr in required_attrs:
            if not hasattr(self.config, attr) or getattr(self.config, attr) in [
                ...
                raise ValueError(
                    f"LightGBMMTModelEvalConfig missing required attribute: {attr}"
                )

        # Validate task_label_names is a non-empty list
        if (
            not isinstance(self.config.task_label_names, list)
            or len(self.config.task_label_names) == 0
        ):
            raise ValueError(
                "task_label_names must be a non-empty list for multi-task evaluation"
            )

        # Validate minimum number of tasks
        if len(self.config.task_label_names) < 2:
            raise ValueError(
                f"task_label_names must contain at least 2 tasks, got {len(self.config.task_label_names)}"
            )
        ...
        self.log_info("LightGBMMTModelEvalConfig validation succeeded.")
    ...
    def _get_environment_variables(self) -> Dict[str, str]:
        ...
        which handles all multi-task evaluation-specific variables.
        ...
        self.log_info("Multi-task evaluation environment variables: %s", env_vars)
        return env_vars
    ...
    def _get_outputs(self, outputs: Dict[str, Any]) -> List[ProcessingOutput]:
        ...
        for _, output_spec in self.spec.outputs.items():
            ...
            if logical_name in outputs:
                ...
                destination = Join(
                    on="/", values=[base_output_path, "model_evaluation", logical_name]
                )
                self.log_info(
    ...
    def _get_job_arguments(self) -> List[str]:
        ...
        # For multi-task model evaluation, we always return just the job_type argument
    ...
    def create_step(self, **kwargs) -> ProcessingStep:
        ...
        """
        self.log_info("Creating LightGBMMTModelEval ProcessingStep...")
