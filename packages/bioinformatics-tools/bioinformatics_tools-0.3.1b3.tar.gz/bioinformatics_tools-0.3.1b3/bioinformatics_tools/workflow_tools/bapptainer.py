#!/usr/bin/env python3
"""
Minimal example for running Apptainer .sif files with arguments.

Usage:
    python main.py <sif_file> [args...]

Example:
    python main.py python-basic.sif "World"
"""
import hashlib
import logging
import shutil
...
def run_apptainer_container(app_obj: ApptainerKey, container_args: list[str]) -> int:
    ...
    if cached := get_cached_file(sif_path):
        verified_sif_file = cached
    else:
        dest = CACHE_DIR / sif_path
        print(f'Did not find in cache...Downloading to {dest}')
        verified_sif_file = download_container_with_progress(url=sif_path, dest=dest)
...
#     print(f'Len of arguments: {len(sys.argv)}')
#     if len(sys.argv) < 2:
#         print(__doc__)
#         sys.exit(1)
...
#     # Run the container
#     exit_code = run_apptainer_container(sif_path, container_args)

#     if exit_code == 0:
#         LOGGER.info("Container execution completed successfully")
#     else:
#         LOGGER.error("Container execution failed with exit code %s", exit_code)
