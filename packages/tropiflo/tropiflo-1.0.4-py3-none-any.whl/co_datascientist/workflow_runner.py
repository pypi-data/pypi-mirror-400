...
def copy_to_tmp(code_base_directory):
    ...
    Ignores 'current_runs' and 'co_datascientist_checkpoints' directories.
...
class WorkflowRunner:
    ...
    def compile_docker_image(self, dockerfile_path: str, is_base_image: bool = False):
        ...
        Shows real-time Docker build output to user.
        ...
        if is_base_image:
            print(f"\n🏗️  Building base image with dependencies: {image_tag}")
            print("   (This is slow but only happens once - all hypotheses will reuse this!)")
        else:
            print(f"\n⚡ Building hypothesis image: {image_tag}")
            print("   (Fast build - only copying code changes)")
        print("-" * 70)
        
        try:
            # Stream output to user in real-time (no capture_output)
            ...
            print("-" * 70)
            if is_base_image:
                print(f"✅ Base image built: {image_tag}")
                print("   All future hypotheses will reuse this image (5-10s builds!)\n")
            else:
                print(f"✅ Hypothesis image built: {image_tag}\n")
    ...
    async def run_workflow(self, working_directory: str, config: dict, spinner=None):
        ...
        while (not self.workflow.finished and not self.should_stop_workflow):
            ...
            # PHASE 1: Build all Docker images (shows build output)
            print(f"\n🏗️  Building {len(code_versions)} Docker images...")
            print("-" * 70)
            ...
            # PHASE 2: Execute with elegant progress tracking
    ...
    async def _save_population_best_checkpoint(self, best_cv, best_kpi: float, project_absolute_path: str, config: dict):
        ...
        try:
            ...
            if is_databricks:
                ...
                # NEW: Generate audit report if enabled (can be easily removed)
                if config.get('generate_audit', False):
                    try:
                        audit_markdown = await co_datascientist_api.generate_audit(
                            self.workflow.workflow_id
                        )
                        
                        if audit_markdown:
                            # Save audit in the checkpoint directory
                            audit_path = checkpoint_dir / f"MODEL_AUDIT_{self._checkpoint_counter}.md"
                            audit_path.write_text(audit_markdown, encoding="utf-8")
                            print(f"Professional audit report saved: {audit_path.name}")
                    except Exception as e:
                        logging.warning(f"Failed to save audit report: {e}")
