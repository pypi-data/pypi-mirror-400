...
def generate_primitive_structure(random_gen, primitive_templates):
    """
    ...
    """
    list_of_tuples = []
    random_template = random_gen.choice(primitive_templates)
...
def find_policy(policy_db, policy_class):
    """ Find the self from the list of policies based on the self class """
    found_policy = None
...
def unk_from_desc(random_gen, item):
...
def make_dynamic_copy(pol):
    """ Makes a new instance of the self class and assigns the same class member references
        as the old one.
    """
    attributes = vars(pol)
...
def generate_property_val(dia_generator, candidate_keys):
    ...
    random_key = dia_generator.random_gen.choice(candidate_keys)
    if random_key == 'location':
        random_val = [dia_generator.random_gen.choice(dia_generator.world.location_positions),
                      dia_generator.random_gen.choice(dia_generator.world.obj_list)]
        ...
        random_val = dia_generator.random_gen.choice(dia_generator.world.obj_list)
    elif random_key in dia_generator.world.directions:
        random_val = dia_generator.random_gen.choice(dia_generator.world.places)
    else:
        random_val = dia_generator.random_gen.choice(dia_generator.world.get_property_values(candidate_keys))

    return random_val


def configure_logging_old(filename, level=None, format_=None, force_=True):
    ...
    logging.basicConfig(filename=filename, level=level,
                        format=format_, force=force_
                        )


def configure_logging(filename, level=None, format_=None):
    """Configures logging to output errors only to a file, not the Colab screen."""
    if level is None:
        level = logging.DEBUG
    if format_ is None:
        format_ = '%(asctime)s - %(name)s - %(levelname)s - %(message)s'
