...
from ..configs.config_lightgbm_model_eval_step import LightGBMModelEvalConfig
...
try:
    from ..specs.lightgbm_model_eval_spec import LIGHTGBM_MODEL_EVAL_SPEC

    SPEC_AVAILABLE = True
except ImportError:
    LIGHTGBM_MODEL_EVAL_SPEC = None
    SPEC_AVAILABLE = False

logger = logging.getLogger(__name__)


class LightGBMModelEvalStepBuilder(StepBuilderBase):
    """
    Builder for a LightGBM Model Evaluation ProcessingStep.
    ...
    def __init__(
        self,
        config: LightGBMModelEvalConfig,
        ...
        Args:
            config: A LightGBMModelEvalConfig instance containing all necessary settings.
            sagemaker_session: The SageMaker session object to manage interactions with AWS.
            role: The IAM role ARN to be used by the SageMaker Processing Job.
            registry_manager: Optional registry manager for dependency injection
            dependency_resolver: Optional dependency resolver for dependency injection
        """
        if not isinstance(config, LightGBMModelEvalConfig):
            raise ValueError(
                "LightGBMModelEvalStepBuilder requires a LightGBMModelEvalConfig instance."
            )

        # Use the model evaluation specification if available
        spec = LIGHTGBM_MODEL_EVAL_SPEC if SPEC_AVAILABLE else None
        ...
        self.config: LightGBMModelEvalConfig = config
    ...
    def validate_configuration(self) -> None:
        ...
        """
        self.log_info("Validating LightGBMModelEvalConfig...")
        ...
        for attr in required_attrs:
            if not hasattr(self.config, attr) or getattr(self.config, attr) in [
                ...
                raise ValueError(
                    f"LightGBMModelEvalConfig missing required attribute: {attr}"
                )
        ...
        self.log_info("LightGBMModelEvalConfig validation succeeded.")
    ...
    def _get_environment_variables(self) -> Dict[str, str]:
        ...
        which handles all model evaluation-specific variables.
    ...
    def create_step(self, **kwargs) -> ProcessingStep:
        ...
        """
        self.log_info("Creating LightGBMModelEval ProcessingStep...")
