...
def test_new_dry_run(tmp_path):
    ...
    result = runner.invoke(cli, ['new', str(proj), '--templates', 'python-cli', '--dry-run'])
    assert result.exit_code == 0
...
def test_add_templates_dry_run(tmp_path):
    ...
    proj.mkdir()
    result = runner.invoke(cli, ['add-templates', str(proj), '--templates', 'python-cli', '--dry-run'])
    assert result.exit_code == 0
    assert 'would-render' in result.output or 'would-copy' in result.output
    assert not (proj / 'README.md').exists()
...
def test_remove_template_cli_dry_run(tmp_path):
    ...
    result = runner.invoke(cli, ['remove-template', str(proj), 'python-cli', '--dry-run'])
    assert result.exit_code == 0
