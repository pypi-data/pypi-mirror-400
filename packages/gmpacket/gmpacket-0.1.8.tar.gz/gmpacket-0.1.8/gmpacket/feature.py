...
class Metric(BaseModel):
    ...
    std_deviations: Optional[
        Union[List[List[Optional[float]]], List[Optional[float]], Optional[float]]
    ] = None
    ...
    def check_dimensions(cls, values):
        if "dimensions" in values and values["dimensions"] is not None:
            ...
            if "std_deviation" in values:
                assert ndims == np.array(values["std_deviation"]).ndim
        return values
