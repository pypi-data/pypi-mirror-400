...
def textFont(font, size=None):
    """
    Activate a font for text rendering.
    font: PFont | str | pygame.font.Font
    """
    if isinstance(font, PFont):
        _P._font_source = font.source
        _P._font_size = size if size is not None else font.size
        ...
        raise TypeError(
            "textFont(): expected PFont, font name, or pygame.font.Font"
        )


def createFont(name, size=16, smooth=True):
    """
    Processing-compatible createFont().
    name: font name or path to .ttf/.otf
    size: default size
    smooth: ignored (pygame always AA-capable)
    """
...
def loadFont(path):
    """
    Processing-compatible loadFont().
    In pycessingame this loads a .ttf/.otf font file.
    """
...
#    leading = _P._text_leading , font.get_height()
    ...
    #        surf.blit(text_surf, (0,0))
        ...
        rect = surf.get_rect()
        rect.center = (px, py)
    #fixme        _P.surface.blit(surf, rect.topleft)
...
def pushStyle():
    _P._style_stack.append(_copy_style())
...
def popStyle():
    ...
    _P.rect_mode     = s["rect_mode"]
    _P.ellipse_mode  = s["ellipse_mode"]
...
def loadImage(path):
    ...
    Supported formats: png, jpg, bmp, gif (pygame)
    """
    try:
...
def _copy_style():
    return {
        ...
        "rect_mode": _P.rect_mode,
        "ellipse_mode": _P.ellipse_mode,
