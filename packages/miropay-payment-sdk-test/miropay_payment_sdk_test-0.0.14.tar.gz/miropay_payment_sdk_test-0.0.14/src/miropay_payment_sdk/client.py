...
class PaymentClient:
    ...
    def __init__(self, api_key: str, api_secret: str, base_url: Optional[str] = None, passphrase: Optional[str] = None) -> None:
        # `api_key` is the key id, `api_secret` is the PEM, passphrase is optional for encrypted PEMs
        self.authenticator = Auth(api_secret, api_key, passphrase=passphrase)
    ...
    # ------------------------------- Basic Methods ------------------------------ #
    async def _call(self, path: str, method: str, request_body: Optional[Union[str, Any]] = None) -> Dict[str, Any]:
        ...
        signature = self.authenticator.make_signature(method.upper(), relative_url)

        headers = {
            "x-signature": signature,
            "x-id": self.authenticator.key_id,
            "Content-Type": "application/json",
        }

        try:
            if request_body is None:
                resp = await self.http_client.request(method.upper(), versioned_url, headers=headers)
            else:
                # request_body is expected to be a JSON string or bytes
                resp = await self.http_client.request(method.upper(), versioned_url, content=request_body, headers=headers)
            ...
            return {"body": json_body, "headers": dict(resp.headers), "status_code": resp.status_code}
        except Exception:
            raise
    ...
    async def get_public_keys(self) -> Dict[str, Any]:
        return await self._call("/get-public-keys", "GET", None)

    # ======================== Public Methods =======================
    async def get_payment_by_id(self, reference_code: str) -> IPaymentDetailsResponse:
        resp = await self._call(f"/status/{reference_code}", "GET", None)
    ...
    async def create_payment(self, payload: ICreatePayment) -> ICreatePaymentResponse:
        resp = await self._call("/create", "POST", payload.__dict__)
    ...
    async def cancel_payment(self, reference_code: str) -> ICancelPaymentResponse:
        resp = await self._call(f"/cancel/{reference_code}", "PATCH", None)
    ...
    async def verify(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(content, dict):
            # If content is a dict, try to extract the actual JWS string
            # Common field names: 'jws', 'token', 'jwt', 'signed'
            for k in ("jws", "token", "jwt", "signed"):
                if k in content and isinstance(content[k], str):
                    content = content[k]
                    break
            else:
                raise RuntimeError("Unexpected type for 'content', dict with no JWS string field")
        if not isinstance(content, str) or not content:
            raise RuntimeError("Internal server error: empty or invalid content")
