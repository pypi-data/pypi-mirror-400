"""
Tests for minmo.gemini_cli_wrapper module (CLI subprocess version).
...
class TestGeminiCLIWrapper:
    """Tests for GeminiCLIWrapper class (CLI subprocess version)."""
    ...
    @pytest.fixture
    def mock_cli_exists(self):
        """Mock CLI existence check."""
        with patch("minmo.gemini_cli_wrapper.shutil.which") as mock_which:
            mock_which.return_value = "/usr/bin/gemini"
            yield mock_which

    @pytest.fixture
    def wrapper(self, mock_scribe, mock_subprocess, mock_cli_exists):
    ...
    def test_init_default_values(self, mock_scribe, mock_subprocess, mock_cli_exists):
        wrapper = GeminiCLIWrapper()
    ...
    def test_init_custom_values(self, mock_scribe, mock_subprocess, mock_cli_exists):
        wrapper = GeminiCLIWrapper(
    ...
    def test_find_gemini_cli_with_env_var(self, mock_scribe, mock_subprocess):
        """Test finding gemini CLI via environment variable."""
    ...
    def test_find_gemini_cli_with_which(self, mock_scribe, mock_subprocess, mock_cli_exists):
        """Test finding gemini CLI via shutil.which."""
        wrapper = GeminiCLIWrapper()
        assert wrapper.gemini_path == "/usr/bin/gemini"
        wrapper.close()

    def test_find_gemini_cli_fallback(self, mock_scribe, mock_subprocess):
        """Test fallback when gemini CLI is not found."""
    ...
    def test_check_login_status_success(self, wrapper, mock_subprocess):
        ...
        mock_subprocess["result"].stdout = "Login successful"
        mock_subprocess["result"].returncode = 0
    ...
    def test_check_login_status_failure(self, wrapper, mock_subprocess):
        ...
        mock_subprocess["run"].side_effect = subprocess.TimeoutExpired("gemini", 5)
        result = wrapper.check_login_status()
...
'''
    ...
    def test_parse_json_response_invalid_json(self, wrapper):
        """Test parsing invalid JSON returns raw_response."""
...
```'''
        ...
        with patch.object(wrapper, '_collect_project_structure', return_value={"files": ["main.py"]}):
...
```'''
    ...
    def test_context_manager(self, mock_scribe, mock_subprocess, mock_cli_exists):
        """Test wrapper works as context manager."""
        with GeminiCLIWrapper() as wrapper:
            assert wrapper is not None
        # close() should be called automatically

    def test_log_callback(self, mock_scribe, mock_subprocess, mock_cli_exists):
        ...
        with patch.object(wrapper, '_collect_project_structure', return_value={"files": []}):
...
```'''
    ...
    def test_run_gemini_success(self, wrapper, mock_subprocess):
        ...
        mock_subprocess["result"].stdout = "Hello from Gemini"
        mock_subprocess["result"].returncode = 0
    ...
    def test_run_gemini_timeout(self, wrapper, mock_subprocess):
        """Test gemini CLI timeout handling returns empty string."""
        mock_subprocess["run"].side_effect = subprocess.TimeoutExpired("gemini", 120)

        result = wrapper._run_gemini("Test prompt")
        # Timeout returns empty string
        assert result == ""

    def test_run_gemini_error(self, wrapper, mock_subprocess):
        """Test gemini CLI error handling returns empty string."""
        mock_subprocess["result"].stderr = "Error: something went wrong"
        mock_subprocess["result"].returncode = 1
        mock_subprocess["result"].stdout = ""
