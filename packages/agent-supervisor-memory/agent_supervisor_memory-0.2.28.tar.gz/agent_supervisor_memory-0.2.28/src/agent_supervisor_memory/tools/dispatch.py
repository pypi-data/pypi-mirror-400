...
def _wait_for_job(
    ...
    *,
    ...
    while True:
        ...
        if not res.get("found"):
            return _format_wait_result(res, job_id, verbosity, state_override="not_found", summary=summary_lines, summary_max_items=summary_max_items)

        state = (res.get("meta") or {}).get("status")
        now = time.time()
        if call_deadline and now >= call_deadline:
            return _format_wait_result(res, job_id, verbosity, state_override="running", summary=summary_lines, summary_max_items=summary_max_items)
        if deadline and now >= deadline:
            return _format_wait_result(res, job_id, verbosity, state_override="timeout", summary=summary_lines, summary_max_items=summary_max_items)
        ...
        if state and state != "running":
            if not include_last_message_while_running:
                ...
                summary_lines = _refresh_summary(job_id, dispatch, store_max_items=30)
            return _format_wait_result(res, job_id, verbosity, summary=summary_lines, summary_max_items=summary_max_items)
    ...
    *,
    ...
    def codex_dispatch_wait_all(
        *,
        ...
        has_problems = bool(failed_job_ids or timeout_job_ids or not_found_job_ids)
        ...
        payload: dict[str, Any] = {"state": overall_state, "jobs": results}
        if include_counts_requested or has_problems:
            payload["counts"] = counts
        if include_problem_job_ids:
            if failed_job_ids:
