...
def add_commitment(
    ...
    # Try patterns first (hacky import fix: we're in same module)
...
def check_challenge_progress(brain_path: Path) -> Dict:
    ...
    # Simple logic for now - can expect expansions
    progress = 0
    completed = False
    
    # Logic depends on challenge type (simplified for MVP)
    # Ideally logic would check closed items history etc.
    # For MVP we'll just return the challenge to be tracked manually or via simple heuristics
