...
def subprocess_editor(filename, editor=None):
    """Launches the editor using the specified filename and editor override."""
    if not filename:
        return

    command_str = discover_editor(editor)
    return subprocess.Popen([command_str, filename], stdin=subprocess.PIPE)
