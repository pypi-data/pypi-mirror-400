...
class PresenceService(GObject.GObject):
    ...
    def get_activity(self, activity_id, warn_if_none=True):
        ...
        if self._activity_cache is not None:
            if self._activity_cache.props.id != activity_id:
                raise RuntimeError(
                    "Activities can only access their own" " shared instance"
                )
            ...
            for account_path, connection in list(connections_per_account.items()):
                ...
                try:
                    room_handle = connection.connection.GetActivity(
                        activity_id, dbus_interface=CONN_INTERFACE_ACTIVITY_PROPERTIES
                    )
                except dbus.exceptions.DBusException as e:
                    name = "org.freedesktop.Telepathy.Error.NotAvailable"
                    if e.get_dbus_name() == name:
                        logging.debug(
                            "There's no shared activity with the id " "%s" % activity_id
                        )
                    elif (
                    ...
                    activity = Activity(
                        account_path, connection.connection, room_handle=room_handle
                    )
    ...
    def get_activity_by_handle(self, connection_path, room_handle):
        if self._activity_cache is not None:
            ...
            connection_manager = get_connection_manager()
            account_path = connection_manager.get_account_for_connection(
                connection_path
            )
            ...
            activity = Activity(account_path, connection, room_handle=room_handle)
            self._activity_cache = activity
            return activity
    ...
    # DEPRECATED
    def get_buddy_by_telepathy_handle(self, tp_conn_name, tp_conn_path, handle):
        ...
        account_paths = account_manager.Get(
            ACCOUNT_MANAGER, "ValidAccounts", dbus_interface=PROPERTIES_IFACE
        )
        for account_path in account_paths:
            ...
            if connection_path == tp_conn_path:
                ...
                contact_ids = connection.InspectHandles(
                    HANDLE_TYPE_CONTACT, [handle], dbus_interface=CONNECTION
                )
                return self.get_buddy(account_path, contact_ids[0])

        raise ValueError(
            "Unknown buddy in connection %s with handle %d" % (tp_conn_path, handle)
        )
    ...
    def share_activity(self, activity, properties=None, private=True):
        ...
        if connection is None:
            self.emit("activity-shared", False, None, "No active connection available")
            return
        ...
        if shared_activity.props.joined:
            raise RuntimeError("Activity %s is already shared." % activity.props.id)

        shared_activity.share(self.__share_activity_cb, self.__share_activity_error_cb)
