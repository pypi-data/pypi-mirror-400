...
def to_markdown(obj, mode="list", headers=None):
    """
    Convert list or numpy.ndarray (1D or 2D) to Markdown string.

    PARAMETERS
    ----------
    obj : list or np.ndarray
        Input object (1D or 2D only)
    mode : {"code", "list", "table"}
        Markdown output format
    headers : list of str, optional
        Column headers for table (2D array only)
    ...
    ------
    TypeError
        If obj is not list or numpy.ndarray
    ValueError
        If ndarray dimension > 2 or incompatible mode
    """
    ...
    if isinstance(obj, list):
        ...
        raise TypeError(
            ...
            "Only list or numpy.ndarray are allowed."
        )
        return None
    ...
    if arr.ndim > 2:
        raise ValueError(
            f"Only 1D or 2D arrays are supported (ndim={arr.ndim})"
        )
    ...
    if mode == "list":
        if arr.ndim != 1:
            raise ValueError("mode='list' supports only 1D data")
        ss = '[' + ", ".join(f"{x}" for x in arr) + ']'
        return ss
    ...
    if mode == "table":
        if arr.ndim != 2:
            raise ValueError("mode='table' supports only 2D data")
    ...
    # Invalid mode
    # ---------------------------
    raise ValueError("mode must be one of {'code', 'list', 'table'}")


def to_markdown_dict( data ):
    data_str = ''
    for key in data.keys():
        if isinstance( data[key], str ):
            data_str = data_str + f'{key}:\n' + data[key] + '\n\n'
        elif isinstance( data[key], (list, np.ndarray) ):
            data_str = data_str + f'{key}:\n' + to_markdown(data[key]) + '\n\n'
        elif isinstance( data[key], pd.DataFrame ):
            data_str = data_str + f'{key}:\n' + data[key].to_markdown() + '\n\n'
        else:
            print(f'WARNING: Contents in {key} is not supported data type. ')
            pass
    return data_str
...
class scoda_ai:
  ...
  def ask(self, data,
      q: str = 'Please explain this data biologically~',
      prompt: str = 'This is my scRNA-seq analysis result data: '):
      if isinstance( data, str ):
          data_str = data
      elif isinstance( data, (list, np.ndarray) ):
          data_str = to_markdown(data)
      elif isinstance( data, dict ):
          data_str = to_markdown_dict( data )
          if len(data_str) == 0:
