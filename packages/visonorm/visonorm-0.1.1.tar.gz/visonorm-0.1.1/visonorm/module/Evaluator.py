...
class Evaluator:
    ...
    def __init__(self, config, logger=None):
        self.config = config
        self.logger = logger
        self.metric = get_config_attr(config, 'training.metric', get_config_attr(config, 'metric', 'f1_score'))
    ...
    def evaluate(self, preds, targets, sources, comment="", verbose=True):
        ...
        for i in range(len(preds)):
            ...
            pred_need_norm = np.sum(source!=pred)
            ...
            tp = np.sum(target[need_norm]==pred[need_norm])
            fp = np.sum(target[need_norm]!=pred[need_norm])
            tn = np.sum(target[normed]==pred[normed])
            fn = np.sum(target[normed]!=pred[normed])

            r = tp/(tp+fp) if (tp+fp) > 0 else 0
            p = tp/pred_need_norm if pred_need_norm > 0 else 0
            norm_r = tn/(tn+fn) if (tn+fn) > 0 else 0
            f1 = (2*p*r)/(p+r) if (p+r) > 0 else 0
            acc = np.sum(pred==target)/total_num
