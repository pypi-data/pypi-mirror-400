...
@mcp.tool()
async def spider_notes_with_comments(note_urls: list[str]) -> list[dict[str, Any] | str]:
    """获取小红书一篇帖子内容和所有一级评论：note_urls: 小红书帖子链接，数组的形式
    """
...
@mcp.tool()
async def save_note_as_word(note: dict[str, Any] | list[dict[str, Any] | str]) -> str | list[str]:
    """根据帖子数据生成word文档

    Args:
        note: 小红书帖子数据（单个帖子或帖子列表，包含帖子内容和评论）
    """
    if not word_save_dir:
        raise ValueError("请先在环境变量WORD_SAVE_DIR中配置文件保存路径")
    base_path = Path(word_save_dir)
    base_path.mkdir(parents=True, exist_ok=True)
    def _save_single(single_note: dict[str, Any]) -> str:
    ...
    if isinstance(note, list):
        ...
        for item in note:
            if isinstance(item, dict):
                filepaths.append(_save_single(item))
        return filepaths
    if isinstance(note, dict):
        return _save_single(note)
    raise ValueError("参数格式错误，应为帖子数据或帖子数据列表")
