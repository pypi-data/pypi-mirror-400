...
try:
    ...
    GetSessionIdCallback = Any  # type: ignore[assignment]
    streamablehttp_client = None  # type: ignore[assignment]
    SessionMessage = Any  # type: ignore[assignment]
    _mcp_import_error: Exception | None = e
else:
...
def _require_mcp() -> None:
    if _mcp_import_error is None:
        return
    raise ImportError(
        "The 'mcp' package is required to run the MCP integration but is not installed.\n"
        "To fix this, install the optional dependency: pip install 'riyadhai[mcp]'"
