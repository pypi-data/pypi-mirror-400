...
class CudaIpcRingBuffer:
    ...
    def write_frame_fast(self, gpu_frame: cp.ndarray, sync: bool = True) -> int:
        """Fast write with slot safety check for cross-container sync.

        Checks if consumer has finished with the slot before writing.
        Does not block - continues anyway to avoid producer stalls.
        """
        next_frame_idx = self._cached_write_idx + 1
        slot = (next_frame_idx - 1) % self.num_slots

        # Safety check - warn if overwriting unacknowledged frame
        # Skip for first num_slots frames (no previous data to overwrite)
        if next_frame_idx > self.num_slots:
            consumer_done = self._read_consumer_done_idx()
            slot_last_frame = next_frame_idx - self.num_slots
            if consumer_done < slot_last_frame:
                # Consumer hasn't finished with this slot yet
                # Continue anyway - logging disabled for performance
                # This indicates backpressure: producer faster than consumer
                pass
