...
class FundamentalTheoremCore:
  ...
  def calculate_riemann_sum(
    self, func_str: str, interval: Tuple[float, float], n_rectangles: int, method: str = "right"
  ) -> Dict:
    '''
  ...
  def calculate_riemann_convergence(
    self, func_str: str, interval: Tuple[float, float], max_n: int = 100) -> Dict:
    '''
  ...
  def calculate_ftc_part1_data(
    self, func_str: str, lower_bound: float, upper_range: Tuple[float, float] = (0, 5)
  ) -> Dict:
    '''
...
def compare_integration_methods_data(
  func_str: str, interval: Tuple[float, float] = (0, 2), n_values: Optional[list] = None
) -> Dict:
  '''
