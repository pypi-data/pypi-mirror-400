...
try:
    ...
    # But don't crash, as some commands (like 'init') might not need Scheduler
    # Only print if verbose or just keep it silent but accessible?
    # For now, let's keep it silent but maybe we should expose why it failed if user tries to use it.
    Scheduler = None
    # We can store the error to show it later if needed
    _SCHEDULER_IMPORT_ERROR = str(e)
except Exception as e:
    # Catch other errors like SyntaxError in modules
    print(f"Critical error importing Scheduler: {e}")
    Scheduler = None
    _SCHEDULER_IMPORT_ERROR = str(e)
else:
...
def format_help():
    ...
    header = "\nNyx CLI (v4.0.0) - AI Workload Orchestrator\nUsage: nyxcli <command> [arguments]\n"
    categories = {}
...
async def handle_terminal(config):
    ...
    if shutil.which("warp"):
        ...
        possible_path = Path(local_app_data) / "Programs" / "Warp" / "Warp.exe"
        if possible_path.exists():
            warp_cmd = str(possible_path)
        else:
            print("❌ Error: Warp Terminal not found.")
            print("Please install Warp Terminal or add it to your PATH to use 'nyxcli terminal'.")
            sys.exit(1)

    print(f"🚀 Launching Warp Terminal ({warp_cmd})...")
    try:
        # Launch Warp with current directory as argument
        subprocess.Popen(f'"{warp_cmd}" .', shell=True)
        print("✅ Warp launched.")
    except Exception as e:
