...
class CheckElementRequest(BaseModel):
    """
    CheckElementRequest
    """ # noqa: E501
    ...
    check: StrictStr = Field(description="Check type: exists or visible")
    __properties: ClassVar[List[str]] = ["selector", "by", "check"]

    @field_validator('check')
    def check_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['exists', 'visible']):
            raise ValueError("must be one of enum values ('exists', 'visible')")
        return value
