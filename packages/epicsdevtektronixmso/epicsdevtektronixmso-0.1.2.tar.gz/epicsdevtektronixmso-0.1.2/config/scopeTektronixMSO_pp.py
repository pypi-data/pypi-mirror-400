"""Pypet page for Tektronix MSO scopes"""
# format: pypeto 1.2+
__version__ = 'v1.2.2 2025-01-02'# 
print(f'Tek5Scope_Setup {__version__}')
...
#``````````````````PyPage Object``````````````````````````````````````````````
class PyPage():
    def __init__(self, instance='tekMSO:', title="Tektronix MSO46 Oscilloscope"):
...
string or device:parameter and the value is dictionary of the features.
        ...
        def ChLine(suffix):
            return [f'{D}c{ch+1}{suffix}' for ch in range(6)]
