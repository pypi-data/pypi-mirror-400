  ...
  <div class="card table-main">
    ...
    <slot name="pagination">
      <Pagination
        ...
        :use-computed-total="useComputedTotal"
...
export interface ProTableProps {
  ...
  useComputedTotal?: boolean; // 是否使用计算后的 total（默认为 true，用于处理后端不返回总记录数的情况）==> 非必传
...
// 监听 props.columns 的变化，更新 tableColumns
watch(
  () => props.columns,
  newColumns => {
    Object.assign(tableColumns, newColumns);
  },
  { deep: true }
);
...
const dragSort = () => {
  nextTick(() => {
    ...
    if (!tbody) {
      console.warn(`ProTable: tbody element not found for uuid ${uuid.value}`);
      return;
    }
