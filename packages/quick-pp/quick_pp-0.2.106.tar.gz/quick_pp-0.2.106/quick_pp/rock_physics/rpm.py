...
from quick_pp.rock_physics.geomechanics import (
    estimate_shear_velocity,
    estimate_compressional_velocity,
...
def qaqc_xplots(rhob, vp=None, vs=None):
    ...
    visually inspect the relationships between bulk density (RHOB), P-wave
    velocity (Vp), and S-wave velocity (Vs). If Vp or Vs are not provided,
    they are estimated using empirical relationships.

    Args:
        rhob (np.ndarray): Bulk density [g/cm³].
        vp (np.ndarray, optional): P-wave velocity [m/s]. If None, it is estimated. Defaults to None.
        vs (np.ndarray, optional): S-wave velocity [m/s]. If None, it is estimated. Defaults to None.

    Returns:
        matplotlib.figure.Figure: A figure containing the 2x3 grid of QC crossplots.
    """
...
def fluid_typing_xplots(rhob, vp=None, vs=None):
    ...
    Lambda-Rho vs. Mu-Rho, and Acoustic Impedance (AI) vs. Vp/Vs ratio.
    These plots are standard tools for identifying fluid effects and
    discriminating between different lithologies in reservoir rocks.
    ...
    Returns:
        matplotlib.figure.Figure: A figure containing the fluid typing crossplots.
...
def QI_screening(gr, rhob, vp):
    ...
    lithology (e.g., sand, shale) and fluid (e.g., brine) scenarios.
    ...
    Returns:
        matplotlib.figure.Figure: A figure containing the QI screening plot.
    """
...
def rpt_plot(
    ...
    Args:
        ...
        vp (np.ndarray, optional): P-wave velocity [m/s]. If None, it is estimated. Defaults to None.
        vs (np.ndarray, optional): S-wave velocity [m/s]. If None, it is estimated. Defaults to None.
        model (str, optional): The rock physics model to use for the template.
                               Options: 'soft_sand', 'stiff_sand', 'contact_cement', 'hertz_mindlin'.
                               Defaults to 'soft_sand'.
        fluid_type (str, optional): The fluid type for the model.
                                    Options: 'water', 'gas', 'oil'. Defaults to 'gas'.
        sigma (float, optional): Effective stress [MPa]. Defaults to 20.
        phi_c (float, optional): Critical porosity [v/v]. Defaults to 0.4.
        Cn (float, optional): Coordination number. Defaults to 8.6.
        f (float, optional): Shear modulus correction factor. Defaults to 0.0.
        scheme (int, optional): Cement deposition scheme for 'contact_cement' model. Defaults to 2.

    Returns:
        matplotlib.figure.Figure: A figure containing the RPT plot.
    """
    ...
    assert fluid_type in ["water", "gas", "oil"], (
        f"Fluid type must be one of {fluid_type_options}"
    )
    ...
    phi = np.linspace(
        0.1, phi_c, 10
    )  # Define porosity range according to critical porosity
...
def elastic_bounds_plot(rhob, vp=None, vs=None, phi_calc=None):
    ...
    Args:
        ...
        vp (np.ndarray, optional): P-wave velocity [m/s]. If None, it is estimated. Defaults to None.
        vs (np.ndarray, optional): S-wave velocity [m/s]. If None, it is estimated. Defaults to None.
        phi_calc (np.ndarray, optional): Porosity log [v/v]. If None, it is estimated from `rhob`.
                                         Defaults to None.

    Returns:
        matplotlib.figure.Figure: A figure containing the elastic bounds plots for K and G.
    """
    if vs is None and vp is not None:
        vs = estimate_shear_velocity(vp)
    if vp is None:
    ...
    if phi_calc is None:
        phi_calc = (Config.GEOMECHANICS_VALUE["RHOB_QUARTZ"] - rhob) / (
            Config.GEOMECHANICS_VALUE["RHOB_QUARTZ"]
            - Config.GEOMECHANICS_VALUE["RHOB_BRINE"]
        )
