...
class Repl:
    ...
    async def run(self):
        print("pw-repl v0.1.33 - Type 'help' for commands")
        # 自动重连上次的连接
  ...
  help [cmd]            - Show help""")
    ...
    async def cmd_save(self, name: str = None):
        """保存当前会话到 s-mgr
        
        用法:
            save              - 保存到当前账号
            save platform/user - 保存到指定账号（不存在则创建）
        """
        client, helper = self._get_smgr()
        if not client:
            print("✗ smgr_url not configured")
        ...
        # 检测当前平台
        if 'kaggle' in current_url:
            current_platform = 'kaggle'
        elif 'github' in current_url:
            current_platform = 'github'
        elif 'google' in current_url or 'gmail' in current_url:
            current_platform = 'google'
        else:
            current_platform = current_url.split('/')[2].split('.')[-2]
        
        if name:
            # 解析 platform/username
            if '/' in name:
                platform, username = name.split('/', 1)
                # 检查平台是否匹配
                if platform != current_platform:
                    print(f"⚠ Warning: saving {platform} session from {current_platform} page")
            else:
            ...
            if not account_id:
                ...
                try:
                    ...
                    account_id = acc['id']
                    print(f"✓ Created account: {platform}/{username}")
                except Exception as e:
                    print(f"✗ Failed to create account: {e}")
                    return
        else:
            if not hasattr(self, '_current_account_id'):
                print(f"✗ No session selected. Use: save <username> or save <platform/username>")
                return
