...
def test_apply_fails_when_verify_true_and_bad_manifest(tmp_path):
    ...
    with pytest.raises(RuntimeError):
        list(engine.apply_template('prov', dest, {'project_name': 'X'}, force=True, verify=True))
...
def test_verify_manifest_hmac_ok(tmp_path, monkeypatch):
    ...
    key = 's3cr3t'
    canonical = json.dumps({'files': manifest_files}, sort_keys=True, separators=(',', ':')).encode('utf-8')
...
def test_apply_fails_when_verify_true_and_bad_signature(tmp_path, monkeypatch):
    ...
    import pytest
    with pytest.raises(RuntimeError):
        list(engine.apply_template('provh', dest, {'project_name': 'X'}, force=True, verify=True))
