...
def get_cci_means( obj, cci_idx_lst = None, 
    ...
    if isinstance(obj, anndata.AnnData):
        if obj.uns['analysis_parameters']['REF_NORMAL_CONDITIONS'] is not None:
            ref_normal_cond = obj.uns['analysis_parameters']['REF_NORMAL_CONDITIONS']
            if isinstance( ref_normal_cond, str ):
                ref_normal_cond = [ref_normal_cond]
            else:
                ref_normal_cond = list(ref_normal_cond)
            b = df['Group'].isin( ref_normal_cond )
            if b.sum() > 0:
                cols = df.drop(columns = ['Group']).columns.values.tolist()
                col_lst_containing_aneuploid = [c for c in cols if 'Aneuploid' in c]
                if len(col_lst_containing_aneuploid) > 0:
                    for c in col_lst_containing_aneuploid:
                        c_a_to_d = c.replace('Aneuploid', 'Diploid')
                        if c_a_to_d in cols:
                            df.loc[b, c] = df.loc[b, c_a_to_d]    
    return df
    

def cci_get_diff_interactions( df_cci_sample, sample_group_map = None, 
                               ref_group = None, pval_cutoff = 0.05, equal_var = False ):
...
def test_group_diff( df_sample_by_items, sample_group_map = None, 
    ...
    return cci_get_diff_interactions( df_sample_by_items, sample_group_map, 
                               ref_group, pval_cutoff, equal_var )
...
def update_markers_dict2(mkrs_all, mkr_dict, X, y, rend = None, cutoff = 0.3, 
    ...
    # presence fraction matrix (celltype × marker)
    df = pd.DataFrame(index = ct_list, columns = mkrs_all)
    df.loc[:,:] = 0
...
def plot_gsa_dot( gsa_res, pval_cutoff = 1e-2,
    ...
    df_qv, df_es = get_gsa_summary(gsa_res_t, max_log_p = -np.log10(pval_clip))

    if df_qv.shape[0] == 0:
        print(f'ERROR: pval_cutoff = {pval_cutoff} seems to be too small.')
        return
...
def plot_gsa_all( gsa_res, 
    ...
    plot_gsa_dot( gsa_res, 
                  ...
                  figsize, dot_size, cbar_frac, cbar_aspect, cmap, pval_clip )    
    return
...
def get_amp_regions_for_known_markers(adata, spot_llst, known_markers_list = None, verbose = False ):

    some_known_markers_list = ['ESR1', 'ERBB2', 'EGFR', 'KRAS', 'CDKN2A', 'TP53', 'SMAD4', 'GATA6', 
                      'NFASC', 'ACOT1', 'GSDMD', 'SOX2', 'BRAF', 'ALK', 'ROS1', 'RET', 'NTRK',
                      'INTS8', 'ECT2', 'LSM1', 'DDHD2', 'COPS5', 'EIF3E', 'TPD52']

    if isinstance( known_markers_list, list ):
        some_known_markers_list = list(set(some_known_markers_list).union(known_markers_list))
    ...
    for j, key in enumerate(spot_llst.keys()):
        ...
        glst_sel = []
        for g in some_known_markers_list:
            loc, gene_s = get_genomic_spot_containing_gene( g, glst_dct, some_known_markers_list, verbose = False )
            if (loc >= 0):
...
def plot_cnv_hit( cna_info, adata = None, title = None, title_fs = 13, label_fs = 12, tick_fs = 10, 
    ...
    if (adata is not None) & (spot_llst is not None) & (df is not None):
        rngs_dct = get_amp_regions_for_known_markers(adata, spot_llst, 
                                                     ...
                                                     verbose = verbose)
        if (some_known_markers_list is not None):
            if some_known_markers_list:
...
def add_to_subset_markers( 
    adata, 
    tumor_mkr_dict,
    ref_taxo_level = 'celltype_major',
    tumor_origin = None,
    normal_ref_cond = None,
    obs_col_to_add = 'celltype_subset_rev'
): 
    if 'ploidy_dec' not in adata.obs.columns.values.tolist():
        print('WARNING: Tumor cell identification not performed.')
        return None

    if normal_ref_cond is None:
        if 'REF_NORMAL_CONDITIONS' in list(adata.uns['analysis_parameters'].keys()):
            normal_ref_cond = adata.uns['analysis_parameters']['REF_NORMAL_CONDITIONS']

    if tumor_origin is None:
        b = adata.obs['tumor_origin_ind']
        pcnt = adata.obs.loc[b, ref_taxo_level].value_counts()
        for i in pcnt.index:
            if (i != 'unassigned'):
                ...
                break
    if tumor_origin is None:
        print('WARNING: Tumor origin celltype not found.')
        return None
        
    scoda_markers_db = adata.uns['Celltype_marker_DB']
    if ref_taxo_level == 'celltype_major':
        subset_to_major_map = copy.deepcopy( scoda_markers_db['subset_to_major_map'] )
    elif ref_taxo_level == 'celltype_minor':
        subset_to_major_map = copy.deepcopy( scoda_markers_db['subset_to_minor_map'] )
        
    to_del = []
    for k in subset_to_major_map.keys():
        if subset_to_major_map[k] == tumor_origin:
            to_del.append(k)
    
    subset_markers_dict = copy.deepcopy( scoda_markers_db['subset_markers_dict'] )
    
    for k in to_del:
        if k in list(subset_markers_dict.keys()):
            del subset_markers_dict[k]
        
    for k in tumor_mkr_dict.keys():
        subset_markers_dict[f'{tumor_origin} ({k})'] = np.array( tumor_mkr_dict[k] )
    
    adata.obs[obs_col_to_add] = adata.obs['celltype_subset'].copy(deep = True).astype(str)
    b1 = adata.obs[ref_taxo_level] == tumor_origin
    b2 = adata.obs['ploidy_dec'] == 'Diploid'
    adata.obs.loc[b1 & b2, obs_col_to_add] = f'{tumor_origin} (Diploid)'

    if (normal_ref_cond is not None):
        if isinstance(normal_ref_cond, str):
            normal_ref_cond = [normal_ref_cond]
        elif isinstance(normal_ref_cond, np.ndarray):
            normal_ref_cond = list(normal_ref_cond)
            
        clst = list(adata.obs['condition'].unique())    
        for c in clst:
            if c not in normal_ref_cond:
                b2 = adata.obs['ploidy_dec'] == 'Aneuploid'
                b3 = adata.obs['condition'] == c
                adata.obs.loc[b1 & b2 & b3, obs_col_to_add] = f'{tumor_origin} ({c})'
            
        for c in normal_ref_cond:
            if c in list(adata.obs['condition'].unique()):
                b2 = adata.obs['condition'] == c
                adata.obs.loc[b1 & b2, obs_col_to_add] = f'{tumor_origin} ({c})'  
    else:
        clst = list(adata.obs['condition'].unique())    
        for c in clst:
            b2 = adata.obs['ploidy_dec'] == 'Aneuploid'
            b3 = adata.obs['condition'] == c
            adata.obs.loc[b1 & b2 & b3, obs_col_to_add] = f'Tumor {tumor_origin} ({c})'
            
    return subset_markers_dict
