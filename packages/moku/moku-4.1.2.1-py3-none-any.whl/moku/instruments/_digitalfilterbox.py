...
class DigitalFilterBox(MultiInstrumentSlottable, Moku, StreamInstrument):
    ...
    def enable_output(self, channel, signal=True, output=True, gain_range="0dB", strict=True):
        """
        enable_output.
    ...
    def set_custom_filter(self, channel, sample_rate, scaling=1, coefficients=None, strict=True):
        """
        set_custom_filter.
    ...
    def get_data(self, timeout=60, wait_reacquire=False, wait_complete=False, measurements=False):
        """
        get_data.
    ...
    def start_streaming(
        ...
        response = self.session.post(f"slot{self.slot}/{self.operation_group}", operation, params)
    ...
    def stop_streaming(self):
        ...
        response = self.session.post(f"slot{self.slot}/{self.operation_group}", operation)
        self.stream_id = None
        return response
