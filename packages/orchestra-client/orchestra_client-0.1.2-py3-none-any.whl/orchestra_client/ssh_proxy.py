...
from orchestra_client.lib.config import get_orchestra_password, get_user_token
...
async def ssh_proxy(session_name: str, server_url: str) -> int:
    ...
    """
    # Build URL with auth and user_token
    url = f"{server_url}/sessions/{session_name}/connect"
    params = []

    user_token = get_user_token()
    params.append(f"user_token={quote(user_token)}")
