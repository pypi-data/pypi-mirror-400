...
class DecisionManager:
    ...
    def _maybe_capture_execution_decision(
        ...
        try:
            ...
            # DEBUG: execution span 데이터 출력
            import json
            print(f"\n{'='*80}")
            print(f"[EXECUTION SPAN DEBUG] span_kind: {span_kind}")
            print(f"[EXECUTION SPAN DEBUG] Keys in span_data: {list(span_data.keys())}")
            
            # Check for options/selected_option
            if "options" in span_data:
                print(f"[EXECUTION SPAN DEBUG] ✅ options found: {len(span_data['options'])} items")
            if "selected_option" in span_data:
                print(f"[EXECUTION SPAN DEBUG] ✅ selected_option found: {len(span_data['selected_option'])} items")
            ...
            if metadata:
                ...
                if "tools" in request_params:
                    print(f"[EXECUTION SPAN DEBUG] ✅ tools found in metadata: {len(request_params['tools'])} tools")
            
            print(f"[EXECUTION SPAN DEBUG] Full span_data:")
            ...
            # RAG or Tool with options
            elif span_kind in ["rag", "tool"]:
            ...
            # Determine agent name
            func_name = span_data.get("name", "unknown")
            result_data = span_data.get("result", {})
