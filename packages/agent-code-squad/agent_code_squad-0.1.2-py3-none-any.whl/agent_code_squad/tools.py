...
def register_tools(mcp: FastMCP, *, config: SquadConfig) -> None:
    ...
    def code_squad_execute(
        ...
        try:
            ...
            if scope_violations:
                ...
                status = "completed" if counts.get("running", 0) == 0 and counts.get("starting", 0) == 0 and counts.get("queued", 0) == 0 else "timeout"
    ...
    def code_squad_report(run_id: str, options: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            response: dict[str, Any] = {"run_id": run_id, "report_json": str(report_json), "report_md": str(report_md), "counts": counts}
            elapsed_ms = int((time.monotonic() - start) * 1000)
            if debug:
