    ...
    def copy_dataset(
        ...
        **init_kwargs
        ...
        with CopyPair(fname_cur, fname_new, **conn_kwargs) as pair:
            ...
            elif not isinstance(pair.src.file[addr_cur], h5py._hl.dataset.Dataset):
                raise TypeError(
                    f"Cannot use copy_dataset for {pair.src.file[addr_cur]}")
            # Check new address
            if addr_new in pair.out.file:
                raise ValueError(
            ...
            if (compression == "default") and (dtype == "default"):
                pair.src.file.copy(
                    pair.src.file[addr_cur],
                    pair.out.file, # TODO test this
                    name=addr_new,
                )
            else:
                # Check dtype
                if dtype == "default":
                    dtype = pair.src.file[addr_cur].dtype
                # Check compression
                if compression == "default":
                    compression = pair.src.file[addr_cur].compression
                # Create dataset
                pair.out.file.create_dataset(
                    addr_new,
                    pair.src.file[addr_cur].shape,
                    dtype=dtype,
                    compression=compression,
                    **init_kwargs
                )
                # Copy values
                pair.out.file[addr_new][...] = pair.src.file[addr_cur][...]

            # Copy attributes
            for key, value in pair.src.file[addr_cur].attrs.items():
                pair.out.file[addr_new].attrs[key] = value
    ...
    def copy(
        ...
        if isinstance(other, str):
           ...
           self.copy_dataset(
            ...
            if isinstance(other, DatasetHandle):
                addr_other = other.addr
            else:
                addr_other = os.path.join(
                    other.addr, 
                    os.path.basename(self.addr),
                )
            self.copy_dataset(
                self.fname,
                self.addr,
