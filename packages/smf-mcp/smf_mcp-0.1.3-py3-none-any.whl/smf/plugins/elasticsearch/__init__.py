...
try:
    ...
    print(f"DEBUG: Failed to import elasticsearch: {e}")
...
# CLI commands (imported at the end to avoid circular imports)
def add_parser(subparsers):
    """Add parser for activate-plugin command."""
    from smf.plugins.elasticsearch.activate import activate_plugin_command
...
# Export CLI functions (lazy import to avoid circular dependencies)
def __getattr__(name: str):
    if name in ("_activate_elasticsearch", "activate_plugin_command"):
        from smf.plugins.elasticsearch.activate import _activate_elasticsearch, activate_plugin_command
        if name == "_activate_elasticsearch":
            return _activate_elasticsearch
        return activate_plugin_command
