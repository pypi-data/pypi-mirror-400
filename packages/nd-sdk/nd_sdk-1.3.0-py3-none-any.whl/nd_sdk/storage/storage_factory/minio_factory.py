from ..storage.storage_clients.minio_client import MinioStorageClient
from ..storage.storage_clients.minio_client import download_file, upload_file, delete_file


class MinioStorageFactory:
    def __init__(self):
        self.client_wrapper = self._initialize_client()

    def _initialize_client(self):
        try:
            return MinioStorageClient().client
        except ImportError as e:
            raise ImportError(f"Module import failed for minio: {str(e)}")
        except Exception as e:
            raise Exception(f"Failed to initialize storage client for minio: {str(e)}")
