...
def test_wrap_install_preserves_top_level(tmp_path):
    ...
    (src / 'ISSUE_TEMPLATE' / 'bug.md.j2').write_text('Bug report for {{ project_name }}')
    user_dir = tmp_path / 'user_templates'
    engine = Engine(user_templates_root=user_dir)
    dest = engine.install_user_template(src, name='ghwrap', force=True, wrap=True)
...
def test_content_install_copies_contents(tmp_path):
    ...
    user_dir = tmp_path / 'user_templates'
    engine = Engine(user_templates_root=user_dir)
    dest = engine.install_user_template(src, name='ghcontent', force=True, wrap=False)
