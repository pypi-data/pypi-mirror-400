...
def check_env_variable(var_name: str, required: bool = True) -> tuple[bool, str]:
    ...
    if value:
        ...
        if required:
            ...
            return False, f"{var_name} is not set (optional)"


def check_required_packages():
    """Check if all required packages are installed."""
    required = ["astor", "boto3", "dotenv", "wrapt"]
    missing = []
    installed = []
    
    for pkg in required:
        # Special case for python-dotenv which imports as 'dotenv'
        module_name = "dotenv" if pkg == "python-dotenv" else pkg
    ...
    return True, f"All required packages installed: {', '.join(installed)}"
...
def run_doctor():
    ...
    checks = [
        ...
        ("AWS_PROFILE env", lambda: check_env_variable("AWS_PROFILE", required=True)),
        ("PROJECT_ID env", lambda: check_env_variable("PROJECT_ID", required=False)),
    ...
    try:
        # Get config to extract project_id
        cfg = get_cfg()
        project_id = cfg.get("project_id", "default")
        
        # Start trace with metadata including project_id
        TraceManager.start_trace("veriskgo_doctor_trace", metadata={"project_id": project_id})
        ...
        bundle.update({
            "event_type": "trace_bundle",  # Add event_type for lambda compatibility
            "user_id": "doctor",
            ...
            "metadata": {"project_id": project_id}  # Add metadata at top level
        })
