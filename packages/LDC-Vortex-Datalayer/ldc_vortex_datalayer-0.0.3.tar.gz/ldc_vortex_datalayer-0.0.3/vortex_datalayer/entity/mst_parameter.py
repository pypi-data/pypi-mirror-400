...
class MstParameter(BaseDataLayer):
    ...
    def get_bulk_new_investment_amount(self):
        """
        Get bulk new investment amount from t_mst_parameter.
        
        Matches stored procedure logic: fetches value_1 from t_mst_parameter
        where logical_group = 'bulk_investment_new_amount'.
        
        Returns:
            float: Bulk new investment amount, or 0 if not found
        """
        sql = """
        ...
        return result['bulk_new_amount'] if result else 0