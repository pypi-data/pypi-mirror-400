...
def get_attachments_and_images(text: str, xpath_expression: str,
                               file_ext: Union[list, str] = '') -> AttachmentsAndImagesResult:
    """
    获取正文中的附件和图片链接及名称
    :param text: HTML文本
    :param xpath_expression: XPath表达式
    ...
    image_extensions_xpath = ' or '.join(
        f'substring(@href, string-length(@src) - {len(ext) - 1}) = "{ext}"'
        for ext in image_extensions
    )
    image_a_extensions_xpath = ' or '.join(
        f'substring(@href, string-length(@href) - {len(ext) - 1}) = "{ext}"'
        for ext in image_extensions
    )
    ...
    file_a_list = selector.xpath(f'{xpath_expression}//a[{file_extensions_xpath}]')

    file_urls = []
    file_names = []
    for file_a in file_a_list:
        file_names.append(''.join(file_a.xpath('string(.)').getall()).strip().replace('\n', ' ')
                          .replace('\t', ' ').replace('\r', ' ').replace(' ', '')
                          .replace('\xa0', ''))
        file_urls.append(file_a.xpath('./@href').get())
...
def get_att(text:str,base_url:str, xpath_expression:str, file_ext: Union[list, str] = '') -> str:
    def join_url(base_url, url: Union[str, list]) -> Union[str, list]:
        """
        获取完整路径
        :param base_url:
        :param url:
    ...
    att = get_attachments_and_images(text, xpath_expression, file_ext=file_ext)
    result = {}
    i = 0
    for name,url in zip(att.file_names, att.file_urls):
        i += 1
        url = join_url(base_url, url)
        result[f'fj{i}'] = f'{name}|{url}'
    if result:
