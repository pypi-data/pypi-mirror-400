...
mod tests {
    ...
    #[tokio::test]
    async fn test_scanner_passive_listener_stop() {
        // Start listener
        ...
        assert!(state.listener_started.load(Ordering::SeqCst));

        // Stop listener
        Scanner::stop_passive_listener();

        // Wait a bit for tasks to exit
        sleep(Duration::from_millis(200)).await;

        assert!(!state.listener_started.load(Ordering::SeqCst));
        assert!(state.cancel_token.read().is_none());
        assert!(state.sockets.read().is_empty());
    }
}
