...
def get_binary_cc18_datasets(n_datasets=None, min_samples=100, max_samples=50000, 
                             exclude_image=True, min_features=2, max_features=10000):
    """
    Fetches binary classification datasets from OpenML CC-18 benchmark suite (Suite ID 99).
    
    CC-18 is a curated benchmark suite commonly used in ML research for standardized evaluation.
    
    Parameters
    ----------
    n_datasets : int or None
        Number of datasets to return. If None, returns all matching datasets.
    min_samples : int
        Minimum number of samples (instances)
    max_samples : int
        Maximum number of samples (instances)
    exclude_image : bool
        Whether to exclude image datasets (MNIST, CIFAR, etc.)
    min_features : int
    ...
    Returns
    -------
    list of tuples: [(dataset_id, dataset_name), ...]
    """
    try:
        import openml
    except ImportError:
        print("ERROR: openml package required for OpenML benchmarking.")
        print("Install with: pip install openml")
        raise
    
    print(f"Querying OpenML CC-18 Benchmark Suite (Suite ID 99) for binary datasets...")
    print(f"  Filters: {min_samples} <= samples <= {max_samples}, {min_features} <= features <= {max_features}")
    ...
    for task_id in tasks:
        if n_datasets is not None and len(datasets) >= n_datasets:
            break
        
        try:
            # Get task metadata (lightweight, no data download)
            task = openml.tasks.get_task(task_id, download_data=False)
            
            # Get dataset metadata (lightweight, no data download)
            ...
            # Check size constraints from metadata
            ...
            if n_features < min_features or n_features > max_features:
                excluded_reasons['features'] += 1
                continue
            
            # Exclude image datasets if requested
            if exclude_image:
                image_keywords = ['mnist', 'cifar', 'fashion', 'kuzushiji', 'image', 'pixel']
                if any(keyword in ds_meta.name.lower() for keyword in image_keywords):
                    excluded_reasons['image'] += 1
                    continue
            
            # Add to datasets list
            datasets.append((task.dataset_id, ds_meta.name))
            
        except Exception as e:
            excluded_reasons['error'] += 1
            print(f"  Warning: Error processing task {task_id}: {e}")
            continue
    
    print(f"\n  Found {len(datasets)} binary datasets matching criteria")
    print(f"  Selected datasets:")
    for ds_id, ds_name in datasets[:15]:  # Show first 15
    ...
    if len(datasets) == 0:
        print("\n  ⚠️  WARNING: No datasets found matching criteria!")
        print("  Consider relaxing filters (e.g., increase max_samples or max_features)")
...
def load_openml_dataset(dataset_id):
    """
    Load a specific OpenML dataset by ID.
    ...
    X : pd.DataFrame
        Feature matrix
    y : np.ndarray
        Target vector (binary, encoded as 0/1)
    categorical_cols : list
        List of categorical column names
    imbalance_ratio : float
        Ratio of minority to majority class
    ...
    # Handle target encoding (Ensure binary 0/1)
    le = LabelEncoder()
    y_encoded = le.fit_transform(y)
    
    # Ensure it's strictly binary
    if len(le.classes_) != 2:
        raise ValueError(f"Dataset {dataset_id} ({dataset_name}) is not binary. Found {len(le.classes_)} classes: {le.classes_}")
    ...
    return X, y_encoded, categorical_cols, imbalance_ratio, dataset_name
...
if __name__ == "__main__":
    ...
    # Configuration for OpenML CC-18 dataset selection
    N_DATASETS_TO_RUN = None  # None = use all matching datasets, or set to a number
    ...
    EXCLUDE_IMAGE = True  # Exclude image datasets
    
    print("\n" + "="*70)
    print("  OPENML CC-18 BENCHMARK SUITE (Binary Classification)")
    print("  CC-18 is a curated benchmark suite commonly used in ML research")
    print("="*70)
    
    # Fetch binary classification datasets from OpenML CC-18 suite
    try:
    ...
    if len(target_datasets) == 0:
        print("\n✗ No datasets found matching criteria. Exiting.")
        exit(1)
    
    print(f"\n  Running benchmarks for {len(target_datasets)} datasets")
    print("="*70)
    
    all_results = []
    
    # Process each dataset
    for dataset_id, dataset_name in target_datasets:
        try:
            print(f"\n{'='*70}")
            print(f"PROCESSING: {dataset_name} (CC-18, OpenML ID: {dataset_id})")
            print(f"{'='*70}")
            
            # Load dataset
            X, y, categorical_cols, imbalance_ratio, dataset_name = load_openml_dataset(dataset_id)
            ...
            # Sanitize dataset name for filename (remove special characters)
            ...
            aggregate_file = results_dir / "cc18_aggregate_summary_latest.csv"
