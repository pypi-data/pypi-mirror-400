...
def get_schema_files(package_name):
    ...
    if os.path.isdir(node_dir):
        ...
        schema_files.extend(glob.glob(os.path.join(node_dir, "*.yaml")))
    return schema_files
...
class Channel:
    ...
    def __init__(self, channel_file):
        ...
        while not done:
            with open(channel_file, 'r') as f: cfg = yaml.safe_load(f)
                    ...
                    try:
                        with open(schema_file, "r") as f:
                            ...
                            if schema_module is None:
                                logger.warning(f"Node file '{schema_file}' is empty")
                                continue
                            if "schemas" not in schema_module:
                                logger.warning(f"Node file '{schema_file}' is missing 'schemas' key")
                                continue
                    except:
        ...
        for package_name in package_list:
            ...
            package_init = package.__file__
            package_dir = os.path.dirname(os.path.realpath(package_init))
            if os.path.isfile(os.path.join(package_dir, "package.yml")):
            ...
            elif os.path.isfile(os.path.join(package_dir, "package.yaml")):
                package_config_file = os.path.join(package_dir, "package.yaml")
                with open(package_config_file, "r") as f:
                    self.packages[package_name] = yaml.safe_load(f)
            else:
