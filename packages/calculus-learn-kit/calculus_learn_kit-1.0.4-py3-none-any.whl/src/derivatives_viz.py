...
class CommonDerivativesVisualizer:
  ...
  def visualize_function_and_derivative(self, func_str: str, x_range: Tuple[float, float] = (-3, 3), title: str = None) -> None:
    '''
  ...
  def slope_field_visualization(
    self, func_str: str, x_range: Tuple[float, float] = (-3, 3), y_range: Tuple[float, float] = (-3, 3)
  ) -> None:
    '''
  ...
  def derivative_chain_rule_examples(self) -> None:
    ...
    for example in examples:
      ...
      try:
        self.visualize_function_and_derivative(example['func_str'], (-2, 2), f"Chain Rule: f(x) = {example['function']}")
      except Exception as e:
        print(f"Could not visualize {example['func_str']}: {e}")

  def higher_order_derivatives(
    self, func_str: str = "x**4 - 4*x**3 + 6*x**2 - 4*x + 1", n_derivatives: int = 4
  ) -> None:
    '''
    ...
    for i, deriv in enumerate(derivatives):
      try:
        ...
        axes[i].plot(x_vals, y_vals, color=colors[i % len(colors)], linewidth=2, label=f'{order} = {deriv}')
  ...
  def critical_points_analysis(self, func_str: str = "x**3 - 3*x**2 + 2") -> None:
    ...
    if not analysis['critical_points']:
      return
    ...
    for point_data in analysis['analyzed_points']:
      ...
      ax1.plot(cp, y_cp, marker, color=color, markersize=10, label=f'{point_type} at x={cp:.2f}')
