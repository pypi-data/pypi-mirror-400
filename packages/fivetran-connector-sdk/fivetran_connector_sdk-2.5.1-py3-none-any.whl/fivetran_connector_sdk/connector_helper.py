...
def validate_naming(naming):
    ...
    if not naming:
        return None
    value_upper = naming.upper()
    if value_upper in (SOURCE_NAMING_VALUE, FIVETRAN_NAMING_VALUE):
        return value_upper
    print_library_log(
        f"Unrecognised naming strategy: '{naming}'. Must be either '{SOURCE_NAMING_VALUE}' or '{FIVETRAN_NAMING_VALUE}'.",
        Logging.Level.SEVERE)
    sys.exit(1)

def get_naming(args):
    """Fetch and validate naming field from """
    # Feature Disabled
    return None
...
def validate_requirements_file(project_path: str, is_deploy: bool, version: str):
    ...
    update_unused_requirements = verify_unused_deps(is_deploy, requirements, tmp_requirements, version)
...
def verify_unused_deps(is_deploy, requirements, tmp_requirements, version):
...
def run_pipreqs_with_retries(is_deploy, project_path, tmp_requirements_file_path):
    ...
    while attempt < MAX_RETRIES:
        ...
        if attempt < MAX_RETRIES:
            ...
            print_library_log(
                f"Skipping validation of requirements.txt due to error connecting to PyPI (Python Package Index) APIs. Continuing with {'deploy' if is_deploy else 'debug'}...",
                Logging.Level.SEVERE)
...
def package_project(connection_id: Optional[str], project_path: str, deploy_key: str) -> str:
    """Packages the project for deployment.

    Args:
        connection_id (Optional[str]): The connection ID, or None for new connections.
    ...
    """
    # For new connections, connection_id is None and we intentionally skip get_package_id.
    # In that case, existing_package_id remains None and upload_package will create a new package.
    existing_package_id: Optional[str] = (
        get_package_id(connection_id, deploy_key) if connection_id else None
    )
    ...
    try:
        ...
        if not uploaded_package_id:
            sys.exit(1)
        print_library_log(f"Package ID: {uploaded_package_id}")
...
def get_package_id(connection_id: str, deploy_key: str) -> Optional[str]:
    """Retrieves the package ID for the specified connection ID if it exists.
    Args:
        connection_id (str): The connection ID.
        deploy_key (str): The deployment key.

    Returns:
        Optional[str]: The package ID if found, None otherwise.
    """
    resp = rq.get(f"{constants.PRODUCTION_BASE_URL}/v1/connectors/{connection_id}",
                  headers={"Authorization": f"Basic {deploy_key}"})
    if not resp.ok:
        print_library_log(f"Error while fetching package details for connectionId: {connection_id}", Logging.Level.SEVERE)
        sys.exit(1)
    return resp.json().get('data', {}).get('config', {}).get('package_id')
...
def create_connection(deploy_key: str, group_id: str, config: dict, hd_agent_id: str, package_id: str, naming: str) -> rq.Response:
    ...
    Args:
        ...
        hd_agent_id (str): The hybrid deployment agent ID within the Fivetran system.
        package_id (str): The package ID.
        naming (enum): SOURCE/FIVETRAN- The naming choice passed during deploy.
    ...
    config["package_id"] = package_id
    destination_schema_names = FIVETRAN_NAMING_VALUE + UNDERSCORE_NAMING if not naming else naming + UNDERSCORE_NAMING
    response = rq.post(f"{constants.PRODUCTION_BASE_URL}/v1/connectors",
                       ...
                       json={
                           ...
                           "run_setup_tests": True,
                           "sync_frequency": "360",
                           "destination_schema_names": destination_schema_names,
...
def upload_package(local_path: str, package_id: Optional[str], deploy_key: str) -> Optional[str]:
    """Uploads a connector package to Fivetran's package management system.

    If package_id is provided, updates the existing package via PATCH.
    Otherwise, creates a new package via POST.

    Args:
        local_path (str): The absolute path to the package zip file.
        package_id (Optional[str]): Existing package ID to update, or None to create new.
    ...
    """
    if package_id:
        print_library_log("Overriding the existing package")
        url = f"{constants.PRODUCTION_BASE_URL}/v1/connector-sdk/packages/{package_id}"
        request_method = rq.patch
    else:
        print_library_log("Creating a new package for deployment...")
        url = f"{constants.PRODUCTION_BASE_URL}/v1/connector-sdk/packages"
        request_method = rq.post
    ...
    with open(local_path, 'rb') as f:
        response = request_method(url, files={'file': f}, headers=headers)
    ...
    print_library_log(f"Unable to upload the package, failed with error. {error_message}", Logging.Level.SEVERE)
    return None
...
def cleanup_uploaded_code(deploy_key: str, package_id: str) -> bool:
    ...
    """
    print_library_log(f"Cleaning up orphaned package: {package_id}")
    ...
    print_library_log(f"Unable to cleanup orphaned package, failed with error: {response.reason}",
                      Logging.Level.SEVERE)
    return False
...
def run_tester(java_exe_str: str, root_dir: str, project_path: str, port: int, state_json: str,
               configuration_json: str, profile: bool = False, naming: str = None):
    ...
    """
    working_dir = os.path.join(project_path, OUTPUT_FILES_DIR)
    naming = FIVETRAN_NAMING_VALUE if not naming else naming
    try:
