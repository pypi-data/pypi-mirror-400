...
COMMANDS = {
    "init": {"description": "Initialize configuration", "category": "Setup"},
    "start": {"description": "Start the scheduler daemon", "category": "Management"},
...
def format_help():
    ...
    header = "\nNyx CLI (v4.0.7) - AI Workload Orchestrator\nUsage: nyxcli <command> [arguments]\n"
    categories = {}
...
async def handle_init(config_arg=None):
    ...
    print("Configuration complete. Run 'nyxcli start' to launch the daemon.")


async def handle_start(config):
    """Start the scheduler daemon."""
    # Auto-launch terminal for user convenience
    print("Initializing environment...")
    try:
        await handle_terminal(config)
    except Exception as e:
        print(f"Warning: Could not launch terminal automatically: {e}")
