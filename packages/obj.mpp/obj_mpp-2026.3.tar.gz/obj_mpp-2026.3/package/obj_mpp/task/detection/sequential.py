...
from p_pattern.hint.domain import chunked_domain_h, domain_h, interval_h
from p_pattern.hint.instance import coordinate_h
...
def DetectObjectsInOneChunk(
    ...
    *,
    ...
    if (sampler is None) or (sampler.position is None):
        ...
        SendFeedback(-1, 0)
        return
    size, hierarchy = ObjectSize(sampler, should_return_details=True, lock=lock)
    if hierarchy is None:
        logger.info(f"Sampler size: {size:_}", **extra)
    else:
