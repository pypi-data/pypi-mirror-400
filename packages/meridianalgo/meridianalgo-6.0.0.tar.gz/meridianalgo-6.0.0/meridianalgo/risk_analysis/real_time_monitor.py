...
class RiskMetricCalculator:
    ...
    @staticmethod
    def calculate_portfolio_beta(portfolio_returns: pd.Series, 
...
class RealTimeRiskMonitor:
    ...
    def _update_risk_metrics(self) -> None:
        ...
        if hasattr(self, '_previous_portfolio_value') and self._previous_portfolio_value != 0:
            portfolio_return = (total_market_value - self._previous_portfolio_value) / self._previous_portfolio_value
        ...
        self.risk_metrics = {
            ...
            'portfolio_return_pct': (total_unrealized_pnl / (total_market_value - total_unrealized_pnl)) * 100 if total_market_value != total_unrealized_pnl else 0,
        ...
        if len(self.returns_history) >= 30:
            ...
            for confidence_level in self.var_confidence_levels:
                for method in self.var_methods:
                    ...
                    try:
                        var_value = RiskMetricCalculator.calculate_var(returns_series, confidence_level, method)
                        es_value = RiskMetricCalculator.calculate_expected_shortfall(returns_series, confidence_level)
                        
                        self.risk_metrics[var_key] = var_value
                        self.risk_metrics[es_key] = es_value
                        
                        # Convert to dollar terms
                        self.risk_metrics[f"{var_key}_dollar"] = var_value * total_market_value
                        self.risk_metrics[f"{es_key}_dollar"] = es_value * total_market_value
                        
                    except Exception as e:
                        logger.warning(f"Failed to calculate {var_key}: {e}")
            
            # Calculate drawdown metrics
            drawdown_metrics = RiskMetricCalculator.calculate_maximum_drawdown(returns_series)
    ...
    def _get_limit_value(self, limit: RiskLimit) -> float:
        ...
        if limit.limit_type == "var_95_historical":
            ...
            return self.risk_metrics.get("concentration_risk", {}).get("max_position_weight", 0)
        ...
        return {
            ...
            'risk_limits_status': {
                name: {
                    ...
                    'utilization': self._get_limit_value(limit) / limit.threshold if limit.threshold > 0 else 0
                }
