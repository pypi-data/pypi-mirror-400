...
With schema validation (Pydantic):
    ...
    with track(prompt_id="checkout_agent", schema=RefundResponse) as t:
        response = client.chat.completions.create(...)
        result = t.record(response)  # Validates and returns parsed object

Auto-wrapping (zero code changes):
...
class PromptTracker:
    ...
    def __init__(
        ...
        self.prompt_hash = hash_messages(messages) if messages else None
        self.tool_schema_hash = hash_tools(tools) if tools else None
        self.system_prompt_hash = hash_content(system_prompt) if system_prompt else None
    ...
    def record(
        ...
        *,
        ...
        # Extract response data based on provider
