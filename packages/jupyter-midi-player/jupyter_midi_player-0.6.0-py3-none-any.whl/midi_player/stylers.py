...
def _load_local_js():
    """Load self-hosted JavaScript libraries"""
    ...
    for js_file in js_files:
        ...
        if os.path.exists(file_path):
            with open(file_path, 'r', encoding='utf-8') as f:
                scripts.append(f'<script>{f.read()}</script>')
        else:
            # Fallback to CDN if local file not found
            print(f"Warning: {js_file} not found, using CDN fallback")
    
    return '\n'.join(scripts) if scripts else '''<script src="https://cdn.jsdelivr.net/combine/npm/tone@14.7.58,npm/@magenta/music@1.23.1/es6/core.js,npm/focus-visible@5,npm/html-midi-player@1.5.0"></script>'''

src_header = _load_local_js()

dl_str = lambda url, dl: f'<a href="{url}" target="_blank">Download MIDI</a><br>' if dl else ''



def basic(url, viz_type="piano-roll", dl=False, title='', sound_font=None):
    sf_attr = f'sound-font="{sound_font}"' if sound_font else 'sound-font'
    return f'''{src_header}  {title} {dl_str(url, dl)}
            <midi-player src="{url}" {sf_attr} visualizer="#myVisualizer"></midi-player>
...
def general(url, viz_type="piano-roll", dl=True, css='', title='', sound_font=None):
    ...
    return f'''{src_header}\n{css}
          ...
          <midi-player src={url} {sf_attr} visualizer="#section{secnum} midi-visualizer"></midi-player>
          <midi-visualizer src={url} type={viz_type}></midi-visualizer>
          </section>
          '''

def cifka_advanced(url, viz_type="piano-roll", dl=True, css=cifka_css, title='', sound_font=None):
...
def dark(url, viz_type="piano-roll", dl=True, css=dark_css, title='', sound_font=None):
    return general(url, viz_type=viz_type, dl=dl, css=css, title=title, sound_font=sound_font)
