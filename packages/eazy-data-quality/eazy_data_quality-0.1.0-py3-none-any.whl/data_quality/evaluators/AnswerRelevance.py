...
class AnswerRelevanceEvalProcessor(BaseProcessor):
    """三元组评估处理器"""

    def __init__(self, yaml_path='prompt/answer_relevance_prompt.yaml', required_columns=None):
        extractor = YamlVariableExtractor(yaml_path)
        self.var_list = extractor.extract_variables()  # required_columns顺序和var_list一致
        if required_columns is None:
            required_columns = ['正确上下文', '问题', '标准答案']
