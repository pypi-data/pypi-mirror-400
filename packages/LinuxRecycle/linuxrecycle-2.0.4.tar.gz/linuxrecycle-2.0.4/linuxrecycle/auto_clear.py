...
import shutil
import os
from linuxrecycle import config, DB
...
def update(rows):
    ...
    conn = DB.conn_to_db(config.local_config.get("core", "data_base_file"))
    try:
        cursor = conn.cursor()
...
def main():
    ...
    """
    data_base_file = config.local_config.get("core", "data_base_file")
    ...
    print(
        "delete files more than {} days".format(
