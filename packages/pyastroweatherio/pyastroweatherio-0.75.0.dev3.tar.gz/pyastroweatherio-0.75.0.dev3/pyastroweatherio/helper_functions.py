...
class AtmosphericRoutines:
    ...
    async def calculate_seeing_11(
        ...
        """
        Seeing estimate in arcseconds (lower = better), tuned to behave more like
        common forecast products (e.g., meteoblue) in terms of directionality.

        Still a heuristic: with only surface inputs you cannot model free-atmosphere seeing.
        """
        values = {
            "temperature": temperature,
            "humidity": humidity,
        ...
        # Clamp to your global max (if you have one)
        seeing = max(0.4, min(float(SEEING_MAX), seeing))

        # _LOGGER.debug(float(seeing))
        return float(seeing)
