...
class MQISolver:
    ...
    def _ensure_initialized(self) -> None:
        ...
        if not self._initialized:
            ...
            try:
                ...
                # Define helper predicates for constraint solving
                query_once("assert((all_different(List) :- all_distinct(List)))")
                query_once("assert((exactly_one(Item, List) :- include(=(Item), List, Matches), length(Matches, 1)))")
    ...
    def query_prolog(
        ...
        with self._lock:
            ...
            try:
                # Build Prolog program from fact strings
                program = "\n".join(facts)
...
class MQIPooledSolver:
    ...
    def query(
        ...
        if not self.solver:
            ...
            return execute_query(facts, rules_program, query_text, max_solutions)
