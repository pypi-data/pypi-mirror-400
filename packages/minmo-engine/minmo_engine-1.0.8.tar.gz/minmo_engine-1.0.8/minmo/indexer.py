...
class SQLiteFTS5Storage(IndexStorage):
    ...
    def __init__(self, db_path: str | None = None):
        if db_path is None:
            # 기본 경로: .minmo/indexer.db
            minmo_dir = Path.cwd() / ".minmo"
            minmo_dir.mkdir(parents=True, exist_ok=True)
            db_path = str(minmo_dir / "indexer.db")
...
class CodeIndexer:
    ...
    def __init__(
        ...
        if storage is None:
            # 기본 경로: .minmo/indexer.db (프로젝트 루트 기준)
            minmo_dir = self.project_path / ".minmo"
            minmo_dir.mkdir(parents=True, exist_ok=True)
            self.storage = SQLiteFTS5Storage(str(minmo_dir / "indexer.db"))
        else:
