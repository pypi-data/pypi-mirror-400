...
WHERE           status_subquery.status_date BETWEEN
                    To_date({logged_date_from}, 'yyyy-mm-dd HH24:MI:SS') --arg logged_date_from (:class:`pandas.Timestamp`): earliest date to search for 
            AND     To_date({logged_date_to}, 'yyyy-mm-dd HH24:MI:SS')   --arg logged_date_to (:class:`pandas.Timestamp`): latest date to search for
ORDER BY        dh_no,
                logged_date