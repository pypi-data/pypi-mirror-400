...
def property_updater(kb_state, sent):
    ...
    and calls the :func:`property_update_alt() <pydialogue.state.kn_updaters.property_update_alt>` function.
...
def property_update_alt(kb_state, ent, pkey, pval, pneg):
    ...
    ..
        ...
        :attr:`ent.prop_seen <pydialogue.environment.entities.Entity.prop_seen>`,
        :attr:`ent.prop_seen_neg <pydialogue.environment.entities.Entity.prop_seen_neg>`,

        :attr:`ent.attr_seen <pydialogue.environment.entities.Entity.attr_seen>`,
        :attr:`ent.attr_seen_neg <pydialogue.environment.entities.Entity.attr_seen_neg>`
...
def multiple_prop_updater(kb_state, sent):
    mem = kn_helpers.check_prop(sent)
    if mem is not None:
        ent, pkey, pval, pneg = mem
        if isinstance(pval, (set)):
            for val in pval:
                property_update_alt(kb_state, ent, pkey, val, pneg)
...
def elem_exists_alt_updater(kb_state, ent, elem, pneg):
    ...
    Updates one of the following dictionaries:
        ...
        :attr:`ent.elem_exists <pydialogue.environment.entities.Entity.elem_exists>` or
        :attr:`ent.elem_not_exists <pydialogue.environment.entities.Entity.elem_not_exists>`
        ...
        if lc.verb_inf(rel.lower()) == "go" and lc.verb_tense(rel) in lc.PRESENT_TENSES:
            ...
            if mapped_desc == describer:
                if not (isinstance(start_point, list) and start_point[0] == "from"
                    ...
                    return None
                player_look = sent.describers[1].get_arg("Arg-PAG")
                thing_looked = sent.describers[1].get_arg("Arg-PPT")
                ...
                if lc.verb_inf(rel_look.lower()) == "look" and lc.verb_tense(rel_look) in lc.PRESENT_TENSES:
                    ...
                    if (look_desc == sent.describers[1] and player == player_look and isinstance(direction, str)
                        ...
                        if direction is not None and player is not None:
                            ...
                            old_loc = start_point[1]
                            property_update_alt(kb_state, old_loc, "location", old_loc.properties['location'], None)
                        property_update_alt(kb_state, player, "location", thing_looked, None)
                    looks_and_sees = lc.Sentence()
                    looks_and_sees.describers.extend(sent.describers[1:])
                    look_updater(kb_state, looks_and_sees)
    return None
...
def look_updater(kb_state, sent):
    ...
    if len(sent.describers) > 0:
        ...
        if lc.verb_inf(rel.lower()) != "look" or lc.verb_tense(rel) not in lc.PRESENT_TENSES:
            return
        player = describer.get_arg("Arg-PAG")
        thing_looked = describer.get_arg("Arg-PPT")
    ...
    if len(sent.describers) > 0:
        ...
        if rel == 'permitted':
            ...
            dropping_items = tsentences.drop(rel=("dropping", None),
                                             entity=(["the", "item", set(kb_state.world.location_positions), "itself"], None),
                                             )
            dropping_not_permitted = tsentences.permit(action_allowed=dropping_items,
                                                       neg=("not", None),
                                                       rel=("permitted", None))

            if sent in [getting_players, dropping_not_permitted]:
                basic_updater(kb_state, sent)
