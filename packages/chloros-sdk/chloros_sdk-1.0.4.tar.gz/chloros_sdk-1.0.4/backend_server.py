...
# ========================================
# GOOGLE MAPS API RATE LIMITING & QUOTAS
# ========================================
# Protects against API abuse and excessive usage

class MapsAPIRateLimiter:
    """Rate limiter and quota manager for Google Maps API access"""
    ...
    def __init__(self):
        ...
        self.sessions = {}  # Track sessions by identifier

        # Configuration - adjust these values as needed
        self.rate_limit_window = 60  # seconds
        self.rate_limit_max_requests = 10  # max requests per window
        self.daily_quota = 1000  # max requests per day
        self.session_quota = 100  # max requests per session
    ...
    def _get_state_file(self):
        ...
        state_dir = os.path.join(app_data, 'Chloros', 'config')
        os.makedirs(state_dir, exist_ok=True)
        return os.path.join(state_dir, 'maps_usage.json')
    ...
    def _load_state(self):
        ...
        try:
            ...
            logger.error(f"Failed to load maps usage state: {e}")
        ...
        try:
            ...
            with open(state_file, 'w') as f:
                json.dump({
                    ...
                    'last_reset_date': self.last_reset_date or datetime.datetime.now().strftime('%Y-%m-%d'),
                    'last_updated': datetime.datetime.now().isoformat()
                }, f)
        except Exception as e:
            logger.error(f"Failed to save maps usage state: {e}")

    def check_rate_limit(self, session_id=None):
        """
        ...
        with self.lock:
            ...
            if self.last_reset_date != today:
                self.daily_count = 0
                self.last_reset_date = today
                self.sessions = {}  # Reset session counts too

            # Check daily quota
            if self.daily_count >= self.daily_quota:
                seconds_until_midnight = self._seconds_until_midnight()
                return False, 'daily_quota_exceeded', seconds_until_midnight

            # Check session quota
            if session_id:
                session_count = self.sessions.get(session_id, 0)
                if session_count >= self.session_quota:
                    return False, 'session_quota_exceeded', 3600  # 1 hour
            ...
            # Check rate limit
            if len(self.requests) >= self.rate_limit_max_requests:
                oldest_in_window = min(self.requests) if self.requests else now
                retry_after = int(self.rate_limit_window - (now - oldest_in_window)) + 1
    ...
    def _seconds_until_midnight(self):
        """Calculate seconds until midnight for quota reset"""
    ...
    def get_usage_stats(self):
        ...
        with self.lock:
            ...
            return {
                ...
                'daily_remaining': max(0, self.daily_quota - self.daily_count),
...
maps_rate_limiter = MapsAPIRateLimiter()
...
# Google Maps API key - loaded from environment variable or config file
def _load_maps_api_key():
    """Load Google Maps API key from secure sources (not hardcoded)"""
    # 1. Check environment variable first (most secure for deployment)
    key = os.environ.get('GOOGLE_MAPS_API_KEY')
    if key:
        return key

    # 2. Check config file in user's local app data (not in repo)
    config_path = os.path.join(
        os.environ.get('LOCALAPPDATA', os.path.join(os.path.expanduser('~'), 'AppData', 'Local')),
        'Chloros', 'config', 'maps_config.json'
    )
    if os.path.exists(config_path):
        try:
            ...
            logger.error(f"Failed to load maps config: {e}")
...
def get_maps_config():
    """Return Maps API key with rate limiting protection"""
    try:
        ...
        if not GOOGLE_MAPS_API_KEY:
            return jsonify({
                ...
                'message': 'Google Maps API key not configured. Set GOOGLE_MAPS_API_KEY environment variable or create config file.',
                'config_path': os.path.join(
        ...
        # Get session identifier from request (use IP + User-Agent as fallback)
        session_id = request.headers.get('X-Session-ID')
        if not session_id:
            # Generate session ID from request characteristics
            session_id = f"{request.remote_addr}:{hash(request.headers.get('User-Agent', ''))}"

        # Check rate limits
        allowed, reason, retry_after = maps_rate_limiter.check_rate_limit(session_id)

        if not allowed:
            response = jsonify({
                'success': False,
                'error': reason,
                'message': _get_rate_limit_message(reason),
        ...
        # Return API key with usage info
        stats = maps_rate_limiter.get_usage_stats()
        return jsonify({
            'success': True,
        ...
        logger.error(f"Error in maps config: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500

def _get_rate_limit_message(reason):
    """Get user-friendly rate limit message"""
    messages = {
        'rate_limit_exceeded': 'Too many requests. Please wait a moment before trying again.',
        'daily_quota_exceeded': 'Daily usage limit reached. Quota resets at midnight.',
        'session_quota_exceeded': 'Session usage limit reached. Please restart the application.'
    }
    return messages.get(reason, 'Request limit exceeded.')

@app.route('/api/maps/usage', methods=['GET'])
def get_maps_usage():
    """Get Maps API usage statistics"""
    try:
        stats = maps_rate_limiter.get_usage_stats()
        return jsonify({
            'success': True,
