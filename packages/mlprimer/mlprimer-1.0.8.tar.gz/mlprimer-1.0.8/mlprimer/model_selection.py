...
class ModelRegistry:
    ...
    def get_ordinal_models(
        ...
        """
        Get ordinal regression model templates (for ranked/ordered targets).
        
        Ordinal models respect the ordering of target classes (e.g., 1 < 2 < 3).
        They penalize large prediction errors more than small ones.
        ...
        dict
            Model name -> unfitted model instance
        """
        # Start with regression models (which respect ordering)
        # since ordinal regression is often treated as regression with ordered targets
        models = {
            # Linear models
            "Ridge (Ordinal)": Ridge(alpha=1.0),
            "ElasticNet (Ordinal)": ElasticNet(alpha=0.1, l1_ratio=0.5, max_iter=2000, random_state=42),
            
            # Tree-based (naturally handle ordering through splits)
            "Decision Tree": DecisionTreeRegressor(
                max_depth=10,
                min_samples_split=5,
                random_state=42,
            ...
            # Distance-based
            "k-NN (Ordinal)": KNeighborsRegressor(n_neighbors=5, n_jobs=-1),
            "SVR": SVR(kernel="rbf", C=1.0, gamma="scale"),
        }
        ...
        if include_advanced:
            if XGBOOST_AVAILABLE:
                models["XGBoost (Ordinal)"] = xgb.XGBRegressor(
                    n_estimators=100,
                    learning_rate=0.1,
                    max_depth=5,
                    subsample=0.8,
            ...
            if LIGHTGBM_AVAILABLE:
                models["LightGBM (Ordinal)"] = lgb.LGBMRegressor(
                    n_estimators=100,
                    learning_rate=0.1,
                    max_depth=7,
                    num_leaves=31,
...
def suggest_models(
    target_type: Literal["binary", "multiclass", "ordinal", "continuous", "count"],
    n_samples: int,
    n_features: int,
    ...
    ----------
    target_type : str
        From infer_target_type(): "binary", "multiclass", "ordinal", "continuous", "count"
    n_samples : int
        Number of samples
    n_features : int
        Number of features
    ...
    if target_type == "ordinal":
        # Ordinal regression suggestions - use regression models that respect ordering
        suggestions.append("Ridge (Ordinal)")
        suggestions.append("Random Forest")
        suggestions.append("Extra Trees")
        suggestions.append("Gradient Boosting")
        suggestions.append("ElasticNet (Ordinal)")
        
        if include_advanced:
            if XGBOOST_AVAILABLE:
                suggestions.append("XGBoost (Ordinal)")
            if LIGHTGBM_AVAILABLE:
                suggestions.append("LightGBM (Ordinal)")
        
        if is_high_dim or is_collinear:
            suggestions.insert(1, "Ridge (Ordinal)")
        
        if is_small_sample:
            suggestions.append("k-NN (Ordinal)")
...
def get_metrics(
    ...
    elif task in ("ordinal", "regression", "count"):
        return {
            "r2": "r2",
...
def get_cv_splitter(
    task: Literal["binary", "multiclass", "ordinal", "regression", "count"],
    y: pd.Series,
    n_splits: int = 5,
...
def train_models(
    ...
    y_train: pd.Series,
    task: Literal["binary", "multiclass", "ordinal", "regression", "count"] = "binary",
    model_names: Optional[List[str]] = None,
    cv: int = 5,
    scale_features: bool = True,
    sample_weight: Optional[np.ndarray] = None,
) -> Tuple[Dict[str, Any], pd.DataFrame, StandardScaler, Dict[str, Dict[int, str]]]:
    """
    ...
    -------
    tuple
        (trained_models_dict, cv_results_dataframe, scaler, categorical_encodings)
    """
    # Select model registry
    if task in ("binary", "multiclass"):
        all_models = ModelRegistry.get_classification_models()
    elif task == "ordinal":
        all_models = ModelRegistry.get_ordinal_models()
    else:
    ...
    # Store categorical encodings
    categorical_encodings = {}
    if categorical_cols:
        for col in categorical_cols:
            unique_cats = X[col].unique()
            cat_codes = pd.Categorical(unique_cats).codes
            categorical_encodings[col] = {int(code): str(cat) for code, cat in zip(cat_codes, unique_cats)}
    ...
    return trained_models, cv_results_df, scaler, categorical_encodings
...
def run_full_model_selection(
    ...
    y_test : pd.Series
        Test target
    target_type : str
        From preprocessing.infer_target_type(): "binary", "multiclass", "ordinal", "continuous", "count"
    ...
    # Step 2: Train models
    trained_models, cv_results, feature_scaler, categorical_encodings = train_models(
        X_train, y_train,
        task=task,
...
def train_models_legacy(X_train: pd.DataFrame, y_train: pd.Series) -> Dict[str, Any]:
    ...
    trained_models, cv_results, _, _ = train_models(
        X_train, y_train,
        task="binary",
