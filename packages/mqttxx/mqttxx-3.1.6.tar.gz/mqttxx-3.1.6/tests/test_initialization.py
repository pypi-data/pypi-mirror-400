...
import asyncio
import pytest
from mqttxx import MQTTClient, MQTTConfig, ReconnectConfig, RPCManager, EventChannelManager
    ...
    async with MQTTClient(config) as mqtt:
        ...
        async with MQTTClient(caller_config) as caller:
            ...
            caller_rpc = RPCManager(
                caller,
                my_topic=f"caller/{caller_config.client_id}"
            )
...
async def test_events_and_rpc_coexist():
    ...
    async with MQTTClient(config) as mqtt:
        ...
        async with MQTTClient(test_config) as test_client:
            ...
            test_rpc = RPCManager(
                test_client,
                my_topic=f"test/{test_config.client_id}"
            )
