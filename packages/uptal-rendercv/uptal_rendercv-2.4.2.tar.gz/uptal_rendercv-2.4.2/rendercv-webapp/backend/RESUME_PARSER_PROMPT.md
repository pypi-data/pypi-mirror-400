...
### References Section
Each reference entry MUST use BulletEntry format:
```json
{
  "bullet": "string"
}
...
- If the resume says "References available upon request" or similar → single bullet: "Available upon request"
- If actual references are provided → format each as: "Name - Title, Organization - Email - Phone"
- Examples:
  - `{"bullet": "Available upon request"}`
  - `{"bullet": "Dr. Jane Smith - Professor, MIT - jsmith@mit.edu - +1-555-0123"}`
  - `{"bullet": "John Doe - Senior Manager, Tech Corp - john.doe@techcorp.com"}`

**Important:** Look for actual names with contact information before defaulting to "Available upon request"
...
10. **References Section**:
   - Look for sections titled: References, Professional References, Referees, Recommendations
   - Also check the END of the resume (references often appear last)
   - Detection patterns:
     - Names followed by titles and organizations
     - Email addresses (@ symbol) near names
     - Phone numbers near names
     - Words like "Professor", "Manager", "Director" near names
   - Extraction priority:
     1. FIRST check if actual references with names exist
     2. Look for contact details (email, phone) as confirmation
     3. ONLY use "Available upon request" if no actual references found
   - Format each reference as: "Name - Title, Organization - Contact"
   - Common variations to detect:
     - "References: Available upon request" → single bullet
     - Name with title and contact info → extract and format
     - Multiple names listed → create bullet for each
...
REFERENCES
Dr. Sarah Johnson - Professor, Stanford University - sjohnson@stanford.edu - (555) 234-5678
Michael Chen - Senior Manager, Tech Corp - m.chen@techcorp.com - (555) 345-6789
Available upon request for additional references
```
...
```json
{
  "cv": {
    ...
    "sections": {
      ...
      "references": [
        {"bullet": "Dr. Sarah Johnson - Professor, Stanford University - sjohnson@stanford.edu - (555) 234-5678"},
        {"bullet": "Michael Chen - Senior Manager, Tech Corp - m.chen@techcorp.com - (555) 345-6789"},
        {"bullet": "Available upon request for additional references"}
      ]
    }
  },
  "design": {
