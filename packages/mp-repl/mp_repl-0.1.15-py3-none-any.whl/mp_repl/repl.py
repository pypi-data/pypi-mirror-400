...
class Repl:
    ...
    async def run(self):
        print("pw-repl v0.1.15 - Type 'help' for commands")
        # 自动重连上次的连接
    ...
    async def cmd_run(self, filepath: str):
        ...
        # debug 函数
        def debug():
            builtins.print("🔴 Debug breakpoint (continue not implemented yet)")
            sys.stdout.flush()
    ...
    async def cmd_pages(self):
        ...
        try:
            ...
            for i, t in enumerate(targets):
                ...
                print(f"  {mark} [{i}] {url}")
            self._targets = targets  # 保存用于 page 命令
        except Exception as e:
    ...
    async def cmd_history(self, pattern: str = None):
        ...
        for i, cmd in enumerate(items):
            seen[cmd] = i
        items = [cmd for cmd, idx in sorted(seen.items(), key=lambda x: x[1])]
