...
class LangGraphAuthBackend(AuthenticationBackend):
    ...
    def _locate_auth_file(self) -> Path | None:
        """Find auth.py by checking repo root first, then cwd."""
    ...
    def _load_auth_instance(self) -> Auth | None:
        ...
        try:
            # Import the auth instance from the project auth.py
            auth_path = self._locate_auth_file()
            if auth_path is None:
                logger.warning("Auth file not found in repo root or current working directory")
                return None
