...
class Auth:
    ...
    def make_signature(self, method: str, relative_url: str) -> str:
        ...
        # Try splitting single-line PEMs (header+body+footer all on one line)
        if private_key_obj is None and pk_source.count("-----") == 2 and "\n" not in pk_source:
            m = re.match(r"(-----BEGIN [^-]+-----)(.+)(-----END [^-]+-----)", pk_source)
            if m:
        ...
        if private_key_obj is None:
            ...
            msg = (
                ...
                "Common causes: key missing PEM headers, escaped newlines (\\n) not converted, single-line PEMs, or malformed base64 framing. "
