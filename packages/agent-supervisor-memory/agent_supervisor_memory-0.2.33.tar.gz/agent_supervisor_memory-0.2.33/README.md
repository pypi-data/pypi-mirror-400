...
- 摘要优先：`project_status` 返回每个 task 的最近摘要行（优先读取 dispatch 的 `summary.json`，缺失时从 events 生成并回写）。
- 并行冲突控制（轻量）：task 可声明 `locks:[\"path/prefix\"]`（按前缀匹配），`project_run` 会避免同时运行冲突锁的任务。
- Patch-only（默认）：`project_run` 会为每个 task 创建独立 git worktree 并在结束后生成 `git diff` patch，落盘到 `.codex/projects/<project_id>/patches/`（主工作区不被直接修改）。
