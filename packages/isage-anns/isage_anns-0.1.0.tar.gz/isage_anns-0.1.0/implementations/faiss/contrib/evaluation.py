...
def range_PR_multiple_thresholds(
    ...
    recalls = np.zeros(nt)
    for t in range(nt):
        p, r = counts_to_PR(counts[:, t, 0], counts[:, t, 1], counts[:, t, 2], mode=mode)
        precisions[t] = p
        recalls[t] = r
...
class OperatingPointsWithRanges(OperatingPoints):
    ...
    def sample_experiments(self, n_autotune, rs=np.random):
        ...
        if n_autotune == 0 or totex < n_autotune:
            ...
            experiments = rs.choice(totex - 2, size=n_autotune - 2, replace=False)
    ...
    def get_parameters(self, k):
        ...
        return {name: values[k[i]] for i, (name, values) in enumerate(self.ranges)}
