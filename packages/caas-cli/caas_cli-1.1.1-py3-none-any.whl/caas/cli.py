...
# src/caas/cli.py  (replace ONLY your show_port with this)

from .memory import list_positions
from .pnl import enrich_pending_trades_now
...
def show_port():
    ...
    for it in port.items:
        # round token amounts to nearest hundredth
        amt = f"{it.ui_amount:,.2f}"
        ...
        if p:
            ...
            if cost > 0:
                ...
                # round pnl to nearest hundredth; pct to 1 decimal (looks best)
                ...
                pnl_suffix = f" [{style}]({sign}${pnl_rounded:,.2f} {pct_rounded:+.1f}%)[/{style}]"
