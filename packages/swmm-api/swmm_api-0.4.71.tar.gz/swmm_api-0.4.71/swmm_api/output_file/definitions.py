...
class SUBCATCHMENT_VARIABLES:
    ...
    Attributes:
        RAINFALL (str): "rainfall" - Rainfall (Precipitation) intensity [mm/hr] or [in/hr]
        SNOW_DEPTH (str): "snow_depth" - snow depth [mm] or [in]
        EVAPORATION (str): "evaporation" - Evaporation loss [mm/day] or [in/day]
        INFILTRATION (str): "infiltration" - Infiltration loss [mm/h] or [in/hr]
        ...
        GW_ELEVATION (str): "groundwater_elevation" - elevation of saturated gw table [m] or [ft]
...
class NODE_VARIABLES:
    ...
    Attributes:
        DEPTH (str): 'depth' | water depth (ft or m above the node invert elevation)
        HEAD (str): 'head' | hydraulic head (ft or m, absolute elevation per vertical datum)
        VOLUME (str): 'volume' | stored water volume (including ponded water, ft3 or m3)
        LATERAL_INFLOW (str): 'lateral_inflow' | lateral inflow (runoff + all other external inflows, in flow units)
        TOTAL_INFLOW (str): 'total_inflow' | total inflow (lateral inflow + upstream conduit inflows, in flow units)
        FLOODING (str): 'flooding' | surface flooding (excess overflow when the node is at full depth, in flow units)
        LIST_ (list[str]): list of all variables
    """
...
class LINK_VARIABLES:
    ...
    Attributes:
        FLOW (str): 'flow' | flow rate (flow units)
        DEPTH (str): 'depth' | average water depth (ft or m)
        VELOCITY (str): 'velocity' | flow velocity (ft/sec or m/sec)
        VOLUME (str): 'volume' | volume of water (ft3 or m3)
        CAPACITY (str): 'capacity' | capacity (fraction of full area filled by flow for conduits; control setting for pumps and regulators)
        LIST_ (list[str]): list of all variables
    """
...
class SYSTEM_VARIABLES:
    ...
    Attributes:
        AIR_TEMPERATURE (str): 'air_temperature' - [°C] or [°F]
        RAINFALL (str): 'rainfall' - Total rainfall (Precipitation) intensity [mm/hr] or [in/hr]
        SNOW_DEPTH (str): 'snow_depth' - Total snow depth [mm] or [in]
        INFILTRATION (str): 'infiltration' - sum in infiltration of all sub-catchments (weighted by the sc-area) [mm/hr] or [in/hr]
        ...
        VOLUME (str): 'volume' - Total nodal storage volume in the system [m³] or [ft³]
        EVAPORATION (str): 'evaporation' - Actual evaporation [mm/day] or [in/day]
        PET (str): 'PET' - Potential evaporation [mm/day] or [in/day]
        LIST_ (list[str]): list of all variables
    """
