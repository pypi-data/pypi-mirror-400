...
class PaymentClient:
    ...
    def __init__(self, api_key: str, api_secret: str, base_url: Optional[str] = None) -> None:
        ...
        self.authenticator = Auth(api_key, api_secret)
        self.is_test = self.check_is_test(api_secret)
        self.base_url = self.__trim_base_url(base_url) if base_url else "https://api.miropay.example"

        # simple httpx client with sensible defaults
    ...
    # ------------------------------- Basic Methods ------------------------------ #
    def __call(self, path: str, method: str, request_body: Optional[Union[str, bytes]] = None) -> Dict[str, Any]:
        ...
        try:
            if request_body is None:
                resp = self.http_client.request(method.upper(), versioned_url, headers=headers)
            else:
                # request_body is expected to be a JSON string or bytes
                resp = self.http_client.request(method.upper(), versioned_url, content=request_body, headers=headers)
    ...
    def get_public_keys(self) -> Dict[str, Any]:
        return self.__call("/get-public-keys", "GET", None)

    # ======================== Public Methods =======================
    def get_payment_by_id(self, reference_code: str) -> Dict[str, Any]:
        return self.__call(f"/status/{reference_code}", "GET", None)
    ...
    def create_payment(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        json_payload = {
            ...
            "collectFeeFromCustomer": payload.get("collectFeeFromCustomer"),
            "collectCustomerEmail": payload.get("collectCustomerEmail"),
            "collectCustomerPhoneNumber": payload.get("collectCustomerPhoneNumber"),
        }

        return self.__call("/create", "POST", json.dumps(json_payload).encode("utf-8"))

    def cancel_payment(self, reference_code: str) -> Dict[str, Any]:
        return self.__call(f"/cancel/{reference_code}", "PATCH", None)

    def verify(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        # ensure public keys cached
        if not self.public_keys:
            resp = self.get_public_keys()
