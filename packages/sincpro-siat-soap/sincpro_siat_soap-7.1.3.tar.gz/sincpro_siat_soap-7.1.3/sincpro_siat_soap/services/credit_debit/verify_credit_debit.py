...
class CmdVerifyCreditDebitState(DataTransferObject):
    document_type: int
    emission_code: int
    point_of_sale: int
    system_code: str
...
class ResVerifyCreditDebitState(DataTransferObject):
    reception_code: str | None
    literal_status: SectorDocumentState | str
    raw_response: Any


@siat_soap_sdk.feature(CmdVerifyCreditDebitState)
class VerifyCreditDebitState(Feature):

    def execute(self, dto: CmdVerifyCreditDebitState) -> ResVerifyCreditDebitState:
        response = self.soap_client(
            SIAT_WSDL.NOTA_DE_CREDITO
        ).service.verificacionEstadoDocumentoAjuste(
        ...
        return ResVerifyCreditDebitState(
