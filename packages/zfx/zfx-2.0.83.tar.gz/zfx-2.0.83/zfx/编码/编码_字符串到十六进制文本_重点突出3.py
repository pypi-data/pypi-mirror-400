def 编码_字符串到十六进制文本_重点突出3(完整字符串, 重点字符串1, 重点字符串2, 重点字符串3):
    ...
    try:
        ...
        highlight_bytes3 = 重点字符串3.encode()
        hex_text_list = []
        i = 0
        ...
        while i < len(byte_text):
            ...
            elif byte_text[i:i+len(highlight_bytes3)] == highlight_bytes3:
                # 如果匹配到重点字符串3
                hex_str = ' '.join(f"{b:02x}" for b in byte_text[i:i+len(highlight_bytes3)])
                hex_text_list.append(f"\033[93m{hex_str}\033[0m")
                i += len(highlight_bytes3)
            else:
                # 处理单字节和多字节字符
                if byte_text[i] < 128:
                    # 单字节字符
