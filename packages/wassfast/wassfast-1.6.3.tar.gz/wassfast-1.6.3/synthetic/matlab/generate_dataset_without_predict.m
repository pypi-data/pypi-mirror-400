...
Nt = 10; % Number of frames in each scenario
SCENARIOS = 20000;  % Number of simulation instances
...
%OUT_FILE = sprintf('waves_%dx%d_%dfps_multiple_scenario.h5',Nu,Nu,fps);
OUT_FILE = sprintf('waves_%dx%d.h5',Nu,Nu);
...
%%
    ...
    for curridx=4:(size(ZZ,1)-2)
        ...
        if RENDER_DEBUG_FRAMES
            ...
            imwrite(zimg,sprintf('fig/%05d.png',scn+5612));
            break % Save a single frame for each scenario
        end
    ...
    %h5create( OUT_FILE, [dataset,'data'], [ Nu, Nu, size(Zd,1) ], 'Chunksize', [Nu,Nu,Nt], 'Datatype', 'single' );
