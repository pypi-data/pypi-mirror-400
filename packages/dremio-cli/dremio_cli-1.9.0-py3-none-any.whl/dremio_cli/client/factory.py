...
from dremio_cli.client.auth import authenticate_with_username_password, exchange_external_jwt
...
def _get_token(profile: Dict[str, Any], auth: Dict[str, Any], base_url: str) -> str:
    ...
    elif auth_type == "external_jwt":
        external_token = auth.get("external_token")
        if not external_token:
            # Fallback for older profiles or misconfiguration
            external_token = auth.get("token")
            
        if not external_token:
             raise ConfigurationError("EXTERNAL_JWT auth requires 'external_token' field")
             
        # Exchange for Dremio Token
        token = exchange_external_jwt(base_url, external_token)
        return token
