...
PHASE_FLOOR_MS: dict[str, int] = {
    # Planner is often the longest phase; start with a larger floor to reduce early polling spam.
    "planner_run": 20_000,
    "review": 10_000,
    "coder_run": 8_000,
    "integrate": 10_000,
...
def _fmt_elapsed_bucket(seconds: int) -> str:
    if seconds < 60:
        return "<1m"
    minutes = seconds // 60
    if minutes < 60:
...
def _fmt_activity_bucket(age_s: int) -> str:
    if age_s < 10:
        return "<10s"
    if age_s < 60:
        return "<60s"
...
class RunState:
    ...
    phase: str = "planner_run"  # planner_run|coder_run|integrate|cleanup|done|canceled|error
    ...
    def public_status(self, *, options: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if cache:
            ...
            elapsed_ms = int((now - cached_at).total_seconds() * 1000)
            if elapsed_ms < int(cache.recommended_poll_ms):
                remaining_ms = max(1000, int(cache.recommended_poll_ms) - elapsed_ms)
                # Return a condensed wait hint to reduce repetitive context.
                base = cache.lines[0] if cache.lines else f"模式: {self.mode} | 阶段: {self.phase}"
                lines = [base, f"无新进展 | 强制等待: {_round_wait_s(remaining_ms)}s"]
                return {
                    "run_id": self.run_id,
                    "next_action": self.next_action,
                    "recommended_poll_ms": remaining_ms,
        ...
        try:
            ...
            if activity_age is not None and self.phase in {"planner_run", "coder_run"}:
                activity_hint = f"活跃: {_fmt_activity_bucket(int(activity_age))}"
        except Exception:
            activity_hint = ""

        # Keep planner/coder phases concise by default (2 lines) to avoid context spam;
        # detailed lines are reserved for transitions (review/done/error).
        lines: list[str] = [
            f"模式: {self.mode} | 阶段: {self.phase}" + (f" | {stage_hint}" if stage_hint else "") + f" | 动作: {self.next_action}",
            task_summary,
            f"最近: {self.important_event} | 已等待: {_fmt_elapsed_bucket(elapsed_s)}"
            + (f" | {activity_hint}" if activity_hint else "")
            + (f" | {artifact_hint}" if artifact_hint else "")
            + f" | 强制等待: {_round_wait_s(poll_ms)}s",
        ]
        brief = (self.requirements_brief or "").strip()
        if brief:
            lines[1] = f"{task_summary} | 摘要: {brief[:160]}"

        if self.phase in {"planner_run", "coder_run"}:
            condensed = [
                lines[0],
                f"{lines[1]} | {activity_hint or '活跃: -'} | 强制等待: {_round_wait_s(poll_ms)}s",
            ]
            self.recommended_poll_ms = poll_ms
            self.brief_cache = BriefCache(generated_at=utc_now_iso(), recommended_poll_ms=poll_ms, lines=condensed[:3])
            return {
                "run_id": self.run_id,
                "next_action": self.next_action,
