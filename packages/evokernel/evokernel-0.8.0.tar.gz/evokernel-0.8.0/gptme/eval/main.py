...
def docker_reexec(argv: list[str]) -> None:
    ...
    # Collect environment variables to pass through - OpenRouter only
    env_vars_to_pass = [
        # "ANTHROPIC_API_KEY",
        # "OPENAI_API_KEY",
        "OPENROUTER_API_KEY",
...
def main(
    ...
    # Generate model+format combinations - OpenRouter only
    default_models = []
    ...
    if config.get_env("OPENROUTER_API_KEY"):
        default_models.extend(
            [
                "openrouter/anthropic/claude-sonnet-4.5@tool",
                "openrouter/anthropic/claude-sonnet-4.5@xml",
            ]
        )
