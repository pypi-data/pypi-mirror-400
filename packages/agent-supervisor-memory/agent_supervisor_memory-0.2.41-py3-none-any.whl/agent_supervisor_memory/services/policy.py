...
DEFAULT_POLICY: dict[str, Any] = {
    ...
    "recipes": {
        # Tool-call budget defaults (aim to stay under common 60s MCP client deadlines).
        "lite": {
            "project": {"budget_ms": 2000, "max_refresh": 2, "refresh": False, "include_patch": False},
            "dispatch": {"wait": False, "max_wait_seconds": 10},
            "patch": {"generate_patch": False},
        },
        "spike": {
            "project": {"budget_ms": 5000, "max_refresh": 4, "refresh": True, "include_patch": False},
            "dispatch": {"wait": False, "max_wait_seconds": 20},
            "patch": {"generate_patch": False},
        },
        "heavy": {
            "project": {"budget_ms": 8000, "max_refresh": 6, "refresh": True, "include_patch": False},
            "dispatch": {"wait": False, "max_wait_seconds": 30},
            "patch": {"generate_patch": False},
        },
    },
    "modes": {
...
class AgentPolicy:
    ...
    def recipes(self) -> dict[str, Any]:
        return dict(self.raw.get("recipes") or {})
...
def route_recipe(*, policy: AgentPolicy, workflow_mode: str) -> dict[str, Any]:
    """
    Return a compact execution recipe (recommended tool options) based on workflow_mode.

    The recipe is a policy-level hint and is intentionally conservative to avoid client tool-call timeouts.
    """
    recipes = policy.recipes or {}
    defaults = (DEFAULT_POLICY.get("recipes") or {}) if isinstance(DEFAULT_POLICY, dict) else {}
    key = str(workflow_mode or "").strip().lower() or "lite"
    raw = recipes.get(key) if isinstance(recipes.get(key), dict) else None
    fallback = defaults.get(key) if isinstance(defaults.get(key), dict) else None
    return dict(raw or fallback or {})
