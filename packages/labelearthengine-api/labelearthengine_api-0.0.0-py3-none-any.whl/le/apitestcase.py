...
from labelearthapiclient import discovery
...
class ApiTestCase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.old_get_algorithms = le.data.getAlgorithms
    self.old_compute_value = le.data.computeValue
    self.old_get_map_id = le.data.getMapId
    self.old_get_download_id = le.data.getDownloadId
    self.old_get_thumb_id = le.data.getThumbId
    self.old_get_table_download_id = le.data.getTableDownloadId
    # pylint: disable=protected-access
    self.old_install_cloud_api_resource = le.data._install_cloud_api_resource
    self.old_cloud_api_resource = le.data._cloud_api_resource
    self.old_cloud_api_resource_raw = le.data._cloud_api_resource_raw
    self.old_initialized = le.data._initialized
    self.old_fetch_data_catalog_stac = le.deprecation._FetchDataCatalogStac
    # pylint: enable=protected-access
    self.InitializeApi()

  def tearDown(self):
    super().tearDown()
    le.data.getAlgorithms = self.old_get_algorithms
    le.data.computeValue = self.old_compute_value
    le.data.getMapId = self.old_get_map_id
    le.data.getDownloadId = self.old_get_download_id
    le.data.getThumbId = self.old_get_thumb_id
    le.data.getTableDownloadId = self.old_get_table_download_id
    # pylint: disable=protected-access
    le.data._install_cloud_api_resource = self.old_install_cloud_api_resource
    le.data._cloud_api_resource = self.old_cloud_api_resource
    le.data._cloud_api_resource_raw = self.old_cloud_api_resource_raw
    le.data._initialized = self.old_initialized
    le.deprecation._FetchDataCatalogStac = self.old_fetch_data_catalog_stac
    # pylint: enable=protected-access
  ...
  def InitializeApi(self):
    ...
    le.data._install_cloud_api_resource = lambda: None  # pylint: disable=protected-access
    le.data.getAlgorithms = GetAlgorithms
    le.data.computeValue = lambda x: {'value': 'fakeValue'}
    le.data.getMapId = self._MockMapId
    le.data.getDownloadId = self._MockDownloadUrl
    le.data.getThumbId = self._MockThumbUrl
    le.data.getTableDownloadId = self._MockTableDownload
    # pylint: disable-next=protected-access
    le.deprecation._FetchDataCatalogStac = self._MockFetchDataCatalogStac
...
def _GenerateCloudApiResource(mock_http: Any, raw: Any) -> discovery.Resource:
  ...
  return _cloud_api_utils.build_cloud_resource_from_document(
      ...
      headers_supplier=le.data._make_request_headers,  # pylint: disable=protected-access
      response_inspector=le.data._handle_profiling_response,  # pylint: disable=protected-access
      raw=raw,
  )
...
def UsingCloudApi(
  ...
  old_cloud_api_resource = le.data._cloud_api_resource  # pylint: disable=protected-access
  old_cloud_api_resource_raw = le.data._cloud_api_resource_raw  # pylint: disable=protected-access
  old_initialized = le.data._initialized  # pylint: disable=protected-access
  try:
    ...
    le.data._cloud_api_resource = cloud_api_resource  # pylint: disable=protected-access
    le.data._cloud_api_resource_raw = cloud_api_resource_raw  # pylint: disable=protected-access
    le.data._initialized = True  # pylint: disable=protected-access
    yield
  finally:
    le.data._cloud_api_resource = old_cloud_api_resource  # pylint: disable=protected-access
    le.data._cloud_api_resource_raw = old_cloud_api_resource_raw  # pylint: disable=protected-access
    le.data._initialized = old_initialized  # pylint: disable=protected-access
