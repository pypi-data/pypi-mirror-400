...
class TrumpPolicySimulator:
    ...
    def __init__(self):
        ...
        self.country_policies = {
            "china": {
                "tariff": 0.80,  # 80% 额外关税（贸易战！）
                "block_chance": 0.50,  # 50% 被屏蔽
                "speed_multiplier": 0.1,  # 速度只有10%（蜗牛速度）
            },
            "eu": {
                "tariff": 0.60,
                "block_chance": 0.30,
                "speed_multiplier": 0.3,  # 速度只有30%
            },
            "russia": {
                "tariff": 0.70,
                "block_chance": 0.40,
                "speed_multiplier": 0.2,  # 速度只有20%
            },
            "iran": {
                "tariff": 0.90,
                "block_chance": 0.70,
                "speed_multiplier": 0.05,  # 速度只有5%（几乎不动）
            },
            "usa": {
                "tariff": -0.20,  # 20% 补贴（爱国折扣）
                "block_chance": 0.01,
                "speed_multiplier": 1.5,  # 速度增加 50%（美国优先！）
            },
            "other": {
                "tariff": 0.50,  # 50% 关税
                "block_chance": 0.20,
                "speed_multiplier": 0.4,  # 速度只有40%
            },
        }
    ...
    def simulate_download_with_policies(self, package_name: str, size_mb: float, 
        ...
        # 计算下载时间
        base_speed = 128  # 0.125 MB/s（蜗牛速度）
        effective_speed = base_speed * policy_result["speed_multiplier"]
        
        # 确保最低速度（蜗牛速度）
        effective_speed = max(1, effective_speed)  # 最低 1 KB/s（真正的蜗牛速度）
