...
def identification_comment(data: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    return identification_template(
        sap_cod_fornecedor=data.get('sap_cod_fornecedor'),
        ...
        "content": [
            # 1ª coluna: tableHeader com cod_produto (como no JS)
            {
                ...
                "content": [
                    {
                        "type": "paragraph",
                        "content": [{"type": "text", "text": f"{item.get('cod_produto', '')}"}],
                    }
                ],
            },
            # Demais colunas: tableCell
            _cell_text(f"{item.get('nome_produto', '')}"),
            _cell_text(f"{item.get('cfop', '')}"),
            _cell_text(f"{item.get('quantidade', '')}"),
            _cell_text(f"{item.get('preco', '')}"),
            _cell_text(f"{item.get('valor_produto', '')}"),
            _cell_text(f"{item.get('cod_material_sap', '')}"),
        ],
    }
