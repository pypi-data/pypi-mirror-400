...
# runs catch API changes immediately.
_LOCAL_HTTP_CLIENT_SRC = (
    Path(__file__).resolve().parents[4] / "atlas_asset_http_client_python" / "src"
)
if _LOCAL_HTTP_CLIENT_SRC.exists():
    local_path = str(_LOCAL_HTTP_CLIENT_SRC)
    ...
    def _ensure_event_loop(self) -> None:
        ...
        with self._loop_lock:
            ...
            self._loop_thread = threading.Thread(
                target=run_loop, args=(self._loop,), daemon=True, name="gateway-event-loop"
            )
            ...
            try:
                asyncio.run_coroutine_threadsafe(
                    enter_client(), self._loop
                ).result(timeout=self._DEFAULT_LOOP_START_TIMEOUT)
