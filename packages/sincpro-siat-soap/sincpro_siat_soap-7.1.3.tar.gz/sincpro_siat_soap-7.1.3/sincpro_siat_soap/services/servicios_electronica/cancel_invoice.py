...
class CommandInvoiceCancelElectronica(DataTransferObject):
    nit: int | str
    cuis: str
    cufd: str
    cuf: str
...
class ResponseInvoiceCancelElectronica(DataTransferObject):
    transaction: bool
    raw_response: Any


@siat_soap_sdk.feature(CommandInvoiceCancelElectronica)
class InvoiceCancellationRequest(Feature):

    def execute(
        self, dto: CommandInvoiceCancelElectronica
    ) -> ResponseInvoiceCancelElectronica:
        response = self.soap_client(SIAT_WSDL.SERVICIOS_ELECTRONICA).service.anulacionFactura(
