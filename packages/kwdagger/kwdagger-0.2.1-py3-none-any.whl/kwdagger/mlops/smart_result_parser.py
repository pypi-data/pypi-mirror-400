...
def load_pxl_eval(fpath, expt_dvc_dpath=None, arg_prefix='', mode=0, with_param_types=True):
    ...
    with warnings.catch_warnings():
        ...
        try:
            # New stuff, so might not work with old results
            max_f1_submeasures = result.nocls_measures['max_f1_submeasures']
            metrics['salient_maxF1_F1'] = max_f1_submeasures['f1']
            metrics['salient_maxF1_tpr'] = max_f1_submeasures['tpr']
            metrics['salient_maxF1_ppv'] = max_f1_submeasures['ppv']
            metrics['salient_maxF1_mcc'] = max_f1_submeasures['mcc']
            metrics['salient_maxF1_thresh'] = max_f1_submeasures['thresh']
        except Exception:
            ...
...
def resolve_cross_machine_path(path, dvc_dpath=None):
    ...
    if needs_resolve:
        ...
        if found_idx is not None:
            # import kwdagger
            # dvc_dpath = kwdagger.find_dvc_dpath()
...
def find_metrics_framework_item(info):
    task_aliases = {
        'kwdagger.cli.run_metrics_framework',
    }
