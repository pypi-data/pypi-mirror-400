...
class CATENA(AbstractConstrainedMinimisation):
    """The corrected version of the classical hanging catenary problem.
    ...
    def y0(self):
        ...
        # From CATENA.SIF: TMP = LENGTH / RN+1, VAL = TMP * RI, X(I) = VAL, Y(I) = -VAL
        tmp = self.length / self.n_plus_1
        for i in range(1, n_points):
            val = tmp * i
            x_coords = x_coords.at[i].set(val)
            y_coords = y_coords.at[i].set(-val)  # Y(I) = -VAL in CATENA
    ...
    def expected_objective_value(self):
        ...
        # From SIF: -348529.577196 for n=3003
        return jnp.array(-348529.577196)
