...
def get_pager_data_frame(
    ...
    for pager in detail.getProducts("losspager", version="all"):
        ...
        try:
            contents = pager.getContentsMatching("exposures.json")
        except Exception as e:
            msg = (
                f"Could not retrieve contents from version {pager.version} "
                f"of {detail.id}. Skipping."
            )
            logging.info(msg)
            continue
        if len(contents):
            try:
                ...
                expourl = pager.getContentURL("exposures.json")
                msg = (
                    f"Unable to retrieve data from {expourl} (error {str(e)}). "
                    f"Skipping data retrieval for event {detail.id}, "
                    f"version {pager.version}"
                )
                logging.info(msg)
                continue
            ...
            if get_losses:
                try:
                    ...
                    lossurl = pager.getContentURL("losses.json")
                    msg = (
                        f"Unable to retrieve/read data from {lossurl}. "
                        f"Skipping data retrieval for event {detail.id}, "
                        f"version {pager.version}"
                    )
                    logging.info(msg)
