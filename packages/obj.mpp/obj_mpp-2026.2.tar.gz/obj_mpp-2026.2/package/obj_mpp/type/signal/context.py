...
class signal_context_t:
    ...
    def SetSignals(self, signal: h.Any, dimension: int, /) -> None:
        ...
        if self.map_or_pdf is not None:
            assert self.lengths == self.map_or_pdf.shape, (
                self.lengths,
                self.map_or_pdf.shape,
            )
    ...
    def SetMapOrPDF(self, map_or_pdf: array_t, /) -> None:
        ...
        if self.signal_original is not None:
            assert self.lengths == map_or_pdf.shape, (self.lengths, map_or_pdf.shape)
