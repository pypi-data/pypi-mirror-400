...
from yt.wrapper.external_source_specs import FilesExternalSourceSpec, PrefixExternalSourceSpec
...
def add_attach_table_parser(add_parser):
    def attach_table(source_uris=None, src_prefix_uri=None, src_recursive=None, src_include_regex=None, src_exclude_regex=None, **kwargs):
        if source_uris:
            return yt.attach_table(source_spec=FilesExternalSourceSpec(
                uris=source_uris,
            ), **kwargs)
        if src_prefix_uri:
            return yt.attach_table(source_spec=PrefixExternalSourceSpec(
                prefix_uri=src_prefix_uri,
                recursive=src_recursive,
                include_regexes=src_include_regex,
                exclude_regexes=src_exclude_regex,
            ), **kwargs)
    ...
    common_parser.add_argument(
        "--attach-mode", type=str, choices=["parallel", "sequential"], dest="attach_mode", default="parallel",
        help="""Controls how source URIs produced from the specified source spec are processed.
...
the source spec itself (e.g. it is an explicit list of source URIs) or by the source order option below.""",
    ...
    common_parser.add_argument(
        "--source-order", type=str, choices=["none", "lex_asc", "lex_desc"], dest="source_order", default="none",
        help="""Controls the order in which source URIs are fed into the attach process.
...
By default sources are processed in the order they are produced from the source spec, which may be unspecified. """,
    ...
    source_type_subparsers = parser.add_subparsers(metavar="source_type", **SUBPARSER_KWARGS)

    files_subparser = source_type_subparsers.add_parser("files", parents=[common_parser])
    ...
    prefix_subparser = source_type_subparsers.add_parser("prefix", parents=[common_parser])
    prefix_subparser.add_argument("--src-prefix-uri", help='URI prefix, e.g. "s3://bucket/prefix".')
    recursive_help = 'Whether to include files in subdirectories of the given prefix. True by default.'
    prefix_subparser.add_argument("--recursive", dest="src_recursive", default=None, action="store_true", help=recursive_help)
    prefix_subparser.add_argument("--not-recursive", dest="src_recursive", default=None, action="store_false", help=recursive_help)
    prefix_subparser.add_argument(
        "--src-include-regex", action="append", default=[],
        help="""Filter patterns to *include* file paths in external source.
...
NB: Inclusion is applied before exclusion, i.e. if a path matches both an include and an exclude pattern, it will be excluded.""")
    prefix_subparser.add_argument(
        "--src-exclude-regex", action="append", default=[],
        help="""Filter patterns to *exclude* file paths from external source.
