...
async def _download_bootstrap_impl(
    ...
    *,
    ...
    tar_results_queue = multiprocessing.Queue()
    ...
    tar_proc = multiprocessing.Process(
        ...
        args=(fifo_path, destination, tar_results_queue, shutdown_event),
    )
    tar_proc.start()
    ...
    def queue_bridge_thread():
        ...
        try:
            while True:
                # Blocking wait for result (extraction or verification)
                result = results_queue.get()
                if result is None:  # Poison pill
                    logging.debug("[Bridge] Received shutdown signal")
                    loop.call_soon_threadsafe(async_results_queue.put_nowait, None)
                    break
                # Forward to async queue (logging happens in handler)
                loop.call_soon_threadsafe(async_results_queue.put_nowait, result)
    ...
    # Bridge for tar results
    async_tar_results_queue: asyncio.Queue = asyncio.Queue()

    def tar_queue_bridge_thread():
        """Thread that bridges tar results queue to asyncio."""
        logging.debug("[TarBridge] Starting tar bridge thread")
        try:
            # Wait for single completion message
            result = tar_results_queue.get()
            logging.debug(f"[TarBridge] Received tar completion: {result}")
            loop.call_soon_threadsafe(async_tar_results_queue.put_nowait, result)
        except Exception as e:
            logging.error(f"[TarBridge] Error: {e}")
            import traceback

            logging.error(traceback.format_exc())
            loop.call_soon_threadsafe(async_tar_results_queue.put_nowait, None)

    tar_bridge_thread = threading.Thread(
        target=tar_queue_bridge_thread, daemon=True, name="tar-bridge"
    )
    tar_bridge_thread.start()
    logging.info("Tar bridge thread started")

    # Track verification completion
    total_parts = len(state["parts"])
    verified_parts_set = state_mgr.get_verified_parts(state)
    parts_verified_count = len(verified_parts_set)
    all_parts_verified = asyncio.Event()
    ...
    async def process_verification_results():
        ...
        nonlocal parts_verified_count

        logging.debug("[Results] Starting verification results processor")
        logging.debug(
            f"[Results] Tracking completion: {parts_verified_count}/{total_parts} "
            f"already verified"
        )
        try:
            while not shutdown_event.is_set():
                ...
                # Handle extraction notifications
                if isinstance(result, ExtractionNotification):
                    status = "already verified" if result.already_verified else ""
                    logging.info(
                        f"[Extract {result.part_id}] Part streamed to tar "
                        f"extractor ({result.bytes_written} bytes) {status}".strip()
                    )
                    continue
                ...
                if isinstance(result, VerificationResult):
                    ...
                    if result.verified:
                        ...
                        # Mark part as verified in controller
                        await verification_controller.mark_verified(result.part_id)
                        verification_controller.release()

                        parts_verified_count += 1

                        next_exp = verification_controller.next_expected_part
                        logging.info(
                            f"[Verify {result.part_id}] ✓ SHA256 verified "
                            f"({verification_controller.slots_used}/"
                            f"{MAX_UNVERIFIED_PARTS} slots used, "
                            f"next expected: {next_exp})"
                        )

                        # Check if all parts verified
                        if parts_verified_count >= total_parts:
                            logging.info(
                                f"All {total_parts} parts verified. "
                                f"Extracting to {destination}..."
                            )
                            all_parts_verified.set()
    ...
    try:
        with live_context:
            ...
            # Calculate bytes already downloaded (for resume progress)
            bytes_already_downloaded = sum(
                state["parts"][part_id]["size"] for part_id in verified_parts
            )

            # Download manager
            # Create part started callback for CLI progress display
            part_started_callback = None
            if progress_display is not None:
                def part_started_callback(part_id: int, size: int):
                    progress_display.create_part_task(part_id, size)
            ...
            download_mgr = DownloadManager(
                ...
                part_started_callback=part_started_callback,
            ...
            if progress_callback:
                initial_percent = 0.0
                if total_bytes > 0:
                    initial_percent = (bytes_already_downloaded / total_bytes) * 100
            ...
            if not shutdown_event.is_set():
                ...
                async with aiohttp.ClientSession(
                    ...
                    running_tasks = {}  # task -> part dict mapping
                    failed_parts = []

                    # Queue for completed downloads awaiting notification
                    notification_pending: asyncio.Queue = asyncio.Queue()

                    async def periodic_progress_sampler():
                        """Periodically sample and emit aggregate progress.

                        Runs every progress_interval seconds (user-configured):
                        - Aggregate download speed across all parts
                        - Individual part speeds (guaranteed to sum to aggregate)
                        - All active parts reported in single callback
                        """
                        try:
                            while not shutdown_event.is_set():
                                await asyncio.sleep(download_mgr._progress_interval)
                                download_mgr.emit_aggregate_progress()
                        except asyncio.CancelledError:
                            logging.debug("[Progress] Task cancelled")
                        except Exception as e:
                            logging.error(f"[Progress] Sampler error: {e}")

                    async def notification_sender():
                        """Background task that sends notifications with backpressure.

                        Downloads complete immediately, but notifications wait for
                        verification slots. This decouples download speed from
                        verification throughput.
                        """
                        try:
                            while True:
                                # Wait for a completed download
                                part_info = await notification_pending.get()
                                if part_info is None:  # Shutdown signal
                                    break

                                part_id = part_info["part_id"]
                                filepath = part_info["filepath"]
                                expected_sha256 = part_info["expected_sha256"]
                                size = part_info["size"]

                                # This is where backpressure happens - waits for
                                # verification slot without blocking downloads
                                await download_mgr.send_notification(
                                    part_id, filepath, expected_sha256, size
                                )
                                logging.debug(
                                    f"[Notify {part_id}] Sent to worker"
                                )

                        except asyncio.CancelledError:
                            logging.debug("[Notify] Task cancelled")
                        except Exception as e:
                            logging.error(f"[Notify] Error: {e}")

                    # Start background tasks
                    notify_task = asyncio.create_task(notification_sender())
                    progress_task = asyncio.create_task(periodic_progress_sampler())
                    ...
                    while remaining_parts or running_tasks:
                        ...
                        if running_tasks:
                            ...
                            for task in done:
                                # Use pop with default to avoid KeyError if dict cleared
                                # by earlier failure in this batch
                                part = running_tasks.pop(task, None)

                                if part is None:
                                    # Task already removed by earlier failure cleanup
                                    continue

                                part_id = part["id"]

                                try:
                                    # Get download result (True=success, False=failure)
                                    success = task.result()
                                    ...
                                    if not success:
                                        ...
                                        # Download succeeded - construct part_info dict
                                        # for notification sender
                                        part_id_value = part["id"]
                                        filename = f"part_{part_id_value:04d}.bin"
                                        filepath = download_mgr.parts_dir / filename
                                        part_info = {
                                        ...
                                        # Queue notification
                                        # (backpressure handled by notification_sender)
                                        logging.debug(
                                            f"[Download {part_id}] Bytes on disk, "
                                            f"queuing notification"
                                        )
                                        await notification_pending.put(part_info)

                                        # Cleanup internal tracking (always needed)
                                        download_mgr.cleanup_part_tracking(part_id)

                                        # Update progress display (CLI mode only)
                    ...
                    # (downloads done, but notifications may be backpressured)
                    # Skip if already cancelled during shutdown
                    if not notify_task.done():
                        logging.debug(
                            "[Shutdown] Waiting for pending notifications..."
                        )
                        await notification_pending.put(None)  # Signal shutdown
                        await notify_task  # Wait for all notifications sent

                    # Cancel progress sampler task
            ...
            # If graceful completion, wait for all parts to be verified
            # If interrupted (Ctrl+C), terminate immediately
            if not shutdown_event.is_set():
                # Graceful completion - wait for all parts to be verified
                logging.debug(
                    "[Shutdown] Waiting for all parts to be verified "
                    f"({parts_verified_count}/{total_parts} complete)..."
                )

                # Wait for event-driven completion signal
                await all_parts_verified.wait()

                logging.info(
                    f"All {total_parts} parts verified, waiting for tar extraction..."
                )
            else:
            ...
            # Wait for tar extractor completion signal
            # All parts already streamed to FIFO by worker
            if progress_display:
                progress_display.show_extraction_status()

            logging.debug(
                "[Shutdown] Waiting for tar extraction completion signal..."
            )

            # Wait for event-driven completion from tar process
            try:
                tar_result = await asyncio.wait_for(
                    async_tar_results_queue.get(), timeout=600  # 10 min safety
                )
            except TimeoutError:
                logging.error(
                    "[Shutdown] Tar extraction did not send completion signal "
                    "in 10 minutes, terminating..."
                )
                tar_proc.terminate()
                tar_proc.join(timeout=5)
                return False

            if tar_result is None:
                logging.error("[Shutdown] Tar bridge failed")
                tar_proc.terminate()
                tar_proc.join(timeout=5)
                return False

            logging.info("Tar extraction complete, cleaning up worker processes...")

            # Only clean up worker and results if not interrupted
            # (interrupted path already did this cleanup)
            if not shutdown_event.is_set():
                # Now that tar is done, join worker process (should be instant)
                worker_proc.join(timeout=10)
                if worker_proc.is_alive():
                    logging.warning(
                        "[Shutdown] Worker still alive after tar complete, "
                        "terminating..."
                    )
                    worker_proc.terminate()
                    worker_proc.join()
            ...
            # Wait briefly for tar process to exit cleanly
            tar_proc.join(timeout=10)
            if tar_proc.is_alive():
                logging.warning(
                    "[Shutdown] Tar process still alive after completion, "
                    "terminating..."
                )
                tar_proc.terminate()
                tar_proc.join()

            extraction_success = tar_result.success
            if extraction_success:
                ...
                error_msg = tar_result.error_message or "Unknown error"
                logging.error(f"✗ Tar extraction failed: {error_msg}")
