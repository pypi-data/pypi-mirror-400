...
async def main():
    async with LybicClient() as client:
        sandboxes = await client.sandbox.list()  # Need await
        stats = await client.stats.get()         # Need await

asyncio.run(main())

# Sync usage - same API, no await!
from lybic_sync import LybicSyncClient

with LybicSyncClient() as client:
    sandboxes = client.sandbox.list()  # No await
    stats = client.stats.get()         # No await
```
...
The SDK provides both **asynchronous** (`Pyautogui`) and **synchronous** (`PyautoguiSync`) versions with identical APIs.
...
For async code, use the regular `Pyautogui` class:
...
# Execute commands (internally converts to sync)
llm_output = "pyautogui.moveTo(100, 150)"
eval(llm_output)
