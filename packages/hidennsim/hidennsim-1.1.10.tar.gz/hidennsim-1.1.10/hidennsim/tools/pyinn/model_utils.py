...
from . import dataset_classification, dataset_regression, train
...
def save_model_data(config, data, params, interp_method, data_name=None):
    """
    ...
    Args:
        ...
        data_name (str, optional): Name of the dataset. If None, derived from config.
    """
    # Derive data_name from config if not provided
    if data_name is None:
        data_filenames = config.get('DATA_PARAM', {}).get('data_filenames', ['data'])
        if data_filenames:
            # Use first filename without extension as data_name
            data_name = os.path.splitext(os.path.basename(data_filenames[0]))[0]
        else:
            data_name = 'model'

    # Use model_save_dir from config if available, otherwise default
    model_save_dir = config.get('OUTPUT', {}).get('model_save_dir', './pyinn/model_saved')
...
def save_errors_val(errors_val, interp_method, data_name='model', model_save_dir=None):
    """
    ...
    Args:
        ...
        interp_method (str): Interpolation method used
        data_name (str, optional): Name of the dataset. Defaults to 'model'.
        model_save_dir (str, optional): Directory to save. If None, uses default.
    """
