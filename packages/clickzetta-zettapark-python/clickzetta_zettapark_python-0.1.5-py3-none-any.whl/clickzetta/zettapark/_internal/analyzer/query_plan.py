...
if TYPE_CHECKING:
    import clickzetta.zettapark._connector
...
class QueryPlanBuilder:
    ...
    def build_from_multiple_queries(
        self,
        multi_sql_generator: Callable[["Query", bool], List["Query"]],
        child: QueryPlan,
        source_plan: Optional[LogicalPlan],
        schema_query: str,
        is_ddl_on_temp_object: bool = False,
    ...
    def file(self) -> "FileOperationPlanBuilder":
        return FileOperationPlanBuilder(self)
    ...
    def create_temp_table(
        ...
        *,
        ...
        return self.build_from_multiple_queries(
            lambda query, single_child: self.create_table_and_insert(
                self.session,
                name,
                child.schema_query,
                query,
                is_single_child_query=single_child,
    ...
    def read_file(
        ...
        select_statement = select_from_volume_statement(
            path,
            file_format,
            pattern,
            schema,
    ...
    def copy_into_location(
        ...
        **copy_options: Optional[Any],
        ...
        return self.build(
            lambda x: copy_into_volume_statement(
                query=x,
...
_FOS = FileOperationStatements


class FileOperationPlanBuilder:
    def __init__(self, builder: QueryPlanBuilder) -> None:
        self._builder = builder

    def _query(self, sql: str) -> QueryPlan:
        return self._builder.query(sql, None)

    def put(
        self, local_file: Path, volume_path: VolumePath, options: Dict[str, str]
    ) -> QueryPlan:
        return self._query(_FOS.put(local_file, volume_path, options))

    def get(
        self, volume_path: VolumePath, local_file: Path, options: Dict[str, str]
    ) -> QueryPlan:
        return self._query(_FOS.get(volume_path, local_file, options))

    def list(self, volume_path: VolumePath, options: Dict[str, str]) -> QueryPlan:
        return self._query(_FOS.list_(volume_path, options))

    def delete(self, volume_path: VolumePath) -> QueryPlan:
        return self._query(_FOS.delete(volume_path))

    def copy_files(
        self, dest: VolumePath, src: VolumePath, options: Dict[str, str]
    ) -> QueryPlan:
        return self._query(_FOS.copy_files(dest, src, options))
