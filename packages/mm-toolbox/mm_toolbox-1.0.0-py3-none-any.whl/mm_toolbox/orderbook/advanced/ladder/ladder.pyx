...
cdef class OrderbookLadder:
    ...
    def __cinit__(self, u64 max_levels, bint is_price_ascending) -> None:
        ...
        # Create numpy array from the raw memory
        self._levels_numpy = np.asarray(<OrderbookLevel[:max_levels]> self._levels).view(dtype=levels_dtype)
    ...
    cpdef get_levels(self, bint copy=False):
        ...
        if copy:
            return result.copy()
        # Set the array's base to self to prevent premature garbage collection
        result.base = self
        return result
