        ...
        if self.__include_plural:
            ...
            def new_get_attrs(self, *args, **kwargs):
                ...
                if attr_all_fetched:
                    for _, prev_kwargs in attr_cache.values():
                        if must_update(prev_kwargs, kwargs):
                            logger.info(
                                f"Cache invalidation: {attr_name}s - new kwargs required "
                ...
                if not attr_all_fetched:
                    ...
                    logger.info(
                        f"Bulk refresh completed: {len(fetched_objs)} {attr_name}s fetched in "
                    ...
                    logger.info(
                        f"Using cached {attr_name}s: {len(attr_cache)} cached "
