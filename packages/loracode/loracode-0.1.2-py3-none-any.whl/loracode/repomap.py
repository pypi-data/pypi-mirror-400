...
from loracode.special import filter_important_files
...
UPDATING_REPO_MAP_MESSAGE = "Analyzing repository..."


class RepoMap:
    TAGS_CACHE_DIR = f".loracode.tags.cache.v{CACHE_VERSION}"
    ...
    def __init__(
        ...
        if self.verbose:
            self.io.tool_output(t("repomap.initialized", value=self.map_mul_no_files))
    ...
    def get_repo_map(
        ...
        try:
            ...
            self.io.tool_error(t("repomap.disabled"))
        ...
        if self.verbose:
            ...
            self.io.tool_output(t("repomap.tokens", tokens=num_tokens / 1024))
    ...
    def tags_cache_error(self, original_error=None):
        ...
        try:
            ...
            self.io.tool_warning(t("repomap.cache_fallback", path=path))
            if self.verbose:
                self.io.tool_warning(t("repomap.cache_recreation_error", error=str(e)))
    ...
    def get_mtime(self, fname):
        try:
            ...
            self.io.tool_warning(t("file.not_found_warning", filename=fname))
    ...
    def get_tags_raw(self, fname, rel_fname):
        ...
        try:
            ...
            print(t("repomap.skipping_file", filename=fname, error=err))
            return
    ...
    def get_ranked_tags(
        ...
        if len(fnames) - cache_size > 100:
            self.io.tool_output(t("repomap.initial_scan"))
        ...
        for fname in fnames:
            if self.verbose:
                self.io.tool_output(t("file.processing", filename=fname))
            ...
            if not file_ok:
                if fname not in self.warned_files:
                    self.io.tool_warning(t("repomap.cant_include", filename=fname))
                    self.io.tool_output(t("repomap.deleted_question"))
