...
class AWSPermissionContext(HEAServerPermissionContext, ABC):
    ...
    async def _simulate_perms(self, obj: AWSDesktopObject, actions: Sequence[str]) -> list[Permission]:
        ...
        if not USE_SIMULATE_PRINCIPAL_POLICY_FOR_PERMISSIONS:
...
class S3ObjectPermissionContext(AWSPermissionContext):
    """
    AWS S3 object permission context.
    """

    def __init__(self, request: Request, check_attribute_permissions=True, **kwargs):
        """
        ...
        :param check_attribute_permissions: whether to query AWS for attribute permissions (default True). If False,
        it delegates attribute permission checking to the superclass, which returns default attribute permissions based
        on object-level permissions.
        """
        actions = [S3_GET_OBJECT, S3_PUT_OBJECT, S3_DELETE_OBJECT]
        super().__init__(request=request, actions=actions, **kwargs)
        self.__check_attribute_permissions = bool(check_attribute_permissions)

    @property
    def check_attribute_permissions(self) -> bool:
        """Whether to query AWS for attribute permissions."""
        return self.__check_attribute_permissions
    ...
    async def get_attribute_permissions(self, obj: DesktopObject, attr: str) -> list[Permission]:
        ...
        if not isinstance(obj, S3Object):
            return []
        if not self.check_attribute_permissions or not USE_SIMULATE_PRINCIPAL_POLICY_FOR_PERMISSIONS:
