...
The output from the EnviDat to JSON-LD converter is also compatible with the [POLDER JSON-LD format](https://polder-crew.github.io/Federated-Search-Documentation/inclusion-guidelines.html). [POLDER](https://search.polder.info/about) is an international multidiscplinary Polar Data Search portal created to aid the search for polar data.
