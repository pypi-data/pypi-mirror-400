...
describe('Integration Tests - Complete Workflows', () => {
  ...
  describe('New Project → Add Layers → Save → Load Workflow', () => {
    it('should complete full workflow successfully', async () => {
      ...
      const loadedWindDef: FiberPathWindDefinition = JSON.parse(savedWindDefJson);
      const loadedProject = windDefinitionToProject(loadedWindDef, '/test/project.wind');
  ...
  describe('Load → Edit → Save Workflow', () => {
    it('should load, modify, and save project', async () => {
      ...
      const loadedProject = windDefinitionToProject(initialWindDef, '/test/existing.wind');
      state.loadProject(loadedProject);
      state = useProjectStore.getState();
      ...
      const helicalId = state.project.layers.find(l => l.type === 'helical')?.id;
      if (helicalId) {
        state.updateLayer(helicalId, {
          helical: {
            wind_angle: 55,
      ...
      const helicalLayer = updatedWindDef.layers.find(l => l.windType === 'helical');
  ...
  describe('Layer Operations Workflow', () => {
    it('should handle complex layer operations', async () => {
      ...
      expect(state.project.layers).toHaveLength(4);
      expect(state.project.layers.find(l => l.id === duplicateId)).toBeUndefined();
