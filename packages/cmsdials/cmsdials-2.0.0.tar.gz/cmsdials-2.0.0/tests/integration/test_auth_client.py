...
def __setup_dials_from_creds_and_test(creds: Union[SecretKeyCredentials, BearerCredentials]) -> None:
    dials = Dials(creds, workspace=TEST_WORKSPACE, base_url=BASE_URL)
    # just test if we can access the workspace endpoints
    dials.workspace.my_workspace()
    dials.workspace.list()
