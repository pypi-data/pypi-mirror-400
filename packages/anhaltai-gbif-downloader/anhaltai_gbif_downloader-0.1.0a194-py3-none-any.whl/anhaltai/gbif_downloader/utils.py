...
def prioritize_classes_with_most_missing_objects(
    downloader, species_keys, path, query_params
):
    """
    ...
    average_images_per_occurrence_count = 0
    ...
    for species_key in species_keys:
        ...
        if prefix:
            counts = count_file_endings_in_bucket(
                MINIO_CLIENT, BUCKET, prefix=prefix, file_endings=[".json"]
            )
            counts_images = count_file_endings_in_bucket(
                MINIO_CLIENT, BUCKET, prefix=prefix, file_endings=[".png"]
            )

            json_count = counts.get(".json", 0)
            png_count = counts_images.get(".png", 0)
            data_count = data.get("count", 0)

        species_priority.update({species_key: data_count - json_count})
        logging.info(
        ...
        average_images_per_occurrence_count += (
            png_count / json_count if json_count > 0 else 0
        )
