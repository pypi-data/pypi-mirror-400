"""DMN15332LS - Diamond Light Source powder diffraction data fitting (least squares).
...
def _load_dmn15332ls_data():
    """Load problem data from NPZ file."""
    data_file = Path(__file__).parent / "data" / "dmn15332ls.npz"
    return np.load(data_file)


# Load data once at module level
_DMN15332LS_DATA = _load_dmn15332ls_data()


class DMN15332LS(AbstractUnconstrainedMinimisation):
    """DMN15332LS Diamond Light Source powder diffraction fitting (least squares).
    ...
    DMN15332 (0.002-0.003 differences). Issue likely due to numerical precision
    ...
    def objective(self, y, args):
        ...
        """
        del args
        nvec = int(_DMN15332LS_DATA["nvec"])
        ...
        # Load data
        x_data = jnp.array(_DMN15332LS_DATA["x_data"])
        y_data = jnp.array(_DMN15332LS_DATA["y_data"])
        positions = jnp.array(_DMN15332LS_DATA["positions"])
    ...
    def y0(self):
        """Starting point."""
        if self.y0_iD == 0:
            return jnp.array(_DMN15332LS_DATA["start1"])
        else:
            return jnp.array(_DMN15332LS_DATA["start2"])
