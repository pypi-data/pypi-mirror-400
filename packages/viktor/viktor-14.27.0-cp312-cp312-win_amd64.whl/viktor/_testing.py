...
def _mock_params(mocked_get_entity: Callable, mocked_file_resource: type, deserialize_function: Callable,
                 param_types: dict, params: dict) -> Munch:
    with mock.patch.object(API, '__init__', return_value=None), \
            mock.patch.object(API, 'get_entity', side_effect=mocked_get_entity), \
            mock.patch('viktor.parametrization.FileResource', mocked_file_resource):
        try:
            return deserialize_function(param_types, params, workspace_id=1, entity_id=99, api=None)
