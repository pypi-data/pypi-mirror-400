...
'''
    ...
    def _build_features_section(self) -> str:
        ...
        if self.config_reader.has_migration():
            features.append("- 📦 **Database Migrations**: Alembic support")
        ...
        if self.config_reader.has_auth():
            ...
            if self.config_reader.has_refresh_token():
                features.append("- 🔄 **Refresh Token**: Secure token refresh")
        
        # Redis and Celery
        if self.config_reader.has_redis():
            features.append("- 🔴 **Redis**: Caching and session management")
        
        if self.config_reader.has_celery():
            features.append("- 📋 **Background Tasks**: Celery with Redis broker")
            features.append("- 💾 **Database Backup**: Automated backup tasks (MySQL, PostgreSQL, SQLite)")
        
        if self.config_reader.has_cors():
            features.append("- 🌐 **CORS**: Cross-Origin Resource Sharing enabled")
        
        # Security features (always included)
        security_features = ["Input Validation", "Password Hashing", "Rate Limiting"]
        features.append(f"- 🔒 **Security**: {', '.join(security_features)}")
        
        if self.config_reader.has_testing():
            features.append("- 🧪 **Testing**: pytest with async support and coverage")
        
        if self.config_reader.has_docker():
            features.append("- 🐳 **Docker**: Production-ready containerization")
        
        if self.config_reader.has_dev_tools():
            features.append("- 🛠️ **Development Tools**: Black, Ruff for code quality")
        
        # Always included features
        features.append("- 📚 **API Documentation**: Swagger UI and ReDoc")
        features.append("- 📊 **Logging**: Structured logging with Loguru")
        features.append("- 🏥 **Health Check**: Built-in health check endpoint")
...
Generated with ❤️ by [Forge](https://github.com/ning3739/forge) - A powerful FastAPI project scaffolding tool.
