...
"""
    ...
    # ============================================================
    # Orchestrator 호환 메서드 (gemini_wrapper.py 대체)
    # ============================================================
...
"""
        ...
        try:
            response = self.client.models.generate_content(
                ...
                contents=prompt,
                config=types.GenerateContentConfig(
                    system_instruction=COMMANDER_CLI_CONSTITUTION,
                    temperature=0.3,
                )
            )
            ...
            self._log(f"요구사항 명확화 실패: {e}")
            return {
                "is_clear": True,
                "final_requirements": {"summary": user_goal, "details": []}
            }
...
"""
        ...
        try:
            ...
            self._log(f"코드 분석 실패: {e}")
            return {"summary": "분석 실패", "issues": [], "suggestions": []}
...
"""
        ...
        try:
            ...
            self._log(f"변경사항 리뷰 실패: {e}")
            return {"approved": True, "summary": "리뷰 실패", "issues": [], "suggestions": []}
