...
class HLClient(object):
    ...
    def execute(self, request_str: str, variable_values=None) -> Dict[str, dict]:
        ...
        def _execute(request_str: str, variable_values=None):
            ...
            # Pre-serialize variable_values using our custom JSON encoder to handle
            # UUIDs, datetimes, and other custom types, then parse back to dict
            if variable_values is not None:
                serialized = HLJSONEncoder().encode(variable_values)
                variable_values = json.loads(serialized)
