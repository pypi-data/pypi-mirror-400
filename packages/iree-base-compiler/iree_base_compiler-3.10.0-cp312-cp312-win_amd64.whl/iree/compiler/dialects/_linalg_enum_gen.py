...
class WinogradConv2DFmr(IntEnum):
    ...
    def __str__(self):
        if self is WinogradConv2DFmr.F_2_3:
            return "F_2_3"
        if self is WinogradConv2DFmr.F_4_3:
            return "F_4_3"
        if self is WinogradConv2DFmr.F_2_5:
            return "F_2_5"
        raise ValueError("Unknown WinogradConv2DFmr enum entry.")



@register_attribute_builder("WinogradConv2DFmr")
def _winogradconv2dfmr(x, context):
    return _ods_ir.IntegerAttr.get(_ods_ir.IntegerType.get_signless(32, context=context), int(x))
