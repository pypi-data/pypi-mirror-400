...
def make_indices_copy_from_cpu(nlist, d, qtype, by_residual, metric, clamp):
    ...
    idx_cpu = faiss.IndexIVFScalarQuantizer(quantizer_cp, d, nlist, qtype, metric, by_residual)
...
def make_indices_copy_from_gpu(nlist, d, qtype, by_residual, metric, clamp):
    ...
    res = faiss.StandardGpuResources()
    res.noTempMemory()
    idx_gpu = faiss.GpuIndexIVFScalarQuantizer(res, d, nlist, qtype, metric, by_residual)
