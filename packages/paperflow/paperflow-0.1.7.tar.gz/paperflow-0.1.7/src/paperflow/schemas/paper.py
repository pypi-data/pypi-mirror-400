...
class SearchResult(BaseModel):
    ...
    def __str__(self) -> str:
        ...
        for i, paper in enumerate(self.papers, 1):
            authors = ", ".join(paper.get_author_names()[:3])  # Limit to 3 authors
            if len(paper.get_author_names()) > 3:
                authors += " et al."
            title = paper.title[:60] + "..." if len(paper.title) > 60 else paper.title
            year = paper.year or "N/A"
            source = paper.source.value
            # Add link/ID column
            link_id = paper.arxiv_id or paper.doi or paper.url
            if link_id and len(link_id) > 50:
                link_id = link_id[:47] + "..."
            table_data.append([i, title, authors, year, source, link_id or "N/A"])
