            ...
            if "persistent_load" in error_msg or "persistent id" in error_msg:
                ...
                try:
                    ...
                    try:
                        ...
                        # Last resort: try torch.load with file path directly (might work better for some files)
                        if torch is not None:
                            try:
                                logger.info(f"🔍 Final attempt: trying torch.load with file path directly")
                                result = torch.load(es_path, weights_only=False, map_location='cpu')
                                logger.info(f"✅ Successfully loaded with torch.load (file path)")
