...
def get_app_status(
    ...
    if not ignore_dependencies:
        ...
        if tasks_api_client:
            tasks_api_status = tasks_api_client.get_status() or {'status': 'n/a'}
            tasks_api_worker_status = tasks_api_client.get_worker_status() or {'status': 'n/a'}
            response_data['tasks_api_status'] = tasks_api_status
            response_data['tasks_api_worker_status'] = tasks_api_worker_status
            if tasks_api_status['status'].lower() != 'ok' or tasks_api_worker_status['status'].lower() != 'ok':
                error_messages.append('Error connecting to the Tasks API.')
