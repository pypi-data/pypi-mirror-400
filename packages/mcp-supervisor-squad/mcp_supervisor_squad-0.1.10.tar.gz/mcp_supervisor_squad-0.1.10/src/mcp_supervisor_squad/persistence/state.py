...
class RunState:
    ...
    polling_max_ms: int = 120_000
    polling_jitter_ratio: float = 0.1
    mode: str = "single"  # single|dual
    phase: str = "planner_run"  # planner_run|coder_run|integrate|cleanup|done|error
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "RunState":
        ...
        return cls(
            ...
            polling_max_ms=int(payload.get("polling_max_ms") or 120_000),
    ...
    def _poll_cap_ms(self) -> int:
        return int(min(PHASE_POLL_CAP_MS.get(self.phase, BACKOFF_SCHEDULE_MS[-1]), int(self.polling_max_ms or 120_000)))
    ...
    def compute_recommended_poll_ms(self, *, now: datetime | None = None) -> int:
        ...
        # Phase-aware backoff: long-running phases should reduce polling quickly.
    ...
    def public_status(self, *, options: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if cache:
            ...
            if elapsed_ms < int(cache.recommended_poll_ms):
                ...
                if len(lines) >= 3:
                    # Keep status stable and tell caller how long until next refresh.
                    # Replace the last segment to avoid duplicating wait hints.
                    third = lines[2]
                    if " | 强制等待:" in third:
                        third = third.split(" | 强制等待:")[0]
                    if " | 建议等待:" in third:
                        third = third.split(" | 建议等待:")[0]
                    lines[2] = f"{third} | 强制等待: {remaining_ms//1000}s"
                return {
                    "run_id": self.run_id,
