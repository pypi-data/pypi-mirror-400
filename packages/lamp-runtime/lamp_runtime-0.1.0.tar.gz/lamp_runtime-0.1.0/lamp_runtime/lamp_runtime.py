...
class SandboxClient:
    ...
    def __init__(
        ...
        mounts: list[dict[str, str]] | None = None,  # List of mount configs: [{"src": "/path/src", "dest": "/path/dest"}]
    ):
        ...
        self.mounts = mounts or []  # Initialize with empty list if None

        self.port_forward_process: subprocess.Popen | None = None

        # Validate mounts configuration early
    ...
    def _validate_mounts(self):
        """Validates the mounts configuration."""
        if not self.mounts:
            return
        ...
        for idx, mount in enumerate(self.mounts):
            if not isinstance(mount, dict):
                raise ValueError(
                    f"Mount at index {idx} must be a dictionary, got {type(mount).__name__}")

            if 'src' not in mount:
                raise ValueError(
                    f"Mount at index {idx} is missing required 'src' field")

            if 'dest' not in mount:
                raise ValueError(
                    f"Mount at index {idx} is missing required 'dest' field")

            if not isinstance(mount['src'], str) or not mount['src']:
                raise ValueError(
                    f"Mount at index {idx}: 'src' must be a non-empty string")

            if not isinstance(mount['dest'], str) or not mount['dest']:
                raise ValueError(
                    f"Mount at index {idx}: 'dest' must be a non-empty string")

    def mount_directories(self):
        """
        Mounts directories inside the sandbox pod by executing mount commands.
        This method should be called after the sandbox is ready.
        """
        if not self.mounts:
            logging.info("No directories to mount.")
            return

        logging.info(f"Mounting {len(self.mounts)} director{'y' if len(self.mounts) == 1 else 'ies'}...")

        for mount in self.mounts:
            src = mount['src']
            dest = mount['dest']
            mount_type = mount.get('type', 'bind')  # Default to bind mount
            options = mount.get('options', 'rw')  # Default to read-write

            logging.info(f"Mounting {src} -> {dest} (type: {mount_type}, options: {options})")

            try:
                # Create destination directory if it doesn't exist
                mkdir_result = self.run(f"mkdir -p {dest}")
                if mkdir_result.exit_code != 0:
                    logging.warning(
                        f"Failed to create directory {dest}: {mkdir_result.stderr}")

                # Execute mount command
                if mount_type == 'bind':
                    # For bind mounts
                    mount_cmd = f"mount --bind {src} {dest}"
                else:
                    # For other mount types (e.g., tmpfs, nfs)
                    mount_cmd = f"mount -t {mount_type} -o {options} {src} {dest}"

                result = self.run(mount_cmd)

                if result.exit_code == 0:
                    logging.info(f"Successfully mounted {src} to {dest}")
                else:
                    logging.error(
                        f"Failed to mount {src} to {dest}: {result.stderr}")
                    raise RuntimeError(
                        f"Mount command failed for {src} -> {dest}: {result.stderr}")

            except Exception as e:
                logging.error(f"Error mounting {src} to {dest}: {e}")
                raise RuntimeError(
                    f"Failed to mount directory {src} to {dest}") from e
    ...
    def __enter__(self) -> 'SandboxClient':
        ...
        # Mount directories if configured
        if self.mounts:
    ...
    def exec(self, command: str | list[str], container: str | None = None,
             stdin: bool = False, tty: bool = False) -> ExecutionResult:
        """
        Execute a command in the sandbox pod using Kubernetes exec API.
        ...
        Returns:
            ExecutionResult containing stdout, stderr, and exit_code

        Raises:
            RuntimeError: If sandbox is not ready or pod_name is not set
        """
        if not self.pod_name:
            raise RuntimeError("Cannot exec: pod_name is not set. Ensure the sandbox is created.")

        if not self.namespace:
            raise RuntimeError("Cannot exec: namespace is not set.")
        ...
        logging.info(f"Executing command in pod '{self.pod_name}': {command}")
        ...
        try:
            ...
            logging.info(f"Command execution completed with exit code: {exit_code}")
            ...
            raise RuntimeError(
                f"Failed to execute command in pod '{self.pod_name}': {e}"
            ) from e
        except Exception as e:
            logging.error(f"Unexpected error during exec: {e}")
            raise RuntimeError(
                f"Failed to execute command in pod '{self.pod_name}': {e}"
            ) from e
