...
def inbox_paths(layout: StorageLayout) -> tuple[Path, Path]:
    ...
    return inbox_dir / "external_report.md", inbox_dir / "external_report.json"


def read_external_report(*, layout: StorageLayout, max_kb: int) -> ExternalReport:
    ...
    if json_path.exists():
        try:
            if json_path.stat().st_size > max_b:
                return ExternalReport(kind="json", path=json_path, payload=None, text=None)
        except OSError:
            return ExternalReport(kind="json", path=json_path, payload=None, text=None)
        raw = read_json(json_path)
        if isinstance(raw, dict):
            return ExternalReport(kind="json", path=json_path, payload=raw, text=None)
        return ExternalReport(kind="json", path=json_path, payload=None, text=None)
    ...
    if md_path.exists():
        try:
            ...
            return ExternalReport(kind="md", path=md_path, payload=None, text=None)
        if len(data) > max_b:
            data = data[:max_b]
            try:
                ...
                pass
        return ExternalReport(kind="md", path=md_path, payload=None, text=data.decode("utf-8", errors="replace"))

    return ExternalReport(kind="none", path=None, payload=None, text=None)
...
def write_compact(*, layout: StorageLayout, compact_md: str, max_kb: int) -> Path:
    ...
    raw = (compact_md or "").encode("utf-8", errors="replace")
...
def archive_and_clear_inbox(*, layout: StorageLayout, run_id: str, keep: int, max_kb: int) -> None:
    md_path, json_path = inbox_paths(layout)
    ...
    for src in (json_path, md_path):
        if not src.exists():
            continue
        dst = archive_dir / f"{ts}-{run_id}-{src.name}"
        try:
...
def write_kb_index(*, layout: StorageLayout, run_id: str, note: str) -> None:
    """
    ...
    if not isinstance(existing, dict):
        existing = {}
    existing.update({"updated_at": utc_now_iso(), "last_run_id": run_id, "note": str(note)[:500]})
