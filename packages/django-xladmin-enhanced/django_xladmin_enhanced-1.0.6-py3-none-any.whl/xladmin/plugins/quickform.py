...
from xladmin.util import get_model_from_relation, vendor
from xladmin.views import BaseAdminPlugin, ModelFormAdminView
from xladmin.layout import Layout
...
class QuickFormPlugin(BaseAdminPlugin):
    ...
    def init_request(self, *args, **kwargs):
        if self.request.method == 'GET' and self.is_ajax(self.request) or self.request.GET.get('_ajax'):
            self.admin_view.add_form_template = 'xladmin/views/quick_form.html'
            self.admin_view.change_form_template = 'xladmin/views/quick_form.html'
            return True
        return False
...
class RelatedFieldWidgetWrapper(forms.Widget):
    ...
    @property
    def media(self):
        media = self.widget.media + vendor('xladmin.plugin.quick-form.js')
        return media
