...
def test_from_config():
    codec = numcodecs.registry.get_codec(dict(id="bitmap-index"))
    assert codec.__class__.__name__ == "BitmapIndexCodec"
    assert codec.__class__.__module__ == "numcodecs_bitmap_index"
...
def test_roundtrip():
    ...
    check_roundtrip(np.round(np.sqrt(np.arange(1000).reshape(10, 10, 10))))
    check_roundtrip(
        np.round(np.sqrt(np.arange(1000).reshape(10, 10, 10))).astype(
