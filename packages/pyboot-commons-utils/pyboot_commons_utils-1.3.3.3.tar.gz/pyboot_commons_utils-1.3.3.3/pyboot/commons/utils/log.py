...
class Logger:
    ...
    @staticmethod            
    def level(level:int|str)->LEVEL:
        assert level, f'level必须设置，但是获得{level}'
        old_level = level
        level:str = f'{level}'.strip().lower()
        
        if level == '10' or level == 'debug':
            level = Logger.LEVEL.DEBUG
        elif level == '20' or level == 'info':
            level = Logger.LEVEL.INFO
        elif level == '30' or level == 'warn' or level == 'warning':
            level = Logger.LEVEL.WARNING
        elif level == '40' or level == 'error':
            level = Logger.LEVEL.ERROR
        elif level == '50' or level == 'fatal' or level == 'critical':
            level = Logger.LEVEL.FATAL
        else:
            raise ValueError(f'level设置只能10|debug;20|info;30|warn|warning;40|error;50|fatal|critical,但是获得{old_level}')
