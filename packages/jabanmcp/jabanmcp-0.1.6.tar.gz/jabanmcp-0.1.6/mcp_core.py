    ...
    def _print_mcp_start_info(self):
        ...
        try:
            if sys.version_info >= (3, 11):
                ...
                package_version = version("jabanmcp")
            else:
                # 兼容低版本或未安装时的读取方式，尝试读取 pyproject.toml (开发环境)
                # 注意：生产环境打包后可能没有 pyproject.toml，通常依赖 importlib.metadata
                try:
                    import tomllib
                    with open("pyproject.toml", "rb") as f:
                     ...
                     # Python 3.10 及以下没有 tomllib，且未安装第三方库时
                    package_version = "Unknown (Dev)"
                except FileNotFoundError:
                    package_version = "Unknown (Installed)"
        except Exception:
             package_version = "Unknown"

        self.logger.info(f"MCP 版本：{package_version}")
