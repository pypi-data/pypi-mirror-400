...
class GPUManager:
    ...
    def send_email(self, subject: str, message: str, verbose: bool = True) -> bool:
        """
        Send an email notification to yourself.

        This sends an email to the authenticated user's email address.
        Useful for notifications when long-running tasks complete.
    ...
    def lambda_gpu(
        ...
        the script runs, results are uploaded to S3, and the session is stopped.
        All in one blocking call.

        Output files are uploaded to S3 to avoid saturating local disk.
        If a lambda with the same lambda_id is already in queue, it will be
        replaced by the new execution.
...
'''
                ...
                try:
                    gpu.run(f"pip install boto3 -q")
                    gpu.run(f"cat > /tmp/s3_upload.py << 'EOFSCRIPT'\n{s3_upload_script}\nEOFSCRIPT")
                    s3_result = gpu.run("python /tmp/s3_upload.py")
                    s3_stdout = s3_result.get('stdout', '') if isinstance(s3_result, dict) else str(s3_result)
