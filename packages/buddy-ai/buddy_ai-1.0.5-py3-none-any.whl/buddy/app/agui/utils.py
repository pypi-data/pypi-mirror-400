...
from buddy.run.response import RunEvent, RunResponseContentEvent, RunResponseEvent, RunResponsePausedEvent
from buddy.run.team import RunResponseContentEvent as TeamRunResponseContentEvent
from buddy.run.team import TeamRunEvent, TeamRunResponseEvent


@dataclass
class EventBuffer:
    """Buffer to manage event ordering constraints, relevant when mapping Buddy responses to Buddy-UI events."""
...
def convert_agui_messages_to_BUDDY_messages(messages: List[AGUIMessage]) -> List[Message]:
    """Convert Buddy-UI messages to Buddy messages."""
    result = []
    for msg in messages:
        if msg.role == "tool":
...
def stream_BUDDY_response_as_agui_events(
    response_stream: Iterator[Union[RunResponseEvent, TeamRunResponseEvent]], thread_id: str, run_id: str
) -> Iterator[BaseEvent]:
    """Map the Buddy response stream to Buddy-UI format, handling event ordering constraints."""
    message_id = str(uuid.uuid4())
...
async def async_stream_BUDDY_response_as_agui_events(
