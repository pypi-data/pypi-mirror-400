...
class RunPackageInstallerBase(InstallerBase):
    def __init__(
        ...
        super().__init__(f"run/{production_name}", version, download_path)
        self.os = os_
        self.arch = arch
    ...
    @abstractmethod
    def get_activation_command_list(self, target_path: str) -> List[str]:
        pass

    def post_install_all(self, target_path):
        assert target_path is not None
        activation_commands = self.get_activation_command_list(target_path)
