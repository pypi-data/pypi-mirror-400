...
class TraceManager:
    ...
    def finalize_and_send(
        ...
        *,
        ...
        # Send trace_end immediately without batching (critical message)
        if _sqs_instance and hasattr(_sqs_instance, 'send_immediate'):
            _sqs_instance.send_immediate(trace_end_event)
        else:
            send_to_sqs(trace_end_event)
        
        print(f"[VeriskGO] Trace ended: {trace_id}\n")
        
        # Brief delay to ensure trace_end is processed
        time.sleep(0.2)
        
        # Clear active trace AFTER all messages are flushed
        with cls._lock:
    ...
    def start_trace(cls, name: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        with cls._lock:
            ...
            cls._active["stack"] = []  # Start with empty stack - decorators will manage it
    ...
    @classmethod
    def _get_metadata_with_project_id(cls, additional_metadata: Optional[Dict] = None) -> Dict:
        """Get metadata dict with project_id auto-injected from active trace."""
        from .config import get_cfg
        cfg = get_cfg()
        
        # Start with active trace metadata (includes project_id)
        base_metadata = cls._active.get("metadata", {}).copy()
        
        # Merge with additional metadata
        if additional_metadata:
            base_metadata.update(additional_metadata)
        
        # Ensure project_id is always present
        if "project_id" not in base_metadata:
            base_metadata["project_id"] = cfg.get("project_id", "default")
    ...
    def start_span(cls, name: str, input_data=None, tags=None):
        with cls._lock:
            ...
            span = {
                ...
                "timestamp": cls._now(),
                "input": input_data,
                "metadata": cls._get_metadata_with_project_id(tags),
                "output": None,
                "duration_ms": 0,
            }
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            try:
                ...
                span_event = {
                    ...
                    "metadata": TraceManager._get_metadata_with_project_id(tags)
                }
            ...
            # Pop from stack AFTER function completes
            with TraceManager._lock:
                if TraceManager._active["stack"] and TraceManager._active["stack"][-1]["span_id"] == span_id:
                    TraceManager._active["stack"].pop()
