                ...
                try:
                    ...
                    while current_part in buffered_parts:
                        ...
                        if already_verified:
                            ...
                            # Send extraction notification to main process
                            extraction_notif = ExtractionNotification(
                                part_id=current_part,
                                bytes_written=total_written,
                                already_verified=True,
                            )
                            try:
                                results_queue.put(extraction_notif, timeout=1.0)
                            except Exception as e:
                                logging.warning(
                                    f"Failed to send extraction notification: {e}"
                                )
                        else:
...
def tar_extractor_process(
    ...
    - Sends completion signal via results_queue

    Args:
        fifo_path: Path to FIFO pipe
        destination: Destination directory for extraction
        results_queue: Queue for sending completion signal to main process
        stop_event: Event to signal stop (not currently used but matches pattern)
    """
    from flux_bootstrap.data_structures import TarExtractionComplete
    ...
    try:
        ...
        try:
            results_queue.put(TarExtractionComplete(success=True), timeout=1.0)
        except Exception as e:
            logging.warning(f"Failed to send completion signal: {e}")

        sys.exit(0)

    except tarfile.ReadError as e:
        error_msg = f"Tar extraction stopped: {e}"
        logging.info(error_msg)

        # Send failure signal
        with contextlib.suppress(Exception):
            results_queue.put(
                TarExtractionComplete(success=False, error_message=error_msg),
                timeout=1.0,
            )
