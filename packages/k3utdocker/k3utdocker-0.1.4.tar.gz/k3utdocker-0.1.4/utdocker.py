...
def start_container(name, image, ip=None, command="", port_bindings=None, volume_bindings=None, env=None):
    net_name = "network1"
    dcli = get_client()
    ...
    kwargs["host_config"] = dcli.api.create_host_config(**_config_kwargs)

    if ip is not None:
        net_cfg = dcli.api.create_networking_config(
            {
                net_name: dcli.api.create_endpoint_config(
                    ipv4_address=ip,
                )
            }
        )
    ...
    if not does_container_exist(name):
        dcli.api.create_container(name=name, image=image, command=command, **kwargs)
...
def build_image(image, path):
    ...
    for line in dcli.api.build(path=path, nocache=True, tag=image):
        dd("build " + image + ":", line)
