...
def distance_cost(predictions, targets, img_w):
    """Repeat predictions and targets to generate all combinations use the abs
    distance as the new distance cost."""
    ...
    predictions = torch.repeat_interleave(
        predictions, num_targets, dim=0)[..., 6:]
...
def assign(
    ...
    '''
    computes dynamically matching based on the cost, including cls cost and
    lane similarity cost
    Args:
        predictions (Tensor): predictions predicted by each stage,
            shape: (num_priors, 78)
    ...
    theta_score = torch.cdist(
        predictions[:, 4].unsqueeze(-1), target_thetas, p=1).reshape(
            num_priors, num_targets) * 180
    ...
    iou = line_iou(
        predictions[..., 6:], targets[..., 6:], img_w, aligned=False)
