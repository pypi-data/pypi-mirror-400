...
Options:
    --provider <name>               LLM provider for generation (anthropic, openai, google, outpost)
...
def cmd_generate(goal: str, output: Optional[str] = None, provider: Optional[str] = None, verbose: bool = False) -> int:
    ...
    try:
        ...
        from blueprint.providers.registry import list_providers
    except ImportError:
    ...
    print(f"Generating Blueprint for: {goal}{provider_info}")
    print()
    
    try:
        # Decompose goal into tasks
        if verbose:
            print("[1/3] Decomposing goal into tasks...")
            if provider:
                print(f"      Using provider: {provider}")
            else:
                print(f"      Available providers: {', '.join(list_providers())}")
        tasks = decompose_goal(goal, provider=provider, return_dicts=True)
        if verbose:
            print(f"      Generated {len(tasks)} tasks")

        # Infer interfaces
        if verbose:
            print("[2/3] Inferring interface contracts...")
...
def print_usage():
    ...
    print("  --provider <name>     LLM provider: anthropic, openai, google, outpost")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if command == "parse":
        ...
        # Parse output and provider options
        output = None
        provider = None
        goal_parts = []
        i = 0
        ...
        return cmd_generate(goal, output=output, provider=provider, verbose=verbose)
