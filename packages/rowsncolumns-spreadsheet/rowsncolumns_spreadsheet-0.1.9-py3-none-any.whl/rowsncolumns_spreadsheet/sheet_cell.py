...
class SheetCell(Generic[T]):
    ...
    def _get_cell_shared_string_index(self, cell_data: Optional[T]) -> Optional[int]:
        ...
        index = None
        if isinstance(cell_data, dict):
            index = cell_data.get('ss')
        return self._normalize_shared_string_index(index)

    def _normalize_shared_string_index(self, value: Optional[Any]) -> Optional[int]:
        """Coerce shared string index to an int when possible."""
        if value is None:
            return None
        # Accept ints directly
        if isinstance(value, bool):
            # bool is an int subclass but not valid for indexes here
            return None
        if isinstance(value, int):
            return value
        # Accept numeric strings or floats that are whole numbers
        try:
