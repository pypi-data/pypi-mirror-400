...
class SpeedMonitor:
    ...
    def __init__(
        ...
        # Instantaneous speed tracking (for progress reporting)
    ...
    def get_instantaneous_speed_mbps(self) -> float:
        """
        Get instantaneous download speed since last measurement.

        This calculates speed based on bytes downloaded since the last call,
        providing current network throughput rather than cumulative average.
        Used for progress reporting to show real-time speed.

        Returns:
            Instantaneous speed in Mbps, or 0.0 for first measurement
        """
        now = time.perf_counter()

        if self.last_measurement_time is None:
            # First measurement - initialize but return 0 (no interval yet)
            self.last_measurement_time = now
            self.last_measurement_bytes = self.window_bytes
            return 0.0
        ...
        bytes_delta = self.window_bytes - self.last_measurement_bytes
    ...
    def get_elapsed_time(self) -> float:
        ...
        if self.window_start is None:
            return 0.0
        return time.perf_counter() - self.window_start
