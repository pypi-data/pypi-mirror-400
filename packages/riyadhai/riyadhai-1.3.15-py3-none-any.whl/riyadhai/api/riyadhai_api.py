...
class RiyadhAIAPI:
    ...
    rai_api = api.RiyadhAIAPI()
    rooms = await rai_api.room.list_rooms(api.proto_room.ListRoomsRequest(names=['test-room']))
    ```
    """
    ...
    def __init__(
        ...
        *,
        ...
        """Create a new RiyadhAIAPI instance.

        Args:
            url: RiyadhAI server URL (read from `RIYADHAI_URL` environment variable if not provided)
            api_key: API key (read from `RIYADHAI_API_KEY` environment variable if not provided)
            api_secret: API secret (read from `RIYADHAI_API_SECRET` environment variable if not provided)
        ...
        """
        url = url or os.getenv("RIYADHAI_URL")
