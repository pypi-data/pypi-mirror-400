...
class BaseBuddy(GObject.GObject):
    ...
    owner = GObject.Property(
        type=bool, getter=get_owner, setter=set_owner, default=False
    )
    ...
    ip4_address = GObject.Property(
        type=str, getter=get_ip4_address, setter=set_ip4_address
    )
...
class Buddy(BaseBuddy):
    ...
    def __init__(self, account_path, contact_id):
        ...
        handles = obj.RequestHandles(
            HANDLE_TYPE_CONTACT, [self.contact_id], dbus_interface=CONNECTION
        )
    ...
    def do_get_property(self, pspec):
        if pspec.name == "nick" and self._get_attributes_call is not None:
            _logger.debug(
                "%r: Blocking on GetContactAttributes() because "
                "someone wants property nick",
                self,
            )
            self._get_attributes_call.block()
        elif pspec.name != "nick" and self._get_properties_call is not None:
            _logger.debug(
                "%r: Blocking on GetProperties() because someone " "wants property %s",
                self,
                pspec.name,
            )
