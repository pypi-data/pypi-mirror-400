...
class BaseCLIWrapper(ABC):
    ...
    def ask(self, prompt_text: str, timeout: int = 120, stream_callback: Optional[Callable[[str, str], None]] = None) -> str:
        ...
        try:
            ...
            if process.returncode != 0 and process.returncode != -15 and process.returncode != -9: # Ignore termination signals
    ...
    def terminate(self):
        ...
        if self._current_process:
            # We use terminate() to allow graceful cleanup if possible, or kill() if needed
