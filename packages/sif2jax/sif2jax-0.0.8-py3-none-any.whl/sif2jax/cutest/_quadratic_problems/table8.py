...
_A_ROWS = jnp.asarray(
    np.load(os.path.join(_data_dir, "table8_A_rows.npy")), dtype=jnp.int32
)
_A_COLS = jnp.asarray(
    np.load(os.path.join(_data_dir, "table8_A_cols.npy")), dtype=jnp.int32
)
_A_VALS = jnp.asarray(np.load(os.path.join(_data_dir, "table8_A_vals.npy")))
_LOWER_BOUNDS = jnp.asarray(np.load(os.path.join(_data_dir, "table8_lower_bounds.npy")))
_UPPER_BOUNDS = jnp.asarray(np.load(os.path.join(_data_dir, "table8_upper_bounds.npy")))
_Q_DIAG_VALS = jnp.asarray(np.load(os.path.join(_data_dir, "table8_Q_diag_vals.npy")))
_M_VAL = int(np.load(os.path.join(_data_dir, "table8_m.npy")))
