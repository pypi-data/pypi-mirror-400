...
@click.command()
@click.option("--id", help="The ID of the Asset (file or folder) to index.")
@click.option("--url", help="The Nomad URL of the Asset (file or folder) to index (bucket::object-key).")
@click.option("--object-key", help="Object-key of the Asset (file or folder) to index. This option assumes the default bucket that was previously set with the `set-bucket` command.")
@click.pass_context
def index_asset(ctx, id, url, object_key):
    """Index asset"""
    initialize_sdk(ctx)
