...
class JobsResource:
    ...
    def run(
        ...
        Args:
            ...
            proxy_port: Port for reverse proxy (default: 8080 if not specified)
...
class ProxyTargetsResource:
    """Resource for managing custom proxy targets (IPv6 -> subdomain mappings)"""
    ...
    def create(
        ...
        """
        Create a custom proxy target to expose an IPv6 address via a public URL.

        Args:
            ipv6_address: The IPv6 address to proxy to
            port: The port to proxy to (default: 8080)
            subdomain: Optional custom subdomain (auto-generated if not provided)
            name: Optional friendly name for the proxy target
            description: Optional description

        Returns:
            ProxyTarget with the proxy_url for accessing the target
        ...
        Example:
            ...
            ...     name="My Web Server"
            ... )
            >>> print(target.proxy_url)  # https://abc123def456.app.lfg.run
        """
        data = {
        ...
        response = self.client._make_request("POST", "/api/v1/proxy-targets", data=data)
        return ProxyTarget(**response)

    def list(self) -> List[ProxyTarget]:
        """
        List all proxy targets for the current user.

        Returns:
            List of ProxyTarget objects
        """
        response = self.client._make_request("GET", "/api/v1/proxy-targets")
        targets = response.get("proxy_targets", [])
        return [ProxyTarget(**t) for t in targets]

    def get(self, target_id: str) -> ProxyTarget:
        """
        Get a specific proxy target by ID.

        Args:
            target_id: The proxy target ID

        Returns:
            ProxyTarget object
        """
        response = self.client._make_request("GET", f"/api/v1/proxy-targets/{target_id}")
        return ProxyTarget(**response)
    ...
    def update(
        ...
        Args:
            target_id: The proxy target ID
            ipv6_address: New IPv6 address (optional)
            port: New port (optional)
            name: New name (optional)
            description: New description (optional)

        Returns:
            Updated ProxyTarget object
        ...
        response = self.client._make_request("PUT", f"/api/v1/proxy-targets/{target_id}", data=data)
        return ProxyTarget(**response)
    ...
    def delete(self, target_id: str) -> None:
        ...
        Args:
            target_id: The proxy target ID
        """
        self.client._make_request("DELETE", f"/api/v1/proxy-targets/{target_id}")
