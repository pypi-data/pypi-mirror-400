...
def get_parser():
    ...
    parser.add_argument(
        "-i", "--eventid", metavar="EVENTID", type=str, help="Specify an event ID"
    )
    ...
    ohelp = (
        "If the '-a' argument is used, send the output to a file. "
        "Denote the format using '-f'."
    )
    parser.add_argument("-o", "--outfile", help=ohelp)
    rhelp = (
        "Change the search radius (km) around the specified latitude and longitude. Default is %.0f km."
        % SEARCH_RADIUS
    )
    parser.add_argument("-r", "--radius", type=float, help=rhelp)
    parser.add_argument(
        "-u",
        "--url",
        dest="print_url",
        action="store_true",
        help="Print the URL associated with event.",
        default=False,
    )
    vstr = (
        "Print time and distance deltas, and azimuth from input " "parameters to event."
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    whelp = (
        "Change the window (sec) around the specified time. Default is %.0f seconds."
        % TIME_WINDOW
    )
...
def main():
    ...
    if argsum > 1:
        msg = (
            "The -a, -v, and -u options are mutually exclusive. "
    ...
    event_df = find_nearby_events(time, lat, lon, twindow, radius)

    if event_df is None:
        logging.error("No events found matching your search criteria. Exiting.")
        sys.exit(0)
