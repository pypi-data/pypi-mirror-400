 ...
 */
/***
x86_64-w64-mingw32-g++ -std=c++17 -O2 -DWIN32_LEAN_AND_MEAN -D_WIN32_WINNT=0x0601 \
    -o DumpPEFromMemory.exe DumpPEFromMemory.cpp -lntdll
...
BOOL DumpPEFromMemory(LPCSTR filename, char* outputbin)
{
	...
	if (status == 0) 
	{
        ...
        if (lastNonZero >= 0) {
            FILE *trimFile = fopen(outputbin, "rb+");
            if (trimFile) {
                _chsize_s(_fileno(trimFile), lastNonZero + 1);
                fclose(trimFile);
                printf("[+] Trimmed trailing zeros. New size: 0x%lx\n", lastNonZero + 1);
            }
        }
...
BOOL InflateFromDisk(LPCSTR filename, char* outputbin)
{
	...
	PBYTE PeBase = (PBYTE)MapViewOfFile(hSection, FILE_MAP_READ, 0, 0, 0);
	if (PeBase == NULL) {
