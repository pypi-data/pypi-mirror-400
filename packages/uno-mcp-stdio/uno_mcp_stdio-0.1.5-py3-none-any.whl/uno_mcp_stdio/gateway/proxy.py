...
class GatewayProxy:
    ...
    async def list_tools_with_default_token(self) -> Dict[str, Any]:
        """
        使用 default readonly token 获取工具列表
        
        用于未认证时显示可用工具，但不能调用工具。
        """
        # 构建请求
        payload = {
            "jsonrpc": "2.0",
        ...
        self._log("使用 default token 获取工具列表")
        
        # 发送请求（使用 default token）
        client = await self._get_client()
        ...
        try:
            response = await client.post(
                ...
                headers={
                    "Content-Type": "application/json",
                    "Authorization": f"Bearer {settings.default_readonly_token}"
                }
            )
            
            if response.status_code == 401:
                self._log("Default token 被拒绝")
                raise GatewayError("Default token 无效", status_code=401)
            ...
            result = response.json()
            self._log(f"使用 default token 获取工具列表成功")
            return result
