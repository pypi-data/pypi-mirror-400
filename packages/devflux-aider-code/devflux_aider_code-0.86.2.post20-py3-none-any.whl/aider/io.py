...
# Constants
NOTIFICATION_MESSAGE = "Aider(@horizon) is waiting for your input"
...
class InputOutput:
    ...
    def __init__(
        ...
        if self.input_history_file:
            try:
                Path(f"{self.input_history_file}").parent.mkdir(
    ...
    def rule(self):
        if self.pretty:
            if self.user_input_color:
                self.console.rule("[bold]Aider(@Horizon)", style=self.user_input_color)
            else:
                self.console.rule("[bold]Aider(@Horizon)")
        else:
            print()
    ...
    def get_input(
        ...
        if rel_fnames:
            ...
            show = self.format_files_for_input(
                rel_fnames, rel_read_only_fnames, abs_rule_fnames, enabled_rules
            )

        prompt_prefix = ""
        if edit_format:
            prompt_prefix += self.i18n.t(edit_format)
...
{prefix} {hist}"""
    ...
    def _tool_message(self, message="", strip=True, color=None):
        ...
        try:
            ...
            self.console.file.flush()  # 내부 버퍼 강제 비우기
    ...
    def format_files_for_input(
        self, rel_fnames, rel_read_only_fnames, rule_fnames, enabled_rules
    ):

        if not self.pretty:
            read_only_files = []
            for full_path in sorted(rel_read_only_fnames or []):
                read_only_files.append(f"{full_path} (rule)")

            rule_files = []
            if enabled_rules:
                for full_path in sorted(rule_fnames):
                    rule_files.append(f"{full_path}")
            ...
            return "\n".join(read_only_files + rule_files + editable_files) + "\n"

        output = StringIO()
        console = Console(file=output, force_terminal=False)

        read_only_files = sorted(rel_read_only_fnames or [])
        rule_files = sorted(rule_fnames or [])
        editable_files = [
        ...
        if read_only_files:
            ...
            for rel_path in read_only_files:
                ro_paths.append(
                    Text(
                        rel_path.replace(
                            os.path.join(".aider", "rules") + os.path.sep, ""
                        ),
                        no_wrap=True,
                    )
                )
            files_with_label = [
                Text(f"{self.i18n.t('label.001')}:", no_wrap=True)
            ] + ro_paths
        ...
        if rule_files and enabled_rules:
            text_rule_files = [
                Text(os.path.basename(f), no_wrap=True) for f in rule_files
            ]
            files_with_label = text_rule_files
            if read_only_files:
                files_with_label = [
                    Text(f"{self.i18n.t('label.002')}:", no_wrap=True)
                ] + text_rule_files
                rule_output = StringIO()
                Console(file=rule_output, force_terminal=False).print(
                    Columns(files_with_label)
                )
                rule_lines = rule_output.getvalue().splitlines()

                if len(read_only_lines) > 1 or len(rule_lines) > 1:
                    console.print()

            console.print(Columns(files_with_label))

        if editable_files:
            text_editable_files = [Text(f, no_wrap=True) for f in editable_files]
            files_with_label = text_editable_files
            if read_only_files:
                files_with_label = [
                    Text(f"{self.i18n.t('label.003')}:")
                ] + text_editable_files
