...
class DockerV:
    ...
    def __init__(self, config, timeout=300, docker_url="unix://var/run/docker.sock"):
        if environ.get("DOCKER_HOST"):
            self.__cli = from_env(timeout=timeout).api
            self.__cli2 = from_env(timeout=timeout)
        else:
    ...
    def unset_docker_id(self):
        self.__docker_id = False
    ...
    def exec_cmd(self, cmd, user=None, container_name=False):
        ...
        container_name = container_name or self.docker_id
        try:
            exec_id = self.__cli.exec_create(container_name, cmd, user=user)
            ...
            if 'is not running' in error_msg:
                raise errors.NotRunning(container_name)
            ...
            if hasattr(error, 'is_client_error') and error.is_client_error():
                raise errors.ClientAPIError(
                    "Container {0} is in conflict or stopped unexpectedly".format(container_name))
            raise
    ...
    def create_container(self, env_vars, volume_binds, volumes):
        ...
        safe_ports = self._secure_port_bindings(hconfig.get('PortBindings'))
        hconfig.get('PortBindings').update(safe_ports)
    ...
    def _secure_port_bindings(self, port_bindings):
        """Checks the ports that will be exposed in a container and changes their bindings
        so they are only exposed to localhost, Docker's default gateway, and any network
        defined in the `listen_addresses` parameter in the `deployer` section of the
        DeployV configuration file. This is to avoid security issues by exposing ports
        to the public interfaces.

        param port_bindings: Dict with the ports that will be exposed in the format:

            {
                "8069/tcp": [
                    {'HostIp': '0.0.0.0', 'HostPort': '17006'},
                ]
            }

        This dictionary is obtained from the PortBindings key in the hostconfig dictionary.

        :return: Dictionary in the same format as the one received, but were all the ports
        are exposed only to internal networks.

            {
                "8069/tcp": [
                    {'HostIp': '172.17.0.1', 'HostPort': '17006'},
                    {'HostIp': '127.0.0.1', 'HostPort': '17006'},
                ]
            }
        """
        res = {}
        ...
        for port, bindings in port_bindings.items():
            ...
            if deploy_cfg.get('listen_addresses'):
                for network in deploy_cfg.get('listen_addresses'):
                    safe_bindings.append({'HostIp': network, 'HostPort': host_port})
            res.update({port: safe_bindings})
        return res
    ...
    def exec_cmd_image(self, command, volumes, working_folder=False, image=False):
        ...
        while cont.status != 'exited':
            ...
            sleep(5)
            cont.reload()
        cmd_res = cont.logs(stdout=True, stderr=True)
        info = self.inspect(cont.short_id)
        exit_code = info.get('State').get('ExitCode')
        logger.debug("Command result: %s, Exit Code: %s", cmd_res, exit_code)
        cont.remove()
        if exit_code == 0:
            res = {'result': cmd_res.decode()}
        else:
            error = 'Command %(cmd)s failed with exit code: %(code)s %(error)s' % {
                "cmd": command, "code": exit_code, "error": cmd_res.decode()}
            res = {'error': error}
        return res
