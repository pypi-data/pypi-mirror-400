...
class FunctionTest(unittest.TestCase):
  ...
  def testNameArgs(self):
    ...
    self.assertRaisesRegex(le.EEException, 'Too many', TEST_FUNC.nameArgs,
                           [1, 2, 3])

  def testPromoteArgs(self):
    """Verifies that Functions can promote and verify their arguments."""
    old_promoter = le.Function._promoter
    le.Function._registerPromoter(lambda obj, type_name: [type_name, obj])
    ...
    self.assertRaisesRegex(le.EEException, 'Required argument',
                           TEST_FUNC.promoteArgs, {})

    # Disallow unknown arguments.
    self.assertRaisesRegex(le.EEException, 'Unrecognized',
                           TEST_FUNC.promoteArgs, {
                               'a': 42,
                               'c': 13
                           })
    ...
    # Clean up.
    le.Function._registerPromoter(old_promoter)
