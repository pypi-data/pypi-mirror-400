...
class UnoStdioServer:
    ...
    def _setup_handlers(self):
        ...
        async def handle_list_tools() -> list[Tool]:
            ...
            try:
                if is_authenticated:
                    # 已认证：使用真实 token 获取工具列表
                    response = await gateway_proxy.list_tools()
                else:
                    # 未认证：使用 default token 获取工具列表
                    self._log("未认证，使用 default token 获取工具列表")
                    response = await gateway_proxy.list_tools_with_default_token()
                ...
                if "result" in response and "tools" in response["result"]:
                    ...
                    if not is_authenticated:
                        tools.append(Tool(
                            ...
                            description="⚠️ 需要登录认证才能调用工具。请先调用此工具完成认证。",
                            inputSchema={
                                "type": "object",
                                "properties": {},
                                "required": []
