...
def init_project(
    ...
    """Initialize a project with only bvproject.yaml and main.py."""
    ...
    if create_venv:
        vm = VenvManager(project_root / ".venv")
        vm.create()
        # Install bv-runtime in the venv so it's available immediately
        try:
            vm.install(["bv-runtime"])
        except Exception as exc:
            # Non-fatal: user can install manually later
            import warnings
            warnings.warn(f"Failed to install bv-runtime in venv: {exc}. You can install it manually with: pip install bv-runtime")
...
def build_package(config_path: Path, output: Optional[Path], dry_run: bool) -> Path:
    ...
    # Ensure requirements.lock exists; if missing, create empty one
    lock_path = project_root / "requirements.lock"
    if not lock_path.exists():
        lock_path.write_text("# empty lock\n", encoding="utf-8")
...
def run_project(config_path: Path, entrypoint_name: Optional[str] = None) -> object:
    ...
    # Set BV_SDK_RUN environment variable to allow runtime access
