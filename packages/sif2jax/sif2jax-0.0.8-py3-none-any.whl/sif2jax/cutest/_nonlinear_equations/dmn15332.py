"""DMN15332 - Diamond Light Source powder diffraction data fitting.
...
def _load_dmn15332_data():
    """Load problem data from NPZ file."""
    data_file = Path(__file__).parent / "data" / "dmn15332.npz"
    return np.load(data_file)


# Load data once at module level
_DMN15332_DATA = _load_dmn15332_data()


class DMN15332(AbstractNonlinearEquations):
    """DMN15332 Diamond Light Source powder diffraction fitting problem.
    ...
    Jacobian precision issue: max absolute difference of 0.00227 at starting point
    vs pycutest reference. All other tests pass. Issue likely due to numerical
    precision in large-scale automatic differentiation (4643×66 Jacobian matrix).
    Relative error ~0.0000075% across 300k+ elements may be acceptable.
    """
    ...
    def n(self):
        """Number of variables."""
        return int(_DMN15332_DATA["n"])

    @property
    def m(self):
        """Number of equations."""
        return int(_DMN15332_DATA["m"])
    ...
    def residual(self, y, args):
        ...
        # Load data
        x_data = jnp.array(_DMN15332_DATA["x_data"])
        y_data = jnp.array(_DMN15332_DATA["y_data"])
        positions = jnp.array(_DMN15332_DATA["positions"])
    ...
    def y0(self):
        ...
        if self.y0_iD == 0:
            return jnp.array(_DMN15332_DATA["start1"])
        else:
            return jnp.array(_DMN15332_DATA["start2"])
