...
def search_preassigned(xq, k, index, quantizer, batch_size=0):
    ...
    for i0 in range(0, n, batch_size):
        ...
        D[i0 : i0 + batch_size], I[i0 : i0 + batch_size] = index.search_preassigned(
            xq[i0 : i0 + batch_size], k, Iq, Dq
        )
    return D, I
...
def tiled_search_preassigned(xq, k, index, quantizer, batch_size=32768):
    ...
    for i0 in range(0, n, batch_size):
        ...
        Dq, Iq = qq
        index.search_preassigned(xq[i0:i1], k, Iq=Iq, Dq=Dq, I=I[i0:i1], D=D[i0:i1])
        qq = qq_next.get()
...
class SeparateCoarseQuantizationIndex:
    ...
    def search(self, xq, k):
        ...
        if self.seq_tiling:
            return search_preassigned(xq, k, self.index_ivf, self.quantizer, self.bs)
        else:
            return tiled_search_preassigned(xq, k, self.index_ivf, self.quantizer, self.bs)
    ...
    def search(self, xq, k):
        ...
        if bs <= 0:
            ...
            def do_search(rank):
                ...
                Dall[rank], Iall[rank] = gpu_index.search_preassigned(xq, k, Iq, Dq)
            ...
            def do_search(rank, i0, qq):
                gpu_index = faiss.downcast_index(index.at(rank))
                Dq, Iq = qq
                Dall[rank, i0 : i0 + bs], Iall[rank, i0 : i0 + bs] = gpu_index.search_preassigned(
                    xq[i0 : i0 + bs], k, Iq, Dq
                )
            ...
            for i0 in range(0, nq, bs):
                ...
                list(self.pool.map(lambda rank: do_search(rank, i0, qq), range(ngpu)))
                qq = qq_next.get()
...
def set_index_parameter(index, name, val):
    ...
    if index.__class__ == SeparateCoarseQuantizationIndex:
        if name == "nprobe":
            ...
            set_index_parameter(index.quantizer, name[name.find("_") + 1 :], val)
        else:
            raise RuntimeError()
        return
    ...
    elif isinstance(index, faiss.IndexShards) or isinstance(index, faiss.IndexReplicas):
        for i in range(index.count()):
            sub_index = index.at(i)
            set_index_parameter(sub_index, name, val)
    elif name.startswith("quantizer_"):
        index_ivf = extract_index_ivf(index)
        set_index_parameter(index_ivf.quantizer, name[name.find("_") + 1 :], val)
    elif name == "efSearch":
        index.hnsw.efSearch
...
def main():
    ...
    aa("--useFloat16", default=False, action="store_true", help="GPU cloner options")
    aa("--useFloat16CoarseQuantizer", default=False, action="store_true", help="GPU cloner options")
    aa("--usePrecomputed", default=False, action="store_true", help="GPU cloner options")
    ...
    aa("--ivf_quant_nlist", type=int, default=1024, help="nb of invlists for IVF quantizer")
    aa(
        "--batch_size",
        type=int,
        default=-1,
    ...
    aa("--n_autotune", type=int, default=300, help="max nb of auto-tuning steps")
    ...
    op.add_range(
        "nprobe", [2**i for i in range(20) if 2**i < index_ivf.nlist * 0.1 and 2**i <= 4096]
    )
    ...
    if args.search_type == "cpu":
        ...
        op.add_range("quantizer_efSearch", [2**i for i in range(10)])
        op.restrict_range("nprobe", 2049)
        index = SeparateCoarseQuantizationIndex(quantizer_hnsw, index, bs=args.batch_size)
        ...
        res = faiss.StandardGpuResources()
        new_quantizer = faiss.index_factory(index_ivf.d, f"IVF{args.ivf_quant_nlist},Flat")
        ...
        index = SeparateCoarseQuantizationIndex(quantizer, index, bs=args.batch_size)
        op.restrict_range("nprobe", 2049)
        ...
        op.add_range("quantizer_efSearch", [2**i for i in range(10)])
        op.restrict_range("nprobe", 2049)
    elif args.search_type in ("multi_gpu_flat_quantizer", "multi_gpu_flat_quantizer_sharded"):
        index_ivf = faiss.extract_index_ivf(index)
        ...
        if args.search_type == "multi_gpu_sharded1":
            ...
            quantizer = faiss.index_factory(centroids.shape[1], f"IVF{args.ivf_quant_nlist},Flat")
            quantizer.train(centroids)
            quantizer.add(centroids)
            co.shard = False
        ...
        if args.search_type == "multi_gpu_Csharded1":
            ...
            quantizer = faiss.index_factory(index_ivf.d, f"IVF{args.ivf_quant_nlist},Flat")
    ...
    op.all_experiments = []
    op.platform = {
        "loadavg": open("/proc/loadavg").readlines(),
    ...
    for cno in experiments:
        ...
        op.all_experiments.append(
            {"cno": cno, "key": key, "parameters": parameters, "time": t1 - t0, "recalls": recalls}
        )
