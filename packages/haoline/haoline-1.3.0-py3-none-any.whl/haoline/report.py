...
class InspectionReport(BaseModel):
    ...
    autodoc_version: str = Field(default_factory=lambda: __import__("haoline").__version__)
    ...
    # Format tier for capability-aware reporting (Epic 49.2)
    # Values: "Full", "Graph", "Metadata", "Weights"
    ...
    def to_markdown(self) -> str:
        ...
        if self.format_tier:
            tier_emoji = {
                ...
                "Metadata": "🟡",
                "Weights": "⚪",
            }.get(self.format_tier, "⚪")
            lines.append(f"**Format Tier:** {tier_emoji} {self.format_tier}")
            lines.append("")
        ...
        # LLM Details (GGUF models)
        if self.gguf_info:
            gguf = self.gguf_info
            lines.append("## LLM Model Details")
            lines.append("")
            lines.append("| Property | Value |")
            lines.append("|----------|-------|")
            lines.append(f"| Architecture | {gguf.architecture} |")
            lines.append(f"| Model Name | {gguf.model_name} |")
            if gguf.block_count:
                lines.append(f"| Layers | {gguf.block_count} |")
            if gguf.embedding_length:
                lines.append(f"| Hidden Size | {gguf.embedding_length} |")
            if gguf.head_count:
                lines.append(f"| Attention Heads | {gguf.head_count} |")
            if gguf.head_count_kv:
                lines.append(f"| KV Heads (GQA) | {gguf.head_count_kv} |")
            if gguf.context_length:
                lines.append(f"| Context Length | {gguf.context_length:,} |")
            if gguf.vocab_size:
                lines.append(f"| Vocab Size | {gguf.vocab_size:,} |")
            lines.append("")

            # VRAM estimates
            if hasattr(gguf, "estimate_vram"):
                vram = gguf.estimate_vram()
                lines.append("### VRAM Requirements")
                lines.append("")
                lines.append(f"- **Model Weights**: {self._format_bytes(vram['weights'])}")
                lines.append(f"- **KV Cache**: {self._format_bytes(vram['kv_cache'])}")
                lines.append(f"- **Total VRAM**: {self._format_bytes(vram['total'])}")
                lines.append("")

            # Quantization breakdown
            if gguf.quantization_breakdown:
                lines.append("### Quantization Breakdown")
                lines.append("")
                lines.append("| Type | Tensors | Size |")
                lines.append("|------|---------|------|")
                for qtype in sorted(gguf.quantization_breakdown.keys()):
                    count = gguf.quantization_breakdown[qtype]
                    size = gguf.size_breakdown.get(qtype, 0)
                    lines.append(f"| {qtype} | {count} | {self._format_bytes(size)} |")
                lines.append("")
    ...
    def to_html(
        ...
        <header>
            <h1>{model_name}</h1>
            <p class="subtitle">Model Analysis Report</p>
            <p class="timestamp">Generated by HaoLine v{self.autodoc_version} on {self.generated_at}</p>
        </header>
        """
        )
        ...
        # LLM Model Details (GGUF)
        if self.gguf_info:
            gguf = self.gguf_info
            html_parts.append('<section class="llm-details">')
            html_parts.append("<h2>LLM Model Details</h2>")

            # Architecture table
            html_parts.append("<table>")
            html_parts.append("<tr><th>Property</th><th>Value</th></tr>")
            html_parts.append(f"<tr><td>Architecture</td><td>{gguf.architecture}</td></tr>")
            html_parts.append(f"<tr><td>Model Name</td><td>{gguf.model_name}</td></tr>")
            if gguf.block_count:
                html_parts.append(f"<tr><td>Layers</td><td>{gguf.block_count}</td></tr>")
            if gguf.embedding_length:
                html_parts.append(f"<tr><td>Hidden Size</td><td>{gguf.embedding_length}</td></tr>")
            if gguf.head_count:
                html_parts.append(f"<tr><td>Attention Heads</td><td>{gguf.head_count}</td></tr>")
            if gguf.head_count_kv:
                html_parts.append(f"<tr><td>KV Heads (GQA)</td><td>{gguf.head_count_kv}</td></tr>")
            if gguf.context_length:
                html_parts.append(
                    f"<tr><td>Context Length</td><td>{gguf.context_length:,}</td></tr>"
                )
            if gguf.vocab_size:
                html_parts.append(f"<tr><td>Vocab Size</td><td>{gguf.vocab_size:,}</td></tr>")
            html_parts.append("</table>")

            # VRAM Requirements
            if hasattr(gguf, "estimate_vram"):
                vram = gguf.estimate_vram()
                html_parts.append("<h3>VRAM Requirements</h3>")
                html_parts.append(
                    f"""
                    <div class="metrics-cards" style="grid-template-columns: repeat(3, 1fr);">
                        <div class="card">
                            <div class="card-value">{self._format_bytes(vram["weights"])}</div>
                            <div class="card-label">Model Weights</div>
                        </div>
                        <div class="card">
                            <div class="card-value">{self._format_bytes(vram["kv_cache"])}</div>
                            <div class="card-label">KV Cache</div>
                        </div>
                        <div class="card" style="border-color: var(--accent-cyan);">
                            <div class="card-value">{self._format_bytes(vram["total"])}</div>
            ...
            # Quantization Breakdown
            if gguf.quantization_breakdown:
                html_parts.append("<h3>Quantization Breakdown</h3>")
                html_parts.append("<table>")
                html_parts.append("<tr><th>Type</th><th>Tensors</th><th>Size</th></tr>")
                for qtype in sorted(gguf.quantization_breakdown.keys()):
                    count = gguf.quantization_breakdown[qtype]
                    size = gguf.size_breakdown.get(qtype, 0)
                    html_parts.append(
                        f"<tr><td>{qtype}</td><td>{count}</td>"
                        f"<td>{self._format_bytes(size)}</td></tr>"
                    )
                html_parts.append("</table>")
...
class ModelInspector:
    ...
    def inspect(self, model_path: str | pathlib.Path) -> InspectionReport:
        ...
        # Determine format tier based on file extension
        file_ext = model_path.suffix.lower()
        format_tier = "Full"  # ONNX is full analysis
        if file_ext == ".gguf":
            format_tier = "Metadata"
