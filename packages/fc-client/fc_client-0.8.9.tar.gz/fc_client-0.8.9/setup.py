...
if PKG == "fc-server":
        ...
        **common_setup,
        ...
        entry_points={
            "console_scripts": [
                ...
                "fc-server-cache = fc_server.tools.cache_helper:main",
            ]
        },
        install_requires=[
            "aiohttp>=3.7.4.post0",
    ...
    setup(
        **common_setup,
        ...
        extras_require={
            "labgrid": ["labgrid==23.1a2"],
        },
    )
elif PKG == "fc-client-docker":
    setup(
        **common_setup,
        name="fc_client_docker",
...
elif PKG == "fc-ai-agent":
    setup(
        **common_setup,
        name="fc_ai_agent",
        packages=["fc_common", "fc_agent"] + find_packages(include=("fc_mcp*",)),
        package_data={
            "fc_common": ["VERSION"],
        },
        entry_points={
            "console_scripts": [
                "fc-agent = fc_agent.agent:main",
            ]
        },
        install_requires=[
            "urllib3",
...
elif PKG == "fc-mcp-external":
    setup(
        **common_setup,
        name="fc_mcp_external",
        packages=["fc_common", "fc_mcp_external"],
        package_dir={"fc_mcp_external": "fc_mcp/mcp_plugins/external"},
        package_data={
            "fc_common": ["VERSION"],
        },
        entry_points={
            "fc.mcp.plugins": [
                "mcp1 = fc_mcp_external.uuu_bcu_mcp:Plugin",
                "mcp2 = fc_mcp_external.uboot_mcp:Plugin",
            ]
        },
