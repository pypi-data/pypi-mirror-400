...
update_init_version() {
    local new_version=$1
    local init_file="$PROJECT_ROOT/src/taskng/__init__.py"
...
do_release() {
    ...
    echo "  1b. Update version in src/taskng/__init__.py: $current_version → $new_version"
    ...
    log_info "Step 1b: Updating version in __init__.py..."
    update_init_version "$new_version"
    log_success "Updated __init__.py"

    log_info "Step 2: Committing changes..."
    git add "$PYPROJECT_FILE" "$PROJECT_ROOT/src/taskng/__init__.py"
