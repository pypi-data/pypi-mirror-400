...
class MongoExecutionTracer(BaseExecutionTracer):
    ...
    async def get_step_count(self, execution_id: str, step_name_prefix: str) -> int:
        """Counts the number of steps that start with the given prefix."""
        records = await self.repository.async_read({"execution_id": execution_id})
        ...
        for step in record.get("trace", []):
            if step.get("step", "").startswith(step_name_prefix):
                count += 1
        return count
