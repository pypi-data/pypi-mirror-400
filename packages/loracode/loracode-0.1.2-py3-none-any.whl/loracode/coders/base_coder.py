...
from loracode.models import RETRY_TIMEOUT
from loracode.reasoning_tags import (
    REASONING_TAG,
...
from loracode.repomap import RepoMap
from loracode.run_cmd import run_cmd
from loracode.utils import format_content, format_messages, format_tokens, is_image_file
from loracode.waiting import WaitingSpinner
...
class Coder:
    ...
    repo = None
    last_loracode_commit_hash = None
    loracode_edited_files = None
    ...
    def create(
        ...
        **kwargs,
        ...
        import loracode.coders as coders
        ...
        if from_coder:
            ...
            update = dict(
                ...
                loracode_commit_hashes=from_coder.loracode_commit_hashes,
    ...
    def get_announcements(self):
        ...
        if self.repo:
            ...
            lines.append(f"  Git repo: {rel_repo_dir} ({num_files:,} files)")
            if num_files > 1000:
                lines.append(
                    "  ! Large repo: consider using --subtree-only and .loracodeignore"
                )
        ...
        if self.repo_map:
            ...
            if map_tokens > 0:
                ...
                if map_tokens > max_map_tokens:
                    lines.append(
                        f"  ! map-tokens > {max_map_tokens} not recommended"
                    )
            else:
                lines.append("  Repo-map: disabled (map_tokens == 0)")
        else:
        ...
        for fname in self.abs_read_only_fnames:
            ...
            lines.append(f"  + {rel_fname} (read-only)")

        if self.done_messages:
            lines.append("  ~ Restored previous conversation")

        if self.io.multiline_mode:
            lines.append("  * Multiline mode: Enter = newline, Alt-Enter = submit")
    ...
    def __init__(
        ...
        self.loracode_commit_hashes = set()
        ...
        if loracode_commit_hashes:
            self.loracode_commit_hashes = loracode_commit_hashes
        else:
            self.loracode_commit_hashes = set()
        ...
        for fname in fnames:
            ...
            if self.repo and self.repo.ignored_file(fname):
                self.io.tool_warning(f"Skipping {fname} that matches loracodeignore spec.")
                continue
    ...
    def init_before_message(self):
        self.loracode_edited_files = set()
    ...
    def warm_cache(self, chunks):
        ...
        delay = 5 * 60 - 5
        delay = float(os.environ.get("LORACODE_CACHE_KEEPALIVE_DELAY", delay))
    ...
    def send_message(self, inp):
        ...
        if self.show_pretty():
            model_display = self.main_model.name.split("/")[-1]
            self.waiting_spinner = WaitingSpinner(f"Thinking with {model_display}...")
        ...
        if edited:
            self.loracode_edited_files.update(edited)
    ...
    def auto_commit(self, edited, context=None):
        ...
        try:
            res = self.repo.commit(fnames=edited, context=context, loracode_edits=True, coder=self)
            if res:
    ...
    def show_auto_commit_outcome(self, res):
        commit_hash, commit_message = res
        self.last_loracode_commit_hash = commit_hash
        self.loracode_commit_hashes.add(commit_hash)
        self.last_loracode_commit_message = commit_message
        if self.show_diffs:
    ...
    def show_undo_hint(self):
        ...
        if self.commit_before_message[-1] != self.repo.get_head_commit_sha():
            self.io.tool_output("You can use /undo to undo and discard each loracode commit.")
