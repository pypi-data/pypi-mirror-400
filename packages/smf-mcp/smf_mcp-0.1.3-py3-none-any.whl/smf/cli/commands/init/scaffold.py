...
def _init_elasticsearch_plugin(project_path: Path, args) -> None:
    ...
    from smf.plugins.elasticsearch.settings import write_settings as write_es_settings
    from smf.plugins.elasticsearch.activate import _activate_elasticsearch
    ...
    write_es_settings(project_path, server_name)
    
    # Activate the plugin (creates custom tools file)
    class PluginArgs:
        force = getattr(args, "force", False)
        hosts = es_hosts
    
    _activate_elasticsearch(project_path, PluginArgs())


def _write_elasticsearch_server(project_path: Path, args) -> None:
    """Write Elasticsearch-specific server.py."""
    from smf.plugins.elasticsearch.templates import server_template
...
def _write_elasticsearch_readme(project_path: Path, args) -> None:
    ...
    from smf.plugins.elasticsearch.templates import readme_template
    from smf.cli.io import write_utf8
