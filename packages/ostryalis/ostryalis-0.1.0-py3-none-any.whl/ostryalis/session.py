...
from inceptum import config as _config
...
# Name validation for UDFs: allow dotted names? In SQLite, function names are identifiers;
# keep it conservative and consistent with aliases.
_UDF_NAME_RE = _ALIAS_RE


class SessionManager:
    """
    Singleton-style manager that owns all session/engine/attachment/UDF state.

    Construction is idempotent: SessionManager() always returns the same instance.
    """
    ...
    def __init__(self, config_getter: Callable[[str], object] = _config):
    ...
    def reset_singleton(cls) -> None:
        with cls._singleton_lock:
            if cls._singleton is not None:
                try:
                    cls._singleton.reset()
                finally:
                    cls._singleton = None

    def set_config_getter(self, config_getter: Callable[[str], object]) -> None:
        with self._lock:
            self._config_getter = config_getter
            # Changing config affects db location => must drop engine/pool.
    ...
    def _validate_udf_arity(self, arity: int) -> int:
        ...
        # “overload by arity” requirement in confusing ways; forbid it unless you
        # explicitly want varargs semantics.
        if arity < 0:
            raise ValueError("Function arity must be >= 0.")
        return arity

    def _base_dir(self) -> Path:
        base_dir = Path(str(self._config_getter("ostryalis.directory"))).expanduser()
    ...
    def attach_many(self, aliases: Iterable[str]) -> list[str]:
        """
        Convenience: register many aliases with default paths.
        """
        return [self.attach(a) for a in aliases]
    ...
    def register_function(
        ...
        key = (name, arity)
        with self._lock:
            # Like attachments, be idempotent: first one wins unless you explicitly change it.
            self._udfs.setdefault(key, func)
        return key

    def register_functions(
        self,
        funcs: Iterable[tuple[str, int, Callable[..., Any]]],
    ) -> list[tuple[str, int]]:
        return [self.register_function(name, arity, f) for (name, arity, f) in funcs]
    ...
    def _sqlite_install_udfs(
        ...
        *,
        ...
        # Install each function on this DB-API connection.
        ...
        *,
        ...
        try:
            ...
            for alias in aliases:
                ...
                if path is None:
                    # Defensive: don't KeyError; surface a clear error.
    ...
    def _ensure_required_attached(
        ...
        if not require_attached:
            return
        self.attach_many(require_attached)
