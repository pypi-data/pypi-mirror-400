...
def one_hot(labels, num_classes):
    ...
    one_hot_arr = np.zeros((len(labels), num_classes), dtype=np.float32)
...
class Data_classification:
    ...
    def __init__(self, data_name: str, config: dict, *args: list) -> None:
        ...
        if self.bool_image:
            self.x_data_minmax = {
                "min": np.zeros(self.x_data_org.shape[1], dtype=np.float32),
                "max": np.ones(self.x_data_org.shape[1], dtype=np.float32) * np.max(self.x_data_org)
            }
        else:
            self.x_data_minmax = {
    ...
    def _normalize(self, data):
        ...
        return ((data - self.x_data_minmax["min"]) /
                (self.x_data_minmax["max"] - self.x_data_minmax["min"])).astype(np.float32)
