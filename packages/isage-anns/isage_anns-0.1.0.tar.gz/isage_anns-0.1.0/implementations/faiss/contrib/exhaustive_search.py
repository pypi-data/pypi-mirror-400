...
def range_search_gpu(xq, r2, index_gpu, index_cpu, gpu_k=1024):
    ...
    LOG.debug(f"GPU search {nq} queries with {k=:} {is_binary_index=:} {keep_max=:}")
    t0 = time.time()
    ...
    CombinerRangeKNN = (
        faiss.CombinerRangeKNNint16 if is_binary_index else faiss.CombinerRangeKNNfloat
    )
...
def apply_maxres(res_batches, target_nres, keep_max=False):
    ...
    for i, (nres, dis, ids) in enumerate(res_batches):
        nres, dis, ids = threshold_radius_nres(nres, dis, ids, radius, keep_max=keep_max)
        totres += len(dis)
    ...
    for xqi in query_iterator:
        ...
        if max_results is not None and totres > max_results:
            ...
            radius, totres = apply_maxres(
                res_batches, min_results, keep_max=index.metric_type == faiss.METRIC_INNER_PRODUCT
            )
        t2 = time.time()
        t_search += t1 - t0
