...
static inline void vec_swap(typename StringSet::Iterator a,
                            typename StringSet::Iterator b, size_t n)
{
    while (n-- > 0)
        std::swap(*a++, *b++);
}
...
static inline typename StringSet::Iterator med3func(
    const StringSet& ss, typename StringSet::Iterator a,
    typename StringSet::Iterator b, typename StringSet::Iterator c,
    size_t depth)
{
...
static inline void multikey_quicksort(const StringPtr& strptr, size_t depth,
                                      size_t memory)
{
...

    {
        ...
        {
            while (pb <= pc &&
                   (r = static_cast<int>(ss.get_char(*pb, depth)) - pivot) <= 0)
            {
                if (r == 0)
                    std::swap(*pa++, *pb);
                pb++;
            ...
            while (pb <= pc &&
                   (r = static_cast<int>(ss.get_char(*pc, depth)) - pivot) >= 0)
            {
                if (r == 0)
                    std::swap(*pc, *pd--);
                pc--;
    ...
    if (r > 1)
    {
        multikey_quicksort(strptr.sub(a - ss.begin(), r), depth,
                           memory - memory_use);
    }
    if (ss.get_char(*(a + r), depth) != 0)
    {
        multikey_quicksort(
            strptr.sub(a - ss.begin() + r, (pa - a) + (pn - pd - 1)), depth + 1,
            memory - memory_use);
    }
    r = pd - pc;
    if (r > 0)
    ...
    if ((r = pd - pc) > 1)
    {
        multikey_quicksort(strptr.sub(a - ss.begin() + n - r, r), depth,
                           memory - memory_use);
    }
}
