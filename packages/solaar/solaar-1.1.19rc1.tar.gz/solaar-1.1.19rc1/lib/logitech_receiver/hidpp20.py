...
class SpecialKey:
    """Information about a control present on a device with the SPECIAL_KEYS_AND_BUTTONS feature.
    This is a simpler interface than the earlier ones and thus doesn't inherit from it"""

    @classmethod
    def create(cls, device, cid):
        return cls(device, cid, None, None)

    def __init__(self, device: Device, cid: int, mapped_to, diversion):
        self.device = device
        self.cid = cid
        self.mapped_to = mapped_to
        self.diversion = diversion

    def query_mapped_to(self) -> int:  # read cid data from device
        mapped_to_data = self.device.feature_request(
            SupportedFeature.SPECIAL_KEYS_AND_BUTTONS, 0x30, struct.pack("!H", self.cid)
        )
        if mapped_to_data is not None:
            self.mapped_to = struct.unpack("!H", mapped_to_data[2:4])[0]
        print("READ MAPPED TO", self.cid, self.mapped_to, self.diversion)
        return self.mapped_to

    def get_mapped_to(self) -> int:
        if self.mapped_to is None:
            self.read_mapped_to()
        return self.mapped_to

    def set_mapped_to(self, mapped_to: NamedInt) -> None:
        if mapped_to in special_keys.CONTROL:
            self.mapped_to = mapped_to
            ret = self.device.feature_request(
                SupportedFeature.SPECIAL_KEYS_AND_BUTTONS, 0x40, struct.pack("!HH", self.cid, mapped_to)
            )
            if ret is None and logger.isEnabledFor(logging.DEBUG):
                logger.debug(f"SPECIAL_KEYS_AND_BUTTONS set mapping on device {self.device} didn't respond.")

    def acceptable_mapped_to(self, mapping: NamedInt) -> bool:
        return mapping in special_keys.CONTROL

    def query_diversion(self):  # read cid data from device
        flags_data = self.device.feature_request(SupportedFeature.SPECIAL_KEYS_AND_BUTTONS, 0x10, struct.pack("!H", self.cid))
        if flags_data is not None:
            self.diversion = 1 if (flags_data[2] & 0x03) == 0x02 else 0
        return self.diversion

    def get_diversion(self):
        if self.diversion is None:
            self.query_diversion()
        return self.diversion

    def set_diversion(self, diversion):
        self.diversion = diversion
        flags = 0x01 if int(diversion) == 0 else 0x02
        ret = self.device.feature_request(SupportedFeature.SPECIAL_KEYS_AND_BUTTONS, 0x20, struct.pack("!HB", self.cid, flags))
        if ret is None and logger.isEnabledFor(logging.DEBUG):
            logger.debug(f"SPECIAL_KEYS_AND_BUTTONS set diversion on device {self.device} didn't respond.")

    def acceptable_diversion(self, diversion):
        return True


class SpecialKeysArray(UserDict):
    """A mapping from control ids to their status (mapped to and diversion)"""

    def __init__(self, device: Device):
        self._device = device
        self.data = {}
        for cid in device.cids:
            self[cid] = SpecialKey(device, cid, None, None)  # don't know status on device

    def get_mapping(self, cid):
        if self[cid]:
            return self[cid].get_mapping()
        else:
            key = SpecialKey(self._device, None, None)
            self[cid] = key
            return key.get_mapping()

    def query_key(self, cid):  # get information from device # may be inefficient due to two settings
        key = self[cid]
        key.read_mapped_to()  # can fail
        key.read_diversion()
...
class Hidpp20:
    ...
    def get_special_keys(self, device: Device):
        if SupportedFeature.SPECIAL_KEYS_AND_BUTTONS in device.features:
            return SpecialKeysArray(device)

    def get_cids(self, device: Device):  # get list of integer cids for device
        if SupportedFeature.CONTROL_LIST in device.features:
            cids = []
            offset = 0
            count = int(device.feature_request(SupportedFeature.CONTROL_LIST)[0])
            while count > offset:
                r = device.feature_request(SupportedFeature.CONTROL_LIST, 0x10, offset)
                cids = cids + [
                    int.from_bytes(r[i : i + 2], "big") for i in range(0, 16, 2) if int.from_bytes(r[i : i + 2], "big")
                ]
                offset += 8
            return cids
    ...
    def get_profiles_new(self, device: Device):
        if getattr(device, "_profiles_new", None) is not None:
            return device._profiles_new
        if SupportedFeature.MODULAR_PROFILE_MANAGEMENT in device.features:
            return profile.DeviceProfiles(device)
...
class ForceSensingButton:
    ...
    def create(cls, device, number: int):
        ...
        if buttondata is not None:
            ...
            return cls(device, number, changeable, default, max_value, min_value, None)

    def __init__(self, device, number: int, changeable: bool, default: int, max_value: int, min_value: int, current):
        self._device = device
        self.number = number
    ...
    def query_current(self) -> int:
        buttoncurrent = self._device.feature_request(SupportedFeature.FORCE_SENSING_BUTTON, 0x20, self.number)
        if buttoncurrent is not None:
            self._current = struct.unpack("!H", buttoncurrent[:2])[0]
...
class ForceSensingButtonArray(UserDict):
    ...
    def __init__(self, device: Device):
        ...
        for index in range(0, self._count):
            self[index] = ForceSensingButton.create(device, index)
    ...
    def query_current(self):
        return self[0].query_current()

    def get_current(self):
        return self[0].get_current()

    def set_current(self, current: int) -> None:
        return self[0].set_current(current)

    def acceptable_current(self, value) -> bool:
