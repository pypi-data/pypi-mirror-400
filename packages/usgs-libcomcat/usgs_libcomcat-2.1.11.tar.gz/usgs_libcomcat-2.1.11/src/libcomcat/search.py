...
def count(
    ...
    for stime, etime in segments:
        newargs["starttime"] = stime.replace(tzinfo=timezone.utc)
        newargs["endtime"] = etime.replace(tzinfo=timezone.utc)
...
def _get_time_segments(starttime, endtime, minmag):
    ...
    if endtime < datetime(1951, 1, 1).replace(tzinfo=timezone.utc):
...
def get_authoritative_info(eventid):
    ...
    try:
        ...
        for origin in origins:
            ...
            if (
                "magnitude-source" not in origin.properties
                or origin["magnitude-source"].lower() != source.lower()
            ):
                magval = np.nan
                magtype = "NA"
            else:
