...
CODE EDITING VALIDATION WORKFLOW:
Use sessions when validating code changes BEFORE editing:

MANDATORY for changes to:
- State management (predicate loading/unloading, cleanup sequences)
- Control flow (query ordering, resource allocation/deallocation)
- Isolation requirements (test isolation, session isolation)
- Lifecycle management (initialization, teardown, cleanup patterns)
- Concurrency (thread-safe operations, locking patterns)

Example: Validating MQI solver predicate cleanup
1. session_id = create_session()
2. assert_facts(session_id, [
     "query1_loads([mqi_temp_test(alice), mqi_temp_test(bob)]).",
     "query2_loads([]).",
   ])
3. assert_rules(session_id, [
     "predicates_isolated :- "
     "  query1_loads(Q1), "
     "  retractall_called(Q1), "
     "  query2_loads(Q2), "
     "  \\+ member(mqi_temp_test(_), Q2)."
   ])
4. query(session_id, "predicates_isolated")
   → If returns solution, approach is valid. Then edit code.
5. destroy_session(session_id)

See LOGIC_VALIDATION_GUIDE.md for more patterns and examples.
            """.strip(),
...
When validating code changes, assert facts about:
- Current system state and behavior
- Problem preconditions
- Expected outcomes after changes
- State transitions and constraints

Example: Model predicate cleanup for code fix validation
session_id = create_session()
assert_facts(session_id, [
  "loaded_before_cleanup(mqi_temp_test, 1).",
  "cleanup_operation(retractall).",
  "expected_after_cleanup(empty).",
])
# Then assert rules and query to validate the approach
            """.strip(),
...
CODE VALIDATION PATTERN:
When validating code changes, assert rules that describe:
- Required behaviors and constraints
- State transitions and sequences
- Isolation properties
- Invariants that must be maintained

Example: Define cleanup validation rule
assert_rules(session_id, [
  "cleanup_works :- "
  "  loaded_before_cleanup(Pred, Arity), "
  "  cleanup_operation(retractall), "
  "  retractall_removes(Pred, Arity), "
  "  expected_after_cleanup(empty)."
])
# If this rule can derive successfully, your approach is sound
            """.strip(),
...
CODE VALIDATION PATTERN:
When validating code changes, query for:
- Whether the desired behavior is logically possible
- Edge cases that should be handled
- Invariants that should hold before/after changes
- Constraint satisfaction

Example: Validate predicate cleanup approach
After asserting facts about the problem and rules about your solution, query:
  query(session_id, "cleanup_works")

If this returns solutions (count > 0), your approach is logically sound.
Only then proceed with editing the actual code.

VALIDATION WORKFLOW:
1. Create session
2. Assert facts about current/desired behavior
3. Assert rules about your solution approach
4. Query to test the approach
5. If query succeeds → Edit code with confidence
6. If query fails → Refine approach and re-query
