...
try:
    ...
    supabase = None  # Lazy initialization
...
async def get_db_schemas():
    ...
    result = get_supabase_client().execute_query(query)
    return result
...
async def get_tables(schema_name: str):
    ...
    return get_supabase_client().execute_query(query)
...
async def live_dangerously(service: Literal["api", "database"], enable: bool = False) -> dict:
    ...
    if service == "api":
        ...
        client.switch_mode(DbSafetyLevel.RW if enable else DbSafetyLevel.RO)
        return {"service": "database", "mode": client.mode}
