...
class DeploymentMetadata(BaseModel):
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "trigger": (
                    TriggerDeploymentMetadata.from_dict(obj["trigger"])
                    if obj.get("trigger") is not None
                    else None
                ),
                "database": (
                    DatabaseDeploymentMetadata.from_dict(obj["database"])
                    if obj.get("database") is not None
                    else None
                ),
                "git": (
                    GitDeploymentMetadata.from_dict(obj["git"])
                    if obj.get("git") is not None
                    else None
                ),
                "archive": (
                    ArchiveDeploymentMetadata.from_dict(obj["archive"])
