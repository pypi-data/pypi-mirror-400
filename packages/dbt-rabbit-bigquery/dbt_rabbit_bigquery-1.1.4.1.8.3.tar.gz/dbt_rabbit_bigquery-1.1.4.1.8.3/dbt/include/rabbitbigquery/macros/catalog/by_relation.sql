{% macro rabbitbigquery__get_catalog_relations(information_schema, relations) -%}
    ...
    {%- else -%}
        {%- set query -%}
            with
                table_shards_stage as ({{ _rabbitbigquery__get_table_shards_sql(information_schema) }}),
                table_shards as (
