...
def check_dependencies():
    ...
    Dependencies not included in main le_map_tools are: cartopy, PIL, and scipys
...
def add_layer(
    ...
    if type(ee_object) is not le.image.Image:
        raise ValueError("provided `ee_object` is not of type ee.Image")

    if region is not None:
        map_region = le.Geometry.Rectangle(region).getInfo()["coordinates"]
...
def get_image_collection_gif(
    ...
    **kwargs,
    ...
    from .le_map_tools import png_to_gif, jpg_to_gif
    ...
    dates = dates.map(lambda d: le.Date(d).format(date_format)).getInfo()
    ...
    for i, date in enumerate(dates):
        ...
        for ee_object, style in zip(overlay_layers, overlay_styles):
            if (
                ...
                isinstance(ee_object, le.image.Image)
                or isinstance(ee_object, le.imagecollection.ImageCollection)
                or isinstance(ee_object, le.imagecollection.ImageCollection)
            ):
