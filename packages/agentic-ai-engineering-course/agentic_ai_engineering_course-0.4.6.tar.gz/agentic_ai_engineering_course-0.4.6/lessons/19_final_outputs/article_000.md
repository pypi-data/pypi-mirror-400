...
This tool is a great example of an LLM-as-a-filter pattern for automated curation. Initially, we sent all research results directly to the final compilation step, but this often included low-quality or spammy content. This forced us to add this automated filtering step to ensure only high-quality sources make it through.
...
This tool is a practical example of resource allocation optimization. Instead of scraping every URL, which can be costly and time-consuming, we focus our budget on a few high-leverage sources that promise unique, in-depth information.

## Scrape Selected Research URLs

The `scrape_research_urls_tool` handles the full content extraction from our selected sources. It works very similarly to the guideline URL scraping tool we saw in lesson 16, using Firecrawl for robust web scraping and an LLM for content cleaning. For this reason, we will not go into the details of the scraping process here, and we only provide the code for testing the tool.
...
This tool's goal is to get clean, LLM-ready markdown from the selected URLs. As we covered in Lesson 16, we use Firecrawl because it is purpose-built to convert modern, dynamic web pages into markdown [[4]](https://docs.firecrawl.dev/features/scrape?utm_source=chatgpt.com), [[8]](https://docs.firecrawl.dev/advanced-scraping-guide?utm_source=chatgpt.com). We then run a short LLM pass to clean up any remaining boilerplate. This provides the necessary depth that snippet-based research from Perplexity cannot offer, enriching the final research artifact with comprehensive context.
...
Because our entire workflow is defined in a server-hosted MCP prompt, we can inject these HITL checkpoints without changing any code [[1]](https://modelcontextprotocol.io/docs/concepts/prompts?utm_source=chatgpt.com). The prompt accepts policy tweaks as plain text. This means you can run the agent in a fully autonomous mode or switch to a supervised mode by simply changing the invocation message. For instance, you could add instructions like "Only keep sources from .edu or .gov domains" or "Select exactly 3 URLs to scrape." This is a powerful pattern for building controllable and adaptable AI systems.
...
When testing, run the agent on a few different article guidelines and audit the results. Check that the kept sources are reputable, the deep-scraped content adds value, and the final `research.md` is well-structured, citable, and comprehensive. This systematic testing is key to hardening your agent for production use.

## Use the MCP Server in Cursor

Our research agent can also be used directly within Cursor IDE through the MCP protocol. The `mcp_server` folder contains a `.mcp.json.sample` file that shows how to configure Cursor to use our research agent [[5]](https://cursor.com/docs/context/mcp?utm_source=chatgpt.com).
...
This integration demonstrates the portability of MCP-based agents. The same tools and prompts work across any compatible client without code changes, making your agent reusable and easy to share. You can expand this agent by adding new tools, such as a domain whitelist for source filtering, a fact-checking tool, or a cost-reporting tool to track API usage per run.

## Conclusion

In this lesson, you built a production-minded research agent. It filters Perplexity results for quality, selects high-value URLs for deep scraping, and compiles all findings into a single, citable `research.md` file. The file-first design makes the process auditable, while optional HITL gates provide necessary control.

This matters for several reasons. Automated quality control removes SEO spam and low-trust sources. Resource optimization focuses your budget on high-leverage content. And because the workflow logic lives in a server-hosted prompt, any MCP client can run the same recipe, ensuring interoperability. You now have a complete research pipeline, from ingestion to a final, structured artifact.
...
1. Model Context Protocol. (n.d.). Prompts. Model Context Protocol. https://modelcontextprotocol.io/docs/concepts/prompts?utm_source=chatgpt.com
2. Model Context Protocol. (n.d.). Tools. Model Context Protocol. https://modelcontextprotocol.io/docs/concepts/tools?utm_source=chatgpt.com
3. Perplexity. (n.d.). Structured Outputs. Perplexity. https://docs.perplexity.ai/guides/structured-outputs?utm_source=chatgpt.com
4. Firecrawl. (n.d.). Scrape to Markdown. Firecrawl Docs. https://docs.firecrawl.dev/features/scrape?utm_source=chatgpt.com
5. Cursor. (n.d.). MCP integration. Cursor. https://cursor.com/docs/context/mcp?utm_source=chatgpt.com
6. OpenAI. (n.d.). Model context protocol (MCP). OpenAI GitHub Pages. https://openai.github.io/openai-agents-python/mcp/?utm_source=chatgpt.com
7. Google AI. (n.d.). Video understanding. Google AI for Developers. https://ai.google.dev/gemini-api/docs/video-understanding?utm_source=chatgpt.com
8. Firecrawl. (n.d.). Advanced Scraping Guide. Firecrawl Docs. https://docs.firecrawl.dev/advanced-scraping-guide?utm_source=chatgpt.com
9. gitingest. (n.d.). LLM-ready repo digests. gitingest.com. https://gitingest.com/?utm_source=chatgpt.com
10. FastMCP. (n.d.). MCP JSON configuration. FastMCP. https://gofastmcp.com/integrations/mcp-json-configuration?utm_source=chatgpt.com