...
def cmd_model(ctx: CommandContext) -> None:
    ...
    if ctx.args:
        ...
        print()
        print("Usage:")
        print("  /model <provider/model>  - Switch model (Tab to autocomplete)")
        print("  /model list              - Show all available models")
        print()
        print("Examples:")
        ...
        print()
        print("API Keys:")
        print("  /key                 - Interactive API key setup")
        print("  Or set manually in ~/.config/gptme/gptme.toml")


@command("key", aliases=["apikey"])
def cmd_key(ctx: CommandContext) -> None:
    """Set up API keys interactively."""
    from ..setup import ask_for_api_key  # fmt: skip
