...
def test_templates_search_order(tmp_path, monkeypatch):
    ...
    dest = tmp_path / 'out'
    res = list(engine.apply_template('tpl', dest, {'project_name':'X'}, force=True))
    assert any('rendered' in r for r in res)
    assert 'user X' in (dest / 'a.txt').read_text()

    # override with templates_dir pointing to package -> should use package
    dest2 = tmp_path / 'out2'
    res2 = list(engine.apply_template('tpl', dest2, {'project_name':'Y'}, force=True, templates_dir=pkg))
