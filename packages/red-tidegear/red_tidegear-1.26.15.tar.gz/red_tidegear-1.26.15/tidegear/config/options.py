...
class GlobalConfigOption(BaseModel, Generic[JsonableType]):
    ...
    Warning:
        ...
        Use [`.type()`][tidegear.config.options.GlobalConfigOption.type] instead.
    """
    internal: bool = False
    ...
    @classmethod
    def _unwrap(cls, obj: typing.Any) -> typing.Any:
        if isinstance(obj, pydantic.BaseModel):
            obj = obj.model_dump(mode="python")
        if isinstance(obj, pydantic.Secret):
            return obj.get_secret_value()
        if isinstance(obj, dict):
            return {k: cls._unwrap(v) for k, v in obj.items()}
        if isinstance(obj, (list, tuple)):
            t = type(obj)
            return t(cls._unwrap(v) for v in obj)
        return obj
    ...
    def _serialize(self, obj: Any, /) -> pydantic.JsonValue:
        ...
        serialized = self.__pydantic_serializer__.to_python(self._unwrap(validated), mode="json", warnings=False)
