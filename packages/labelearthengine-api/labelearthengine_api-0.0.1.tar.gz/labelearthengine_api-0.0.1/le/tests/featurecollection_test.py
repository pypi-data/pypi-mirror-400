...
class FeatureCollectionTest(apitestcase.ApiTestCase):
  ...
  def test_constructors(self):
    ...
    from_id = le.FeatureCollection('abcd')
    self.assertEqual(
        le.ApiFunction.lookup('Collection.loadTable'), from_id.func)
    self.assertEqual({'tableId': 'abcd'}, from_id.args)

    from_id_and_geom_column = le.FeatureCollection('abcd', 'xyz')
    self.assertEqual(
        le.ApiFunction.lookup('Collection.loadTable'),
    ...
    feature = le.Feature(geometry)
    geo_json = {'type': 'FeatureCollection', 'features': [geometry.toGeoJSON()]}
    from_geometries = le.FeatureCollection([geometry])
    from_single_geometry = le.FeatureCollection(geometry)
    from_features = le.FeatureCollection([feature])
    from_single_feature = le.FeatureCollection(feature)
    from_geo_json = le.FeatureCollection(geo_json)
    ...
    self.assertEqual(le.ApiFunction.lookup('Collection'), from_geometries.func)
    self.assertEqual({'features': [feature]}, from_geometries.args)

    # Test a computed list object.
    l = le.List([feature]).slice(0)
    from_list = le.FeatureCollection(l)
    self.assertEqual({'features': l}, from_list.args)

    from_computed_object = le.FeatureCollection(
  ...
  def test_get_map_id(self):
    ...
    collection = le.FeatureCollection('test5')
    mapid = collection.getMapId({'color': 'ABCDEF'})
    manual = le.ApiFunction.call_('Collection.draw', collection, 'ABCDEF')
  ...
  def test_download(self):
    ...
    le.FeatureCollection('test7').getDownloadURL()

    self.assertEqual('/table', self.last_table_call['url'])
    self.assertEqual(le.FeatureCollection('test7').serialize(),
                     self.last_table_call['data']['table'].serialize())

    le.FeatureCollection('test8').getDownloadURL(
        'json', 'bar, baz', 'qux')
    self.assertEqual(
        le.FeatureCollection('test8').serialize(),
        self.last_table_call['data']['table'].serialize())
    ...
    self.assertEqual(
        le.FeatureCollection('test7').getDownloadUrl('csv'),
        le.FeatureCollection('test7').getDownloadURL('csv'))
  ...
  def test_download_table_with_cloud_api(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      ...
      fc = le.FeatureCollection([le.Feature(None, {'foo': 'bar'})])
      result = le.data.getTableDownloadId({
          'table': fc, 'selectors': 'foo', 'format': 'CSV',
      })
      url = le.data.makeTableDownloadUrl(result)
  ...
  def test_select(self):
    ...
    fc = le.FeatureCollection(le.Feature(le.Geometry.Point(0, 0), {'a': 5}))
  ...
  def test_aggregate_array(self):
    ...
    expression = le.FeatureCollection(le.Feature(None)).aggregate_array(
        property_name
    )
  ...
  def test_aggregate_count(self):
    ...
    expression = le.FeatureCollection(le.Feature(None)).aggregate_count(
        property_name
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_count_distinct(self):
    ...
    expression = le.FeatureCollection(
        le.Feature(None)
  ...
  def test_aggregate_first(self):
    ...
    expression = le.FeatureCollection(le.Feature(None)).aggregate_first(
        property_name
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_histogram(self):
    ...
    expression = le.FeatureCollection(le.Feature(None)).aggregate_histogram(
        property_name
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_max(self):
    ...
    expression = le.FeatureCollection(le.Feature(None)).aggregate_max(
        property_name
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_mean(self):
    ...
    expression = le.FeatureCollection(le.Feature(None)).aggregate_mean(
        property_name
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_min(self):
    ...
    expression = le.FeatureCollection(le.Feature(None)).aggregate_min(
        property_name
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_product(self):
    ...
    expression = le.FeatureCollection(le.Feature(None)).aggregate_product(
        property_name
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_sample_sd(self):
    ...
    expression = le.FeatureCollection(le.Feature(None)).aggregate_sample_sd(
        property_name
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_sample_var(self):
    ...
    expression = le.FeatureCollection(le.Feature(None)).aggregate_sample_var(
        property_name
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_stats(self):
    ...
    expression = le.FeatureCollection(le.Feature(None)).aggregate_stats(
        property_name
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_sum(self):
    ...
    expression = le.FeatureCollection(le.Feature(None)).aggregate_sum(
        property_name
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_total_sd(self):
    ...
    expression = le.FeatureCollection(le.Feature(None)).aggregate_total_sd(
        property_name
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_total_var(self):
    ...
    expression = le.FeatureCollection(le.Feature(None)).aggregate_total_var(
        property_name
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_bounds(self):
    ...
    collection = le.FeatureCollection('a')
    max_error = 1.1
    proj = 'EPSG:4326'
    expect = make_expression_graph({
        'arguments': {
  ...
  def test_classify(self):
    ...
    classifier = le.Classifier.smileNaiveBayes()
    expression = le.FeatureCollection(le.Feature(None)).classify(
  ...
  def test_cluster(self):
    ...
    expression = le.FeatureCollection(le.Feature(None)).cluster(
  ...
  def test_copy_properties(self):
    ...
    expression = le.FeatureCollection('a').copyProperties(
  ...
  def test_distance(self):
    ...
    features = le.FeatureCollection('a')
    search_radius = 1.1
    max_error = 2.2
    expect = make_expression_graph({
        'arguments': {
  ...
  def test_distinct(self):
    collection = le.FeatureCollection(le.Feature(None))
  ...
  def test_inverse_distance(self):
    ...
    expression = le.FeatureCollection(le.Feature(None)).inverseDistance(
  ...
  def test_kriging(self):
    ...
    expression = le.FeatureCollection(le.Feature(None)).kriging(
  ...
  def test_load_big_query_table(self):
    ...
    expression = le.FeatureCollection.loadBigQueryTable(table, geometry_column)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.FeatureCollection.loadBigQueryTable(
  ...
  def test_make_array(self):
    ...
    expression = le.FeatureCollection(le.Feature(None)).makeArray(
        properties, name
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_merge(self):
    collection1 = le.FeatureCollection('a')
    collection2 = le.FeatureCollection('b')
    expect = make_expression_graph({
  ...
  def test_random_points(self):
    ...
    seed = 2
    max_error_num = 3
    max_error = le.ErrorMargin(max_error_num)
    expect = make_expression_graph({
        'arguments': {
            'region': {
                'functionInvocationValue': {
    ...
    expression = le.FeatureCollection.randomPoints(
    ...
    expression = le.FeatureCollection(le.Feature(None)).randomPoints(
  ...
  def test_run_big_query(self):
    ...
    expression = le.FeatureCollection.runBigQuery(
