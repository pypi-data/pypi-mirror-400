...
from .._type_converter import is_placeholder, is_set, is_nan
...
class Polygon(BaseSectionObject):
    ...
    def to_inp_line(self):
        return '\n'.join([f'{self.subcatchment}  {x:0.{CONFIG.gis_decimals}f} {y:0.{CONFIG.gis_decimals}f}' for x, y in self.polygon])
...
class Groundwater(BaseSectionObject):
    ...
    def __init__(self, subcatchment, aquifer, node, Esurf, A1, B1, A2, B2, A3, Dsw, Egwt=np.nan, Ebot=np.nan, Egw=np.nan, Umc=np.nan):
        ...
        self.node = str(node)
        self.Esurf = float(Esurf)  # surfElev
        self.A1 = float(A1)
        self.B1 = float(B1)
        self.A2 = float(A2)
        self.B2 = float(B2)
        self.A3 = float(A3)
        self.Dsw = float(Dsw)  # fixedDepth
        self.Egwt = np.nan if is_placeholder(Egwt) else float(Egwt)  # nodeElev
        self.Ebot = np.nan if is_placeholder(Ebot) else float(Ebot)  # bottomElev
        self.Egw = np.nan if is_placeholder(Egw) else float(Egw)  # waterTableElev
        self.Umc = np.nan if is_placeholder(Umc) else float(Umc)  # upperMoisture

    def _set_unused_parameters_stars(self):
        if is_nan(self.Egwt):  # always a * if not set - coding error in C-source.
            self.Egwt = '*'
