...
def test_normalize_adt_counts_se():
    se = create_test_se()
    out = scranpy.normalize_adt_counts_se(se)
    ...
    sf = scranpy.compute_clrm1_factors(se.get_assay(0))
    sf = sf/sf.mean()
    out2 = scranpy.normalize_adt_counts_se(se, size_factors=sf, center=False)
    assert numpy.allclose(out.get_column_data()["size_factor"], out2.get_column_data()["size_factor"])

    # Works with blocking.
    block = ["A", "B"] * 5
    out_block = scranpy.normalize_adt_counts_se(se, block=block) 
    ...
    out4 = scranpy.normalize_adt_counts_se(se, factor_name=None)
