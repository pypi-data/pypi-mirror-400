...
from ..configs.config_temporal_split_preprocessing_step import (
    TemporalSplitPreprocessingConfig,
)
...
class TemporalSplitPreprocessingStepBuilder(StepBuilderBase):
    """
    Builder for a Temporal Split Preprocessing ProcessingStep.
    ...
    def __init__(
        self,
        config: TemporalSplitPreprocessingConfig,
        ...
        logger.info("Using unified TEMPORAL_SPLIT_PREPROCESSING_SPEC for all job types")

        super().__init__(
            config=config,
            spec=TEMPORAL_SPLIT_PREPROCESSING_SPEC,
            sagemaker_session=sagemaker_session,
            role=role,
            registry_manager=registry_manager,
            dependency_resolver=dependency_resolver,
        )
        self.config: TemporalSplitPreprocessingConfig = config
    ...
    def validate_configuration(self) -> None:
        ...
        # Validate temporal split specific fields
        if not self.config.date_column.strip():
            raise ValueError("date_column must be a non-empty string")

        if not self.config.group_id_column.strip():
            raise ValueError("group_id_column must be a non-empty string")

        if not self.config.split_date.strip():
            raise ValueError("split_date must be a non-empty string")
    ...
    def _get_environment_variables(self) -> Dict[str, str]:
        ...
        # Update with temporal split preprocessing-specific environment variables from config
        if hasattr(self.config, "temporal_split_environment_variables"):
            env_vars.update(self.config.temporal_split_environment_variables)
    ...
    def _get_job_arguments(self) -> List[str]:
        ...
        # For temporal split preprocessing, we always return the job_type from config
