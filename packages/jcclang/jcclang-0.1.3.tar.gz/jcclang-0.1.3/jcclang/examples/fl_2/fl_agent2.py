...
async def heartbeat_task(cli, role: str, interval: float = 5.0):
    ...
    while True:
        try:
            ...
            # logger.info(f"[Heartbeat] Sent for {role}")
        except Exception as e:
...
async def start_leader(cli, epoch: int):
    ...
    async with trio.open_nursery() as nursery:
        # 启动独立的心跳任务
        nursery.start_soon(heartbeat_task, cli, "leader", 5.0)
...
async def run_as_client(cli, leader_info: Dict[str, Any]):
    ...
    for attempt in range(5):
        try:
            ...
            # await stream.aclose()
            await stream.write(json.dumps({"client_id": SELF_JOB_ID}).encode())
            await stream.write(updated_model_bytes)
            await stream.close()
