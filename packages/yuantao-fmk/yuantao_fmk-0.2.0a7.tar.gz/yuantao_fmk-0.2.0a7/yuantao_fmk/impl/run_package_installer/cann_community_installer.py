...
class CANNCommunityInstaller(RunPackageInstallerBase):
    ...
    def __init__(
        ...
        super().__init__("cann_community", version, os_, arch, download_path)
    ...
    def get_version_list(self, reverse=True, stable=Config.FORCE_STABLE):
    ...
    def get_default_resource_tag_list(self):
        return ["toolkit"]

    def get_available_resource_tag_list(self):
        # the version may like 8.11.0, which results in abnormal comparision, but impossible
        if self.version == "8.5.0" or float(self.version[:3]) > 8.5:
            return ["toolkit", "ops", "nnal"]
        else:
            return ["toolkit", "kernels", "nnal", "nnae", "nnrt", "tfplugin"]
    ...
    def get_activation_command_list(self, target_path):
