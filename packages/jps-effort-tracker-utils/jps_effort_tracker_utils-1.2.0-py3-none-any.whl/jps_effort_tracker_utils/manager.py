...
class Manager:
    ...
    def _get_report_fields(self) -> list:
        """Get the list of fields to include in reports from config.

        Returns:
            list: List of field names to include in report
        """
        default_fields = [
            "title",
            "issue_tracker_id",
        ...
        fields = report_config.get('fields', default_fields)

        return fields if fields else default_fields

    def generate_report(self, task_file: Optional[str], output_file: Optional[str] = None) -> None:
        """Generate a text report of tasks.

        Args:
            task_file: The task file to generate report from. If None, uses current date task file.
            output_file: Path to save the report. If None, generates filename from task file.
        """
        if task_file is None or task_file == "":
            task_file = self.current_date_task_file
            logging.info(f"task_file was not specified and therefore was set to '{task_file}'")
        ...
        if not self.current_date_task_lookup:
            rprint("[yellow]No tasks found to generate report.[/yellow]")
            return

        # Determine output file path
        if output_file is None:
            # Generate output filename based on task file
            task_filename = os.path.basename(task_file)
            task_date = os.path.splitext(task_filename)[0]
            output_file = os.path.join(self.tasks_dir, f"{task_date}-report.txt")

        # Get fields to include in report
        report_fields = self._get_report_fields()

        # Generate report content
        report_lines = []
        report_lines.append("="*80)
        report_lines.append(f"TASK REPORT - {os.path.basename(task_file)}")
        report_lines.append(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        report_lines.append(f"Total Tasks: {len(self.current_date_task_lookup)}")
        report_lines.append("="*80)
        report_lines.append("")

        # Add each task vertically
        for idx, (task_id, task) in enumerate(self.current_date_task_lookup.items(), start=1):
            report_lines.append(f"Task #{idx}")
            report_lines.append("-" * 80)
            ...
            for field in report_fields:
                ...
                if field == "description" and display_value and display_value != "(not set)":
                    report_lines.append(f"{field_name}:")
                    # Indent description text
                    for line in display_value.split('\n'):
                        report_lines.append(f"  {line}")
                else:
                    report_lines.append(f"{field_name}: {display_value}")

            report_lines.append("")

        report_lines.append("="*80)
        report_lines.append(f"End of Report - {len(self.current_date_task_lookup)} tasks")
        report_lines.append("="*80)
        ...
        rprint(f"[bold green]Report generated successfully![/bold green]")
        rprint(f"Report saved to: [cyan]{output_file}[/cyan]")
        logging.info(f"Generated report at '{output_file}'")

        # Display summary
        console.print(f"\n[bold]Report Summary:[/bold]")
        console.print(f"  Tasks: {len(self.current_date_task_lookup)}")
        console.print(f"  Fields: {', '.join(report_fields)}")
