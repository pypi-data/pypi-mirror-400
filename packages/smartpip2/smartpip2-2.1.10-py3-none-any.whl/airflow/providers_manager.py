...
def _create_provider_info_schema_validator():
    ...
    with importlib.resources.files('airflow').joinpath('provider_info.schema.json').open('r', encoding='utf-8') as f:
        schema = json.load(f)  # 直接使用 json.load
...
def _create_customized_form_field_behaviours_schema_validator():
    ...
    schema = json.loads(
        importlib.resources.files('airflow')
        .joinpath('customized_form_field_behaviours.schema.json')
