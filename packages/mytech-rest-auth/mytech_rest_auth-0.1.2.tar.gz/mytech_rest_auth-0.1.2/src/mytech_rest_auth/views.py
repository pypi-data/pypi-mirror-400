...
from core.classes.FullPath import FullPath
from rest_framework import generics, status, views, permissions
# from carteira.models import Conta, ContaUser
...
from core.models import Pessoa, TipoEntidade
# from verification.models import PhoneModel
from .serializers import RegisterSerializer, SetNewPasswordSerializer, ResetPasswordEmailRequestSerializer, EmailVerificationSerializer, LoginSerializer, LogoutSerializer, UserLoginSerializer
...
from core.classes import UserName
import re
...
class RegisterView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        absurl =''
        try:
            absurl = '\n '+request.META['HTTP_ORIGIN']+'/#/verificaremail/'+str(token)
        except:
            absurl = '\n http:mws.mytech.co.mz/#/verificaremail/'+str(token)

        
        html_message = loader.render_to_string( 'email_template_confirmacao.html',
            {
...
class ChangePasswordAPIView(generics.GenericAPIView):
    def post(self, request):
        if 'reset_senha' in request.query_params:
            ...
            return Response({'alert_error': 'OTP errado ou experaado!'}, status=400)
        ...
        user.save()
        return Response({'alert_success': 'Senha alterada com sucesso!'}, status=status.HTTP_200_OK)
