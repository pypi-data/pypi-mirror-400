...
class PajekReader(NetFileFormat):
    EXTENSIONS = ('.net', )
    DESCRIPTION = _tr.m[3, 'Pajek network file format']
    ...
    for (start_line, part_desc), (end_line, *_) \
        ...
        if part_type == "*network":
            ...
            if labels is not None:
                raise ValueError(
                    ("Pajek files with multiple set of vertices are not " + "supported"))
...
def write_pajek(path, network, labels=None):
    if len(network.edges) > 1:
        raise TypeError(
            ("This implementation of Pajek format does not support saving " + "networks with multiple edge types."))
...
def _write_edges(f, network):
    ...
    if np.all(mat.data == 1):
        ...
        for row, rb, re in zip(count(), mat.indptr, mat.indptr[1:]):
            f.write("".join((f"{row + 1:6} {mat.indices[i] + 1:6} " + f"{mat.data[i]:.6f}\n") for i in range(rb, re)))
