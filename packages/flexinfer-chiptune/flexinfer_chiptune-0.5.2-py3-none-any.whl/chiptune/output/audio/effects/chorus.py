...
class Chorus(SampleEffect):
    ...
    def process(
        ...
        for voice in range(self.voices):
            ...
            filtered: NDArray[np.float32] = scipy_signal.lfilter(b_hp, a_hp, voice_output).astype(
                np.float32
            )
            ...
            output += filtered * level

        # Normalize by number of voices
        output = (output / self.voices).astype(np.float32)

        # Mix dry and wet
        result: NDArray[np.float32] = (samples * (1 - self.mix) + output * self.mix).astype(
            np.float32
        )
        ...
        for voice in range(1, self.voices):
            ...
            if resampled_len > 0:
                ...
                resampled: NDArray[np.float32] = np.interp(
                    indices, np.arange(original_len), samples
                ).astype(np.float32)

                # Pad or trim to match original length
                if len(resampled) < original_len:
                    padded: NDArray[np.float32] = np.zeros(original_len, dtype=np.float32)
                ...
                level = 1.0 / (voice + 1)
                output += resampled * level

        # Normalize
        output = (output / (1 + sum(1.0 / (v + 1) for v in range(1, self.voices)))).astype(
            np.float32
        )
    ...
    def process(
        ...
        for _voice in range(self.size):
            ...
            if resampled_len > 0:
                ...
                # Apply delay
                if delay_samples > 0:
                    delayed: NDArray[np.float32] = np.zeros_like(resampled)
                ...
                if self.drift > 0:
                    ...
                    resampled = (resampled * (1 + drift)).astype(np.float32)

                output += resampled

        # Normalize by ensemble size
        output = (output / self.size).astype(np.float32)
