...
def validate_denylist_token_checks(v: list or str):
    if isinstance(v, str):
        v = [v]
    if not set(v).issubset(['access', 'refresh']):
        raise ValueError("The 'authjwt_denylist_token_checks' must be between 'access' or 'refresh'")
    return v


def validate_token_location(v: list or str):
    if isinstance(v, str):
        v = [v]

    v = set(i.lower() for i in v)

    if not v.issubset(['headers', 'cookies']):
        raise ValueError("The 'authjwt_token_location' must be between 'headers' or 'cookies'")
    return v


def validate_csrf_methods(v: list or str):
    if isinstance(v, str):
        v = [v]

    v = set(i.upper() for i in v)

    if not v.issubset(["GET", "HEAD", "POST", "PUT", "DELETE", "PATCH"]):
        raise ValueError("The 'authjwt_csrf_methods' must be between http request methods")
    return v


TokenTypeChecks = Annotated[Optional[Set[StrictStr]], AfterValidator(validate_denylist_token_checks)]
TokenLocation = Annotated[Optional[Set[StrictStr]], AfterValidator(validate_token_location)]
CSRFMethods = Annotated[Optional[Set[StrictStr]], AfterValidator(validate_csrf_methods)]
