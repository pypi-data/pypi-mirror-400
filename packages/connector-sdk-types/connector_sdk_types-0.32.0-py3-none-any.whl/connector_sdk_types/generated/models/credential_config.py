...
class CredentialConfig(BaseModel):
    ...
    input_model: Optional[Any] = Field(default=None, description="The input model expected by the app to authenticate against the 3rd party service. This is provided so that the default model can be overriden by the app.")
    oauth_settings: Optional[Any] = Field(default=None, description="Oauth settings OAuth settings define the behavior of the OAuth Module, each credential can then have its own specific settings This is a way to configure multiple OAuth credentials.  Use the built-in OAuthConfig model (connector.oai.modules.oauth_module_types) to define the config object.")
    __properties: ClassVar[List[str]] = ["id", "type", "description", "optional", "input_model", "oauth_settings"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.input_model is None and "input_model" in self.model_fields_set:
            _dict['input_model'] = None

        # set to None if oauth_settings (nullable) is None
        # and model_fields_set contains the field
        if self.oauth_settings is None and "oauth_settings" in self.model_fields_set:
            _dict['oauth_settings'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "input_model": obj.get("input_model"),
            "oauth_settings": obj.get("oauth_settings")
        })
        return _obj
