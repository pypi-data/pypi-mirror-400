...
def _match_pattern(
    ...
    Return a 2-tuple or raise:
        - match found:
            return (string with the matched part removed, all match groups in a tuple)
        - no match, no errmsg:
            return (unmodified string, None)
        - no match, errmsg:
            raise ValueError()
    """
    ...
    def _parse_range(self, rng: IDT_RangeType) -> tuple[DateTimeType, DateTimeType]:
        ...
        if isinstance(rng, Sequence):
            ...
            if length == 1 and self._RCLOSED_INTERVAL:
                ...
                return (endpoint, endpoint)
            raise ValueError(f"A range cannot have {length} endpoints: {rng}")
