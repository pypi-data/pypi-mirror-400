...
from ..utils.fs import atomic_write_json, codex_path_for, find_ancestor_with_child_dir, global_codex_path, project_codex_path
from ..utils.ops_log import current_context, instrument_tool
...
def _project_trace_path(project_id: str) -> Path:
    return _project_dir(project_id) / "traces.jsonl"


def _append_project_trace(project_id: str, event: dict[str, Any]) -> None:
    if not project_id:
        return
    payload = dict(event)
    ...
    with _TRACE_LOCK:
        p = _project_trace_path(project_id)
...
def _parse_ops_end_line(line: str) -> dict[str, Any] | None:
    # Format: "- <ts> event=end tool=... trace=... state=... elapsed_ms=... code=... msg=..."
    if "event=end" not in line:
        return None
    if "elapsed_ms=" not in line or "tool=" not in line:
        return None
    parts = line.split()
    out: dict[str, Any] = {}
    for p in parts:
    ...
    if "tool" not in out:
        return None
    try:
        out["elapsed_ms"] = int(str(out.get("elapsed_ms") or "0"))
    except Exception:
        out["elapsed_ms"] = 0
    return out
...
def _dispatch_for_job(
    job_id: str,
    *,
    dispatch_base_dir: Path,
    config_factory: Callable[[Path], CodexDispatchConfig] | None = None,
) -> CodexDispatch:
    bucket, _ = _split_bucketed_job_id(job_id)
    if bucket:
        cfg = config_factory(dispatch_base_dir / bucket) if config_factory else CodexDispatchConfig(jobs_dir=dispatch_base_dir / bucket)
        return CodexDispatch(cfg)
    try:
        for child in dispatch_base_dir.iterdir():
            if not child.is_dir():
                continue
            if (child / job_id / "meta.json").exists():
                cfg = config_factory(child) if config_factory else CodexDispatchConfig(jobs_dir=child)
                return CodexDispatch(cfg)
    except FileNotFoundError:
        pass
    cfg = config_factory(dispatch_base_dir) if config_factory else CodexDispatchConfig(jobs_dir=dispatch_base_dir)
    return CodexDispatch(cfg)
...
def register_project_tools(
    ...
    *,
    ...
    prune_settings: DispatchPruneSettings,
    ...
    def _config_with_prune(jobs_dir: Path) -> CodexDispatchConfig:
        return CodexDispatchConfig(
            jobs_dir=jobs_dir,
            prune_enabled=prune_settings.enabled,
            prune_on_job_finish=prune_settings.prune_on_job_finish,
            prune_on_startup=prune_settings.prune_on_startup,
            ...
            prune_dry_run=prune_settings.dry_run,
            prune_verbose=prune_settings.verbose,
        )

    def _dispatch_jobs_dir_for_repo(repo_root: Path) -> Path:
        base = Path(dispatch_base_dir).expanduser()
        try:
            home_codex = Path.home().expanduser().resolve() / ".codex"
            if home_codex in base.resolve().parents or base.resolve() == home_codex:
                return base
        except Exception:
            pass
        return codex_path_for(repo_root, "codex-dispatch")

    def _ensure_worker(project_id: str) -> None:
        if not project_id:
            return
        with worker_lock:
            ev = worker_events.get(project_id)
            if ev is None:
                ev = threading.Event()
                worker_events[project_id] = ev
            t = worker_threads.get(project_id)
            if t and t.is_alive():
                ev.set()
                return

            def _worker_loop() -> None:
                # Single worker per project; starts tasks in background to avoid 60s tool-call deadlines.
                while True:
                    ev.wait(timeout=0.5)
                    ev.clear()
                    try:
                        started_any = True
                        while started_any:
                            started_any = _start_one_queued(project_id)
                    except Exception:
                        # Keep worker alive; errors are recorded per-task.
                        time.sleep(0.5)

            t = threading.Thread(target=_worker_loop, name=f"project-worker-{project_id}", daemon=True)
            worker_threads[project_id] = t
            t.start()
            ev.set()

    def _start_one_queued(project_id: str) -> bool:
        """
        Pop one eligible queued task and perform heavy start steps:
        - git worktree add
        - read prompt
        - dispatch.start(...)
        """
        start_spec: dict[str, Any] | None = None
        task_id: str | None = None
        plan_task: dict[str, Any] | None = None
        task_lock_keys: list[str] = []
        ...
        with _PROJECT_LOCK:
            ...
            # Enforce parallelism based on current running/starting/queued tasks.
            in_flight = 0
            for ts in tasks_state.values():
                if not isinstance(ts, dict):
                    continue
                st = str(ts.get("state") or "")
                if st in {"running", "starting", "queued"}:
                    in_flight += 1
            if in_flight >= DEFAULT_PARALLEL:
                return False
            ...
            for idx, item in enumerate(list(queue)):
                ...
                if not isinstance(ts, dict):
                    continue
                if str(ts.get("state")) != "queued":
                    continue
                pt = plan_by_id.get(tid) or {}

                # deps check
                deps = pt.get("depends_on") if isinstance(pt.get("depends_on"), list) else []
                blocked = False
                for dep in deps:
                ...
                task_lock_keys = _extract_lock_keys(pt.get("locks"))
                if _locks_conflict(task_lock_keys, active_lock_keys):
                    continue

                # Mark as starting and remove from queue.
                ts["state"] = "starting"
                ts["starting_at"] = _utc_now_iso()
                queue.pop(idx)
                task_id = tid
                plan_task = pt
                start_spec = {
                    ...
                    "prompt_path": str(pt.get("prompt_path") or ""),
                    "task_cwd": str(pt.get("cwd") or project.get("cwd") or "").strip() or str(Path.cwd().resolve()),
                    "model": str(pt.get("model")) if pt.get("model") is not None else None,
                    "sandbox": str(pt.get("sandbox")) if pt.get("sandbox") is not None else "workspace-write",
                    "approval_policy": str(pt.get("approval_policy")) if pt.get("approval_policy") is not None else "never",
                    "extra_config": list(pt.get("extra_config")) if isinstance(pt.get("extra_config"), list) else None,
                    "env": dict(pt.get("env")) if isinstance(pt.get("env"), dict) else None,
                }
                _append_project_trace(project_id, {"event": "worker.dequeue", "task_id": tid})
                break

            _save_state(project_id, state)

        if not start_spec or not task_id or not plan_task:
            return False

        # Heavy work outside the project lock.
        prompt_path = str(start_spec.get("prompt_path") or "")
        try:
            prompt_text = Path(prompt_path).expanduser().read_text(encoding="utf-8")
        except FileNotFoundError:
            with _PROJECT_LOCK:
                _, plan, state = _load_bundle(project_id)
                if state and isinstance(state.get("tasks"), dict):
                    ts = state["tasks"].get(task_id)
                    if isinstance(ts, dict):
                        ts["state"] = "failed"
                        ts["last_error"] = "prompt_path not found"
                        ts["finished_at"] = _utc_now_iso()
                        ts.pop("starting_at", None)
                if state:
                    _save_state(project_id, state)
            _append_project_trace(project_id, {"event": "worker.fail", "task_id": task_id, "code": "E_PROMPT_PATH_NOT_FOUND"})
            return True

        task_cwd = str(start_spec.get("task_cwd") or "").strip() or str(Path.cwd().resolve())
        ...
        if not repo_root:
            with _PROJECT_LOCK:
                ...
                if state:
                    _save_state(project_id, state)
            _append_project_trace(project_id, {"event": "worker.fail", "task_id": task_id, "code": "E_GIT_REPO_NOT_FOUND"})
            return True

        # Resolve attempt/worktree/patch paths from state.
        with _PROJECT_LOCK:
            _, _, state = _load_bundle(project_id)
            tasks_state = state.get("tasks") if (state and isinstance(state.get("tasks"), dict)) else {}
            ts = tasks_state.get(task_id) if isinstance(tasks_state.get(task_id), dict) else None
            attempt_num = int(ts.get("attempts") or 0) if isinstance(ts, dict) else 1
            worktrees_dir = _project_dir(project_id) / "worktrees"
            wt_path = worktrees_dir / f"{task_id}_a{attempt_num}"
            patches_dir = _project_dir(project_id) / "patches"
            patch_path = patches_dir / f"{task_id}_a{attempt_num}.patch"
        ...
        if not wt_path.exists():
            ...
            if code != 0:
                with _PROJECT_LOCK:
                    ...
                    if state:
                        _save_state(project_id, state)
                _append_project_trace(project_id, {"event": "worker.fail", "task_id": task_id, "code": "E_WORKTREE_ADD_FAILED"})
                return True

        bucket = _dispatch_bucket_for_cwd(str(repo_root))
        jobs_dir = _dispatch_jobs_dir_for_repo(repo_root)
        dispatch = CodexDispatch(_config_with_prune(jobs_dir / bucket))
        res = dispatch.start(
            prompt=prompt_text,
            model=start_spec.get("model"),
            cwd=str(wt_path),
            sandbox=str(start_spec.get("sandbox") or "workspace-write"),
            approval_policy=str(start_spec.get("approval_policy") or "never"),
            extra_config=start_spec.get("extra_config"),
            env=start_spec.get("env"),
        ...
        with _PROJECT_LOCK:
            ...
            if not state:
                return True
            state = _ensure_state(project_id, _read_json(_project_file(project_id, "plan.json")) or {})
            tasks_state = state.get("tasks") if isinstance(state.get("tasks"), dict) else {}
            ts = tasks_state.get(task_id) if isinstance(tasks_state.get(task_id), dict) else None
            if isinstance(ts, dict):
                ts["state"] = "running" if job_id else "failed"
                if job_id:
                    ts["job_id"] = job_id
            ...
            _save_state(project_id, state)
        _append_project_trace(project_id, {"event": "worker.started", "task_id": task_id, "job_id": job_id})
        return True
    ...
    def _refresh_task_from_dispatch(task_state: dict[str, Any], *, job_id: str) -> None:
        dispatch = _dispatch_for_job(
            job_id,
            dispatch_base_dir=dispatch_base_dir,
            config_factory=_config_with_prune,
        )
    ...
    @instrument_tool("project_create")
    def project_create(
        *,
        title: str | None = None,
        cwd: str | None = None,
    ...
    @instrument_tool("project_plan_set")
    ...
    def project_run(
        *,
        ...
        # Default to background queueing to avoid client 60s tool-call deadlines.
        background = True if options is None else bool(options.get("background", True))
        refresh_running = bool(options.get("refresh_running")) if options else False
        ...
        # Optionally refresh running tasks (can be slow).
        running = 0
        active_lock_keys: list[str] = []
        for tid, ts in list(tasks_state.items()):
            if not isinstance(ts, dict):
                continue
            if refresh_running and str(ts.get("state")) == "running" and ts.get("job_id"):
                _refresh_task_from_dispatch(ts, job_id=str(ts.get("job_id")))
            if str(ts.get("state")) == "running":
                running += 1
                pt = plan_by_id.get(str(tid)) or {}
                active_lock_keys.extend(_extract_lock_keys(pt.get("locks")))
            if str(ts.get("state")) in {"queued", "starting"}:
                running += 1
                pt = plan_by_id.get(str(tid)) or {}
                active_lock_keys.extend(_extract_lock_keys(pt.get("locks")))
        # De-dupe
        ...
        for t in plan_tasks:
            ...
            if not ts:
                continue
            if str(ts.get("state")) in {"running", "completed", "queued", "starting"}:
                skipped.append(tid)
                continue
            ...
            if not prompt_path:
                ts["state"] = "failed"
                ts["last_error"] = "prompt_path required"
                skipped.append(tid)
                continue
            if not Path(prompt_path).expanduser().exists():
                ts["state"] = "failed"
                ts["last_error"] = "prompt_path not found"
                skipped.append(tid)
                continue
            ...
            ts["state"] = "queued" if background else "starting"
            ts["attempts"] = attempt_num
            ts["repo_root"] = str(repo_root)
            ts["worktree_dir"] = str(wt_path)
            ts["patch_path"] = str(patch_path)
            ts["queued_at"] = _utc_now_iso()
            ts.pop("finished_at", None)
            ts.pop("exit_code", None)
            ts.pop("last_error", None)
            started_entry: dict[str, Any] = {"id": tid}
            # No job_id yet when queued; keep handles empty.
        ...
        payload: dict[str, Any] = {"state": "ok", "counts": counts, "queued": started, "skipped": skipped}
    ...
    @instrument_tool("project_status")
    def project_status(
        *,
        project_id: str,
        options: dict[str, Any] | None = None,
        ...
        include_handles = bool(options.get("include_handles")) if options else False
        include_patch = bool(options.get("include_patch")) if options else False
        refresh_requested = bool(options.get("refresh")) if options else False
        max_refresh_raw = options.get("max_refresh") if options else None
        max_refresh = None
        if max_refresh_raw is not None:
            try:
                max_refresh = max(0, int(max_refresh_raw))
            except (TypeError, ValueError):
                max_refresh = 0
        budget_ms = 0
        if options and options.get("budget_ms") is not None:
            try:
                budget_ms = max(0, int(options.get("budget_ms", 0)))
            except (TypeError, ValueError):
                budget_ms = 0
        summary_lines = DEFAULT_SUMMARY_LINES
        include_timing = True if options is None else bool(options.get("include_timing", True))
        refresh_enabled = refresh_requested or include_handles
        refreshed = 0
        refreshable_total = 0
        partial = False
        ...
        _append_project_trace(
            project_id,
            {
                "event": "project_status.start",
                "options": {
        ...
        for t in (plan.get("tasks") if isinstance(plan, dict) else []) or []:
            ...
            needs_patch = bool(
                include_patch and ts.get("patch_path") and not Path(str(ts.get("patch_path"))).exists()
            )
            needs_refresh = refresh_enabled and job_id and (str(ts.get("state")) in {"running"} or needs_patch)
            if needs_refresh:
                refreshable_total += 1
                if not _over_budget() and (max_refresh is None or refreshed < max_refresh):
                    try:
            ...
            if include_patch and ts.get("patch_path"):
                if _over_budget():
                    partial = True
                    budget_exhausted = True
                else:
                    try:
                        ...
                        ts["patch_error"] = "patch generation failed"
            ...
            if st == "completed":
                ...
                running += 1
            elif st in {"pending", "queued", "starting"}:
                pending += 1
            else:
                unknown += 1
        ...
        budget_exhausted = budget_exhausted or _over_budget()
        remaining_budget_ms = None
        if deadline is not None:
            remaining_budget_ms = int(max(0.0, (deadline - time.monotonic()) * 1000.0))
        if max_refresh is not None:
            payload["refresh_limit"] = max_refresh
            if refreshable_total > refreshed:
                partial = True
        if partial or budget_exhausted:
            payload["partial"] = True
            if budget_exhausted:
                payload["budget_exhausted"] = True
            if refreshable_total > refreshed:
                payload["remaining_refresh"] = max(0, refreshable_total - refreshed)
            if remaining_budget_ms is not None:
                payload["remaining_budget_ms"] = remaining_budget_ms
        ...
        _append_project_trace(
            project_id,
            {
                "event": "project_status.end",
                "counts": counts,
                "refreshed": refreshed,
                "partial": bool(payload.get("partial")),
                "budget_exhausted": bool(payload.get("budget_exhausted")),
                "remaining_refresh": payload.get("remaining_refresh"),
    ...
    @instrument_tool("project_collect")
    def project_collect(
        *,
        project_id: str,
        options: dict[str, Any] | None = None,
        ...
        refresh_for_report = True if options is None else bool(options.get("refresh", True))
        status_options: dict[str, Any] = {"include_handles": False, "refresh": refresh_for_report}
        if options and options.get("budget_ms") is not None:
            status_options["budget_ms"] = options.get("budget_ms")
        if options and options.get("max_refresh") is not None:
            status_options["max_refresh"] = options.get("max_refresh")
        _ = project_status(project_id=project_id, options=status_options)
    ...
    @instrument_tool("project_patch_list")
    def project_patch_list(
        *,
        project_id: str,
        options: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        generate_patch = bool(options.get("generate_patch")) if options else False
        budget_ms = int(options.get("budget_ms", 2000)) if options else 2000
        ...
        for t in (plan.get("tasks") if isinstance(plan, dict) else []) or []:
            if budget_ms > 0 and ((time.monotonic() - start) * 1000.0) >= float(budget_ms):
                partial = True
                break
            if not isinstance(t, dict):
                continue
            ...
            p = Path(patch_path)
            if not p.exists():
                # Patch exists in state but wasn't generated yet (or generation failed).
                if isinstance(ts, dict) and ts.get("patch_path"):
                    entry_missing: dict[str, Any] = {
                        "id": tid,
                        "patch_path": str(ts.get("patch_path")),
                        "state": "missing",
                        "message": "patch not generated (pass options.generate_patch=true)",
                    }
                    patches.append(entry_missing)
                continue
        ...
        out: dict[str, Any] = {"state": "ok", "patches": patches}
        if partial:
            out["partial"] = True
        return out
    ...
    @instrument_tool("project_patch_view")
    def project_patch_view(
        *,
        ...
        if not p.exists():
            return {
                "state": "error",
                "code": "E_PATCH_NOT_GENERATED",
                "message": "patch file missing (pass options.generate_patch=true)",
            }
        repo_root = _git_root_for_cwd(str(project.get("cwd") or "")) or Path(str(project.get("cwd") or "")).resolve()
    ...
    @instrument_tool("project_patch_apply")
    ...
    @instrument_tool("project_patch_apply_all")
    ...
    @instrument_tool("project_tick")
    def project_tick(
        *,
        project_id: str,
        options: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        refresh_requested = True if options is None else bool(options.get("refresh", True))
        include_patch = bool(options.get("include_patch")) if options else False
        max_refresh_raw = options.get("max_refresh") if options else None
        max_refresh = None
        if max_refresh_raw is not None:
        ...
        if options and options.get("budget_ms") is not None:
            try:
                ...
                budget_ms = 0
        run_enabled = True if options is None else bool(options.get("run", True))
        deadline = time.monotonic() + (budget_ms / 1000.0) if budget_ms > 0 else None

        def remaining_budget_ms() -> int | None:
            if deadline is None:
                return None
            return int(max(0.0, (deadline - time.monotonic()) * 1000.0))

        status_options: dict[str, Any] = {"include_handles": False, "include_timing": True, "refresh": refresh_requested}
        if include_patch:
            status_options["include_patch"] = True
        if max_refresh is not None:
            status_options["max_refresh"] = max_refresh
        first_budget = remaining_budget_ms()
        if first_budget is not None:
            status_options["budget_ms"] = first_budget

        _append_project_trace(
            project_id,
            {
                "event": "project_tick.start",
                "options": {
                    "refresh": refresh_requested,
                    "include_patch": include_patch,
                    "max_refresh": max_refresh,
        ...
        status_res = project_status(project_id=project_id, options=status_options)
        if status_res.get("state") != "ok":
            return status_res

        partial = bool(status_res.get("partial"))
        remaining_after_status = remaining_budget_ms()
        if deadline is not None and remaining_after_status is not None and remaining_after_status <= 0:
            status_res["partial"] = True
            status_res["budget_exhausted"] = True
            status_res["remaining_ms"] = 0
            return status_res

        run_res: dict[str, Any] = {"state": "disabled"} if not run_enabled else {"state": "ok", "counts": {"started": 0}}
        if run_enabled:
            run_res = project_run(project_id=project_id, options={"include_handles": False})
        if run_res.get("state") not in {"ok", "disabled"}:
            return run_res

        remaining_after_run = remaining_budget_ms()
        if deadline is not None and remaining_after_run is not None and remaining_after_run <= 0:
            ...
            summary.append("budget exhausted before final status refresh")
            return {
                "state": "ok",
                "counts": counts,
                "summary": summary,
        ...
        final_status_options: dict[str, Any] = {"include_handles": False, "include_timing": True, "refresh": refresh_requested}
        if include_patch:
            final_status_options["include_patch"] = True
        if max_refresh is not None:
            final_status_options["max_refresh"] = max_refresh
        final_budget = remaining_budget_ms()
        if final_budget is not None:
            final_status_options["budget_ms"] = final_budget

        status_res = project_status(project_id=project_id, options=final_status_options)
        if status_res.get("state") != "ok":
            return status_res
        ...
        payload: dict[str, Any] = {"state": "ok", "counts": counts, "summary": summary}
        partial = partial or bool(status_res.get("partial"))
        if partial:
            payload["partial"] = True
        remaining_final = remaining_budget_ms()
        if remaining_final is not None:
            payload["remaining_ms"] = remaining_final
            if remaining_final == 0:
                payload["budget_exhausted"] = True
                payload["partial"] = True
        _append_project_trace(
            project_id,
            {"event": "project_tick.end", "counts": counts, "partial": bool(payload.get("partial"))},
        )
        return payload

    @mcp.tool(
        name="project_diagnose",
        description="Diagnose a project for timeout/stall/failure patterns and suggest safer tool options (budget/refresh/patch).",
    )
    @instrument_tool("project_diagnose")
    def project_diagnose(
        *,
        project_id: str,
        options: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        max_ops_lines = int(options.get("max_ops_lines", 200)) if options else 200
        max_trace_lines = int(options.get("max_trace_lines", 200)) if options else 200
        slow_ms = int(options.get("slow_ms", 5000)) if options else 5000
        ...
        ops_path = codex_path_for(Path(cwd) if cwd else Path.cwd(), "operations-log.md")
        trace_path = _project_trace_path(project_id)

        ops_lines = _read_tail_lines(ops_path, max_lines=max_ops_lines)
        end_events = [e for e in (_parse_ops_end_line(ln) for ln in ops_lines) if isinstance(e, dict)]
        slow = [e for e in end_events if int(e.get("elapsed_ms") or 0) >= slow_ms]
        slow_sorted = sorted(slow, key=lambda x: int(x.get("elapsed_ms") or 0), reverse=True)[:10]

        trace_lines = _read_tail_lines(trace_path, max_lines=max_trace_lines)
        ...
        for tid, ts in tasks_state.items():
            ...
            st = str(ts.get("state") or "")
            if st == "running":
                running.append(str(tid))
                if ts.get("stalled_suspected"):
                    stalled.append(str(tid))
            if st in {"queued", "starting"}:
                running.append(str(tid))
            if st == "failed":
                failed.append(str(tid))
            if ts.get("patch_path") and not Path(str(ts.get("patch_path"))).exists():
                missing_patch.append(str(tid))

        suggestions: dict[str, Any] = {
            "project_status": {"refresh": True, "max_refresh": 2, "budget_ms": 2000, "include_patch": False},
            "project_tick": {"refresh": True, "max_refresh": 2, "budget_ms": 4000, "include_patch": False},
            "project_patch_list": {"generate_patch": False, "budget_ms": 2000},
        }
        if stalled:
            suggestions["project_status"]["max_refresh"] = 4
            suggestions["project_tick"]["max_refresh"] = 4
        if slow_sorted:
            # If we see slow calls, suggest stricter budgets.
            suggestions["project_status"]["budget_ms"] = 1500
            suggestions["project_tick"]["budget_ms"] = 3000

        summary: list[str] = []
        if slow_sorted:
            top = slow_sorted[0]
            summary.append(f"Slowest: {top.get('tool')} {top.get('elapsed_ms')}ms")
        if stalled:
            summary.append(f"Stalled: {', '.join(stalled[:8])}{'…' if len(stalled) > 8 else ''}")
        if failed:
            summary.append(f"Failed: {', '.join(failed[:8])}{'…' if len(failed) > 8 else ''}")
        if missing_patch:
            summary.append(f"Missing patch: {', '.join(missing_patch[:8])}{'…' if len(missing_patch) > 8 else ''}")

        return {
            "state": "ok",
            "summary": summary,
            "signals": {"running": running, "stalled": stalled, "failed": failed, "missing_patch": missing_patch},
            "slow_calls": [{"tool": e.get("tool"), "elapsed_ms": e.get("elapsed_ms"), "state": e.get("state")} for e in slow_sorted],
            "traces_tail": traces,
            "suggestions": suggestions,
            "paths": {"ops_log": str(ops_path), "traces": str(trace_path)},
        }
    ...
    @instrument_tool("project_worktree_list")
    ...
    @instrument_tool("project_worktree_cleanup")
