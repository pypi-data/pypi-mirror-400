...
def print_report(reports: List[FileReport], verbose: bool = True) -> Tuple[int, int, int]:
    ...
    for report in reports:
        ...
        if report.has_invalid_utf8:
            ...
            print(f"\n  {report.path}")
            print(f"   ??  INVALID UTF-8: {report.error}")
...
def utf8_main(args) -> int:
    """
    Main entry point for utf8 command.
    ...
    if invalid_utf8:
        print(f"  ??  Files with INVALID UTF-8: {invalid_utf8}")
    ...
    if args.report:
        ...
        return 1 if invalid_utf8 else 0
