        ...
        if not ELASTICSEARCH_AVAILABLE:
            try:
                ...
                error_msg = (
                    ...
                    f"Install with: pip install smf-mcp[elasticsearch7|elasticsearch8|elasticsearch9] or uv add smf-mcp[elasticsearch7|elasticsearch8|elasticsearch9]"
                )
...
def create_elasticsearch_tools(
    es_client: Union[ElasticsearchClient, "Elasticsearch"],
    ...
    Provides tools for searching, reading documents, listing indices, and checking cluster health.
    All tools are read-only and do not modify data.

    Args:
        es_client: ElasticsearchClient instance or native Elasticsearch client
    ...
    # Determine if we have a wrapper or native client
    is_wrapper = isinstance(es_client, ElasticsearchClient)
    if is_wrapper:
    ...
    def list_indices_tool(pattern: Optional[str] = None) -> List[str]:
        ...
        # Use native Elasticsearch API
        if pattern:
            indices = list(client.indices.get_alias(index=pattern).keys())
        else:
