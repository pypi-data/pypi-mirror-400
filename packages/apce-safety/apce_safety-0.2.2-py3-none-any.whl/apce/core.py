...
# ============================================================================
# Shared verifier for convenience functions (v0.2.2)
# ============================================================================
_shared_verifier: Optional[APCEVerifier] = None


def get_shared_verifier(mode: str = "balanced") -> APCEVerifier:
    """Get or create the shared verifier instance."""
    global _shared_verifier
    if _shared_verifier is None:
        _shared_verifier = APCEVerifier(mode=VerificationMode(mode))
    return _shared_verifier


def calibrate_baseline(
    clean_hidden_states: np.ndarray,
    mode: str = "balanced",
) -> None:
    """
    Calibrate the shared verifier baseline from clean model outputs.
    
    IMPORTANT: Call this BEFORE using verify_extended() for detection.
    The baseline should be computed from known-clean model outputs.
    
    Args:
        clean_hidden_states: Hidden states from clean model inference.
            Shape: (batch, seq, hidden_dim) or (n_samples, seq, hidden_dim)
    ...
    Example:
        ...
        for prompt in calibration_prompts:
            _, hidden = get_hidden_states(clean_model, prompt)
            clean_outputs.append(hidden)
        
        # Calibrate (handles variable sequence lengths)
        for h in clean_outputs:
            calibrate_baseline(h)
        
        # Now verify_extended() uses calibrated baseline
        result = verify_extended(attn, hidden)
    """
    global _shared_verifier
    verifier = get_shared_verifier(mode)
    
    # Compute norms from hidden states
    if clean_hidden_states.ndim == 2:
        clean_hidden_states = clean_hidden_states[np.newaxis, ...]
    
    norms = np.linalg.norm(clean_hidden_states.reshape(-1, clean_hidden_states.shape[-1]), axis=-1)
    
    if verifier._value_baseline_mean is None:
        # First calibration
        verifier._value_baseline_mean = float(np.mean(norms))
        verifier._value_baseline_std = float(np.std(norms))
    else:
        # Incremental update (running average)
        new_mean = float(np.mean(norms))
        new_std = float(np.std(norms))
        # Weighted average favoring existing baseline
        verifier._value_baseline_mean = 0.9 * verifier._value_baseline_mean + 0.1 * new_mean
        verifier._value_baseline_std = 0.9 * verifier._value_baseline_std + 0.1 * new_std


def reset_baseline() -> None:
    """Reset the shared verifier baseline to uncalibrated state."""
    global _shared_verifier
    if _shared_verifier is not None:
        _shared_verifier._value_baseline_mean = None
        _shared_verifier._value_baseline_std = None
...
def verify_extended(
    ...
    Uses a shared verifier instance with persistent baseline.
    Call calibrate_baseline() first with clean model outputs for accurate detection.
    ...
    Example:
        # First calibrate with clean data
        calibrate_baseline(clean_hidden_states)
        
        # Then verify potentially backdoored model
        result = verify_extended(attn, hidden)
        if result.D_V > 0.5:
...
def is_backdoor_free(
    ...
    Uses shared verifier. Call calibrate_baseline() first for accurate detection.
