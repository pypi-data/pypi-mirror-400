...
LIST_SECTIONS_NO_GIS = [TITLE, OPTIONS, REPORT, FILES, RAINGAGES, EVAPORATION, TEMPERATURE, ADJUSTMENTS,
                        LID_CONTROLS, LID_USAGE, AQUIFERS, GROUNDWATER, GWF, SNOWPACKS, CONTROLS, POLLUTANTS, LANDUSES, COVERAGES, LOADINGS, BUILDUP, WASHOFF,
                        TREATMENT, RDII,  # kann den Knoten noch zugeordnet werden
                        HYDROGRAPHS, CURVES, TIMESERIES, PATTERNS, STREETS, INLETS,
                        INLET_USAGE, MAP, BACKDROP, PROFILES,
                        # DIVIDERS, LABELS, SYMBOLS,
                        ]
...
def set_crs(inp, crs=None):
    ...
    """
    for sec in [VERTICES, COORDINATES, POLYGONS, SYMBOLS]:
        if sec in inp:
...
def write_geo_package(inp, gpkg_fn, driver='GPKG', label_sep='.', crs=None,
    ...
    todo_sections = NODE_SECTIONS + LINK_SECTIONS + [SUBCATCHMENTS] + [RAINGAGES]
    ...
    if POLYGONS in inp:
        ...
        print(f'{f"{time.perf_counter() - t0:0.1f}s":^{len(SUBCATCHMENTS)}s}', end=' | ')
    ...
    # %%
    if (SYMBOLS in inp) and (RAINGAGES in inp):
        df = inp[RAINGAGES].frame.rename(columns=lambda c: f'{RAINGAGES}{label_sep}{c}')
        df = df.join(inp[SYMBOLS].geo_series)

        if not df.empty:
            gpd.GeoDataFrame(df).to_file(gpkg_fn, driver=driver, layer=RAINGAGES)
        print(f'{f"{time.perf_counter() - t0:0.1f}s":^{len(RAINGAGES)}s}')
    else:
        print(f'{f"-":^{len(RAINGAGES)}s}')
    # t0 = time.perf_counter()
