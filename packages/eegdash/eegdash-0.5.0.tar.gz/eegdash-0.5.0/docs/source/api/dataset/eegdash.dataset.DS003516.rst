..
   This documentation page is generated during the Sphinx build.
   The underlying code is manually maintained and not autogenerated.

eegdash.dataset.DS003516
========================

EEG: Attended Speaker Paradigm (Own Name in Ignored Stream) (OpenNeuro ``ds003516``). Access recordings and metadata through EEGDash.

.. rst-class:: sd-badges

:bdg-light:`Modality: eeg` :bdg-light:`Tasks: 1` :bdg-light:`License: CC0` :bdg-light:`Subjects: 25` :bdg-light:`Recordings: 25` :bdg-light:`Source: openneuro`

Dataset Information
-------------------

.. list-table::
   :widths: 25 75
   :header-rows: 0

   * - Dataset ID
     - ``DS003516``
   * - Title
     - EEG: Attended Speaker Paradigm (Own Name in Ignored Stream)
   * - Year
     - Unknown
   * - Authors
     - Bjoern Holtze, Manuela Jaeger, Stefan Debener, Kamil Adiloglu, Bojana Mirkovic
   * - License
     - CC0
   * - Citation / DOI
     - `10.18112/openneuro.ds003516.v1.1.1 <https://doi.org/10.18112/openneuro.ds003516.v1.1.1>`__
   * - Source links
     - `OpenNeuro <https://openneuro.org/datasets/ds003516>`__ | `NeMAR <https://nemar.org/dataexplorer/detail?dataset_id=ds003516>`__

.. dropdown:: Copy-paste BibTeX
   :class-container: sd-shadow-sm
   :class-title: sd-bg-light

   .. code-block:: bibtex

      @dataset{ds003516,
        title = {EEG: Attended Speaker Paradigm (Own Name in Ignored Stream)},
        author = {Bjoern Holtze and Manuela Jaeger and Stefan Debener and Kamil Adiloglu and Bojana Mirkovic},
        doi = {10.18112/openneuro.ds003516.v1.1.1},
        url = {https://doi.org/10.18112/openneuro.ds003516.v1.1.1},
      }

Highlights
----------

.. grid:: 1 2 3 3
   :gutter: 2

   .. grid-item-card:: Subjects & recordings
      :class-card: sd-border-1

      - Subjects: 25
      - Recordings: 25
      - Tasks: 1

   .. grid-item-card:: Channels & sampling rate
      :class-card: sd-border-1

      - Channels: 47
      - Sampling rate (Hz): 500
      - Duration (hours): 22.57

   .. grid-item-card:: Tasks & conditions
      :class-card: sd-border-1

      - Tasks: 1
      - Experiment type: Attention
      - Subject type: Healthy

   .. grid-item-card:: Files & format
      :class-card: sd-border-1

      - Size on disk: 7.59 GB
      - File count: Unknown
      - Format: BIDS

   .. grid-item-card:: License & citation
      :class-card: sd-border-1

      - License: CC0
      - DOI: 10.18112/openneuro.ds003516.v1.1.1

   .. grid-item-card:: Provenance
      :class-card: sd-border-1

      - Source: openneuro
      - OpenNeuro: `ds003516 <https://openneuro.org/datasets/ds003516>`__
      - NeMAR: `ds003516 <https://nemar.org/dataexplorer/detail?dataset_id=ds003516>`__

Quickstart
----------

**Install**

.. code-block:: bash

   pip install eegdash

**Load a recording**

.. code-block:: python

   from eegdash.dataset import DS003516

   dataset = DS003516(cache_dir="./data")
   recording = dataset[0]
   raw = recording.load()

**Filter/query**

.. tab-set::

   .. tab-item:: Basic

      .. code-block:: python

         dataset = DS003516(cache_dir="./data", subject="01")

   .. tab-item:: Advanced

      .. code-block:: python

         dataset = DS003516(
             cache_dir="./data",
             query={"subject": {"$in": ["01", "02"]}},
         )


Quality & caveats
-----------------

- No dataset-specific caveats are listed in the available metadata.

API
---

.. currentmodule:: eegdash.dataset

.. autoclass:: eegdash.dataset.DS003516
   :members: __init__, save
   :show-inheritance:
   :member-order: bysource


See Also
--------

* :class:`eegdash.dataset.EEGDashDataset`
* :mod:`eegdash.dataset`
* `OpenNeuro dataset page <https://openneuro.org/datasets/ds003516>`__
* `NeMAR dataset page <https://nemar.org/dataexplorer/detail?dataset_id=ds003516>`__

