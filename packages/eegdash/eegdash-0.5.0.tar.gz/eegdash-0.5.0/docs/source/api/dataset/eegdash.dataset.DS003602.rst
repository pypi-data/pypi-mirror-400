..
   This documentation page is generated during the Sphinx build.
   The underlying code is manually maintained and not autogenerated.

eegdash.dataset.DS003602
========================

Childhood Sexual Abuse and problem drinking in women: Neurobehavioral mechanisms (OpenNeuro ``ds003602``). Access recordings and metadata through EEGDash.

.. rst-class:: sd-badges

:bdg-light:`Modality: eeg` :bdg-light:`Tasks: 6` :bdg-light:`License: CC0` :bdg-light:`Subjects: 118` :bdg-light:`Recordings: 699` :bdg-light:`Source: openneuro`

Dataset Information
-------------------

.. list-table::
   :widths: 25 75
   :header-rows: 0

   * - Dataset ID
     - ``DS003602``
   * - Title
     - Childhood Sexual Abuse and problem drinking in women: Neurobehavioral mechanisms
   * - Year
     - Unknown
   * - Authors
     - Ozlem Korucuoglu, Andrey P. Anokhin
   * - License
     - CC0
   * - Citation / DOI
     - `10.18112/openneuro.ds003602.v1.0.0 <https://doi.org/10.18112/openneuro.ds003602.v1.0.0>`__
   * - Source links
     - `OpenNeuro <https://openneuro.org/datasets/ds003602>`__ | `NeMAR <https://nemar.org/dataexplorer/detail?dataset_id=ds003602>`__

.. dropdown:: Copy-paste BibTeX
   :class-container: sd-shadow-sm
   :class-title: sd-bg-light

   .. code-block:: bibtex

      @dataset{ds003602,
        title = {Childhood Sexual Abuse and problem drinking in women: Neurobehavioral mechanisms},
        author = {Ozlem Korucuoglu and Andrey P. Anokhin},
        doi = {10.18112/openneuro.ds003602.v1.0.0},
        url = {https://doi.org/10.18112/openneuro.ds003602.v1.0.0},
      }

Highlights
----------

.. grid:: 1 2 3 3
   :gutter: 2

   .. grid-item-card:: Subjects & recordings
      :class-card: sd-border-1

      - Subjects: 118
      - Recordings: 699
      - Tasks: 6

   .. grid-item-card:: Channels & sampling rate
      :class-card: sd-border-1

      - Channels: 35
      - Sampling rate (Hz): 1000
      - Duration (hours): 159.35

   .. grid-item-card:: Tasks & conditions
      :class-card: sd-border-1

      - Tasks: 6
      - Experiment type: Decision-making
      - Subject type: Other

   .. grid-item-card:: Files & format
      :class-card: sd-border-1

      - Size on disk: 73.21 GB
      - File count: Unknown
      - Format: BIDS

   .. grid-item-card:: License & citation
      :class-card: sd-border-1

      - License: CC0
      - DOI: 10.18112/openneuro.ds003602.v1.0.0

   .. grid-item-card:: Provenance
      :class-card: sd-border-1

      - Source: openneuro
      - OpenNeuro: `ds003602 <https://openneuro.org/datasets/ds003602>`__
      - NeMAR: `ds003602 <https://nemar.org/dataexplorer/detail?dataset_id=ds003602>`__

Quickstart
----------

**Install**

.. code-block:: bash

   pip install eegdash

**Load a recording**

.. code-block:: python

   from eegdash.dataset import DS003602

   dataset = DS003602(cache_dir="./data")
   recording = dataset[0]
   raw = recording.load()

**Filter/query**

.. tab-set::

   .. tab-item:: Basic

      .. code-block:: python

         dataset = DS003602(cache_dir="./data", subject="01")

   .. tab-item:: Advanced

      .. code-block:: python

         dataset = DS003602(
             cache_dir="./data",
             query={"subject": {"$in": ["01", "02"]}},
         )


Quality & caveats
-----------------

- No dataset-specific caveats are listed in the available metadata.

API
---

.. currentmodule:: eegdash.dataset

.. autoclass:: eegdash.dataset.DS003602
   :members: __init__, save
   :show-inheritance:
   :member-order: bysource


See Also
--------

* :class:`eegdash.dataset.EEGDashDataset`
* :mod:`eegdash.dataset`
* `OpenNeuro dataset page <https://openneuro.org/datasets/ds003602>`__
* `NeMAR dataset page <https://nemar.org/dataexplorer/detail?dataset_id=ds003602>`__

