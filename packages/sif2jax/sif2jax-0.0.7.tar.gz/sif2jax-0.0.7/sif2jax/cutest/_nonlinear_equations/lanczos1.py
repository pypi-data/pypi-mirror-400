...
class LANCZOS1(AbstractNonlinearEquations):
    ...
    def _get_data(self):
        ...
        # Y data points computed from SIF file:
        # Y = 0.0951*exp(-X) + 0.8607*exp(-3*X) + 1.5576*exp(-5*X)
        y_data = jnp.zeros_like(x_data)
        y_data += 0.0951 * jnp.exp(-x_data)
        y_data += 0.8607 * jnp.exp(-3.0 * x_data)
        y_data += 1.5576 * jnp.exp(-5.0 * x_data)
