(function () {
  const EXT_ID = "My.CircleMarkers";
  const TOOL_DRAW = "My.CircleMarkers.DrawTool";
  const TOOL_REMOVE = "My.CircleMarkers.RemoveTool";
  const OVERLAY = "my-circlemarkers-overlay";
  ...
  function ensureUiStyles(viewer) {
    ...
    const id = "my-circlemarkers-styles";
    if (doc.getElementById(id)) return;

    const style = doc.createElement("style");
    style.id = id;
    style.textContent = `
      .my-circlemarkers-flyout{
        position:absolute;
        z-index:9999;
      ...
      .my-circlemarkers-menu button:hover{
        background:rgba(55,61,68,.96);
        border-color:rgba(255,255,255,.24);
      }
      .my-circlemarkers-menu button:active{ transform:translateY(1px); }
      .my-circlemarkers-menu button:focus-visible{
        outline:none;
        box-shadow:0 0 0 2px rgba(255,255,255,.10);
        border-color:rgba(255,255,255,.28);
      }

      /* ===== Minimal Circle size panel ===== */
      .my-circlemarkers-size{
        display:none;
        right:12px;
        top:12px;
        width:165px;
      ...
      .my-circlemarkers-size input[type="number"]{
        width:48px;
        height:22px;
        padding:0 6px;
        border-radius:8px;
      ...
      .my-circlemarkers-size input[type="number"]:focus{
        outline:none;
      }
      .my-circlemarkers-size input[type="number"]::-webkit-outer-spin-button,
      .my-circlemarkers-size input[type="number"]::-webkit-inner-spin-button{
        -webkit-appearance:none;
        margin:0;
      }
      .my-circlemarkers-size input[type="number"]{ -moz-appearance:textfield; }

      .my-circlemarkers-size .slider{
        width:100%;
        height:16px;
        background:transparent;
        -webkit-appearance:none;
      ...
      .my-circlemarkers-size .slider::-webkit-slider-runnable-track{
        height:2px;
        background:rgba(255,255,255,.28);
        border-radius:999px;
      }
      .my-circlemarkers-size .slider::-webkit-slider-thumb{
        -webkit-appearance:none;
        appearance:none;
        width:10px;
        height:10px;
      ...
      .my-circlemarkers-size .slider::-moz-range-track{
        height:2px;
        background:rgba(255,255,255,.28);
        border-radius:999px;
      }
      .my-circlemarkers-size .slider::-moz-range-thumb{
        width:10px;
        height:10px;
        border-radius:50%;
        background:#fff;
  ...
  function pickModelPoint(viewer, event) {
    // In 2D, allow picking the paper/background, so do NOT ignore transparent geometry.
    const ignoreTransparent = !is2d(viewer);

    const res = viewer.impl.hitTest(event.canvasX, event.canvasY, ignoreTransparent);
    if (res && res.intersectPoint) return res;

    // Fallback: some 2D cases behave better with clientToWorld
    const hit = viewer.clientToWorld(event.canvasX, event.canvasY, false);
    if (hit && hit.point) {
      return { intersectPoint: hit.point, face: hit.face, dbId: hit.dbId, model: hit.model };
    }
    return null;
  }
  ...
  function createContextMenu(viewer) {
    ...
    menu.className = "my-circlemarkers-flyout my-circlemarkers-menu";
  ...
  function createSizePanel(viewer, state) {
    ...
    panel.className = "my-circlemarkers-flyout my-circlemarkers-size";
  ...
  class DrawCircleTool {
    ...
    handleSingleClick(event) {
      if (!is2d(this.viewer)) return false;
      const res = pickModelPoint(this.viewer, event);
      if (!res) return false;
      this.api.addCircle(res);
      return true;
    }
  }

  class RemoveCircleTool {
    constructor(viewer, api) {
      this.viewer = viewer;
      this.api = api;
    }
    ...
    handleSingleClick(event) {
      if (!is2d(this.viewer)) return false;
      const hit = this.api.pickCircle(event);
      if (!hit) return false;
  ...
  class CircleMarkersExtension extends Autodesk.Viewing.Extension {
    ...
    async load() {
      ...
      this._menu.removeBtn.addEventListener("click", () => {
        if (this.state.selectedId != null) this.removeCircle(this.state.selectedId);
        this.hideRemoveMenu();
      });

      this._sizePanel = createSizePanel(this.viewer, this.state);

      this._toolDraw = new DrawCircleTool(this.viewer, this);
      this._toolRemove = new RemoveCircleTool(this.viewer, this);
    ...
    _createToolbar() {
      ...
      const groupId = "my-circlemarkers-group";
      this._group =
        tb.getControl(groupId) || new Autodesk.Viewing.UI.ControlGroup(groupId);
      if (!tb.getControl(groupId)) tb.addControl(this._group);

      this._btnDraw = new Autodesk.Viewing.UI.Button("my-circlemarkers-draw");
      this._btnDraw.setToolTip("Draw circles");
      this._btnDraw.onClick = () => this._activateMode("draw");
      this._group.addControl(this._btnDraw);

      this._btnRemove = new Autodesk.Viewing.UI.Button("my-circlemarkers-remove");
      this._btnRemove.setToolTip("Remove circles (click circle -> Remove)");
      this._btnRemove.onClick = () => this._activateMode("remove");
      this._group.addControl(this._btnRemove);

      this._btnSize = new Autodesk.Viewing.UI.Button("my-circlemarkers-size");
      this._btnSize.setToolTip("Circle size");
      this._btnSize.onClick = () => this._sizePanel.toggle();
      this._group.addControl(this._btnSize);

      this._btnClear = new Autodesk.Viewing.UI.Button("my-circlemarkers-clear");
      this._btnClear.setToolTip("Clear all circles");
    ...
    addCircle(hitRes) {
      ...
      if (hitRes.face && hitRes.face.normal) {
        const n = hitRes.face.normal.clone();
        n.normalize();
    ...
    clearAll() {
      for (const [, entry] of this._circleById.entries()) {
    ...
    pickCircle(event) {
      if (!is2d(this.viewer)) return null;
    ...
    showRemoveMenuForHit(hit, event) {
      const circleId =
        hit.object && hit.object.userData ? hit.object.userData.circleId : null;
      if (circleId == null) return;

      this.state.selectedId = circleId;
  ...
  Autodesk.Viewing.theExtensionManager.registerExtension(EXT_ID, CircleMarkersExtension);
})();
