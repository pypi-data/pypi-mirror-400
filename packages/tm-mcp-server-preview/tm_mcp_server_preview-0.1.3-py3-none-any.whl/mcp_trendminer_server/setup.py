...
def run_setup() -> bool:
    ...
    try:
        ...
        # Copy README.md from package metadata to current directory
        try:
            # Get package metadata which includes the README
            pkg_metadata = metadata("tm-mcp-server-preview")
            readme_content = pkg_metadata.get("Description")

            if readme_content:
                Path("README.md").write_text(readme_content, encoding="utf-8")
                print("✅ Copied README.md for reference")

        except Exception:
            # Non-critical - silently continue if README copy fails
            pass
