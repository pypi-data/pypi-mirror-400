...
def test_normalize_crispr_counts_se():
    se = create_test_se()
    out = scranpy.normalize_crispr_counts_se(se)
    ...
    out2 = scranpy.normalize_crispr_counts_se(se, size_factors=libs)
    assert numpy.allclose(out.get_column_data()["size_factor"], out2.get_column_data()["size_factor"])

    sf = libs/libs.mean()
    out3 = scranpy.normalize_crispr_counts_se(se, size_factors=sf, center=False)
    assert numpy.allclose(out.get_column_data()["size_factor"], out3.get_column_data()["size_factor"])

    # Works with blocking.
    block = ["A", "B"] * 5
    out_block = scranpy.normalize_crispr_counts_se(se, block=block)
    ...
    out4 = scranpy.normalize_crispr_counts_se(se, factor_name=None)
