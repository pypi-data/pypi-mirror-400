...
from .ladder.ladder cimport OrderbookLadderView
...
cdef class PyAdvancedOrderbook:
    ...
    cpdef get_bbo(self):
        ...
        cdef OrderbookLadderView* bids_view = self._core.get_bids_view()
        cdef OrderbookLadderView* asks_view = self._core.get_asks_view()
        cdef OrderbookLevel* bid0 = &bids_view.levels[0]
        cdef OrderbookLevel* ask0 = &asks_view.levels[0]
        return PyOrderbookLevel.from_struct(bid0[0]), PyOrderbookLevel.from_struct(ask0[0])

    cpdef get_bids(self):
        """Get all bid levels."""
        cdef OrderbookLadderView* v = self._core.get_bids_view()
        cdef u64 n = v.num_levels
        return PyOrderbookLevels.from_ptr(v.levels, n)

    cpdef get_asks(self):
        """Get all ask levels."""
        cdef OrderbookLadderView* v = self._core.get_asks_view()
        cdef u64 n = v.num_levels
        return PyOrderbookLevels.from_ptr(v.levels, n)
