...
class Client:
    ...
    def _determine_url_for_version(self):
        ...
        base_api_url = cloudcix.conf.settings.CLOUDCIX_API_V2_URL.rstrip('/')

        if app_name in FULLY_DEPLOYED:
            # Fully deployed: app name as subdomain
            protocol, host = base_api_url.split("://")
            full_host = f'{app_name}.{host}'
            version = cloudcix.conf.settings.CLOUDCIX_API_VERSION
            if version == 2:
                full_url = f'{protocol}://{full_host}/{self.service_uri}/'
            else:
                full_url = f'{protocol}://{full_host}/v{version}/{self.service_uri}/'
            return full_url.lower()

        # Not fully deployed: app name in path, always v1
        full_url = f'{self.server_url}/{self.application}/v1/{self.service_uri}/'
        return full_url
