...
class OWNxEmbedding(OWWidget):
    name = _tr.m[50, "Network Embeddings"]
    description = _tr.m[51, "Embed network elements"]
    icon = "icons/NetworkEmbedding.svg"
    priority = 6450

    class Inputs:
        network = Input(_tr.m[52, "Network"], Network, default=True)

    class Outputs:
        items = Output(_tr.m[53, "Items"], Table)
    ...
    def __init__(self):
        ...
        gui.spin(box, self, "p", 0.0, 10.0, 0.1, label=_tr.m[54, "Return parameter (p): "],
                 spinType=float, **kwargs)
        gui.spin(box, self, "q", 0.0, 10.0, 0.1, label=_tr.m[55, "In-out parameter (q): "],
                 spinType=float, **kwargs)
        gui.spin(box, self, "walk_len", 1, 100_000, 1, label=_tr.m[56, "Walk length: "],
                 **kwargs)
        gui.spin(box, self, "num_walks", 1, 10_000, 1, label=_tr.m[57, "Walks per node: "],
                 **kwargs)
        gui.spin(box, self, "emb_size", 1, 10_000, 1, label=_tr.m[58, "Embedding size: "],
                 **kwargs)
        gui.spin(box, self, "window_size", 1, 20, 1, label=_tr.m[59, "Context size: "],
                 **kwargs)
        gui.spin(box, self, "num_epochs", 1, 100, 1, label=_tr.m[60, "Number of epochs: "],
                 **kwargs)

        gui.auto_commit(self.controlArea, self, "auto_commit", _tr.m[61, "Commit"],
                        checkbox_label=_tr.m[62, "Auto-commit"], orientation=Qt.Horizontal)
        commit()
