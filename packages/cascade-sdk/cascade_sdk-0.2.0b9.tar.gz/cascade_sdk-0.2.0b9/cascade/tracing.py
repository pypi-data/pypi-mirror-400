...
def init_tracing(
    ...
    Args:
        ...
        json_export_path: Optional path to export spans to a local JSON file (e.g., "/workspace/traces/trace.json")
    """
    ...
    # Add OTLP span processor (exports to Cascade backend)
    span_processor = BatchSpanProcessor(otlp_exporter)
    _tracer_provider.add_span_processor(span_processor)
    
    # Add JSON file exporter if requested (optional, parallel export)
    if json_export_path:
        try:
            from cascade.exporters.json_exporter import JSONFileExporter
            json_exporter = JSONFileExporter(json_export_path)
            json_span_processor = BatchSpanProcessor(json_exporter)
            _tracer_provider.add_span_processor(json_span_processor)
            logger.info(f"JSON file export enabled: {json_export_path}")
        except Exception as e:
            logger.error(f"Failed to initialize JSON file exporter: {e}")
            logger.warning("Continuing without JSON file export")
