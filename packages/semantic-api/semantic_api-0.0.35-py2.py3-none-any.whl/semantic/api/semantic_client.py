...
from semantic.common.utils import msg, err, greenMsg, redMsg, subscription_topics, getDbId, config_request_topic, service_request_topic, isObject
from semantic.common.common_types import PipeArg, SemanticConfig, SchemaId, QueryOptions
...
class semanticClient:
  ...
  def insert(self, doc: str, pipeType: str):
      ...
      '''
        hasObjectFormat is characterization for batch: if one triple object is in object format and not URI format
      '''
      hasObjectFormat = isObject(doc)
      ...
      try:
        ...
        promise = self.insert_primitive(pipe_args)
  ...
  async def frame(self, frame: str, options: Optional[QueryOptions] = None):
      ...
      #greenMsg('pipe_args: '+json.dumps(pipe_args , indent=6))
      response = None
      try:
  ...
  def insert_primitive(self, args: PipeArg):
      '''
          this is no longer async as inserts (not queries) have no response correlated
          the client must subscribe to a topic for inserts seperately
          this is to avoid waiting on insertion and to interoperate with temporal filters
      '''
  ...
  async def semantic_config(self, command: str, args: any):
      ...
      pipe_args: PipeArg = {
          ...
          "doc": body,
          "frame": None,
          "pipeType": "AsyncInsert"
        }
      greenMsg('semantic_config : publishing on topic '+pipe_args.get('topic'))
     # redMsg(json.dumps(pipe_args, indent=6))
  ...
  async def schemaQuery(self):
    ...
    greenMsg('schemaQuery: '+json.dumps(result, indent=6))
    return result.get('doc')
