...
from opentelemetry.instrumentation.aiobotocore.extensions.dynamodb import (
...
class TestDynamoDbExtension(TestBase):
    def setUp(self):
        ...
        self.client = AsyncClient(
            session.create_client("dynamodb", region_name="us-west-2")
        )
    ...
    def assert_span(self, operation: str) -> Span:
        ...
        self.assertEqual(operation, span.attributes[SpanAttributes.DB_OPERATION])
        self.assertEqual(
    ...
    def assert_item_col_metrics(self, span: Span):
        actual = span.attributes[SpanAttributes.AWS_DYNAMODB_ITEM_COLLECTION_METRICS]
        self.assertIsNotNone(actual)
        json.loads(actual)

    def assert_provisioned_read_cap(self, span: Span, expected: int):
        actual = span.attributes[SpanAttributes.AWS_DYNAMODB_PROVISIONED_READ_CAPACITY]
        self.assertEqual(expected, actual)

    def assert_provisioned_write_cap(self, span: Span, expected: int):
        actual = span.attributes[SpanAttributes.AWS_DYNAMODB_PROVISIONED_WRITE_CAPACITY]
        self.assertEqual(expected, actual)
    ...
    def assert_limit(self, span: Span, expected: int):
        self.assertEqual(expected, span.attributes[SpanAttributes.AWS_DYNAMODB_LIMIT])

    def assert_select(self, span: Span, expected: str):
        self.assertEqual(expected, span.attributes[SpanAttributes.AWS_DYNAMODB_SELECT])
    ...
    def test_create_table(self):
        ...
        self.assertEqual(
            ...
            span.attributes[SpanAttributes.AWS_DYNAMODB_GLOBAL_SECONDARY_INDEXES],
        )
        self.assertEqual(
            (json.dumps(local_sec_idx),),
            span.attributes[SpanAttributes.AWS_DYNAMODB_LOCAL_SECONDARY_INDEXES],
        )
    ...
    def test_list_tables(self):
        ...
        self.assertEqual(1, span.attributes[SpanAttributes.AWS_DYNAMODB_TABLE_COUNT])
    ...
    def test_query(self):
        ...
        self.assert_table_names(span, self.default_table_name)
        self.assertTrue(span.attributes[SpanAttributes.AWS_DYNAMODB_SCAN_FORWARD])
    ...
    def test_scan(self):
        ...
        self.assert_table_names(span, self.default_table_name)
        self.assertEqual(16, span.attributes[SpanAttributes.AWS_DYNAMODB_SEGMENT])
        self.assertEqual(
        ...
        self.assertEqual(0, span.attributes[SpanAttributes.AWS_DYNAMODB_SCANNED_COUNT])
    ...
    def test_update_table(self):
        ...
        self.assertEqual(
            ...
            span.attributes[SpanAttributes.AWS_DYNAMODB_GLOBAL_SECONDARY_INDEX_UPDATES],
        )
