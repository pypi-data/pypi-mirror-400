...
def execute_evolve(
    ...
    if code and not kwargs:
        try:
            ...
            if isinstance(parsed_yaml, dict):
                kwargs = {k: str(v) if not isinstance(v, list) else v for k, v in parsed_yaml.items()}
    ...
    if not target:
        yield Message("system", "Error: No target kernel specified. Use: evo_evolve path/to/kernel.cu")
        return
    ...
    valid_gpus = ["t4", "l4", "a10g", "a100", "a100-80gb", "l40s", "h100", "h200", "b200"]
    if evaluator == "local":
        gpu_info = detect_local_gpu()
        if not gpu_info["gpu_available"]:
            yield Message("system", "Error: Local evaluator requested but no GPU detected. Use evaluator: modal instead.")
            return
        gpu_desc = f"{gpu_info['gpu_name']} ({gpu_info['memory_gb']:.1f}GB, CUDA {gpu_info['cuda_version']})"
    else:
        evaluator = "modal"
        if gpu_type not in valid_gpus:
            yield Message("system", f"Error: Invalid GPU type '{gpu_type}'. Valid options: {', '.join(valid_gpus)}")
            return
    ...
    if task is None:
        yield Message("system", """⚠️ **STANDALONE MODE WARNING**
...
""")
    ...
    standalone_note = "\n⚠️ **STANDALONE MODE** - No correctness verification. Verify output manually!" if task is None else ""
    yield Message("system", f"""Evolution started!
...
""",
    ...
    parameters=[
        Parameter(name="target", type="string", description="Path to CUDA kernel file (required)", required=True),
        Parameter(name="task", type="string", description="Task name (default: layernorm)", required=False),
        Parameter(name="iterations", type="integer", description="Number of evolution iterations (default: 100)", required=False),
        Parameter(name="population_size", type="integer", description="Size of population pool (default: 100)", required=False),
        Parameter(name="exploration_ratio", type="number", description="Exploration vs exploitation 0.0-0.8 (default: 0.25)", required=False),
        Parameter(name="evaluator", type="string", description="'modal' (cloud) or 'local' (user's GPU) (default: modal)", required=False),
        Parameter(name="gpu", type="string", description="GPU type for modal: t4, l4, a10g, a100, a100-80gb, l40s, h100, h200, b200 (default: a100)", required=False),
        Parameter(name="include_dirs", type="string", description="Extra include directories for local mode (comma-separated)", required=False),
        Parameter(name="models", type="array", description="List of models with 'name' and 'weight' for the LLM ensemble", required=False),
    ],
)
