...
def ee_initialize(
    ...
    user_agent_prefix: str = "le_map_tools",
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        user_agent_prefix (str, optional): If set, the prefix (version-less)
            value used for setting the user-agent string. Defaults to "le_map_tools".
    ...
    # le.data.setUserAgent(user_agent)
    #
    # if le.data._credentials is not None:
    #     return
    #
    ...
    #     )
    #
    #     le.Initialize(credentials=credentials, **kwargs)
    #     return
    #
    ...
    #     if in_colab_shell() and (le.data._credentials is None):
    #         le.Authenticate()
    #         le.Initialize(**kwargs)
    #         return
    #     else:
    #         auth_mode = "notebook"
    #
    ...
    #
    # le.Authenticate(**auth_args)
    le.Initialize(**kwargs)


def get_info(
    ee_object: Union[le.FeatureCollection, le.Image, le.Geometry, le.Feature],
    layer_name: str = "",
    opened: bool = False,
    ...
    if isinstance(ee_object, le.FeatureCollection):
        ee_object = ee_object.map(lambda f: le.Feature(None, f.toDictionary()))
...
def geometry_type(ee_object: Any) -> str:
    ...
    elif isinstance(ee_object, le.Feature):
        return ee_object.geometry().type().getInfo()
    elif isinstance(ee_object, le.FeatureCollection):
        return le.Feature(ee_object.first()).geometry().type().getInfo()
    else:
        raise TypeError(
...
def geojson_to_ee(
    ...
    geodesic: bool = False,
    encoding: str = "utf-8",
) -> Union[le.Geometry, le.FeatureCollection]:
    ...
    try:
        ...
        if geo_json["type"] == "FeatureCollection":
            ...
            features = le.FeatureCollection(geo_json)
            return features
            ...
            if "style" in geo_json["properties"]:
                ...
                if "radius" in keys:  # Checks whether it is a circle
                    geom = le.Geometry(geo_json["geometry"])
                    radius = geo_json["properties"]["style"]["radius"]
                    geom = geom.buffer(radius)
                elif geo_json["geometry"]["type"] == "Point":
                    geom = le.Geometry(geo_json["geometry"])
                else:
                    geom = le.Geometry(geo_json["geometry"], "", geodesic)
            elif (
                ...
                geom = le.Geometry.Point(longitude, latitude)
            else:
                geom = le.Geometry(geo_json["geometry"], "", geodesic)
            return geom
        else:
