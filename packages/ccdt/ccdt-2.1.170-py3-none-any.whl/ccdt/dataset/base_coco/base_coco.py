...
import numpy as np
import json
from itertools import chain  # itertools 是 Python 标准库中的一个模块，提供了许多用于操作迭代器和生成器的工具函数。chain 是其中的一个函数，用于将多个可迭代对象连接起来，形成一个更大的迭代器。
...
class BaseCoco(BaseLabelme):
    ...
    def add_image(self, file_name: str, width: int, height: int, img_id: int = None, md5_value: str = None, ):
        """
    ...
    def self2coco(self, parameter):
        ...
        for dataset in tqdm(self.datasets):
            ...
            if dataset.get('background') is False:  # 为真表示为处理背景图片
                ...
                for ann in one_img_ann_list:  # 一张图像的多个标注及组合标注在这里进行迭代，迭代前必须组合完成。列表中存放字典元素
                    category_id = 0
    ...
    def labelme_shapes2coco_ann(self, dataset):
        ...
        for structure_shape_key, structure_shape_value in structure_shapes.items():
            ...
            if isinstance(structure_shape_key, int) and self.parameter.coco_to_bbox is not True:  # 处理打组的数据
    ...
    def image_handle(self, dataset):
        ...
        if dataset.get('http_url'):  # 如果http_url有值，表示转换为带http路径的coco图像路径
            ...
            self.result_addImage_id = self.add_image(file_name, dataset.get('image_width'), dataset.get('image_height'), None, dataset.get('md5_value'))
    ...
    def shape_processing(self, structure_shape_value, dataset, one_img_ann_list, point_list, rebuild_polygon_point_shape):
        ...
        shape_type = list()
        for shape in structure_shape_value:
            if shape.setdefault('shape_type') == 'polygon' and self.parameter.coco_to_bbox is not True:
    ...
    def polygon_shape(self, shape, dataset, one_img_ann_list, rebuild_polygon_point_shape):
        """
        处理标注形状为多边形的shape，1、多边形转segmentation列表，2、多边形转矩形框
        :param shape: 标注属性字典
    ...
    def polygon_shape_bbox(self, shape, dataset):
        ...
        if len(shape['points']) == 4:
            ...
            point_min, point_max = polygon_points.min(axis=0), polygon_points.max(axis=0)
            # 求出坐标点的极大值和极小值后，无需管先后顺序，直接填入shape['points']中,变成矩形框左上角坐标、右下角坐标
            bbox = [[point_min[0], point_min[1]], [point_max[0], point_max[1]]]  # 把多边形标注转成矩形框
            if self.rectangle_cross_the_border(bbox, dataset, True):  # 如果函数返回为True，表示标注形状已经超出图像边界
                print(f'标注形状超出图像边界。人工核对后，选择程序进行矫正错误{file_path}')
                ...
                try:
                    ...
                    rebuild_polygon_point_shape.update({'bbox': bbox})  # 更新多边形点的坐标为矩形框坐标
                    rebuild_polygon_point_shape.update({'label': shape['label']})
                    flattened_points = list(chain.from_iterable(shape.get('points')))  # 将多个列表中的元素连接起来成为一个单一的列表。
                    rebuild_polygon_point_shape.update({'segmentation': flattened_points})  # 同时处理把多边形点的坐标列表转换到segmentation单列表中
                    if 'text' in shape:  # 判断 'text' 键是否存在
                        if shape['text'] is not None:  # 判断 'text' 值是否存在
                            rebuild_polygon_point_shape.update({'score': shape['text']})  # 如果阈值存在，就把阈值添加进去
                    one_img_ann_list.append(rebuild_polygon_point_shape)
                except Exception as e:
                    print(e)
                    file_path = dataset.get('full_path')
                    print(f'多边形排序出错{file_path}')
        else:
            print(f'车牌多边形标注不为4个点,已经存储到自定义默认错误数据集目录，修改完成后覆盖真实数据集，并重新转换')
            dataset.update({'output_dir': dataset.get('error_path')})
        return one_img_ann_list

    @staticmethod
    def rectangle_shape_bbox(shape):
        one_img_ann_list = list()
        rebuild_rectangle_shape = {}  # 存储shape重构为coco标注属性字典变量
        rebuild_rectangle_shape.update({'bbox': shape['points']})  # 更新多边形点的坐标为矩形框坐标
        rebuild_rectangle_shape.update({'label': shape['label']})
        if 'text' in shape:  # 判断 'text' 键是否存在
            if shape['text'] is not None:  # 判断 'text' 值是否存在
                rebuild_rectangle_shape.update({'score': shape['text']})  # 如果阈值存在，就把阈值添加进去
        one_img_ann_list.append(rebuild_rectangle_shape)
        return one_img_ann_list
