...
from spacedata.models.donki import DONKIIps, DONKIIpsInput
...
class DonkiIpsResult(SpaceDataResult, typing.Generic[T]):
    URL_KEY = "link"
...
async def _get_response(
    ...
    response = await resource.client.make_request(
        method="GET",
        url=f"{resource.client.settings.main_base_url}/DONKI/IPS",
        params=params,
    )
    return [DONKIIps.model_validate(item) for item in response]


@api_resource_pipeline(input_model=DONKIIpsInput)
async def ips(
    self: typing.Annotated[BaseResourceProtocol, SkipValidation],
    *,
    input_data: DONKIIpsInput | None = None,
    **kwargs,
) -> DonkiIpsResult[DONKIIps]:
    self.client.logger.debug("Calling ips endpoint with input_data: %s", input_data)
    ...
    return DonkiIpsResult(items=items_generator)
