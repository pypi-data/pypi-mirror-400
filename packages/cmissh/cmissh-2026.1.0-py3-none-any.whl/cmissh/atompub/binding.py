...
from cmissh import messages
from cmissh.cmis_services import Binding, RepositoryServiceIfc
from cmissh.domain import (
    ACE,
    ACL,
    ChangeEntry,
    CmisId,
...
class AtomPubBinding(Binding):
    ...
    def get(self, url, session, **kwargs):
        ...
        there.
        """
        response = super().get(url, session, **kwargs)
        if "+xml" in response.headers.get("content-type"):
            try:
                return minidom.parse(BytesIO(response.content))
            except ExpatError:
                msg = "Could not parse server response"
                raise CmisException(msg, url)
        return response
    ...
    def post(self, url, session, payload, contentType, **kwargs):
        ...
        response = super().post(url, session, payload, contentType, **kwargs)
    ...
    def put(self, url, session, payload, contentType, **kwargs):
        ...
        been checked out, you'd call :class:`Document.checkin` on the PWC.
        """
        response = super().put(url, session, payload, contentType, **kwargs)
        if response.content and "+xml" in response.headers.get("content-type"):
            try:
                return minidom.parse(BytesIO(response.content))
            except ExpatError:
...
class RepositoryService(RepositoryServiceIfc):
    ...
    def reload(self, obj):
        ...
        obj.xmlDoc = obj._cmisClient.binding.get(
            obj._cmisClient.repositoryUrl, obj._cmisClient.session
        )
        obj._initData()
...
class AtomPubCmisObject(CmisObject):
    ...
    def reload(self, **kwargs):
        ...
        self.xmlDoc = self._cmisClient.binding.get(
            byObjectIdUrl, self._cmisClient.session, **addOptions
        )
        self._initData()
    ...
    def getObjectParents(self, **kwargs):
        ...
        if parentUrl is None:
            msg = "Root folder does not support getObjectParents"
            raise NotSupportedException(msg)

        # invoke the URL
        result = self._cmisClient.binding.get(
            parentUrl, self._cmisClient.session, **kwargs
        )
    ...
    def updateProperties(self, properties):
        ...
        objectTypeId = None
        if (
            "cmis:objectTypeId" in self.properties
            and "cmis:objectTypeId" not in properties
        ):
    ...
    def delete(self, **kwargs):
        ...
        self._cmisClient.binding.delete(url, self._cmisClient.session, **kwargs)
    ...
    def applyPolicy(self, policyId):
        ...
        if self.getAllowableActions()["canApplyPolicy"]:
            ...
            msg = "This object has canApplyPolicy set to false"
            raise CmisException(msg)
    ...
    def createRelationship(self, targetObj, relTypeId):
        ...
        result = self._cmisClient.binding.post(
            url, self._cmisClient.session, xmlDoc.toxml(encoding="utf-8"), ATOM_XML_TYPE
        )
    ...
    def getRelationships(self, **kwargs):
        ...
        result = self._cmisClient.binding.get(url, self._cmisClient.session, **kwargs)
    ...
    def removePolicy(self, policyId):
        ...
        if self.getAllowableActions()["canRemovePolicy"]:
            ...
            msg = "This object has canRemovePolicy set to false"
            raise CmisException(msg)
    ...
    def getAppliedPolicies(self):
        ...
        if self.getAllowableActions()["canGetAppliedPolicies"]:
            ...
            msg = "This object has canGetAppliedPolicies set to false"
            raise CmisException(msg)
    ...
    def getACL(self):
        ...
        if self._repository.getCapabilities()["ACL"]:
            ...
            result = self._cmisClient.binding.get(aclUrl, self._cmisClient.session)
    ...
    def applyACL(self, acl):
        ...
        if self._repository.getCapabilities()["ACL"] == "manage":
            ...
            if not isinstance(acl, ACL):
                msg = "The ACL to apply must be an instance of the ACL class."
                raise CmisException(msg)
            aclUrl = self._getLink(ACL_REL)
...
class AtomPubRepository:
    ...
    def reload(self):
        ...
        self.logger.debug("Reload called on object")
        self.xmlDoc = self._cmisClient.binding.get(
            self._cmisClient.repositoryUrl, self._cmisClient.session
        )
        self._initData()
    ...
    def getTypeDescendants(self, typeId=None, **kwargs):
        ...
        if not descendUrl:
            msg = "Could not determine the type descendants URL"
            raise NotSupportedException(msg)

        typesXmlDoc = self._cmisClient.binding.get(
            descendUrl, self._cmisClient.session, **kwargs
        )
        entryElements = typesXmlDoc.getElementsByTagNameNS(ATOM_NS, "entry")
        types = []
        for entryElement in entryElements:
    ...
    def getTypeDefinitions(self, **kwargs):
        ...
        typesXmlDoc = self._cmisClient.binding.get(
            typesUrl, self._cmisClient.session, **kwargs
        )
        entryElements = typesXmlDoc.getElementsByTagNameNS(ATOM_NS, "entry")
        types = []
        for entryElement in entryElements:
    ...
    def getObjectByPath(self, path, **kwargs):
        ...
        result = self._cmisClient.binding.get(
            byObjectPathUrl, self._cmisClient.session, **addOptions
        )
    ...
    def getContentChanges(self, **kwargs):
        ...
        result = self._cmisClient.binding.get(
            changesUrl, self._cmisClient.session, **kwargs
        )
    ...
    def createDocumentFromString(
        ...
        return parentFolder.createDocument(
            name,
            properties,
            BytesIO(contentString.encode("utf-8")),
            contentType,
            contentEncoding,
        )
    ...
    def getCollection(self, collectionType, **kwargs):
        ...
        >>> from cmissh.atompub.atompub_binding import TYPES_COLL
        ...
        >>> from cmissh.atompub.atompub_binding import CHECKED_OUT_COLL
        ...
        result = self._cmisClient.binding.get(
            self.getCollectionLink(collectionType), self._cmisClient.session, **kwargs
        )
...
class AtomPubResultSet(ResultSet):
    ...
    def _getPageResults(self, rel):
        ...
        if link:
            result = self._cmisClient.binding.get(link, self._cmisClient.session)
...
class AtomPubDocument(AtomPubCmisObject):
    ...
    def getAllVersions(self, **kwargs):
        ...
        # invoke the URL
        result = self._cmisClient.binding.get(
            versionsUrl, self._cmisClient.session, **kwargs
        )
    ...
    def getContentStream(self):
        ...
        if node_has_attibute(contentElements[0], "src"):
            ...
            response = self._cmisClient.binding.get(srcUrl, self._cmisClient.session)
    ...
    def setContentStream(self, contentFile, contentType=None):
        ...
        result = self._cmisClient.binding.put(
            srcUrl, self._cmisClient.session, contentFile.read(), mimetype, **args
        )
    ...
    def deleteContentStream(self):
        ...
        self._cmisClient.binding.delete(srcUrl, self._cmisClient.session, **args)
...
class AtomPubFolder(AtomPubCmisObject):
    ...
    def getChildren(self, **kwargs):
        ...
        # invoke the URL
        result = self._cmisClient.binding.get(
            childrenUrl, self._cmisClient.session, **kwargs
        )
    ...
    def getDescendants(self, **kwargs):
        ...
        if not self._repository.getCapabilities()["GetDescendants"]:
            msg = "This repository does not support getDescendants"
            raise NotSupportedException(msg)
        ...
        # invoke the URL
        result = self._cmisClient.binding.get(
            descendantsUrl, self._cmisClient.session, **kwargs
        )
    ...
    def getParent(self):
        ...
        parentUrl = self._getLink(UP_REL)
        # invoke the URL
        result = self._cmisClient.binding.get(parentUrl, self._cmisClient.session)
    ...
    def deleteTree(self, **kwargs):
        ...
        if not self._repository.getCapabilities()["GetDescendants"]:
            msg = "This repository does not support deleteTree"
            raise NotSupportedException(msg)
    ...
    def addObject(self, cmisObject, **kwargs):
        ...
        if not self._repository.getCapabilities()["Multifiling"]:
            msg = "This repository does not support multifiling"
            raise NotSupportedException(msg)
    ...
    def removeObject(self, cmisObject):
        ...
        if not self._repository.getCapabilities()["Unfiling"]:
            msg = "This repository does not support unfiling"
            raise NotSupportedException(msg)
...
class AtomPubObjectType(ObjectType):
    ...
    def getLink(self, rel, linkType):
        ...
        >>> from cmissh.atompub.atompub_binding import ATOM_XML_FEED_TYPE
        ...
        for linkElement in linkElements:
            if node_has_attibute(linkElement, "rel") and node_has_attibute(
                linkElement, "type"
            ):
                relAttr = linkElement.attributes["rel"].value
                typeAttr = linkElement.attributes["type"].value
    ...
    def reload(self, **kwargs):
        ...
        result = self._cmisClient.binding.get(
            byTypeIdUrl, self._cmisClient.session, **kwargs
        )
...
def getElementNameAndValues(propType, propName, propValue, isList=False):
    ...
    if propType == "id" or propType == CmisId:
        ...
        if isList:
            propValueStrList = list(propValue)
        else:
        ...
        if isList:
            propValueStrList = [
                val.isoformat() if val is not None else val for val in propValue
            ]
        else:
        ...
        if isList:
            propValueStrList = [
                unicode(val).lower() if val is not None else val for val in propValue
            ]
        else:
            if propValue is not None:
        ...
        if isList:
            propValueStrList = [
                unicode(val) if val is not None else val for val in propValue
            ]
        else:
            if propValue is not None:
