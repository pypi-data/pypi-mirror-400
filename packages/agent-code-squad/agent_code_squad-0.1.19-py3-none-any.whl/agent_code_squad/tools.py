...
from .core.execution import EXECUTION_MODES, coerce_execution_mode as _coerce_execution_mode, select_execution_mode as _select_execution_mode
from .core.scopes import (
    extract_glob_prefixes as _extract_glob_prefixes,
    matches_any as _matches_any,
    normalize_relpath as _normalize_relpath,
    parse_globs as _parse_globs,
    rg_list_files as _rg_list_files,
)
from .core.supervisor import (
    DEFAULT_SUPERVISOR_CONTEXT,
    build_supervisor_plan,
    plan_run_id as _supervisor_plan_run_id,
...
from .event_utils import (
    ...
    NOISE_EVENT_TYPES,
    NOISY_ITEM_TYPES,
    describe_timeline_event as _describe_timeline_event,
...
def register_tools(mcp: FastMCP, *, config: SquadConfig) -> None:
    ...
    def _timeline_all(run_id: str) -> list[dict[str, Any]]:
        path = _timeline_path(run_id)
        try:
    ...
    def _timeline_has_event(events: list[dict[str, Any]], name: str) -> bool:
        return any(isinstance(ev, dict) and ev.get("event") == name for ev in events)

    def _recent_timeline_summary(events: list[dict[str, Any]]) -> str | None:
        shared: str | None = None
        other: str | None = None
        for entry in reversed(events or []):
            if not isinstance(entry, dict):
                continue
            text = _describe_timeline_event(entry)
            if not text:
                continue
            name = str(entry.get("event") or "")
            if name.startswith("shared_kb."):
                if shared is None:
                    shared = text
            else:
                if other is None:
            ...
            if shared and other:
                break
        if shared and other:
            return f"{shared} | {other}"
        return shared or other

    def _phase_summary_from_meta(run_meta: dict[str, Any], timeline_events: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], str, str | None]:
        tasks = run_meta.get("tasks", [])
        timeline_events = timeline_events or []
        phases: list[dict[str, Any]] = []
        preflight_info = run_meta.get("preflight") if isinstance(run_meta.get("preflight"), dict) else {}
        if preflight_info.get("needs_user_confirm"):
            phases.append({"name": "preflight", "status": "failed", "detail": "等待确认"})
        elif preflight_info:
            phases.append({"name": "preflight", "status": "done"})
        else:
            phases.append({"name": "preflight", "status": "done", "detail": "跳过"})

        context_count = sum(1 for t in tasks if t.get("context_paths"))
        context_detail = f"{context_count} 个上下文" if context_count else "禁用"
        phases.append({"name": "context_pack", "status": "done", "detail": context_detail})

        queued_waiting = [t for t in tasks if str(t.get("state") or "").lower() == "queued" and not t.get("worker_enqueued_at")]
        starting_task = next((t for t in tasks if str(t.get("state") or "").lower() == "starting"), None)
        if starting_task:
            phases.append({"name": "worktree", "status": "running", "detail": starting_task.get("slug")})
        elif queued_waiting:
            phases.append({"name": "worktree", "status": "pending"})
        else:
            phases.append({"name": "worktree", "status": "done"})

        running_tasks = [t for t in tasks if str(t.get("state") or "").lower() in ("running", "starting")]
        failed_tasks = [t for t in tasks if _is_failed_like(t.get("state"))]
        terminal_tasks = [t for t in tasks if _is_terminal_state(t.get("state"))]
        if failed_tasks:
            phases.append({"name": "agent_run", "status": "failed", "detail": failed_tasks[0].get("slug")})
        elif running_tasks:
            phases.append({"name": "agent_run", "status": "running", "detail": running_tasks[0].get("slug")})
        elif tasks and len(terminal_tasks) == len(tasks):
            phases.append({"name": "agent_run", "status": "done"})
        else:
            phases.append({"name": "agent_run", "status": "pending"})

        finalize_cfg = run_meta.get("finalize") if isinstance(run_meta.get("finalize"), dict) else {}
        finalize_state = str(finalize_cfg.get("state") or "").lower()
        finalize_failed = finalize_state == "failed"
        has_collect = _timeline_has_event(timeline_events, "finalize.collect")
        has_done = _timeline_has_event(timeline_events, "finalize.done")
        if finalize_failed:
            phases.append({"name": "collect", "status": "failed"})
        elif has_collect or finalize_state == "completed" or has_done:
            phases.append({"name": "collect", "status": "done"})
        elif finalize_state == "running":
            phases.append({"name": "collect", "status": "running"})
        else:
            phases.append({"name": "collect", "status": "pending"})

        cleanup_enabled = bool(finalize_cfg.get("cleanup", True))
        has_cleanup = _timeline_has_event(timeline_events, "finalize.cleanup")
        if not cleanup_enabled:
            phases.append({"name": "cleanup", "status": "done", "detail": "跳过"})
        elif finalize_failed:
            phases.append({"name": "cleanup", "status": "failed"})
        elif has_cleanup or finalize_state == "completed" or has_done:
            phases.append({"name": "cleanup", "status": "done"})
        elif finalize_state == "running" and (has_collect or tasks and all(_is_terminal_state(t.get("state")) for t in tasks)):
            phases.append({"name": "cleanup", "status": "running"})
        else:
            phases.append({"name": "cleanup", "status": "pending"})

        current_phase = "全部完成"
        for phase in phases:
            status = phase.get("status")
            label = _phase_label(phase.get("name", ""))
            if status == "failed":
                current_phase = f"{label}异常"
                break
            if status == "running":
                current_phase = f"{label}中"
                break
            if status == "pending":
                current_phase = f"等待{label}"
                break

        recent_summary = _recent_timeline_summary(timeline_events)
        return phases, current_phase, recent_summary
    ...
    def _status_verbosity(options: Any | None) -> str:
        opts = _coerce_options(options)
        debug = _debug_enabled(opts)
        if _debug_enabled(opts):
            return "debug"
        raw = str(opts.get("status_verbosity") or "").strip().lower()
        if raw in ("brief", "normal", "debug"):
            return "debug" if raw == "debug" else raw
        return "brief"

    def _coerce_max_tasks_out(opts: dict[str, Any]) -> int:
        raw = opts.get("max_tasks_out", 20)
        try:
            value = int(raw)
    ...
    def _coerce_interrupt_mode(value: Any, *, default: str = "detach") -> str:
        if isinstance(value, str):
            normalized = value.strip().lower()
            if normalized in ("detach", "cancel"):
                return normalized
        return default

    def _resolve_auto_context_pack(opts: dict[str, Any], effective_config: SquadConfig, *, execution_mode: str) -> dict[str, Any] | None:
        """
        ...
        explicit = "auto_context_pack" in opts
        if explicit:
        ...
        if not explicit and execution_mode in ("direct", "batch-worktree"):
            auto_context_pack = None
    ...
    def _dependency_waiting_slugs(task: dict[str, Any], task_map: dict[str, dict[str, Any]]) -> list[str]:
        depends_on = task.get("depends_on") if isinstance(task.get("depends_on"), list) else []
        waiting: list[str] = []
        for dep in depends_on:
            ...
            if not dep_slug:
                continue
            dep_task = task_map.get(dep_slug)
            if not isinstance(dep_task, dict):
                continue
            dep_state = str(dep_task.get("state") or "").lower()
            if dep_state != "completed":
                waiting.append(dep_slug)
        return waiting

    def _clean_status_line(value: Any) -> str | None:
        if value is None:
            return None
        ...
        for ln in reversed(text.splitlines()):
            ...
            if stripped:
                last_line = stripped
                break
        candidate = last_line or text
        if not candidate:
            return None
        if candidate.startswith("{") or candidate.startswith("**") or "/bin/zsh -lc" in candidate:
            return None
        return truncate_text(candidate, 240)
    ...
    def _enqueue_ready_tasks(run_id: str, run_meta: dict[str, Any], *, max_starts: int | None = None) -> int:
        ...
        enqueued = 0
        now = utc_now_iso()
        execution_mode = str(run_meta.get("execution_mode") or "")
        sequential_mode = execution_mode == "batch-worktree"

        def _has_inflight_tasks() -> bool:
            for t in run_meta.get("tasks", []):
                state = str(t.get("state") or "").lower()
                if state in ("running", "starting"):
                    return True
                if t.get("worker_enqueued_at") and state == "queued":
                    return True
            return False

        if sequential_mode and _has_inflight_tasks():
            return 0
    ...
    def _shared_kb_session_id(opts: dict[str, Any]) -> str:
        raw = opts.get("shared_kb_session_id")
        if raw is None:
            raw = opts.get("session_id")
        session = str(raw or "default").strip()
        return session or "default"

    def _shared_kb_settings(opts: dict[str, Any], *, execution_mode: str, task_count: int) -> dict[str, Any] | None:
        if "shared_kb" not in opts:
            return None
        raw = opts.get("shared_kb")
        ...
        if isinstance(raw, bool):
            ...
            if mode == "auto" and enabled is None:
                enabled = True
        else:
        ...
        if enabled is None:
            enabled = False
        if mode == "auto":
            enabled = bool(enabled) and execution_mode == "fanout" and int(task_count or 0) >= 2
        return {"enabled": bool(enabled), "mode": mode or "force", "session_id": _shared_kb_session_id(opts)}
    ...
    def _preflight_tasks(cwd: str, tasks: list[dict[str, Any]], opts: dict[str, Any]) -> dict[str, Any]:
        ...
        missing_allow_tasks: list[str] = []
        scope_prefix_owners: dict[str, list[str]] = {}
        scope_analysis: list[dict[str, Any]] = []
        ...
        for idx, task in enumerate(tasks):
            ...
            if allow_globs is None:
                ...
                missing_allow_tasks.append(name)
            else:
                prefixes = _extract_glob_prefixes(allow_globs)
                for prefix in prefixes:
                    scope_prefix_owners.setdefault(prefix, []).append(name)
            if allow_globs and scope_samples == [] and scope_warning is None:
            ...
            scope_analysis.append({"allow_globs": allow_globs, "deny_globs": deny_globs, "sample_count": len(scope_samples)})
        ...
        execution_mode, mode_meta = _select_execution_mode(
            repo_root=repo_root,
            tasks=tasks,
            opts=opts,
            effective_config=effective_config,
            precomputed_scope=scope_analysis,
        )
        auto_context_pack = _resolve_auto_context_pack(opts, effective_config, execution_mode=execution_mode)

        for prepared in prepared_tasks:
            allow_globs = prepared["allow_globs"]
            deny_globs = prepared["deny_globs"]
            context_pack = prepared["context_pack"]
            scope_name = prepared["scope_name"]
            ...
            task_questions = [f"Task {prepared['name']}: {item}" for item in prepared["scope_issues"] + context_issues + prepared["prompt_issues"]]
            ...
            task_reports.append(
                {
                    ...
                    "scope": {
                        "allow_globs": allow_globs or [],
                        "deny_globs": deny_globs or [],
                        "sample_paths": prepared["scope_samples"],
                        "sample_count": len(prepared["scope_samples"]),
                        "warning": prepared["scope_warning"],
                        "issues": prepared["scope_issues"],
                    },
                    "context": context_detail,
                    "prompt_rules": prepared["prompt_rules"],
        ...
        allow_many_tasks = _coerce_bool(opts.get("preflight_allow_many_tasks")) is True
        if not allow_many_tasks:
            merge_reasons: list[str] = []
            worker_limit = getattr(effective_config, "worker_concurrency", 1) or 1
            recommended_max = max(6, int(worker_limit) * 2)
            if len(tasks) > recommended_max:
                merge_reasons.append(f"{len(tasks)} tasks > suggested max {recommended_max}")
            if len(missing_allow_tasks) >= 2:
                merge_reasons.append(f"{len(missing_allow_tasks)} tasks missing allow_globs")
            overlapping_prefixes = [
                prefix for prefix, owners in scope_prefix_owners.items() if prefix and len(owners) >= 2
            ]
            if overlapping_prefixes:
                merge_reasons.append("overlapping scopes around: " + ", ".join(sorted(overlapping_prefixes)[:3]))
            if merge_reasons:
                merge_question = (
                    "Plan looks over-split (" + "; ".join(merge_reasons) + "); consider merging tasks to reduce overhead/context."
                )
                questions.append(merge_question)
                needs_confirm = True
        ...
        return {
            ...
            "execution_mode": {
                ...
                "explicit": bool(mode_meta.get("explicit")),
                "reason": mode_meta.get("reason"),
                "analysis": mode_meta.get("analysis"),
            },
    ...
    def _collect_run(run_id: str, run_meta: dict[str, Any], *, include_patch: bool, debug: bool) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        ...
        kb: shared_kb.SharedKB | None = None
        shared_meta = run_meta.get("shared_kb") if isinstance(run_meta.get("shared_kb"), dict) else None
        if isinstance(shared_meta, dict) and shared_meta.get("enabled") is True:
            session_id = str(shared_meta.get("session_id") or "default")
            repo_root = Path(run_meta.get("repo_root") or ".").resolve()
            dispatch_dir = Path(run_meta.get("dispatch_base") or config.dispatch_dir)
            try:
                kb = shared_kb.load_or_init(dispatch_dir=dispatch_dir, session_id=session_id, repo_root=repo_root)
            except Exception:
                kb = None
        for task in run_meta.get("tasks", []):
            job_id = task.get("job_id")
            ...
            if kb is not None:
                kb.append_collect(
                    run_id=run_id,
                    slug=str(task.get("slug") or slug),
                    touched_files=normalized_touched,
                    patch_bytes=patch_bytes,
                    scope_ok=bool(scope_ok),
                    error=str(task.get("error") or "").strip() or None,
                )
            entry: dict[str, Any] = {
                "job_id": job_id,
                "name": task.get("name"),
        ...
        *,
        ...
        for task in run_meta.get("tasks", []):
            ...
            if resolved_key in seen_paths:
                kept.append({"slug": slug, "state": state, "path": str(resolved), "reason": "shared_path"})
                continue
            seen_paths.add(resolved_key)
            if resolved == repo_root.resolve():
                kept.append({"slug": slug, "state": state, "path": str(resolved), "reason": "repo_root"})
                continue
                ...
                errors.append(
                    {
                        ...
                        "stderr": truncate_text((res.stderr or "").strip(), 400),
                    }
                )
        return {"removed": removed, "kept": kept, "errors": errors}

    def _prune_git_worktrees(repo_root: Path) -> None:
        subprocess.run(["git", "worktree", "prune"], cwd=repo_root, check=False, capture_output=True)

    def _remove_worktree_base_dir(repo_root: Path, worktree_base: str | None) -> bool:
        if not worktree_base:
            return False
        try:
            base_path = ensure_repo_subpath(repo_root, Path(worktree_base))
        except ValueError:
            return False
        if not base_path.exists():
            return False
        try:
            has_children = next(base_path.iterdir(), None) is not None
        except OSError:
            return False
        if has_children:
            return False
    ...
    def _finalize_run(run_id: str) -> None:
        ...
        if cleanup_enabled:
            ...
            repo_root = Path(final_meta.get("repo_root") or find_repo_root(Path("."))).resolve()
            base_removed = _remove_worktree_base_dir(repo_root, final_meta.get("worktree_base"))
            ...
            cleanup_summary = {
                ...
                "worktree_base_removed": base_removed,
            }
            ...
            _timeline_append(
                run_id,
                "finalize.cleanup",
                removed=len(cleanup_summary["removed"]),
                kept=len(cleanup_summary["kept"]),
                base_removed=base_removed,
            )
            if cleanup_summary["errors"]:
                _timeline_append(run_id, "cleanup.error", errors=len(cleanup_summary["errors"]))
    ...
    def code_squad_run(
        ...
        try:
            ...
            current_interrupt_mode = str(run_meta.get("interrupt_mode") or "detach")
            requested_interrupt_mode = opts.get("interrupt_mode")
            desired_interrupt_mode = _coerce_interrupt_mode(requested_interrupt_mode, default=current_interrupt_mode)
            if desired_interrupt_mode != current_interrupt_mode or "interrupt" not in run_meta:
                run_meta["interrupt_mode"] = desired_interrupt_mode
                run_meta["interrupt"] = {"mode": desired_interrupt_mode, "updated_at": utc_now_iso()}
            ...
            selected_mode = str(run_meta.get("execution_mode") or "")
            shared_worktree_path: Path | None = None
            mode_meta: dict[str, Any] | None = None
            if not selected_mode:
                selected_mode, mode_meta = _select_execution_mode(
                    repo_root=repo_root,
                    tasks=[spec["raw"] for spec in task_specs],
                    opts=opts,
                    effective_config=effective_config,
                )
                decision_meta = {
                    "mode": selected_mode,
                    "explicit": bool(mode_meta.get("explicit")) if mode_meta else False,
                    "reason": mode_meta.get("reason") if mode_meta else None,
                    "analysis": mode_meta.get("analysis") if mode_meta else None,
                    "timestamp": utc_now_iso(),
                }
                run_meta["execution_decision"] = decision_meta
                _timeline_append(
                    resolved_run_id,
                    "execution.mode",
                    mode=selected_mode,
                    reason=mode_meta.get("reason") if mode_meta else None,
                    explicit=bool(mode_meta.get("explicit")) if mode_meta else False,
                    multi_project=bool((mode_meta or {}).get("analysis", {}).get("multi_project")) if mode_meta else None,
                )
            elif "execution_decision" not in run_meta:
                run_meta["execution_decision"] = {"mode": selected_mode, "explicit": True, "timestamp": utc_now_iso()}
            if selected_mode not in EXECUTION_MODES:
                selected_mode = "fanout"
            if isinstance(run_meta.get("execution_decision"), dict):
                run_meta["execution_decision"]["mode"] = selected_mode
            if selected_mode == "batch-worktree":
                shared_raw = run_meta.get("shared_worktree")
                if shared_raw:
                    try:
                        shared_path = ensure_repo_subpath(repo_root, Path(shared_raw))
                    except ValueError:
                        shared_path = ensure_repo_subpath(repo_root, worktree_base / "_batch")
                else:
                    shared_path = ensure_repo_subpath(repo_root, worktree_base / "_batch")
                ensure_dir(shared_path.parent)
                shared_worktree_path = shared_path
                run_meta["shared_worktree"] = str(shared_path)
            else:
                run_meta["shared_worktree"] = None
            run_meta["execution_mode"] = selected_mode
            auto_context_pack = _resolve_auto_context_pack(opts, effective_config, execution_mode=selected_mode)

            shared_cfg = _shared_kb_settings(opts, execution_mode=selected_mode, task_count=len(task_specs))
            if shared_cfg is not None:
                session_id = str(shared_cfg.get("session_id") or "default")
                if not bool(shared_cfg.get("enabled")):
                    run_meta["shared_kb"] = {"enabled": False, "session_id": session_id}
                    _timeline_append(resolved_run_id, "shared_kb.disabled")
                else:
                    head_sha = _git_head_sha(repo_root)
                    kb = shared_kb.load_or_init(dispatch_dir=Path(config.dispatch_dir), session_id=session_id, repo_root=repo_root)
                    state = kb.maybe_rebuild_on_head_change(head_sha)
                    kb.append_run_start(
                        run_id=resolved_run_id,
                        execution_mode=selected_mode,
                        guardrails=run_meta.get("guardrails") if isinstance(run_meta.get("guardrails"), dict) else {},
                        defaults=run_meta.get("defaults") if isinstance(run_meta.get("defaults"), dict) else {},
                    )
                    run_meta["shared_kb"] = {"enabled": True, "session_id": session_id, "epoch": kb.epoch, "head_sha": kb.head_sha}
                    _timeline_append(
                        resolved_run_id,
                        "shared_kb.rebuild" if state == "rebuild" else "shared_kb.hit",
                        epoch=kb.epoch,
                        head_sha=kb.head_sha,
                    )
            ...
            for spec in task_specs:
                ...
                elif selected_mode == "batch-worktree" and shared_worktree_path is not None:
    ...
    def code_squad_execute(
        ...
        opts = _coerce_options(options)
        debug = _debug_enabled(opts)
        max_tasks_out = _coerce_max_tasks_out(opts)
        ...
        try:
            ...
            for task in run_meta.get("tasks", []):
                ...
                entry: dict[str, Any] = {
                    ...
                    "slug": task.get("slug"),
                    "state": state,
                    "message": truncate_text(message, 320),
                }
                if debug and include_handles:
                    entry["job_id"] = task.get("job_id")
                tasks_out.append(entry)
            tasks_total = len(tasks_out)
            tasks_more = max(0, tasks_total - max_tasks_out)
            tasks_display = tasks_out[:max_tasks_out] if tasks_more else tasks_out
    ...
    @mcp.tool(name="code_squad_supervise")
    def code_squad_supervise(
        cwd: str,
        query: str,
        allow_globs: list[str] | None = None,
        deny_globs: list[str] | None = None,
        ...
        debug = _debug_enabled(opts)
        elapsed_ms: int | None = None
        ops_log("code_squad_supervise", "start", trace_id, cwd=cwd)
        try:
            cleaned_query = str(query or "").strip()
            if not cleaned_query:
                return {"error": "query required"}
            plan_tasks, decision_reason = build_supervisor_plan(cleaned_query, allow_globs, deny_globs, options=opts)
            if not plan_tasks:
                return {"error": "no tasks planned"}

            requested_run_id = opts.get("run_id")
            if requested_run_id:
                resolved_run_id = slugify(requested_run_id, fallback="run")
            else:
                resolved_run_id = _supervisor_plan_run_id(opts.get("run_id_prefix"))

            execute_opts = dict(opts)
            execute_opts.pop("plan", None)
            default_context = dict(DEFAULT_SUPERVISOR_CONTEXT)
            default_context["hints"] = list(DEFAULT_SUPERVISOR_CONTEXT.get("hints", []))
            execute_opts.setdefault("auto_context_pack", default_context)

            result = code_squad_execute(cwd, plan_tasks, run_id=resolved_run_id, options=execute_opts)
            final_run_id = result.get("run_id")

            if not final_run_id:
                result["decision_reason"] = decision_reason
                return result

            _timeline_append(final_run_id, "supervisor.start", query=cleaned_query, tasks=len(plan_tasks))
            _timeline_append(final_run_id, "supervisor.plan", tasks=len(plan_tasks), reason=decision_reason)
            _timeline_append(final_run_id, "supervisor.execute", next_action=result.get("next_action"))

            if result.get("error"):
                _timeline_append(final_run_id, "supervisor.done", status="error", error=result.get("error"))
                return result

            progress = result.get("progress") or {}
            summary_lines = supervisor_summary_lines(
            ...
            elapsed_ms = int((time.monotonic() - start) * 1000)
            if debug:
                response["debug"] = {"trace_id": trace_id, "elapsed_ms": elapsed_ms, "plan_tasks": plan_tasks}
            _timeline_append(final_run_id, "supervisor.done", next_action=result.get("next_action"))
            return response
        finally:
            if elapsed_ms is None:
                elapsed_ms = int((time.monotonic() - start) * 1000)
            ops_log("code_squad_supervise", "end", trace_id, cwd=cwd, elapsed_ms=elapsed_ms)
    ...
    def code_squad_status(run_id: str, include_last_message: bool = True, options: Any | None = None) -> dict[str, Any]:
        ...
        opts = _coerce_options(options)
        verbosity = _status_verbosity(opts)
        ...
        try:
            ...
            tasks_list = run_meta.get("tasks", [])
            task_map = {str(t.get("slug") or ""): t for t in tasks_list if t.get("slug")}

            def _blocked_dep_text(task_obj: dict[str, Any]) -> str | None:
                details = _dependency_block_details(task_obj, task_map)
                if not details:
                    return None
                joined = ", ".join(f"{item['slug']} ({item['state']})" for item in details)
                return truncate_text(joined, 240) if joined else None

            def _waiting_dep_text(task_obj: dict[str, Any]) -> str | None:
                waiting = _dependency_waiting_slugs(task_obj, task_map)
                if not waiting:
                    return None
                return truncate_text(", ".join(waiting), 240)

            def _task_message(task_obj: dict[str, Any]) -> str:
                state_val = str(task_obj.get("state") or "unknown")
                state = state_val.lower()
                if state == "completed":
                    return "Completed (patch ready)" if task_obj.get("has_patch") else "Completed"
                if state in ("running", "starting"):
                    msg = "Running"
                    if include_last_message:
                        detail = _clean_status_line(task_obj.get("last_message"))
                        if detail:
                            msg = f"{msg}: {detail}"
                    return msg
                if state == "queued":
                    dep_status = _dependency_status(task_obj, task_map)
                    if dep_status == "waiting":
                        waiting = _waiting_dep_text(task_obj)
                        if waiting:
                            return f"Queued (waiting on: {waiting})"
                    if dep_status == "blocked":
                        blocked = _blocked_dep_text(task_obj)
                        if blocked:
                            return f"Blocked (deps: {blocked})"
                        return "Blocked"
                    return "Queued"
                if state == "blocked":
                    blocked = _blocked_dep_text(task_obj)
                    ...
                    error_detail = _clean_status_line(task_obj.get("error"))
                    return f"Blocked: {error_detail}" if error_detail else "Blocked"
                if state in ("failed", "cancelled", "timeout"):
                    prefix = state.capitalize()
                    error_detail = _clean_status_line(task_obj.get("error"))
                    if not error_detail and include_last_message:
                        error_detail = _clean_status_line(task_obj.get("last_message"))
                    return f"{prefix}: {error_detail}" if error_detail else prefix
                return state_val or "unknown"

            tasks_out: list[dict[str, Any]] = []
            for task in tasks_list:
                state = str(task.get("state") or "unknown")
                message = truncate_text(_task_message(task), 320)
                entry: dict[str, Any] = {
                    "name": task.get("name"),
                    "slug": task.get("slug"),
                    "state": state,
                    "message": message,
                    "has_patch": bool(task.get("has_patch")),
                }
                if include_handles:
                    entry["job_id"] = task.get("job_id")
                tasks_out.append(entry)
            ...
            progress_view = {"done": int(progress.get("terminal", 0)), "total": int(progress.get("total", 0))}
            next_action = "wait" if progress.get("active", 0) else "collect/report"
            timeline_events_full = _timeline_all(run_id)
            phases, current_phase_text, recent_summary = _phase_summary_from_meta(run_meta, timeline_events_full)
            next_step_text = "继续等待" if next_action == "wait" else next_action
            interrupt_meta = run_meta.get("interrupt") if isinstance(run_meta.get("interrupt"), dict) else {}
            interrupt_mode = str(interrupt_meta.get("mode") or run_meta.get("interrupt_mode") or "detach")
            summary_lines: list[str] = [
                f"进度: {progress_view['done']}/{progress_view['total']} | 正在: {current_phase_text} | 动作: {next_action} | Interrupt: {interrupt_mode}",
                f"最近进展: {recent_summary or '暂无'}；下一步: {next_step_text}",
            ]
            preflight_info = run_meta.get("preflight") if isinstance(run_meta.get("preflight"), dict) else {}
            if preflight_info.get("needs_user_confirm"):
                summary_lines.append("需要确认: options.preflight_allow_many_tasks=true")
    ...
    def code_squad_events(
        ...
        try:
            ...
            if not debug:
                timeline_events_full = _timeline_all(run_id)
                phases, current_phase_text, recent_summary = _phase_summary_from_meta(run_meta, timeline_events_full)
                line = f"{current_phase_text} | 最近: {recent_summary or '暂无'}"
                elapsed_ms = int((time.monotonic() - start) * 1000)
                return {"ok": True, "phases": phases, "lines": [line]}
            dispatch = _dispatch_for_run(run_id)
    ...
    @mcp.tool(name="code_squad_interrupt")
    def code_squad_interrupt(run_id: str, mode: str | None = None, options: Any | None = None) -> dict[str, Any]:
        trace_id = uuid.uuid4().hex
        start = time.monotonic()
        run_meta = _load_run(run_id)
        log_cwd = run_meta.get("repo_root") if run_meta else None
        opts = _coerce_options(options)
        elapsed_ms: int | None = None
        ops_log("code_squad_interrupt", "start", trace_id, cwd=log_cwd)
        try:
            if not run_meta:
                return {"error": "run not found", "run_id": run_id}
            requested = mode or opts.get("mode") or opts.get("interrupt_mode")
            current_mode = str(run_meta.get("interrupt_mode") or "detach")
            resolved_mode = _coerce_interrupt_mode(requested, default=current_mode)
            if resolved_mode != current_mode or "interrupt" not in run_meta:
                run_meta["interrupt_mode"] = resolved_mode
                run_meta["interrupt"] = {"mode": resolved_mode, "updated_at": utc_now_iso()}
                _save_run(run_id, run_meta)
            ...
            next_action = "wait" if progress.get("active", 0) else "collect/report"
            summary_lines: list[str] = [
                f"Mode: {resolved_mode}; progress {progress.get('terminal', 0)}/{progress.get('total', 0)}",
                f"Next action: {next_action}",
            ]

            cancelled: int | None = None
            if resolved_mode == "cancel":
                cancel_res = code_squad_cancel(run_id)
                cancelled = int(cancel_res.get("cancelled") or 0)
                _timeline_append(run_id, "interrupt.cancel_requested", cancelled=cancelled)
                next_action = "collect/report"
                run_meta = _load_run(run_id) or run_meta
                progress = _compute_progress(run_meta)
                counts = dict(progress.get("counts") or {})
                summary_lines.append(f"Cancelled tasks: {cancelled}")
            ...
            if elapsed_ms is None:
                elapsed_ms = int((time.monotonic() - start) * 1000)
            ops_log("code_squad_interrupt", "end", trace_id, cwd=log_cwd, elapsed_ms=elapsed_ms)
    ...
    def code_squad_cleanup(run_id: str, delete_run_artifacts: bool = False, options: Any | None = None) -> dict[str, Any]:
        ...
        try:
            if not run_meta:
                return {"error": "run not found", "run_id": run_id}
            repo_root = Path(run_meta.get("repo_root") or find_repo_root(Path("."))).resolve()
            worktree_base_path = ensure_repo_subpath(repo_root, Path(run_meta.get("worktree_base") or repo_root / ".codex" / "code-squad" / "worktrees" / run_id))
            cleanup = _cleanup_worktrees_for_run(run_meta, keep_failed=False)
            removed_entries = cleanup.get("removed", [])
            cleanup_errors = cleanup.get("errors", [])
            removed_worktrees = len(removed_entries)
            removed_paths: list[str] = [str(entry.get("path")) for entry in removed_entries if debug and entry.get("path")]
            base_removed = _remove_worktree_base_dir(repo_root, str(worktree_base_path))
            _prune_git_worktrees(repo_root)
            if debug and base_removed:
                removed_paths.append(str(worktree_base_path))
            ...
            cleanup_summary = {
                ...
                "worktree_base_removed": base_removed,
                "deleted_run_artifacts": delete_run_artifacts,
            }
            cleanup_path = ensure_dir(run_dir / "artifacts") / "cleanup.json"
            cleanup_path.write_text(json.dumps(cleanup_summary, ensure_ascii=False, indent=2), encoding="utf-8")
            if cleanup_errors:
                _timeline_append(run_id, "cleanup.error", errors=len(cleanup_errors))
            response: dict[str, Any] = {
                "run_id": run_id,
