...
from cmissh.domain import ACE, ACL, ChangeEntry, CmisId, CmisObject, ObjectType
...
class BrowserCmisObject:
    ...
    def getObjectParents(self, **kwargs):
        ...
        if not self.getAllowableActions()["canGetObjectParents"]:
            msg = "Object does not support getObjectParents"
        ...
        result = self._cmisClient.binding.get(url, self._cmisClient.session, **params)
    ...
    def updateProperties(self, properties):
        ...
        url = self._repository.getRootFolderUrl()
        props = {"objectId": self.id, "cmisaction": "update"}
    ...
    def createRelationship(self, targetObj, relTypeId):
        ...
        if not self.allowableActions["canCreateRelationship"]:
            msg = "Not allowed to create a relationship"
    ...
    def applyACL(self, acl):
        ...
        if self._repository.getCapabilities()["ACL"] == "manage":
            ...
            props = {"objectId": self.id, "cmisaction": "applyACL"}
...
class BrowserRepository:
    ...
    def getObjectByPath(self, path, **kwargs):
        ...
        result = self._cmisClient.binding.get(url, self._cmisClient.session, **params)
        return getSpecializedObject(
            BrowserCmisObject(self._cmisClient, self, data=result, **kwargs), **kwargs
        )
    ...
    def getTypeDefinition(self, typeId):
        ...
        """
        url = self.getRepositoryUrl()
        params = {"cmisselector": "typeDefinition", "typeId": typeId}
        result = self._cmisClient.binding.get(url, self._cmisClient.session, **params)
    ...
    def getObject(self, objectId, **kwargs):
        ...
        return getSpecializedObject(
            BrowserCmisObject(self._cmisClient, self, CmisId(objectId), **kwargs),
            **kwargs,
        )
    ...
    def query(self, statement, **kwargs):
        ...
        props = {"cmisaction": "query", "q": statement}
        data = encode_multipart_formdata(props, None, None)
        # do the POST
        result = self._cmisClient.binding.post(
    ...
    def getContentChanges(self, **kwargs):
        ...
        return BrowserResultSet(
            self._cmisClient, self, data=result, serializer=ChangeEntrySerializer()
        )
    ...
    def createDocumentFromString(
        ...
        return parentFolder.createDocument(
            name, properties, contentString, contentType, contentEncoding
        )
    ...
    def createDocumentFromSource(self, sourceId, properties=None, parentFolder=None):
        """
        This is not yet implemented.
...
class BrowserDocument(BrowserCmisObject):
    ...
    def setContentStream(self, contentFile, contentType=None):
        ...
        # get the root folder URL
        url = self._repository.getRootFolderUrl()
        params = {"objectId": self.id, "cmisaction": "setContent"}
    ...
    def deleteContentStream(self):
        ...
        if not self.allowableActions["canDeleteContentStream"]:
            msg = "Not allowed to delete the content stream"
            raise CmisException(msg)
...
class BrowserFolder(BrowserCmisObject):
    ...
    def getChildren(self, **kwargs):
        ...
        # return the result set
        return BrowserResultSet(
            self._cmisClient, self._repository, result, serializer=ChildrenSerializer()
        )
    ...
    def getDescendants(self, **kwargs):
        ...
        # return the result set
        return BrowserResultSet(
            self._cmisClient, self._repository, result, serializer=TreeSerializer()
        )
    ...
    def getObjectId(self):
        ...
        if self._objectId is None:
            if "cmis:objectId" in self._data.get("properties").keys():
                self._objectId = (
                    self._data.get("properties").get("cmis:objectId").get("value")
                )
        return self._objectId
    ...
    def getResults(self):
        ...
        if self._data:
            ...
            for entry in self._data:
                changeEntry = BrowserChangeEntry(
                    self._cmisClient, self._repository, entry
                )
...
def getSpecializedObject(obj, **kwargs):
    ...
    if "cmis:baseTypeId" in obj.getProperties():
        ...
        if baseType == "cmis:folder":
            return BrowserFolder(
                obj._cmisClient, obj._repository, obj.getObjectId(), obj.data, **kwargs
            )
...
def encode_multipart_formdata(fields, contentFile, contentType):
    ...
    if contentFile:
        _fields["content"] = (
            file_name or "",
            contentFile,
            contentType or "application/binary",
        )
    m = MultipartEncoder(fields=_fields)
    return m
...
class FolderSerializer:
    ...
    def fromJSON(self, client, repo, jsonString):
        ...
        folder = BrowserFolder(
            client, repo, objectId, properties=obj["succinctProperties"]
        )
        return folder
...
class ChangeEntrySerializer:
    ...
    def fromJSON(self, client, repo, jsonObj):
        """Transforms from JSON to the object."""
        logger = logging.getLogger("cmislib.browser.binding.ChangeEntrySerializer")
        entries = []
        for obj in jsonObj["objects"]:
