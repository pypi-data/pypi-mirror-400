...
def add_overlay(
    collection: le.ImageCollection,
    overlay_data: Union[str, le.Geometry, le.FeatureCollection],
    color: str = "black",
    width: int = 1,
    opacity: float = 1.0,
    region: Union[le.Geometry, le.FeatureCollection] = None,
) -> le.ImageCollection:
    ...
    if not isinstance(overlay_data, le.FeatureCollection):
        if isinstance(overlay_data, str):
            try:
                if overlay_data.lower() in public_assets:
                    overlay_data = le.FeatureCollection(
                    ...
                    overlay_data = le.FeatureCollection(overlay_data)
                ...
                raise Exception(e)
        elif isinstance(overlay_data, le.Feature):
            overlay_data = le.FeatureCollection([overlay_data])
        elif isinstance(overlay_data, le.Geometry):
            overlay_data = le.FeatureCollection([le.Feature(overlay_data)])
        else:
            raise Exception(
...
def create_timeseries(
    ...
    if not isinstance(collection, le.ImageCollection):
        if isinstance(collection, str):
            collection = le.ImageCollection(collection)
        else:
            raise Exception(
    ...
    def create_image(date):
        ...
        if region is None:
            ...
            image = le.Image(
                le.Algorithms.If(
                    le.Algorithms.ObjectType(region).equals("FeatureCollection"),
        ...
        return image.set(
            {
                "system:time_start": le.Date(date).millis(),
                "system:date": le.Date(date).format(date_format),
                "empty": sub_col.limit(1).size().eq(0),
            }
        ).rename(bands)

    try:
        images = le.ImageCollection(dates.map(create_image))
        if drop_empty:
...
def create_timelapse(
    ...
    """
    import geemap_le.colormaps as cm
    ...
    col = col.map(
        lambda img: img.rename(
            img.bandNames().map(lambda name: le.String(name).replace(f"_{reducer}", ""))
        )
    )
    ...
    try:
        ...
        def get_annual_NAIP(year):
            try:
                ...
                if roi is not None:
                    if isinstance(roi, le.Geometry):
                        image = le.Image(le.ImageCollection(naip).mosaic().clip(roi))
                    elif isinstance(roi, le.FeatureCollection):
                        image = le.Image(
                            le.ImageCollection(naip).mosaic().clipToCollection(roi)
                        )
                else:
                    image = le.Image(le.ImageCollection(naip).mosaic())
                return image.set(
                    {
                        "system:time_start": le.Date(start_date).millis(),
                        "system:time_end": le.Date(end_date).millis(),
        ...
        years = le.List.sequence(start_year, end_year, step)
        collection = le.ImageCollection(years.map(get_annual_NAIP))
...
def sentinel1_filtering(
    ...
    **kwargs,
    ...
    col = collection.filter(le.Filter.eq("instrumentMode", instrumentMode)).filter(
        le.Filter.listContains(
    ...
    for k, v in kwargs.items():
        col = col.filter(le.Filter.eq(k, v))
    if orbitProperties_pass:
        col = col.filter(le.Filter.eq("orbitProperties_pass", orbitProperties_pass))
...
def sentinel1_timeseries(
    ...
    **kwargs,
    ...
    col = le.ImageCollection("COPERNICUS/S1_GRD").filterBounds(roi)
    ...
    def transform(date):  # coll, frequency
        start = date
        end = le.Date(date).advance(n, frequency).advance(-1, "day")
        return (
            col.filterDate(start, end)
            .mean()
            .set(
                {
                    "system:time_start": le.Date(start).millis(),
                    "system:time_end": le.Date(end).millis(),
                    "system:date": start,
                }
            )
        )
    ...
    imgColl = le.ImageCollection.fromImages(imgList)
    if clip:
...
def sentinel2_timeseries(
    ...
    doy_start = le.Number.parse(le.Date(start).format("D"))
    doy_end = le.Number.parse(le.Date(end).format("D"))
    collection = (
        le.ImageCollection("COPERNICUS/S2_HARMONIZED")
        .filterDate(start, end)
        .filter(le.Filter.calendarRange(doy_start, doy_end, "day_of_year"))
        .filter(le.Filter.lt("CLOUDY_PIXEL_PERCENTAGE", cloud_pct))
        .filterBounds(roi)
    )
...
def sentinel2_timeseries_legacy(
    ...
    MSILCcol = le.ImageCollection("COPERNICUS/S2")
    MSI2Acol = le.ImageCollection("COPERNICUS/S2_SR")
    ...
    def prepMSI(img):
        ...
        if apply_fmask:
            img = fmask(img)
        return le.Image(img.copyProperties(orig, orig.propertyNames())).resample(
            "bicubic"
        )

    # Get annual median collection.
    def getAnnualComp(y):
        startDate = le.Date.fromYMD(
            le.Number(y), le.Number(start_month), le.Number(start_day)
        )
        endDate = startDate.advance(le.Number(n_days), "day")
        ...
        yearImg = le.Image(le.Algorithms.If(nBands, yearImg, dummyImg))
        return calcNbr(yearImg).set(
            {
                "year": y,
                "system:time_start": startDate.millis(),
                "nBands": nBands,
                "system:date": le.Date(startDate).format(date_format),
            }
        )

    # Get quarterly median collection.
    def getQuarterlyComp(startDate):
        startDate = le.Date(startDate)
    ...
    fillerValues = le.List.repeat(0, bandNames.size())
    dummyImg = le.Image.constant(fillerValues).rename(bandNames).selfMask().int16()
    ...
    imgCol = le.ImageCollection.fromImages(imgList)
...
def landsat_timeseries(
    ...
    LC09col = le.ImageCollection("LANDSAT/LC09/C02/T1_L2")
    LC08col = le.ImageCollection("LANDSAT/LC08/C02/T1_L2")
    LE07col = le.ImageCollection("LANDSAT/LE07/C02/T1_L2")
    LT05col = le.ImageCollection("LANDSAT/LT05/C02/T1_L2")
    LT04col = le.ImageCollection("LANDSAT/LT04/C02/T1_L2")
    ...
    def getMonthlyComp(startDate):
        ...
        monthImg = le.Image(le.Algorithms.If(nBands, monthImg, dummyImg))
        return monthImg.set(
            {
                "system:time_start": startDate.millis(),
                "nBands": nBands,
    ...
    def getQuarterlyComp(startDate):
        ...
        quarter = le.Image(le.Algorithms.If(nBands, quarter, dummyImg))
        return quarter.set(
            {
                "system:time_start": startDate.millis(),
                "nBands": nBands,
    ...
    # Make a dummy image for missing years.
    bandNames = le.List(["Blue", "Green", "Red", "NIR", "SWIR1", "SWIR2", "pixel_qa"])
    fillerValues = le.List.repeat(0, bandNames.size())
    dummyImg = le.Image.constant(fillerValues).rename(bandNames).selfMask().int16()
...
def landsat_timeseries_legacy(
    ...
    # Landsat collection preprocessingEnabled
    # Get Landsat surface reflectance collections for OLI, ETM+ and TM sensors.
    LC08col = le.ImageCollection("LANDSAT/LC08/C01/T1_SR")
    LE07col = le.ImageCollection("LANDSAT/LE07/C01/T1_SR")
    LT05col = le.ImageCollection("LANDSAT/LT05/C01/T1_SR")
    LT04col = le.ImageCollection("LANDSAT/LT04/C01/T1_SR")
...
def modis_timeseries(
    ...
    try:
        ...
        collection = le.ImageCollection(asset_id)
        ...
        def monthly_modis(start_d):
            end_d = le.Date(start_d).advance(1, "month")
            return le.Image(collection.filterDate(start_d, end_d).mean())

        images = le.ImageCollection(seq.map(monthly_modis))
        return images
...
def landsat_timelapse(
    ...
    elif isinstance(roi, le.Feature) or isinstance(roi, le.FeatureCollection):
        roi = roi.geometry()
    elif isinstance(roi, le.Geometry):
        pass
    else:
...
def sentinel2_timelapse(
    ...
    **kwargs,
    ...
    Args:
        ...
        kwargs (optional): Additional arguments to pass the le_map_tools.create_timeseries() function.
...
def landsat_ts_norm_diff_gif(
    ...
    """
    coordinates = le.Image(collection.first()).get("coordinates")
    roi = le.Geometry.Polygon(coordinates, None, False)
...
def goes_timeseries(
    ...
    col = le.ImageCollection(f"NOAA/GOES/{data[-2:]}/{scan_types[scan.lower()]}")

    if region is None:
        region = le.Geometry.Polygon(
            [
                [
    ...
    def applyScaleAndOffset(img):
        def getFactorImg(factorNames):
            ...
            return le.Image.constant(factorList)
    ...
    def showNighta(img):
        ...
        IR_n = img.select("CMI_C13").unitScale(le.Number(90), le.Number(313))
...
def goes_fire_timeseries(
    ...
    if region is None:
        region = le.Geometry.BBox(-123.17, 36.56, -118.22, 40.03)
    ...
    if not merge:
        ...
        joinFilter = le.Filter.equals(
            **{"leftField": "system:time_start", "rightField": "system:time_start"}
        )
        joinedCol = le.Join.saveFirst("match").apply(geosVisCol, fdcVisCol, joinFilter)

        def overlayVis(img):
            cmi = le.Image(img).visualize(
                **{
...
def goes_fire_timelapse(
    ...
    **kwargs,
    ...
    try:
        ...
        if roi is None:
            roi = le.Geometry.BBox(-123.17, 36.56, -118.22, 40.03)
...
def modis_ndvi_doy_ts(
    ...
    if data == "Terra":
        col = le.ImageCollection("MODIS/006/MOD13A2").select(band)
    else:
        col = le.ImageCollection("MODIS/006/MYD13A2").select(band)

    if (start_date is not None) and (end_date is not None):
        col = col.filterDate(start_date, end_date)

    def set_doy(img):
        doy = le.Date(img.get("system:time_start")).getRelative("day", "year")
    ...
    filter = le.Filter.equals(**{"leftField": "doy", "rightField": "doy"})

    # Define a join.
    join = le.Join.saveAll("doy_matches")

    # Apply the join and convert the resulting FeatureCollection to an
    # ImageCollection.
    joinCol = le.ImageCollection(join.apply(distinctDOY, col, filter))

    # Apply median reduction among matching DOY collections.

    def match_doy(img):
        doyCol = le.ImageCollection.fromImages(img.get("doy_matches"))
        return doyCol.reduce(le.Reducer.median())
...
def modis_ocean_color_timeseries(
    ...
    col = le.ImageCollection(f"NASA/OCEANDATA/MODIS-{satellite}/L3SMI").filterDate(
        start_date, end_date
    )
...
def modis_ocean_color_timelapse(
    ...
    if roi is None:
        roi = le.Geometry.BBox(-99.755133, 18.316722, -79.761194, 31.206929)
...
def dynamic_world_timeseries(
    ...
    if (
        isinstance(region, le.FeatureCollection)
        or isinstance(region, le.Feature)
        or isinstance(region, le.Geometry)
    ):
        pass
    else:
        raise ValueError(
    ...
    s2 = (
        ...
        .filterBounds(region)
        .filter(le.Filter.lte("CLOUDY_PIXEL_PERCENTAGE", cloud_pct))
    )

    ids = s2.aggregate_array("system:index")

    dw = le.ImageCollection("GOOGLE/DYNAMICWORLD/V1").filter(
        le.Filter.inList("system:index", ids)
    )
    ...
    if return_type == "class":
        ...
        prob_images = le.ImageCollection(
            prob_col.map(
                lambda img: img.reduce(le.Reducer.max()).set(
                    "system:time_start", img.get("system:time_start")
                )
            )
        )
        ...
        if return_type == "probability":
            ...
            nums = le.List.sequence(0, count.subtract(1))

            def create_hillshade(d):
                proj = le.Projection("EPSG:3857").atScale(10)
                img = le.Image(images.toList(images.size()).get(d))
                prob_img = le.Image(prob_images.toList(prob_images.size()).get(d))
            ...
            result = le.ImageCollection(nums.map(create_hillshade))
            return result
...
def sentinel1_timelapse(
    ...
    **kwargs,
    ...
    collection = (
        le.ImageCollection("COPERNICUS/S1_GRD").filterDate(start, end).filterBounds(roi)
    )
