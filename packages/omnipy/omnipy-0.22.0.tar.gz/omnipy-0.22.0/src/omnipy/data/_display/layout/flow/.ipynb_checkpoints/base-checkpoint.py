...
def optimize_layout_to_fit_frame(
    ...
    #     context = _set_inner_panel_heights_2(context)
...
def _set_inner_panel_heights_2(context: LayoutFlowContext[FrameT]) -> LayoutFlowContext[FrameT]:
    """
    Adjust heights of panels in a layout based on the frame's constraints.
    ...
    return _resize_inner_panel_heights_2(
        context,
        lambda _panel: per_inner_panel_height,
    )
...
def _calc_inner_panel_height_2(context: LayoutFlowContext[FrameT]) -> int:
    assert has_height(context.resized_panel.inner_frame.dims)

    inner_frame_height = context.resized_panel.inner_frame.dims.height
    layout_dims_height = context.resized_panel.dims.height

    return inner_frame_height - (layout_dims_height - inner_frame_height)
...
def _resize_inner_panel_heights_2(
    ...
    for key, panel in context.dim_aware_layout.items():
        ...
        if not should_use_panel_original_height:
            ...
            if new_frame != panel.frame:
                ...
                # print(f'Setting panel frame height: {key}: {panel.frame} -> {new_frame}')
...
def _tighten_panel_frame_widths(context: LayoutFlowContext[FrameT]) -> LayoutFlowContext[FrameT]:
    ...
    for key, panel in context.dim_aware_layout.items():
        ...
        if new_frame != panel.frame:
            ...
            # print(f'Tightening panel frame width: {key}: {panel.frame} -> {new_frame}')
            # # context.dim_aware_layout[key] = create_panel_with_updated_frame(
            # #     context.draft_layout[key],
            # #     new_frame,
            # )
...
def _reduce_panel_heights(context: LayoutFlowContext[FrameT]) -> LayoutFlowContext[FrameT]:
    context = _resize_inner_panel_heights_2(
        context,
