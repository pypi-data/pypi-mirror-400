...
- `rm -r` on `src/`, `tests/`, `docs/`, `skills/`, or any code directories
- Any `rm` command with wildcards (`*`) in project directories
- `del /s /q` or `rmdir /s /q` on project folders (Windows)

**WHY:** A previous session accidentally deleted the entire codebase with an `rm` command. This project was lost and had to be restored from GitHub.

**SAFE ALTERNATIVES:**
- To remove a single file: `rm specific-file.txt` (explicit filename only)
- To clean build artifacts: Use the project's clean script if available
- To reset: `git checkout .` or `git clean -fd` (recoverable via git)
- When in doubt: ASK THE USER before any deletion

**If you need to delete something, ALWAYS:**
1. List exactly what will be deleted first
2. Ask for explicit user confirmation
3. Never use recursive delete on directories containing source code
...
## 🧠 Mind v5 Specialist Skills

### Skills Location

**All Mind v5 skills are located in this project:**

```
C:\Users\USER\Desktop\vibeship-mind\skills\
```
...
| Skill | Path | Focus |
|-------|------|-------|
| event-architect | `skills/event-architect/skill.yaml` | Event sourcing, NATS, CQRS |
| graph-engineer | `skills/graph-engineer/skill.yaml` | FalkorDB, causal graphs |
| vector-specialist | `skills/vector-specialist/skill.yaml` | Qdrant, pgvector, embeddings, RRF fusion |
| temporal-craftsman | `skills/temporal-craftsman/skill.yaml` | Temporal.io workflow orchestration |
| ml-memory | `skills/ml-memory/skill.yaml` | Hierarchical memory systems |
| causal-scientist | `skills/causal-scientist/skill.yaml` | DoWhy, counterfactuals |
| privacy-guardian | `skills/privacy-guardian/skill.yaml` | Differential privacy, encryption |
| performance-hunter | `skills/performance-hunter/skill.yaml` | Profiling, optimization |

### Extended Team Skills (12)

| Skill | Path | Focus |
|-------|------|-------|
| infra-architect | `skills/infra-architect/skill.yaml` | Kubernetes, Terraform, GitOps |
| postgres-wizard | `skills/postgres-wizard/skill.yaml` | PostgreSQL internals, pgvector |
| api-designer | `skills/api-designer/skill.yaml` | REST/gRPC design, versioning |
| test-architect | `skills/test-architect/skill.yaml` | Testing strategy, quality gates |
| observability-sre | `skills/observability-sre/skill.yaml` | Prometheus, tracing, alerting |
| data-engineer | `skills/data-engineer/skill.yaml` | Pipelines, ETL, data quality |
| code-reviewer | `skills/code-reviewer/skill.yaml` | Code quality, patterns |
| docs-engineer | `skills/docs-engineer/skill.yaml` | Technical writing, API docs |
| migration-specialist | `skills/migration-specialist/skill.yaml` | Zero-downtime migrations |
| chaos-engineer | `skills/chaos-engineer/skill.yaml` | Resilience testing |
| python-craftsman | `skills/python-craftsman/skill.yaml` | Python idioms, async, types |
| sdk-builder | `skills/sdk-builder/skill.yaml` | Client libraries, DX |

### Bonus Skills

| Skill | Path | Focus |
|-------|------|-------|
| llm-architect | `skills/llm-architect/skill.yaml` | LLM integration patterns |
| redis-specialist | `skills/redis-specialist/skill.yaml` | Redis caching, pub/sub |
| realtime-engineer | `skills/realtime-engineer/skill.yaml` | WebSockets, SSE |
| mcp-developer | `skills/mcp-developer/skill.yaml` | MCP server development |

### How to Use Skills

**1. Read a skill before working in its domain:**
```
Read: C:\Users\USER\Desktop\vibeship-mind\skills\{skill-name}\skill.yaml
```

**2. Each skill folder contains 4 files:**
- `skill.yaml` - Identity, patterns, anti-patterns, handoffs
- `sharp-edges.yaml` - Common gotchas and pitfalls
- `validations.yaml` - Code validation rules
- `collaboration.yaml` - Handoff protocols

**3. Load skills for specific work:**

| Working On | Load These Skills |
|------------|-------------------|
| Memory retrieval | vector-specialist, ml-memory |
| Causal graphs | graph-engineer, causal-scientist |
| Event handling | event-architect, temporal-craftsman |
| Database work | postgres-wizard, data-engineer |
| API endpoints | api-designer, python-craftsman |
| Testing | test-architect, chaos-engineer |
...
When working in one domain and hitting another specialist's territory:

1. **Recognize the boundary** - Check if task matches another skill's triggers
2. **Load the specialist skill** - Read their skill.yaml
3. **Pass context** - What was built, current goal, constraints
4. **Follow their patterns** - Use their established approaches

Example handoffs:
- `vector-specialist` → `graph-engineer` when dealing with knowledge graphs
- `event-architect` → `temporal-craftsman` for durable workflows
- `api-designer` → `privacy-guardian` for auth/security concerns
- `performance-hunter` → `postgres-wizard` for database optimization
...
type skills\vector-specialist\skill.yaml

# Check sharp edges before implementation
type skills\{skill-name}\sharp-edges.yaml
```

### Spawner Integration

Skills are also available via Spawner MCP for 13 of 20 skills:
- `spawner_skills({ action: "get", name: "event-architect" })`
- `spawner_skills({ action: "get", name: "privacy-guardian" })`
- etc.

For skills not in Spawner index, read directly from local paths above.
...
**Always load the relevant specialist skill before working in their domain.**
