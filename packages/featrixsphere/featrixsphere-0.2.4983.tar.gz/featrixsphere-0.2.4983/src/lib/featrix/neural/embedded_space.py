...
def check_abort_and_raise(job_id: str, output_dir: str = None, context: str = "training") -> None:
    """
    Check for ABORT file and raise FeatrixTrainingAbortedException if found.
    ...
    Raises:
        FeatrixTrainingAbortedException: If ABORT file is found
    """
    if not job_id:
        return
    ...
    if abort_file_path:
        raise FeatrixTrainingAbortedException(
            f"Training aborted due to ABORT file during {context} at {abort_file_path}",
            job_id=job_id,
...
def debug_log_gradient_flow(encoder, epoch_idx: int, batch_idx: int) -> None:
    """
...
class EmbeddingSpace(object):
    ...
    def find_latest_checkpoint(self, max_epoch: int = 10000) -> Optional[int]:
        """
        Find the latest valid checkpoint epoch.
        
        Searches backwards from max_epoch to find the most recent checkpoint
        that exists and can be loaded without errors.
        
        Args:
            max_epoch: Maximum epoch to search from (default 10000)
            
        Returns:
            The epoch number of the latest valid checkpoint, or None if no checkpoint found.
        """
        from pathlib import Path
        ...
        for epoch in range(max_epoch, -1, -1):
            ...
            if Path(checkpoint_path).exists():
                try:
                    ...
                    # Use map_location='cpu' to avoid GPU memory issues during probe
                    torch.load(checkpoint_path, weights_only=False, map_location='cpu')
                    logger.info(f"✅ Found valid checkpoint at epoch {epoch}: {checkpoint_path}")
                    return epoch
                except Exception as e:
                    logger.warning(f"⚠️  Checkpoint at epoch {epoch} is corrupted: {e}")
                    continue
    ...
    def _log_embedding_quality_summary(self):
        """
        ...
        try:
            ...
            for col in numeric_cols[:10]:  # Limit for speed
                ...
                is_marginal_prop = corr > 0.5
                has_sensitivity = mean_sensitivity > 0.0001
                has_consistent = consistency < 0.5
                has_variation = dist_range > 0.001
                
                quality_score = 0
                if is_proportional:
                    quality_score += 2
                elif is_marginal_prop:
                    quality_score += 1
                if has_sensitivity:
                    quality_score += 1
                if has_consistent:
            ...
            if numeric_results:
                ...
                logger.info("")
                logger.info("   Linearity = correlation between input change and embedding change (higher = proportional response)")
                logger.info("   Sensitivity = embedding distance per unit input change (higher = model responds to changes)")
                logger.info("   Consistency = stability of sensitivity across different perturbation sizes (lower = more stable)")
                logger.info("")
                
                max_col_len = max(len(col) for col in numeric_results.keys())
                col_width = max(22, max_col_len)
                
                # Column widths: Col(var) | Linearity(16) | Sensitivity(18) | Consistency(18) | Quality(15)
                logger.info(f"┌{'─' * (col_width + 2)}┬──────────────────┬────────────────────┬────────────────────┬───────────────┐")
                logger.info(f"│ {'Column':<{col_width}} │ Linearity        │ Sensitivity        │ Consistency        │ Quality       │")
                logger.info(f"├{'─' * (col_width + 2)}┼──────────────────┼────────────────────┼────────────────────┼───────────────┤")
                ...
                for col, res in sorted(numeric_results.items()):
                    ...
                    # Build each cell: value + emoji (emoji takes 2 display cols, so add 1 less space)
                    # Linearity cell: 16 display cols → 8.3f(8) + space(1) + emoji(2) + pad(5) = 16
                    lin_cell = f"{corr:>8.3f} {mark(prop_ok)}     "
                    # Sensitivity cell: 18 display cols → 10.4f(10) + space(1) + emoji(2) + pad(5) = 18
                    sens_cell = f"{sens:>10.4f} {mark(sens_ok)}     "
                    # Consistency cell: 18 display cols → 10.3f(10) + space(1) + emoji(2) + pad(5) = 18
                    cons_cell = f"{cons:>10.3f} {mark(cons_ok)}     "
                    
                    logger.info(f"│ {col:<{col_width}} │ {lin_cell}│ {sens_cell}│ {cons_cell}│ {qual_label(qual)}│")
            ...
            if categorical_results:
                ...
                for col, res in sorted(categorical_results.items()):
                    ...
                    # Build cells: 18 display cols each
                    val_cell = f"{mean_dist:>10.4f} {mark(sens_ok)}     "
                    spr_cell = f"{var:>10.4f} {mark(var_ok)}     "
    ...
    def detect_capacity_bottlenecks(self, epoch_idx: int, loss_dict: Optional[Dict] = None) -> Dict[str, any]:
        ...
        try:
            ...
            if hasattr(self.encoder, 'joint_encoder') and hasattr(self.encoder.joint_encoder, 'config'):
                ...
                rel_extractor = getattr(self.encoder.joint_encoder, 'relationship_extractor', None)
                if rel_extractor:
                    total_seq = 1 + n_cols + len(rel_extractor.all_pairs) * 6
                    metrics['total_sequence_length'] = total_seq
                    
                    # Rule of thumb: attention degrades when seq >> d_model
                    if total_seq > d_model * 4:
                        bottlenecks.append("SEQUENCE_TOO_LONG")
                        recommendations.append(
                            f"Sequence length ({total_seq}) is {total_seq/d_model:.1f}x d_model ({d_model}). "
                            f"Consider increasing d_model or reducing relationship pairs."
                        )
                    
                    # Few layers with long sequences
                    if n_layers < 3 and total_seq > 200:
                        bottlenecks.append("FEW_LAYERS")
                        recommendations.append(
                            f"Only {n_layers} transformer layers for sequence of {total_seq}. "
    ...
    def log_ablation_comparison(self, epoch_idx: int):
        ...
        This is EXPENSIVE - only run every 100 epochs or on demand.
        ...
        try:
            ...
            with torch.no_grad():
                ...
                for batch in val_loader:
                    ...
                    improvement = (marginal_without - marginal_with) / marginal_without * 100 if marginal_without > 0 else 0
                    
                    logger.info(f"")
                    logger.info(f"📊 Marginal Reconstruction Loss:")
                    logger.info(f"   WITH relationships:    {marginal_with:.6f}")
                    logger.info(f"   WITHOUT relationships: {marginal_without:.6f}")
        ...
        try:
            ...
            if hasattr(self, 'val_input_data') and self.val_input_data is not None:
                ...
                dataset = SuperSimpleSelfSupervisedDataset(
                    ...
                    col_codecs=self.col_codecs,
                    string_cache=getattr(self, 'string_cache', None),
                )
                return DataLoader(
                    dataset,
        ...
        return CurriculumLearningConfig(
            ...
            phases=[
                CurriculumPhaseConfig(
                    ...
                    marginal_weight=0.1,    # Increased from 0.02 to allow some marginal learning (marginal is 100× larger than spread, so 0.1 is still low)
                ...
                CurriculumPhaseConfig(
                    ...
                    start_progress=0.30,
                    end_progress=0.85,
                    spread_weight=1.0,      # Moderate spread to maintain geometry
    ...
    def _handle_cuda_oom_error(
        ...
        try:
            from lib.system_health_monitor import SystemHealthMonitor
            monitor = SystemHealthMonitor()
            workers = monitor.get_dataloader_workers()
            if workers:
    ...
    def _record_epoch_loss_history(
        ...
        get_lr, epoch_start_time_now: float
    ) -> None:
        """
        ...
        Args:
            ...
            get_lr: Function to get current learning rate
    ...
    def _get_lr_string(self, get_lr_fn) -> str:
        """Format learning rate as string."""
        try:
            current_lr = get_lr_fn()
    ...
    def _log_validation_loss_summary(self, epoch_idx: int, val_loss: float, val_components: dict, get_lr_fn) -> None:
        """Log validation loss summary with all components."""
        if not val_components:
            logger.info(f"📊 VAL LOSS: {val_loss:.4f}")
            return
        
        lr_str = self._get_lr_string(get_lr_fn)
        ...
        logger.info(
            f"📊 [{phase_str}] {elapsed_str} VAL LOSS: {val_loss:.4f} {lr_str} {marg_w_str} "
            ...
            f"| grad_norm={grad_norm_str}, param_update_norm={param_update_norm_str}"
        )
    ...
    def _recreate_optimizer_and_schedulers_for_resume(
        ...
        if use_lr_scheduler:
            if lr_schedule_segments is not None:
                ...
                # K-fold aware total steps calculation
                ...
                if total_steps <= 0:
                    raise ValueError(
                        f"Cannot create OneCycleLR with total_steps={total_steps}. "
                        f"n_epochs={n_epochs}, batches_per_epoch={batches_per_epoch}."
                    )
                
                logger.info(f"OneCycleLR: recreating with total_steps={total_steps}")
                scheduler = OneCycleLR(optimizer, max_lr=optimizer_params["lr"], total_steps=total_steps)
            ...
            if "scheduler" in self.training_state and self.training_state["scheduler"] is not None:
                try:
                    ...
                    # Fix total_steps after loading checkpoint
                    ...
                    # Fix last_epoch to match resume position
                    if hasattr(scheduler, 'last_epoch'):
                        correct_last_epoch = existing_epochs * batches_per_epoch
                        if scheduler.last_epoch != correct_last_epoch:
                            logger.warning(f"⚠️ Correcting scheduler last_epoch: {scheduler.last_epoch} → {correct_last_epoch}")
    ...
    def _build_epoch_timeline_entry(
        ...
        d: dict, get_lr_fn, lr_boost_multiplier: float, temp_boost_multiplier: float,
    ...
    def _log_validation_loss_components(self, epoch_idx: int, val_loss, val_components, get_lr) -> None:
    ...
    def _check_val_loss_early_stopping(
        ...
        training_event_callback, training_event_dict: dict, max_progress: int, get_lr
    ) -> bool:
        """
    ...
    def _log_lr_and_mw_timeseries(self, epoch_idx: int, lr_value, mw_value) -> None:
        """Log LR and marginal weight time series."""
        # Log LR time series
        lr_nums = self._get_time_moving_loss_values(self._val_loss_tracker['lr_history'], epoch_idx, lr_value)
        lr_nums.append(f"{lr_value:8.6f}")
        logger.info(f"{'LR':8s}: {' '.join(lr_nums)}")
        
        # Log marginal weight time series
        mw_nums = self._get_time_moving_loss_values(self._val_loss_tracker['marginal_weight_history'], epoch_idx, mw_value)
    ...
    def _save_timeline_to_json(
        ...
        """Save training timeline to JSON file every 5 epochs."""
    ...
    def _handle_no_learning_intervention(
        ...
        """Handle NO_LEARNING failures with gradual LR ramping and temperature boosts.
        ...
        if lr_boost_multiplier != old_boost:
            ...
            logger.warning(f"🚀 [{epoch_idx}] Gradual LR boost: {old_boost:.2f}x → {lr_boost_multiplier:.2f}x")
        ...
        if lr_boost_multiplier >= 3.0 and temp_boost_multiplier < 2.0:
            ...
            logger.warning(f"🌡️  [{epoch_idx}] Temperature boost: 1.0x → 2.0x")
        
        if lr_boost_multiplier >= 6.0 and self._failure_repeat_count >= 10:
            logger.warning(f"🛑 [{epoch_idx}] CONVERGED: LR boost maxed, stopping")
            corrective_action = {
                "epoch": epoch_idx, "trigger": "CONVERGED", "action_type": "STOP_TRAINING",
                "details": {"val_loss": val_loss, "lr_boost": lr_boost_multiplier}
            }
            epoch_entry["corrective_actions"].append(corrective_action)
            self._corrective_actions.append(corrective_action)
            should_break = True
        ...
        if intervention_made:
            ...
            corrective_action = {
                "epoch": epoch_idx, "trigger": "NO_LEARNING", "action_type": "GRADUAL_LR_BOOST",
                "details": {
                    "blocked_for_epochs": 10, "lr_multiplier": lr_boost_multiplier,
                    "temp_multiplier": temp_boost_multiplier, "intervention_stage": intervention_stage,
                    "current_dropout": getattr(self, '_current_dropout', None)
                }
            }
            epoch_entry["corrective_actions"].append(corrective_action)
            self._corrective_actions.append(corrective_action)
    ...
    def _handle_val_loss_tracking_and_early_stop(
        ...
        if scheduler is not None and isinstance(scheduler, OneCycleLR):
            min_epoch_for_early_stop = int(n_epochs * 0.70)
    ...
    def _handle_checkpoint_saves(
        ...
        Returns the updated lowest_val_loss.
        """
        ...
        # Save best model if this is the lowest validation loss
    ...
    def _periodic_embedding_quality_check(self, epoch_idx: int, n_epochs: int) -> None:
        ...
        try:
            ...
            logger.info("=" * 80)
            self._log_embedding_quality_summary()
            logger.info("=" * 80)
            logger.info("")
        except Exception as e:
        ...
        try:
            ...
            if ww_result.get('should_stop', False):
                ...
                if check_no_stop_file(job_id, output_dir_str):
                    ...
                    logger.info(f"   → OneCycleLR cooldown period - continuing training")
    ...
    def _defragment_gpu_memory(self, epoch_idx: int, force_full_defrag: bool = False) -> None:
        ...
        try:
            ...
            if do_full_defrag:
                ...
                # Only do full defrag if there's significant fragmentation (reserved > 1.5× allocated)
                if reserved_before > allocated_before * 1.5 or force_full_defrag:
    ...
    def _train_impl(
        ...
        if existing_epochs is None:
            ...
            if latest_checkpoint is not None:
                ...
                logger.info("   To start fresh, delete checkpoint files or pass existing_epochs=-1")
                logger.info("=" * 80)
        ...
        has_relationship_features = (
            hasattr(self, 'relationship_features') and self.relationship_features is not None
        ) or (
            ...
            for epoch_idx in range(base_epoch_index, n_epochs):
                ...
                if hasattr(self.encoder, 'joint_encoder') and hasattr(self.encoder.joint_encoder, 'relationship_extractor'):
                    ...
                    if not hasattr(rel_extractor, '_metadata_set'):
                        if hasattr(self, '_dataset_hash') and self._dataset_hash:
                            rel_extractor._dataset_hash = self._dataset_hash
                        if hasattr(self, 'session_id') and self.session_id:
                            rel_extractor._session_id = self.session_id
                        elif hasattr(self, 'job_id') and self.job_id:
                            rel_extractor._session_id = self.job_id
                ...
                try:
                    for batch_idx, batch in enumerate(timed_data_loader):
                        ...
                        if batch_idx % 10 == 0:
                            save_fn = (lambda: self.save_state(epoch_idx, batch_idx, self.encoder, optimizer, scheduler, dropout_scheduler)) if save_state_after_every_epoch else None
                        ...
                        try:
                            ...
                            if isinstance(forward_oom_err, torch.OutOfMemoryError) or "out of memory" in error_str or "cuda" in error_str:
                                ...
                                if should_reduce_batch:
                                    ...
                                    if oom_stats["batch_size_reductions"] < oom_stats["max_batch_size_reductions"]:
                                        next_retry = oom_stats["retry_attempt"] + 1
                                        logger.warning(f"🔄 Will retry #{next_retry}/{oom_stats['max_batch_size_reductions']} with batch_size={suggested_batch_size}")
                                        ...
                                        logger.error(f"❌ Already reduced batch size {oom_stats['batch_size_reductions']} times - cannot retry")
                                        raise forward_oom_err
                    ...
                    if oom_stats["batches_skipped_this_epoch"] >= batches_per_epoch:
                        ...
                        if oom_stats["batch_size_reductions"] < oom_stats["max_batch_size_reductions"]:
                            ...
                            logger.error(f"❌ Already reduced batch size {oom_stats['batch_size_reductions']} times - cannot retry")
                            raise RuntimeError(f"All batches skipped due to OOM and max batch size reductions ({oom_stats['max_batch_size_reductions']}) reached")
                ...
                try:
                    ...
                    if val_dataloader is not None:
                        ...
                        self._log_validation_loss_summary(epoch_idx, val_loss, val_components, get_lr)
                ...
                epoch_entry = self._build_epoch_timeline_entry(
                    epoch_idx, val_loss, val_components, d, get_lr,
                ...
                if print_progress_step is not None:
                    if print_callback is not None:
                        ...
                        self._record_epoch_loss_history(
                            d, epoch_idx, loss, val_loss, val_components, get_lr, epoch_start_time_now
                        )
                        print_callback(d)
