...
def s3_make_client(
    ...
    client.meta.credentials = session.get_credentials()
    try:
        yield client
    finally:
        client.close()
...
def s3_make_progressed_client(
    ...
    :param region_name: AWS service region name.
    :param endpoint_url: AWS service endpoint URL.
    :return: An instance of ``S3ClientProgressProxy`` that wraps an ``S3Client`` with progress reporting.
    """
    with Progress(
        ...
        with s3_make_client(access_key_id, secret_access_key, region_name, endpoint_url) as client:
            yield S3ClientProgressProxy(client, progress)


def s3_options_from_s3_client(client: S3Client) -> JsonObject:
    """
    ...
    s3_options: JsonObject = {}

    try:
        # Extract credentials from the client
        # First try the public meta attribute, which is injected by our `s3_make_client` function, if available
        credentials = client.meta.credentials
    except AttributeError:
        try:
            # Fallback to private attribute (may not be stable across boto3 versions)
            credentials = client._request_signer._credentials
        except AttributeError:
            # Unable to extract credentials
            raise AttributeError("cannot extract credentials from the provided S3Client instance")
