...
def get_pattern_time_frame(index, pattern_section):
    ...
    Args:
        index (pandas.DatetimeIndex): datetime-index of the resulting timeseries
        pattern_section (swmm_api.input_file.helpers.InpSection): pattern-section of inp-file

    Returns:
        pandas.DataFrame: index=index, columns=pattern.names
    """
    res = []
    ...
    for p in pattern_section.values():  # type: Pattern
        ...
        res.append(
            temp[p.cycle].map(dict(zip(i[p.cycle], p.factors))).fillna(1).round(3).rename(p.name)
        )
...
def get_pattern_series_prod(df_pattern, group, patterns_section,
    ...
    Args:
        df_pattern (pandas.DataFrame): frame with patterns as Series
        ...
        weekend_index_bool (pandas.Series[bool]): bool-series to show weekends

    Returns:
        pandas.Series: index=index, dtype=Float
    """
...
def get_weekend_bool_series(index):
    ...
    Returns:
        pandas.Series[bool]: bool-series to show weekends
    """
...
def get_dwf_node_series(inp, dwf: DryWeatherFlow, index):
    """
    Get the time-series dry weather flow in one node.

    Args:
        inp (swmm_api.SwmmInput): SWMM input-file data.
        dwf (swmm_api.input_file.sections.DryWeatherFlow): dry weather flow object at one node.
        index (pandas.DatetimeIndex): datetime-index of the resulting timeseries.

    Returns:
        pandas.Series: dwf time-series at one node (index=index, dtype=Float)
    """
...
class PatternRegression:
    def __init__(self, ts, set_cycles=None, with_year=True):
        ...
        # ---
        # TODO: if only one list of cycles given
