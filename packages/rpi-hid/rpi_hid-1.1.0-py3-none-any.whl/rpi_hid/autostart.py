...
[Service]
Type=oneshot
ExecStartPre=/bin/sh -c 'for i in $(seq 1 20); do [ -e /dev/hidg0 ] && exit 0; sleep 0.5; done; exit 1'
...
"""
    ...
    # enable AND start (critical)
    subprocess.run(["systemctl", "enable", service], check=True)
    subprocess.run(["systemctl", "start", service], check=True)

    print(f"[+] AutoStart ENABLED and STARTED for {script_path}")
