...
class DigiKeyApi:
    ...
    async def refresh_auth(self) -> Result[AuthTokenResponse, Any]:
        ...
        if self.refresh_token:
            # 3-legged OAuth: Use refresh token grant
            data = {
            ...
            # FIXME: no redirect_uri needed if purely refreshing?
            if self.redirect_uri:
                data["redirect_uri"] = self.redirect_uri
        else:
            # 2-legged OAuth: Use client credentials grant
            data = {
                "client_id": self.client_id,
                "client_secret": self.client_secret,
        ...
        async with httpx.AsyncClient() as temp_client:
            ...
            match result:
                case Result(Ok=data, Err=None):
                    self.access_token = f'Bearer {data.access_token}'
                    self.token_expiry = time.time() + data.expires_in
                    self.refresh_token = data.refresh_token
                    if data.refresh_token_expires_in:
                        self.refresh_token_expiry = time.time() + data.refresh_token_expires_in
                    else:
    ...
    async def _ensure_token_valid(self):
        if time.time() + 30. > self.token_expiry:
            await self.refresh_token()
        elif time.time() > self.refresh_token_expiry:
