def bitmask(bitwidth: int) -> int:
    """
    Convenience function to generate a bitmask that can be applied to a value to
    clamp it within the bitwidth.
    """
    return (1 << bitwidth) - 1
...
def ctlz(x: int, bitwidth=32) -> int:
    """
    Count the number of leading zeros in the binary representation of the given
    input, given a bitwidth (default: 32).
...
def clo(x: int, bitwidth=32) -> int:
    """
    Count the number of leading ones in the binary representation of the given
    input, given a bitwidth (default: 32).
