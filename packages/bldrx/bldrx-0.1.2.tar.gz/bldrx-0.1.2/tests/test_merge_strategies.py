...
def test_merge_append_strategy(tmp_path):
    ...
    list(engine.apply_template('m', dest, {'project_name': 'X'}, force=False, atomic=True, merge='append'))
...
def test_merge_marker_strategy(tmp_path):
    ...
    engine = Engine(templates_root=templates, user_templates_root=tmp_path / "user_templates")
    list(engine.apply_template('m', dest, {'project_name': 'Y'}, force=False, atomic=True, merge='marker'))
