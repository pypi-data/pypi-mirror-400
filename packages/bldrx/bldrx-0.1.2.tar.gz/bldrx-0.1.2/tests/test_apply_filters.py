...
def test_only_filter(tmp_path):
    ...
    res = list(engine.apply_template('tpl', dest, {'project_name': 'X'}, force=True, templates_dir=str(pkg), only_files=only))
...
def test_except_filter(tmp_path):
    ...
    # exclude b.txt
    exclude = ['b.txt']
    res = list(engine.apply_template('tpl', dest, {'project_name': 'Y'}, force=True, templates_dir=str(pkg), except_files=exclude))
    assert (dest / 'a.txt').exists()
    assert (dest / 'README.md').exists()
    assert not (dest / 'b.txt').exists()
...
def test_only_and_except(tmp_path):
    ...
    exclude = ['b.txt']
    res = list(engine.apply_template('tpl', dest, {'project_name': 'Z'}, force=True, templates_dir=str(pkg), only_files=only, except_files=exclude))
    assert (dest / 'a.txt').exists()
    assert (dest / 'README.md').exists()
    assert not (dest / 'b.txt').exists()