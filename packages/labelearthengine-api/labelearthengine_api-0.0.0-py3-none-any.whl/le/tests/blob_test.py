...
class BlobTest(apitestcase.ApiTestCase):
  ...
  def test_blob(self):
    ...
    blob_func = le.ApiFunction.lookup('Blob')
    self.assertEqual(blob_func, blob.func)

    self.assertFalse(blob.isVariable())
    self.assertEqual({'url': le.String(URL)}, blob.args)
    ...
    result_cast = json.loads(le.Blob(blob).serialize())
  ...
  def test_computed_object(self):
    ...
    result = le.ApiFunction.call_('Blob', URL)
  ...
  def test_wrong_arg_type(self):
    ...
    with self.assertRaisesRegex(ValueError, message):
      le.Blob(123)  # pytype: disable=wrong-arg-types
  ...
  def test_string(self):
    ...
    expression = le.Blob(URL).string(encoding)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Blob(URL).string(encoding=encoding)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
