...
def _load_lib():
    ...
    # Priority 1: Check inside package directory (for Pre-built or Dev)
    package_dir = os.path.dirname(__file__)
    package_lib = os.path.join(package_dir, lib_name)
    
    # Priority 2: Check for Extension Module (Linux/Mac pip install)
    # Be robust: look for any .so file starting with '_libnncc' in this directory
    ext_lib = None
    if os.name != "nt":
        import glob
        # Look for _libnncc*.so or libnncc.so
        candidates = glob.glob(os.path.join(package_dir, "_libnncc*.so")) + \
                     glob.glob(os.path.join(package_dir, "libnncc.so"))
        if candidates:
            ext_lib = candidates[0]

    # Priority 3: Check root (Dev mode fallback)
    root_lib = os.path.join(os.path.dirname(package_dir), lib_name)

    # Resolve
    lib_path = None
    if os.path.exists(package_lib):
        lib_path = package_lib
    elif ext_lib and os.path.exists(ext_lib):
        lib_path = ext_lib
    elif os.path.exists(root_lib):
        lib_path = root_lib
