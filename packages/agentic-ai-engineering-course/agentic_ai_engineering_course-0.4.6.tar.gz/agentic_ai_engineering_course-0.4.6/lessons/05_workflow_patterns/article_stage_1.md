...
If you’ve spent any time building with Large Language Models (LLMs), you’ve likely been tempted to write a single, massive prompt to solve a complex problem. I’ve been there. In the early days, I thought prompt chaining was my unique, state-of-the-art technique for building complex JSON objects. The reality check came when I saw my API bills skyrocket. A system I thought was clever was actually a great way to throw away money for worse accuracy.
...
This lesson is about that engineering discipline. We will move away from the hype and focus on the fundamental patterns for building robust LLM workflows. We’ll start by deconstructing the single-prompt fallacy and then introduce the core building blocks: sequential chaining, parallelization, routing, and the orchestrator-worker pattern. Mastering these techniques is the first real step toward building everything from simple, deterministic workflows to the advanced AI agents we’ll explore later in this course.

## The Challenge with Complex Single LLM Calls

Attempting to solve a multi-step problem with a single, complex LLM call is an anti-pattern. While it might seem efficient, it introduces a host of engineering challenges that make your system fragile and difficult to maintain. When the LLM inevitably fails, where do you start debugging? A monolithic prompt gives you no visibility into the intermediate steps, making it nearly impossible to pinpoint the source of an error. This lack of modularity means a small change to one part of the instruction can have unpredictable effects on the entire output.

Furthermore, long prompts are susceptible to the "lost in the middle" problem. The model pays less attention to instructions buried deep in a large context window, leading to parts of your request being ignored [[5]](https://arxiv.org/html/2410.23884v1). Models can also struggle with output consistency, hallucinating formats or labels that weren't requested, which undermines any downstream automation [[1]](https://arxiv.org/pdf/2309.08181). In short, you trade control and reliability for a false sense of simplicity.

Let's make this concrete with a practical example. Imagine we want to generate a Frequently Asked Questions (FAQ) page from a few documents about renewable energy. A naive approach would be to stuff everything into one prompt.

First, let's set up our environment. We'll use the Google Gemini library throughout this lesson.
...
Next, we'll define our source documents. For brevity, we'll just show the titles and a high-level description.
...
Now, let's build our single, complex prompt. We'll ask the model to generate questions, provide answers, and cite the sources, all in one go. We’ll use Pydantic for structured output, a concept we covered in Lesson 4.
...
```json
{
  "question": "Why is energy storage crucial for renewable energy sources like solar and wind, and what are the common types of large-scale storage solutions?",
...
At first glance, this output looks pretty good. The question is relevant, and the answer is accurate. However, looking closer, we can spot a subtle but critical failure. The answer mentions that storage is key for "solar and wind," but it only cites "Energy Storage Solutions" as the source. The context about wind energy's intermittency, which makes storage so important, actually comes from the "Understanding Wind Turbines" document. The LLM missed a source.
...
The engineering solution to the unreliability of complex prompts is a "divide-and-conquer" strategy known as prompt chaining. Instead of asking the LLM to perform multiple distinct tasks at once, we break down the problem into a sequence of smaller, simpler steps. Each step is a focused LLM call, and the output of one step becomes the input for the next, forming a chain.

This modular approach brings several key benefits, moving us from hopeful prompting to deliberate engineering [[25]](https://www.vellum.ai/blog/what-is-prompt-chaining).

First, it dramatically improves **modularity and debuggability**. Each link in the chain is an isolated component with a clear, single responsibility. If the final output is wrong, you don't have to guess what went wrong in a massive prompt. You can inspect the output of each intermediate step, pinpoint the exact point of failure, and fix it [[26]](https://dev.to/kapusto/enhancing-large-language-model-performance-with-prompt-chaining-2p84). This makes the system far easier to maintain and improve over time.

Second, accuracy often increases. A simpler, more focused prompt is easier for the LLM to understand and execute correctly. By asking the model to do one thing at a time—like generating questions first, then answering them, then finding sources—we reduce the cognitive load and minimize the chance of instructions being missed or misinterpreted [[27]](https://www.voiceflow.com/blog/prompt-chaining).

Third, chaining offers greater **flexibility**. You can use different models or configurations for different steps in the chain. For a simple classification task, a fast, inexpensive model like Haiku might be sufficient. For a complex reasoning step, you can switch to a more powerful model like Opus. This allows you to optimize your workflow for cost, latency, and quality, using the right tool for the right job [[25]](https://www.vellum.ai/blog/what-is-prompt-chaining).

However, prompt chaining isn't a silver bullet. It introduces its own set of trade-offs. The most obvious are increased **latency and cost**; you're making multiple API calls instead of one [[8]](https://blog.promptlayer.com/what-is-prompt-chaining/). Each call adds to the total execution time and the final bill. There's also a risk of **information loss** between steps. If an early step in the chain produces a summary, important details might be dropped, leading to a degraded final output [[7]](https://www.humanfirst.ai/blog/prompt-chaining). Finally, managing the "glue code" that connects the steps can add engineering complexity, especially as chains become longer and more intricate [[9]](https://ai.plainenglish.io/prompt-chaining-is-dead-long-live-prompt-stuffing-58a1c08820c5).

Despite these drawbacks, the control, reliability, and debuggability gained from a modular approach almost always outweigh the convenience of a single prompt for any non-trivial task.
...
And here's one of the resulting FAQ objects:
...
Our sequential workflow is reliable, but it's slow. The total execution time is the sum of the latencies of all individual LLM calls. For tasks where intermediate steps are independent of each other, we can significantly speed things up by running them in parallel. In our FAQ example, once we have the list of questions, the process of answering and sourcing for each question is independent of the others. We don't need to finish answering Question 1 before we start on Question 2.

This is where parallelization comes in. We can use Python's `asyncio` library to make concurrent API calls, processing multiple questions simultaneously. This can dramatically reduce the total processing time from the sum of all calls to the time of the longest single call in a batch.

Let's refactor our workflow to use parallel processing.

First, we need asynchronous versions of our `answer_question` and `find_sources` functions. The logic remains the same, but we'll use `async def` and `await` with the `aio` version of the Gemini client.
...
Next, we'll create a new processing function that handles a single question by running its answer and sourcing steps in parallel. However, since finding the sources depends on the answer, these two specific steps must remain sequential for a given question. The parallelization happens across different questions.

Let's define a function to process one question, which we will then run in parallel for all questions.
...
When we run this, the output is striking:

```
Parallel processing completed in 8.98 seconds
```

By parallelizing the independent tasks, we cut the execution time from 22.2 seconds to just under 9 seconds—a more than 2x speedup. For a larger number of questions, the improvement would be even more dramatic. This demonstrates the power of parallelization for optimizing workflows.

However, this speed comes with a new consideration.

⚠️ **A Note on Rate Limits:** When you make many parallel API calls, you are more likely to hit the rate limits imposed by the LLM provider (e.g., requests per minute). Production systems must handle this gracefully. Best practices include implementing an exponential backoff strategy, which retries a failed request with an increasing delay, and adding jitter (a small, random delay) to prevent all retries from happening at once [[11]](https://platform.openai.com/docs/guides/rate-limits). Your code should be prepared to catch `429 Too Many Requests` errors and manage a request queue to stay within your account's limits.

## Introducing Dynamic Behavior: Routing and Conditional Logic

So far, our workflows have been linear. Whether sequential or parallel, they follow a fixed path from start to finish. But what if the processing logic needs to change based on the input? For example, in a customer support system, a query about a billing issue should be handled differently from a technical support question. Trying to create a single, massive prompt to handle all possible user intents would be incredibly complex and brittle.

This is where **routing** comes in. Routing is a workflow pattern that introduces conditional logic, or branching. It uses an initial LLM call to classify the input and then directs, or "routes," it to a specialized handler or sub-workflow designed for that specific category [[34]](https://docs.aws.amazon.com/prescriptive-guidance/latest/agentic-ai-patterns/workflow-for-routing.html).

Think of routing as another application of the "divide-and-conquer" principle. Instead of one complex prompt that tries to be a jack-of-all-trades, we create multiple, smaller, expert prompts. A "router" prompt acts as a dispatcher, analyzing the incoming request and deciding which expert is best suited for the job [[40]](https://arize.com/blog/best-practices-for-building-an-ai-agent-router/).

This approach keeps our components modular and specialized. Optimizing the prompt for handling billing inquiries won't accidentally degrade performance for technical questions [[35]](https://spring.io/blog/2025/01/21/spring-ai-agentic-patterns). This approach also allows you to route to different LLM models, leveraging their specific strengths for different tasks. For instance, a smaller, faster model might handle simple classifications, while a more powerful one tackles complex reasoning. We will explore model routing in more detail in future lessons. Each branch of the workflow can be developed, tested, and maintained independently. It provides a structured way to manage complexity as your application's capabilities grow, forming a foundational pattern for building more sophisticated, multi-turn conversational agents.
...
graph TD
    ...
    F --> G;
```
Figure 2: A routing workflow for customer service. An LLM first classifies the intent, then directs the query to a specialized handler.

Here's how we implement this in code.

**Step 1: Intent Classification**

First, we define the possible intents and create a classifier function. The prompt asks the LLM to categorize the user's query into one of our predefined intents, returning a structured output.
...
We've seen how to build linear and branching workflows. But what happens when a task is so complex that the necessary steps can't be predicted in advance? Consider a customer query like: "I think invoice #INV-7890 is wrong, I want to return the 'SuperWidget 5000', and can I get an update on order #A-12345?"

A simple routing workflow isn't enough here. This single query contains three distinct sub-tasks: a billing inquiry, a product return, and a status update. This is where the **orchestrator-worker** pattern comes in. It's a more advanced workflow designed for dynamic task decomposition [[22]](https://huggingface.co/blog/dcarpintero/design-patterns-for-building-agentic-workflows).

In this pattern, a central "orchestrator" LLM acts as a project manager. Its job is to analyze a complex input, break it down into a list of smaller, independent sub-tasks, and delegate each sub-task to a specialized "worker." These workers can run in parallel. Finally, a "synthesizer" component gathers the results from all workers and combines them into a single, coherent response [[21]](https://www.anthropic.com/research/building-effective-agents).

The key difference from simple parallelization is its **flexibility**; subtasks aren't predefined. The orchestrator determines them on the fly based on the specific input. This makes the pattern incredibly powerful for handling unpredictable, multi-faceted requests [[23]](https://fme.safe.com/guides/ai-agent-architecture/ai-agentic-workflows/).
...
We then define a worker for each task type. These are simple Python functions that simulate interacting with backend systems (e.g., fetching an order status, generating a return authorization). They don't need to be LLM-based, but could be. For our billing worker, we'll use an LLM to extract the specific user concern.

```python
# Billing Worker (simulates opening an investigation)
...
# Return Worker (simulates generating an RMA number)
...
# Status Worker (simulates fetching order status)
...
Now, let's test the entire workflow with our complex query.
...
def process_user_query(user_query):
    ...
    # 2. Run workers based on tasks_list
    worker_results = []
...
The workers process these tasks in parallel, and the synthesizer then crafts the final response:
...
We’ve moved from the naive approach of a single, complex prompt to a more disciplined, engineering-driven methodology for building LLM applications. The core idea is simple but powerful: break down complex problems. By mastering the fundamental workflow patterns—sequential chaining, parallelization, routing, and the orchestrator-worker model—you gain the tools to build robust, scalable, and reliable AI systems.
...
Mastering these workflows is the foundation upon which more advanced agentic systems are built. In the upcoming lessons, we will build directly on these concepts. In **Lesson 6 - Agent Tools & Function Calling**, we'll see how to give our workflows the ability to take 'actions' in the real world. Following that, in **Lesson 7 - Planning & Reasoning**, we will explore how to implement reasoning capabilities, allowing our agents to think and plan their steps. Your journey into building true AI agents has just begun.

## References

- [1] [Failure-Mode-Guided Failure Discovery in Video Recognition Models](https://arxiv.org/pdf/2309.08181)
- [5] [Narrative-Driven Causal Analysis in Large Language Models](https://arxiv.org/html/2410.23884v1)
- [25] [What Is Prompt Chaining?](https://www.vellum.ai/blog/what-is-prompt-chaining)
- [26] [Enhancing Large Language Model Performance with Prompt Chaining](https://dev.to/kapusto/enhancing-large-language-model-performance-with-prompt-chaining-2p84)
- [27] [What is Prompt Chaining?](https://www.voiceflow.com/blog/prompt-chaining)
- [8] [What is Prompt Chaining?](https://blog.promptlayer.com/what-is-prompt-chaining/)
- [7] [Prompt Chaining: An Introduction](https://www.humanfirst.ai/blog/prompt-chaining)
- [9] [Prompt Chaining Is Dead, Long Live Prompt Stuffing](https://ai.plainenglish.io/prompt-chaining-is-dead-long-live-prompt-stuffing-58a1c08820c5)
- [11] [Rate limits](https://platform.openai.com/docs/guides/rate-limits)
- [34] [Routing workflow](https://docs.aws.amazon.com/prescriptive-guidance/latest/agentic-ai-patterns/workflow-for-routing.html)
- [40] [Best Practices For Building An AI Agent Router](https://arize.com/blog/best-practices-for-building-an-ai-agent-router/)
- [35] [Spring AI - Agentic Patterns](https://spring.io/blog/2025/01/21/spring-ai-agentic-patterns)
...
- [21] [Building effective agents](https://www.anthropic.com/research/building-effective-agents)
- [23] [AI Agentic Workflows](https://fme.safe.com/guides/ai-agent-architecture/ai-agentic-workflows/)