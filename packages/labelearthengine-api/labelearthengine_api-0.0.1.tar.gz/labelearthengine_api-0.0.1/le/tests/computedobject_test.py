...
class ComputedObjectTest(apitestcase.ApiTestCase):
  ...
  def test_internals(self):
    """Test eq(), ne() and hash()."""
    a = le.ApiFunction.call_('DateRange', 1, 2)
    b = le.ApiFunction.call_('DateRange', 2, 3)
    c = le.ApiFunction.call_('DateRange', 1, 2)
  ...
  def test_bad_init_with_both_func_and_var(self):
    ...
    func = le.ApiFunction.lookup('Date')
    with self.assertRaisesRegex(le.EEException, message):
  ...
  def test_cast_different(self):
    number = le.Number(1)
    result = le.String._cast(number).getInfo()
    expect = le.String('1').getInfo()
  ...
  def test_serialize_opt_params(self):
    obj = le.ComputedObject(func=None, args=None, varName='test')
