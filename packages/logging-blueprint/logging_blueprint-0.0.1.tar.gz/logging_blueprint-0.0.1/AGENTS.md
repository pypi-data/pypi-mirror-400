...
Write for Python 3.10 through the latest GA Python release. Maintain strict 3.10
compatibility, using shims such as `typing-extensions` when needed. GitHub Actions will
enforce this matrix; prefer running Python 3.10 locally to catch issues sooner.

Always use modern Python practices appropriate for Python 3.10-3.13, adding shims only
when necessary.
...
- You may use the following shortcuts
  ...
  # Run linting (with ruff) and type checking (with mypy in strict mode).
...
- To exercise tests on other Python versions locally (without touching the project venv):
  - Install the interpreter once: `uv python install 3.13` (or another version in range).
  - Run tests in isolation: `uv run --isolated --python 3.13 pytest -m "not json_extra"` or full `uv run --isolated --python 3.13 pytest`.
  - For JSON-marked tests, include the extra: `uv run --isolated --extra json --python 3.13 pytest -m json_extra`.

## General Development Practices

- Be sure to resolve the mypy (strict) type checker errors as you develop and make
  changes.

- If type checker errors are hard to resolve, you may add a comment
  `# type: ignore[error-code]` to silence them but ONLY if you know they are not a real
  problem and are difficult to fix. Prefer fixing the code or adjusting types first.
...
- Maintain a strict testing posture: all changes must be covered by relevant tests and
  `make lint` and `make test` must be clean before changes are considered done.
...
- For realistic flows, run isolated subprocesses that execute temporary scripts copied
  from `tests/resources/examples` instead of relying on installed entry points.
- Use pytest tempdir factories (`tmp_path`/`tmp_path_factory`) to create per-test
  working directories for those subprocess runs so the repo stays untouched.
- Avoid pytest’s `monkeypatch`; prefer `from unittest import mock` with context managers
  to keep patch scope tight.
- When behavior depends on environment (TTY detection, env vars, platform), mock those
  checks in tests so results are deterministic across local runs, CI, and Codex sandboxes.
...
- If a function does not use a parameter but it should still be present, prefix the name
  with an underscore to silence unused-parameter warnings. Avoid ignore comments unless
  strictly necessary.
...
- Once we reach version 1.0, interfaces are locked. Avoid breaking changes and plan
  new behavior via additive, backward-compatible extensions.
