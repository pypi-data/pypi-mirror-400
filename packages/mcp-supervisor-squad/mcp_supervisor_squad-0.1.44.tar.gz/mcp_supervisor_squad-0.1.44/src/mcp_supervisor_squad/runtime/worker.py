...
class Worker:
    ...
    def start(self, *, cwd: str, query: str, options: dict[str, Any]) -> RunState:
        ...
        cfg = options  # minimalist: use options directly for models/policies if provided
        ...
        tasks: list[RunTask] = [
            RunTask(slug="single", project_root=str(decision.primary_root), worktree=str(decision.repo_root))
        ]
        ...
        state = RunState(
            ...
            query=query,
            sandbox=str(cfg.get("sandbox") or "workspace-write"),
            approval_policy=str(cfg.get("approval_policy") or "never"),
            planner_model=str(cfg.get("planner_model") or "gpt-5.2"),
            planner_reasoning_effort=str(cfg.get("planner_reasoning_effort") or "medium"),
            planner_extra_config=list(cfg.get("planner_extra_config") or []),
            coder_model=str(cfg.get("coder_model") or "gpt-5.1-codex-mini"),
            coder_reasoning_effort=str(cfg.get("coder_reasoning_effort") or "medium"),
            coder_extra_config=list(cfg.get("coder_extra_config") or []),
