...
import hpcai
import torch
from hpcai.cookbook.utils.misc_utils import safezip
from hpcai.cookbook.utils.trace import scope


def compute_kl_sample_train(
    data_D: List[hpcai.Datum], training_logprobs_D: List[torch.Tensor]
) -> Dict[str, float]:
...
async def compute_post_kl(
    data_D: List[hpcai.Datum], post_sampling_client: hpcai.SamplingClient
) -> Dict[str, float]:
...
async def incorporate_kl_penalty(
    ...
    base_sampling_client: hpcai.SamplingClient,
    ...
    for i, datum in enumerate(data_D):
        ...
        datum.loss_fn_inputs["advantages"] = hpcai.TensorData.from_torch(
