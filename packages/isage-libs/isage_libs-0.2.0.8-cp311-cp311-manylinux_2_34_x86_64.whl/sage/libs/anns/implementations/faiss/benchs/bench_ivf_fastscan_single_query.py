...
def eval_recall(index, name, single_query=False):
    ...
    recall = corrects / nq
    print(f"\tnprobe {index.nprobe:3d}, 1Recall@{k}: {recall:.6f}, speed: {speed:.6f} ms/query")

    return recall, qps


def eval_and_plot(
    name, rescale_norm=True, plot=True, single_query=False, implem=None, num_threads=1
):
