-- Consolidated Stored Procedure: PP to Vortex Repayment Migration with Fee Details
-- This procedure handles the complete data migration from PP to Vortex architecture
-- including repayment details and fee breakdown in a single transaction

CREATE OR REPLACE PROCEDURE prc_repayments_pp_to_vortex_migration(
...
BEGIN
    ...
    VALUES (
        v_migration_batch_id, 'PP', 'PROCESSING', 0, 0,
    ...
    FOR v_repayment_record IN 
        ...
        JOIN t_loan tl ON imt.secondary_loan_id = tl.loan_ref_id
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        BEGIN
            ...
            VALUES ('PRC_REPAYMENTS_PP_TO_VORTEX_MIGRATION', my_ex_state, my_ex_message, my_ex_detail, my_ex_hint, my_ex_ctx, NOW(), NOW());
