...
class KeysTab(QtWidgets.QWidget):
    ...
    def _build_ui(self):
        ...
        self.pub_copy_btn = QtWidgets.QPushButton("Copy")
    ...
    def copy_public_key(self):
        """
        Copy the selected public key's value (OpenSSH text) to the clipboard.
        """
        item = self.public_keys_list.currentItem()
        if not item:
            if self.main:
                self.main.show_toast(
                    "Select a public key to copy.",
                    level="warning",
                )
            return

        key_id = item.data(256)
        if not key_id or not self.main:
            return
        ...
        if not pk:
            if self.main:
                self.main.show_toast(
                    "Could not find public key data.",
                    level="error",
                )
            return

        data = pk.get("data", "").strip()
        if not data:
            if self.main:
                self.main.show_toast(
                    "Selected public key has no data.",
                    level="warning",
                )
            return
