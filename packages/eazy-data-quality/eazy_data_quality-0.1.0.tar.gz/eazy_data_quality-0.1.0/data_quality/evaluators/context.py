...
class ContextEvalProcessor(BaseProcessor):
    """答案评估处理器"""

    def __init__(self, yaml_path='prompt/context_prompt.yaml', required_columns=None):
        extractor = YamlVariableExtractor(yaml_path)
        self.var_list = extractor.extract_variables()
        if required_columns is None:
            required_columns = ['问题', '召回上下文', '正确上下文']
