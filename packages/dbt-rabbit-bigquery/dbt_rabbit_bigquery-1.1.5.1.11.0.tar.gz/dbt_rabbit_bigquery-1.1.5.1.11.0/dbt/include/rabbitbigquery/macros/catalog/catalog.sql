{% macro _rabbitbigquery__get_table_shards_sql(information_schema) %}
    select
...
{% macro _rabbitbigquery__get_tables_sql() %}
    select distinct
        table_catalog,
        table_schema,
        table_name,
...
{% macro _rabbitbigquery__get_table_stats_sql() %}
    select
        table_catalog,
        table_schema,
        table_name,
...
{% macro _rabbitbigquery__get_columns_sql(information_schema) %}
    select
...
{% macro _rabbitbigquery__get_column_stats_sql() %}
    select
        table_catalog,
        table_schema,
        shard_name,
...
{% macro _rabbitbigquery__get_extended_catalog_sql() %}
    select
