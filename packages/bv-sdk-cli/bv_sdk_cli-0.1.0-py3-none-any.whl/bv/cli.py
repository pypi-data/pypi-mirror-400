...
@app.command(help="Initialize a new project (minimal: bvproject.yaml + main.py)")
def init(
	name: str = typer.Argument(..., help="Project name / folder to create"),
	...
	venv: bool = typer.Option(True, "--venv/--no-venv", help="Create a virtual environment"),
) -> None:
	import os
	init_project(project_name=name, python_version=python_version, keep_main=keep_main, create_venv=venv)
	typer.echo(f"Initialized project '{name}' with bvproject.yaml and main.py")
	if venv:
		typer.echo(f"Virtual environment created in {name}/.venv")
		if os.name == "nt":
			typer.echo(f"To activate: {name}\\.venv\\Scripts\\activate")
		else:
			typer.echo(f"To activate: source {name}/.venv/bin/activate")
