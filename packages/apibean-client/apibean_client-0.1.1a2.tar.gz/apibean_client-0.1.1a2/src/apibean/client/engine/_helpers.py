...
from ._presenters import TextResponsePresenter
from ._presenters import JsonResponsePresenter
from ._presenters import YamlResponsePresenter
...
class ResponseWrapper:
    def __init__(self, wrapped_object, session_store, account_store, presenter_type = "yaml", **kwargs):
        self._wrapped_object = wrapped_object
        self._session_store = session_store
        self._account_store = account_store
        self._presenter_type = presenter_type
        self._presenter = dict(
            json=JsonResponsePresenter(),
            yaml=YamlResponsePresenter(),
            text=TextResponsePresenter(),
        )
        self._other_kwargs = kwargs or dict()
    ...
    def _get_presenter_of(self, presenter_type):
        _type = presenter_type or self._presenter_type
        if _type not in self._presenter.keys():
            _type = "text"
        return self._presenter.get(_type)

    def print(self, presenter_type: str|None = None):
        print(self._get_presenter_of(presenter_type).render(self._wrapped_object))

    def print_body(self, presenter_type: str|None = None):
        print(self._get_presenter_of(presenter_type).render(self._wrapped_object, body_only=True))
