...
class SupportForestRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if self.base_model is None:
            self.base_model_ = SVR(kernel='rbf', gamma='scale', C=1.0, epsilon=0.1, max_iter=-1, tol=1e-3, cache_size=200, verbose=False)
        else:
...
class SupportLinearRegressor(BaseEstimator, RegressorMixin):
...
class SupportDecisionRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        # 1. Initialize defaults
        if self.base_model is None:
            self.base_model_ = SVR(shrinking=True, kernel='rbf', gamma='scale', C=1.0, epsilon=0.1, max_iter=-1, tol=1e-3, cache_size=200, verbose=False)
        else:
            self.base_model_ = clone(self.base_model)
...
class SupportKNNRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if self.meta_model is None:
            self.meta_model_ = KNeighborsRegressor(n_neighbors=5, weights='distance', algorithm='auto', leaf_size=30, p=2, metric='minkowski', n_jobs=-1)
        else:
            self.meta_model_ = clone(self.meta_model)
...
class SupportXGBRegressor(BaseEstimator, RegressorMixin):
...
class SupportCatRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if self.meta_model is None:
            self.meta_model_ = CatBoost.CatBoostRegressor(verbose=0, random_state=42, iterations=1000, learning_rate=0.1, depth=10, l2_leaf_reg=3)
        else:
            self.meta_model_ = clone(self.meta_model)
...
class SupportGradientRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv # Number of cross-validation folds
    ...
    def fit(self, X, y):
        ...
        if self.meta_model is None:
            self.meta_model_ = GradientBoostingRegressor(n_estimators=100, random_state=42, learning_rate=0.1, max_depth=10, subsample=0.8, max_features='sqrt')
        else:
            self.meta_model_ = clone(self.meta_model)
...
class SupportAdaRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv # Number of cross-validation folds
...
class SupportLightRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv # Number of cross-validation folds
