...
class Mudra(BleServiceDelegate):
    ...
    def on_serial_number_right_part_received(self, device_address: str, serial_number: int):
        if device_address in self._mudra_devices:
            self._mudra_devices[device_address].on_serial_number_right_part_received(serial_number)

    def on_serial_number_left_part_received(self, device_address: str, serial_number: int):
        if device_address in self._mudra_devices:
            self._mudra_devices[device_address].on_serial_number_left_part_received(serial_number)

    def on_firmware_version_received(self, device_address: str, version: str):
        if device_address in self._mudra_devices:
            self._mudra_devices[device_address].on_firmware_version_received(version)
