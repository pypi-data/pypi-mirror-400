...
# With S3 support for Lambda GPU outputs
...
- **S3 Output**: Save Lambda GPU outputs directly to S3 buckets
...
8. [S3 Output (Lambda GPU)](#s3-output-lambda-gpu)
9. [Remote Functions (Decorator)](#remote-functions-decorator)
10. [Async Jobs (Long-Running Tasks)](#async-jobs-long-running-tasks)
11. [Jupyter Magic](#jupyter-magic)
...
## S3 Output (Lambda GPU)

Save your Lambda GPU outputs directly to Amazon S3 or compatible storage (MinIO, etc.).
...
The `lambda_output()` method automatically detects the format based on file extension:
...
url = session.lambda_output("model.pt", model_state, s3)
print(f"Model saved to: {url}")

# Save NumPy arrays
import numpy as np
embeddings = np.random.randn(1000, 768)
url = session.lambda_output("embeddings.npy", embeddings, s3)

# Save JSON metrics
metrics = {"accuracy": 0.95, "loss": 0.05, "epoch": 100}
url = session.lambda_output("metrics.json", metrics, s3)

# Save any picklable object
results = {"predictions": [1, 2, 3], "model_config": config}
url = session.lambda_output("results.pkl", results, s3)
```
...
url = session.lambda_output_file("./checkpoints/best_model.pt", s3)
print(f"Uploaded to: {url}")

# Upload with a custom S3 name
url = session.lambda_output_file(
    "./model.pt",
    s3,
...
session.lambda_output("model.pt", model_state, s3_minio)
```

### Complete Lambda GPU Workflow
...
url = session.lambda_output("trained_model.pt", model_state, s3)
print(f"Model saved to: {url}")

# Save training metrics
metrics = {"final_loss": 0.05, "epochs": 100}
session.lambda_output("metrics.json", metrics, s3)
...
| `lambda_output(filename, data, s3_connection)` | Save Python object to S3 |
| `lambda_output_file(local_path, s3_connection)` | Upload file to S3 |
