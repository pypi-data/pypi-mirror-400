...
class DrawingsExtractor():
    ...
    def get_default_transformation():
        defualt_settings = ImgExtractionSettings()
        return ImageExtractor.get_transformations(defualt_settings)
    ...
    @staticmethod
    def save_transformed_file(fname, transform=None):
        if transform is None:
            transform = DrawingsExtractor.get_default_transformation()
...
class DrawingTaskExtractor(MongoExtractor):
    ...
    def extract_images(self, image_dir=None, imsize=224, transform_file=False, highres=False, stroke_settings=StrokeSettings(), transform_settings=None):
