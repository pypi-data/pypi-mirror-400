...
def pairwise_distances(xq, xb, metric=METRIC_L2, metric_arg=0):
    ...
    if metric == METRIC_L2:
        ...
        pairwise_extra_distances(
            d, nq, swig_ptr(xq), nb, swig_ptr(xb), metric, metric_arg, swig_ptr(dis)
        )
    return dis
...
def matrix_bucket_sort_inplace(tab, nbucket=None, nt=0):
    ...
    if nbucket is None:
        nbucket = int(tab.max() + 1)
    lims = np.empty(nbucket + 1, dtype="int64")
    matrix_bucket_sort_inplace_c(nrow, ncol, faiss.swig_ptr(tab), nbucket, faiss.swig_ptr(lims), nt)
    return lims
...
class ResultHeap:
    ...
    def add_result_subset(self, subset, D, I):
        ...
        assert I.ndim == 2 and D.shape == I.shape or I.ndim == 1 and I.shape == (kd,)
        D = np.ascontiguousarray(D, dtype="float32")
        I = np.ascontiguousarray(I, dtype="int64")
...
def merge_knn_results(Dall, Iall, keep_max=False):
    ...
    func(n, k, nshard, swig_ptr(Dall), swig_ptr(Iall), swig_ptr(Dnew), swig_ptr(Inew))
    return Dnew, Inew
...
class MapInt64ToInt64:
    ...
    def add(self, keys, vals):
        ...
        faiss.hashtable_int64_to_int64_add(
            self.log2_capacity, swig_ptr(self.tab), n, swig_ptr(keys), swig_ptr(vals)
        )
...
def knn(xq, xb, k, metric=METRIC_L2):
    ...
    if metric == METRIC_L2:
        knn_L2sqr(swig_ptr(xq), swig_ptr(xb), d, nq, nb, k, swig_ptr(D), swig_ptr(I))
    elif metric == METRIC_INNER_PRODUCT:
        knn_inner_product(swig_ptr(xq), swig_ptr(xb), d, nq, nb, k, swig_ptr(D), swig_ptr(I))
    else:
...
def knn_hamming(xq, xb, k, variant="hc"):
    ...
    if variant == "hc":
        ...
        faiss.hammings_knn_hc(heap, faiss.swig_ptr(xq), faiss.swig_ptr(xb), nb, d, 1)
    elif variant == "mc":
...
def pack_bitstrings(a, nbit):
    ...
    if is_sequence(nbit):
        ...
        pack_bitstrings_c(n, M, swig_ptr(nbit), swig_ptr(a), swig_ptr(b), code_size)
    else:
...
def unpack_bitstrings(b, M_or_nbits, nbit=None):
    ...
    if nbit is None:
        ...
        unpack_bitstrings_c(n, M, swig_ptr(nbit), swig_ptr(b), code_size, swig_ptr(a))
    else:
        M = M_or_nbits
