...
- `/spp.constitution` commands → PHR created
- `/spp.specify` commands → PHR created
- `/spp.plan` commands → PHR created + ADR suggestion
- `/spp.tasks` commands → PHR created
...
| ------------------- | -------------------------------------- | ------------ | ------------------ |
| `/spp.constitution`  | Define project principles              | ✅ Automatic | ❌ No              |
| `/spp.specify`       | Write feature spec                     | ✅ Automatic | ❌ No              |
| `/spp.plan`          | Design architecture                    | ✅ Automatic | 📋 Suggestion only |
| `/spp.adr`           | Review architectural decisions         | ❌ No\*      | ✅ Explicit        |
| `/spp.tasks`         | Break down implementation              | ✅ Automatic | ❌ No              |
| `/spp.implement`     | Execute TDD cycle                      | ✅ Automatic | ❌ No              |
| `/spp.git.commit_pr` | Generate commits/PR from finished loop | ❌ No        | ❌ No              |
| Debugging           | Fix errors                             | ✅ Automatic | ❌ No              |
| Refactoring         | Clean up code                          | ✅ Automatic | ❌ No              |
| `/spp.phr` (manual)  | Override automatic PHR                 | ✅ Explicit  | ❌ No              |
...
> **Loop reminder:** The core workflow now spans eight steps—wrap every finished feature with `/spp.git.commit_pr` after `/analyze` confirms coverage.
