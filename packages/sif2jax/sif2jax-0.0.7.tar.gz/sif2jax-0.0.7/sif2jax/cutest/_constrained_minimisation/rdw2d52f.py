...
def _precompute_rdw2d52f_data(N=256):
...
_rdw2d52f_target_data = _precompute_rdw2d52f_data()


class RDW2D52F(AbstractConstrainedMinimisation):
    """RDW2D52F problem - A finite-element approximation to distributed optimal
    control with bounds.
    ...
    quadrilateral elements. There are simple bounds on the controls
    f and states u.
    ...
    Attempts made: [fully vectorized implementation, precomputed target data]
    Suspected issues: [computational complexity with 132k variables and 65k constraints]
    Resources needed: [sparse matrix representations or advanced optimization]
    """
    ...
    # Grid parameters
    N: int = 256
    beta: float = 0.005  # Regularisation parameter
    ...
    def bounds(self):
        ...
        # Interior F variables have complex bounds based on SIF file lines 153-175
        for i in range(1, N):
            x1 = i * h
            x1_squared = x1 * x1
            minus_x1_squared = -x1_squared
            factor_2_minus_x1 = 2.0 - x1
            factor_01_2_minus_x1 = 0.1 * factor_2_minus_x1

            for j in range(1, N):
                idx = i * (N + 1) + j
                x2 = j * h
                x2_squared = x2 * x2
                arg = minus_x1_squared - x2_squared
                earg = jnp.exp(arg)
                ua = factor_01_2_minus_x1 * earg

                # Lower bound: ZL F(I,J) UA
                lower = lower.at[idx].set(ua)

                # Upper bounds depend on position
                if j <= N // 2:
                    # XU F(I,J) 0.6 for j in [1, N/2]
                    upper = upper.at[idx].set(0.6)
                else:
                    # XU F(I,J) 0.9 for j in [N/2+1, N-1]
        ...
        # Interior U variables are unbounded (no additional bounds like in RDW2D52B)
