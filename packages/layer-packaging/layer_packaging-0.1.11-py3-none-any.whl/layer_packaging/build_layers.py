...
def build_code_layer(
    pyproject: Path, layers_dir: Path, layer_name: str, code_path: Path,  keep_dir=False
) -> Tuple[Path, Path]:
    ...
    for f in selected:
        relative_path = str(f.relative_to(base_dir.parent if keep_dir else base_dir))
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    p.add_argument(
        ...
        default=[],
        action='append',
        help="Define code layer (e.g. --code-layer app=./src/*)"
    )
    ...
    for name, path in code_layer_map.items():
        keep_dir = not path.endswith("*")
        built.append(build_code_layer(pyproject, layers_dir, name, Path(path), keep_dir))
