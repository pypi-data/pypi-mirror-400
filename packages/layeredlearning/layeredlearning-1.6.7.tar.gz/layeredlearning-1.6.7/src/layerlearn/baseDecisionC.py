...
class DecisionForestClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        if self.base_model is None:
            self.base_model_ = DecisionTreeClassifier(random_state=42, max_depth=10, min_samples_split=5, min_samples_leaf=2, max_features='sqrt')
        else:
...
class DecisionSupportClassifier(BaseEstimator, ClassifierMixin):
...
class DecisionKNNClassifier(BaseEstimator, ClassifierMixin):
...
class DecisionLogisticClassifier(BaseEstimator, ClassifierMixin):
...
class DecisionXGBClassifier(BaseEstimator, ClassifierMixin):
...
class DecisionCatClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        if self.meta_model is None:
            self.meta_model_ = CatBoost.CatBoostClassifier(verbose=0, random_state=42, iterations=100, depth=10, learning_rate=0.1, l2_leaf_reg=3, border_count=32)
        else:
            self.meta_model_ = clone(self.meta_model)
...
class DecisionGradientClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv # Number of cross-validation folds
    ...
    def fit(self, X, y):
        ...
        if self.meta_model is None:
            self.meta_model_ = GradientBoostingClassifier(n_estimators=100, random_state=42, max_depth=10, learning_rate=0.1, subsample=0.8, max_features='sqrt')
        else:
            self.meta_model_ = clone(self.meta_model)
...
class DecisionAdaClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv # Number of cross-validation folds
...
class DecisionLightClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv # Number of cross-validation folds
