...
def setup_trace_config(
    ...
    if settings.ENVIRONMENT == "dev":
        """
        Since the new version of Jaeger has turned to the Opentelemetry specification
        the thrift protocol and the corresponding port 6831 have been Deprecated.
        
        set USE_OTEL_COLLECTOR to True to use newly otlp collector or jaeger v2
        remain it as False to stay Compatible
        """
        use_otlp_collector = os.getenv("USE_OTEL_COLLECTOR", False)
        if use_otlp_collector:
            # use otlp collector or jaeger v2 or local bk_datalink as trace service
            # set BKAPP_OTEL_GRPC_URL to custom collector address
            otel_grpc_host = "http://localhost:4317" if not otel_grpc_host else otel_grpc_host
            ...
            trace.get_tracer_provider().add_span_processor(span_processor(otlp_exporter))
        else:
