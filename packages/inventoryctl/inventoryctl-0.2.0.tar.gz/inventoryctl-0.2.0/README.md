...
- **Bulk Sync**: Sync hosts from external JSON/YAML sources (e.g., Terraform
  outputs, cloud APIs) into your inventory.
- **Validation**: Ensure your inventory follows the required structure.
- **Rendering**: Convert your inventory into Ansible-compatible files or SSH
  configurations.
- **Formatting**: Canonicalize your YAML files.
- **Metadata Support**: Track source ownership of resources using `_meta` tags
  to safely mix manual and automated entries.
...
- `--source <ID>`: Mark this host as managed by a specific source (e.g., `aws`,
  `terraform`).
...
Sync hosts from an external source (JSON/YAML) into a specific group. This is
useful for pipelines filling the inventory.
...
- `--continue-on-error`: Continue processing remaining items if one fails
  (default: true)
...
**Render for Ansible** Outputs a clean YAML inventory file compatible with
Ansible (strips internal metadata).
