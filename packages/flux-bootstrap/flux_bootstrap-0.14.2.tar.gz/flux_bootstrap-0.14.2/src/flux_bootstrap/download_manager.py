...
class DownloadManager:
    ...
    def __init__(
        ...
        self._last_progress_bytes = 0  # Bytes downloaded at last progress emission
    ...
    async def download_part(
        ...
        Returns IMMEDIATELY after bytes are on disk. Does NOT wait for
        verification slot - caller handles notification separately.
        ...
        Returns:
            Dict with part info for notification if successful, None if failed
        """
        ...
        while True:
            ...
            if success:
                ...
                # Return part info for notification (caller handles backpressure)
    ...
    async def _download_part_once(
        ...
        Uses buffered writes to decouple network speed from disk speed.
        Network reads go into a memory buffer, background task writes to disk.
        This prevents disk I/O from slowing down network throughput.
        ...
        # Create write buffer queue - decouples network from disk I/O
        write_queue: asyncio.Queue[bytes | None] = asyncio.Queue(
            maxsize=WRITE_BUFFER_CHUNKS
        )
        writer_error: Exception | None = None
        writer_task: asyncio.Task | None = None

        async def disk_writer() -> None:
            """Background task that writes chunks from queue to disk."""
            nonlocal writer_error
            try:
                async with aiofiles.open(filepath, "wb") as f:
                    while True:
                        chunk = await write_queue.get()
                        if chunk is None:  # Sentinel signals completion
                            break
                        await f.write(chunk)
        ...
        try:
            async with session.get(url) as response:
                ...
                # Start background disk writer
                writer_task = asyncio.create_task(disk_writer())

                # Read from network into buffer (fast - doesn't wait for disk)
                bytes_downloaded = 0
                async for chunk in response.content.iter_chunked(
                    DOWNLOAD_CHUNK_SIZE
                ):
                    # Check for writer error before queueing more data
                    if writer_error:
                        raise writer_error

                    # Put chunk in buffer (blocks only if buffer full)
                ...
                # Check for writer error after completion
                if writer_error:
                    raise writer_error
            ...
            # Return success - caller handles notification with backpressure
            return (True, served_by)

        except SlowDownloadError as e:
            # Already logged above
            if writer_task and not writer_task.done():
                await write_queue.put(None)  # Signal writer to stop
                writer_task.cancel()
    ...
    async def stream_verified_part(
        ...
        # Add verified part's bytes to progress tracking for accurate percentage
    ...
    def _emit_progress(self, part_id: int) -> None:
        ...
        # Calculate aggregate speed from total bytes delta since last progress update
        now = time.monotonic()
        time_delta = now - self._last_progress_emit
        bytes_delta = self._bytes_downloaded - self._last_progress_bytes

        # Calculate aggregate speed (0.0 if no time has passed)
        if time_delta > 0:
            speed_bytes_per_sec = bytes_delta / time_delta
            overall_speed = (speed_bytes_per_sec * 8) / 1_000_000  # Convert to Mbps
        else:
            overall_speed = 0.0
        ...
        progress_data = {
            ...
            "percent": percent,
            "speed_mbps": overall_speed,  # True aggregate speed from total bytes
