...
class MockInstructionPacket(MockFeetechPacket):
    ...
    def ping(
        ...
        """
        return cls.build(
            scs_id=scs_id, params=[], length=2, instruction=scs.INST_PING
        )
    ...
    def sync_read(
        ...
        return cls.build(
            scs_id=scs.BROADCAST_ID,
            params=params,
            length=length,
            instruction=scs.INST_SYNC_READ,
        )
    ...
    def sync_write(
        ...
        return cls.build(
            ...
            params=params,
            length=length,
            instruction=scs.INST_SYNC_WRITE,
        )
...
class MockStatusPacket(MockFeetechPacket):
    ...
    @classmethod
    def read(
        cls, scs_id: int, value: int, param_length: int, error: int = 0
    ) -> bytes:
...
class MockMotors(MockSerial):
    ...
    def build_read_stub(
        ...
        return_packet = (
            MockStatusPacket.read(scs_id, value, length, error)
            if reply
            else b''
        )
