...
✅ Step 1-3 complete with explicit user confirmation to proceed
...
**Always include** remote config for all agents:
```yaml
remote:
  protocol: "a2a"
  enabled: false
...
This allows easy switching between local and remote execution.
...
**🔍 CRITICAL**: When agents need to access files or databases, they **MUST** use `project_dir` for absolute paths.
...
- ✅ **Database operations**: `sqlite_query`, `sqlite_execute`, `sqlite_schema`
- ✅ **File operations**: `fs_listdir`, `fs_move_file`, `fs_makedirs`, `fs_delete`
- ✅ **Document operations**: Any agent referencing file paths
- ❌ **Not needed**: Agents that only process text/data without file system access
...
- ❌ **Wrong**: `"Database path: projects/ensemble/data/ag/ag_database.db (relative)"`
- ✅ **Correct**: `"Resolve as '<project_dir>/data/ag/ag_database.db' (absolute)"`
- ❌ **Wrong**: Not passing `project_dir` in agent YML inputs
- ✅ **Correct**: Include `- Project Directory: {{project_dir}}` in `inputs.inline`
...
- Agent ID: `article_research_analyst` → Title: "Research Analyst"
- Agent ID: `contract_analyzer_extractor` → Title: "Extractor"
