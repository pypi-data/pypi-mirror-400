...
from .exceptions import ProviderError, NetworkError, RateLimitError
...
class SimulatedDataProvider(DataProvider):
    ...
    def get_historical_data(self, request: DataRequest) -> DataResponse:
        ...
        for symbol in request.symbols:
            ...
            highs = prices * (1 + np.abs(np.random.normal(0, intraday_vol/4, n_periods)))
            lows = prices * (1 - np.abs(np.random.normal(0, intraday_vol/4, n_periods)))
...
class FreeDataProviderManager:
    ...
    def get_available_symbols(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            'commodities': [
                'GC=F', 'SI=F', 'CL=F', 'NG=F', 'ZC=F', 'ZS=F'  # Gold, Silver, Oil, Gas, Corn, Soybeans
            ],
            'economic_indicators': [
                'GDP', 'UNRATE', 'CPIAUCSL', 'FEDFUNDS', 'DGS10', 'DGS2'
            ]
        }
