...
from app.db.strategies.tables_strategies import Strategy, StrategyStatus
...
def build_strategy_response(strategy: Strategy) -> StrategyResponse:
    ...
    try:
        ...
        if hasattr(strategy, 'status') and strategy.status:
            status_value = strategy.status.value if hasattr(strategy.status, 'value') else str(strategy.status)
...
def build_detail_response(strategy: Strategy) -> StrategyDetailResponse:
    ...
    return StrategyDetailResponse(
        ...
        embed=embed,
        is_public=strategy.is_public,
        status=strategy.status.value if strategy.status else "executing",
        error=strategy.error,
        created_at=strategy.created_at,
        updated_at=strategy.updated_at,
...
async def list_strategies(
    ...
    strategies = crud_strategies.list_strategies(db, user_id, status_filter)
