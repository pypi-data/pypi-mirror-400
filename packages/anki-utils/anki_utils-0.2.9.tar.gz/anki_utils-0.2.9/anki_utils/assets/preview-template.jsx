...
const {
  ...
  THEME_LABELS,
  CLOZE_PATTERN,
  getCardCSS: _getCardCSS,  // Renamed: wraps with local THEME_CSS below
  getImageSource,
  escapeHtml,
  getClozeNumbers,
...
const ACTIONS_WITHOUT_VALUE = new Set(['clear']);
...
export default function AnkiReviewer() {
  ...
  const [editAction, setEditAction] = useState('replace');
  ...
  const editableFields = useMemo(() => getEditableFields(currentCard), [currentCard]);
  const requiresEditValue = editAction && !ACTIONS_WITHOUT_VALUE.has(editAction);
  const canSaveEdit = Boolean(editField) && (!requiresEditValue || editValue.trim());

  useEffect(() => {
    if (!editField && editableFields.length > 0) {
      setEditField(editableFields[0].value);
    }
  }, [editField, editableFields]);

  useEffect(() => {
    if (ACTIONS_WITHOUT_VALUE.has(editAction)) {
      setEditValue('');
    }
  }, [editAction]);
  ...
  const handleActionClick = useCallback((status) => {
    ...
    if (status === 'needs_edit') {
      ...
      const fallbackField = editableFields[0]?.value || '';
      setEditField(existingEdit.field || fallbackField);
      setEditAction(existingEdit.action || 'replace');
      setEditValue(existingEdit.value || '');
    } else {
  ...
  }, [currentIndex, totalCards, feedback, returnToSummary, editableFields]);

  const submitEdit = useCallback(() => {
    const editPayload = {
      field: editField,
      action: editAction,
      value: ACTIONS_WITHOUT_VALUE.has(editAction) ? '' : editValue,
    };
  ...
  }, [currentIndex, editField, editAction, editValue, totalCards, returnToSummary, currentCard]);
  ...
  const generateExportData = () => {
    const instructions = "Apply the following feedback to the card batch. Each cardInstance includes an explicit templateKey and optional edit payload with a target field. Remove only the referenced card instance (not the entire note if it has multiple cards).";
          ...
          <div className="app-container">
            <div className="onboarding">
              ...
              <div className="onboarding-instructions">
                ...
                <p>• Tap cards to flip between front and back</p>
                <p>• Use the buttons to approve, edit, or remove</p>
                <p>• For edits, select the field and action, then provide the exact value</p>
                ...
                return (
                  <div key={idx} className="summary-card" onClick={() => goToCard(idx)}>
                    ...
                    <div className="summary-card-info">
                      ...
                      {fb.note && <div className="summary-card-note">✎ {fb.note}</div>}
                    </div>
      ...
      <div className="preview-wrapper">
        <div className="app-container">
          ...
          <div className="keyboard-hints">
            ...
            <span className="keyboard-hint"><kbd>A</kbd> Approve</span>
          ...
          {!hasFlippedOnce && <div className="flip-hint">Tap card to flip</div>}
          ...
          <div className={`edit-overlay ${showEditOverlay ? 'visible' : ''}`}>
            <div className="edit-title">Specify the exact change</div>
            ...
            {requiresEditValue && (
              <div className="edit-row">
                <label className="edit-label">New value</label>
                <input
                  className="edit-input"
                  placeholder="Enter the exact replacement text"
                  value={editValue}
            ...
            <div className="edit-help">
              Changes are applied to the selected note field for this specific card instance.
            </div>
            <div className="edit-buttons">
              <button className="edit-cancel-btn" onClick={cancelEdit}>Cancel</button>
              <button className="edit-submit-btn" onClick={submitEdit} disabled={!canSaveEdit}>Save Edit</button>
            </div>
          </div>
        </div>
      </div>
