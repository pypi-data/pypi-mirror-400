...
async def resolve_future(loop, future, value):
        future.set_result(value)      
        future = loop.create_future()
        return future

async def outofscope_promise(future):
    '''
        out of scope services holds a future and resolves on event with future.set_result(value)
        this out of scope service holds the future and 
        a map to future could be used to decouple in-scope promise from outofscope future
        i.e. outofscope_promise(key)
    '''
    value = await future
...
async def await_outofscope(future):
        await future
...
class MQTTClient:
    ...
    async def get_promise(self):
        return await self.isSubscribed
    ...
    async def is_subscribed(self):
        ...
        #value = await outofscope_promise(self.isSubscribed)

        value = await self.isSubscribed
     

        msg(f'<is_subscribed value: {value}')
     #   return value
        return Promise.insert(value)
    ...
    def _on_subscribe(self, client, userdata, mid, reason_code_list, properties):
        ...
        #self.resolve(True)
        self.isSubscribed =  asyncio.run_coroutine_threadsafe(resolve_future(self.loop, self.isSubscribed, True),self.loop)       
        #self.isSubscribed.set_result(True)

        #self.isSubscribed = renew_promise(self.subscribe_function).__await__()
        #self.isSubscribed = self.loop.create_future()


        #asyncio.run_coroutine_threadsafe(self.isSubscribed.resolve(True), self.loop)
        msg('<_on_subscribe ')
        #return True
        ...
        try:
                ...
                if "frame" not in payload_json or "doc" not in payload_json:
                    ...
                    '''
                        ...
                        do not need self.got_message since on_message handles this
                    '''
                    greenMsg('MQTTClient: on_message: run coroutine threadsafe: calling resolve_response')
    ...
    async def publish_with_response(self, topic: str, args):
        ...
        try: 
            ...
            promise = Promise(lambda resolve, reject : self.resolve if self.resolve is not None else self.reject)
