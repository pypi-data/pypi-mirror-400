    ...
    for index, network in enumerate(networks):
        ...
        print(
            formatting.table(
                ...
                % (network.to_json()["networkAddress"], index + 1, len(networks)),
        ...
        if skip_confirmation or formatting.ask("Accept changes?"):
            ...
            try:
                ...
                if not dry:
                    ...
                    kwargs["flagsToSet"] = [
                        f for f in network_json["flagsToEnable"] if f != "UNVERIFIED"
                    ]
                    kwargs["verified"] = (
                        "UNVERIFIED" not in network_json["flagsToEnable"]
                    )
            ...
            # to the summary report and log an error.
                ...
                if as_json:
                    summary.append(
                        {
                            "timestamp": datetime.now().isoformat(),
                            "message": "Failed to create network %s on Argus: %s"
                    ...
                    summary.append(
                        failure(
                            "Failed to create network %s on Argus: %s"
                            % (network.to_json()["networkAddress"], error)
                        )
                    )
                    log.error(summary[-1])
