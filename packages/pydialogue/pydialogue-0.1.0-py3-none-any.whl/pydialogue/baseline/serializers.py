...
def serialize(elem, world):
    ...
    ..  code-block:: python
        ...
        ['<sent>',
          '<desc>',
            '<Arg-PAG>', '<entity>', 'small', 'person', '</entity>', '</Arg-PAG>',
            '<Rel>', 'has', '</Rel>',
            '<Arg-PPT>', '<set>', '<entity>', 'red', 'apple', '</entity>', '<entity>', 'chicken', '</entity>', '</set>',
            '</Arg-PPT>',
          '</desc>',
         '</sent>']
    ...
    if isinstance(elem, lc.Sentence):
        ...
        for delem in description.elements:
            if delem in elem.properties:
                if not isinstance(elem.properties[delem], (tuple, list)):
                ...
                if isinstance(delem, tuple):
                    elem_vals.extend(serialize(delem, world))
                else:
...
def deserialize(cmds, world, stack=None):
    ...
    for cmd in cmds:
        ...
        elif cmd.startswith('<Arg-') or cmd.startswith('<AM-') or cmd == '<Rel>':
            if isinstance(stack[-1], lc.Describer):
                if cmd == '<Rel>':
                    new_arg = lc.RelArg()
                else:
                    new_arg = lc.Arg()
                stack[-1].args[cmd[1:-1]] = new_arg
        ...
        for desc in world.all_description_objects:
            for celem in desc.cand_elements:
                if elem_to_attach in world.all_attributes or elem_to_attach == 'abstract':
                    ...
                    elem_attached = True
                elif celem in world.all_properties \
                        and ((isinstance(celem, str) and elem_to_attach in getattr(world, celem+"s"))
                            or (isinstance(celem, tuple) and elem_to_attach in getattr(world, "_".join(celem)+"s"))):
                    elem.properties[celem] = elem_to_attach
                    elem_attached = True
                elif isinstance(celem, tuple) and isinstance(elem_to_attach, list) and tuple(elem_to_attach) in world.all_attributes:
                    elem.attributes[tuple(elem_to_attach)] = None
                    elem_attached = True
                elif elem_to_attach in ['entity']: #'the'
                    elem_attached = True
