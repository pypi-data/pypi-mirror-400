...
from labelme.widgets import (AiPromptWidget, BrightnessContrastDialog, Canvas,
                             FileDialogPreview, ImageGridViewer, LabelDialog,
                             LabelListWidget, LabelListWidgetItem, StatusStats,
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        self.fileListWidget.customContextMenuRequested.connect(
            self._show_file_list_context_menu
        )
        ...
        # 使用 QStackedWidget 来管理单图模式和网格视图模式的切换
        self.scrollArea = scrollArea
        self.centralStack = QtWidgets.QStackedWidget()
        self.centralStack.addWidget(scrollArea)  # index 0: 单图模式
        ...
        openGridView = action(
            self.tr("Grid View"),
            self._open_grid_view,
            None,
            icon="circles-four.svg",
            tip=self.tr("Open image grid viewer"),
            enabled=False,
        )
        openNextImg = action(
            self.tr("&Next Image"),
...
""",
    ...
    def _show_file_list_context_menu(self, position):
        ...
        copy_filename_action = menu.addAction(self.tr("复制文件名"))
        copy_path_action = menu.addAction(self.tr("复制文件路径"))

        action = menu.exec_(self.fileListWidget.mapToGlobal(position))
        if action == copy_filename_action:
            clipboard = QtWidgets.QApplication.clipboard()
            clipboard.setText(osp.basename(item.text()))
    ...
    def fileSelectionChanged(self):
        ...
        if currIndex < len(self.imageList):
            ...
            if filename:
                ...
                # 同步网格视图的选中状态
                if self._grid_viewer is not None:
                    self._grid_viewer.set_selected_image(filename)
    ...
    def remLabels(self, shapes):
        # 临时断开 itemDropped 信号，避免删除时触发 labelOrderChanged
        self.labelList.itemDropped.disconnect(self.labelOrderChanged)
        for shape in shapes:
            item = self.labelList.findItemByShape(shape)
    ...
    def _open_prev_image(self, _value=False) -> None:
        ...
        if current_row < 0:
            logger.debug("no current row selected")
            return
        row_prev: int = current_row - 1
        if row_prev < 0:
    ...
    def _open_next_image(self, _value=False) -> None:
        ...
        if current_row < 0:
            ...
            if self.fileListWidget.count() > 0:
                logger.debug("no current row selected, selecting first row")
                self.fileListWidget.setCurrentRow(0)
                self.fileListWidget.repaint()
            else:
                logger.debug("no images in list")
            return
        row_next: int = current_row + 1
    ...
    def deleteFile(self):
        ...
        if osp.exists(label_file):
            ...
            # 保存当前文件名，删除标签后重新加载图片（不带标注）
            current_filename = self.filename
            self.resetState()
            if current_filename:
                self._load_file(current_filename)
    ...
    def _open_grid_view(self, _value: bool = False) -> None:
        """切换图片网格视图模式"""
        if not self.imageList:
            QtWidgets.QMessageBox.warning(
                self,
                self.tr("Warning"),
                self.tr("Please open a directory first."),
            )
            return
        
        if self._grid_view_active:
            # 切换回单图编辑模式
            self._switch_to_single_image_mode()
        else:
            # 切换到网格视图模式
            self._switch_to_grid_view_mode()
    
    def _switch_to_grid_view_mode(self):
        """切换到网格视图模式"""
        # 如果网格浏览器还不存在，创建它
        if self._grid_viewer is None:
            self._grid_viewer = ImageGridViewer(
                image_list=self.imageList,
                get_annotation_status=self._has_annotation,
                get_label_file_path=self._get_label_file_path,
                parent=self
            )
            # 连接图片点击信号
            self._grid_viewer.imageClicked.connect(self._on_grid_image_clicked)
            # 添加到 stack 中
            self.centralStack.addWidget(self._grid_viewer)  # index 1: 网格视图
        else:
            # 刷新网格视图
            self._grid_viewer.refresh()
        
        # 切换到网格视图
        self.centralStack.setCurrentWidget(self._grid_viewer)
        
        # 更新状态
        self._grid_view_active = True
        
        # 更新按钮状态
        self.actions.openGridView.setToolTip(self.tr("Exit grid view"))
    
    def _switch_to_single_image_mode(self):
        """切换回单图编辑模式"""
        # 切换回单图模式
        self.centralStack.setCurrentWidget(self.scrollArea)
        
        # 更新状态
        self._grid_view_active = False
        
        # 恢复按钮状态
        self.actions.openGridView.setToolTip(self.tr("Open image grid viewer"))
    
    def _on_grid_image_clicked(self, image_path: str):
        """网格视图中的图片被点击"""
        # 切换回单图模式
        self._switch_to_single_image_mode()
        
        # 加载选中的图片
        self._load_file(image_path)
    ...
    def _has_annotation(self, image_path: str) -> bool:
        ...
        return osp.exists(label_file) and LabelFile.is_label_file(label_file)
    
    def _get_label_file_path(self, image_path: str) -> str | None:
        """获取图片对应的标注文件路径"""
        label_file = osp.splitext(image_path)[0] + ".json"
        if self.output_dir:
            label_file = osp.join(self.output_dir, osp.basename(label_file))
    ...
    def _import_images_from_dir(
        ...
        if self.fileListWidget.count() > 0:
            self.actions.openNextImg.setEnabled(True)
            self.actions.openPrevImg.setEnabled(True)
            self.actions.openGridView.setEnabled(True)
    ...
    def _update_status_stats(self, mouse_pos: QtCore.QPointF) -> None:
        ...
        if self.image and not self.image.isNull():
            stats.append(f"{self.image.width()} × {self.image.height()}像素")
        
        # 显示文件大小
        if self.imagePath and osp.exists(self.imagePath):
            file_size = osp.getsize(self.imagePath)
            if file_size >= 1024 * 1024:
                stats.append(f"大小: {file_size / (1024 * 1024):.1f}MB")
            else:
                stats.append(f"大小: {file_size / 1024:.1f}KB")
        
        # 显示缩放比例
        stats.append(f"缩放: {int(self.zoomWidget.value())}%")
        
        # 显示鼠标坐标
        stats.append(f"X={int(mouse_pos.x())}, Y={int(mouse_pos.y())}")
        ...
        if self.image and not self.image.isNull():
            if 0 <= x < self.image.width() and 0 <= y < self.image.height():
                ...
                stats.append(f"RGB({color.red()},{color.green()},{color.blue()})")
    ...
    def _update_selection_stats(self) -> None:
        ...
        if self.canvas.selectedShapes:
            for shape in self.canvas.selectedShapes:
                ...
                w = int(rect.width())
                h = int(rect.height())
                self.status_left.setText(f"选区: {w}×{h}像素 (面积: {w*h}像素)")
                break
        else:
