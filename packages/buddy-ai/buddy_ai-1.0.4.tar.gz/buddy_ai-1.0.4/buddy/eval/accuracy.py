...
from buddy.eval.utils import async_log_eval_run, log_eval_run, store_result_in_file
from buddy.exceptions import EvalError
from buddy.models.base import Model
from buddy.team.team import Team
from buddy.utils.log import logger, set_log_level_to_debug, set_log_level_to_info
...
class AccuracyEval:
    ...
    # Enable debug logs
    debug_mode: bool = getenv("BUDDY_DEBUG", "false").lower() == "true"
    # Log the results to the Buddy platform. On by default.
    ...
    def get_evaluator_agent(self) -> Agent:
        ...
        if model is None:
            try:
                ...
                logger.exception(e)
                raise EvalError(
                    "Buddy uses `openai` as the default model provider. Please run `pip install openai` to use the default evaluator."
                )
...
""",
    ...
    def run(
        ...
        *,
        ...
        # Log results to the Buddy platform if requested
        if self.agent is not None:
