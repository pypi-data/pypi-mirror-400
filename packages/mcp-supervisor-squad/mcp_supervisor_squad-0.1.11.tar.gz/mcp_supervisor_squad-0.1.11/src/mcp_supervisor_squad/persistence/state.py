...
class RunState:
    ...
    phase: str = "planner_run"  # planner_run|review|coder_run|integrate|cleanup|done|canceled|error
    next_action: str = "wait"  # wait|apply
    wait_reason: str = "idle"  # agent_running|review_window|cleanup_running|blocked|idle|ready_to_apply|canceled|error
    ...
    def compute_recommended_poll_ms(self, *, now: datetime | None = None) -> int:
        ...
        if self.phase == "review" or self.wait_reason == "review_window":
            remaining_ms = 15_000
            try:
                deadline = _parse_iso(self.review_expires_at)
                if deadline:
                    remaining_ms = max(1_000, int((deadline - now_dt).total_seconds() * 1000))
            except Exception:
                remaining_ms = 15_000
            return min(max(remaining_ms, phase_floor), self._poll_cap_ms())
    ...
    def public_status(self, *, options: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if cache:
            ...
            if elapsed_ms < int(cache.recommended_poll_ms):
                ...
                lines = [base, f"无新进展 | 强制等待: {remaining_ms//1000}s"]
                return {
                    "run_id": self.run_id,
        ...
        try:
            ...
            if activity_age is not None and self.phase in {"planner_run", "coder_run"}:
                activity_hint = f"活跃: {activity_age}s"
            if self.phase == "review" and self.review_expires_at:
                try:
                    deadline = _parse_iso(self.review_expires_at)
                    if deadline:
                        remain = max(0, int((deadline - now).total_seconds()))
                        review_countdown = f"自动继续: {remain}s"
                except Exception:
                    review_countdown = ""
        except Exception:
            activity_hint = ""
        ...
        lines: list[str] = [
            ...
            task_summary,
            f"最近: {self.important_event} | 已等待: {elapsed_s}s"
            + (f" | {review_countdown}" if review_countdown else "")
            + (f" | {activity_hint}" if activity_hint else "")
            + (f" | {artifact_hint}" if artifact_hint else "")
            + f" | 强制等待: {poll_ms//1000}s",
        ]
