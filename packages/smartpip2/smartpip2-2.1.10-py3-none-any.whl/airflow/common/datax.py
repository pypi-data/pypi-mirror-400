...
import codecs
import platform
from airflow.settings import DATAX_HOME
...
def datax_cmdStr(argv):
    parser = getOptionParser()
    options, args = parser.parse_args(argv)
    return buildStartCommand(options, args)

def flinkcdc_cmdStr(argv):
    parser = getOptionParser()
    options, args = parser.parse_args(argv)
    cmdStr = f'{DATAX_HOME}/flinkcdc/bin/flink-cdc.sh {args[0]}'
    return cmdStr



if __name__ == "__main__":
    sys.exit(datax_main(sys.argv[1:]))
