...
class BuddyLogger(logging.Logger):
...
def build_logger(logger_name: str, source_type: Optional[str] = None) -> Any:
    ...
    logging.setLoggerClass(BuddyLogger)
    ...
    rich_handler = ColoredRichHandler(
        ...
        show_path=True if getenv("BUDDY_API_RUNTIME") == "dev" else False,
...
agent_logger: BuddyLogger = build_logger(LOGGER_NAME, source_type="agent")
team_logger: BuddyLogger = build_logger(TEAM_LOGGER_NAME, source_type="team")
workflow_logger: BuddyLogger = build_logger(WORKFLOW_LOGGER_NAME, source_type="workflow")

# Set the default logger to the agent logger
logger: BuddyLogger = agent_logger
