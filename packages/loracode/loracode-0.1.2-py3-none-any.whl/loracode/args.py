...
from loracode.args_formatter import (
    DotEnvFormatter,
    MarkdownHelpFormatter,
    YamlHelpFormatter,
)
from loracode.deprecated import add_deprecated_model_args

from .dump import dump  # noqa: F401


def resolve_loracodeignore_path(path_str, git_root=None):
    path = Path(path_str)
    if path.is_absolute():
        return str(path)
    elif git_root:
...
def get_parser(default_config_files, git_root):
    parser = configargparse.ArgumentParser(
        description="Lora Code - AI pair programming in your terminal",
        add_config_file_help=True,
        default_config_files=default_config_files,
        config_file_parser_class=configargparse.YAMLConfigFileParser,
        auto_env_var_prefix="LORACODE_",
    )
    from loracode import coders as _loracode_coders

    edit_format_choices = sorted(
        {
            c.edit_format
            for c in _loracode_coders.__all__
    ...
    group = parser.add_argument_group("Lora Code API settings")
    group.add_argument(
        "--api-key",
        metavar="API_KEY",
        default=os.environ.get("LORA_CODE_API_KEY"),
        help="Specify the Lora Code API key (or set LORA_CODE_API_KEY env var)",
    )
    group.add_argument(
        "--api-base",
        metavar="API_BASE_URL",
        default=os.environ.get("LORA_CODE_API_BASE", "https://api.loratech.dev"),
        help="Specify the Lora Code API base URL (default: https://api.loratech.dev)",
    )
    group.add_argument(
        "--no-cache-credentials",
        action="store_true",
        default=False,
        help="Disable credential caching (default: False)",
    )
    group.add_argument(
        "--set-env",
        action="append",
    ...
    group.add_argument(
        ...
        default=".loracode.model.settings.yml",
        help="Specify a file with loracode model settings for unknown models",
    ).complete = shtab.FILE
    group.add_argument(
        "--model-metadata-file",
        metavar="MODEL_METADATA_FILE",
        default=".loracode.model.metadata.json",
    ...
    default_input_history_file = (
        os.path.join(git_root, ".loracode.input.history") if git_root else ".loracode.input.history"
    )
    default_chat_history_file = (
        os.path.join(git_root, ".loracode.chat.history.md") if git_root else ".loracode.chat.history.md"
    )
    group.add_argument(
        "--input-history-file",
    ...
    group.add_argument(
        ...
        default=None,
        help="Log the conversation with the LLM to this file (for example, .loracode.llm.history)",
    ).complete = shtab.FILE
    ...
    group.add_argument(
        "--user-input-color",
        default="#8B8B8B",
        help="Set the color for user input (default: #8B8B8B)",
    )
    group.add_argument(
        "--tool-output-color",
        default="#6B6B6B",
        help="Set the color for tool output (default: #6B6B6B)",
    )
    group.add_argument(
        "--tool-error-color",
        default="#E06C75",
        help="Set the color for tool error messages (default: #E06C75)",
    )
    group.add_argument(
        "--tool-warning-color",
        default="#E5C07B",
        help="Set the color for tool warning messages (default: #E5C07B)",
    )
    group.add_argument(
        "--assistant-output-color",
        default="#ABB2BF",
        help="Set the color for assistant output (default: #ABB2BF)",
    )
    group.add_argument(
    ...
    group.add_argument(
        ...
        default="one-dark",
        help=(
            "Set the markdown code theme (default: one-dark, other options include monokai,"
    ...
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Enable/disable adding .loracode* to .gitignore (default: True)",
    )
    group.add_argument(
        "--add-gitignore-files",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Enable/disable the addition of files listed in .gitignore to LoraCode's editing scope.",
    )
    default_loracodeignore_file = (
        os.path.join(git_root, ".loracodeignore") if git_root else ".loracodeignore"
    )

    group.add_argument(
        "--loracodeignore",
        metavar="LORACODEIGNORE",
        type=lambda path_str: resolve_loracodeignore_path(path_str, git_root),
        default=default_loracodeignore_file,
        help="Specify the loracode ignore file (default: .loracodeignore in git root)",
    ).complete = shtab.FILE
    group.add_argument(
        "--subtree-only",
        action="store_true",
    ...
    group.add_argument(
        ...
        default=None,
        help=(
            "Attribute loracode code changes in the git author name (default: True). If explicitly set"
    ...
    group.add_argument(
        ...
        default=None,
        help=(
            "Attribute loracode commits in the git committer name (default: True). If explicitly set"
            " to True, overrides --attribute-co-authored-by precedence for loracode edits."
        ),
    )
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Prefix commit messages with 'loracode: ' if loracode authored the changes (default: False)",
    )
    group.add_argument(
        "--attribute-commit-message-committer",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Prefix all commit messages with 'loracode: ' (default: False)",
    )
    group.add_argument(
        ...
        default=True,
        help=(
            "Attribute loracode edits using the Co-authored-by trailer in the commit message"
    ...
    group.add_argument(
        "--check-update",
        action=argparse.BooleanOptionalAction,
        help="Check for new loracode versions on launch",
        default=True,
    )
    group.add_argument(
        "--show-release-notes",
    ...
    group.add_argument(
        ...
        "--update",
        action="store_true",
        help="Upgrade loracode to the latest version from PyPI",
        default=False,
    )
    group.add_argument(
        "--version",
    ...
    group.add_argument(
        ...
        "--browser",
        action=argparse.BooleanOptionalAction,
        help="Run loracode in your browser (default: False)",
        default=False,
    )
    group.add_argument(
        "--copy-paste",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Enable automatic copy/paste of chat between loracode and web UI (default: False)",
    )
    group.add_argument(
        "--apply",
        metavar="FILE",
    ...
    group.add_argument(
        ...
        default=None,
        choices=["en", "tr"],
        help="Specify the UI language (en=English, tr=Türkçe) (default: en)",
    )
    group.add_argument(
        "--commit-language",
    ...
    group.add_argument(
        ...
        metavar="CONFIG_FILE",
        help=(
            "Specify the config file (default: search for .loracode.conf.yml in git root, cwd"
            " or home directory)"
        ),
    ).complete = shtab.FILE
    group.add_argument(
    ...
    group.add_argument(
        ...
        help=(
            ...
            f" {', '.join(supported_shells_list)}. Example: loracode --shell-completions bash"
        ),
    )
