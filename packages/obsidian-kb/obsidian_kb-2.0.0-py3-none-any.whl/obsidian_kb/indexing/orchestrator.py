...
class IndexingOrchestrator:
    ...
    async def run_job(
        ...
        try:
            ...
            for i, file_path in enumerate(job.paths):
                ...
                try:
                    ...
                    logger.debug(
                        f"Processed document {i+1}/{len(job.paths)}: {file_path}"
                    )
    ...
    async def _store(
        ...
        for i, (enriched_chunk, embedding) in enumerate(
            ...
            chunk_id = f"{vault_name}::{processed.file_path}::{i}"
            ...
            if processed.file_path.is_absolute():
                # Нужно найти vault_path для вычисления относительного пути
                # Пока используем имя файла
                relative_path = processed.file_path.name
            else:
                relative_path = str(processed.file_path)
        ...
        logger.debug(
            f"Stored {len(chunks)} chunks for document {document.enriched.processed.file_path}"
        )
