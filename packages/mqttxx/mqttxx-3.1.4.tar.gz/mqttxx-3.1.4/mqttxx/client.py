...
class MQTTClient:
    ...
    def subscribe(self, pattern: str, handler: Callable[[str, bytes], Any]):
        ...
        if self._client:
            asyncio.create_task(self._do_subscribe(pattern))
        else:
            logger.info(f"订阅已队列化 (等待连接) - pattern: {pattern}")

        logger.debug(f"订阅成功 - pattern: {pattern}")
    ...
    def unsubscribe(self, pattern: str, handler: Optional[Callable] = None):
        ...
        if handler is None:
            ...
            if self._client:
                asyncio.create_task(self._client.unsubscribe(pattern))
