...
class ADCP():
    ...
    def _generate_velocity_data_masks(self) ->None:
        ...
        * and, if bottom track is active, a bottom-track-based mask
        ...
        if self.masking.err_vel_max == 'auto':
            ...
            ev_mask = (ev > self.masking.err_vel_max)
    ...
    def _generate_beam_data_masks(self) -> None:
        ...
        * and, if bottom track is active, a bottom-track-based mask
        ...
        if self._bt_mode_active:
            ...
            master_mask = np.logical_or.reduce([cmag_mask, echo_mask, pg_mask, sv_mask, ens_mask, dt_mask, ~bt_mask])
        else:
           master_mask = np.logical_or.reduce([cmag_mask, echo_mask, pg_mask, sv_mask, ens_mask, dt_mask])
    ...
    def _backscatter_to_ssc(self,backscatter):
        ...
        if background_ssc_mode.lower() == 'percentile':
            background_ssc = np.nanpercentile(ssc, self.corrections.background_ssc)
        mask = ~np.isnan(ssc)
