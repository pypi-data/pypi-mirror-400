...
from flask import render_template, render_template_string, flash as _flash
...
def flash_with_markup(message: str, category: str = "message") -> None:
    """
    This converts Markup into string and will escape any HTML tags if a string is passed.
    """
    if isinstance(message, Markup):
        _flash(str(message), category)
    else:
        _flash(str(Markup.escape(message)), category)
