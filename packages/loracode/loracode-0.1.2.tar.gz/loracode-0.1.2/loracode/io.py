...
NOTIFICATION_MESSAGE = "Lora Code is waiting for your input"


_C_RESET = "\033[0m"
_C_DARK = "\033[38;2;45;45;45m"
_C_GRAY = "\033[38;2;90;90;90m"
_C_LIGHT = "\033[38;2;160;160;160m"
_C_WHITE = "\033[38;2;200;200;200m"

LORACODE_BANNER = f"""
{_C_DARK}╭────────────────────────────────────────────────────────────────╮{_C_RESET}
{_C_DARK}│{_C_RESET}                                                                {_C_DARK}│{_C_RESET}
{_C_DARK}│{_C_RESET}  {_C_LIGHT}██      ██████  ██████   █████      ██████  ██████  ██████  ███████{_C_RESET}{_C_DARK}│{_C_RESET}
{_C_DARK}│{_C_RESET}  {_C_LIGHT}██      ██  ██  ██  ██  ██   ██    ██      ██  ██  ██   ██ ██{_C_RESET}     {_C_DARK}│{_C_RESET}
{_C_DARK}│{_C_RESET}  {_C_LIGHT}██      ██  ██  ██████  ███████    ██      ██  ██  ██   ██ █████{_C_RESET}  {_C_DARK}│{_C_RESET}
{_C_DARK}│{_C_RESET}  {_C_LIGHT}██      ██  ██  ██  ██  ██   ██    ██      ██  ██  ██   ██ ██{_C_RESET}     {_C_DARK}│{_C_RESET}
{_C_DARK}│{_C_RESET}  {_C_LIGHT}███████ ██████  ██  ██  ██   ██     ██████ ██████  ██████  ███████{_C_RESET}{_C_DARK}│{_C_RESET}
{_C_DARK}│{_C_RESET}                                                                {_C_DARK}│{_C_RESET}
{_C_DARK}│{_C_RESET}  {_C_GRAY}AI-powered coding assistant{_C_RESET}                                    {_C_DARK}│{_C_RESET}
{_C_DARK}│{_C_RESET}                                                                {_C_DARK}│{_C_RESET}
{_C_DARK}╰────────────────────────────────────────────────────────────────╯{_C_RESET}
"""

LORACODE_BANNER_MINIMAL = f"""
{_C_DARK}┌───────────────────────────────────────┐{_C_RESET}
{_C_DARK}│{_C_RESET}  {_C_LIGHT}LORA CODE{_C_RESET}  {_C_DARK}│{_C_RESET}  {_C_GRAY}AI coding assistant{_C_RESET}   {_C_DARK}│{_C_RESET}
{_C_DARK}└───────────────────────────────────────┘{_C_RESET}
"""

LORACODE_BANNER_TINY = f"{_C_LIGHT}LORA CODE{_C_RESET} {_C_DARK}·{_C_RESET} {_C_GRAY}AI assistant{_C_RESET}"
...
class InputOutput:
    ...
    THEME_COLORS = {
        "dark": {
            ...
            "tool_error": "#B06060",
            "tool_warning": "#A08050",
            "assistant_output": "#909090",
            "completion_menu": "#909090",
            "completion_menu_bg": "#1A1A1A",
            "completion_current": "#E0E0E0",
            "completion_current_bg": "#303030",
            "border": "#2A2A2A",
            "dim": "#505050",
            "accent": "#808080",
            "success": "#608060",
        ...
        "light": {
            ...
            "tool_error": "#E45649",
            "tool_warning": "#C18401",
            "assistant_output": "#383A42",
            "completion_menu": "#383A42",
            "completion_menu_bg": "#FAFAFA",
            "completion_current": "#FFFFFF",
            "completion_current_bg": "#4078F2",
            "border": "#E5E5E6",
            "dim": "#A0A1A7",
            "accent": "#4078F2",
            "success": "#50A14F",
    ...
    def __init__(
        ...
        input=None,
        output=None,
        user_input_color="#C5C5C5",
        tool_output_color="#8B8B8B",
        tool_error_color="#F44747",
        tool_warning_color="#CCA700",
        assistant_output_color="#D4D4D4",
        ...
        if self.pretty:
            theme = self.THEME_COLORS["dark"]
            if not self.user_input_color:
                self.user_input_color = theme["user_input"]
            if not self.tool_output_color:
                self.tool_output_color = theme["tool_output"]
            if not self.tool_error_color:
                self.tool_error_color = theme["tool_error"]
            if not self.tool_warning_color:
                self.tool_warning_color = theme["tool_warning"]
            if not self.assistant_output_color:
                self.assistant_output_color = theme["assistant_output"]
            
            if not self.completion_menu_color:
                self.completion_menu_color = theme["completion_menu"]
            if not self.completion_menu_bg_color:
                self.completion_menu_bg_color = theme["completion_menu_bg"]
            if not self.completion_menu_current_color:
                self.completion_menu_current_color = theme["completion_current"]
            if not self.completion_menu_current_bg_color:
                self.completion_menu_current_bg_color = theme["completion_current_bg"]
        ...
        self.append_chat_history(f"\n# loracode chat started at {current_time}\n\n")
        ...
        if fancy_input:
            ...
            try:
                ...
                self.console = Console(file=sys.stdout, force_terminal=False, no_color=True)
                self.tool_error(f"Can't initialize prompt toolkit: {err}")
        else:
            self.console = Console(file=sys.stdout, force_terminal=False, no_color=True)
            if self.is_dumb_terminal:
                self.tool_output(t("terminal.dumb_detected"))
    ...
    def _get_style(self):
        ...
        if self.user_input_color:
            ...
            style_dict.update(
                {
                    "pygments.literal.string": f"bold {self.user_input_color}",
                    "pygments.name": self.user_input_color,
                }
            )
        ...
        style_dict.update({
            "bottom-toolbar": "bg:#1A1A1A #505050",
    ...
    def read_image(self, filename):
        try:
            ...
            self.tool_error(t("file.unable_to_read", filename=filename, error=err))
            return
        except FileNotFoundError:
            self.tool_error(t("file.not_found", filename=filename))
            return
        except IsADirectoryError:
            self.tool_error(t("file.is_directory", filename=filename))
            return
    ...
    def read_text(self, filename, silent=False):
        ...
        try:
            ...
            if not silent:
                self.tool_error(f"{filename}: {e}")
                self.tool_error(t("file.encoding_hint"))
            return
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    self.tool_error(
                        t("file.unable_to_write_retries", filename=filename, retries=max_retries, error=err)
                    )
                    raise
            except OSError as err:
                self.tool_error(t("file.unable_to_write", filename=filename, error=err))
                raise
...
{_C_DARK}│{_C_RESET}  {_C_LIGHT}███████ ██████  ██  ██  ██   ██     ██████ ██████  ██████  ███████{_C_RESET}{_C_DARK}│{_C_RESET}
{_C_DARK}│{_C_RESET}                                                                {_C_DARK}│{_C_RESET}
{_C_DARK}│{_C_RESET}  {_C_GRAY}{subtitle:<60}{_C_RESET}{_C_DARK}│{_C_RESET}
{_C_DARK}│{_C_RESET}                                                                {_C_DARK}│{_C_RESET}
{_C_DARK}╰────────────────────────────────────────────────────────────────╯{_C_RESET}
"""
                print(banner)
                ...
                banner_minimal = f"""
{_C_DARK}┌───────────────────────────────────────┐{_C_RESET}
{_C_DARK}│{_C_RESET}  {_C_LIGHT}LORA CODE{_C_RESET}  {_C_DARK}│{_C_RESET}  {_C_GRAY}{subtitle[:20]}{_C_RESET}   {_C_DARK}│{_C_RESET}
{_C_DARK}└───────────────────────────────────────┘{_C_RESET}
"""
                print(banner_minimal)
            else:
                banner_tiny = f"{_C_LIGHT}LORA CODE{_C_RESET} {_C_DARK}·{_C_RESET} {_C_GRAY}{subtitle[:15]}{_C_RESET}"
                print(banner_tiny)
            
            if version:
                self.console.print(f"  [#606060]v{version}[/#606060]", end="")
                print()
        except UnicodeEncodeError:
            print()
            print(f"  LORA CODE - {t('banner.ascii_fallback')}")
            if version:
                print(f"  v{version}")
            print()
        except Exception:
            print(LORACODE_BANNER_MINIMAL)
            if version:
                print(f"  v{version}")
    ...
    def show_user_info(self, user_info):
        ...
        try:
            ...
            if display_name:
                info_parts.append(f"[#505050]●[/#505050] [#909090]{display_name}[/#909090]")
            ...
            if plan:
                ...
                plan_color = plan_colors.get(plan.lower(), "#707070")
                info_parts.append(f"[{plan_color}]{plan.capitalize()}[/{plan_color}]")
            ...
            if requests_limit > 0 and remaining == 0:
                remaining = requests_limit - requests_used
            
            requests_label = t("user.requests").lower()
            tokens_label = t("user.tokens").lower()
            remaining_label = t("user.remaining").lower()
            
            if requests_limit > 0 and requests_used >= 0:
                ...
                if pct > 50:
                    ...
                    color = "#909090"
                info_parts.append(f"[#505050]{requests_label}[/#505050] [{color}]{requests_used:,}/{requests_limit:,}[/{color}]")
            elif requests_used > 0:
                info_parts.append(f"[#505050]{requests_label}[/#505050] [#707070]{requests_used:,}[/#707070]")
            
            if tokens_limit > 0 and tokens_used >= 0:
                info_parts.append(f"[#505050]{tokens_label}[/#505050] [#707070]{tokens_used:,}/{tokens_limit:,}[/#707070]")
            elif tokens_used > 0:
                info_parts.append(f"[#505050]{tokens_label}[/#505050] [#707070]{tokens_used:,}[/#707070]")
            
            if remaining > 0 and requests_limit > 0:
                info_parts.append(f"[#505050]{remaining_label}[/#505050] [#707070]{remaining:,}[/#707070]")
            
            if self.pretty and info_parts:
                separator = " [#2A2A2A]│[/#2A2A2A] "
                self.console.print("  " + separator.join(info_parts))
            elif info_parts:
                plain_parts = []
                if display_name:
                    plain_parts.append(f"● {display_name}")
                if plan:
                    plain_parts.append(plan.capitalize())
                if requests_used > 0:
                    if requests_limit > 0:
                        plain_parts.append(f"{requests_label} {requests_used}/{requests_limit}")
                    else:
                        plain_parts.append(f"{requests_label} {requests_used}")
                if tokens_used > 0:
                    if tokens_limit > 0:
                        plain_parts.append(f"{tokens_label} {tokens_used}/{tokens_limit}")
                    else:
                        plain_parts.append(f"{tokens_label} {tokens_used}")
                if remaining > 0:
                    plain_parts.append(f"{remaining_label} {remaining}")
                print("  " + " | ".join(plain_parts))
    ...
    def get_input(
        ...
        prompt_prefix = ""
        if edit_format:
            prompt_prefix += f"[#505050]{edit_format}[/#505050] "
        if self.multiline_mode:
            prompt_prefix += "[#505050]multi[/#505050] "
        prompt_prefix += "> "
...
{prefix} {hist}"""
    ...
    def confirm_ask(
        ...
        valid_responses = [
            "yes", "no", "skip", "all",
            t("confirm.yes"), t("confirm.no"), t("confirm.skip"), t("confirm.all"),
            t("confirm.y"), t("confirm.n")
        ]
        
        options = " " + t("confirm.options_yn")
        if group:
            if not explicit_yes_required:
                options += t("confirm.options_all")
            options += t("confirm.options_skip")
        if allow_never:
            options += t("confirm.options_dont")
            valid_responses.extend(["don't", t("confirm.dont")])

        if default.lower().startswith("y"):
            question += options + " " + t("confirm.default_yes") + ": "
        elif default.lower().startswith("n"):
            question += options + " " + t("confirm.default_no") + ": "
        else:
        ...
        if self.yes is True:
            ...
            while True:
                ...
                self.tool_error(t("input.please_answer", responses=", ".join(valid_responses)))

        res = res.lower()[0]

        yes_chars = ["y", t("confirm.y")[0] if t("confirm.y") else "y"]
        no_chars = ["n", t("confirm.n")[0] if t("confirm.n") else "n"]
        all_chars = ["a", t("confirm.all")[0] if t("confirm.all") else "a"]
        skip_chars = ["s", t("confirm.skip")[0] if t("confirm.skip") else "s"]
        dont_chars = ["d", t("confirm.dont")[0] if t("confirm.dont") else "d"]

        if res in dont_chars and allow_never:
        ...
        if explicit_yes_required:
            is_yes = res in yes_chars
        else:
            is_yes = res in yes_chars or res in all_chars

        is_all = res in all_chars and group is not None and not explicit_yes_required
        is_skip = res in skip_chars and group is not None
    ...
    def _tool_message(self, message="", strip=True, color=None, prefix=None):
        ...
        if prefix and self.pretty:
            display_msg = Text(f"{prefix} ") + message
        else:
            display_msg = message
        ...
        try:
            ...
            if isinstance(display_msg, Text):
                display_msg = display_msg.plain
            display_msg = str(display_msg).encode("ascii", errors="replace").decode("ascii")
            self.console.print(display_msg, **style)
    ...
    def agent_status(self, message="", action="thinking"):
        ...
        icon = icons.get(action, icons["thinking"])
        color = "#707070"
    ...
    def assistant_output(self, message, pretty=None):
        if not message:
            self.tool_warning(t("llm.empty_response"))
            return
    ...
    def get_default_notification_command(self):
        ...
        if system == "Darwin":
            if shutil.which("terminal-notifier"):
                return f"terminal-notifier -title 'Lora Code' -message '{NOTIFICATION_MESSAGE}'"
            return (
                f'osascript -e \'display notification "{NOTIFICATION_MESSAGE}" with title "Lora Code"\''
            )
        elif system == "Linux":
            for cmd in ["notify-send", "zenity"]:
                if shutil.which(cmd):
                    if cmd == "notify-send":
                        return f"notify-send 'Lora Code' '{NOTIFICATION_MESSAGE}'"
                    elif cmd == "zenity":
    ...
    def toggle_multiline_mode(self):
        ...
        if self.multiline_mode:
            ...
            self.tool_output(t("multiline.disabled"))
