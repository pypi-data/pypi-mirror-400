...
```bash
sudo atlas setup      # Download binary, generate config
sudo atlas start      # Start monitoring daemon
sudo atlas stop       # Stop daemon
sudo atlas status     # Check status and trial info
sudo atlas logs       # View recent logs
sudo atlas claim      # Open dashboard to claim this server
sudo atlas uninstall  # Remove Atlas
```
