...
async def get_presigned_url_form(request: web.Request) -> web.Response:
    ...
    responses:
      ...
      '404':
        $ref: '#/components/responses/404'
    """
    return await _get_project(request, include_data=False)
...
async def _get_project_move_template(request: web.Request) -> web.Response:
    ...
    try:
        context = aws.S3ObjectPermissionContext(request)
        return await _get_project(request, context=context)
    except KeyDecodeException as e:
        logger.exception('Error getting parent key')
        return response.status_not_found()

async def _get_project(request: web.Request, context: PermissionContext | None = None, include_data = True) -> web.Response:
    """
    Gets the project with the specified id.
    ...
    async with DesktopObjectActionLifecycle(request=request,
        ...
        include_data = parse_bool(request.url.query.get('data') or 'true') if include_data is None else include_data
        if 'path' in request.query:
            path: list[str] | None = request.query.getall('path')
        else:
            path = None
        cache_key = (sub, volume_id, bucket_name, project_id, 'actual')
        is_full_permissions_context = isinstance(context, aws.S3ObjectPermissionContext)
        if is_full_permissions_context and (cached_value := request.app[HEA_CACHE].get(cache_key)):
            ...
            if include_data:
                async with aws.S3ClientContext(request=request, volume_id=volume_id) as s3_client:
                    try:
                        ...
                        if path is not None:
                            activity.new_context_dependent_object_path = path
                        if is_full_permissions_context:
                            assert context is not None, 'context cannot be None here'
