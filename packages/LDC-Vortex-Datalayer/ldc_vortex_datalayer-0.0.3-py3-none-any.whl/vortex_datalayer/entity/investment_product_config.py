...
class InvestmentProductConfig(BaseDataLayer):
    ...
    def get_cp_product_details(
        self, 
        source: str,
        partner_code_id,
        custom_order_partner_ids: List[int] = None,
        bulk_new_amount: float = 0,
    ) -> List:
        """
        Get CP product details for a given source.
        
        Args:
            source: Partner source code
            partner_code_id: Partner code
            custom_order_partner_ids: List of partner IDs for custom ordering
            bulk_new_amount: Bulk new investment amount to include in results
            
        Returns:
            List of investment product details
        """
        ...
        sql = """
            SELECT
                ...
                %(bulk_new_amount)s AS bulk_new_amount,
                CASE
        ...
        params = {
            ...
            'bulk_new_amount': bulk_new_amount,
            'medium_term': InvestmentType.MEDIUM_TERM_LENDING,
            'one_time': InvestmentType.ONE_TIME_LENDING,
            'manual': InvestmentType.MANUAL_LENDING
        }
        results = self.execute_fetch_all(sql, params)
