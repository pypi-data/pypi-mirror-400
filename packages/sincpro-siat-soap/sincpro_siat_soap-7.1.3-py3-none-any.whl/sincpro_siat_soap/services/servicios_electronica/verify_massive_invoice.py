...
class CommandVerifyMassiveInvoiceElectronic(DataTransferObject):
    document_type: int
    emission_code: int
    point_of_sale: int
    system_code: str
...
class ResponseVerifyMassiveInvoiceElectronic(DataTransferObject):
    literal_status: SectorDocumentState | str
    reception_code: str
    raw_response: Any


@siat_soap_sdk.feature(CommandVerifyMassiveInvoiceElectronic)
class VerifyMassiveInvoiceSiatElectronic(Feature):

    def execute(
        self, dto: CommandVerifyMassiveInvoiceElectronic
