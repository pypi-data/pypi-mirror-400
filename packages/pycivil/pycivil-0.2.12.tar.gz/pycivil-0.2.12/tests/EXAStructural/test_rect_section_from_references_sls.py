...
def buildTest(
    ...
    pointCloud, bb = section.interactionDomainBuild2d(
        nbPoints=200, SLS=True, bounding=True
    )
    ...
    lines = []
    lines.append([tensionPoint, pintersect[0]])
    lines.append([zeroPoint, tensionPoint])
    ...
    if sigmacRef is None:
        ...
        sigmacErr = abs(sigmacRef - sol[0])
    else:
        sigmacErr = abs((sigmacRef - sol[0]) / sigmacRef)

    if sigmasRef is None:
        sigmasRef = -1
        sigmasErr = -1
    else:
        sigmasErr = abs((sigmasRef - sol[1]) / sigmasRef)

    if xiRef is None:
        xiRef = -1
        xiErr = -1
    else:
        xiErr = abs((xiRef - sol[2]) / xiRef)
    ...
    if sigmacReg is None:
        ...
        sigmacErrReg = abs(sigmacReg - sol[0])
    else:
        sigmacErrReg = abs((sigmacReg - sol[0]) / sigmacReg)

    if sigmasReg is None:
        sigmasErrReg = -1
    else:
        sigmasErrReg = abs((sigmasReg - sol[1]) / sigmasReg)

    if xiReg is None:
        xiErrReg = -1
    else:
        xiErrReg = abs((xiReg - sol[2]) / xiReg)
    ...
    if sigma_c_domain != 0.0:
        sigmacErrDom = abs((sigma_c_domain - sol[0]) / sigma_c_domain)
    else:
        sigmacErrDom = abs(sigma_c_domain - sol[0])

    sigmasErrDom = abs((sigma_s_domain - sol[1]) / sigma_s_domain)
    xiErrDom = abs((xi_domain - sol[2]) / xi_domain)
    ...
    print(
        "      reg sigmac =%8.3f -       reg sigmas =%8.3f -       reg xi =%8.3f"
