...
class S3Connection:
    """
    S3 connection configuration for saving outputs to S3.
...
'''
    ...
    def s3_connect(
        ...
        """
        Create an S3 connection for saving outputs to S3.
        ...
        Returns:
            S3Connection object to use with output() and output_file()
        ...
        Example:
            ...
            ...     prefix="experiments/"
            ... )
            >>> session.output("model.pt", model_state, s3_connection=s3)
        """
        return S3Connection(
            bucket=bucket,
    ...
    def output(
        ...
        Example:
            ...
            >>>
            >>> # Save PyTorch model
            >>> session.output("model.pt", model.state_dict(), s3)
            >>>
            >>> # Save NumPy array
            >>> session.output("embeddings.npy", embeddings, s3)
            >>>
            >>> # Save JSON data
            >>> session.output("metrics.json", {"accuracy": 0.95}, s3)
        """
        try:
            import boto3
        except ImportError:
    ...
    def output_file(
        ...
        Example:
            ...
            >>> session.output_file("./checkpoints/best_model.pt", s3)
            >>>
            >>> # Upload with custom name
            >>> session.output_file("./model.pt", s3, remote_filename="models/v2/model.pt")
        """
        try:
            import boto3
        except ImportError:
