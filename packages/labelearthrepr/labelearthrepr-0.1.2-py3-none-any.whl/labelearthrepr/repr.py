...
from labelearthrepr.config import Config
from labelearthrepr.html import convert_to_html, escape_object
from labelearthrepr.style import CSS

REPR_HTML = "_repr_html_"
EEObject = Union[le.Element, le.ComputedObject]
    ...
    if mbs > options.max_repr_mbs:
        warn(
            message=(
                ...
                " can set `labelearthrepr.options.max_repr_mbs` to print larger objects,"
...
def initialize(
    ...
    for cls in [le.Element, le.ComputedObject]:
        _attach_html_repr(cls, _ee_repr)


def reset():
    """Remove HTML repr methods added by labelearthrepr to EE objects and reset the cache."""
