...
class PlaylistDownloader:
    ...
    def download_spotify_playlist(self, playlist_url: str, output_dir: str,
        ...
        for i, item in enumerate(tracks):
            ...
            added_at = item.get("added_at", "")  # When added to playlist
            ...
            if os.path.exists(f"{output_path}.mp3"):
                # Still save metadata if we don't have it
                if mp3_filename not in metadata:
                    metadata[mp3_filename] = {
                        "added_at": added_at,
                        "source": "spotify"
                    }
                    # Save immediately so dates are preserved
        ...
        try:
            with yt_dlp.YoutubeDL(extract_opts) as ydl:
                ...
                if info and 'entries' in info:
                    ...
                    # Build order map - invert so most recent (idx 0) gets highest value
                    for idx, entry in enumerate(entries):
                        if entry and entry.get('id'):
                            track_order[entry['id']] = total_tracks - 1 - idx
                    if progress_callback:
