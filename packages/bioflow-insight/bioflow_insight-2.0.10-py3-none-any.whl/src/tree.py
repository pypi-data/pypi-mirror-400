        ...
        for child in self.children:
            if(child.is_a_leaf()):
                ...
                if(dico['subworkflows'][child.get_id_subworkflow()]=={}):
                    ...
                    dico['subworkflows'].pop(child.get_id_subworkflow())
                #if(temp_dico!={}):
                #    if(len(temp_dico['nodes'])!=1):
                #        dico['subworkflows'][child.get_id_subworkflow()] = {}
                #        dico['subworkflows'][child.get_id_subworkflow()]['nodes'] = temp_dico['nodes'].copy()
                #        dico['subworkflows'][child.get_id_subworkflow()]["edges"] = temp_dico['edges'].copy()
                #        dico['subworkflows'][child.get_id_subworkflow()]["subworkflows"] = temp_dico['subworkflows'].copy()
                #
                #    else:
                #        print(temp_dico['nodes'])
                #        dico["nodes"] += temp_dico['nodes'].copy()
        return dico
