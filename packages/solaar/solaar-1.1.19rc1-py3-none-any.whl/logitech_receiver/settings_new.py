...
## A new way of supporting settings, using a feature-specific device class to store, read, and write relevant information
## The setting uses a persist structure to keep track of the setting.
## The setting uses the device class to interact with the device feature.
## The Device class has to have a method to initialize the device structure (value of setup in setting)
## The device class has methods to
## query data on the device and put it into the device structure (value of query in setting)
## get data from the device structure (value of get in setting)
## set data on the device structure and the device (value of set in setting)
## determine what values are acceptable (value of acceptable in setting)
...
class Setting:
    ...
    setup = "setup"  # method name on Device class to get the device object
    query = "query"  # method name on the device object to query the setting value on the device
    get = "get"  # method name on the device object to get the setting value
    set = "set"  # method name on the device object to set the setting value on the device
    acceptable = "acceptable"  # method name on the device object to check for acceptable values
    ...
    def build(cls, device):
        if cls.check_properties(cls, device):
    ...
    @classmethod
    def check_properties(cl, cls, device):
    ...
    def initialize(self):  # do whatever is needed to initialize the setting at run time, such as set up choices
        pass
    ...
    def read(self, cached=True):
        """Get all the data for the setting.  If cached is True the data in the _value and the device object can be used."""
        ...
        if self._device.online:
            self._value = getattr(self._device_object, self.query())  # query the device
            self._value = getattr(self._device_object, self.get)()  # get the value
            return self._value
    ...
    def write(self, value, save=True):
        ...
        if self._value != value:
            self._value = value
            if save and self._device.persister:
                self._device.persister[self.name] = value if self.persist else None
        if self._device.online:
            response = getattr(self._device_object, self.set())  # write value to the device
            if response:
                return value
...
class Settings(Setting):
    ...
    # setup creates a dictionary with actual entries for all the keys (they don't have to have device data in them)
    # _value is a map from keys to values
    # query, get, set, and acceptable are methods of dict value objects, not of the device object itself
    ...
    def read(self, cached=True):
        ...
        for key in self._device_object:
            self._value[key] = self.read_key(key, cached)
        return self._value
    ...
    def read_key(self, key, cached=True):
        ...
        if cached and key in self._value and self._value[key] is not None:  # read from _value
            return self._value[key]
        if cached:  # read from device object
            data = self._device_object[key]
            self._value[key] = getattr(data, self.get)()
            return self._value[key]
        if self._device.online:  # read from device
            data = self._device_object[key]
            self._value[key] = getattr(data, self.query)()
            return self._value[key]
    ...
    def write_key_value(self, key, value, save=True):
        ...
        if self._device.online:
            ...
            if self._device.persister and self.persist and save:
                self._device.persister[self.name] = self._value  # need to trigger configuration saving
