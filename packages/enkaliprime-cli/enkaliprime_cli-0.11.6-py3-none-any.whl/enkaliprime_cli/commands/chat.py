    ...
    try:
        ...
        while True:
            ...
            if user_input.startswith('/'):
                ...
                if command == '':
                    ...
                    console.print(cyber_panel(
                        ...
                        "[bold cyan]/history[/] - Show conversation history\n"
                    ...
                    if not conversation_history:
                        ...
                        console.print(f"[bold {COLOR_PRIMARY}]📝 Conversation History ({len(conversation_history)//2} exchanges)[/]")
                        console.print()

                        for i, message in enumerate(conversation_history[-20:], 1):  # Show last 20 messages
                            role = message["role"]
                            content = message["content"]

                            if role == "user":
                                console.print(f"[bold {COLOR_ACCENT}]You:[/] {content}")
                            else:
                                console.print(cyber_panel(
                                    content,
                                    title=agent_name,
            ...
            try:
                ...
                # Prepare the message with conversation history
                final_message = user_input

                # Include conversation history for context (last 20 messages)
                if len(conversation_history) > 1:
                    history_context = "\n\nPrevious conversation:\n"
                    # Get the last 19 messages (excluding current user message) to stay within 20 total
                    recent_history = conversation_history[-20:-1] if len(conversation_history) > 20 else conversation_history[:-1]
                    ...
                    final_message = f"{history_context.strip()}\n\nCurrent user message: {user_input}"
