...
class Worker:
    ...
    def _run(self, run_id: str, repo_root_hint: str | None = None) -> None:
        ...
        try:
            lines = ["# Planner Light Summary", "", summary]
            for ev in light_evidence[:5]:
                if not isinstance(ev, dict):
                    continue
                lines.append(f"- {ev.get('path','')}")
            light_summary_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
        except Exception:
            pass
        update(important_event=summary, important_event_at=utc_now_iso())
        ...
        update(mutator=set_requirements_brief, important_event="已生成需求文档", important_event_at=utc_now_iso())
    ...
    def _plan_scan(self, *, repo_root: Path, query: str, layout: StorageLayout, light: bool) -> list[Path]:
        """
        Deterministic scan plan: pick a small set of high-signal files to read.
        """
        ...
        add("charts/web-react/README.md")
        add("charts/web-react/values.yaml")
        add("charts/web-react/values-dev.yaml")
        add("charts/web-react/templates/deployment.yaml")
        add("charts/web-react/templates/ingress.yaml")
        add("apps/web-react/README.md")
        add("apps/web-react/deploy/build-docker.sh")
        add("apps/web-react/deploy/README.md")
        add("apps/web-react/package.json")
        add("apps/web-react/next.config.js")
