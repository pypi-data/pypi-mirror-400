...
class SupportForestClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        if self.base_model is None:
            self.base_model_ = SVC(kernel='rbf', gamma='scale', C=1.0,  max_iter=-1, tol=1e-3, cache_size=200, verbose=False)
        else:
...
class SupportLogisticClassifier(BaseEstimator, ClassifierMixin):
...
class SupportDecisionClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        # 1. Initialize defaults
        if self.base_model is None:
            self.base_model_ = SVC(shrinking=True, kernel='rbf', gamma='scale', C=1.0,  max_iter=-1, tol=1e-3, cache_size=200, verbose=False)
        else:
            self.base_model_ = clone(self.base_model)
...
class SupportKNNClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        if self.meta_model is None:
            self.meta_model_ = KNeighborsClassifier(n_neighbors=5, weights='distance', algorithm='auto', leaf_size=30, p=2, metric='minkowski', n_jobs=-1)
        else:
            self.meta_model_ = clone(self.meta_model)
...
class SupportXGBClassifier(BaseEstimator, ClassifierMixin):
...
class SupportCatClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        if self.meta_model is None:
            self.meta_model_ = CatBoost.CatBoostClassifier(verbose=0, random_state=42, iterations=1000, learning_rate=0.1, depth=10, l2_leaf_reg=3)
        else:
            self.meta_model_ = clone(self.meta_model)
...
class SupportGradientClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv # Number of cross-validation folds
    ...
    def fit(self, X, y):
        ...
        if self.meta_model is None:
            self.meta_model_ = GradientBoostingClassifier(n_estimators=100, random_state=42, learning_rate=0.1, max_depth=10, subsample=0.8, max_features='sqrt')
        else:
            self.meta_model_ = clone(self.meta_model)
...
class SupportAdaClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv # Number of cross-validation folds
...
class SupportLightClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv # Number of cross-validation folds
