...
class TestAllocation(unittest.TestCase):
    ...
    def test_allocation_solver(self):
        ...
        production_df = pd.concat([production_df, pd.DataFrame({'Index_Prod': ['.SHORTAGES'], 'Value': [0]})],
                                  ignore_index=True)
        sales_df = pd.concat([sales_df, pd.DataFrame({'Index_Sales': ['.STOCK'], 'Value': [0]})], ignore_index=True)
