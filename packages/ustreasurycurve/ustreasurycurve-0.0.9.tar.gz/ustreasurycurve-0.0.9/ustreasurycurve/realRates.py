...
def realRates(date_start=None, date_end=None):
    ...
    for year in [x for x in range(2023, int(current_year) + 1)]:
        cdf = pl.read_csv(f'https://home.treasury.gov/resource-center/data-chart-center/interest-rates/daily-treasury-rates.csv/{year}/all?type=daily_treasury_real_yield_curve&field_tdr_date_value={year}&page&_format=csv')
        cdfs.append(cdf)
