...
from ..services.policy import AgentPolicy, route_models, route_recipe
    ...
    *,
    ...
    def workflow_route(
        *,
        ...
        recipe = route_recipe(policy=agent_policy, workflow_mode=str(route.get("workflow_mode") or "lite"))
        return {
            ...
            "recipe": recipe,
            "bounds": {
                "tool_call_budget_hint": "Prefer async+polling; keep single tool calls under client deadlines (e.g., 60s).",
                "patch_generation_hint": "Avoid implicit git diff; generate patches explicitly (include_patch / generate_patch).",
            },
        }
