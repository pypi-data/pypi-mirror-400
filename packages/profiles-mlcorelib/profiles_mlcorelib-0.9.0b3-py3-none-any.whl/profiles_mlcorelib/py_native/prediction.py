...
class PredictionModel(BaseModelType):
    ...
    def __init__(self, build_spec: dict, schema_version: int, pb_version: str) -> None:
        ...
        # Note: ALL output columns (both feature and non-feature) are created as nested column models
        # via BSN_AddChildInfo messages in propensity.sanitize_buildspec()
        # with destination=[".", "prediction"] to register them under the prediction model
        # Features are also listed in the buildspec's "features" field for feature_view discovery
        # Go's populateChildFeatureModels() skips creating columns that already exist

        # No contract needed - children are created via AddChildInfo messages
...
class PredictionRecipe(PyNativeRecipe):
    ...
    def execute(self, this: WhtMaterial):
        ...
        _predict(
            ...
            train_output,
            whtService.get_inputs(self.build_spec["inputs"]),
            this.string(),  # Use string() for proper table name with path conversion
            config,
            runtime_info,
