...
"""
    ...
    def get_top_relevant_notes(self, notes, query, limit=10, sort_desc=True, max_chars=70000):
        """获取按相关度排序的前N条记录 - 统一使用V5算法计算分数，并进行大小控制
        ...
        # 循环删除评分最低的记录，直到总字符数不超过max_chars
...
if __name__ == "__main__":
    ...
    # 测试降序（默认）
    top_notes_desc = v5.get_top_relevant_notes(notes, query, limit=2, sort_desc=True)
    print(f"降序排列:\n{top_notes_desc}")
    # for note in top_notes_desc:
    #     print(f"  ID: {note[0]}, 分数: {note[3]}")
    
    # 测试升序
    # top_notes_asc = v5.get_top_relevant_notes(notes, query, limit=2, sort_desc=False)
    # print("升序排列前2条相关笔记:")
