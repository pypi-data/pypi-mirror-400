...
def mgr(monkeypatch, tmp_path):
    """
    Provide a configured singleton SessionManager with isolated state:
    - reset internal state between tests
    - force config to use tmp_path as ostryalis.directory
    """
    import ostryalis.session as session

    m = session.SessionManager()
    m.reset()
    m.set_config_getter(lambda key: str(tmp_path))
...
def test_ensure_session_commit_true_persists_changes(mgr):
    engine = mgr.get_engine()

    with mgr.ensure_session(commit=True) as s:
...
def test_ensure_session_rollback_on_exception(mgr):
    engine = mgr.get_engine()
    with engine.begin() as c:
...
def test_ensure_session_with_existing_session_does_not_close_it(mgr):
    factory = mgr._get_session_factory()
    s = factory()
    try:
        with mgr.ensure_session(s, require_attached=["xalias"]) as s2:
            assert s2 is s
...
def test_attachment_cache_skips_repeated_pragma_calls(mgr, monkeypatch):
    ...
    with mgr.ensure_session(require_attached=["cachetest"]) as s:
...
def test_attachment_cache_is_per_connection(mgr, monkeypatch, tmp_path):
    ...
    with mgr.ensure_session(require_attached=["perconn"]) as s:
        pass
    ...
    engine.dispose()
    mgr.reset()
    mgr.set_config_getter(lambda key: str(tmp_path))
    mgr.get_engine()
...
def test_concurrency_ensure_session_require_attached_no_exceptions(mgr):
    mgr.get_engine()
    mgr.attach_many(["conc"])
    ...
    def worker():
        try:
            barrier.wait()
            with mgr.ensure_session(require_attached=["conc"], commit=False) as s:
                s.execute(sa.text("SELECT 1")).scalar_one()
                sa_conn = s.connection()
...
def test_concurrency_mixed_alias_registration_and_attachment(mgr):
    ...
    def attacher(i: int):
        try:
            barrier.wait()
            with mgr.ensure_session(require_attached=[aliases[i]], commit=False) as s:
                s.execute(sa.text("SELECT 1")).scalar_one()
        except Exception as e:
            errors.append(e)
...
def test_register_function_and_require_functions_installs_and_executes(mgr):
    ...
    with mgr.ensure_session(require_functions=[("add1", 1)], commit=False) as s:
...
def test_udf_overload_by_arity_works(mgr):
    ...
    with mgr.ensure_session(require_functions=[("ovl", 1), ("ovl", 2)], commit=False) as s:
...
def test_sqlite_on_connect_installs_registered_udfs(mgr):
    # Register before engine creates first connection; connect event should install automatically.
    def mul2(x):
        return x * 2
...
def test_require_functions_missing_registration_raises_keyerror(mgr):
    ...
    with pytest.raises(KeyError):
        with mgr.ensure_session(require_functions=[("nope", 1)], commit=False) as s:
            pass
...
def test_udf_cache_is_per_connection(mgr, monkeypatch, tmp_path):
    ...
    with mgr.ensure_session(require_functions=[("inc", 1)], commit=False) as s:
        pass
...
def test_concurrency_register_function_is_thread_safe(mgr):
    ...
    assert not errors, f"Unexpected errors: {errors!r}"
    # Should have at most 3 overloads due to setdefault semantics.
    assert len([k for k in mgr._udfs.keys() if k[0] == "cf"]) <= 3


def test_concurrency_ensure_session_require_functions_no_exceptions(mgr):
    mgr.get_engine()
