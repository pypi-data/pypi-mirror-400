...
class SessionLockMixin:
    ...
    async def run_with_resume_lock(
        ...
        run_fn: Callable[[str, ResumeToken | None], AsyncIterator[PochiEvent]],
    ) -> AsyncIterator[PochiEvent]:
        resume_token = resume
...
class BaseRunner(SessionLockMixin):
    ...
    def run(self, prompt: str, resume: ResumeToken | None) -> AsyncIterator[PochiEvent]:
