...
class VmessProtocol(AbstractXrayProtocol):
    ...
    def _call_network_protocol_func(self, network_protocol):
        network_protocol_functions = {
            ...
            "xhttp": self.vmess_dict_xhttp_network,
            "httpupgrade": self.vmess_dict_httpupgrade_network
        }
    ...
    class CommonVmessData:
        ...
        def __post_init__(self):
            ...
            self.security_type = self.vnext_setting.get("users", [{}])[0].get("security", "auto")
    ...
    def security_fields(self, common_vmess_data: CommonVmessData) -> dict:
        security = common_vmess_data.security.lower()
        if security == "none" or security == "":
            return {"tls": ""}
        elif security == "tls":
            return {"tls": "tls", "sni": common_vmess_data.tlsSettings.get("serverName", "")}
    ...
    def vmess_dict_ws_network(self):
        ...
        fields = {
            ...
            "scy": common_vmess_data.security_type
        }
        security_fields = self.security_fields(common_vmess_data)
        return {**fields, **security_fields}
    ...
    def vmess_dict_tcp_network(self):
        common_vmess_data = self.CommonVmessData(config=self.config)
        host = common_vmess_data.stream_settings.get("tcpSettings", {}).get("header", {}).get("request", {}).get(
            "headers", {}).get("Host", [])
        host = ",".join(host)
        path = common_vmess_data.stream_settings.get("tcpSettings", {}).get("header", {}).get("request", {}).get("path",
                                                                                                                 [])
        path = ",".join(path)
        fields = {
            "v": "2",
    ...
    def vmess_dict_xhttp_network(self):
        common_vmess_data = self.CommonVmessData(config=self.config)
        host = common_vmess_data.stream_settings.get("xhttpSettings", {}).get("host", "")
        path = common_vmess_data.stream_settings.get("xhttpSettings", {}).get("path", "")
        fields = {
            "v": "2",
            "ps": "",
            "add": common_vmess_data.vnext_setting.get("address", ""),
            ...
            "aid": common_vmess_data.alter_id,
            "net": "xhttp",
            "type": common_vmess_data.stream_settings.get("xhttpSettings", {}).get("header", {}).get("type", ""),
            "host": host,
            "path": path,
            "scy": common_vmess_data.security_type
        }
    ...
    def vmess_dict_httpupgrade_network(self):
        common_vmess_data = self.CommonVmessData(config=self.config)
        host = common_vmess_data.stream_settings.get("httpupgradeSettings", {}).get("host", "")
        path = common_vmess_data.stream_settings.get("httpupgradeSettings", {}).get("path", "")
        fields = {
            "v": "2",
            "ps": "",
            "add": common_vmess_data.vnext_setting.get("address", ""),
            ...
            "aid": common_vmess_data.alter_id,
            "net": "httpupgrade",
            "type": common_vmess_data.stream_settings.get("httpupgradeSettings", {}).get("header", {}).get("type", ""),
            "host": host,
            "path": path,
            "scy": common_vmess_data.security_type
        }
