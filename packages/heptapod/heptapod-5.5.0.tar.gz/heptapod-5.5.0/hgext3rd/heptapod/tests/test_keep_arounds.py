...
def test_create_delete_keep_around(tmpdir):
    wrapper = RepoWrapper.init(tmpdir)
    repo = wrapper.repo
    sha1 = b"ef57" * 10
    sha2 = b"c512" * 10
    delete_keep_around(repo, sha1)  # covering case of missing file
    create_keep_around(repo, sha1)
    assert list(iter_keep_arounds(repo)) == [sha1]
    delete_keep_around(repo, sha2)  # covering case of line not matching
    assert list(iter_keep_arounds(repo)) == [sha1]
    create_keep_around(repo, sha2)
    assert list(iter_keep_arounds(repo)) == [sha1, sha2]
    delete_keep_around(repo, sha1)
    assert list(iter_keep_arounds(repo)) == [sha2]
