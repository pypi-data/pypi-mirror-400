...
# Note: Environment variables for thread control are NOT set here
# to allow multi-core training/optimization. Single-core profiling
# will be handled by setting n_jobs=1 only in profiling functions.
import os
...
class ModelBenchmark:
    ...
    def _get_models_config(self, n_jobs=1):
        ...
        def get_catboost(seed):
            ...
            params.update({
                ...
                'verbose': 0,
                'random_state': seed,
                'thread_count': n_jobs  # Use specified n_jobs
            })
    ...
    def evaluate_models_multiple_runs(self):
        ...
        for model_name in self.model_names:
            ...
            if model_name == "TabPFN":
                try:
                    ...
                    cv_results = cross_validate(
                        ...
                        scoring=scoring,
                        cv=cv,
                        n_jobs=self.n_jobs,  # Use n_jobs=10 for fast training
                        return_train_score=False,
            ...
            if model_name == 'No-Model':#'TabPFN':
                ...
                # Set environment variables for single-core profiling
                self._set_single_core_env()
                try:
                    # Use n_jobs=1 for profiling to get accurate single-core timing
                    models_config_profile = self._get_models_config(n_jobs=1)
                    model_for_profile = models_config_profile[model_name](self.base_random_state)
    ...
    def _set_single_core_env(self):
        """Set environment variables to force single-core execution."""
        self._old_env_vars = {}
        env_vars = {
            'OMP_NUM_THREADS': '1',
            'MKL_NUM_THREADS': '1',
        ...
        for key, value in env_vars.items():
            self._old_env_vars[key] = os.environ.get(key)
...
def load_hd_dataset(mat_file_path):
    ...
    # Alternative binary conversion: map specific classes to 1 (as in benchmark_hd.py)
    ...
    for mat_file in mat_files:
        try:
            ...
            benchmark = ModelBenchmark(
                ...
                n_jobs=10,  # Use 10 cores for fast training/optimization
