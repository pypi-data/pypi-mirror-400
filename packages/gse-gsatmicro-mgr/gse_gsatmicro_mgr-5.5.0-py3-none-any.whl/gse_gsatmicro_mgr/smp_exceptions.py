# Exceptions related to SMP processing
...
# Raised when an errors occurs while reading a response frame
class FrameReadError(Exception):
    pass

# Raise when a frame has an invalid format or unexpected data
class InvalidFrameError(Exception):
    pass

# Raised when a valid frame with an unexpected response was received
class InvalidResponseError(Exception):
    pass

# Raised when a valid frame with invalid data was received
class InvalidResponseData(Exception):
    def __init__(self, message, data):
        num_rc = None
        if "rc" in data:
            num_rc, rc = smp.decode_error_in_response(data)
            message += ", error={}".format(rc)
        if len(data) > 1 or not "rc" in data:
            message += ", full response data:{}".format(str(data))
        self.num_rc = num_rc
