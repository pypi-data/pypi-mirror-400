...
def calculate_billing_units(
    ...
    # 2. Token Fee
    token_usage_dict = token_usage or {}
    ...
    # 3. Compute Fee
    compute_units = math.ceil(duration_seconds) if duration_seconds else 0
...
async def create_strategy(
    ...
    try:
        ...
        logger.info(f"[{correlation_id}] Processing execution results...")
        dates = execution_result.get("dates", []) if execution_result else []
        returns = execution_result.get("returns", []) if execution_result else []
        positions = execution_result.get("position_history", []) if execution_result else []
        start_dates_data = (execution_result.get("start_dates") or {}) if execution_result else {}
        ...
        history = {
            ...
            "positions": positions,
            "availability": {
                "partial": start_dates_data.get("partial_universe_start_date") if start_dates_data else None,
                "complete": start_dates_data.get("complete_universe_start_date") if start_dates_data else None,
            },
        }

        timing_data = final_state.get("timing") or {}
        execution_timing = timing_data.get("n4_modal_execution") or {}
        ...
        # Mark as failed with full traceback for debugging
        try:
            crud_strategies.mark_strategy_failed(
                db=db,
                strategy_id=strategy_id,
            ...
            logger.error(f"Failed to mark strategy as failed: {nested_error}")
        raise HTTPException(
            status_code=500,
            detail={
