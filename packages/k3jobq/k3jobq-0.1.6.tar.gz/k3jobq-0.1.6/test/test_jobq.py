...
class TestDispatcher(unittest.TestCase):
    def test_dispatcher_job_manager(self):
        ...
        # of order output
        self.assertRaises(k3jobq.JobWorkerError, jm.set_thread_num, _collect_by_tid, 10)
...
class TestJobManager(unittest.TestCase):
    def test_manager(self):
        ...
        jm = k3jobq.JobManager([_sleep, _collect])
    ...
    def test_set_thread_num_keep_order(self):
        ...
        ths = []
        for ii in range(3):
            th = k3thread.daemon(_change_thread_nr)
            ths.append(th)
