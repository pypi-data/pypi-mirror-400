...
from xladmin.util import model_format_dict, model_ngettext
from xladmin.views import BaseAdminPlugin, ListAdminView
from xladmin.views.base import filter_hook, ModelAdminView

from xladmin import views
...
class ActionPlugin(BaseAdminPlugin):
    ...
    def get_media(self, media):
        if self.actions and self.admin_view.result_count:
            media = media + self.vendor('xladmin.plugin.actions.js', 'xladmin.plugins.css')
        return media

    # Block Views
    def block_results_bottom(self, context, nodes):
        if self.actions and self.admin_view.result_count:
            nodes.append(loader.render_to_string('xladmin/blocks/model_list.results_bottom.actions.html',
