...
@user.command("create")
@click.option("--name", required=False, help="User full name")
@click.option("--email", required=False, help="User email")
@click.option("--username", required=True, help="Username")
@click.option("--password", help="Initial password (required for LOCAL users)")
@click.option("--type", "user_type", type=click.Choice(["LOCAL", "SERVICE"]), default="LOCAL", help="User type")
@click.option("--from-file", "user_file", type=click.Path(exists=True), help="Load user from JSON file")
@click.pass_context
def create_user(ctx, name: str, email: str, username: str, password: str, user_type: str, user_file: str) -> None:
    """Create a new user.
    
    Examples:
        dremio user create --username john --password secret --name "John Doe" --email john@company.com
        dremio user create --username svc_account --type SERVICE
    """
    try:
        # Get profile
        manager = ProfileManager()
        ...
        if user_file:
            ...
            if user_type == "SERVICE":
                user_data["identityType"] = "SERVICE_USER"
                # Service users don't have passwords
            else:
                if not password:
                     raise click.BadParameter("Password is required for LOCAL users", param_hint="--password")
