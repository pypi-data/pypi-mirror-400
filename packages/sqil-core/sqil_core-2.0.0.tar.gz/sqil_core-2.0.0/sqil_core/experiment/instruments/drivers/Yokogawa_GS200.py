...
class YokogawaGS200(VisaInstrument):
    ...
    def __init__(
        ...
        **kwargs: "Unpack[VisaInstrumentKWArgs]",
        ...
        """returns the oldest unread error message from
        the event log and removes it from the log."""

        # self.connect_message() # REMOVED DAMN CONNECT MESSAGE - andrea
    ...
    def _assert_mode(self, mode: ModeType) -> None:
        ...
        if self.source_mode.get_latest() != mode:
            raise ValueError(
                f"Cannot get/set {mode} settings while in"
                f"{self.source_mode.get_latest()} mode"
            )
