...
def test_concurrent_install_waits(tmp_path, monkeypatch):
    ...
    def installer():
        engine.install_user_template(src, name='concurrent', force=True, wrap=False, lock_timeout=5)
...
def test_concurrent_install_timeout(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError):
        engine.install_user_template(src, name='concurrent', force=True, wrap=False, lock_timeout=0.1)
    t.join()
