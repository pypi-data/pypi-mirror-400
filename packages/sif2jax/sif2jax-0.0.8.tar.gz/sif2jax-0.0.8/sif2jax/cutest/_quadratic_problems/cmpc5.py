"""
CMPC5 - Model Predictive Control Problem
...
def _load_cmpc5_data():
    """Load problem data from NPZ file."""
    data_file = Path(__file__).parent / "data" / "cmpc5.npz"
    return np.load(data_file)


# Load data once at module level
_CMPC5_DATA = _load_cmpc5_data()

# Pre-compute sparse constraint matrices at module level
_n_vars = int(_CMPC5_DATA["n_vars"])
_n_eq = int(_CMPC5_DATA["n_eq"])
_n_ineq = int(_CMPC5_DATA["n_ineq"])

# Equality constraint matrix
if _n_eq > 0:
    _Aeq_indices = jnp.stack([_CMPC5_DATA["Aeq_rows"], _CMPC5_DATA["Aeq_cols"]], axis=1)
    _Aeq_sparse = sparse.BCOO(
        (_CMPC5_DATA["Aeq_vals"], _Aeq_indices), shape=(_n_eq, _n_vars)
    )
    _beq = jnp.array(_CMPC5_DATA["beq"])
else:
    _Aeq_sparse = None
    _beq = None
...
if _n_ineq > 0:
    _Aineq_indices = jnp.stack(
        [_CMPC5_DATA["Aineq_rows"], _CMPC5_DATA["Aineq_cols"]], axis=1
    )
    _Aineq_sparse = sparse.BCOO(
        (_CMPC5_DATA["Aineq_vals"], _Aineq_indices), shape=(_n_ineq, _n_vars)
    )
    _bineq = jnp.array(_CMPC5_DATA["bineq"])
else:
    _Aineq_sparse = None
    _bineq = None


class CMPC5(AbstractConstrainedQuadraticProblem):
    """CMPC5 Model Predictive Control problem.
    ...
    def n(self):
        """Number of variables."""
        return int(_CMPC5_DATA["n_vars"])

    @property
    def m(self):
        """Number of constraints."""
        return int(_CMPC5_DATA["n_cons"])
