...
def test_ping(id_, mock_motors, dummy_motors):
    ...
    mobel_nb_stub = mock_motors.build_read_stub(
        addr, length, id_, expected_model_nb
    )
...
def test__read_error(raise_on_error, mock_motors, dummy_motors):
    ...
    if raise_on_error:
        with pytest.raises(
            RuntimeError,
            match=re.escape('[RxPacketError] Input voltage error!'),
        ):
            bus._read(addr, length, id_, raise_on_error=raise_on_error)
    else:
        _, _, read_error = bus._read(
            addr, length, id_, raise_on_error=raise_on_error
        )
...
def test__read_comm(raise_on_error, mock_motors, dummy_motors):
    ...
    if raise_on_error:
        with pytest.raises(
            ConnectionError,
            match=re.escape('[TxRxResult] There is no status packet!'),
        ):
            bus._read(addr, length, id_, raise_on_error=raise_on_error)
    else:
        _, read_comm, _ = bus._read(
            addr, length, id_, raise_on_error=raise_on_error
        )
...
def test__sync_read_comm(raise_on_error, mock_motors, dummy_motors):
    ...
    stub = mock_motors.build_sync_read_stub(
        addr, length, ids_values, reply=False
    )
    bus = FeetechMotorsBus(port=mock_motors.port, motors=dummy_motors)
    bus.connect(handshake=False)
...
def test_reset_calibration(mock_motors, dummy_motors):
    ...
    assert all(
        mock_motors.stubs[stub].wait_called() for stub in write_homing_stubs
    )
    assert all(
        mock_motors.stubs[stub].wait_called() for stub in write_mins_stubs
    )
    assert all(
        mock_motors.stubs[stub].wait_called() for stub in write_maxes_stubs
    )
...
def test_record_ranges_of_motion(mock_motors, dummy_motors):
    ...
    with patch(
        'lerobot.motors.motors_bus.enter_pressed', side_effect=[False, True]
    ):
        bus = FeetechMotorsBus(port=mock_motors.port, motors=dummy_motors)
        bus.connect(handshake=False)
