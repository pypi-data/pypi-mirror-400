...
class HS91(AbstractConstrainedQuadraticProblem):
    """Hock and Schittkowski problem 91.
    ...
    def y0(self):
        ...
        return jnp.array([0.5, -0.5, 0.5, -0.5, 0.5], dtype=jnp.float64)
    ...
    def constraint(self, y):
        ...
        # For N=5: p[k] = sum(x_i^2, i=k..N) for k=1..N, p[N+1]=0
        n = 5
        ...
        # Alternating sign pattern: [1, -2, 2, -2, 2]
        signs = jnp.array([1.0, -2.0, 2.0, -2.0, 2.0])

        # Compute exp terms for all combinations of mu and p
        # muj2[:, None] has shape (30, 1), p[None, :n] has shape (1, 5)
        # Broadcasting will give shape (30, 5)
        exp_terms = jnp.exp(-muj2[:, None] * p[None, :n])

        # Apply signs and sum across p dimension
        # signs has shape (5,), exp_terms has shape (30, 5)
        # Broadcasting will apply signs to each row
        u = jnp.sum(signs[None, :] * exp_terms, axis=1)

        # Final constant term: alpha would be -2 after 4 iterations (starting from -2)
