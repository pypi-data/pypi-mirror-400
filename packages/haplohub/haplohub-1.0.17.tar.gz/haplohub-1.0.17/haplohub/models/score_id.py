...
SCOREID_ANY_OF_SCHEMAS = ["int", "str"]


class ScoreId(BaseModel):
    """
    ScoreId
    """
    ...
    if TYPE_CHECKING:
        ...
        actual_instance: Any
    any_of_schemas: List[str] = Field(SCOREID_ANY_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in ScoreId with anyOf schemas: int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ScoreId:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> ScoreId:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into ScoreId with anyOf schemas: int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
