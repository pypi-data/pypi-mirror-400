..
   This documentation page is generated during the Sphinx build.
   The underlying code is manually maintained and not autogenerated.

eegdash.dataset.DS004602
========================

Registered Replication Report of ERN/Pe Psychometrics (OpenNeuro ``ds004602``). Access recordings and metadata through EEGDash.

.. rst-class:: sd-badges

:bdg-light:`Modality: eeg` :bdg-light:`Tasks: 3` :bdg-light:`License: CC0` :bdg-light:`Subjects: 182` :bdg-light:`Recordings: 546` :bdg-light:`Source: openneuro`

Dataset Information
-------------------

.. list-table::
   :widths: 25 75
   :header-rows: 0

   * - Dataset ID
     - ``DS004602``
   * - Title
     - Registered Replication Report of ERN/Pe Psychometrics
   * - Year
     - Unknown
   * - Authors
     - Peter E Clayson, Michael J Larson
   * - License
     - CC0
   * - Citation / DOI
     - `doi:10.18112/openneuro.ds004602.v1.0.1 <https://doi.org/10.18112/openneuro.ds004602.v1.0.1>`__
   * - Source links
     - `OpenNeuro <https://openneuro.org/datasets/ds004602>`__ | `NeMAR <https://nemar.org/dataexplorer/detail?dataset_id=ds004602>`__

.. dropdown:: Copy-paste BibTeX
   :class-container: sd-shadow-sm
   :class-title: sd-bg-light

   .. code-block:: bibtex

      @dataset{ds004602,
        title = {Registered Replication Report of ERN/Pe Psychometrics},
        author = {Peter E Clayson and Michael J Larson},
        doi = {10.18112/openneuro.ds004602.v1.0.1},
        url = {https://doi.org/10.18112/openneuro.ds004602.v1.0.1},
      }

Highlights
----------

.. grid:: 1 2 3 3
   :gutter: 2

   .. grid-item-card:: Subjects & recordings
      :class-card: sd-border-1

      - Subjects: 182
      - Recordings: 546
      - Tasks: 3

   .. grid-item-card:: Channels & sampling rate
      :class-card: sd-border-1

      - Channels: 128
      - Sampling rate (Hz): 500*
      - Duration (hours): 87.11

   .. grid-item-card:: Tasks & conditions
      :class-card: sd-border-1

      - Tasks: 3
      - Experiment type: Perception
      - Subject type: Healthy

   .. grid-item-card:: Files & format
      :class-card: sd-border-1

      - Size on disk: 73.91 GB
      - File count: Unknown
      - Format: BIDS

   .. grid-item-card:: License & citation
      :class-card: sd-border-1

      - License: CC0
      - DOI: doi:10.18112/openneuro.ds004602.v1.0.1

   .. grid-item-card:: Provenance
      :class-card: sd-border-1

      - Source: openneuro
      - OpenNeuro: `ds004602 <https://openneuro.org/datasets/ds004602>`__
      - NeMAR: `ds004602 <https://nemar.org/dataexplorer/detail?dataset_id=ds004602>`__

Quickstart
----------

**Install**

.. code-block:: bash

   pip install eegdash

**Load a recording**

.. code-block:: python

   from eegdash.dataset import DS004602

   dataset = DS004602(cache_dir="./data")
   recording = dataset[0]
   raw = recording.load()

**Filter/query**

.. tab-set::

   .. tab-item:: Basic

      .. code-block:: python

         dataset = DS004602(cache_dir="./data", subject="01")

   .. tab-item:: Advanced

      .. code-block:: python

         dataset = DS004602(
             cache_dir="./data",
             query={"subject": {"$in": ["01", "02"]}},
         )


Quality & caveats
-----------------

- No dataset-specific caveats are listed in the available metadata.

API
---

.. currentmodule:: eegdash.dataset

.. autoclass:: eegdash.dataset.DS004602
   :members: __init__, save
   :show-inheritance:
   :member-order: bysource


See Also
--------

* :class:`eegdash.dataset.EEGDashDataset`
* :mod:`eegdash.dataset`
* `OpenNeuro dataset page <https://openneuro.org/datasets/ds004602>`__
* `NeMAR dataset page <https://nemar.org/dataexplorer/detail?dataset_id=ds004602>`__

