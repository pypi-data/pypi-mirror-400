...
This tool is a great example of an LLM-as-a-filter pattern for automated curation. We originally went straight to `research.md` after the loop, but spammy results forced us to add this automated keep step. Later, we realized some Perplexity URLs were worth scraping in full, so we added a second selector rather than re-running research or editing the guideline mid-flight.
...
This tool's goal is to get clean, LLM-ready markdown from the selected URLs. As we covered in Lesson 16, we use Firecrawl because it is purpose-built to convert modern, dynamic web pages into markdown. We then run a short LLM pass to clean up any remaining boilerplate. This provides the necessary depth that snippet-based research from Perplexity cannot offer, enriching the final research artifact with comprehensive context [[1]](https://docs.firecrawl.dev/features/scrape?utm_source=chatgpt.com), [[2]](https://docs.firecrawl.dev/advanced-scraping-guide?utm_source=chatgpt.com).
...
This final artifact is the culmination of our agent's work, providing a clean, citable, and comprehensive knowledge base for the writing workflow we will build in Lessons 19-22.
...
Because our entire workflow is defined in a server-hosted MCP prompt, we can inject these HITL checkpoints without changing any code. The prompt accepts policy tweaks as plain text. This means you can run the agent in a fully autonomous mode or switch to a supervised mode by simply changing the invocation message. For instance, you could add instructions like "Only keep sources from .edu or .gov domains" or "Select exactly 3 URLs to scrape." This is an effective pattern for building controllable and adaptable AI systems [[3]](https://modelcontextprotocol.io/docs/concepts/prompts?utm_source=chatgpt.com).
...
Our research agent can also be used directly within Cursor IDE through the MCP protocol. The `mcp_server` folder contains a `.mcp.json.sample` file that shows how to configure Cursor to use our research agent [[4]](https://cursor.com/docs/context/mcp?utm_source=chatgpt.com).
...
*   Add a **domain whitelist/blacklist** control to the source selection tool.
*   Implement a **fact-checking** or **quote-extraction** tool that runs after scraping.
*   Add a **deduplication** tool to cluster and merge similar paragraphs from different sources.
*   Create a **cost report** tool that summarizes API calls and scraping counts for each run.
...
1. Firecrawl. (n.d.). Scrape to Markdown. Firecrawl Docs. https://docs.firecrawl.dev/features/scrape?utm_source=chatgpt.com
2. Firecrawl. (n.d.). Advanced Scraping Guide. Firecrawl Docs. https://docs.firecrawl.dev/advanced-scraping-guide?utm_source=chatgpt.com
3. Model Context Protocol. (n.d.). Prompts. Model Context Protocol. https://modelcontextprotocol.io/docs/concepts/prompts?utm_source=chatgpt.com
4. Cursor. (n.d.). MCP integration. Cursor. https://cursor.com/docs/context/mcp?utm_source=chatgpt.com
5. Model Context Protocol. (n.d.). Tools. Model Context Protocol. https://modelcontextprotocol.io/docs/concepts/tools?utm_source=chatgpt.com
6. Perplexity. (n.d.). Structured Outputs. Perplexity. https://docs.perplexity.ai/guides/structured-outputs?utm_source=chatgpt.com
7. OpenAI. (n.d.). Model context protocol (MCP). OpenAI GitHub Pages. https://openai.github.io/openai-agents-python/mcp/?utm_source=chatgpt.com
8. Google AI. (n.d.). Video understanding. Google AI for Developers. https://ai.google.dev/gemini-api/docs/video-understanding?utm_source=chatgpt.com
