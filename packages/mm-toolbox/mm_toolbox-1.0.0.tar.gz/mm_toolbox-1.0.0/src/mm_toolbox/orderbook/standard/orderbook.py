...
class Orderbook:
    ...
    def __init__(
        ...
        if initial_bids is not None and initial_asks is not None:
            self.consume_snapshot(bids=initial_bids, asks=initial_asks, has_ticks_and_lots=True)
    ...
    def consume_snapshot(
        ...
        Args:
            ...
            has_ticks_and_lots: Whether levels already include ticks/lots precision info.

        """
        if len(bids) < self._size:
            raise ValueError(f"Invalid bids with snapshot; expected >= {self._size} levels but got {len(bids)}")
        if len(asks) < self._size:
            raise ValueError(f"Invalid asks with snapshot; expected >= {self._size} levels but got {len(asks)}")

        self.reset()

        for ask in asks:
            if not has_ticks_and_lots:
    ...
    def get_wmid_price(self) -> float:
        ...
        wmid_ticks = (best_bid_ticks * best_bid_lots + best_ask_ticks * best_ask_lots) / total_lots
