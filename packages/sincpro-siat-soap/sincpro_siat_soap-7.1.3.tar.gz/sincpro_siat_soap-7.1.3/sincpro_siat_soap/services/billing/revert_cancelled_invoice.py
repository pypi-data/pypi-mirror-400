...
class CommandRevertCancelledInvoice(DataTransferObject):
    sector_document: int
    emission_code: int
    point_of_sale: int
    system_code: str
...
class ResponseRevertCancelledInvoice(DataTransferObject):
    reception_code: str | None
    literal_status: type_verified_status | str
    raw_response: Any


@siat_soap_sdk.feature(CommandRevertCancelledInvoice)
class RevertCancelledInvoice(Feature):

    def execute(self, dto: CommandRevertCancelledInvoice) -> ResponseRevertCancelledInvoice:
        ...
        return ResponseRevertCancelledInvoice(
