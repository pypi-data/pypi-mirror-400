...
from .api.candidate_sets import (
...
class SelectionClient:
    ...
    def generate_candidate_set(self, request: SelectionGenerateCandidateSetRequest) -> CandidateSet:
        resp = generate_candidate_set.sync_detailed(client=self._client, body=request.body)
        return _unwrap(resp)

    def health(self, request: SelectionHealthRequest) -> Health:
        _ = request
        resp = health.sync_detailed(client=self._client)
        return _unwrap(resp)

    def version(self, request: SelectionVersionRequest) -> VersionInfo:
        _ = request
