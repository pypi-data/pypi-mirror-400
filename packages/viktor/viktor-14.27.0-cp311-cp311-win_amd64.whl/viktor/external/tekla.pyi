...
__all__ = ['TeklaAnalysis']

class TeklaAnalysis(PythonAnalysis):
    ''' ::version(14.17.0)

    TeklaAnalysis can be used to evaluate a tekla-python script on third-party infrastructure. The script is expected
    ...
    .. code-block:: python
        ...
        script = vkt.File.from_path(Path(__file__).parent / "run_tekla.py")
        files = [
            (\'input1.txt\', file1),
        ]
        analysis = TeklaAnalysis(script=script, files=files, output_filenames=["output.txt"])
