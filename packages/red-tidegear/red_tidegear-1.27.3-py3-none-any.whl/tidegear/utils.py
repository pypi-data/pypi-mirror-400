...
async def send_error(
    ...
    **kwargs: Any,
    ...
    The `ephemeral` keyword argument will default to `True`.
    The `allowed_mentions` keyword argument will default to disallowing role and `@everyone` mentions.
    The `wait` keyword argument will explicitly be ignored, so as to not break interaction handling.
    ...
    Args:
        ...
        reply: Determines whether or not to reply to the source message.
            Only does anything if `messageable` is a [`Context`][redbot.core.commands.Context] object.
        **kwargs: Additional keyword arguments to pass to [`Messageable.send()`][discord.abc.Messageable.send].

    Returns:
        The sent message.
    """
    # pop "wait" so as to not break `discord.Interaction.followup.send`.
    kwargs.pop("wait", None)
    # error messages should usually be ephemeral if possible, so default to them being ephemeral.
    kwargs.setdefault("ephemeral", True)
    # prevent error messages from accidentally mentioning roles / @everyone
    kwargs.setdefault("allowed_mentions", discord.AllowedMentions(everyone=False, users=True, roles=False, replied_user=True))
    ...
    if isinstance(messageable, commands.Context):
        if reply:
            return await call_with_kwargs(messageable.reply, **kwargs)
        return await call_with_kwargs(messageable.send, inspect_func=DPYContext.send, extra={"filter"}, **kwargs)

    # ? My typechecker was complaining that `messageable` is somehow still typehinted as a `discord.Message`, even
    # ? though that branch was already handled earlier in the method. Doesn't do this with interactions, not sure what's wrong.
