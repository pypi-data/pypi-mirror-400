...
from ..evaluators.answer import AnswerEvalProcessor
...
class AnswerProcessor(BaseProcessor):
    def pre_process(self, args) -> Any:
        ...
        for file in files:
            ...
            data = handler.read()
            new_filename = f"{file.stem}_answer{file.suffix}"
    ...
    def core_process(self, pre_data):
        ...
        required_columns_keys = ["question", "truth_context", "truth_answer", "model_answer"]
        required_columns = [columns_config[k] for k in required_columns_keys if k in columns_config]

        if args.template is None:
            template_name = "answer_prompt.yaml"
        else:
    ...
    def post_process(self, result: Any) -> Tuple[bool, str]:
        ...
        try:
            ...
            columns_config = getattr(args, "columns_config", {}) or {}
            col_score = columns_config.get('answer_correctness_score', '答案正确性分数')
            col_score_desc = columns_config.get('answer_correctness_score_description', '答案正确性得分说明')
