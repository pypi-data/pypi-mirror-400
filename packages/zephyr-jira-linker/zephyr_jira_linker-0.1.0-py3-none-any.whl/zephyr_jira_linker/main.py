...
def update_jira_testcase_link_field(issue_key: str, testcase_keys: list) -> bool:
    try:
        ...
        if testcase_id:
            ...
            zephyrlog.warning(f"Using testcase key instead of ID for URL construction")
...
def process_branch_testcase_linking(branch_name: str, project_key: str = "PROJECT") -> bool:
...
def main():
    """Main entry point for the zephyr-test-linker command-line tool."""
    if len(sys.argv) < 2:
        zephyrlog.error("Please provide the branch name as an argument")
        zephyrlog.info("Usage: zephyr-test-linker <branch_name>")
        zephyrlog.info("")
        zephyrlog.info("This script will:")
        zephyrlog.info("- Extract issue key from branch name")
    ...
    project_key = sys.argv[1].split("-")[0].strip()

    success = process_branch_testcase_linking(branch_name, project_key)
