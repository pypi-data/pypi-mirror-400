...
class ServerDaemon:
    ...
    def __action(self):
        ...
        while True:
            ...
            except Exception as register_exception:  # pylint: disable=broad-except
                self.logger.error(
                    "Error registering to etcd: %s, retrying...", register_exception
                )
                time.sleep(30)
                continue
            else:
                self.logger.info("Register meta to etcd successfully")
                break
        ...
        while True:
            try:
                ...
                with etcd().lock(
                    ...
                    self.logger.info(
                        "Current endpoint: %s",
                        etcd()._current_endpoint_label,  # pylint: disable=protected-access
                    )
                    while True:
                        time.sleep(ttl / 4)
                        lock.refresh()
            except Exception as lock_exec:  # pylint: disable=broad-except
                self.logger.info(lock_exec)
