...
class TestWriteAdr:
    ...
    def test_updates_index(self, temp_dir: Path, minimal_criteria: CriteriaModel) -> None:
        """Test that index file is created and updated."""
        docs_dir = temp_dir / "docs"
        output_path = write_adr(minimal_criteria, docs_dir)
        index_path = docs_dir / "adr" / "index.md"
        assert index_path.exists()
        assert output_path.name in index_path.read_text()
...
class TestWriteAdrDocx:
    ...
    def test_updates_index(self, temp_dir: Path, minimal_criteria: CriteriaModel) -> None:
        """Test that index is written for docx-only output."""
        docs_dir = temp_dir / "docs"
        output_path = write_adr_docx(minimal_criteria, docs_dir)
        index_path = docs_dir / "adr" / "index.md"
        assert index_path.exists()
