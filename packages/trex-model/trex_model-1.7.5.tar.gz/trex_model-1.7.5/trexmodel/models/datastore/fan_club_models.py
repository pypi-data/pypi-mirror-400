...
class FanClubSetup(BaseNModel, DictModel):
    ...
    enabled_by              = ndb.KeyProperty(name="enabled_by", kind=MerchantUser)
    enabled_by_username     = ndb.StringProperty(required=False)
    
    disabled_by             = ndb.KeyProperty(name="disabled_by", kind=MerchantUser)
    disabled_by_username    = ndb.StringProperty(required=False)
    
    
    dict_properties = [
                        'group_name', 'desc', 'fan_club_type',  'invite_link',
                        'created_datetime', 'published_datetime', 'archived_datetime', 
                        'is_published', 'is_enabled',
                        'published_by_username', 'disabled_by_username',
    ...
    def enable(self, enabled_by=None):
        
        enabled_by_username = None
        if is_not_empty(enabled_by):
            if isinstance(enabled_by, MerchantUser):
                enabled_by_username = enabled_by.username
        
        self.is_enabled                 = True
        self.published_datetime         = datetime.utcnow()
        self.enabled_by                 = enabled_by.create_ndb_key()
        self.enabled_by_username        = enabled_by_username
        self.put()    
        
        merchant_acct = self.merchant_acct
        merchant_acct.add_fan_club_setup_configuration(self.to_configuration()) 
        
    def disable(self, diabled_by=None):
        
        diabled_by_username = None
        if is_not_empty(diabled_by):
            if isinstance(diabled_by, MerchantUser):
                diabled_by_username = diabled_by.username
        
        self.is_enabled                 = False
        self.published_datetime         = datetime.utcnow()
        self.diabled_by                 = diabled_by.create_ndb_key()
        self.diabled_by_username        = diabled_by_username
        self.put()    
