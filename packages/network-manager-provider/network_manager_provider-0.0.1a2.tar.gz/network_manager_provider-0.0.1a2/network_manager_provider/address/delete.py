...
from network_manager_provider.defaults import ADDRESS, DELETE
...
async def delete(address, link_name, delete_kwargs=None, ip_route_init_kwargs=None):
    ...
    if not found_dynamic:
        response["msg"] = (
            f"Failed to find interface to delete address from: {dynamic_response}"
        )
    ...
    if not validated:
        response["msg"] = (
            f"Failed to validate the lookup response of interface: {link_name} before deletion, err: {validation_response}"
        )
        return False, response

    if len(dynamic_response) > 1:
        response["msg"] = (
            f"Found multiple interface indicies to delete address from: {dynamic_response}"
        )
        return False, response

    func_name = discover_pyroute2_function_name(ADDRESS)
    func_args = discover_pyroute2_func_args(DELETE)
    func_kwargs = discover_pyroute2_func_kwargs(ADDRESS, address=address)
    ...
    if not success:
        response["msg"] = f"Failed to delete address from link: {link_name}"
        if operation_msg:
            response["msg"] += f", err: {operation_msg}"
        return False, response

    response["msg"] = f"Deleted successfully address: {address} from link: {link_name}"
