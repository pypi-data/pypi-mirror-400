...
def check_environment_complete(raise_on_failure=False, print_missing=True, verbose=False, requires_admin=False):
    ...
    missing = set()
    required_environment = ['ELEMENTS_API_HOST', 'ELEMENTS_API_TOKEN', 'ELEMENTS_AUTHOR']
    if requires_admin:
        required_environment.append('ELEMENTS_API_ADMIN_TOKEN')
    var_list = []
...
async def deactivate_algorithm_versions(
    ...
    """
    """
    oi_papi_host = os.environ["ELEMENTS_API_HOST"]
    oi_papi_port_string = os.getenv("ELEMENTS_API_PORT", default="443")
    oi_papi_port = int(oi_papi_port_string)
    secure_string = os.getenv("ELEMENTS_API_SECURE", default="True")
    secure = True if secure_string in ["True", "true", "Yes", "yes", "1"] else False
    # admin_api_token = os.environ["ELEMENTS_API_ADMIN_TOKEN"]

    client = ElementsAsyncClient(oi_papi_host, oi_papi_port, secure=secure)
...
async def set_credit(
    ...
    export ELEMENTS_API_ADMIN_TOKEN=<admin_token>
    ...
    terrascope_host = os.environ['ELEMENTS_API_HOST']
    terrascope_api_admin_token = os.getenv('ELEMENTS_API_ADMIN_TOKEN')
