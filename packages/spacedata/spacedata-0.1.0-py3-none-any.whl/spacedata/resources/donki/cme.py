...
from spacedata.models.donki import DONKICme, DONKIDateRangeInput
...
class DonkiCmeResult(SpaceDataResult, typing.Generic[T]):
    URL_KEY = "link"
...
async def _get_response(
    ...
    response = await resource.client.make_request(
        method="GET",
        url=f"{resource.client.settings.main_base_url}/DONKI/CME",
        params=params,
    )
    return [DONKICme.model_validate(item) for item in response]
...
async def cme(
    ...
    **kwargs,
) -> DonkiCmeResult[DONKICme]:
    self.client.logger.debug("Calling cme endpoint with input_data: %s", input_data)
    ...
    return DonkiCmeResult(items=items_generator)
