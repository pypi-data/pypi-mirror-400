...
class ServingImageBuilder(ImageBuilder):
    ...
    def _render_dockerfile(
        ...
        has_custom_run_as_user = (
            config.docker_server is not None
            and config.docker_server.run_as_user_id is not None
        )
        run_as_user_id = (
            config.docker_server.run_as_user_id if has_custom_run_as_user else 60000
        )
        run_as_user_name = (
            config.docker_server.run_as_user_name if has_custom_run_as_user else "app"
        )
