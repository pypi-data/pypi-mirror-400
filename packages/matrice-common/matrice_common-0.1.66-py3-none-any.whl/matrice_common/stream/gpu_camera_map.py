...
class GpuCameraMap:
    ...
    SHM_PATH = "/dev/shm/gpu_camera_map"
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            # Memory map it
            self._mmap = mmap.mmap(self._fd, self.MAX_SIZE, mmap.MAP_SHARED,
    ...
    def connect(self) -> bool:
        """Connect as consumer - open existing shared memory.

        Should be called by the inference engine to read GPU assignments.
        ...
        try:
            ...
            self._mmap = mmap.mmap(self._fd, self.MAX_SIZE, mmap.MAP_SHARED, mmap.PROT_READ)
            self._initialized = True

            logger.info(f"GpuCameraMap connected at {self.SHM_PATH}")
            return True
