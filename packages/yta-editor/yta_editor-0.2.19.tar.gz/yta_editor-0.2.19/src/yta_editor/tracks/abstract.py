...
from yta_editor.utils.cache import FramesCacheFactory
...
class _TrackWithAudio(_Track):
    ...
    #) -> list['AudioFrameWrapped']:
        ...
        generating a delay that makes the audio fit not the video,
        but the old code is working correctly.
        """
        # if not hasattr(self, '_audio_silent'):
        #     self._audio_silent = FramesCacheFactory.silent_audio_frame(
        #         # TODO: I think I need the 'video_fps'
        ...
        #     self._audio_silent = [
        #         AudioFrameWrapped(
        #             frame = self._audio_silent,
        #             is_from_gap = False,
        #             is_silent = True
        #         )
        #     ]

        #     # TODO: It is expected to be a list[AudioFrameWrapped]
        #     # but, is it necessary (?)
...
class _TrackWithVideo(_Track):
    ...
    def video_empty_frame(
        ...
        # TODO: We need one black and completely opaque (when it
        # is the only one) and another black but fully 
        # transparent (to mix with other frames)
        video_frame = FramesCacheFactory.black_video_frame(
            size = self.size,
            video_fps = self.fps,
            transparency = 1.0
        )

        # TODO: This is already cached, we don't need to set
        # an internal variable to cache it again
        video_frame.time_base = fps_to_time_base(self.fps)

        VideoFrameWrapped(
            frame = video_frame
            # TODO: Do we need the 'is_from_gap' (?)
        )
        ...
        """
        if not hasattr(self, '_video_empty_frame'):
            self._video_empty_frame = VideoFrameWrapped(
                frame = VideoFrameGenerator.background.full_black(
                    size = self.size,
