...
public:
    ...
    explicit NonOwningPtr(const WeakNonOwningPtr<T>& ptr): ptr_(ptr.ptr_.lock()) {}
...
public:
    ...
    explicit WeakNonOwningPtr(const ShareableNonOwningPtr<T>& ref): ptr_(ref.ptr_) {}
    ...
    explicit operator bool() const noexcept
    {
        ...
        // initialized.
        return ptr_.owner_before(std::weak_ptr<T>{}) || std::weak_ptr<T>{}.owner_before(ptr_);
    }
