"""
Tests for Plan Mode feature (CLI subprocess version).
"""
...
from minmo.gemini_cli_wrapper import (
    GeminiCLIWrapper as GeminiWrapper,
    InterviewFocus,
    InterviewQuestion,
    InterviewAnswer,
    FeatureSpec,
...
class TestGeminiWrapperPlanMode:
    """Tests for GeminiWrapper Plan Mode methods (CLI subprocess version)."""

    @pytest.fixture
    def mock_scribe(self):
        """Mock Scribe functions."""
        with patch("minmo.gemini_cli_wrapper.log_event") as mock_log, \
             patch("minmo.gemini_cli_wrapper.init_database"):
...
```'''
        ...
        questions = wrapper.generate_interview_questions(
            ...
            project_analysis=ProjectAnalysis(project_path="/test", languages=["Python"])
        )
...
```'''
    ...
    def test_run_plan_mode_full_flow(self, wrapper, mock_scribe):
        ...
        with patch.object(wrapper, 'analyze_project') as mock_analyze, \
             patch.object(wrapper, 'generate_interview_questions') as mock_q, \
