{% materialization incremental, adapter='spark', supported_languages=['sql', 'python'] -%}
   ...
   {% do log("[DBT_MACRO] INCREMENTAL_MATERIALIZATION: Starting incremental materialization", info=True) %}
   {% do log("[DBT_MACRO] INCREMENTAL_MATERIALIZATION: Checking for custom Spark incremental materialization!", info=True) %}
  {%- set file_format = config.get('file_format', 'openhouse') -%}
  {% do log("[DBT_MACRO] INCREMENTAL_MATERIALIZATION: file_format = " ~ file_format, info=True) %}
  {% if file_format == 'openhouse' %}
      {% do log("[DBT_MACRO] INCREMENTAL_MATERIALIZATION: Using LI incremental materialization (openhouse)", info=True) %}
      {{ return(adapter.dispatch('indbt_incremental_materialization')()) }}
  {% endif %}
  {% do log("[DBT_MACRO] INCREMENTAL_MATERIALIZATION: Using standard incremental materialization", info=True) %}
  ...
  {%- if existing_relation is none -%}
    ...
    {% do log("[DBT_MACRO] INCREMENTAL_MATERIALIZATION: Creating new table (existing_relation is none)", info=True) %}
  ...
  {%- elif existing_relation.is_view or should_full_refresh() -%}
    ...
    {% do log("[DBT_MACRO] INCREMENTAL_MATERIALIZATION: Dropping and recreating table (view or full refresh)", info=True) %}
  ...
  {%- else -%}
    ...
    {% do log("[DBT_MACRO] INCREMENTAL_MATERIALIZATION: Using incremental merge strategy", info=True) %}
