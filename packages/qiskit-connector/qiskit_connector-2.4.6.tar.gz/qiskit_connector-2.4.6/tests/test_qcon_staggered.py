...
def test_connector_returns_backend_staggered():
    try:
        backend = connector()
...
def test_qplan_is_string_staggered():
    try:
...
def test_load_environment_staggered():
    try:
...
def test_get_credentials_staggered():
    try:
...
def test_get_plan_value_error_no_plan_staggered(monkeypatch):
...
def test_get_plan_value_error_missing_name_staggered(monkeypatch):
    from qiskit_connector import _get_plan
...
def test_save_account_missing_creds_staggered(monkeypatch):
...
def test_list_backends_staggered(monkeypatch):
...
def test_connector_no_backend_staggered(monkeypatch):
    class MockService:
...
def test_save_account_success_staggered(monkeypatch):
    from qiskit_connector import save_account
...
def test_connector_lists_qpus_staggered(monkeypatch):
    class MockBackend:
        def __init__(self, name): self.name = name
        version = "1.0"
        num_qubits = 7
