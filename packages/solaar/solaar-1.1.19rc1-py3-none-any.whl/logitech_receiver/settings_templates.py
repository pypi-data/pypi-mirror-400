...
class ForceSensing(settings_new.Settings):
    ...
    def initialize(self):  ## If there is only one force button a simpler interface can be used
        self.label = _("Force Sensing Button")
        self.min_value = self._device_object[0].min_value
        self.max_value = self._device_object[0].max_value
        self.kind = settings.Kind.RANGE
...
class SpecialKeysMapping(settings_new.Settings):
    name = "special_keys_mapping"
    label = _("Key/Button Actions")
    description = (
        _("Change the action for the key or button.")
        + "  "
    ...
    feature = _F.SPECIAL_KEYS_AND_BUTTONS
    setup = "special_keys"
    query = "query_mapped_to"
    get = "get_mapped_to"
    set = "set_mapped_to"
    acceptable = "acceptable_mapped_to"
    choices_universe = special_keys.CID
    kind = settings.Kind.MAP_CHOICE

    @classmethod
    def check_properties(cl, cls, device):  # check that remapping bit is set for feature
        if super().check_properties(cls, device):
            response = device.feature_request(cls.feature, 0x00)
            return response[0] & 0x01

    def initialize(self):  # set up remappings for remappable keys
        self._choices = {special_keys.CID[key]: self.choices_universe for key in self._device.cids}


### needs to be generalized to allow for mouse gestures and sliding DPI
class SpecialKeysDiversion(settings_new.Settings):
    name = "special_keys_diversion"
    label = _("Key/Button Diversion")
    description = _("Make the key or button send HID++ notifications (Diverted) or initiate Mouse Gestures or Sliding DPI")
    feature = _F.SPECIAL_KEYS_AND_BUTTONS
    setup = "special_keys"
    query = "query_diversion"
    get = "get_diversion"
    set = "set_diversion"
    acceptable = "acceptable_diversion"
    choices_universe = common.NamedInts(**{_("Regular"): 0, _("Diverted"): 1})
    kind = settings.Kind.MAP_CHOICE
