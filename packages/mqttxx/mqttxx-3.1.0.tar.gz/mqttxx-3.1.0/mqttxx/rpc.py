...
class RPCManager:
    ...
    def register_request_handler(self, request_topic: str):
        """注册 RPC 请求处理主题

        让 RPCManager 监听指定主题的 RPC 请求

        Args:
            request_topic: 请求主题（例如：device/rpc_requests）
        ...
        示例：
            ...
            rpc.register_request_handler("device/rpc")
        """
        async def handle_bytes(topic: str, payload: bytes):
            try:
                message = parse_message_from_bytes(payload, self._codec)
                ...
                logger.exception(f"RPC 请求处理失败: {e}")

        self._client.raw.subscribe(request_topic, handle_bytes)
