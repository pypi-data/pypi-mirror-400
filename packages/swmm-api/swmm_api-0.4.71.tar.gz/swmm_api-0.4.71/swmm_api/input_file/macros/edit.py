...
def delete_node(inp, node_label, graph=None, alt_node=None):
    ...
    if alt_node is not None:
        ...
        if SUBCATCHMENTS in inp:
            ...
            #     sc.outlet = to_node
            for sc in inp[SUBCATCHMENTS].values():  # type: swmm_api.input_file.sections.SubCatchment
                if node_label == sc.outlet:
                    # TODO
                    warnings.warn(f'delete_node in SUBCATCHMENTS section not implemented. Node "{node_label}" is the outlet of SC "{sc.name}". Outlet is not changed.', SwmmApiInputMacrosWarning)
                    # delete_subcatchment()
...
def rename_curve(inp, old_label, new_label):
    if old_label in inp[CURVES]:
        obj = inp[CURVES].pop(old_label)  # type: swmm_api.input_file.sections.Curve
        obj.name = new_label
        inp[CURVES].add_obj(obj)

    for section in [STORAGE, OUTFALLS, OUTLETS, PUMPS, XSECTIONS]:
        if section in inp:
            for name in inp[section]:
                if inp[section][name].curve_name == old_label:
                    inp[section][name].curve_name = new_label
...
def rename_obj_in_control_section(inp, kind, old_label: str, new_label: str):
    """
    Rename an object in the CONTROLS section.
