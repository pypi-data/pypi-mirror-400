 ...
 */
function detectZodType(typeString) {
 ...
 */
function assessContractQuality(typeString, hasRuntimeCheck) {
    // Check for value constraints
    const hasValueConstraint = typeString.includes('.min(') ||
    ...
    const hasBoundaryConstraint = typeString.includes('.length(') ||
        typeString.includes('.minLength(') ||
        typeString.includes('.maxLength(');
    // Check for enum/literal constraints
    const hasEnumConstraint = typeString.includes('.enum(') ||
 ...
 */
function analyzeFunction(node, sourceFile, checker, includePrivate) {
    // Get function name
    let name;
 ...
 */
function detectBlindSpots(functions) {
    const blindSpots = [];
 ...
 */
function generateSuggestedSchema(func) {
 ...
 */
function resolveImportPath(fromFile, importPath, program) {
 ...
 */
function getDependents(file, graph, transitive = false) {
    const dependents = new Set();
    const visited = new Set();
    const findDependents = (target) => {
 ...
 */
export function analyzeImpact(file, graph) {
