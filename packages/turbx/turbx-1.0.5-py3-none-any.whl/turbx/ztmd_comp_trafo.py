...
def _calc_VDII(self, **kwargs):
    ...
    - Reδ2 = (μe/μw)·Reθ
    - cfi = Fc·cf
    Sometimes referred to as 'Van Driest II' compressibility transform
    -----
...
def _calc_comp_trafo(self, schemes=['VD','VIPL','TL','GFM'], **kwargs):
    '''
    Calculate compressible transformations of data
    -----
    ...
    for i in range(self.nx):
        ...
        if 'VD' in schemes:
            u_VD_ = comp_transform_VD(self.y, u[i,:], rho[i,:])
            u_VD[i,:] = u_VD_
            y_VD_     = self.y
            ...
            d99_VD[i] = calc_d99_1d(y=self.y, u=u_VD_, j_edge=je, interp_kind='cubic')
    ...
    ## Helpers for writing 1D datasets to data_1Dx/
    ...
    ## Helpers for writing 2D datasets to data/
