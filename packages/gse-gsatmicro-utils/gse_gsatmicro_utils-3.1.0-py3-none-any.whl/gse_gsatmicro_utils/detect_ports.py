...
if (sys_name := system()) == "Darwin":
    from .detect_ports_osx import platform_detect
elif sys_name == "Linux": # this is simpple enough to be inlined
    from .detect_ports_linux import platform_detect
elif sys_name == "Windows":
    from .detect_ports_windows import platform_detect
else:
    utils.warning("detect_ports can't find a supported platform")
    # Generic "do nothing" detect ports function
    def platform_detect(serial=None):
        return []

# Detect the serial ports used by GSE devices and return a map of {serial_number: port_data}
# port_data is a map {"mgmt": mgmt port, "logs": log port, "console": console port}
# The returned map will be empty if the detection fails.
def detect():
    res = {}
    # Warn if a port is set more than once (which should not happen)
    def unique(ser, key, value):
        if res[ser].get(key, None) != None:
            utils.warning(f"Port detection: key '{key}' set more than once for device {ser}")
        res[ser][key] = value
    for (ser, kind, port) in platform_detect():
        if not ser in res:
            res[ser] = {}
        if kind == ct.USB_MGMT_PORT_NAME:
            unique(ser, "mgmt", port)
        elif kind == ct.USB_CONSOLE_PORT_NAME:
            unique(ser, "console", port)
        elif kind == ct.USB_LOGS_PORT_NAME:
            unique(ser, "logs", port)
        else:
            utils.warning(f"Port detection: invalid port kind '{kind}', this shouldn't happen")
    return res