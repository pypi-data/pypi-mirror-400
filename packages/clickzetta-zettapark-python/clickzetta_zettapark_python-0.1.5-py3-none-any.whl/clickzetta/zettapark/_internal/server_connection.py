...
class ServerConnection:
        ...
        def log_msg_and_perf_telemetry(cls, msg):
            def log_and_telemetry(func):
                ...
                def wrap(*args, **kwargs):
                    ...
                    #     )
                    _logger.debug(f"Finished in {duration:.4f} secs")
                    return result
    ...
    def run_query(
        ...
        **kwargs,
        ...
        try:
            ...
            _logger.debug(f"Execute query [queryID: {results_cursor.job_id}] {query}")
        except Exception as ex:
            if log_on_exception:
                query_id_log = (
                    f" [queryID: {ex.job_id}]" if hasattr(ex, "job_id") else ""
                )
                _logger.error(f"Failed to execute query{query_id_log} {query}\n{ex}")
            raise ex
    ...
    def run_batch_insert(self, query: str, rows: List[Row], **kwargs) -> None:
        ...
        _logger.debug("Execute batch insertion query %s", query)
