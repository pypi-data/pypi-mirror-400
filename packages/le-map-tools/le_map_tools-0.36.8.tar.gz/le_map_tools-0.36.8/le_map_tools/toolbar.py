...
def inspector_gui(m=None):
    ...
    Args:
        m (le_map_tools.Map, optional): The leaflet Map object. Defaults to None.
    ...
    def __init__(self, m, **kwargs):
        ...
        def get_ee_example(asset_id):
            try:
                ...
                #     pkg_resources.resource_filename("le_map_tools", "le_map_tools.py")
                # )
        ...
        def search_result_change(change):
            ...
            m.search_loc_geom = le.Geometry.Point(location.lng, location.lat)
            ...
            if text.value != "":
                if search_type.value == "name/address":
                    ...
                    if g is None and latlon_from_text(text.value):
                        ...
                        m.search_loc_geom = le.Geometry.Point(latlon[1], latlon[0])
                ...
                if g is not None and len(g) > 0:
                    ...
                    m.search_loc_geom = le.Geometry.Point(top_loc.lng, top_loc.lat)
                    if m.search_loc_marker is None:
                        marker = ipyleaflet.Marker(
                            location=latlon,
                            draggable=False,
...
def tool_template(m=None, opened=True):
    ...
    Args:
        m (le_map_tools.Map, optional): The le_map_tools.Map instance. Defaults to None.
...
def timelapse_gui(m=None, basemap="HYBRID"):
    ...
    Args:
        m (le_map_tools.Map, optional): A le_map_tools Map instance. Defaults to None.
            ...
            if m is not None:
                ...
                if region.value != "User-drawn ROI" or (
                    ...
                    if region.value == "User-drawn ROI":
                        ...
                        if isinstance(roi_object, le.Geometry):
                            geom = roi_object
                            image1 = image1.clip(geom)
                            image2 = image2.clip(geom)
                        else:
                            roi_object = le.FeatureCollection(roi_object)
...
def split_basemaps(
    ...
    from .le_map_tools import basemaps
