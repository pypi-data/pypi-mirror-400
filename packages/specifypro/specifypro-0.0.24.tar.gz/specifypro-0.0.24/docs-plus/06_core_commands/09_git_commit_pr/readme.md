# **Step 8: Ship with `/spp.git.commit_pr`**
...
Use `/spp.git.commit_pr` after you finish the Spec → Plan → Tasks → Implement loop and have already:
...
```text
/spp.git.commit_pr Wrap up the basic calculator feature. Tests are green and tasks.md is fully checked off.
```
...
- Command template: `templates/commands/spp.git.commit_pr.md`
