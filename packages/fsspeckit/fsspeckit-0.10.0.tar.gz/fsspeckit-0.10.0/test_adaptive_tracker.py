...
# Mock the pybloom module
sys.modules["pybloom_live"] = type(sys)("pybloom_live")
sys.modules["pybloom_live"].BloomFilter = MockBloomFilter
sys.modules["pybloom_live"].ScalableBloomFilter = MockScalableBloomFilter
...
class AdaptiveKeyTracker:
    ...
    Automatically transitions between tiers as cardinality grows.
    """
    ...
    def __init__(
        ...
        self.bloom_filter: Optional[object] = None
    ...
    def _upgrade_to_bloom(self) -> None:
        """Upgrade from LRU to Bloom filter."""
        logger.warning(
            "Upgrading from LRU to BLOOM tier - using probabilistic tracking"
        )

        try:
            import pybloom_live

            # Use scalable bloom filter for dynamic sizing
            self.bloom_filter = pybloom_live.ScalableBloomFilter(
                initial_capacity=len(self.lru_keys) if self.lru_keys else 1000,
            ...
            logger.warning("pybloom-live not available, staying in LRU mode")
            # Stay in LRU mode if bloom not available
    ...
    def __contains__(self, key: Any) -> bool:
        ...
        with self._lock:
            ...
            if self.tier == "EXACT":
                ...
                return key in self.lru_keys
            else:  # BLOOM
                if self.bloom_filter is not None:
                    return key in self.bloom_filter
                # Fallback if bloom not available
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if self.tier == "EXACT":
                ...
                metrics["keys_tracked"] = getattr(self.bloom_filter, "capacity", 0)
                metrics["estimated_accuracy"] = 1.0 - self.false_positive_rate
