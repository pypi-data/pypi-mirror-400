...
def execute_command(
    ...
    Args:
        ...
        response_format: Output format - "concise" (minimal tokens),
            "detailed" (full data), or "markdown" (human-readable).

    Returns:
        Formatted result based on response_format:
        - concise: {success, output, [exit_code], [error]}
        - detailed: {exit_code, stdout, stderr, duration_ms, host, command}
    ...
    Example:
        >>> execute_command("server", "uptime", response_format="concise")
        {"success": true, "output": "up 5 days..."}
    """
    ...
    try:
        ...
        try:
            ...
            return formatter.format_command_result(result.to_dict(), fmt)
        ...
        return formatter.format_error(
            ...
            hint="Check if the command is allowed and the host is reachable.",
        )
    except Exception as e:
        audit.log_command_failed(host, command, str(e))
        formatter = get_formatter()
        return formatter.format_error(
            f"SSH error: {e}",
            hint="Verify host configuration and network connectivity.",
        )
...
def execute_on_multiple(
    ...
    formatter = get_formatter()
    fmt = ResponseFormat(response_format)
    return formatter.format_multi_host_result(raw_result, fmt)
