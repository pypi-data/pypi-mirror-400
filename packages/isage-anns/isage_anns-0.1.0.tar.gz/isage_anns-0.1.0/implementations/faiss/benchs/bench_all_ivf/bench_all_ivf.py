...
def run_train(args, ds, res):
    ...
    index = faiss.index_factory(
        d, args.indexkey, faiss.METRIC_L2 if ds.metric == "L2" else faiss.METRIC_INNER_PRODUCT
    )
    ...
    if index_ivf:
        ...
        if isinstance(quantizer, faiss.IndexRefine):
            ...
            if isinstance(base_index, faiss.IndexIVF):
                ...
                base_index.nprobe = (
                    16 if base_index.nlist < 1e5 else 32 if base_index.nlist < 4e6 else 64
                )
    ...
    if args.get_centroids_from == "":
        ...
        if args.train_on_gpu:
            ...
            train_index = faiss.index_cpu_to_all_gpus(faiss.IndexFlatL2(index_ivf.d))
...
def run_search(args, ds, index, res):
    ...
    if args.inter:
        header = "%-40s     inter@%3d time(ms/q)   nb distances #runs" % ("parameters", args.k)
    else:
...
def main():
    ...
    aa("--nthreads", default=-1, type=int, help="nb of threads to use at train and add time")
    aa("--json", default=False, action="store_true", help="output stats in JSON format at the end")
    aa(
        "--todo",
        default=["check_files"],
    ...
    aa("--compute_gt", default=False, action="store_true", help="compute and store the groundtruth")
    aa("--force_IP", default=False, action="store_true", help="force IP search instead of L2")
    aa(
        "--accept_short_gt",
        default=False,
        action="store_true",
    ...
    aa("--trained_indexfile", default="", help="file to read or write a trained index from")
    aa(
        "--maxtrain",
        default=256 * 256,
        type=int,
    ...
    aa("--LSQ_encode_ils_iters", default=-1, type=int, help="ILS iterations for LSQ")
    aa("--RQ_use_beam_LUT", default=-1, type=int, help="use beam LUT at add time")
    ...
    res.cpu_model = [l for l in open("/proc/cpuinfo") if "model name" in l][0]
