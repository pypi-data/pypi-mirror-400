...
def is_docker_ip(ip_addr, docker_ip=False):
    """Recevives an ip and checks if it is in the docker network range.
    The optional `docker_ip` parameter is used to receive the docker gateway IP
    from another source instead of obtaining it from the docker0 network. This is
    useful in cases where the docker0 network doesn't exist, e.g: when we use deployv
    inside a container.
    """
    ip_addr = socket.gethostbyname(ip_addr)
    docker_ip = docker_ip or get_ip_address('docker0')
    docker0 = re.sub(r"\.\d{1,3}$", ".0/24", docker_ip)
...
def read_manifest(backup_path):
    """Checks if there is a manifest.json file in the provided path and returns
    its data if it exists.

    This method is used to avoid adding more complexity to the
    `InstanceV.restore_database` method.

    :param backup_path: Path to a decompressed backup.
    :return: Dict with the JSON data found in the manifest file. An empty dict
    is returned if the file does not exist.
    """
    logger.debug("Searching for a manifest.json in the path %s", backup_path)
    manifest_file = os.path.join(backup_path, 'manifest.json')
    if not os.path.exists(manifest_file):
        logger.debug("Manifest file not found in %s", backup_path)
        return {}
    logger.debug("Manifest file found in %s", manifest_file)
    manifest_data = json_helper.load_json(manifest_file)
