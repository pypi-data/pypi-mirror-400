...
def execute_mark(
    ...
    if info.has_evolve_markers:
        yield Message("system", f"File already has EVOLVE markers at lines {info.evolve_regions[0].start_line}-{info.evolve_regions[0].end_line}")
        return
    ...
    if kernel_name:
        ...
        if not kernel:
            ...
            yield Message("system", f"Error: Kernel '{kernel_name}' not found. Available: {names}")
            return
    else:
        kernel = info.kernels[0]
        if len(info.kernels) > 1:
            names = ", ".join(k.name for k in info.kernels)
            yield Message("system", f"Multiple kernels found: {names}\nMarking first kernel: {kernel.name}\nUse kernel=<name> to specify a different one.")
    ...
    if not confirm(f"Add EVOLVE markers around {kernel.name} (lines {kernel.start_line}-{kernel.end_line})?"):
    ...
    yield Message("system", f"""Added EVOLVE markers to {path.name}:
...
""",
    ...
    parameters=[
        ...
        Parameter(name="kernel", type="string", description="Kernel function name (optional, defaults to first)", required=False),
    ],
)
