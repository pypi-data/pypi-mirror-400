    ...
    def public_status(self, *, options: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        for ev in list(self.events or []):
            if not isinstance(ev, dict):
                continue
            kind = (ev.get("kind") or "info").strip()
            phase = (ev.get("phase") or "").strip()
            tag = f"{phase}:{kind}" if phase else kind
            slim = {
                "tag": tag,
            }
            if isinstance(ev.get("message"), str) and ev.get("message").strip():
                slim["message"] = ev.get("message").strip()
            if isinstance(ev.get("data_lines"), list):
                slim["data_lines"] = [str(x) for x in ev.get("data_lines") if str(x).strip()]
            elif isinstance(ev.get("data"), str) and ev.get("data").strip():
                slim["data_lines"] = [ln.strip() for ln in ev.get("data").splitlines() if ln.strip()]
            events_to_send.append(slim)
            # Build digest lines for quick skim.
            if slim.get("message"):
                digest.append(str(slim["message"]))
            if "data_lines" in slim:
                digest.extend(slim["data_lines"])
        # Clear events once delivered to avoid re-sending.
        self.events = []
        has_update = bool(events_to_send)
        # When无事件时避免过长等待，压到5s内。
        if not has_update:
            poll_ms = min(poll_ms, 5000)
        self.forced_wait_ms = poll_ms
        summary_payload = lines[:3] if has_update else []
        return {
            "run_id": self.run_id,
