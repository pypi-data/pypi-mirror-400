...
except ImportError:  # Babel isn't installed – we will fall back to a small mapping
    Locale = None
...
from ..mcpm import (
    ...
    process_llm_tool_requests,
    get_available_tools_reminder_prompt,
)
...
class Coder:
    ...
    def create(
        ...
        **kwargs,
        ...
        cls.i18n = i18n or I18n(cls.chat_language or locale.getlocale()[0])
        cls.abs_rule_fnames = abs_rule_fnames or list()
        cls.enabled_rules = enabled_rules
        cls.output_format = kwargs.get("output_format")
        ...
        if from_coder:
            ...
            update = dict(
                ...
                abs_rule_fnames=from_coder.abs_rule_fnames,  # Copy rule files
                enabled_rules=from_coder.enabled_rules,
    ...
    def get_announcements(self):
        lines = [f"Aider(@Horizon) v{__version__}"]
        ...
        #    lines.append(f"Added {rel_fname} to the chat (rule).")
    ...
    def __init__(
        ...
        # Fill in a fake Analytics if needed, but it is never .enable()'d
        self.mdstream = None
        self.waiting_spinner = None
        self.usage_report = None
        self.partial_response_function_call = None
        self.ended_reasoning_content = None
        self.got_reasoning_content = None
        self.reflected_message = None
        self.mcp_tool_result_message = None
        ...
        self.abs_rule_fnames = abs_rule_fnames or list()
            ...
            for fname in read_only_fnames:
                ...
                if os.path.exists(abs_fname):
                    ...
                    self.io.tool_warning(
                        f"Error: Rule file {fname} does not exist. Skipping."
                    )
    ...
    def get_read_only_files_content(self):
        # abs_fnames가 비어있는 경우에만 abs_read_only_fnames를 처리
        fnames = self.get_abs_rule_files()
    ...
    def get_input(self):
        ...
        # 기본 rule 설정
        if not self.abs_rule_fnames and self.enabled_rules:
            self.abs_rule_fnames = [
                fname
                for fname in self.abs_read_only_fnames
                if (".rules." not in fname.lower())
            ]
        ...
        try:
            while message:
                ...
                if self.num_mcp_iterations >= self.max_mcp_iterations:
                    self.io.tool_warning(
                        f"Only {self.max_mcp_iterations} MCP iterations allowed, stopping."
                    )
                    return
        ...
        for url in urls:
            if url not in self.rejected_urls:
                ...
                if self.io.confirm_ask(
                    self.i18n.t("ask.009"), subject=url, group=group, allow_never=True
                ):
                    inp += "\n\n"
    ...
    def fmt_system_prompt(self, prompt):
        ...
        mcp_tools_reminder = ""
        if is_enabled():
            mcp_tools = get_available_tools_prompt(self.suggest_shell_commands)
            mcp_tools_reminder = get_available_tools_reminder_prompt()
        ...
        prompt = prompt.format(
            ...
            language=language,
            mcp_tools=mcp_tools,
            mcp_tools_reminder=mcp_tools_reminder,
        )
    ...
    def check_tokens(self, messages):
        ...
        if max_input_tokens and input_tokens >= max_input_tokens:
            ...
            if not self.io.confirm_ask(self.i18n.t("ask.010")):
                return False
        return True
    ...
    def send_message(self, inp):
        ...
        if edited and self.auto_lint:
            ...
            if lint_errors:
                ok = self.io.confirm_ask(self.i18n.t("ask.011"))
                if ok:
        ...
        if edited and self.auto_test:
            ...
            if test_errors:
                ok = self.io.confirm_ask(self.i18n.t("ask.012"))
                if ok:
    ...
    def show_exhausted_error(self):
        ...
        res = [
            "",
            "",
            f"Model {self.main_model.name} has hit a token limit!",
    ...
    def check_for_tool_calls(self, content):
        ...
        if is_enabled():
            return process_llm_tool_requests(content, self.io, self.edit_format)
        else:
            return []
    ...
    def check_for_file_mentions(self, content):
        ...
        for rel_fname in sorted(new_mentions):
            if self.io.confirm_ask(
                self.i18n.t("ask.004"), subject=rel_fname, group=group, allow_never=True
            ):
    ...
    def allowed_to_edit(self, path):
        ...
        if not Path(full_path).exists():
            if not self.io.confirm_ask(self.i18n.t("ask.005"), subject=path):
    ...
    def handle_shell_commands(self, commands_str, group):
        ...
        prompt = (
            self.i18n.t("ask.007") if command_count == 1 else self.i18n.t("ask.007")
        )
        if not self.io.confirm_ask(
            prompt,
            subject="\n".join(commands),
        ...
        if accumulated_output.strip() and self.io.confirm_ask(
            self.i18n.t("ask.008"), allow_never=True
        ):
    ...
    # devflux 추가 함수
    def get_abs_rule_files(self):
        if not self.enabled_rules:
            return self.abs_read_only_fnames

        if not self.abs_rule_fnames:
            fnames = self.abs_read_only_fnames
        else:
            fnames = self.abs_rule_fnames
