...
SWMM_NOT_FOUND_ERROR = SWMMRunError('SWMM executable not found. Set the path manually using CONFIG in the swmm5_run* function or use swmm-toolkit or pyswmm to run SWMM.')
# SWMMRunError('Path to SWMM command line executable not found. Please pass a custom path to the swmm5.exe using the "swmm_path" argument.')
# warnings.warn(f'Executable of SWMM not found ({swmm_path}).')
# raise FileNotFoundError('SWMM executable not found. Set the path manually using CONFIG in the swmm5_run* function or use swmm-toolkit or pyswmm to run SWMM.')
