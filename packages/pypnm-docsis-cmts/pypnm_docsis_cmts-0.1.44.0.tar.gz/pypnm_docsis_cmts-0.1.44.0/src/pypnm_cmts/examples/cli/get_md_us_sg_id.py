...
class MdUsSgIdCli:
    """
    CLI helper for fetching DocsIf3MdNodeStatusMdUsSgId entries.
    """
    ...
    def build_parser() -> argparse.ArgumentParser:
        """
        Build the argument parser for the DocsIf3MdNodeStatusMdUsSgId CLI.
        """
        parser = argparse.ArgumentParser(
            description="Fetch docsIf3MdNodeStatusMdUsSgId via SNMPv2c."
        )
        parser.add_argument(
            "--cmts-hostname",
            required=True,
    ...
    @staticmethod
    async def fetch_md_us_sg_id(inet: Inet, community: str) -> list[MdNodeStatus]:
        """
        Fetch DocsIf3MdNodeStatusMdUsSgId entries from the CMTS.
        """
        operation = CmtsOperation(inet=inet, write_community=community)
        try:
            return await operation.getDocsIf3MdNodeStatusMdUsSgId()
    ...
    def main() -> int:
        """
        CLI entry point for fetching DocsIf3MdNodeStatusMdUsSgId.
        """
        parser = MdUsSgIdCli.build_parser()
        args = parser.parse_args()

        try:
            inet = MdUsSgIdCli.resolve_inet(args.cmts_hostname)
        except ValueError as exc:
        ...
        if community == "":
            MdUsSgIdCli._emit_error("SNMP community string is empty.")
            return MdUsSgIdCli.EXIT_FAILURE

        try:
            entries = asyncio.run(
                MdUsSgIdCli.fetch_md_us_sg_id(inet, community)
            )
        except Exception as exc:
            MdUsSgIdCli._emit_error(str(exc))
            return MdUsSgIdCli.EXIT_FAILURE
        ...
        return MdUsSgIdCli.EXIT_SUCCESS


if __name__ == "__main__":
    raise SystemExit(MdUsSgIdCli.main())
