...
def get_user_count() -> int:
    ...
    try:
        return get_user_model()._default_manager.count()
    except Exception:
        return 0


@register.simple_tag
def get_user_changelist_url() -> str:
    """Get admin changelist URL for the user model."""
    try:
        User = get_user_model()
        app_label = User._meta.app_label
        model_name = User._meta.model_name
...
@register.simple_tag
def get_model_entry_count(app_entry: Dict[str, Any], model_entry: Dict[str, Any]) -> int:
    """Get total count for a model entry from the admin app list."""
    try:
        app_label = app_entry.get("app_label") if isinstance(app_entry, dict) else None
        model_name = None
        admin_url = ""

        if isinstance(model_entry, dict):
            model_name = model_entry.get("object_name") or model_entry.get("model_name")
            admin_url = model_entry.get("admin_url", "")

        if not app_label or not model_name:
            match = re.search(r"/admin/([^/]+)/([^/]+)/", admin_url or "")
            if match:
                app_label = app_label or match.group(1)
                model_name = model_name or match.group(2)
