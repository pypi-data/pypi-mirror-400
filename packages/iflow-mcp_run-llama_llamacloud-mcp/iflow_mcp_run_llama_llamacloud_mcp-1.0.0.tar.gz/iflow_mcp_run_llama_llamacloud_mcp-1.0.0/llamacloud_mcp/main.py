...
async def demo_tool(ctx: Context, message: str) -> str:
    """A demo tool for testing MCP server functionality."""
    try:
        await ctx.info(f"Demo tool called with message: {message}")
        return f"Demo tool response: {message}"
    except Exception as e:
        await ctx.error(f"Error in demo tool: {str(e)}")
        return f"Error in demo tool: {str(e)}"
...
def main(
    ...
    # For testing purposes, allow running without API key if no indexes/agents are specified
    if not api_key and (indexes or extract_agents):
        raise click.BadParameter(
            "API key not found. Please provide an API key or set the LLAMA_CLOUD_API_KEY environment variable when using indexes or extract agents."
        )
    elif api_key:
    ...
    # Register demo tool if no real tools are registered (for testing)
    if not index_info and not extract_agent_info:
        mcp.tool(name="demo", description="A demo tool for testing MCP server functionality")(demo_tool)
