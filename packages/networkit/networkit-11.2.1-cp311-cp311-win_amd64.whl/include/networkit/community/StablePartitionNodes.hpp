...
/**
 * Evaluates how stable a given partition is. A node is considered to be stable if it has strictly
 * more connections to its own partition than to other partitions. Isolated nodes are considered to
 * be stable. The value of a cluster is the percentage of stable nodes in the cluster. Larger values
 * indicate that a clustering is more stable and thus better defined.
 */
...
public:
    ...
    /**
     * Check if a given node is stable, i.e. more connected to its own partition than to other
     * partitions.
     *
