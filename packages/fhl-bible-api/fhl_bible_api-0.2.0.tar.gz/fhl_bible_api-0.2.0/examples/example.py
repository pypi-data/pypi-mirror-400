...
def main() -> None:
    ...
    with FHLBibleClient() as client:
        ...
        # Example 8: Apocrypha verses
        print("\n8. Apocrypha (次經) - 1 Maccabees 1:10")
        print("-" * 60)
        apoc_response = client.get_apocrypha_verse(book_id=101, chapter=1, verse=10)
        verse = apoc_response.records[0]
        print(f"Book: {verse.chinese_abbr} ({verse.english_abbr})")
        print(f"Version: {apoc_response.v_name}")
        print(f"Text: {verse.text[:60]}...")

        # Example 9: Apostolic Fathers writings
        print("\n9. Apostolic Fathers (使徒教父著作) - 1 Clement 1:1")
        print("-" * 60)
        af_response = client.get_apostolic_fathers_verse(book_id=201, chapter=1, verse=1)
        verse = af_response.records[0]
        print(f"Book: {verse.chinese_abbr} ({verse.english_abbr})")
        print(f"Version: {af_response.v_name}")
        print(f"Text: {verse.text[:60]}...")

        # Example 10: Bible Commentary
        print("\n10. Bible Commentary (聖經註釋) - Genesis 1:9")
        print("-" * 60)
        comm_response = client.get_commentary(book_id=1, chapter=1, verse=9)
        if comm_response.records:
            comm = comm_response.records[0]
            print(f"Title: {comm.title}")
            print(f"Source: {comm.book_name}")
            print(f"Content: {comm.com_text[:80]}...")
            if comm_response.next_verse:
                print(f"Next: {comm_response.next_verse.english_abbr} {comm_response.next_verse.chapter}:{comm_response.next_verse.verse}")
