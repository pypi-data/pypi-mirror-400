...
from xladmin.layout import Field, render_field
from xladmin.plugins.inline import Inline
from xladmin.plugins.actions import BaseActionView
from xladmin.plugins.inline import InlineModelAdmin
from xladmin.sites import site
from xladmin.util import unquote, quote, model_format_dict, is_related_field2
from xladmin.views import BaseAdminPlugin, ModelAdminView, CreateAdminView, UpdateAdminView, DetailAdminView, ModelFormAdminView, DeleteAdminView, ListAdminView
from xladmin.views.base import csrf_protect_m, filter_hook
from xladmin.views.detail import DetailAdminUtil
...
class RevisionListView(BaseReversionView):
    ...
    @filter_hook
    def get_media(self):
        return super(RevisionListView, self).get_media() + self.vendor('xladmin.plugin.revision.js', 'xladmin.form.css')
...
class BaseRevisionView(ModelFormAdminView):
    ...
    @filter_hook
    def get_media(self):
        return super(BaseRevisionView, self).get_media() + self.vendor('xladmin.plugin.revision.js')
    ...
    def render(self, form, context, template_pack=TEMPLATE_PACK, extra_context=None, **kwargs):
        ...
        for field in self.fields:
            html += ('<div class="diff_field" rel="tooltip"><textarea class="org-data" style="display:none;">%s</textarea>%s</div>' %
                     (_('Current: %s') % self.attrs.pop('orgdata', ''), render_field(field, form, context, template_pack=template_pack, attrs=self.attrs)))
        return html
