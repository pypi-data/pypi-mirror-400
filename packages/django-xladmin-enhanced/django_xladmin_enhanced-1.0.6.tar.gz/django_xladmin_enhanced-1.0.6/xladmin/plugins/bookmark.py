...
from xladmin.filters import FILTER_PREFIX, SEARCH_VAR
from xladmin.plugins.relate import RELATE_PREFIX
from xladmin.plugins.utils import get_context_dict
from xladmin.sites import site
from xladmin.views import ModelAdminView, BaseAdminPlugin, ListAdminView
from xladmin.views.list import COL_LIST_VAR, ORDER_VAR
from xladmin.views.dashboard import widget_manager, BaseWidget, PartialBaseWidget

from xladmin.models import Bookmark
...
class BookmarkPlugin(BaseAdminPlugin):
    ...
    def get_context(self, context):
        ...
        list_base_url = reverse('xladmin:%s_%s_changelist' %
        ...
        bookmarks_queryset = Bookmark.objects.filter(
            ...
            url_name='xladmin:%s_%s_changelist' % model_info
        ...
        for bk in bookmarks_queryset:
            ...
            bookmarks.append({'title': bk.title, 'selected': selected, 'url': bk.url, 'edit_url':
                              reverse('xladmin:%s_%s_%s' % (bk_model_info[0], bk_model_info[1], change_or_detail),
                                      args=(bk.id,))})
            if selected:
                menu_title = bk.title
                has_selected = True

        post_url = reverse('xladmin:%s_%s_bookmark' % model_info,
        ...
        new_context = {
            ...
            'has_add_permission_bookmark': self.admin_view.request.user.has_perm('xladmin.add_bookmark'),
            'has_change_permission_bookmark': self.admin_view.request.user.has_perm('xladmin.change_bookmark')
        }
    ...
    def get_media(self, media):
        return media + self.vendor('xladmin.plugin.bookmark.js')

    # Block Views
    def block_nav_menu(self, context, nodes):
        if self.show_bookmarks:
            nodes.insert(0, loader.render_to_string('xladmin/blocks/model_list.nav_menu.bookmarks.html',
...
class BookmarkWidget(PartialBaseWidget):
    ...
    template = "xladmin/widgets/list.html"
