...
from cdpy.errors import ElementHandleError, NetworkError
...
if TYPE_CHECKING:
    from cdpy.frame_manager import Frame, FrameManager  # noqa: F401
...
class ElementHandle(JSHandle):
    ...
    by the :meth:`cdpy.page.Page.querySelector` method.
    ...
    :meth:`cdpy.page.Page.querySelectorEval` and
    :meth:`cdpy.page.Page.evaluate` methods.
    """
    ...
    async def type(self, text: str, options: Dict = None, **kwargs: Any) -> None:
        ...
        Details see :meth:`cdpy.input.Keyboard.type` method.
        """
        options = merge_dict(options, kwargs)
        await self.focus()
    ...
    async def press(self, key: str, options: Dict = None, **kwargs: Any) -> None:
        ...
        :meth:`cdpy.input.keyboard.down` and
        :meth:`cdpy.input.keyboard.up`.
    ...
    async def screenshot(self, options: Dict = None, **kwargs: Any) -> bytes:
        ...
        Available options are same as :meth:`cdpy.page.Page.screenshot`.
        """
        options = merge_dict(options, kwargs)
