...
class ExecutionSummary(BaseModel):
    ...
    """ # noqa: E501
    effect: ExecutionEffect = Field(description="The actual effect that occurred in the target system as a result of this operation.  This indicates what change (if any) was made: a new record was created, an existing record was updated, an existing record was deleted, or no operation was needed because the system already matched the desired state.")
    effect_reason: Optional[ExecutionEffectReason] = Field(default=None, description="Optional reason why the operation resulted in the given effect.  This field provides structured information about why a particular effect occurred. Currently, this field is only used when `effect` is `noop` to provide specific reasons why no changes were made (e.g., `state_already_matches`).  This is useful for: - Understanding the specific circumstances of the operation - Filtering and analyzing operations by reason - Providing more detailed feedback to end users")
    request_fingerprint: StrictStr = Field(description="A unique fingerprint that identifies this specific execution request.  This value can be used to deduplicate requests, track operation history, and correlate related operations. It should be stable for the same input parameters.")
    is_idempotent: StrictBool = Field(description="Indicates whether this operation is idempotent and safe to retry with the same request fingerprint.  When true, executing this operation multiple times with the same request fingerprint will produce the same result without causing unintended side effects. This is useful for retry logic and ensuring operation safety.")
    description: Optional[StrictStr] = Field(default=None, description="Optional human-readable description of the execution result.  Provides additional context or details about what happened during execution that may be useful for debugging or user-facing messages. When `effect` is `noop`, this can provide additional details beyond what's captured in `effect_reason`.")
    caught_errors: Optional[List[Error]] = Field(default=None, description="Optional list of non-fatal errors that occurred during execution.  These are errors that were caught and handled gracefully during the operation. The operation completed successfully despite these errors, but they may contain important information for the end user (e.g., warnings about partial failures, deprecation notices, or validation issues that were automatically corrected).")
    __properties: ClassVar[List[str]] = ["effect", "effect_reason", "request_fingerprint", "is_idempotent", "description", "caught_errors"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in caught_errors (list)
        _items = []
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "effect_reason": obj.get("effect_reason"),
            "request_fingerprint": obj.get("request_fingerprint"),
            "is_idempotent": obj.get("is_idempotent"),
            "description": obj.get("description"),
            "caught_errors": [Error.from_dict(_item) for _item in obj["caught_errors"]] if obj.get("caught_errors") is not None else None
        })
        return _obj
