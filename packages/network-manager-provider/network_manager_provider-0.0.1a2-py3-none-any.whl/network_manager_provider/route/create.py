...
from network_manager_provider.defaults import ROUTE, CREATE
...
async def create(to, via, create_kwargs=None, ip_route_init_kwargs=None):
    ...
    func_kwargs = discover_pyroute2_func_kwargs(ROUTE, to=to, via=via, **create_kwargs)
    ...
    response = {}
    if not success:
        response["msg"] = f"Failed add route, to: {to}, via: {via}"
        if operation_msg:
            response["msg"] += f", err: {operation_msg}"
        return False, response

    response["msg"] = f"Added route to: {to} via: {via}"
