...
class ErrorMarginTest(apitestcase.ApiTestCase):
  ...
  def test_init(self):
    value = 2
    unit = PROJECTED
    errormargin = le.ErrorMargin(value, unit)

    errormargin_func = le.ApiFunction.lookup('ErrorMargin')
  ...
  def test_cast(self):
    value = 42
    unit = METERS
    errormargin = le.ErrorMargin(le.ErrorMargin(value, unit))
    result = json.loads(errormargin.serialize())
    expect = {
        'result': '0',
        'values': {
  ...
  def test_only_value(self):
    value = 3
    errormargin = le.ErrorMargin(value)
    result = json.loads(errormargin.serialize())
    expect = {
        'result': '0',
        'values': {
  ...
  def test_infinite_without_value(self):
    unit = INFINITE
    errormargin = le.ErrorMargin(unit=unit)
    result = json.loads(errormargin.serialize())
    expect = {
        'result': '0',
        'values': {
  ...
  def test_computed_object(self):
    ...
    value = 5
    unit = METERS
    result = le.ApiFunction.call_('ErrorMargin', value, unit)
  ...
  def test_need_value(self):
    ...
    with self.assertRaisesRegex(TypeError, message):
      le.ErrorMargin().getInfo()

    with self.assertRaisesRegex(TypeError, message):
      le.ErrorMargin(unit=PROJECTED).getInfo()

  def test_bad_casting_with_unit_not_none(self):
    error_margin = le.ErrorMargin(value=1)
    message = r'unit must be None if value is an ErrorMargin'
    with self.assertRaisesRegex(TypeError, message):
      le.ErrorMargin(error_margin, PROJECTED)
