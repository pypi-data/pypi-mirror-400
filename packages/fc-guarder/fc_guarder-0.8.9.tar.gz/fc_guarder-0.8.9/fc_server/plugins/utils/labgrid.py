...
class Labgrid(AsyncRunMixin):
    ...
    async def labgrid_get_comments(self):
        ...
        for line in place_line:
            ...
            if line.find("acquired:") >= 0:
                owner = line.split(":")[-1].strip().split("/")[-1]
                if owner not in ("fc", "None"):
                    if place in comments:
                        comments[place] = f"{owner}: {comments[place]}"
                    else:
                        comments[place] = owner
        return comments

    async def labgrid_get_reservations(self):
        cmd = "labgrid-client reservations"
        ret, reservations_text, _ = await self._run_cmd(cmd)

        if ret != 0:
            return
        if reservations_text == "":
            return ""
    ...
    async def labgrid_get_lock_info(self):
        cmd = "labgrid-client who | grep -v fc"
        _, locks_text, _ = await self._run_cmd(cmd)
