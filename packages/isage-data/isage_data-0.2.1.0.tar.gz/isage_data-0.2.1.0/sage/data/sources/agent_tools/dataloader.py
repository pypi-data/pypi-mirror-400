            ...
            for line_num, line in enumerate(f, 1):
                ...
                try:
                    ...
                    if tool.tool_id in self.tools:
                        raise ValueError(f"Duplicate tool_id '{tool.tool_id}' at line {line_num}")

                    # Check for duplicate name
                    if tool.name in self.name_to_id:
                        raise ValueError(f"Duplicate tool name '{tool.name}' at line {line_num}")

                    self.tools[tool.tool_id] = tool
                    self.name_to_id[tool.name] = tool.tool_id

                except Exception as e:
                    raise ValueError(f"Error parsing tool at line {line_num}: {e}") from e
