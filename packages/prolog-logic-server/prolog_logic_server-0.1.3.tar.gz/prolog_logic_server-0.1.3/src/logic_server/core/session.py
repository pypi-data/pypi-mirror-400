...
from logic_server.core.solver import execute_query, _format_term
...
class SessionManager:
    ...
    def assert_facts(self, session_id: str, facts: List[str]) -> Dict[str, Any]:
        ...
        # Support both string facts and structured dict facts
        normalized_facts: List[str] = []
        for fact in facts:
            # Convert dict facts to Prolog string format
            if isinstance(fact, dict):
                try:
                    fact = _format_term(fact)
                except (ValueError, KeyError, TypeError) as e:
                    return {
                        "success": False,
                        "error": f"Invalid fact format: {e}"
                    }

            # Normalize string facts
            fact = fact.strip()
    ...
    def assert_rules(self, session_id: str, rules: List[Any]) -> Dict[str, Any]:
        """
    ...
    def retract_rules(self, session_id: str, rules: List[Any]) -> Dict[str, Any]:
        """
