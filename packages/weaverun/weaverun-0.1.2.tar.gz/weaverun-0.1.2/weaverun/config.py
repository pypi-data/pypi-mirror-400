...
class Config:
    ...
    # Environment variable names to set to the proxy URL (default: OPENAI_BASE_URL)
    # This allows intercepting SDKs that use different env var names
    base_url_env_keys: list[str] = field(default_factory=lambda: ["OPENAI_BASE_URL"])
...
def load_config() -> Config:
    ...
    if config_path:
        ...
        # Check for custom base URL env keys
        base_url_keys = config_data.get("base_url_env_keys", None)
        if base_url_keys:
            if isinstance(base_url_keys, list):
                # Ensure OPENAI_BASE_URL is always included (it's the default)
                if "OPENAI_BASE_URL" not in base_url_keys:
                    base_url_keys = ["OPENAI_BASE_URL"] + base_url_keys
                config.base_url_env_keys = base_url_keys
            elif isinstance(base_url_keys, str):
                # Single string value
                keys = ["OPENAI_BASE_URL"]
                if base_url_keys != "OPENAI_BASE_URL":
                    keys.append(base_url_keys)
                config.base_url_env_keys = keys
