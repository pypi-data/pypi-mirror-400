...
def split_document():
    """
    Copies a predefined LangChain + Streamlit RAG application
    code into the user's clipboard silently.
    """
...
if st.sidebar.button("CREATE vector database"):
    ...
    # --------------------------------------------------
    # TEXT-LEVEL TRIMMING (STRUCTURE AGNOSTIC)
    # --------------------------------------------------
...
if user_input:
    ...
    (
        ...
        "You may summarize, paraphrase, or infer cautiously from the context.\n"
        "Do NOT use external knowledge.\n"
        "If the context is completely unrelated or empty, say:\n"
        "'No relevant information found in the provided data.'"
    ),
    ("human", "Question: {question}\n\nContext:\n{context}")
        ...
        if not retrieved_context.strip():
            bot_reply = "No information available in the given data"
        else:
            response = llm.invoke(
                prompt.format(
                    question=user_input,
