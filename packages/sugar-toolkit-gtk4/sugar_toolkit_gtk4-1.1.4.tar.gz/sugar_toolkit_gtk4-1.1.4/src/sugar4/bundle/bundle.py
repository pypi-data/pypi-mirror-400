...
class Bundle(object):
        ...
        if not os.path.isdir(self._path):
            try:
                ...
                raise MalformedBundleException(
                    "Error accessing zip file %r: %s" % (self._path, exception)
                )
    ...
    def _check_zip_bundle(self):
        ...
        if self._zip_root_dir.startswith("."):
            raise MalformedBundleException("root directory starts with .")
        if self._unzipped_extension is not None:
            ...
            if ext != self._unzipped_extension:
                raise MalformedBundleException(
                    ...
                    + "directory whose name ends with %r" % self._unzipped_extension
                )
    ...
    def _unzip(self, install_dir):
        ...
        if os.spawnlp(
            ...
            shutil.rmtree(
                os.path.join(install_dir, self._zip_root_dir), ignore_errors=True
            )
            # indicate failure.
