...
class PerformanceAnalyzer:
    ...
    def analyze_returns(self, returns: pd.Series, 
                       benchmark_returns: Optional[pd.Series] = None,
                       trade_data: Optional[pd.DataFrame] = None) -> PerformanceMetrics:
        """
        ...
        calmar_ratio = annualized_return / abs(drawdown_metrics['max_drawdown']) if drawdown_metrics['max_drawdown'] < 0 else 0.0
        
        # Omega ratio
        omega_ratio = self._calculate_omega_ratio(returns, self.risk_free_rate / periods_per_year)
        ...
        sterling_ratio = self._calculate_sterling_ratio(returns, drawdown_metrics['avg_drawdown'])
        ...
        metrics = PerformanceMetrics(
            ...
            total_days=(returns.index[-1] - returns.index[0]).days if len(returns) > 0 else 0,
    ...
    def _calculate_drawdown_metrics(self, cumulative_returns: pd.Series) -> Dict[str, float]:
    ...
    def _calculate_trade_statistics(self, trade_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'pnl' not in trade_data.columns:
            ...
            if all(col in trade_data.columns for col in ['quantity', 'entry_price', 'exit_price']):
                trade_data['pnl'] = trade_data['quantity'] * (trade_data['exit_price'] - trade_data['entry_price'])
            else:
    ...
    def _estimate_trade_statistics_from_returns(self, returns: pd.Series) -> Dict[str, Any]:
    ...
    def _calculate_sterling_ratio(self, returns: pd.Series, avg_drawdown: float) -> float:
    ...
    def _calculate_burke_ratio(self, returns: pd.Series, cumulative_returns: pd.Series) -> float:
    ...
    def _calculate_stability_metrics(self, cumulative_returns: pd.Series) -> Dict[str, float]:
    ...
    def _calculate_time_based_metrics(self, returns: pd.Series) -> Dict[str, float]:
        ...
        try:
            ...
            positive_months = (monthly_returns > 0).sum() / len(monthly_returns) * 100 if len(monthly_returns) > 0 else 0.0
        except:
    ...
    def _calculate_benchmark_metrics(self, returns: pd.Series, 
        ...
        # Information ratio
        information_ratio = active_returns.mean() / active_returns.std() * np.sqrt(periods_per_year) if active_returns.std() > 0 else 0.0
        ...
        if len(port_ret) > 1 and bench_ret.var() > 0:
            ...
            jensen_alpha = port_ann_ret - (self.risk_free_rate + beta * (bench_ann_ret - self.risk_free_rate))
            
            # Treynor ratio
            treynor_ratio = (port_ann_ret - self.risk_free_rate) / beta if beta != 0 else 0.0
        else:
            beta = 0.0
            jensen_alpha = 0.0
            treynor_ratio = 0.0
    ...
    def create_performance_report(self, metrics: PerformanceMetrics, 
        ...
        report.append(f"Max DD Duration:           {metrics.max_drawdown_duration:8d} periods")
        ...
        report.append(f"Stability:                 {metrics.stability_of_timeseries:8.2f}")
        ...
        if metrics.tracking_error is not None:
            ...
            report.append(f"Information Ratio:         {metrics.information_ratio:8.2f}")
...
class RollingPerformanceAnalyzer:
    ...
    def calculate_rolling_metrics(self, returns: pd.Series) -> pd.DataFrame:
        ...
        if len(returns) < self.window:
            raise ValueError(f"Need at least {self.window} periods for rolling analysis")
