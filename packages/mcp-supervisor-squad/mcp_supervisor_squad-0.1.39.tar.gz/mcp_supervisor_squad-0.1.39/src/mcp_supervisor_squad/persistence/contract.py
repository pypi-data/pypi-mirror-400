...
def write_repo_contract(*, layout: StorageLayout, cfg: RepoConfig, package_version: str) -> Path:
    ...
    payload: dict[str, Any] = {
        ...
        "principles": {
            "external_output": "squad_status returns event_lines + forced_wait_s (terminal adds next_step); details are persisted under .codex/supervisor-squad/",
            "max_project_roots": 2,
            "no_compat": True,
        },
        "paths": {
        ...
        "workflow": {
            ...
            "recommended_call_chain": ["squad_start", "poll squad_status (stop on next_step/forced_wait_s=0)", "squad_apply (no-op in direct-write mode)"],
            "phases": ["planner_light", "planner_heavy", "coder_run", "done", "cleanup", "error"],
            "required_parameters": {
                "squad_status": {"options.cwd": "absolute path inside the target repo"},
                "squad_apply": {"options.cwd": "absolute path inside the target repo"},
            },
            "notes": [
                ...
                "squad_status returns run_id + forced_wait_s and event_lines (no has_update/event_id/summary_lines); non-terminal responses keep only the latest event_lines, terminal responses include full event_lines plus next_step='stop polling; print event_lines once'.",
                "When stopped, forced_wait_s=0; clients should print event_lines (if present), follow next_step, and stop polling squad_status.",
                "Coder writes changes directly to the repo (no patch.diff generation/apply). squad_apply is a no-op wrapper for compatibility.",
