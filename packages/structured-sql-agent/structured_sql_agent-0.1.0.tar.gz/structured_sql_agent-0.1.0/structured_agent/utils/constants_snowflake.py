...
decomposer_template = """
You are a highly skilled snowflake SQL Query generator.
...
Crucially, also identify and apply all relevant default filter values from [Custom Instructions] if the [Question] does not specify them.
...
question 1: Get the Average Value of SAT Excellence Rate of Charter Schools
SQL
"'sql
...
"'
question 2: List School Names of Charter Schools with SAT Excellence Rate Over the Average
SQL
"'sql
SELECT T2."sname"
FROM frpm AS T1
...
Solve the task step by step if you need to and use verified queries as reference to curate the query if applicable to the [Question].
