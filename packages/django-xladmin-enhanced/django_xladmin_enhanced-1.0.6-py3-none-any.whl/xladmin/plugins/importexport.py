...
from xladmin.views import BaseAdminPlugin, ListAdminView, ModelAdminView
...
class ImportMenuPlugin(BaseAdminPlugin):
    ...
    def block_top_toolbar(self, context, nodes):
        ...
        if has_change_perm and has_add_perm:
            ...
            import_url = reverse('xladmin:%s_%s_import' % model_info, current_app=self.admin_site.name)
            context = get_context_dict(context or {})  # no error!
            context.update({
                'import_url': import_url,
            })
            nodes.append(loader.render_to_string('xladmin/blocks/model_list.top_toolbar.importexport.import.html',
                                                 context=context))
...
class ImportBaseView(ModelAdminView):
    ...
    import_template_name = 'xladmin/import_export/import.html'
    #: resource class
...
class ImportView(ImportBaseView):
    ...
    def get_media(self):
        ...
        media = media + self.vendor('xladmin.plugin.importexport.css')
        return media
...
class ImportProcessView(ImportBaseView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if confirm_form.is_valid():
            ...
            model_info = (self.opts.app_label, self.opts.model_name)
            url = reverse('xladmin:%s_%s_changelist' % model_info,
...
class ExportMenuPlugin(ExportMixin, BaseAdminPlugin):
    ...
    def get_media(self, media):
        return media + self.vendor('xladmin.plugin.importexport.css', 'xladmin.plugin.importexport.js')
    ...
    def block_top_toolbar(self, context, nodes):
        ...
        nodes.append(loader.render_to_string('xladmin/blocks/model_list.top_toolbar.importexport.export.html',
                                             context=context))
