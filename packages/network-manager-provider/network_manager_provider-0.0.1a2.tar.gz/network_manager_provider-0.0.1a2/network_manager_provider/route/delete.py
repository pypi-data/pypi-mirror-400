...
from network_manager_provider.defaults import ROUTE, DELETE
...
async def delete(to, via=None, delete_kwargs=None, ip_route_init_kwargs=None):
    ...
    func_name = discover_pyroute2_function_name(ROUTE)
    func_args = discover_pyroute2_func_args(DELETE)
    func_kwargs = discover_pyroute2_func_kwargs(ROUTE, to=to, via=via, **delete_kwargs)
    ...
    response = {}
    if not success:
        response["msg"] = f"Failed remove route: {to}/{via}"
        if operation_msg:
            response["msg"] += f", err: {operation_msg}"
        return False, response

    response["msg"] = f"Removed route: {to}, via: {via}"
