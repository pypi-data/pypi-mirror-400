...
from .env import TEST_RUN_NUMBER
...
def test_query_runs() -> None:
    ...
    data = dials.oms.query(
        endpoint="runs", filters=[OMSFilter(attribute_name="run_number", value=TEST_RUN_NUMBER, operator="EQ")]
    )
...
def test_query_lumisections() -> None:
    ...
    data = dials.oms.query(
        ...
        filters=[OMSFilter(attribute_name="run_number", value=TEST_RUN_NUMBER, operator="EQ")],
...
def test_query_datasetrates() -> None:
    ...
    data = dials.oms.query(
        ...
        filters=[
            OMSFilter(attribute_name="run_number", value=TEST_RUN_NUMBER, operator="EQ"),
            OMSFilter(attribute_name="dataset_name", value="ZeroBias", operator="EQ"),
        ],
        pages=[OMSPage(attribute_name="limit", value=5000)],
    )
    assert isinstance(data, dict)
    assert data["meta"]["totalResourceCount"] == 98
