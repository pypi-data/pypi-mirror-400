    ...
    display(
        HTML(
            ...
            <style>
                ...
                .le_map_tools-dark .jupyter-button {
                ...
                .le_map_tools-colab .jupyter-button {
...
class Theme:
    ...
    def apply(cls: Any) -> Any:
        ...
        def wrapper(self, *args, **kwargs):
            ...
            self.add_class("le_map_tools-{}".format(Theme.current_theme))
...
class Inspector(ipywidgets.VBox):
    ...
    def _query_point(
        self, latlon: List[float], ee_object: le.ComputedObject
        ...
        """
        point = le.Geometry.Point(latlon[::-1])
        scale = self._host_map.get_scale()
        if isinstance(ee_object, le.ImageCollection):
            ee_object = ee_object.mosaic()
        if isinstance(ee_object, le.Image):
            return ee_object.reduceRegion(le.Reducer.first(), point, scale).getInfo()
        return None
    ...
    def _get_bbox(self, latlon: List[float]) -> le.Geometry.BBox:
        ...
        lat, lon = latlon
        delta = 0.005
        return le.Geometry.BBox(lon - delta, lat - delta, lon + delta, lat + delta)
    ...
    def _objects_info(self, latlon: List[float]) -> ipytree.Node:
        ...
        for layer_name, layer in layers.items():
            ...
            if isinstance(ee_object, le.FeatureCollection):
                geom = le.Feature(ee_object.first()).geometry()
                bbox = self._get_bbox(latlon)
                is_point = le.Algorithms.If(
                    geom.type().compareTo(le.String("Point")), point, bbox
                )
...
class LayerManager(ipywidgets.VBox):
    ...
    def __init__(self, host_map: "le_map_tools.Map"):
        """Initializes a layer manager widget.

        Args:
            host_map (geemap.Map): The le_map_tools.Map object.
        """
        self._host_map = host_map
        if not host_map:
...
class LayerEditor(ipywidgets.VBox):
    ...
    def __init__(self, host_map: "le_map_tools.Map", layer_dict: Optional[Dict[str, Any]]):
        ...
        if layer_dict is not None:
            ...
            if isinstance(self._ee_object, (le.Feature, le.Geometry)):
                self._ee_object = le.FeatureCollection(self._ee_object)

            self._ee_layer = layer_dict["ee_layer"]
            self._label.value = self._ee_layer.name
            if isinstance(self._ee_object, le.FeatureCollection):
                self._embedded_widget = _VectorLayerEditor(
                    host_map=host_map, layer_dict=layer_dict
                )
            elif isinstance(self._ee_object, le.Image):
...
class _RasterLayerEditor(ipywidgets.VBox):
    ...
    def __init__(self, host_map: "le_map_tools.Map", layer_dict: Dict[str, Any]):
    ...
    def _update_stretch(self, *args: Any) -> None:
        ...
        map_bbox = le.Geometry.BBox(west=w, south=s, east=e, north=n)
    ...
    def _style_chk_changed(self, change: Dict[str, Any]) -> None:
        ...
        if change["new"]:
            ...
            self._field_dropdown.options = (
                le.Feature(self._ee_object.first()).propertyNames().getInfo()
            )
            self._compute_label.value = ""
