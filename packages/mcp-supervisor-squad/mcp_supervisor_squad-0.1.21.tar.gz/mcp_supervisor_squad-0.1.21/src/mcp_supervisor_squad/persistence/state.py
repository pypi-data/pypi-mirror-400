...
class RunState:
    ...
    def public_status(self, *, options: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if isinstance(options, dict):
            try:
                since_event = int(options.get("since_event_id")) if options.get("since_event_id") is not None else None
            except Exception:
                since_event = None
        ...
        has_update = True
        if since_event is not None and since_event >= int(self.event_seq):
            has_update = False
