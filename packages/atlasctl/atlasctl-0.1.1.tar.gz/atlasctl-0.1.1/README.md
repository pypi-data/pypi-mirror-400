...
```bash
sudo atlasctl setup      # Download binary, generate config
sudo atlasctl start      # Start monitoring daemon
sudo atlasctl stop       # Stop daemon
sudo atlasctl status     # Check status and trial info
sudo atlasctl logs       # View recent logs
sudo atlasctl claim      # Open dashboard to claim this server
sudo atlasctl uninstall  # Remove Atlas
```
