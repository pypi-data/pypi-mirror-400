...
class SentinelCog(Cog):
    ...
    async def sentinel_log_channel(self, guild: discord.Guild) -> GuildMessageableChannel | None:
    ...
    async def sentinel_contact_target(
        ...
        *,
        ...
        if channel := next((ch for ch in guild.channels if isinstance(ch, GuildMessageableChannel)), None):
    ...
    async def sentinel_send_log(self, moderation: Moderation) -> discord.Message:
        ...
        if not (log_channel := await self.sentinel_log_channel(guild)) or not isinstance(log_channel, GuildMessageableChannel):
