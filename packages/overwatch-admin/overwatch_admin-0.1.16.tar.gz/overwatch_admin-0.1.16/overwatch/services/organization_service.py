...
class OrganizationService:
    ...
    async def get_organizations_paginated(
        ...
        """
        query = select(Organization).options(selectinload(Organization.parent))
    ...
    async def create_organization(
        ...
        if organization_data.parent_id:
            ...
            parent = await self.get_organization_by_id(organization_data.parent_id)
            if not parent:
                raise ValueError(
                    f"Parent organization with ID {organization_data.parent_id} not found"
                )
    ...
    async def update_organization(
        ...
        if "parent_id" in updates:
            ...
            if new_parent_id:
                # Query new parent organization
                parent = await self.get_organization_by_id(new_parent_id)
                if not parent:
                    raise ValueError(
                        f"Parent organization with ID {new_parent_id} not found"
                    )
                # Validate: parent cannot be a descendant of this organization
                if parent.id == organization_id:
                    raise ValueError("Organization cannot be its own parent")
                # Check if parent is a descendant (would create a cycle)
                current = parent
                while current.parent_id is not None:
                    current_parent_id = current.parent_id
                    assert isinstance(current_parent_id, int), "parent_id should be an int when not None"
                    if current_parent_id == organization_id:
                        raise ValueError(
                            "Cannot set parent to a descendant (would create a cycle)"
                        )
                    current = await self.get_organization_by_id(current_parent_id)
                    if not current:
                        break
                organization.level = parent.level + 1
            else:
                # No parent, set to root level
        ...
        try:
            for org_data in organizations_data:
                ...
                if org_data.parent_id:
                    # Query parent organization
                    parent = await self.get_organization_by_id(org_data.parent_id)
                    if not parent:
                        raise ValueError(
                            f"Parent organization with ID {org_data.parent_id} not found"
                        )
                    level = parent.level + 1
    ...
    def _serialize_organization_to_dict(self, organization: Organization) -> dict[str, Any]:
        ...
        if organization.parent:
            data["parent"] = self._serialize_organization_to_dict(organization.parent)
