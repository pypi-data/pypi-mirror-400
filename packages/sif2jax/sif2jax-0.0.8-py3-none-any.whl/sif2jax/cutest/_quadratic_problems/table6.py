...
_A_ROWS = jnp.asarray(
    np.load(os.path.join(_data_dir, "table6_A_rows.npy")), dtype=jnp.int32
)
_A_COLS = jnp.asarray(
    np.load(os.path.join(_data_dir, "table6_A_cols.npy")), dtype=jnp.int32
)
_A_VALS = jnp.asarray(np.load(os.path.join(_data_dir, "table6_A_vals.npy")))
_LOWER_BOUNDS = jnp.asarray(np.load(os.path.join(_data_dir, "table6_lower_bounds.npy")))
_UPPER_BOUNDS = jnp.asarray(np.load(os.path.join(_data_dir, "table6_upper_bounds.npy")))
_Q_DIAG_VALS = jnp.asarray(np.load(os.path.join(_data_dir, "table6_Q_diag_vals.npy")))
_M_VAL = int(np.load(os.path.join(_data_dir, "table6_m.npy")))


class TABLE6(AbstractConstrainedQuadraticProblem):
    ...
    SIF input: Jordi Castro, 2006 as L2_table6.mps
    see http://www-eio.upc.es/~jcastro/data.html

    classification QLR2-RN-1584-510
    """
