"""
Problem OET4: Nonlinear Chebyshev approximation problem (discretized).
...
class OET4(AbstractConstrainedMinimisation):
    """OET4: Nonlinear Chebyshev approximation problem.
    ...
    def _phi(self, w, x1, x2, x3):
        """Evaluate phi(x,w) = exp(w) - (x1 + x2*w) / (1 + x3*w)."""
        return jnp.exp(w) - (x1 + x2 * w) / (1.0 + x3 * w)

    def constraint(self, y):
        """Constraints from SIF file:
        LO(I): U - (X1 + X2*W)/(1 + X3*W) >= -EXP(W)
              →  U - (X1 + X2*W)/(1 + X3*W) + EXP(W) >= 0
        UP(I): U + (X1 + X2*W)/(1 + X3*W) >= EXP(W)
              →  U + (X1 + X2*W)/(1 + X3*W) - EXP(W) >= 0
        """
        w_points = self.args
        u, x1, x2, x3 = y

        # Compute the ratio element
        ratio = (x1 + x2 * w_points) / (1.0 + x3 * w_points)

        # LO(I): U - ratio + EXP(W) >= 0
        lower_constraints = u - ratio + jnp.exp(w_points)

        # UP(I): U + ratio - EXP(W) >= 0
        upper_constraints = u + ratio - jnp.exp(w_points)
