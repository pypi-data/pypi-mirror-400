...
SAMPLEID_ANY_OF_SCHEMAS = ["int", "str"]
...
class SampleId(BaseModel):
    ...
    if TYPE_CHECKING:
        ...
        actual_instance: Any
    any_of_schemas: List[str] = Field(SAMPLEID_ANY_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in SampleId with anyOf schemas: int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> SampleId:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> SampleId:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into SampleId with anyOf schemas: int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
