...
def create_rightshifted_model_input_and_leftshifted_targets(
    chunks: list[hpcai.ModelInputChunk],
) -> tuple[hpcai.ModelInput, list[int]]:
    """
    ...
    last_chunk = chunks[-1]
    if not isinstance(last_chunk, hpcai.types.EncodedTextChunk):
        raise ValueError(
    ...
    input_chunks: list[hpcai.ModelInputChunk] = list(chunks[:-1])
    if last_chunk.length > 1:
        input_chunks.append(hpcai.types.EncodedTextChunk(tokens=last_chunk.tokens[:-1]))

    # Build target tokens: collect all tokens, then slice off first
    all_tokens: list[int] = []
    for chunk in chunks:
        if isinstance(chunk, hpcai.types.EncodedTextChunk):
            all_tokens.extend(chunk.tokens)
        else:
            all_tokens.extend([0] * chunk.length)
    target_tokens = all_tokens[1:]

    return hpcai.ModelInput(chunks=input_chunks), target_tokens


def datum_from_model_input_weights(
    model_input: hpcai.ModelInput,
    ...
    if max_length is not None:
        ...
        while total_length > max_length and model_input_chunks:
            ...
            if isinstance(last, hpcai.types.EncodedTextChunk):
                overflow = total_length - max_length
                if overflow < last.length:
                    # Partial truncation of text chunk
                    model_input_chunks[-1] = hpcai.types.EncodedTextChunk(
    ...
    while model_input_chunks and isinstance(
        model_input_chunks[-1], (hpcai.types.ImageChunk, hpcai.types.ImageAssetPointerChunk)
    ):
        model_input_chunks.pop()
