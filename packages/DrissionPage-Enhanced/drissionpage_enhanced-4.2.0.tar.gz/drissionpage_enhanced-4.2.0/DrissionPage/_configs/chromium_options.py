...
class ChromiumOptions(object):
    ...
    def set_proxy(self, proxy):
        """Set proxy. Supports authentication format: username:password@host:port"""
        if proxy.lower().startswith('socks'):
            print(_S._lang.UNSUPPORTED_SOCKS_PROXY)
        
        # Store original proxy (with auth if present)
        self._proxy = proxy
        
        # For Chrome argument, remove auth credentials
        if search(r'.*?:.*?@.*', proxy):
            # Extract host:port without credentials
            from re import sub
            proxy_without_auth = sub(r'(https?://)?[^:]+:[^@]+@', r'\1', proxy)
            return self.set_argument('--proxy-server', proxy_without_auth)
