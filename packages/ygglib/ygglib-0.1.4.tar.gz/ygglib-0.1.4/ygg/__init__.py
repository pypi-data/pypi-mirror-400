...
class YGGClient:
    ...
    def _make_request(self, method: str, endpoint: str, data: Optional[Dict] = None) -> Any:
        ...
        """
        # Ensure base_url is always the hardcoded production API
        if self.config.base_url != YGG_API_BASE_URL:
            self.config.base_url = YGG_API_BASE_URL
...
def init(api_key=None, **kwargs):
    """Initialize the YGG client with your API key.
    
    Args:
        api_key (str, optional): Your YGG_API_KEY for authentication. 
                                 If not provided, will use YGG_API_KEY environment variable.
    ...
    if api_key is None:
        ...
        if api_key is None:
            raise ValueError("YGG_API_KEY is required. Either pass it to init() or set YGG_API_KEY environment variable.")
...
def get_client():
    ...
    Automatically initializes the client using YGG_API_KEY environment variable if not already initialized.
    
    Returns:
        YGGClient: The initialized client instance
        
    Raises:
        ValueError: If YGG_API_KEY environment variable is not set and client is not initialized
    """
    global _client
    if _client is None:
        # Auto-initialize using environment variable
        api_key = os.environ.get("YGG_API_KEY")
        if api_key is None:
            raise ValueError("YGG_API_KEY environment variable is required. Set it or call ygg.init(api_key) first.")
        init(api_key)
    return _client
