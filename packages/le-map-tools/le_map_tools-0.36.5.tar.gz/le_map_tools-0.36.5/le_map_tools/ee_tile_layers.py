...
# *******************************************************************************#
# This module contains core features features of the le_map_tools package.             #
...
def _get_tile_url_format(
    ee_object: Union[
        le.Geometry, le.Feature, le.FeatureCollection, le.Image, le.ImageCollection
    ],
...
def _ee_object_to_image(
    ...
    """
    if isinstance(ee_object, (le.Geometry, le.Feature, le.FeatureCollection)):
        ...
        return (
            ...
            .updateMask(le.Image.constant(0.5))
            .blend(image_outline)
        )
    elif isinstance(ee_object, le.Image):
        return ee_object
...
class EELeafletTileLayer(ipyleaflet.TileLayer):
    ...
    def _calculate_vis_stats(
        self,
        *,
        bounds: Union[le.Geometry, le.Feature, le.FeatureCollection],
        bands: Tuple[str, ...],
        ...
        stat_reducer = (
            le.Reducer.minMax()
            .combine(le.Reducer.mean().unweighted(), sharedInputs=True)
            .combine(le.Reducer.stdDev(), sharedInputs=True)
        )
