...
class TaskTest(unittest.TestCase):
  ...
  def testStartWithoutConfig(self):
    ...
    with self.assertRaisesRegex(le.EEException, 'Task config'):
      task.start()
  ...
  def testStartUnknownTaskType(self):
    ...
    with self.assertRaisesRegex(
        le.EEException, f'Unknown Task type "{task_type}"'
    ):
      task.start()
...
class BatchTestCase(apitestcase.ApiTestCase):
  ...
  def testTaskStartCloudApi(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=mock_cloud_api_resource):
      task = le.batch.Export.table(le.FeatureCollection('foo'), 'bar')
      task.start()
  ...
  def testExportImageTrivialRegionCloudApi(self):
    ...
    with apitestcase.UsingCloudApi():
      ...
      task = le.batch.Export.image.toAsset(
          le.Image(42), assetId='users/foo/bar', region=region, scale=1000
      )
      expected_expression = le.Image(42).clipToBoundsAndScale(
          geometry=le.Geometry.LineString(region), scale=1000
      )
  ...
  def testExportImageCloudApi(self):
    ...
    with apitestcase.UsingCloudApi():
      region = le.Geometry.Rectangle(1, 2, 3, 4)
      config = dict(
      ...
      task = le.batch.Export.image(le.Image(1), 'TestDescription', config)
      expected_expression = (
          le.Image(1)
  ...
  def testExportImageToAssetCloudApi(self):
    ...
    with apitestcase.UsingCloudApi():
      ...
      expected_expression = le.Image(1)

      # Test keyed parameters.
      task_keyed = le.batch.Export.image.toAsset(
          image=config['image'],
      ...
      task_ordered = le.batch.Export.image.toAsset(
          config['image'],
          'TestDescription',
          config['assetId'],
          maxPixels=1000,
  ...
  def testExportImageToCloudStorageCloudApi(self):
    ...
    with apitestcase.UsingCloudApi():
      ...
      expected_expression = le.Image(1).clip(region)
      self.assertIsNone(task.id)
      self.assertIsNone(task.name)
      self.assertEqual('EXPORT_IMAGE', task.task_type)
      self.assertEqual('UNSUBMITTED', task.state)
      ...
      task_with_priority = le.batch.Export.image.toCloudStorage(
          le.Image(1),
          'TestDescription',
          config['outputBucket'],
          None,
  ...
  def testExportImageToGoogleDriveCloudApi(self):
    ...
    with apitestcase.UsingCloudApi():
      region = le.Geometry.Rectangle(1, 2, 3, 4)
      drive_task_by_keys = le.batch.Export.image.toDrive(
          image=le.Image(1),
          region=region['coordinates'],
          folder='foo',
          maxPixels=10**10,
      ...
      drive_task_with_old_keys = le.batch.Export.image.toDrive(
          image=le.Image(1),
          region=region['coordinates'],
          driveFolder='foo',
      ...
      with self.assertRaisesRegex(
          le.EEException, 'Unknown configuration options.*'
      ):
        le.batch.Export.image.toDrive(image=le.Image(1), framesPerSecond=30)

      drive_task_with_priority = le.batch.Export.image.toDrive(
          image=le.Image(1),
          region=region['coordinates'],
          folder='foo',
          maxPixels=10**10,
  ...
  def testExportMapToCloudStorageCloudApi(self):
    ...
    with apitestcase.UsingCloudApi():
      ...
      # Test keyed parameters.
      task_keyed = le.batch.Export.map.toCloudStorage(
          image=config['image'],
      ...
      with self.assertRaisesRegex(
        ...
        le.batch.Export.map.toCloudStorage(**config_with_bogus_option)

      # Test ordered parameters.
      task_ordered = le.batch.Export.map.toCloudStorage(
          config['image'],
          'TestDescription',
          config['bucket'],
          'jpeg',
      ...
      task_with_priority = le.batch.Export.map.toCloudStorage(
          image=config['image'],
          bucket=config['bucket'],
          maxZoom=config['maxZoom'],
          path=config['path'],
  ...
  def testExportTableCloudApi(self):
    ...
    with apitestcase.UsingCloudApi():
      task = le.batch.Export.table(
          le.FeatureCollection('drive test FC'), config={'maxWorkers': 100}
      )
      self.assertIsNone(task.id)
      self.assertIsNone(task.name)
      ...
      self.assertEqual(
          {
              'expression': le.FeatureCollection('drive test FC'),
  ...
  def testExportTableCloudApiBogusParameter(self):
    ...
    with apitestcase.UsingCloudApi():
      with self.assertRaisesRegex(
        ...
        le.batch.Export.table.toDrive(
            le.FeatureCollection('drive test FC'), framesPerSecond=30
        )
  ...
  def testExportTableToCloudStorageCloudApi(self):
    ...
    with apitestcase.UsingCloudApi():
      ...
      self.assertEqual(
          {
              'expression': le.FeatureCollection('foo'),
              'description': 'myExportTableTask',
              'fileExportOptions': {
                  'fileFormat': 'CSV',
                  'cloudStorageDestination': {
      ...
      task_with_priority = le.batch.Export.table.toCloudStorage(
          collection=le.FeatureCollection('foo'),
          outputBucket='test-bucket',
          maxVertices=1e6,
          priority=999,
  ...
  def testExportTableToGoogleDriveCloudApi(self):
    ...
    with apitestcase.UsingCloudApi():
      test_collection = le.FeatureCollection('foo')
      ...
      # Ordered parameters
      task_ordered = le.batch.Export.table.toDrive(
          test_collection,
          test_description,
          None,
      ...
      # still work.
      task_old_keys = le.batch.Export.table.toDrive(
      ...
      task_new_keys = le.batch.Export.table.toDrive(
          collection=test_collection,
          folder='fooFolder',
      ...
      task_with_priority = le.batch.Export.table.toDrive(
          test_collection,
          test_description,
          None,
          test_file_name_prefix,
  ...
  def testExportTableToAssetCloudApi(self):
    ...
    with apitestcase.UsingCloudApi():
      ...
      task_with_priority = le.batch.Export.table.toAsset(
          collection=le.FeatureCollection('foo'),
          description='foo',
          assetId='users/foo/bar',
          priority=999,
  ...
  def testExportTableToFeatureViewCloudApi(self):
    ...
    with apitestcase.UsingCloudApi():
      ...
      task_with_priority = le.batch.Export.table.toFeatureView(
          collection=le.FeatureCollection('foo'),
          description='foo',
          assetId='users/foo/bar',
          ingestionTimeParameters={
  ...
  def testExportTableToFeatureViewBadRankByOneThingRule(self):
    ...
    with self.assertRaisesRegex(
        le.EEException, 'Ranking rule format is invalid.*'
    ):
      le.batch.Export.table.toFeatureView(
          collection=le.FeatureCollection('foo'),
          assetId='users/foo/bar',
  ...
  def testExportTableToFeatureViewBadRankingRule(self):
    ...
    with self.assertRaisesRegex(
        le.EEException, 'Unable to build ranking rule from rules.*'
    ):
      le.batch.Export.table.toFeatureView(
          collection=le.FeatureCollection('foo'),
          assetId='users/foo/bar',
  ...
  def testExportTableToBigQueryRequiredParams(self):
    ...
    with apitestcase.UsingCloudApi():
      task = le.batch.Export.table.toBigQuery(
          collection=le.FeatureCollection('foo'),
      ...
      with self.subTest(name='PriorityIsSet'):
        task_with_priority = le.batch.Export.table.toBigQuery(
            collection=le.FeatureCollection('foo'),
            table='project.dataset.table',
            description='foo',
            priority=999,
  ...
  def testExportTableToBigQueryBadTableName(self):
    ...
    with apitestcase.UsingCloudApi():
      with self.assertRaisesRegex(
        ...
        le.batch.Export.table.toBigQuery(
            collection=le.FeatureCollection('foo'),
  ...
  def testExportVideoCloudApi(self):
    ...
    with apitestcase.UsingCloudApi():
      ...
      collection = le.ImageCollection([le.Image(1), le.Image(2)])
      task = le.batch.Export.video(collection, 'TestVideoName', config)
      self.assertIsNone(task.id)
      self.assertIsNone(task.name)
      ...
      gcs_task = le.batch.Export.video(collection, 'TestVideoName', config)
  ...
  def testExportVideoToCloudStorageCloudApi(self):
    ...
    with apitestcase.UsingCloudApi():
      ...
      # Test keyed parameters.
      task_keyed = le.batch.Export.video.toCloudStorage(
      ...
      # Test ordered parameters.
      task_ordered = le.batch.Export.video.toCloudStorage(
          collection,
          'TestVideoName',
          'test-bucket',
          None,
      ...
      task_with_priority = le.batch.Export.video.toCloudStorage(
          collection=collection,
          description='TestVideoName',
          bucket='test-bucket',
          dimensions=16,
  ...
  def testExportVideoToDriveCloudApi(self):
    ...
    with apitestcase.UsingCloudApi():
      ...
      # Test keyed parameters.
      task_keyed = le.batch.Export.video.toDrive(
          collection=collection,
          description='TestVideoName',
          folder='test-folder',
          dimensions=16,
      ...
      # Test ordered parameters.
      task_ordered = le.batch.Export.video.toDrive(
          collection,
          'TestVideoName',
          'test-folder',
          None,
      ...
      task_with_priority = le.batch.Export.video.toDrive(
          collection=collection,
          description='TestVideoName',
          folder='test-folder',
          dimensions=16,
  ...
  def testExportWorkloadTag(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=mock_cloud_api_resource):
      le.data.setWorkloadTag('test-export')
      task = le.batch.Export.table(le.FeatureCollection('foo'), 'bar')
      le.data.setWorkloadTag('not-test-export')
