...
from spacedata.models.donki import DONKIDateRangeInput, DONKIRbe
...
async def _get_response(
    ...
    response = await resource.client.make_request(
        method="GET",
        url=f"{resource.client.settings.main_base_url}/DONKI/RBE",
        params=params,
    )
    return [DONKIRbe.model_validate(item) for item in response]
...
async def rbe(
    ...
    **kwargs,
) -> SpaceDataResult[DONKIRbe]:
    self.client.logger.debug("Calling rbe endpoint with input_data: %s", input_data)
