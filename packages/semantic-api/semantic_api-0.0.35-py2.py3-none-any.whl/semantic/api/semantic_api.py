...
from semantic.common.utils import msg, err, redMsg, greenMsg, to_json_ld_type, resolve_either, reject_promised, reject_either,  instance_count, idForPath
from semantic.common.common_types import SchemaId, PipeArg, QueryOptions
...
class semantic:
   ...
   async def inserted_uris(self):
      #greenMsg('inserted URIs:')
      uris = list()
      while len(self.futures) > 0:
         done = await self.futures.pop(0)
         if done is not None:
            batch = done.get('doc').get('URIs').get('additions')
            if batch is not None:
               uris.extend(batch)
            batch = done.get('doc').get('URIs').get('updates')
            if batch is not None:
               uris.extend(batch)
         else:
            uris = []
         #greenMsg(json.dumps(batch, indent=6))
      return uris
         
      
    
   
   def insert(self) -> asyncio.Future:
    '''
      preconditions: 
    ...
    if insert_either.is_right() and self.type_map_is_not_empty():
       try: 
         redMsg("insert")
         #redMsg("jsonStr "+json.dumps(jsonStr, indent=6))
         self.futures.append(self.client.insert(jsonStr, "SynchInsert"))
         #redMsg("size of futures: "+str(len(self.futures)))
         self.clear_map()
  ...
  '''
   ...
   def update_map(self, obj_thunk, *arg):
      ...
      if obj.is_right():
        ...
        '''
        json_ld_object = to_json_ld_type(obj.either(lambda : {}, lambda x: x))
        #msg(f'update_map json_ld_object {json.dumps(json_ld_object, indent=6)}')
   ...
   #def query_constructor(self, type):
   #   return query_object(type)
   
   def query_constructor(self, limit: int= 10, order: bool= True):
      async def query_request(frame): 
        result: PipeArg = { }
        options: QueryOption = { "limit": limit, "order": order}

        response_either: Either = await self.client.frame(frame, options)

        if response_either is None :
            return reject_either(f'Error: query with frame {json.dumps(frame, indent=6)}')
            ...
            #greenMsg('query_request: result.doc '+json.dumps(result['doc'], indent=6))
   ...
   def batch_controller(self):
        ...
        if over_count and self.auto_insert == True:
            greenMsg(f'batch_controller inserting : count: {count}  ')
   ...
   async def get_usd_path_for_segment(self, segmentName: str) -> Either:
          ...
          if (prim_either is None or prim_either.is_left()):
            ...
            '''
            greenMsg('get_usd_path_for_segment: @id '+json.dumps(prim.get('@id'), indent=6))
            name = prim.get('name')
            if name is not None:
              result = Right(name)
   ...
   async def get_id_for_usd_segment(self, segmentName: str) -> Either:
          ...
          if (prim_either is None or prim_either.is_left()):
            txt = f'get_id_for_usd_segment: failure in query (not empty query return) for frame @type: UsdPrim segmentName: {segmentName} '
            redMsg(txt)
            result = Left(txt)
          else:
            prim = prim_either.either(lambda e: f'get_id_for_usd_segment: Error: {e}', lambda x: x)
            '''
                an empty query will have a context object and nothing else in it
                so we have naught yet caught an empty query
            '''
            ...
            id = prim.get('@id')
            #greenMsg('get_id_for_usd_segment: @id '+json.dumps(id, indent=6))
            if id is not None:
              result = Right(id)
              #greenMsg('get_id_for_usd_segment: '+name)
            else:
              txt = f'get_id_for_usd_segment: frame @type: UsdPrim segmentName: {segmentName} returned empty'
              redMsg(txt)
              result = Left(txt)
   ...
   async def get_usd_parent_ref(self, segmentName):
      ...
      ref_either : Either = await self.get_id_for_usd_segment(segmentName)
   ...
   if obj is not None :
      ...
      if len(key_values)  == 0:
        ...
        return None
         ...
         #greenMsg('key_values: '+json.dumps(key_values))
