...
class Worker:
    ...
    def _run(self, run_id: str, repo_root_hint: str | None = None) -> None:
        ...
        planner_worktree = layout.worktrees_dir / run_id / "planner"
        planner_worktree.parent.mkdir(parents=True, exist_ok=True)
        if not planner_worktree.exists():
            code, out = _run_git(repo_root, ["worktree", "add", "--detach", str(planner_worktree)])
            if code != 0:
                _ = out
                update(
                    phase="error",
                    next_action="wait",
                    wait_reason="error",
                    important_event="planner worktree 创建失败",
                    important_event_at=utc_now_iso(),
                )
                return

        # Best-effort sparse checkout to keep planner context small.
        ...
        try:
            _run_git(planner_worktree, ["sparse-checkout", "init", "--no-cone"])
            patterns = [f"{p}/**" for p in allow_prefixes if p] + [".codex/**"]
            if patterns:
                _run_git(planner_worktree, ["sparse-checkout", "set", "--no-cone", "--skip-checks", *patterns])
        except Exception:
            pass
        ...
        def set_requirements_brief(current: RunState) -> None:
            ...
            current.review_expires_at = (
                (datetime.now(timezone.utc) + timedelta(seconds=15)).isoformat()
            )

        update(mutator=set_requirements_brief, phase="review", wait_reason="review_window", important_event=important, important_event_at=utc_now_iso())

        # Short auto-review window: give director a chance to cancel before coder starts.
        while True:
            time.sleep(1.0)
            try:
            ...
            if current.phase != "review":
                break
            try:
                deadline = datetime.fromisoformat((current.review_expires_at or "").replace("Z", "+00:00"))
            except Exception:
                deadline = datetime.now(timezone.utc)
            if datetime.now(timezone.utc) >= deadline:
                break

        update(phase="coder_run", wait_reason="agent_running", important_event=important, important_event_at=utc_now_iso())
    ...
    def _cleanup(self, run_id: str, repo_root_hint: str | None = None) -> None:
        ...
        for task in state.tasks:
            try:
                ...
                pass
        try:
            _run_git(repo_root, ["worktree", "remove", "--force", str(layout.worktrees_dir / run_id / "planner")])
        except Exception:
            pass
    ...
    def _build_planner_prompt(
        ...
        *,
        ...
        parts = [
            ...
            "- Reduce hallucinations: do not invent repo facts; every claim must cite evidence (file:line).",
        ...
        parts += [
            ...
            *scope_lines,
            ...
            "Requirements must include a recommended approach (bullets), with evidence references like `path/file.ext:123`.",
            "Call out critical risks/unknowns only if necessary; keep it short.",
            contract_rule,
            f"Keep `compact_md` <= {compact_max_kb}KB. Its first section must be a <=1KB summary; include an evidence list with file:line.",
            "",
