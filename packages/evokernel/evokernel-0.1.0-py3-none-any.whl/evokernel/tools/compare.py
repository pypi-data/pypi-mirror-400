...
def execute_compare(
    ...
    if not file1 or not file2:
        yield Message("system", """Error: Two files required.
...
```""")
    ...
    lines.append(f"| Device funcs | {len(info1.device_functions)} | {len(info2.device_functions)} |")
...
""",
    ...
    parameters=[
        Parameter(name="file1", type="string", description="First CUDA file (original)", required=True),
        Parameter(name="file2", type="string", description="Second CUDA file (optimized)", required=True),
    ],
)
