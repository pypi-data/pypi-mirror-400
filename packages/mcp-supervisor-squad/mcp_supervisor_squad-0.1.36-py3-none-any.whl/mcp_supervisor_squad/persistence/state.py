...
class RunState:
    ...
    def public_status(self, *, options: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        # Summary lines (concise)
        lines: list[str] = []
        wait_hint = f"强制等待: {_round_wait_s(poll_ms)}s"
        activity_hint_line = f"活跃: {activity_hint}" if activity_hint else ""
        lines.append(f"阶段: {self.phase}" + (f" | {stage_hint}" if stage_hint else "") + (f" | {activity_hint_line}" if activity_hint_line else ""))
        lines.append(f"最近: {self.important_event} | 已等待: {_fmt_elapsed_precise(elapsed_s)} | {wait_hint}")
        ...
        wait_s = max(1, int(poll_ms // 1000)) if poll_ms else 0
        if terminal:
            payload["next_step"] = "stop polling (terminal phase)"
        else:
            parts: list[str] = []
            if summary_payload:
                parts.append("打印 summary_lines")
            if events_lines:
                parts.append("打印 event_lines")
            if parts:
                payload["next_step"] = f"{' & '.join(parts)} 后强制等待 {wait_s}s 再查询 squad_status"
        return payload
