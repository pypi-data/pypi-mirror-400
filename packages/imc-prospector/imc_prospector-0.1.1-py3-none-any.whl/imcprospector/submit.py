...
def download_logs(data: dict[str, Any], output_file: Path) -> None:
    """Download submission logs to a file."""
...
def log_profit_loss(output_file: Path) -> None:
    """Extract and display final profit/loss from logs."""
...
def submit(
    ...
    if data["status"] == "ERROR":
        print("❌ Submission errored. Check logs for details.")
        sys.exit(1)
    else:
