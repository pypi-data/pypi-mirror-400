...
def register_tools(mcp: FastMCP, *, config: SquadConfig) -> None:
    ...
    def _analyze_scope_layout(
        ...
        *,
        ...
        for idx, task in enumerate(task_defs):
            allow = _parse_globs(task.get("allow_globs"))
            deny = _parse_globs(task.get("deny_globs"))
            if allow is None:
                missing_allow = True
            for prefix in _extract_glob_prefixes(allow):
                prefix_counts[prefix] = prefix_counts.get(prefix, 0) + 1
            sample_count: int | None = None
            if precomputed_scope and idx < len(precomputed_scope):
            ...
            if sample_count is None and allow is not None:
                try:
                    sample_count = len(_rg_list_files(repo_root, allow_globs=allow, deny_globs=deny, limit=4))
    ...
    def _select_execution_mode(
        *,
        ...
        explicit = _coerce_execution_mode(opts.get("execution_mode"))
        if explicit:
            return explicit, {"explicit": True}
        analysis = _analyze_scope_layout(repo_root, tasks, precomputed_scope=precomputed_scope)
        ...
        if task_count <= 2 and small_scope and not missing_allow:
            mode = "direct"
        else:
            max_tasks_for_fanout = max(4, worker_limit_val * 2)
            if has_overlap or task_count > max_tasks_for_fanout:
                mode = "batch-worktree"
            elif worker_limit_val > 1 and not has_overlap and not missing_allow:
                mode = "fanout"
            elif task_count <= 1:
                mode = "direct"
            else:
                mode = "batch-worktree"
        return mode, {"explicit": False, "analysis": {**analysis, "worker_concurrency": worker_limit_val}}
    ...
    def _preflight_tasks(cwd: str, tasks: list[dict[str, Any]], opts: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "execution_mode": {"selected": execution_mode, "explicit": bool(mode_meta.get("explicit")), "analysis": mode_meta.get("analysis")},
    ...
    def _rg_list_files(repo_root: Path, *, allow_globs: list[str] | None, deny_globs: list[str] | None, limit: int) -> list[str]:
        ...
        for line in (res.stdout or "").splitlines():
            ...
            if path_obj.is_absolute():
                ...
                rel = _normalize_relpath(str(path_obj))
    ...
    def code_squad_run(
        ...
        try:
            ...
            if not selected_mode:
                selected_mode, _ = _select_execution_mode(
                    repo_root=repo_root,
    ...
    def code_squad_status(run_id: str, include_last_message: bool = True, options: Any | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            summary_lines: list[str] = [
                f"进度: {progress_view['done']}/{progress_view['total']} | 正在: {current_phase_text} | 动作: {next_action}",
