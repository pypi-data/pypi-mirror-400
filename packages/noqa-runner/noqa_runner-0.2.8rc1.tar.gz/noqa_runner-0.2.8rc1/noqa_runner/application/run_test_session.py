...
class RunnerSession:
    ...
    async def execute(
        ...
        Args:
            device_id: Device UDID (mutually exclusive with simulator_id)
            simulator_id: Simulator UDID (mutually exclusive with device_id)
            appium_url: Appium server URL
            app_build_path: Path to .ipa (device) or .app (simulator) build
            ...
            apple_developer_team_id: Apple Developer Team ID for device code signing
        ...
        # Determine if using simulator or device
        is_simulator = simulator_id is not None
        target_id = simulator_id if is_simulator else device_id

        appium_caps = dict(DEFAULT_APPIUM_CAPABILITIES)

        # Configure for simulator or device
        if is_simulator:
            appium_caps["appium:udid"] = simulator_id
            # Simulators don't need code signing
            logger.info("Configuring for iOS Simulator", simulator_id=simulator_id)
        else:
            ...
            logger.info("Configuring for physical device", device_id=device_id)
        ...
        for test_info in tests:
            ...
            await AppiumClient.cleanup_all_sessions()
            ...
            try:
                async with AppiumClient(
                    ...
                    # Pass app_store_id only if no build_path was provided
                    task = asyncio.create_task(
                        self._run_single_test(
                            ...
                            max_steps=max_steps,
                            app_store_id=app_store_id if not app_build_path else None,
                        )
                    )
                    register_task(task)
                    await task
    ...
    async def _run_single_test(
        ...
        if app_store_id:
            if "-appstore" in app_store_id:
                ...
                result = await mobile_service.client.install_app_appstore(
                    app_bundle_id=state.bundle_id,
                ...
                result = await mobile_service.client.install_app_testflight(
                    app_bundle_id=state.bundle_id, app_store_id=app_store_id
                )

            if not result:
                raise AppiumError(
                    f"App installation timeout for bundle_id={state.bundle_id}",
