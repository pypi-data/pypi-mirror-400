...
METHODS = (
    ...
    ("degrees", lambda network: network.degrees(), _tr.m[13, "Degree"], NODELEVEL, GENERAL),

    ("number_of_nodes", lambda n: n, _tr.m[14, "Number of nodes"], GRAPHLEVEL, GENERAL),
    ("number_of_edges", lambda e: e, _tr.m[15, "Number of edges"], GRAPHLEVEL, GENERAL),
    ("average_degree", lambda network: avg_degree(network), _tr.m[16, "Average degree"], GRAPHLEVEL, GENERAL),
    ("density", lambda network: density(network), _tr.m[17, "Density"], GRAPHLEVEL, GENERAL),

    ("shortest_paths", shortest_paths_nan_diag, _tr.m[18, "Shortest paths"]),
    ("diameter",
     lambda shortest_paths: np.nanmax(shortest_paths), _tr.m[19, "Diameter"], GRAPHLEVEL, GENERAL),
    ("radius",
     lambda shortest_paths: np.nanmin(np.nanmax(shortest_paths, axis=1)),
     _tr.m[20, "Radius"], GRAPHLEVEL, GENERAL),
    ("average_shortest_path_length",
     lambda shortest_paths: np.nanmean(shortest_paths),
     _tr.m[21, "Average shortest path length"], GRAPHLEVEL, GENERAL),

    ("number_strongly_connected_components", lambda network:
         csgraph.connected_components(network.edges[0].edges, False)[0],
         _tr.m[22, "Number of strongly connected components"], GRAPHLEVEL, DIRECTED),
    ("number_weakly_connected_components", lambda network:
         csgraph.connected_components(network.edges[0].edges, True)[0],
         _tr.m[23, "Number of weakly connected components"], GRAPHLEVEL, DIRECTED),

    ("in_degrees", lambda network: network.in_degrees(), _tr.m[24, "In-degree"], NODELEVEL, GENERAL),
    ("out_degrees", lambda network: network.out_degrees(), _tr.m[25, "Out-degree"], NODELEVEL, GENERAL),
    ("average_neighbour_degrees",
     ...
     _tr.m[26, "Average neighbor degree"], NODELEVEL, GENERAL),
    ("degree_centrality",
     lambda degrees, n: n and degrees / (n - 1) if n > 1 else 0,
     _tr.m[27, "Degree centrality"], NODELEVEL, GENERAL),
    ("in_degree_centrality",
     lambda in_degrees, n: in_degrees / (n - 1) if n > 1 else 0,
     _tr.m[28, "In-degree centrality"], NODELEVEL, GENERAL),
    ("out_degree_centrality",
     lambda out_degrees, n: out_degrees / (n - 1) if n > 1 else 0,
     _tr.m[29, "Out-degree centrality"], NODELEVEL, GENERAL),
    ("closeness_centrality",
     lambda shortest_paths: 1 / np.nanmean(shortest_paths, axis=1),
     _tr.m[30, "Closeness centrality"], NODELEVEL, GENERAL)
)
...
class OWNxAnalysis(widget.OWWidget):
    name = _tr.m[31, 'Network Analysis']
    description = _tr.m[32, 'Statistical analysis of network data.']
    ...
    class Inputs:
        network = Input(_tr.m[33, "Network"], Network)
        items = Input(_tr.m[34, "Items"], Table)

    class Outputs:
        network = Output(_tr.m[35, "Network"], Network)
        items = Output(_tr.m[36, "Items"], Table)

    class Information(widget.OWWidget.Information):
        computing = Msg(_tr.m[37, "Computing {}"])
    ...
    def __init__(self):
        ...
        graph_indices = gui.createTabPage(tabs, _tr.m[38, "Graph-level indices"],
                                          orientation=Qt.Horizontal)
        node_indices = gui.createTabPage(tabs, _tr.m[39, "Node-level indices"],
                                         orientation=Qt.Horizontal)
