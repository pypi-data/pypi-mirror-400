...
def register_tools(mcp: FastMCP, *, config: SquadConfig) -> None:
            ...
            if not worktree:
                entry = {
                    ...
                    "scope": {
                        ...
                        "violations": {"not_allowed": [], "denied": [], "limits": {}},
                    },
                }
    ...
    def code_squad_execute(
        ...
        try:
            ...
            timeout_seconds = opts.get("timeout_seconds", 600)
            poll_interval = opts.get("poll_interval", 1.0)
            ...
            verify_results, verify_debug = _verify_run(resolved_run_id, final_meta, job_id=None, options=opts, debug=debug)
            counts: dict[str, int] = {}
            for task in final_meta.get("tasks", []):
                state = str(task.get("state") or "unknown")
                counts[state] = counts.get(state, 0) + 1
            ...
            cleanup_enabled = bool(opts.get("cleanup", True))
            keep_failed = bool(opts.get("keep_failed", True))
            cleanup_delete_run_artifacts = bool(opts.get("cleanup_delete_run_artifacts", False))
            ...
            if cleanup_enabled:
                ...
                if cleanup_delete_run_artifacts:
                    _ = code_squad_cleanup(resolved_run_id, delete_run_artifacts=True, options=opts)
