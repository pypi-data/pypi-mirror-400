...
from exponent.core.graphql.generated_client.chats import ChatsChatsChatsChats
...
async def fetch_chats(
    ...
    if result.chats.typename__ == "UnauthenticatedError":
        click.secho(f"Error: {result.chats.message}", fg="red")
        sys.exit(1)

    if result.chats.typename__ == "Chats":
...
def get_open_pr_info(chat: ChatsChatsChatsChats) -> tuple[int, str] | None:
    for pr in chat.pr_status:
        if pr.typename__ == "PRInfo" and pr.status == PRStatus.OPEN:
            return (pr.number, pr.title)
    return None


def format_chat_choice(chat: ChatsChatsChatsChats) -> dict[str, Any]:
    pr_info = get_open_pr_info(chat)
    if pr_info:
        number, title = pr_info
        display = f"#{number} {title}"
    else:
        display = chat.name or "Untitled"
    return {"name": display, "value": chat}


def filter_chats_with_open_prs(
    chats: list[ChatsChatsChatsChats],
) -> list[ChatsChatsChatsChats]:
    return [chat for chat in chats if get_open_pr_info(chat) is not None]
...
MAX_CHATS_TO_SHOW = 20
DEFAULT_POLL_INTERVAL = 3


def select_chat_interactive(chats: list[ChatsChatsChatsChats]) -> ChatsChatsChatsChats:
    if not chats:
        click.secho("No chats found.", fg="yellow")
        sys.exit(1)

    if len(chats) == 1:
        selected = chats[0]
        click.secho(f"Using chat: {selected.name or 'Untitled'}", fg="cyan")
        return selected

    chats_to_show = chats[:MAX_CHATS_TO_SHOW]
    choices = [format_chat_choice(chat) for chat in chats_to_show]
    ...
    return cast(ChatsChatsChatsChats, selected)


def find_chat_by_id(
    chats: list[ChatsChatsChatsChats], chat_id: str
) -> ChatsChatsChatsChats | None:
    for chat in chats:
        if chat_id in {chat.chat_uuid, chat.id}:
            return chat
    return None
...
def checkout(
    ...
    """Checkout a cloud chat locally.

    If CHAT_ID is provided, checks out that specific chat.
    Otherwise, presents an interactive selector.
    """
    ...
    chats_with_prs = filter_chats_with_open_prs(sorted_chats)

    if not chats_with_prs:
        click.secho("No chats with open PRs found.", fg="yellow")
        sys.exit(1)

    if chat_id:
        selected_chat = find_chat_by_id(chats_with_prs, chat_id)
        if not selected_chat:
            click.secho(f"Chat not found: {chat_id}", fg="red")
            sys.exit(1)
    else:
        selected_chat = select_chat_interactive(chats_with_prs)

    pr_info = get_open_pr_info(selected_chat)
    if pr_info:
        check_gh_installed()
        pr_number, pr_title = pr_info
        chat_url = f"{settings.base_url}/chats/{selected_chat.chat_uuid}"
