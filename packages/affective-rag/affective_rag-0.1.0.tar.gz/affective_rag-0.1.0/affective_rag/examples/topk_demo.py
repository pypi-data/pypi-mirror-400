"""
Demo: show effect of `activation_top_k` — with top_k=1 only the highest activation hit is considered.

Pattern: External storage + callbacks.
"""
...
def main():
    ...
    # Run twice to compare top_k behavior
    for k in (1, 2):
        result = graph.retrieve(
            query="What happened at the party?",
            vector_lookup=fake_vector_lookup,
            cpt_config=CPTConfig(max_depth=3, spreading_activation=True, activation_top_k=k, activation_min_score=0.0),
            event_data_provider=event_data_provider,
        )
        print(f"topk_demo (activation_top_k={k}):")
