...
class SinaFuturesRealtime(RealtimeFuturesDataProvider):
    ...
    @cache(
        "futures_realtime_cache",
        key=lambda self: f"sina_futures_{self.symbol if self.symbol else 'all'}_{self.contract if self.contract else 'all'}",
    )
    def get_current_data(self) -> pd.DataFrame:
        ...
        if self.symbol:
            ...
            if self.contract and self.contract.lower() != "main":
                ...
                # Filter by variety code (symbol_root) to get all contracts of that variety
