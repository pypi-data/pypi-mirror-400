                ...
                while page <= max_pages:
                    ...
                    if save_raw_response and raw_response_dir:
                        try:
                            ...
                            # 파일명: {symbol}_{date_from}_{date_to}_{page}.json
                            # timestamp를 파일명에 넣지 않으면 재수행 시 덮어써지는데, 이는 의도된 동작일 수 있음.
                            # 중복 방지를 위해선 이게 나을 수도 있음.
                            filename = f"{symbol}_{date_from}_{date_to}_p{page}.json"
                            file_path = save_dir / filename
                            
                            with open(file_path, "w", encoding="utf-8") as f:
                                json.dump(response, f, ensure_ascii=False, indent=2)
                        except Exception as e:
                            logger.error(f"Failed to save raw response for {symbol}: {e}")
                    ...
                    for item in data:
                        ...
                        processed_item['symbol_req'] = symbol  # 요청한 심볼 (검색 기준)
                        ...
                        # item.copy()를 했으므로 이미 entities와 named_entities 키가 존재함 (API 응답에 있다면)
                        # 따라서 덮어쓰는 로직 제거
                        # 호환성을 위해 named_entities가 없으면 빈 리스트라도 보장하고 싶다면 아래와 같이 처리 가능하지만,
                        # 여기서는 있는 그대로 두거나 필요한 경우에만 빈 리스트 할당
    ...
    **kwargs
    ...
    for symbol in symbols:
        ...
        if not force_refresh:
            last_date = cache.get_last_news_date(symbol)
            if last_date:
