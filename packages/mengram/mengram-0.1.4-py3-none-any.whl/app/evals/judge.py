...
class JudgeRunner:
    ...
    def _metadata(self, prompt: str, output: str) -> dict:
        ...
        meta = {"template_name": self.config.template_name, "prompt_hash": prompt_hash}
        if self.config.record_previews:
            meta["prompt_preview"] = prompt[: self.config.prompt_preview_chars]
            meta["output_preview"] = str(output)[: self.config.output_preview_chars]
        return meta
