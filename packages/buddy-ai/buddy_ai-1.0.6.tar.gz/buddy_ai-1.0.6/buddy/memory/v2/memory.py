...
from buddy.media import AudioArtifact, ImageArtifact, VideoArtifact
from buddy.memory.v2.db.base import MemoryDb
from buddy.memory.v2.db.schema import MemoryRow
from buddy.memory.v2.manager import MemoryManager
from buddy.memory.v2.schema import SessionSummary, UserMemory
from buddy.memory.v2.summarizer import SessionSummarizer
from buddy.models.base import Model
from buddy.models.message import Message
from buddy.run.base import RunStatus
from buddy.run.response import RunResponse
from buddy.run.team import TeamRunResponse
from buddy.utils.log import log_debug, log_warning, logger, set_log_level_to_debug, set_log_level_to_info
from buddy.utils.prompts import get_json_output_prompt
from buddy.utils.string import parse_response_model_str
...
class Memory:
    ...
    def get_model(self) -> Model:
        if self.model is None:
            try:
                ...
                logger.exception(e)
                logger.error(
                    "Buddy uses `openai` as the default model provider. Please provide a `model` or install `openai`."
                )
                exit(1)
    ...
    def set_log_level(self):
        if self.debug_mode or getenv("BUDDY_DEBUG", "false").lower() == "true":
