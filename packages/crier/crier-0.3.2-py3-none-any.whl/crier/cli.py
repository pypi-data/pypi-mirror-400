...
@click.option("--verbose", "-v", is_flag=True, help="Show all columns including ID")
@click.option(
    "--format",
    "-f",
    "output_format",
...
def list_articles(platform: str, limit: int, verbose: bool, output_format: str):
    ...
    try:
        ...
        # URLs only output (one per line, clickable)
        ...
        if verbose:
            ...
            table.add_column("Published", style="yellow", max_width=10)
            table.add_column("URL", style="blue")

            for article in articles:
                article_id = str(article.get("id", ""))
                # Truncate ID for display
                if len(article_id) > 22:
            ...
            # Default: just Title and URL (what humans care about)
