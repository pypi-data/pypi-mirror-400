...
class ClustererTest(apitestcase.ApiTestCase):
  ...
  def test_cobweb_args(self):
    clusterer = le.Clusterer.wekaCobweb(2, 0.01, 3)
    self.assertEqual({'value': 'fakeValue'}, clusterer.getInfo())

    join_func = le.ApiFunction.lookup('Clusterer.wekaCobweb')
  ...
  def test_cobweb_kwargs(self):
    clusterer = le.Clusterer.wekaCobweb(acuity=2, cutoff=0.01, seed=3)
    self.assertEqual({'value': 'fakeValue'}, clusterer.getInfo())
  ...
  def test_cast(self):
    clusterer = le.Clusterer(
        le.Clusterer.wekaCobweb(acuity=2, cutoff=0.01, seed=3)
    )
    result = json.loads(clusterer.serialize())
    self.assertEqual(_WEKA_COBWEB_SERIALIZED, result)
  ...
  def test_no_args(self):
    ...
    with self.assertRaisesRegex(TypeError, message):
      le.Clusterer()  # pytype:disable=missing-parameter
  ...
  def test_wrong_type(self):
    ...
    with self.assertRaisesRegex(TypeError, message):
      le.Clusterer(1234)  # pytype:disable=wrong-arg-types

  def test_schema(self):
    clusterer = le.Clusterer.wekaCobweb(1.1, 2.2, 3)
    ...
    expression = le.Clusterer(clusterer).schema()
  ...
  def test_train(self):
    ...
    expression = le.Clusterer(clusterer).train(
  ...
  def test_weka_cascade_k_means(self):
    ...
    expression = le.Clusterer.wekaCascadeKMeans(
        min_clusters,
        max_clusters,
        restarts,
        manual,
  ...
  def test_weka_cobweb(self):
    ...
    expression = le.Clusterer.wekaCobweb(acuity, cutoff, seed)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Clusterer.wekaCobweb(
  ...
  def test_weka_k_means(self):
    ...
    expression = le.Clusterer.wekaKMeans(
        n_clusters,
        init,
        canopies,
        max_candidates,
  ...
  def test_weka_lvq(self):
    ...
    expression = le.Clusterer.wekaLVQ(
  ...
  def test_weka_x_means(self):
    ...
    expression = le.Clusterer.wekaXMeans(
        min_clusters,
        max_clusters,
        max_iterations,
        max_k_means,
