...
def _parse_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> Any | UpdateJobResponse400 | UpdateJobResponse404 | UpdateJobResponse409 | UpdateJobResponse500 | UpdateJobResponse503 | None:
    ...
    if response.status_code == 400:
        response_400 = UpdateJobResponse400.from_dict(response.json())
        return response_400
    if response.status_code == 404:
        response_404 = UpdateJobResponse404.from_dict(response.json())
        return response_404
    if response.status_code == 409:
        response_409 = UpdateJobResponse409.from_dict(response.json())
        return response_409
    if response.status_code == 500:
        response_500 = UpdateJobResponse500.from_dict(response.json())
        return response_500
    if response.status_code == 503:
        response_503 = UpdateJobResponse503.from_dict(response.json())
        return response_503
...
def sync(job_key: str, *, client: AuthenticatedClient | Client, body: UpdateJobData, **kwargs) -> Any:
    """Update job
...
async def asyncio(job_key: str, *, client: AuthenticatedClient | Client, body: UpdateJobData, **kwargs) -> Any:
    """Update job
