...
JOBSET_ID = os.getenv("JOBSET_ID", "17671630592002")
SELF_JOB_ID = os.getenv("JOB_ID", "1")
LEADER_JOB_ID = LEADER_PREFIX + SELF_JOB_ID
    ...
    async with JobHubClient.connect(JOBHUB_HOST, JOBHUB_PORT, JOBSET_ID, LEADER_JOB_ID, SHARED_SECRET) as cli:
        while True:
            if not await is_leader_alive(cli) and await try_become_leader(cli=cli, job_id=LEADER_JOB_ID):
                ...
                await run_as_leader(job_id=LEADER_JOB_ID, jobhub_cli=cli, scheduler=None, model_template=cnn_model,
                                    initial_config_path="./initial_ownership.json", checkpoint_path="./checkpoint")
