...
class ProfilePlotLegendHandler(HandlerPatch):
    def __init__(self, *args, facecolor=None, linecolor=None, facealpha=None, linewidth=None, **kwargs):
    ...
    def create_artists(self, legend, orig_handle,
        ...
        # Calculate the rectangle coordinates (only lower half)
        lower_half_height = 0.5 * (height + ydescent)
        rect = mpatches.Rectangle(
            (0.0, 0.0),  # lower-left corner
            width + xdescent,
            lower_half_height,
            facecolor=self._facecolor,
            alpha=self._facealpha,
            edgecolor='none'  # no edge
        )
        #self.update_prop(rect, orig_handle, legend)
        rect.set_transform(trans)

        # Add a line in front of the rectangle
        line = Line2D(
            [0, width],
            [lower_half_height] * 2,
...
def plot_longitudinal(inp, start_node, end_node, out=None, ax=None, zero_node=None, depth_agg_func=None,
                      add_node_labels=False, add_legend=False):
    """
    Make a longitudinal profile plot.
    ...
    Args:
        ...
        add_legend (bool): adding a legend.
    ...
    if add_legend:
        ...
        class Groundlevel: ...
        class Pipe: ...
        ax.legend([Groundlevel(), Pipe(), Waterline()], ["ground level", "sewer pipe", "water level"],
                  handler_map={Waterline: ProfilePlotLegendHandler(linecolor='blue', linewidth=0.7, facecolor='#00D7FF', facealpha=0.7),
                               Groundlevel: ProfilePlotLegendHandler(linecolor='brown', linewidth=0.5, facecolor='#C49B98', facealpha=0.5),
                               Pipe: ProfilePlotLegendHandler(linecolor='black', linewidth=1, facecolor='#B0B0B0', facealpha=0.5)},
                  handleheight=1.5, handlelength=1.5)
