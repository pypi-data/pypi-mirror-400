"""
MAGA包管理器核心模块

实现受特朗普政策影响的包管理功能：
- 美利坚CDN流量包
- 特朗普流量税
- 贸易战模式
- 美国优先政策
"""

import os
import sys
import time
import random
import subprocess
import json
from pathlib import Path
from typing import Dict, List, Optional, Tuple
import colorama
from colorama import Fore, Style

# 尝试导入export_license
try:
    sys.path.insert(0, "/data/data/com.termux/files/home/none-pip-package/export_license")
    from license_manager import LicenseManager
    from license_validator import LicenseValidator
    EXPORT_LICENSE_AVAILABLE = True
except ImportError:
    EXPORT_LICENSE_AVAILABLE = False
    LicenseManager = None
    LicenseValidator = None

from .real_pip_manager import real_pip

from .cdn_traffic import AmericanCDNTrafficPackage, download_american_cdn_package
from .traffic_tax import TrumpTrafficTax, SlowDownloadSimulator
from .policy_simulator import TrumpPolicySimulator

colorama.init()

class MAGAPackageManager:
    """MAGA包管理器 - 让包管理再次伟大！"""
    
    def __init__(self, skip_cdn_check=False):
        self.config_dir = Path.home() / ".maga"
        self.config_file = self.config_dir / "config.json"
        self.packages_file = self.config_dir / "packages.json"
        
        # 初始化组件
        self.cdn_package = AmericanCDNTrafficPackage()
        self.traffic_tax = TrumpTrafficTax()
        self.download_simulator = SlowDownloadSimulator(self.traffic_tax)
        self.policy_simulator = TrumpPolicySimulator()
        
        # 初始化出口许可证管理器
        if EXPORT_LICENSE_AVAILABLE:
            try:
                self.license_manager = LicenseManager()
                self.license_validator = LicenseValidator()
                self.export_license_enabled = True
            except Exception:
                self.export_license_enabled = False
        else:
            self.export_license_enabled = False
        
        # 加载配置
        self.config = self._load_config()
        
        # 检查CDN流量包（init命令可以跳过）
        if not skip_cdn_check:
            self._check_cdn_package()
    
    def _load_config(self) -> Dict:
        """加载配置"""
        default_config = {
            "version": "1.0.0",
            "maga_mode": "full",  # full, lite, patriotic
            "trump_tax_enabled": True,
            "trade_war_enabled": True,
            "america_first_enabled": True,
            "build_wall_enabled": True,
            "cdn_package_required": True,
            "download_speed": "🐌",  # 🐌, 🐢, 🚗
            "maga_score": "A+",
            "install_count": 0,
            "blocked_count": 0,
        }
        
        if self.config_file.exists():
            try:
                with open(self.config_file, 'r') as f:
                    user_config = json.load(f)
                    default_config.update(user_config)
            except:
                pass
        
        return default_config
    
    def _save_config(self):
        """保存配置"""
        self.config_dir.mkdir(parents=True, exist_ok=True)
        with open(self.config_file, 'w') as f:
            json.dump(self.config, f, indent=2)
    
    def _check_cdn_package(self):
        """检查CDN流量包"""
        if self.config["cdn_package_required"] and not self.cdn_package.check_package_exists():
            print(f"{Fore.RED}❌ 错误：未找到美利坚CDN流量包！{Style.RESET_ALL}")
            print(f"{Fore.YELLOW}请先运行 'maga init' 下载流量包{Style.RESET_ALL}")
            sys.exit(1)
    
    def _load_packages(self) -> Dict:
        """加载已安装的包"""
        if self.packages_file.exists():
            try:
                with open(self.packages_file, 'r') as f:
                    return json.load(f)
            except:
                return {}
        return {}
    
    def _save_packages(self, packages: Dict):
        """保存包列表"""
        self.config_dir.mkdir(parents=True, exist_ok=True)
        with open(self.packages_file, 'w') as f:
            json.dump(packages, f, indent=2)
    
    def init(self, force: bool = False):
        """初始化MAGA-PM（下载CDN流量包）"""
        print(f"{Fore.CYAN}🇺🇸 初始化MAGA包管理器...{Style.RESET_ALL}")
        
        # 显示特朗普政策通知
        self.traffic_tax.show_tax_notice()
        
        # 下载CDN流量包
        success = download_american_cdn_package(force)
        
        if success:
            print(f"\n{Fore.GREEN}✅ MAGA-PM 初始化完成！{Style.RESET_ALL}")
            print(f"{Fore.YELLOW}现在你可以使用 'maga install' 命令了{Style.RESET_ALL}")
            print(f"{Fore.RED}⚠️  注意：所有下载都会受到特朗普流量税影响 🐌{Style.RESET_ALL}")
        else:
            print(f"{Fore.RED}❌ 初始化失败！{Style.RESET_ALL}")
        
        return success
    
    def install(self, package_name: str, country: Optional[str] = None):
        """
        安装包（受特朗普政策影响）
        
        Args:
            package_name: 包名
            country: 包的国家（默认随机）
        """
        print(f"{Fore.CYAN}📦 准备安装包：{package_name}{Style.RESET_ALL}")
        
        # 随机选择国家（如果未指定）
        if not country:
            countries = ["china", "eu", "russia", "usa", "other"]
            weights = [0.3, 0.2, 0.2, 0.2, 0.1]
            country = random.choices(countries, weights=weights, k=1)[0]
        
        # 模拟政策影响
        print(f"{Fore.YELLOW}🔍 应用特朗普政策...{Style.RESET_ALL}")
        time.sleep(1)
        
        # 随机包大小
        package_size_mb = random.uniform(1.0, 50.0)
        
        # 模拟下载
        allowed, download_time, messages = self.policy_simulator.simulate_download_with_policies(
            package_name, package_size_mb, country
        )
        
        # 显示结果
        if not allowed:
            print(f"\n{Fore.RED}🚫 安装被拒绝！{Style.RESET_ALL}")
            print(f"{Fore.YELLOW}原因：{Style.RESET_ALL}")
            for msg in messages:
                print(f"  • {msg}")
            
            self.config["blocked_count"] += 1
            self._save_config()
            return False
        
        # 显示下载详情
        print(f"\n{Fore.GREEN}✅ 包允许安装{Style.RESET_ALL}")
        print(f"{Fore.CYAN}国家：{country}{Style.RESET_ALL}")
        print(f"{Fore.CYAN}大小：{package_size_mb:.1f} MB{Style.RESET_ALL}")
        
        for msg in messages:
            print(f"  • {msg}")
        
        # 检查出口许可证（如果是AI包）
        if self.export_license_enabled and self._is_ai_package(package_name):
            print(f"\n{Fore.YELLOW}🔍 检查出口许可证（AI包限制）...{Style.RESET_ALL}")
            license_valid = self._check_export_license(package_name, country)
            
            if not license_valid:
                print(f"\n{Fore.RED}🚫 安装被拒绝！{Style.RESET_ALL}")
                print(f"{Fore.YELLOW}原因：缺少有效的出口许可证{Style.RESET_ALL}")
                print(f"{Fore.RED}此AI包需要特朗普政府批准的出口许可证{Style.RESET_ALL}")
                return False
            else:
                print(f"{Fore.GREEN}✅ 出口许可证有效{Style.RESET_ALL}")
        
        # 检查pip是否可用
        pip_available, pip_message = real_pip.check_availability()
        if not pip_available:
            print(f"\n{Fore.RED}{pip_message}{Style.RESET_ALL}")
            return False
        
        print(f"\n{Fore.YELLOW}⏳ 开始下载（受特朗普流量税影响）...{Style.RESET_ALL}")
        
        # 显示特朗普流量税通知
        self.traffic_tax.show_tax_notice()
        time.sleep(1)
        
        # 使用真实的pip安装（带进度条）
        success, message = real_pip.install_package(package_name, show_progress=True)
        
        if success:
            # 更新包列表
            packages = self._load_packages()
            packages[package_name] = {
                "version": "1.0.0",
                "country": country,
                "size_mb": package_size_mb,
                "install_time": time.strftime("%Y-%m-%d %H:%M:%S"),
                "download_time": download_time,
                "trump_tax_applied": True,
                "via": "pip"
            }
            self._save_packages(packages)
            
            # 更新配置
            self.config["install_count"] += 1
            self._save_config()
            
            print(f"\n{Fore.GREEN}{message}{Style.RESET_ALL}")
            print(f"{Fore.YELLOW}⚠️  注意：下载受到特朗普流量税影响，速度较慢 🐌{Style.RESET_ALL}")
            return True
        else:
            print(f"\n{Fore.RED}{message}{Style.RESET_ALL}")
            return False
    
    def _is_ai_package(self, package_name: str) -> bool:
        """检查是否是AI包"""
        ai_keywords = [
            "ai", "ml", "machine", "learning", "deep", "neural", "tensor", 
            "pytorch", "torch", "transformers", "huggingface", "openai",
            "llm", "gpt", "bert", "transformer", "vision", "nlp"
        ]
        
        package_lower = package_name.lower()
        for keyword in ai_keywords:
            if keyword in package_lower:
                return True
        
        return False
    
    def _check_export_license(self, package_name: str, country: str) -> bool:
        """检查出口许可证"""
        if not self.export_license_enabled:
            print(f"{Fore.YELLOW}⚠️  出口许可证系统不可用，跳过检查{Style.RESET_ALL}")
            print(f"{Fore.YELLOW}⚠️  注意：AI包需要特朗普政府批准的出口许可证{Style.RESET_ALL}")
            return True  # 如果许可证系统不可用，默认允许
        
        try:
            # 检查许可证
            if hasattr(self, 'license_validator') and self.license_validator:
                license_valid = self.license_validator.validate_package_license(
                    package_name=package_name,
                    country=country,
                    license_type="AI_EXPORT"
                )
                
                if license_valid:
                    print(f"{Fore.GREEN}✅ 出口许可证有效{Style.RESET_ALL}")
                else:
                    print(f"{Fore.RED}❌ 出口许可证无效或已过期{Style.RESET_ALL}")
                
                return license_valid
            else:
                print(f"{Fore.YELLOW}⚠️  许可证验证器不可用，跳过检查{Style.RESET_ALL}")
                return True
        except Exception as e:
            # 如果检查失败，显示友好提示并允许安装
            print(f"{Fore.YELLOW}⚠️  许可证检查失败: {str(e)}{Style.RESET_ALL}")
            print(f"{Fore.YELLOW}⚠️  继续安装（模拟模式）{Style.RESET_ALL}")
            return True    
    def list_packages(self):
        """列出已安装的包"""
        # 首先显示MAGA记录的包
        maga_packages = self._load_packages()
        
        # 然后显示真实的pip安装的包
        real_packages = real_pip.list_installed_packages()
        
        print(f"{Fore.CYAN}📦 已安装的包：{Style.RESET_ALL}")
        print(f"{Fore.YELLOW}{'='*80}{Style.RESET_ALL}")
        
        # 显示MAGA记录的包
        if maga_packages:
            print(f"{Fore.GREEN}📋 MAGA记录的包（共 {len(maga_packages)} 个）：{Style.RESET_ALL}")
            for i, (name, info) in enumerate(maga_packages.items(), 1):
                country_flag = {
                    "usa": "🇺🇸",
                    "china": "🇨🇳", 
                    "eu": "🇪🇺",
                    "russia": "🇷🇺",
                    "other": "🌍",
                }.get(info.get("country", "other"), "🌍")
            
                print(f"{Fore.GREEN}{i:3}. {name:30}{Style.RESET_ALL}", end="")
                print(f"{Fore.CYAN} v{info.get('version', '1.0.0'):10}{Style.RESET_ALL}", end="")
                print(f"{Fore.YELLOW} {country_flag} {info.get('country', 'unknown'):10}{Style.RESET_ALL}", end="")
                print(f"{Fore.MAGENTA} {info.get('size_mb', 0):6.1f} MB{Style.RESET_ALL}", end="")
                print(f"{Fore.RED} 🐌 {info.get('download_time', 0):6.1f}s{Style.RESET_ALL}")
        
        # 显示真实的pip安装的包
        if real_packages:
            print(f"\n{Fore.BLUE}🐍 真实pip安装的包（共 {len(real_packages)} 个）：{Style.RESET_ALL}")
            for i, pkg in enumerate(real_packages, 1):
                print(f"{Fore.BLUE}{i:3}. {pkg['name']:30}{Style.RESET_ALL}", end="")
                print(f"{Fore.CYAN} v{pkg.get('version', '?'):10}{Style.RESET_ALL}", end="")
                print(f"{Fore.GREEN} via pip{Style.RESET_ALL}")
        
        print(f"{Fore.YELLOW}{'='*80}{Style.RESET_ALL}")
        
        # 显示统计
        total_maga_packages = len(maga_packages)
        total_real_packages = len(real_packages)
        
        print(f"{Fore.CYAN}统计：{Style.RESET_ALL}")
        print(f"{Fore.GREEN}MAGA记录的包：{total_maga_packages} 个{Style.RESET_ALL}")
        print(f"{Fore.BLUE}真实pip安装的包：{total_real_packages} 个{Style.RESET_ALL}")
        print(f"{Fore.YELLOW}总计：{total_maga_packages + total_real_packages} 个包{Style.RESET_ALL}")
        
        if maga_packages:
            total_size = sum(info.get("size_mb", 0) for info in maga_packages.values())
            total_time = sum(info.get("download_time", 0) for info in maga_packages.values())
            print(f"{Fore.RED}总下载时间：{total_time:.1f} 秒（受特朗普流量税影响）{Style.RESET_ALL}")
            print(f"{Fore.YELLOW}平均速度：{total_size*1024/max(1, total_time):.1f} KB/s 🐌{Style.RESET_ALL}")
    
    def uninstall(self, package_name: str):
        """卸载包"""
        # 首先检查是否在MAGA记录中
        packages = self._load_packages()
        in_maga = package_name in packages
        
        # 使用真实的pip卸载
        print(f"{Fore.YELLOW}🗑️  卸载包：{package_name}{Style.RESET_ALL}")
        
        success, message = real_pip.remove_package(package_name)
        
        if success:
            # 如果也在MAGA记录中，则移除
            if in_maga:
                del packages[package_name]
                self._save_packages(packages)
                print(f"{Fore.GREEN}✅ 包 '{package_name}' 已从MAGA记录中移除{Style.RESET_ALL}")
            
            print(f"{Fore.GREEN}{message}{Style.RESET_ALL}")
            return True
        else:
            # 如果pip卸载失败，但包在MAGA记录中，询问用户
            if in_maga:
                print(f"{Fore.YELLOW}⚠️  pip卸载失败，但包在MAGA记录中{Style.RESET_ALL}")
                print(f"{Fore.CYAN}是否从MAGA记录中移除？(y/n): {Style.RESET_ALL}", end="")
                # 这里简化处理，直接移除
                del packages[package_name]
                self._save_packages(packages)
                print(f"{Fore.GREEN}✅ 已从MAGA记录中移除{Style.RESET_ALL}")
                return True
            
            print(f"{Fore.RED}{message}{Style.RESET_ALL}")
            return False
    
    def policy(self):
        """显示特朗普政策状态"""
        self.policy_simulator.show_policy_dashboard()
    
    def tax(self):
        """显示流量税信息"""
        from .traffic_tax import show_traffic_tax_info
        show_traffic_tax_info()
    
    def cdn(self):
        """显示CDN流量包信息"""
        from .cdn_traffic import show_traffic_package_info
        show_traffic_package_info()
    
    def stats(self):
        """显示统计信息"""
        packages = self._load_packages()
        
        print(f"{Fore.CYAN}📊 MAGA-PM 统计信息{Style.RESET_ALL}")
        print(f"{Fore.YELLOW}{'='*50}{Style.RESET_ALL}")
        
        # 基本统计
        print(f"{Fore.GREEN}已安装包数量：{len(packages)}{Style.RESET_ALL}")
        print(f"{Fore.GREEN}总安装次数：{self.config.get('install_count', 0)}{Style.RESET_ALL}")
        print(f"{Fore.RED}被拒绝安装次数：{self.config.get('blocked_count', 0)}{Style.RESET_ALL}")
        
        # 政策统计
        policy_stats = self.policy_simulator.get_statistics()
        print(f"\n{Fore.CYAN}政策统计：{Style.RESET_ALL}")
        for key, value in policy_stats.items():
            key_display = key.replace("_", " ").title()
            print(f"{Fore.GREEN}{key_display:25}{Fore.WHITE}: {value}{Style.RESET_ALL}")
        
        # 速度统计
        if packages:
            total_size = sum(info.get("size_mb", 0) for info in packages.values())
            total_time = sum(info.get("download_time", 0) for info in packages.values())
            avg_speed = total_size * 1024 / max(1, total_time)
            
            print(f"\n{Fore.CYAN}速度统计：{Style.RESET_ALL}")
            print(f"{Fore.GREEN}平均下载速度：{avg_speed:.1f} KB/s 🐌{Style.RESET_ALL}")
            
            # 速度评级
            if avg_speed < 10:
                speed_rating = "🐌 蜗牛速度（特朗普税收生效中）"
            elif avg_speed < 50:
                speed_rating = "🐢 乌龟速度（高关税）"
            elif avg_speed < 100:
                speed_rating = "🚶 步行速度（中关税）"
            else:
                speed_rating = "🚗 汽车速度（低关税）"
            
            print(f"{Fore.YELLOW}速度评级：{speed_rating}{Style.RESET_ALL}")
        
        print(f"{Fore.YELLOW}{'='*50}{Style.RESET_ALL}")
    
    def make_maga_great_again(self):
        """执行MAGA优化（实际上什么都不做，只是显示信息）"""
        print(f"{Fore.CYAN}🇺🇸 执行MAGA优化...{Style.RESET_ALL}")
        print(f"{Fore.YELLOW}特朗普总统正在优化你的包管理器...{Style.RESET_ALL}")
        
        optimizations = [
            "增加特朗普流量税...",
            "加强贸易战屏蔽...",
            "加高网络墙...",
            "实施美国优先政策...",
            "抽干依赖沼泽...",
            "让包管理再次伟大！",
        ]
        
        for opt in optimizations:
            print(f"{Fore.GREEN}✅ {opt}{Style.RESET_ALL}")
            time.sleep(0.5)
        
        print(f"\n{Fore.GREEN}🎉 MAGA优化完成！{Style.RESET_ALL}")
        print(f"{Fore.YELLOW}你的包管理器现在更慢了，但更爱国了！ 🇺🇸{Style.RESET_ALL}")
        print(f"{Fore.RED}⚠️  注意：下载速度可能进一步降低 🐌🐌{Style.RESET_ALL}")


def create_package_manager(skip_cdn_check=False) -> MAGAPackageManager:
    """创建包管理器实例"""
    return MAGAPackageManager(skip_cdn_check=skip_cdn_check)