...
class OneDriveClient(O365Client):
    ...
    def _start_(self, **kwargs):
        """Initialize OneDrive-specific configuration."""
        self._logger.info("OneDrive client initialized successfully")
        return True
    ...
    def connection(self):
        ...
        self._logger.info("OneDrive connection established successfully")
        return self
    ...
    async def verify_onedrive_access(self):
        ...
        try:
            ...
            self._logger.info(f"OneDrive accessible: {self._drive_info.name or 'Personal OneDrive'}")

        except Exception as e:
            self._logger.error(f"OneDrive access verification failed: {e}")
    ...
    async def _resolve_drive(self) -> DriveItem:
        ...
        try:
            ...
            if drive and drive.id:
                ...
                self._logger.info(f"OneDrive resolved: {drive.name or drive.id}")
                return drive
            else:
        ...
        try:
            ...
            if folder_path:
                ...
                try:
                    ...
                    raise FileError(f"Folder '{folder_path}' not found: {e}") from e
            else:
                # Get root folder
    ...
    async def file_download(self, item_id: str, destination: Path) -> str:
        ...
        try:
            ...
            if not item.file:
                raise FileError(f"Item {item_id} is not a file")

            self._logger.info(f"Downloading {item.name} to {destination}")
            ...
            self._logger.info(f"Downloaded {item.name} successfully")
        ...
        for item in items:
            try:
                ...
                if not item_id or not file_name:
                    self._logger.warning(f"Skipping invalid item: {item}")
                    continue
                ...
                self._logger.error(f"Failed to download {item.get('name', 'unknown')}: {e}")
                continue
    ...
    async def folder_download(self, folder_id: str, destination_folder: Path) -> bool:
        ...
        try:
            ...
            if not folder_item.folder:
                raise FileError(f"Item {folder_id} is not a folder")
    ...
    async def file_delete(self, item_id: str) -> bool:
        ...
        try:
            ...
            self._logger.info(f"Deleted {item_type}: {item_name}")
            return True
    ...
    async def upload_file(self, file_path: Path, destination_folder: str = None) -> dict:
        ...
        try:
            ...
            self._logger.info(f"Uploading {target_name} ({file_size:,} bytes)")
    ...
    async def upload_folder(self, local_folder: Path, destination_folder: str = None) -> List[dict]:
        ...
        try:
            ...
            if not local_path.exists() or not local_path.is_dir():
                raise FileNotFound(f"Local folder does not exist or is not a directory: {local_folder}")
    ...
    async def upload_dataframe_as_excel(self, df: pd.DataFrame, file_name: str, destination_folder: str = None) -> dict:
        ...
        try:
            ...
            self._logger.info(f"Uploading DataFrame as Excel: {file_name}")
            ...
            self._logger.info(f"Uploaded DataFrame as Excel successfully: {uploaded_item.name}")
    ...
    async def _upload_large_file(self, upload_session: UploadSession, local_path: Union[str, Path]) -> DriveItem:
        ...
        raise FileError("Upload session completed without final item response")
    ...
    async def test_permissions(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self._logger.info(f"Drive access: {drive_info.name or 'Personal OneDrive'}")

            # Test 2: Folder access (list root)
            file_list = await self.file_list()
            results["folder_access"] = True
            self._logger.info(f"Folder access: Listed {len(file_list)} items")
