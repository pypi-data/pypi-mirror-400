...
# 2. Create extractor with key parameters
extractor = Extractor(
    schema=Invoice,
    llm=AnthropicLLM(),
    searcher=BM25Searcher(),
    top_k=5,  # Chunks per field (critical for quality)
)
...
# Or async for multiple documents:
# results = await asyncio.gather(*[extractor.extract_async(d) for d in docs])
...
### top_k - Chunks Per Field

Controls how many document chunks are retrieved per field. **Critical for extraction quality.**
...
extractor = Extractor(
    ...
    llm=AnthropicLLM(),
    searcher=BM25Searcher(),
    top_k=5,  # Default. Tune up for complex docs, down for simple ones.
)
```

- Higher = more context, better accuracy, more tokens
- Lower = faster, cheaper, may miss context
- Start with 5, tune based on your documents

---

## Chunking

Chunking directly affects extraction quality. Configure explicitly:

```python
from pullcite import Document, SlidingWindowChunker, SentenceChunker

# Option 1: Sliding window (predictable, good default)
doc = Document.from_file(
    "report.pdf",
    chunker=SlidingWindowChunker(size=500, stride=250),  # 50% overlap
)

# Option 2: Sentence-aware (better for prose)
doc = Document.from_file(
    "contract.pdf",
    chunker=SentenceChunker(target_size=1000, overlap=200),
)

# Option 3: Defaults (SentenceChunker, 1200 chars, 200 overlap)
doc = Document.from_file("invoice.pdf")
```

Available chunkers:
- `SlidingWindowChunker(size, stride)` - Fixed window with step size
- `SentenceChunker(target_size, overlap)` - Breaks at sentence boundaries
- `ParagraphChunker(target_size, overlap_paragraphs)` - Breaks at paragraphs
...
```python
import asyncio
from pullcite import Document, Extractor

async def extract_batch(paths: list[str]) -> list:
    extractor = Extractor(schema=Invoice, llm=llm, searcher=searcher)

    docs = [Document.from_file(p) for p in paths]
    results = await asyncio.gather(*[
        extractor.extract_async(doc) for doc in docs
    ])
    return results

# Run
results = asyncio.run(extract_batch(["inv1.pdf", "inv2.pdf", "inv3.pdf"]))
```
...
**Always provide descriptions** - they tell the LLM what each field means:

```python
class Invoice(ExtractionSchema):
    # Good: Clear description helps the LLM
    total = DecimalField(
        query="total amount due",
        description="The final total amount due, including tax",
    )

    # Without description, LLM only sees "total" and the query
...
Pullcite verifies extracted values against the source document. Here's how it works:

### How Verification Works

1. **Search**: For each field, retrieve relevant chunks using the field's query
2. **Parse**: Use `field.parse_from_text()` to find values in chunk text
3. **Compare**: Use `field.compare()` to check if extracted matches found

**Field-specific comparison:**
- `StringField`: Case-insensitive, whitespace-normalized
- `DecimalField`: Tolerance-based (±0.01 by default)
- `PercentField`: Tolerance-based
- `BooleanField`: Exact match after normalizing "yes"/"no"/etc.
...
for vr in result.verification_results:
    ...
    # MATCH: Value found and matches
    # MISMATCH: Found different value in source
    # NOT_FOUND: Required field not in document
    # SKIPPED: No context to verify against
```
...
extractor = Extractor(
    ...
    llm=llm,
    searcher=searcher,
    verify=False,  # Skip verification step
)
```
