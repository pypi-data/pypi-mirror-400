...
TEXTURE_FILE =  os.path.join(os.path.dirname(__file__),"sea_texture.jpg" ) 
...
class WaveView(app.Canvas):
    ...
    def __init__(self, title, width=800, height=600, pixel_scale=1.0 ):
        ...
        self.sea_texture = cv.imread( TEXTURE_FILE )
        self.tx_shift = 0.0
    ...
    def setup_field( self, XX, YY, P0 ):
        ...
        TX, TY = np.meshgrid( np.linspace(0.0,1.0,self.N), np.linspace(0.0,1.0,self.M) )
        TX = TX.reshape( (TX.size,1) ).astype( np.float32 )
        TY = TY.reshape( (TY.size,1) ).astype( np.float32 )

        self.grid = gloo.Program(vertex, fragment_bg )
        ...
        self.grid["texcoord"] = np.concatenate( (TX,TY), axis=1 )
    ...
    def on_draw(self, event):
        ...
        with self._fbo:
            ...
            self.grid.draw( "triangles", self.index_buff )
    ...
    def render( self, image, elevations ):
        ...
        self.grid['texture'] = gloo.Texture2D( self.sea_texture, wrapping="repeat" )
        #self.tx_shift += 0.001
        self.grid["txshift"] = self.tx_shift
