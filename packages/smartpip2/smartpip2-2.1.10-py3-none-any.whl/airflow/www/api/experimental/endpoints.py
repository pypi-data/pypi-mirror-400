...
from airflow.api.common.experimental.gen_dag_file import gen_dag_file,rm_dag_file,gen_dag_config
...
@api_experimental.route('/gen_dagconfig', methods=['GET'])
@requires_authentication
def gen_dagconfig():
    """generate the dag config base on etl script."""
    try:
        name = request.args.get('name')
        dag_str = gen_dag_config(name)
    except AirflowException as err:
        log.error(err)
        response = jsonify(error="{}".format(err))
        response.status_code = err.status_code
