...
class Repl:
    ...
    async def run(self):
        print("pw-repl v0.1.32 - Type 'help' for commands")
        # 自动重连上次的连接
        ...
        if name:
            ...
            if '/' in name:
                ...
                if 'kaggle' in url:
                    ...
                    platform = url.split('/')[2].split('.')[-2]  # 提取域名
                username = name
            ...
            if not hasattr(self, '_current_account_id'):
                print("✗ No session selected. Use: save <platform/username>")
                return
