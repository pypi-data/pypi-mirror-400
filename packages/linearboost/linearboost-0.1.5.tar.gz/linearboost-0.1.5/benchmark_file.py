...
class ModelBenchmark:
    ...
    def create_linearboost_preprocessor(self):
    ...
    def print_results(self, results, stat_results):
        ...
        for model in df.index:
            eff_data.append({
                'Model': model,
                'Avg Fit Time (s)': f"{df.loc[model, 'avg_fit_time']:.3f}±{df.loc[model, 'std_fit_time']:.3f}",
                'CPU Time (s)': f"{df.loc[model, 'cpu_time']:.3f}",
                'Wall Time (s)': f"{df.loc[model, 'wall_time']:.3f}",
                'CPU Efficiency': f"{(df.loc[model, 'cpu_time']/df.loc[model, 'wall_time']*100):.1f}%"
            })
...
if __name__ == "__main__":
    ...
    file_path = "/Users/hamidrezakeshavarz/Downloads/leukemia_big.csv"
    ...
    for col in categorical_cols:
        if len(X.loc[:, col].mode()) > 0:
            X.loc[:, col] = X.loc[:, col].fillna(X.loc[:, col].mode()[0])
