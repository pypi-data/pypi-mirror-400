...
class S3Connection:
    """
    S3 connection configuration for saving Lambda GPU outputs.
    ...
    Attributes:
        ...
        prefix: Optional prefix/folder for all uploads
...
'''
    ...
    def s3_connect(
        ...
        """
        Create an S3 connection for saving Lambda GPU outputs.

        Args:
            bucket: S3 bucket name
            access_key: AWS Access Key ID
            secret_key: AWS Secret Access Key
            endpoint: S3 endpoint URL (default: AWS S3, can use MinIO, etc.)
            region: AWS region (default: us-east-1)
            prefix: Optional prefix/folder for all uploads (e.g., "outputs/run-001/")

        Returns:
            S3Connection object to use with lambda_output() and lambda_output_file()
        ...
        Example:
            ...
            ...     prefix="experiments/"
            ... )
            >>> session.lambda_output("model.pt", model_state, s3_connection=s3)
        """
        return S3Connection(
            bucket=bucket,
    ...
    def lambda_output(
        ...
        """
        Save a Python object to S3 as a Lambda GPU output.

        Automatically detects the format based on filename extension:
        - .pt, .pth: PyTorch state dict (torch.save)
        - .npy: NumPy array (np.save)
        ...
        Args:
            ...
            s3_connection: S3Connection from s3_connect()
            format: Force format ("pt", "npy", "json", "pkl") instead of auto-detect

        Returns:
            S3 URL of the uploaded file

        Raises:
            ClouditiaError: If upload fails or boto3 not installed

        Example:
            >>> s3 = session.s3_connect(bucket="my-bucket", access_key="...", secret_key="...")
            >>>
            >>> # Save PyTorch model
            >>> session.lambda_output("model.pt", model.state_dict(), s3)
            >>>
            >>> # Save NumPy array
            >>> session.lambda_output("embeddings.npy", embeddings, s3)
            >>>
            >>> # Save JSON data
            >>> session.lambda_output("metrics.json", {"accuracy": 0.95}, s3)
        """
        try:
            import boto3
        except ImportError:
            raise ClouditiaError(
                "boto3 is required for S3 uploads. Install with: pip install clouditia[s3]"
            )
        ...
        if format == 'pt':
            try:
                ...
                raise ClouditiaError("PyTorch is required to save .pt files")
        elif format == 'npy':
            try:
                import numpy as np
                np.save(buffer, data)
            except ImportError:
                raise ClouditiaError("NumPy is required to save .npy files")
        elif format == 'json':
        ...
        # Upload to S3
        return self._upload_to_s3(buffer, filename, s3_connection)
    ...
    def lambda_output_file(
        ...
        """
        Upload an existing file to S3 as a Lambda GPU output.

        Args:
            local_path: Path to the local file to upload
            s3_connection: S3Connection from s3_connect()
            remote_filename: Optional custom filename in S3 (default: same as local)
        ...
        Example:
            ...
            >>> session.lambda_output_file("./checkpoints/best_model.pt", s3)
            >>>
            >>> # Upload with custom name
            >>> session.lambda_output_file("./model.pt", s3, remote_filename="models/v2/model.pt")
        """
        try:
            import boto3
        except ImportError:
        ...
        if not local_path.is_file():
            raise ClouditiaError(f"Not a file: {local_path}")

        filename = remote_filename or local_path.name

        with open(local_path, 'rb') as f:
            return self._upload_to_s3(f, filename, s3_connection)
    ...
    def _upload_to_s3(
        ...
        """
        Internal method to upload a file object to S3.

        Args:
            file_obj: File-like object to upload
            filename: Destination filename
            s3_connection: S3 connection configuration
        ...
        s3_key = s3_connection.prefix + filename if s3_connection.prefix else filename
        ...
        s3_client = boto3.client(
            ...
            endpoint_url=s3_connection.endpoint if s3_connection.endpoint != "https://s3.amazonaws.com" else None,
            region_name=s3_connection.region,
            config=Config(signature_version='s3v4')
        )

        # Upload
        try:
            s3_client.upload_fileobj(file_obj, s3_connection.bucket, s3_key)
        except Exception as e:
            raise ClouditiaError(f"S3 upload failed: {e}")

        # Return the S3 URL
        if s3_connection.endpoint == "https://s3.amazonaws.com":
            url = f"s3://{s3_connection.bucket}/{s3_key}"
        else:
            url = f"{s3_connection.endpoint}/{s3_connection.bucket}/{s3_key}"

        return url

    # ==================== END S3 OUTPUT METHODS ====================
