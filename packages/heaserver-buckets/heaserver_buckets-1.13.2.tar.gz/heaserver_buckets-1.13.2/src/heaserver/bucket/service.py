...
async def get_bucket_collaborators(request: web.Request) -> web.Response:
    ...
    async with DesktopObjectActionLifecycle(request=request,
        ...
        async with aws.S3ClientContext(request=request, credentials=credentials) as s3_client:
            try:
                resp = await asyncio.to_thread(s3_client.list_buckets, Prefix=bucket_id)
...
async def __get_bucket(request: web.Request, volume_id: str, s3_client: S3Client, cache,
    ...
    if creation_date:
        ...
        async def _get_creation_date(b: AWSBucket):
            ...
            try:
                assert b.name is not None, "bucket name cannot be None"
                buckets_filtered = (await asyncio.to_thread(s3_client.list_buckets, Prefix=b.name))['Buckets']
                if not buckets_filtered:
                    raise response.status_not_found()
                creation_date = buckets_filtered[0]['CreationDate']
                b.created = creation_date
    ...
    async def _get_version_status(b: AWSBucket):
        ...
        try:
            ...
            bucket_versioning = await asyncio.to_thread(s3_client.get_bucket_versioning, Bucket=b.name)
    ...
    async def _get_region(b: AWSBucket):
        ...
        try:
            assert b.name is not None, 'b.name cannot be None'
            loc = await asyncio.to_thread(s3_client.get_bucket_location, Bucket=b.name)
    ...
    async def _get_tags(b: AWSBucket):
        ...
        try:
            assert b.name is not None, 'b.name cannot be None'
            tagging = await asyncio.to_thread(s3_client.get_bucket_tagging, Bucket=b.name)
    ...
    async def _get_encryption_status(b: AWSBucket):
        ...
        try:
            assert b.name is not None, 'b.name cannot be None'
            encrypt = await asyncio.to_thread(s3_client.get_bucket_encryption, Bucket=b.name)
    ...
    async def _get_bucket_lock_status(b: AWSBucket):
        ...
        try:
            assert b.name is not None, 'b.name cannot be None'
            lock_config = await asyncio.to_thread(s3_client.get_object_lock_configuration, Bucket=b.name)
...
async def _detach_and_delete_policy(iam_client: IAMClient, policy_arn: str):
    ...
    await asyncio.to_thread(policy_detacher)
    await asyncio.to_thread(policy_deleter)
