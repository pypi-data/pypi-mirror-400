...
This is the entrypoint for the `buddy ws` application.
"""
...
ws_cli = typer.Typer(
    ...
    help="""\b
Use `buddy ws [COMMAND]` to create, setup, start or stop your workspace.
Run `buddy ws [COMMAND] --help` for more info.
""",
...
def create(
    ...
    \b
    Examples:
    > buddy ws create -t ai-app                -> Create an `ai-app` in the current directory
    > buddy ws create -t ai-app -n my-ai-app   -> Create an `ai-app` named `my-ai-app` in the current directory
    """
    if print_debug_log:
        set_log_level_to_debug()

    from buddy.workspace.operator import create_workspace
...
def setup(
    ...
    \b
    Examples:
    > `buddy ws setup`           -> Setup the current directory as a workspace
    > `buddy ws setup ai-app`    -> Setup the `ai-app` folder as a workspace
    """
    if print_debug_log:
        set_log_level_to_debug()

    from buddy.workspace.operator import setup_workspace
...
def up(
    ...
    > `buddy ws up prd`        -> Deploy all prd resources
    > `buddy ws up prd:aws`    -> Deploy all prd aws resources
    > `buddy ws up prd:::s3`   -> Deploy prd resources matching name s3
    """
    if print_debug_log:
        set_log_level_to_debug()
    ...
    from buddy.utils.resource_filter import parse_resource_filter
    from buddy.workspace.config import WorkspaceConfig
    from buddy.workspace.helpers import get_workspace_dir_path
    from buddy.workspace.operator import setup_workspace, start_workspace
    ...
    if not BUDDY_config:
        ...
        if not BUDDY_config:
            log_config_not_available_msg()
            return
    BUDDY_config = cast(BuddyCliConfig, BUDDY_config)
    ...
    ws_at_current_path: Optional[WorkspaceConfig] = BUDDY_config.get_ws_config_by_path(current_path)
    if ws_at_current_path is not None:
        logger.debug(f"Found workspace at: {ws_at_current_path.ws_root_path}")
        if str(ws_at_current_path.ws_root_path) != BUDDY_config.active_ws_dir:
            logger.debug(f"Updating active workspace to {ws_at_current_path.ws_root_path}")
            BUDDY_config.set_active_ws_dir(ws_at_current_path.ws_root_path)
    ...
    if ws_to_start is None:
        active_ws_config: Optional[WorkspaceConfig] = BUDDY_config.get_active_ws_config()
    ...
    if ws_to_start is None:
        log_active_workspace_not_available()
        avl_ws = BUDDY_config.available_ws
        if avl_ws:
...
def down(
    ...
    from buddy.workspace.config import WorkspaceConfig
    from buddy.workspace.helpers import get_workspace_dir_path
    from buddy.workspace.operator import setup_workspace, stop_workspace
...
def patch(
    ...
    Examples:
    \b
    > `buddy ws patch`           -> Patch all resources
    """
    if print_debug_log:
        set_log_level_to_debug()
    ...
    from buddy.workspace.config import WorkspaceConfig
    from buddy.workspace.helpers import get_workspace_dir_path
    from buddy.workspace.operator import setup_workspace, update_workspace
...
def restart(
    ...
    """\b
    Restarts the active workspace. i.e. runs `buddy ws down` and then `buddy ws up`.
...
def config(
    ...
    from buddy.cli.config import BuddyCliConfig
    from buddy.cli.operator import initialize_buddy
    from buddy.utils.load_env import load_env
    from buddy.workspace.config import WorkspaceConfig
...
def delete(
    ...
    all_workspaces: bool = typer.Option(
        ...
        "-a",
        "--all",
        help="Delete all workspaces from Buddy",
    ),
    print_debug_log: bool = typer.Option(
        False,
        "-d",
    ...
    """\b
    Deletes the workspace record from buddy.
    NOTE: Does not delete any physical files.

    \b
    Examples:
    $ `buddy ws delete`         -> Delete the active workspace from Buddy
    $ `buddy ws delete -a`      -> Delete all workspaces from Buddy
    """
    if print_debug_log:
        set_log_level_to_debug()
    ...
    from buddy.cli.config import BuddyCliConfig
    from buddy.cli.operator import initialize_buddy
    from buddy.workspace.operator import delete_workspace
    ...
    if ws_name is not None:
        ws_config = BUDDY_config.get_ws_config_by_dir_name(ws_name)
        ...
        if all_workspaces:
            ws_to_delete = [ws.ws_root_path for ws in BUDDY_config.available_ws if ws.ws_root_path is not None]
        else:
            # By default, we assume this command is run for the active workspace
            if BUDDY_config.active_ws_dir is not None:
