...
def fetch_market_data_auto():
    ...
    # one global lock so we don't stack multiple auto runs
    ...
    try:
        # Use existing market fetch task, but route through a dedicated queue if configured.
        # This keeps behavior consistent while letting you run a low-concurrency ESI worker.
...
def refresh_contracts_for_character(
    ...
    try:
        ...
        db_log(
            source="contracts",
            event="fetched",
            data=_ctx({**ctx, "contracts_seen": seen}),
        )
...
def get_doctrine_needles() -> list[str]:
    needles: set[str] = set()
    for tc in TrackedContract.objects.filter(mode=TrackedContract.Mode.DOCTRINE, is_active=True).select_related("fitting"):
...
def _recalculate_contract_statuses_and_alert(all_contracts, yellow, red, allow_esi_fetch: bool = True):
    ...
    if suspicious_all_zero:
        db_log(
            ...
            source="contracts",
            event="suspicious_all_zero",
            message="All tracked contracts went to zero in one snapshot; rolling back status updates",
            data=_ctx({
                "changed": len(changed),
                "tracked_count": tracked_count,
                "snapshot_count": snapshot_count,
        ...
        for c in changed:
            TrackedContract.objects.filter(pk=c["tc_id"]).update(last_status=c["old_status"] or "OK")
