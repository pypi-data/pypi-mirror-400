...
- `logic_server.cli.main` runs queries over Prolog fact files.
- `logic_server.cli.main` provides a CLI entry point for querying Prolog facts.
...
**Stateless**: Each `logic_query` call is independent and must include all facts.
