...
def _try_import_torch() -> Optional[Any]:  # type: ignore[misc]
    ...
    Note:
        Return type is intentionally Any to avoid requiring torch as a dependency.
    """
    try:
        import torch
    except Exception:
...
def embed_texts(
    ...
    if dim_opt is None:
        raise ValueError("Embedding dimension unavailable from model")
    dim = int(dim_opt)
