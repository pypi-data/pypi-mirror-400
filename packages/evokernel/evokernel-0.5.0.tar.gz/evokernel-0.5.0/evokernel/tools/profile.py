    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ncu_result = subprocess.run(
                ncu_cmd, capture_output=True, text=True, timeout=120
            )
...
def run_basic_analysis(kernel_path: Path) -> dict:
    ...
    result["analysis"]["warps"] = len(
        re.findall(r"__shfl|__ballot|__any|__all", content)
    )
    result["analysis"]["math_intrinsics"] = len(
        re.findall(r"__f\w+|__exp|__log|__sqrt|__rsqrt", content)
    )
...
def execute_profile(
    ...
    if not filepath:
        yield Message(
            "system", "Error: No file path provided. Use: evo_profile kernel.cu"
        )
        return
    ...
    if tools["ncu"]:
        ...
        include_dirs = (
            [d.strip() for d in include_dirs_str.split(",") if d.strip()]
            if include_dirs_str
            else None
        )
        ...
        lines.append(
            "[dim]Note: Install Nsight Compute (ncu) for runtime profiling[/dim]"
        )
        lines.append("")
...
""",
    ...
    parameters=[
        Parameter(
            name="file",
            type="string",
            description="Path to CUDA kernel file",
            required=True,
        ),
        Parameter(
            name="include_dirs",
            type="string",
            description="Extra include directories (comma-separated)",
            required=False,
        ),
    ],
)
