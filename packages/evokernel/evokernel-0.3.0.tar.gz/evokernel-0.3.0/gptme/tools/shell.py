...
def _format_shell_output(
    ...
    # Apply shortening for very long output (model gets substantial content)
    # Higher limits than before since model needs full context
    stdout = _shorten_stdout(
        stdout, pre_tokens=2000, post_tokens=4000, logdir=logdir, cmd=cmd
    )
    stderr = _shorten_stdout(
        stderr, pre_tokens=1000, post_tokens=1000, logdir=logdir, cmd=f"{cmd} (stderr)"
    )
...
def execute_shell_impl(
    ...
    try:
        # Suppress live output for allowlisted commands - model gets full output,
        # user sees truncated version when message is displayed
        returncode, stdout, stderr = shell.run(
            cmd, output=not allowlisted, timeout=timeout
        )
        interrupted = False
