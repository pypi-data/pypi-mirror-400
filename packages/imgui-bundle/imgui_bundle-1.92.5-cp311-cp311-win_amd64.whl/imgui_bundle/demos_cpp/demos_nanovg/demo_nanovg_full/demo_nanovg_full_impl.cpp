...
int loadDemoData(NVGcontext* vg, DemoData* data)
{
    ...
    data->fontIcons = nvgCreateFont(vg, "icons", HelloImGui::AssetFileFullPath("fonts/entypo.ttf").c_str());
	if (data->fontIcons == -1) {
		printf("Could not add font icons.\n");
		return -1;
	}

    data->fontNormal = nvgCreateFont(vg, "sans", HelloImGui::AssetFileFullPath("fonts/Roboto/Roboto-Regular.ttf").c_str());
	if (data->fontNormal == -1) {
		printf("Could not add font italic.\n");
		return -1;
	}

    data->fontBold = nvgCreateFont(vg, "sans-bold", HelloImGui::AssetFileFullPath("fonts/Roboto/Roboto-Bold.ttf").c_str());
	if (data->fontBold == -1) {
		printf("Could not add font bold.\n");
		return -1;
	}

    data->fontEmoji = nvgCreateFont(vg, "emoji", HelloImGui::AssetFileFullPath("fonts/NotoEmoji-Regular.ttf").c_str());
    if (data->fontEmoji == -1) {
