...
from xladmin import widgets as exwidgets
from xladmin.layout import FormHelper
from xladmin.models import UserSettings, UserWidget
from xladmin.plugins.utils import get_context_dict
from xladmin.sites import site
from xladmin.views.base import CommAdminView, ModelAdminView, filter_hook, csrf_protect_m
from xladmin.views.edit import CreateAdminView
from xladmin.views.list import ListAdminView
from xladmin.util import unquote
import copy
...
class BaseWidget(forms.Form):
    ...
    template = 'xladmin/widgets/base.html'
...
class ModelChoiceIterator(object):
    ...
    def __iter__(self):
        from xladmin import site as g_admin_site
...
class QuickBtnWidget(BaseWidget):
    ...
    template = "xladmin/widgets/qbutton.html"
...
class AddFormWidget(ModelBaseWidget, PartialBaseWidget):
    ...
    template = "xladmin/widgets/addform.html"
    model_perm = 'add'
    ...
    def media(self):
        return self.add_view.media + self.add_view.form_obj.media + self.vendor('xladmin.plugin.quick-form.js')
...
class Dashboard(CommAdminView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return self.template_response('xladmin/views/dashboard.html', self.get_context())
    ...
    def get_media(self):
        media = super(Dashboard, self).get_media() + \
            self.vendor('xladmin.page.dashboard.js', 'xladmin.page.dashboard.css')
        if self.widget_customiz:
            media = media + self.vendor('xladmin.plugin.portal.js')
