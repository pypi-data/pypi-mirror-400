...
class DownloadManager:
    ...
    def _should_emit_progress(self) -> bool:
        ...
        return now - self._last_progress_emit >= self._progress_interval
    ...
    def _emit_progress(self, part_id: int) -> None:
        ...
        # Calculate per-part speed using same time_delta (guarantees sum = total)
        part_bytes_current = self._part_bytes_downloaded.get(part_id, 0)
        part_bytes_last = self._last_part_bytes.get(part_id, 0)
        part_bytes_delta = part_bytes_current - part_bytes_last

        if time_delta > 0:
            part_speed_bytes_per_sec = part_bytes_delta / time_delta
            part_speed = (part_speed_bytes_per_sec * 8) / 1_000_000  # Convert to Mbps
        else:
            part_speed = 0.0

        # Update tracking for next calculation (AFTER calculating deltas)
        self._last_progress_emit = now
        self._last_progress_bytes = self._bytes_downloaded
        self._last_part_bytes[part_id] = part_bytes_current
        ...
        progress_data = {
            ...
            "part_speed_mbps": part_speed,  # Per-part speed (sum of parts = total)
        }
