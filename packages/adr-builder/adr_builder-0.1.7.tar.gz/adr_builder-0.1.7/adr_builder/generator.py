...
from .fs import ensure_dir, format_index, next_index, parse_adr_filename, slugify
from .models import CriteriaModel

# Built-in template names mapped to their filenames
BUILTIN_TEMPLATES = {
    "madr": "madr.md.j2",
    "hld": "hld.md.j2",
    "lld": "lld.md.j2",
...
def resolve_builtin_template(name: str) -> Path | None:
    """Resolve a built-in template name to its path, or None if not a built-in."""
    if name.lower() in BUILTIN_TEMPLATES:
        return get_default_template_dir() / BUILTIN_TEMPLATES[name.lower()]
    return None
...
def prepare_adr_output(criteria: CriteriaModel, docs_dir: Path) -> AdrOutputInfo:
    ...
    # Ensure we don't collide with existing ADRs if the index changed between scans.
    while any(adr_dir.glob(f"{format_index(index)}-*.*")):
        index += 1
...
def _require_docx() -> tuple[object, object, object]:
    try:
        ...
        raise RuntimeError(
            "Word output requires 'python-docx'. Install with "
            "`pipx install 'adr-builder[docx]'` or `pip install 'adr-builder[docx]'`."
        ) from exc
    return Document, WD_PARAGRAPH_ALIGNMENT, Pt


def _title_from_md(path: Path, fallback: str) -> str:
    try:
...
def write_index(adr_dir: Path) -> Path:
    entries: dict[int, tuple[Path, str]] = {}

    for md_path in adr_dir.glob("*.md"):
        if md_path.name == "index.md":
            continue
        parsed = parse_adr_filename(md_path.name)
        if not parsed:
            continue
        idx, slug, _ = parsed
        title = _title_from_md(md_path, slug)
        entries[idx] = (md_path, title)

    for docx_path in adr_dir.glob("*.docx"):
        parsed = parse_adr_filename(docx_path.name)
        if not parsed:
            continue
        idx, slug, _ = parsed
        if idx not in entries:
            entries[idx] = (docx_path, slug)

    lines = ["# ADR Index", ""]
    if entries:
        for idx in sorted(entries):
            path, title = entries[idx]
            lines.append(f"- {format_index(idx)} - [{title}]({path.name})")
    else:
...
def write_adr(
    ...
    write_index(output_info.adr_dir)
    return output_path
...
def write_adr_docx(
    ...
    Document, WD_PARAGRAPH_ALIGNMENT, Pt = _require_docx()
    doc = Document()
