...
class EndSessionView(LogoutView):
    # Django 4.1+ changed LogoutView to POST-only by default
    # OIDC end_session_endpoint requires GET support per spec
    ...
    def get(self, request, *args, **kwargs):
        # OIDC spec requires immediate logout on GET, not a confirmation page
        # Django 5+ LogoutView.get() renders a template, so we call post() instead
