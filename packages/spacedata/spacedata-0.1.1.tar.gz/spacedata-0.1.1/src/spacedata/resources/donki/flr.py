...
from spacedata.models.donki import DONKIDateRangeInput, DONKIFlr
...
async def _get_response(
    ...
    response = await resource.client.make_request(
        method="GET",
        url=f"{resource.client.settings.main_base_url}/DONKI/FLR",
        params=params,
    )
    return [DONKIFlr.model_validate(item) for item in response]
...
async def flr(
    ...
    **kwargs,
) -> SpaceDataResult[DONKIFlr]:
    self.client.logger.debug("Calling flr endpoint with input_data: %s", input_data)
