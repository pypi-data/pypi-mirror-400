                ...
                if col in data.columns.get_level_values(0):
                    ...
                    for column in data.columns[mask]:
                        ...
                        if negative_mask.any():
                            logger.warning(f"Found {negative_mask.sum()} negative values in {column}")
        ...
        if isinstance(data.columns, pd.MultiIndex):
            if 'Volume' in data.columns.get_level_values(0):
                ...
                for column in data.columns[mask]:
                    ...
                    if zero_volume > 0:
                        logger.info(f"Found {zero_volume} zero volume periods in {column}")
        else:
    ...
    def _check_ohlc_consistency(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if isinstance(data.columns, pd.MultiIndex):
            ...
            for symbol in symbols:
                if all(col in data.columns.get_level_values(0) 
                    ...
                    if inconsistent.any():
                        logger.warning(f"Found {inconsistent.sum()} periods where High < Low for {symbol}")
            ...
            if all(col in data.columns for col in ['Open', 'High', 'Low', 'Close']):
                ...
                if inconsistent.any():
                    logger.warning(f"Found {inconsistent.sum()} periods where High < Low")
                    # Fix by swapping values
                ...
                for col in price_cols:
                    if (col, symbol) in data.columns:
                        ...
                        if len(series) > 1:
                            ...
                            if large_gaps.any():
                                logger.warning(f"Found {large_gaps.sum()} large price gaps (>50%) in {col} for {symbol}")
        else:
            for col in price_cols:
                if col in data.columns:
                    series = data[col]
                    if len(series) > 1:
                        ...
                        large_gaps = pct_change > 0.5  # 50% price change
                        if large_gaps.any():
                            logger.warning(f"Found {large_gaps.sum()} large price gaps (>50%) in {col}")
                    ...
                    if ('Volume', symbol) in data.columns:
                        ...
                        if len(volume) > 10:
                            ...
                            if volume_spikes.any():
                                logger.info(f"Found {volume_spikes.sum()} volume spikes (>10x median) for {symbol}")
        else:
            if 'Volume' in data.columns:
                volume = data['Volume']
                if len(volume) > 10:
                    ...
                    volume_spikes = volume > (median_volume * 10)  # 10x median volume
                    if volume_spikes.any():
                        logger.info(f"Found {volume_spikes.sum()} volume spikes (>10x median)")
    ...
    def _check_timestamp_consistency(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if isinstance(data.index, pd.DatetimeIndex):
            ...
            if duplicates.any():
                ...
                if self.strict:
                    raise ValidationError(f"Duplicate timestamps found: {duplicates.sum()}")
...
class OutlierDetector(DataProcessor):
    ...
    def __init__(self, method: str = 'iqr', threshold: float = 3.0, handle_method: str = 'cap', 
    ...
    def fit(self, data: pd.DataFrame) -> 'OutlierDetector':
        ...
        if self.method in ['isolation_forest', 'elliptic_envelope', 'dbscan']:
            ...
            if len(clean_data) < 10:
                logger.warning("Insufficient data for ML outlier detection, falling back to IQR")
                self.method = 'iqr'
            else:
                ...
                if self.handle_method == 'remove':
                    ...
                    for col in numeric_cols:
                        if col in result.columns:
                            ...
                            result.loc[outlier_indices, col] = result.loc[outlier_indices, col].clip(
        ...
        # Handle statistical methods
            ...
            for col, (lower_bound, upper_bound) in self.bounds_.items():
                ...
                if self.handle_method == 'remove':
                    ...
                    # Add a flag column
                    flag_col = f"{col}_outlier" if not isinstance(col, tuple) else f"{col[0]}_{col[1]}_outlier"
                    result[flag_col] = outliers
        else:
            raise ValueError("OutlierDetector must be fitted before transform")
        
        if outlier_count > 0:
            logger.info(f"Detected and handled {outlier_count} outliers using {self.method} method")
...
class MissingDataHandler(DataProcessor):
    ...
    def __init__(self, method: str = 'forward_fill', max_consecutive: int = 5, 
        ...
        if self.method == 'forward_fill':
            ...
            if isinstance(result.index, pd.DatetimeIndex):
                if self.interpolation_method == 'linear':
                    result = result.interpolate(method='time', limit=self.max_consecutive)
                elif self.interpolation_method == 'polynomial':
                    result = result.interpolate(method='polynomial', order=2, limit=self.max_consecutive)
                elif self.interpolation_method == 'spline':
                    result = result.interpolate(method='spline', order=3, limit=self.max_consecutive)
                elif self.interpolation_method == 'akima':
                    result = result.interpolate(method='akima', limit=self.max_consecutive)
                else:
                    result = result.interpolate(method='time', limit=self.max_consecutive)
            else:
                result = result.interpolate(method=self.interpolation_method, limit=self.max_consecutive)
