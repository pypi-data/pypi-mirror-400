...
async def rollout_openai_responses(
    ...
    **kwargs: Any,
    ...
    try:
        return await _execute_openai_responses_request(client, actor, on_chunk)
    finally:
        await client.close()


async def _execute_openai_responses_request(
    client: AsyncOpenAI,
    actor: Actor,
    on_chunk: Callable[[StreamEvent], Awaitable[None]],
) -> Actor:
    """Execute the OpenAI Responses API request. Separated for try/finally cleanup in caller."""
