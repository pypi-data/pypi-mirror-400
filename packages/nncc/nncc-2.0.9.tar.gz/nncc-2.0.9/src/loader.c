            ...
            if (strcmp(prop, "dtype") == 0) {
                ...
                while (parser.pos < parser.len && parser.src[parser.pos] != ']') {
                    if (ndim >= shape_cap) {
                        ...
                        if (!new_arr) { free(shape); free(key); return NNCC_ERROR_OUT_OF_MEMORY; }
                        shape = new_arr;
                        shape_cap = new_cap;
                    }
    ...
    while (parser.pos < parser.len && parser.src[parser.pos] != '}') {
        ...
        if (strcmp(key, "name") == 0) {
            ...
            while (parser.pos < parser.len && parser.src[parser.pos] != ']') {
                ...
                // Conv2D/Pool attributes (defaults from op_create will be used if not present)
                int kernel_h = 0, kernel_w = 0;  // 0 means use defaults
                ...
                while (parser.pos < parser.len && parser.src[parser.pos] != '}') {
                    ...
                    } else if (strcmp(prop, "kernel_size") == 0) {
                        json_match(&parser, '[');
                        kernel_h = json_parse_int(&parser);
                        json_match(&parser, ',');
                        kernel_w = json_parse_int(&parser);
                        json_match(&parser, ']');
                    } else if (strcmp(prop, "stride") == 0) {
                        json_match(&parser, '[');
                        stride_h = json_parse_int(&parser);
                        json_match(&parser, ',');
                        stride_w = json_parse_int(&parser);
                        json_match(&parser, ']');
                    } else if (strcmp(prop, "padding") == 0) {
                        json_match(&parser, '[');
                        pad_h = json_parse_int(&parser);
                        json_match(&parser, ',');
                        pad_w = json_parse_int(&parser);
                        json_match(&parser, ']');
                    } else if (strcmp(prop, "groups") == 0) {
                        groups = json_parse_int(&parser);
                    } else {
                        json_skip_value(&parser);
                    }
                    free(prop);
                ...
                if (layer_name && op_str) {
                    ...
                    if (op) {
                        ...
                        // Apply parsed conv/pool attributes (only if values were found in JSON)
                        if (op_type == OP_CONV2D && (kernel_h > 0 || kernel_w > 0)) {
                            op_set_conv_attrs(op, kernel_h, kernel_w, 
                                              stride_h > 0 ? stride_h : 1, 
                                              stride_w > 0 ? stride_w : 1, 
                                              pad_h, pad_w, groups);
                        } else if ((op_type == OP_MAXPOOL2D || op_type == OP_AVGPOOL2D) && (kernel_h > 0 || kernel_w > 0)) {
                            op_set_pool_attrs(op, kernel_h, kernel_w,
                                              stride_h > 0 ? stride_h : kernel_h,  // Pool default stride = kernel size
                                              stride_w > 0 ? stride_w : kernel_w,
                                              pad_h, pad_w);
                        }
