...
class RunnerAbstraction(BaseAbstraction):
    def __init__(
        ...
        gpu: Union[GpuTypeAlias, List[GpuTypeAlias], str] = GpuType.NoGPU,
        gpu_count: int = 0,
        image: Image = Image(),
        workers: int = 1,
        ...
        self.handler: str = ""
        self.on_start: str = ""
        self.on_deploy: Optional["AbstractCallableWrapper"] = self.parse_on_deploy(on_deploy)
    ...
    def parse_memory(self, memory_str: Union[int, str]) -> int:
    ...
    def _map_callable_to_attr(self, *, attr: str, func: Callable):
        ...
        elif module and getattr(module, "__file__", None):
            # Normal module case - use relative path
            module_file = os.path.relpath(cast(str, module.__file__), start=os.getcwd()).replace(
                "/", "."
            )
    ...
    def parse_gpu(self, gpu: Union[GpuTypeAlias, List[GpuTypeAlias], str]) -> str:
    ...
    def parse_on_deploy(
        self, func: Optional[Union[Callable, "AbstractCallableWrapper"]]
    ) -> Optional["AbstractCallableWrapper"]:
        if func is None:
            return None

        if isinstance(func, AbstractCallableWrapper):
            return func
        ...
        return cast(AbstractCallableWrapper, func)
