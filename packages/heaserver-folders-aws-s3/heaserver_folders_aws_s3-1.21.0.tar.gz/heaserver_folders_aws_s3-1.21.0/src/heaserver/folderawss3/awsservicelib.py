            ...
            try:
                ...
                async def item_processor(obj_sum):
                    ...
                    try:
                        ...
                        if client_error_code(e) == CLIENT_ERROR_RESTORE_ALREADY_IN_PROGRESS:
                            if skip_restore_already_in_progress:
                                logger.debug('Restore already in progress for %s -- skipping', obj_sum['Key'])
                            else:
                                raise response.status_bad_request(f'Restore already in progress for {s3_object_message_display_name(bucket_name, obj_sum["Key"])}')
                        else:
                            raise e
