...
if __name__ == "__main__":
    ...
    # 测试降序（默认）
    top_notes_desc = v5.get_top_relevant_notes(notes, query, limit=2, sort_desc=True, max_chars=50)
    print(f"降序排列:\n{top_notes_desc}")
    for note in top_notes_desc:
        print(f"  ID: {note[0]}, 分数: {note[3]}")
    
    top_notes_asc = v5.get_top_relevant_notes(notes, query, limit=2, sort_desc=False, max_chars=50)
    print(f"升序排列:\n{top_notes_asc}")
