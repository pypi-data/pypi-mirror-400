...
@dataclass
class ExtractionNotification:
    """Notification sent from worker to main when part is streamed to tar."""

    part_id: int  # Part number (0-indexed)
    bytes_written: int  # Bytes written to FIFO
    already_verified: bool  # True if from resume (skip verification)


@dataclass
class TarExtractionComplete:
    """Completion signal sent from tar extractor to main process."""
