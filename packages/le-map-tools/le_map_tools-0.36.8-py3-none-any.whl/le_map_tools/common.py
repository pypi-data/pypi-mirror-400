...
# *******************************************************************************#
# This module contains core features and extra features of the le_map_tools package.   #
# The Earth Engine team and the le_map_tools community will maintain the core features.#
...
def ee_export_image_collection(
    ...
    if not isinstance(ee_object, le.ImageCollection):
    ...
    try:
        ...
        for i in range(0, count):
            image = le.Image(ee_object.toList(count).get(i))
...
def ee_export_image_to_drive(
    ...
    **kwargs,
    ...
    if not isinstance(image, le.Image):
        raise ValueError("Input image must be an instance of ee.Image")

    task = le.batch.Export.image.toDrive(
        image,
        description,
        folder,
        fileNamePrefix,
...
def ee_export_image_to_asset(
    ...
    **kwargs,
    ...
    if isinstance(image, le.Image) or isinstance(image, le.image.Image):
        pass
    else:
        raise ValueError("Input image must be an instance of ee.Image")
    ...
    task = le.batch.Export.image.toAsset(
        image,
        description,
        assetId,
        pyramidingPolicy,
...
def ee_export_image_to_cloud_storage(
    ...
    **kwargs,
    ...
    try:
        task = le.batch.Export.image.toCloudStorage(
            image,
            description,
            bucket,
            fileNamePrefix,
...
def ee_export_geojson(
    ...
    if not isinstance(ee_object, le.FeatureCollection):
...
def ee_export_vector_to_drive(
    ...
    **kwargs,
    ...
    """
    if not isinstance(collection, le.FeatureCollection):
    ...
    task = le.batch.Export.table.toDrive(
        collection,
        description,
        folder,
        fileNamePrefix,
...
def ee_export_vector_to_asset(
    ...
    **kwargs,
    ...
    task = le.batch.Export.table.toAsset(
        collection,
        description,
        assetId,
        maxVertices,
...
def ee_export_vector_to_cloud_storage(
    ...
    **kwargs,
    ...
    task = le.batch.Export.table.toCloudStorage(
        collection,
        description,
        bucket,
        fileNamePrefix,
...
def ee_export_vector_to_feature_view(
    ...
    **kwargs,
    ...
    task = le.batch.Export.table.toFeatureView(
        collection,
        description,
        assetId,
...
def ee_export_video_to_drive(
    ...
    **kwargs,
    ...
    """
    if not isinstance(collection, le.ImageCollection):
    ...
    task = le.batch.Export.video.toDrive(
        collection,
        description,
        folder,
        fileNamePrefix,
...
def ee_export_video_to_cloud_storage(
    ...
    **kwargs,
    ...
    task = le.batch.Export.video.toCloudStorage(
        collection,
        description,
        bucket,
        fileNamePrefix,
...
def ee_export_map_to_cloud_storage(
    ...
    **kwargs,
    ...
    task = le.batch.Export.map.toCloudStorage(
        image,
        description,
        bucket,
        fileFormat,
...
# ******************************************************************************#
# The classes and functions above are the core features of the le_map_tools package.  #
# The Earth Engine team and the le_map_tools community will maintain these features.  #
# ******************************************************************************#

# ******************************************************************************#
# The classes and functions below are the extra features of the le_map_tools package. #
# The le_map_tools community will maintain these features.                            #
# ******************************************************************************#
...
def update_package():
    """Updates the le_map_tools package from the le_map_tools GitHub repository without the need to use pip or conda.
    ...
    try:
        ...
        pkg_dir = os.path.join(download_dir, "le_map_tools-master")
        work_dir = os.getcwd()
        os.chdir(pkg_dir)
        ...
        print(
            "\nPlease comment out 'le_map_tools.update_package()' and restart the kernel to take effect:\nJupyter menu -> Kernel -> Restart & Clear Output"
        )
...
def clone_repo(out_dir=".", unzip=True):
    """Clones the le_map_tools GitHub repository.
    ...
    filename = "le_map_tools-master.zip"
...
def open_youtube():
    """Opens the YouTube tutorials for le_map_tools."""
    import webbrowser
...
def ee_to_geojson(ee_object, filename=None, indent=2, **kwargs):
    ...
    try:
        if (
            isinstance(ee_object, le.Geometry)
            or isinstance(ee_object, le.Feature)
            or isinstance(ee_object, le.FeatureCollection)
        ):
...
def ee_to_bbox(ee_object):
    ...
    """
    if (
        isinstance(ee_object, le.Image)
        or isinstance(ee_object, le.Feature)
        or isinstance(ee_object, le.FeatureCollection)
    ):
        geometry = ee_object.geometry()
    elif isinstance(ee_object, le.Geometry):
        geometry = ee_object
    else:
        raise Exception(
...
def filter_polygons(ftr):
    ...
    geometries = geometries.map(
        lambda geo: le.Feature(le.Geometry(geo)).set("geoType", le.Geometry(geo).type())
    )

    polygons = (
        le.FeatureCollection(geometries)
        .filter(le.Filter.eq("geoType", "Polygon"))
        .geometry()
    )
    return le.Feature(polygons).copyProperties(ftr)
...
def dict_to_csv(data_dict, out_csv, by_row=False, timeout=300, proxies=None):
    ...
    if not by_row:
        csv_feature = le.Feature(None, data_dict)
        csv_feat_col = le.FeatureCollection([csv_feature])
    else:
        keys = data_dict.keys()
        data = keys.map(lambda k: le.Dictionary({"name": k, "value": data_dict.get(k)}))
        csv_feature = data.map(lambda f: le.Feature(None, f))
        csv_feat_col = le.FeatureCollection(csv_feature)
...
def numpy_to_ee(np_array, crs=None, transform=None, transformWkt=None, band_names=None):
    ...
    try:
        projection = le.Projection(crs, transform, transformWkt)
        coords = le.Image.pixelCoordinates(projection).floor().int32()
        x = coords.select("x")
        y = coords.select("y")
        s = np_array.shape
        if len(s) < 3:
        ...
        def list_to_ee(a_list):
            ee_data = le.Array(a_list)
            image = le.Image(ee_data).arrayGet(coords)
            return image
...
def ee_to_numpy(ee_object, region=None, scale=None, bands=None, **kwargs):
    ...
    try:
        struct_array = le.data.computePixels(kwargs)
...
def ee_to_xarray(
    ...
    **kwargs,
    ...
    if ee_initialize:
        ...
        le.Initialize(opt_url=opt_url)

    if isinstance(dataset, str):
        if not dataset.startswith("ee://"):
            dataset = "ee://" + dataset
    elif isinstance(dataset, le.Image):
        dataset = le.ImageCollection(dataset)
    elif isinstance(dataset, le.ImageCollection):
        pass
...
def download_ee_video(collection, video_args, out_gif, timeout=300, proxies=None):
    ...
    if "region" in video_args.keys():
        ...
        if not isinstance(roi, le.Geometry):
            try:
                roi = roi.geometry()
            except Exception as e:
...
def api_docs():
    """Open a browser and navigate to the le_map_tools API documentation."""
    import webbrowser
...
def create_colorbar(
    ...
    # pkg_dir = os.path.dirname(pkg_resources.resource_filename("le_map_tools", "le_map_tools.py"))
    pkg_dir = os.path.dirname(files("le_map_tools") / "le_map_tools.py")
    if out_file is None:
...
def ee_user_id():
    ...
    """
    # ee_initialize()
    roots = le.data.getAssetRoots()
    if len(roots) == 0:
        return None
    else:
        root = le.data.getAssetRoots()[0]
...
def date_sequence(start, end, unit, date_format="YYYY-MM-dd", step=1):
    ...
    start_date = le.Date(start)
    end_date = le.Date(end)

    if unit != "quarter":
        count = le.Number(end_date.difference(start_date, unit)).toInt()
        num_seq = le.List.sequence(0, count)
        if step > 1:
        ...
        unit = "month"
        count = le.Number(end_date.difference(start_date, unit)).divide(3).toInt()
        num_seq = le.List.sequence(0, count.multiply(3), 3)
        date_seq = num_seq.map(
            lambda d: start_date.advance(d, unit).format(date_format)
        )
...
def load_GeoTIFF(URL):
    ...
    cloud_image = le.Image.loadGeoTIFF(uri)
    return cloud_image
...
def load_GeoTIFFs(URLs):
    ...
    URIs = le.List(URIs)
    collection = URIs.map(lambda uri: le.Image.loadGeoTIFF(uri))
    return le.ImageCollection(collection)
...
def image_props(img, date_format="YYYY-MM-dd"):
    ...
    """
    if not isinstance(img, le.Image):
    ...
    props = le.Dictionary.fromLists(keys, values)

    names = keys.getInfo()

    bands = img.bandNames()
    scales = bands.map(lambda b: img.select([b]).projection().nominalScale())
    scale = le.Algorithms.If(
        scales.distinct().size().gt(1),
        le.Dictionary.fromLists(bands.getInfo(), scales),
        scales.get(0),
    )
    ...
    if "system:time_start" in names:
        image_date = le.Date(img.get("system:time_start")).format(date_format)
        time_start = le.Date(img.get("system:time_start")).format("YYYY-MM-dd HH:mm:ss")
        # time_end = ee.Date(img.get('system:time_end')).format('YYYY-MM-dd HH:mm:ss')
        time_end = le.Algorithms.If(
            le.List(img.propertyNames()).contains("system:time_end"),
            le.Date(img.get("system:time_end")).format("YYYY-MM-dd HH:mm:ss"),
            time_start,
        )
    ...
    if "system:asset_size" in names:
        asset_size = (
            le.Number(img.get("system:asset_size"))
            .divide(1e6)
            .format()
            .cat(le.String(" MB"))
        )
...
def image_stats(img, region=None, scale=None):
    ...
    stat_results = le.List([image_min, image_max, image_mean, image_std, image_sum])

    stats = le.Dictionary.fromLists(stat_types, stat_results)
...
def zonal_stats(
    ...
    **kwargs,
    ...
    if isinstance(in_value_raster, le.ImageCollection):
        in_value_raster = in_value_raster.toBands()

    if not isinstance(in_value_raster, le.Image):
        print("The input raster must be an ee.Image.")
        return

    if not isinstance(in_zone_vector, le.FeatureCollection):
    ...
    allowed_statistics = {
        "COUNT": le.Reducer.count(),
        "MEAN": le.Reducer.mean(),
        "MEAN_UNWEIGHTED": le.Reducer.mean().unweighted(),
        "MAXIMUM": le.Reducer.max(),
        "MEDIAN": le.Reducer.median(),
        "MINIMUM": le.Reducer.min(),
        "MODE": le.Reducer.mode(),
        "STD": le.Reducer.stdDev(),
        "MIN_MAX": le.Reducer.minMax(),
        "SUM": le.Reducer.sum(),
        "VARIANCE": le.Reducer.variance(),
        "HIST": le.Reducer.histogram(
            maxBuckets=max_buckets, minBucketWidth=min_bucket_width, maxRaw=max_raw
        ),
        "FIXED_HIST": le.Reducer.fixedHistogram(hist_min, hist_max, hist_steps),
        "COMBINED_COUNT_MEAN": le.Reducer.count().combine(
            le.Reducer.mean(), sharedInputs=True
        ),
        "COMBINED_COUNT_MEAN_UNWEIGHTED": le.Reducer.count().combine(
            le.Reducer.mean().unweighted(), sharedInputs=True
        ),
    }
    ...
    elif isinstance(stat_type, le.Reducer):
        reducer = stat_type
    else:
...
def zonal_stats_by_group(
    ...
    **kwargs,
    ...
    try:
        ...
        class_values = (
            le.Dictionary(hist.get(band_name))
            .keys()
            .map(lambda v: le.Number.parse(v))
            .sort()
        )

        class_names = class_values.map(
            lambda c: le.String("Class_").cat(le.Number(c).format())
        )

        # class_count = class_values.size().getInfo()
        dataset = le.Image.pixelArea().divide(denominator).addBands(in_value_raster)

        init_result = dataset.reduceRegions(
            **{
                "collection": in_zone_vector,
                "reducer": le.Reducer.sum().group(
                    **{
                        "groupField": 1,
                        "groupName": "group",
                    }
        ...
        def get_keys(input_list):
            return input_list.map(
                lambda x: le.String("Class_").cat(
                    le.Number(le.Dictionary(x).get("group")).format()
                )
            )
        ...
        def get_values(input_list):
            decimal_format = "%.{}f".format(decimal_places)
            return input_list.map(
                lambda x: le.Number.parse(
                    le.Number(le.Dictionary(x).get("sum")).format(decimal_format)
                )
            )

        def set_attribute(f):
            groups = le.List(f.get("groups"))
            keys = get_keys(groups)
            values = get_values(groups)
            total_area = le.List(values).reduce(le.Reducer.sum())

            def get_class_values(x):
                cls_value = le.Algorithms.If(
                    keys.contains(x), values.get(keys.indexOf(x)), 0
                )
                cls_value = le.Algorithms.If(
                    le.String(stat_type).compareTo(le.String("SUM")),
                    le.Number(cls_value).divide(le.Number(total_area)),
                    cls_value,
                )
                return cls_value

            full_values = class_names.map(lambda x: get_class_values(x))
            attr_dict = le.Dictionary.fromLists(class_names, full_values)
...
def image_date(img, date_format="YYYY-MM-dd"):
    ...
    """
    return le.Date(img.get("system:time_start")).format(date_format)
...
def image_dates(img_col, date_format="YYYY-MM-dd"):
    ...
    new_dates = dates.map(lambda d: le.Date(d).format(date_format))
    return new_dates
...
def image_area(img, region=None, scale=None, denominator=1.0):
    ...
    pixel_area = (
        img.unmask().neq(le.Image(0)).multiply(le.Image.pixelArea()).divide(denominator)
    )
...
def image_area_by_group(
    ...
    values = []
    if region is None:
        region = le.Geometry.BBox(-179.9, -89.5, 179.9, 89.5)
...
def image_value_list(img, region=None, scale=None, return_hist=False, **kwargs):
    ...
    if region is None:
        ...
        region = le.FeatureCollection([le.Feature(geom)])
    elif isinstance(region, le.Geometry):
        region = le.FeatureCollection([le.Feature(region)])
    elif isinstance(region, le.FeatureCollection):
        pass
    else:
    ...
    reducer = le.Reducer.frequencyHistogram()
    kwargs["scale"] = scale
    kwargs["reducer"] = reducer
    kwargs["collection"] = region

    result = img.reduceRegions(**kwargs)
    hist = le.Dictionary(result.first().get("histogram"))
    if return_hist:
        return hist
    else:
        return hist.keys()
...
def image_stats_by_zone(
    ...
    **kwargs,
    ...
    elif isinstance(reducer, le.Reducer):
        pass
    else:
        raise ValueError(
            "reducer must be one of: {}".format(", ".join(allowed_stats.keys()))
    ...
    values = values.map(lambda x: le.Number.parse(x))

    def get_stats(value):
        img = image.updateMask(zones.eq(le.Number(value)))
        kwargs["reducer"] = reducer
        kwargs["scale"] = scale
        kwargs["geometry"] = region
        kwargs["bestEffort"] = bestEffort
        stat = img.reduceRegion(**kwargs)
        return le.Image().set({"zone": value}).set({"stat": stat.values().get(0)})

    collection = le.ImageCollection(values.map(lambda x: get_stats(x)))
...
def latitude_grid(step=1.0, west=-180, east=180, south=-85, north=85):
    ...
    """
    values = le.List.sequence(south, north - step, step)

    def create_feature(lat):
        return le.Feature(
            le.Geometry.BBox(west, lat, east, le.Number(lat).add(step))
        ).set(
            {
                "south": lat,
                "west": west,
                "north": le.Number(lat).add(step),
                "east": east,
            }
        )

    features = le.FeatureCollection(values.map(create_feature))
    return features
...
def longitude_grid(step=1.0, west=-180, east=180, south=-85, north=85):
    ...
    values = le.List.sequence(west, east - step, step)

    def create_feature(lon):
        return le.Feature(
            le.Geometry.BBox(lon, south, le.Number(lon).add(step), north)
        ).set(
            {
                "south": south,
                "west": lon,
                "north": north,
                "east": le.Number(lon).add(step),
            }
        )
...
def latlon_grid(lat_step=1.0, lon_step=1.0, west=-180, east=180, south=-85, north=85):
    ...
    """
    longitudes = le.List.sequence(west, east - lon_step, lon_step)
    latitudes = le.List.sequence(south, north - lat_step, lat_step)

    def create_lat_feature(lat):
        def create_lon_features(lon):
            return le.Feature(
                le.Geometry.BBox(
                    lon, lat, le.Number(lon).add(lon_step), le.Number(lat).add(lat_step)
                )
            ).set(
                {
                    "south": lat,
                    "west": lon,
                    "north": le.Number(lat).add(lat_step),
                    "east": le.Number(lon).add(lon_step),
                }
            )

        return le.FeatureCollection(longitudes.map(create_lon_features))

    return le.FeatureCollection(latitudes.map(create_lat_feature)).flatten()
...
def fishnet(
    ...
    **kwargs,
    ...
    if isinstance(data, le.FeatureCollection) or isinstance(data, le.Feature):
        data = data.geometry()
    elif isinstance(data, le.Geometry):
        pass
    else:
        raise ValueError(
...
def extract_values_to_points(
    ...
    **kwargs,
    ...
    if not isinstance(in_fc, le.FeatureCollection):
        try:
...
def bands_to_image_collection(img):
    ...
    """
    collection = le.ImageCollection(img.bandNames().map(lambda b: img.select([b])))
    return collection
...
def find_landsat_by_path_row(landsat_col, path_num, row_num):
    ...
    try:
        if isinstance(landsat_col, str):
            landsat_col = le.ImageCollection(landsat_col)
            collection = landsat_col.filter(le.Filter.eq("WRS_PATH", path_num)).filter(
                le.Filter.eq("WRS_ROW", row_num)
            )
            return collection
    except Exception as e:
        print(e)
...
def array_sum(arr):
    ...
    Returns:
        object: ee.Number
    """
    return le.Array(arr).accum(0).get([-1])
...
def array_mean(arr):
    ...
    Returns:
        object: ee.Number
    """
    total = le.Array(arr).accum(0).get([-1])
    size = arr.length()
    return le.Number(total.divide(size))
...
def get_annual_NAIP(year, RGBN=True):
    ...
    """
    try:
        collection = le.ImageCollection("USDA/NAIP/DOQQ")
        start_date = str(year) + "-01-01"
        end_date = str(year) + "-12-31"
        naip = collection.filterDate(start_date, end_date)
        if RGBN:
            naip = naip.filter(le.Filter.listContains("system:band_names", "N"))
        return naip
    except Exception as e:
        print(e)
...
def get_all_NAIP(start_year=2009, end_year=2019):
    ...
    try:
        ...
        def get_annual_NAIP(year):
            try:
                collection = le.ImageCollection("USDA/NAIP/DOQQ")
                start_date = le.Date.fromYMD(year, 1, 1)
                end_date = le.Date.fromYMD(year, 12, 31)
                naip = collection.filterDate(start_date, end_date).filter(
                    le.Filter.listContains("system:band_names", "N")
                )
                return le.ImageCollection(naip)
            except Exception as e:
                print(e)

        years = le.List.sequence(start_year, end_year)
...
def annual_NAIP(year, region):
    ...
    end_date = le.Date.fromYMD(year, 12, 31)
    collection = (
        le.ImageCollection("USDA/NAIP/DOQQ")
        .filterDate(start_date, end_date)
        .filterBounds(region)
    )

    time_start = le.Date(
        le.List(collection.aggregate_array("system:time_start")).sort().get(0)
    )
    time_end = le.Date(
        le.List(collection.aggregate_array("system:time_end")).sort().get(-1)
    )
    image = le.Image(collection.mosaic().clip(region))
    NDWI = le.Image(image).normalizedDifference(["G", "N"]).select(["nd"], ["ndwi"])
    NDVI = le.Image(image).normalizedDifference(["N", "R"]).select(["nd"], ["ndvi"])
...
def find_NAIP(region, add_NDVI=True, add_NDWI=True):
    ...
    init_collection = (
        ...
        .filter(le.Filter.listContains("system:band_names", "N"))
    )
    ...
    init_years = yearList.map(lambda y: le.Date(y).get("year"))

    # remove duplicates
    init_years = le.Dictionary(
        init_years.reduce(le.Reducer.frequencyHistogram())
    ).keys()
    years = init_years.map(lambda x: le.Number.parse(x))
    ...
    naip = le.ImageCollection(years.map(NAIPAnnual))
    mean_size = le.Number(naip.aggregate_mean("tiles"))
    total_sd = le.Number(naip.aggregate_total_sd("tiles"))
    threshold = mean_size.subtract(total_sd.multiply(1))
    naip = naip.filter(
        le.Filter.Or(le.Filter.gte("tiles", threshold), le.Filter.gte("tiles", 15))
    )
    naip = naip.filter(le.Filter.gte("tiles", 7))

    naip_count = naip.size()
    naip_seq = le.List.sequence(0, naip_count.subtract(1))

    def set_index(index):
        img = le.Image(naip.toList(naip_count).get(index))
        return img.set({"system:uid": le.Number(index).toUint8()})
...
def filter_NWI(HUC08_Id, region, exclude_riverine=True):
    ...
    nwi_huc = le.FeatureCollection(nwi_asset_path).filterBounds(region)

    if exclude_riverine:
        nwi_huc = nwi_huc.filter(
            le.Filter.notEquals(**{"leftField": "WETLAND_TY", "rightValue": "Riverine"})
        )
    return nwi_huc
...
def filter_HUC08(region):
    ...
    USGS_HUC08 = le.FeatureCollection("USGS/WBD/2017/HUC08")  # Subbasins
...
def filter_HUC10(region):
    ...
    USGS_HUC10 = le.FeatureCollection("USGS/WBD/2017/HUC10")  # Watersheds
...
def find_HUC08(HUC08_Id):
    ...
    USGS_HUC08 = le.FeatureCollection("USGS/WBD/2017/HUC08")  # Subbasins
    HUC08 = USGS_HUC08.filter(le.Filter.eq("huc8", HUC08_Id))
    return HUC08
...
def find_HUC10(HUC10_Id):
    ...
    USGS_HUC10 = le.FeatureCollection("USGS/WBD/2017/HUC10")  # Watersheds
    HUC10 = USGS_HUC10.filter(le.Filter.eq("huc10", HUC10_Id))
    return HUC10
...
def find_NWI(HUC08_Id, exclude_riverine=True):
    ...
    nwi_asset_suffix = "_Wetlands"
    nwi_asset_path = nwi_asset_prefix + HUC08_Id + nwi_asset_suffix
    nwi_huc = le.FeatureCollection(nwi_asset_path)
    if exclude_riverine:
        nwi_huc = nwi_huc.filter(
            le.Filter.notEquals(**{"leftField": "WETLAND_TY", "rightValue": "Riverine"})
        )
...
def nwi_add_color(fc):
    ...
    Returns:
        object: ee.Image
    """
    emergent = le.FeatureCollection(
        fc.filter(le.Filter.eq("WETLAND_TY", "Freshwater Emergent Wetland"))
    )
    emergent = emergent.map(lambda f: f.set("R", 127).set("G", 195).set("B", 28))
    # print(emergent.first())

    forested = fc.filter(
        le.Filter.eq("WETLAND_TY", "Freshwater Forested/Shrub Wetland")
    )
    forested = forested.map(lambda f: f.set("R", 0).set("G", 136).set("B", 55))

    pond = fc.filter(le.Filter.eq("WETLAND_TY", "Freshwater Pond"))
    pond = pond.map(lambda f: f.set("R", 104).set("G", 140).set("B", 192))

    lake = fc.filter(le.Filter.eq("WETLAND_TY", "Lake"))
    lake = lake.map(lambda f: f.set("R", 19).set("G", 0).set("B", 124))

    riverine = fc.filter(le.Filter.eq("WETLAND_TY", "Riverine"))
    riverine = riverine.map(lambda f: f.set("R", 1).set("G", 144).set("B", 191))

    fc = le.FeatureCollection(
...
def nwi_rename(names):
    ...
    new_names = le.List(names).map(lambda name: name_dict.get(name))
    return new_names
...
def summarize_by_group(
    ...
    results = le.List(le.Dictionary(stats).get("groups"))
    if return_dict:
        keys = results.map(lambda k: le.Dictionary(k).get(group_name))
        values = results.map(lambda v: le.Dictionary(v).get(stats_type))
        results = le.Dictionary.fromLists(keys, values)
...
def ee_num_round(num, decimal=2):
    ...
    return le.Number.parse(le.Number(num).format(format_str))
...
def vector_styling(
    ...
    if isinstance(ee_object, le.FeatureCollection):
        prop_names = le.Feature(ee_object.first()).propertyNames().getInfo()
            ...
            lambda f: f.set(
                {
                    "style": {
                        ...
                        "fillColor": colors.get(
                            le.Number(
                                le.Number(f.get("styleIndex")).divide(step)
                            ).floor()
                        ),
                    }
                }
...
def ee_to_df(
    ...
    **kwargs,
    ...
    Returns:
        pd.DataFrame: pandas DataFrame
    """
    if isinstance(ee_object, le.Feature):
        ee_object = le.FeatureCollection([ee_object])
    ...
    try:
        if remove_geom:
            data = ee_object.map(
                lambda f: le.Feature(None, f.toDictionary(f.propertyNames().sort()))
            )
        else:
            data = ee_object
        ...
        df = le.data.computeFeatures(kwargs)
...
def ee_to_gdf(
    ...
    **kwargs,
    ...
    try:
        ...
        gdf = le.data.computeFeatures(kwargs)
    ...
    if date is not None:
        try:
            fc = fc.map(
                lambda x: x.set(
                    "system:time_start",
                    le.Date.parse(date_format, x.get(date)).millis(),
                )
            )
        except Exception as e:
            raise Exception(e)
...
def extract_transect(
    ...
    **kwargs,
    ...
    try:
        ...
        distances = le.List.sequence(0, length, dist_interval)
        lines = line.cutLines(distances, maxError).geometries()

        def set_dist_attr(l):
            l = le.List(l)
            geom = le.Geometry(l.get(0))
            distance = le.Number(l.get(1))
            geom = le.Geometry.LineString(geom.coordinates())
            return le.Feature(geom, {"distance": distance})

        lines = lines.zip(distances).map(set_dist_attr)
        lines = le.FeatureCollection(lines)

        transect = image.reduceRegions(
            **{
                "collection": le.FeatureCollection(lines),
                "reducer": reducer,
                "scale": scale,
                "crs": crs,
                "crsTransform": crsTransform,
...
def create_contours(
    ...
    if region is not None:
        if isinstance(region, le.FeatureCollection) or isinstance(region, le.Geometry):
            pass
        else:
            raise TypeError(
    ...
    if kernel is None:
        kernel = le.Kernel.gaussian(5, 3)

    if isinstance(values, list):
        values = le.List(values)
    elif isinstance(values, le.List):
        pass

    if values is None:
        values = le.List.sequence(min_value, max_value, interval)

    def contouring(value):
        mycountour = (
            image.convolve(kernel)
            .subtract(le.Image.constant(value))
            .zeroCrossing()
            .multiply(le.Image.constant(value).toFloat())
        )
    ...
    if region is not None:
        if isinstance(region, le.FeatureCollection):
            return le.ImageCollection(contours).mosaic().clipToCollection(region)
        elif isinstance(region, le.Geometry):
            return le.ImageCollection(contours).mosaic().clip(region)
    else:
        return le.ImageCollection(contours).mosaic()
...
def vector_centroids(ee_object):
    ...
    centroids = ee_object.map(
        lambda f: le.Feature(f.geometry().centroid(0.001), f.toDictionary())
    )
...
def ee_join_table(ee_object, data, src_key, dst_key=None):
    ...
    table = le.Dictionary(df.to_dict("index"))

    fc = ee_object.map(lambda f: f.set(table.get(f.get(src_key), le.Dictionary())))
    return fc
...
def view_lidar(filename, cmap="terrain", backend="pyvista", background=None, **kwargs):
    ...
    if backend in ["pyvista", "ipygany", "panel"]:
        try:
            ...
            print(
                "The pyvista and pyntcloud packages are required for this function. Use pip install le_map_tools[lidar] to install them."
            )
            return
...
def blend(
    ...
    **kwargs,
    ...
    if not isinstance(top_layer, le.Image):
        raise ValueError("top_layer must be an ee.Image.")

    if bottom_layer is None:
        bottom_layer = top_layer

    if not isinstance(bottom_layer, le.Image):
    ...
    if hillshade:
        bottom = le.Terrain.hillshade(bottom_layer).visualize(**bottom_vis).divide(255)
    else:
    ...
    result = le.Image().expression(expression, {"a": top, "b": bottom})
    return result
...
def clip_image(image, mask, output):
    ...
    if isinstance(mask, le.Geometry):
...
def image_count(
    ...
    if region is not None:
        if isinstance(region, le.Geometry) or isinstance(region, le.FeatureCollection):
            pass
        else:
    ...
    image = (
        collection.filter(le.Filter.listContains("system:band_names", band))
        .select([band])
        .reduce(le.Reducer.count())
    )
...
def dynamic_world(
    ...
    if reducer is None:
        reducer = le.Reducer.mode()

    dw = le.ImageCollection("GOOGLE/DYNAMICWORLD/V1").filter(
        le.Filter.date(start_date, end_date)
    )
    ...
    if clip and (region is not None):
        ...
        elif isinstance(region, le.Feature):
    ...
    if return_type == "class":
        ...
        meanProbability = probabilityCol.reduce(le.Reducer.mean())
        ...
        proj = le.Projection(projection).atScale(scale)
        meanProbability = meanProbability.setDefaultProjection(proj)

        # Create the Top1 Probability Hillshade
        top1Probability = meanProbability.reduce(le.Reducer.max())
        ...
        if return_type == "probability":
            ...
            hillshade = le.Terrain.hillshade(top1Confidence).divide(255)
...
def dynamic_world_s2(
    ...
    """
    s2 = (
        le.ImageCollection("COPERNICUS/S2_HARMONIZED")
        .filterDate(start_date, end_date)
        .filter(le.Filter.lt("CLOUDY_PIXEL_PERCENTAGE", cloud_pct * 100))
    )
...
def download_ee_image_tiles(
    ...
    **kwargs,
    ...
    if not isinstance(features, le.FeatureCollection):
    ...
    for i in range(count):
        region = le.Feature(collection.get(i)).geometry()
...
def download_ee_image_tiles_parallel(
    ...
    **kwargs,
    ...
    def download_data(index):
        ...
        region = le.Feature(collection.get(index)).geometry()
        filename = os.path.join(
...
def download_ee_image_collection(
    ...
    **kwargs,
    ...
    try:
        ...
        for i in range(0, count):
            image = le.Image(collection.toList(count).get(i))
            if filenames is not None:
                name = filenames[i]
...
def bbox_coords(geometry, decimals=4):
    ...
    """
    if isinstance(geometry, le.FeatureCollection):
        geometry = geometry.geometry()

    if geometry is not None:
        if not isinstance(geometry, le.Geometry):
...
def requireJS(lib_path=None, Map=None):
    ...
    Args:
        ...
        Map (le_map_tools.Map, optional): An le_map_tools.Map object. Defaults to None.
...
def ee_vector_style(
    ...
    prop_names = le.Feature(collection.first()).propertyNames().getInfo()
    if column not in prop_names:
        raise ValueError(
    ...
    if labels is None:
        ...
        collection = collection.filter(le.Filter.inList(column, labels))
    ...
    style = le.Dictionary(style_dict)
...
def jrc_hist_monthly_history(
    ...
    **kwargs,
    ...
    if collection is None:
        collection = le.ImageCollection("JRC/GSW1_4/MonthlyHistory")
    ...
    images = (
        ...
        .filter(le.Filter.calendarRange(start_month, end_month, "month"))
        .map(lambda img: img.eq(2).selfMask())
    )

    def cal_area(img):
        pixel_area = img.multiply(le.Image.pixelArea()).divide(denominator)
        img_area = pixel_area.reduceRegion(
            **{
                "geometry": region,
                "reducer": le.Reducer.sum(),
...
def image_convolution(
    ...
    """
    if isinstance(image, le.ImageCollection):
        image = image.mosaic()
    elif not isinstance(image, le.Image):
        raise ValueError("image must be an ee.Image or ee.ImageCollection.")

    if kernel is None:
        kernel = le.Kernel.gaussian(radius=3, sigma=2, units="pixels", normalize=True)
    elif not isinstance(kernel, le.Kernel):
...
def download_3dep_lidar(region, filename, scale=1.0, crs="EPSG:3857"):
    dataset = le.ImageCollection("USGS/3DEP/1m")

    if isinstance(region, le.Geometry) or isinstance(region, le.Feature):
        region = le.FeatureCollection([region])
...
def ee_to_geotiff(
    ...
    **kwargs,
    ...
    if (
        not isinstance(ee_object, le.Image)
        and not isinstance(ee_object, le.ImageCollection)
        and not isinstance(ee_object, le.FeatureCollection)
        and not isinstance(ee_object, le.Feature)
        and not isinstance(ee_object, le.Geometry)
    ):
        err_str = "\n\nThe image argument in 'addLayer' function must be an instance of one of ee.Image, ee.Geometry, ee.Feature or ee.FeatureCollection."
        raise AttributeError(err_str)

    if (
        isinstance(ee_object, le.geometry.Geometry)
        or isinstance(ee_object, le.feature.Feature)
        or isinstance(ee_object, le.featurecollection.FeatureCollection)
    ):
        features = le.FeatureCollection(ee_object)
    ...
    elif isinstance(ee_object, le.image.Image):
        image = ee_object
    elif isinstance(ee_object, le.imagecollection.ImageCollection):
    ...
    map_id_dict = le.Image(image).getMapId(vis_params)
...
def create_grid(ee_object, scale, proj=None):
    ...
    if isinstance(ee_object, le.FeatureCollection) or isinstance(ee_object, le.Image):
        geometry = ee_object.geometry()
    elif isinstance(ee_object, le.Geometry):
        geometry = ee_object
    else:
