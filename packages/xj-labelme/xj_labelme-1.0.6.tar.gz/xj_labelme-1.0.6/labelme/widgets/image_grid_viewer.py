...
class ImageCard(QtWidgets.QWidget):
    ...
    def __init__(self, filepath: str, has_annotation: bool, label_file_path: str | None = None, parent=None):
        ...
        layout.setContentsMargins(8, 8, 8, 8)  # 留出边框空间
        layout.setSpacing(5)
        ...
        self.thumbnail_label.setStyleSheet("background: #f5f5f5;")
    ...
    def paintEvent(self, event):
        ...
        if self._is_selected:
            # 选中状态：蓝色粗边框
            pen = QtGui.QPen(QtGui.QColor(33, 150, 243), 4)  # 蓝色 #2196F3
            painter.setPen(pen)
            painter.drawRect(2, 2, self.width() - 4, self.height() - 4)
        else:
            # 未选中状态：灰色细边框
            pen = QtGui.QPen(QtGui.QColor(224, 224, 224), 1)  # 灰色 #e0e0e0
            painter.setPen(pen)
    ...
    def _update_style(self):
        ...
        if self._is_selected:
            ...
            self.thumbnail_label.setStyleSheet(
                "border: 3px solid #2196F3; background: #e3f2fd; border-radius: 4px;"
            )
            self.setStyleSheet("background: #e3f2fd; border-radius: 6px;")
        else:
            # 未选中状态
            self.thumbnail_label.setStyleSheet(
                "border: 1px solid #e0e0e0; background: #f5f5f5; border-radius: 4px;"
            )
...
class ImageGridViewer(QtWidgets.QWidget):
    ...
    def __init__(
        ...
        self._selected_filepath = None  # 当前选中的文件路径（用路径而非卡片引用）
    ...
    def _load_images(self):
        ...
        for index, filepath in enumerate(filtered_images):
            ...
            # 恢复选中状态
            if self._selected_filepath and filepath == self._selected_filepath:
    ...
    def _on_card_clicked(self, filepath: str):
        """卡片被点击"""
        # 取消之前选中的卡片
        for card, card_filepath in self._cached_cards:
            if card_filepath == self._selected_filepath:
        ...
        # 找到并选中新卡片
        for card, card_filepath in self._cached_cards:
            if card_filepath == filepath:
                card.set_selected(True)
                break
        
        # 发送信号
        self.imageClicked.emit(filepath)
    
    def set_selected_image(self, filepath: str):
        """从外部设置选中的图片（用于同步主界面的选择）"""
