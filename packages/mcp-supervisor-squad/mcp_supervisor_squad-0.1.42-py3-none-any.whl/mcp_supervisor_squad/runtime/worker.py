...
from ..persistence.kb import read_director_report
...
class Worker:
    ...
    def _run(self, run_id: str, repo_root_hint: str | None = None) -> None:
        ...
        # Planner phase: read director report (if any) and produce requirements.
        ...
        if difficulty in {"medium", "hard"}:
            try:
                ...
                update(
                    ...
                    next_action="wait",
                    wait_reason="error",
                    important_event="planner(轻量) 失败",
                    important_event_at=utc_now_iso(),
                    event={"kind": "error", "message": f"planner(轻量) 异常: {str(exc)[:200]}"},
                )
                return
            ...
            try:
                ...
                query_line = (state.query.strip() or "(empty query)")[:200]
                lines = [
                ...
                pass
            # 压缩版摘要用于事件行，防止撑爆 event_lines
            short_lines = [
                f"需求概述: {query_line}",
                f"目标范围: {scope}/**，保持改动最小",
                "预期交付: 简明介绍 + coder 产出 requirements/补丁",
                "提醒: 避免兼容代码，沿用现有工具链，日志简短",
                summary,
            ]
            update(
                ...
                event={
                    "kind": "light_summary",
                    "message": "Planner Light 摘要",
                    "data_lines": [ln for ln in short_lines if ln.strip()],
                },
            )
        else:
            update(
        ...
        # Requirements generation removed: pass query directly to coder.
        ...
        for slug, project_root_str in task_specs:
            ...
            prompt = self._build_coder_prompt(requirements_text=state.query.strip() or "")
        ...
        try:
            ...
            # KB/compact generation removed for simplicity.
        except Exception:
            update(
                phase="error",
                next_action="wait",
        ...
        # Prepare worktrees and run coder tasks concurrently (direct write, no patch aggregation).
        task_threads: list[threading.Thread] = []
        ...
        task_specs = [(t.slug, t.project_root) for t in (state.tasks or [])]
        for slug, _project_root_str in task_specs:
            worktree_path = repo_root
            requirements_hint = (
        ...
        # Direct-write mode: mark done after tasks finish.
        update(phase="done", next_action="done", wait_reason="completed", important_event="coder 已完成并写入代码", important_event_at=utc_now_iso(), event={"kind": "info", "message": "coder 已完成并写入代码"})

    def _build_coder_prompt(self, *, requirements_text: str) -> str:
        parts = [
            ...
            f"- User query: {requirements_text[:2000]}",
        ]
        parts.append("Apply changes directly to files. No patch output. No Markdown fences.")
        return "\n".join(parts) + "\n"

    def apply(self, state: RunState, *, options: dict[str, Any]) -> dict[str, Any]:
        """
        Direct-write mode: apply is a no-op; changes already written by coder.
        """
        ...
        base["forced_wait_s"] = 0.0
        base["applied"] = True
        base["dry_run"] = bool(options.get("dry_run", True))
        base["summary"] = ["已直接写入代码，无需 apply"]
        return base
