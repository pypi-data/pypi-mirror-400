...
class AggregateLoader(DataConfig):
    ...
    cache_resolved_results = Value(True, isflag=True, help=ub.paragraph(
        ...
        changed. Note, that when set to False, the cache file will still
        be written, so subsequent loads with the value True will use the new
        results.
        '''))
...
class AggregateEvluationConfig(AggregateLoader):
    ...
    query = Value(None, type=str, help=ub.paragraph(
        ...
        The same query is run for each aggregator.
        Note, the `--embed` and `--snapshot` commands can help interactively
        determine the correct query, but some knowledge of code structure
        is currently needed. Look at the `if config.query` block.
        '''
    ))
...
def run_aggregate(config):
    ...
    if config.query:
        ...
        for key, agg in eval_type_to_aggregator.items():
            ...
            # Use new query logic
            new_index = query_plan.apply(agg.table, node=key).index
            new_agg = agg.filterto(index=new_index)
    ...
    def report_best(agg, top_k=100, shorten=True, per_group=None, verbose=1,
        ...
        Args:
            ...
            concise (bool | str):
                ...
                more concise report. Can also be set to 'split', which will
                print the bigger table in multiple vertical sections.
        ...
        if verbose:
            ...
            if only_one_top_item and len(region_id_to_summary):
                ...
                rich.print(f'Top results for {agg.node_type}')
                ...
                if concise:
                    if concise == 'split':
                        try:
                            _justone_split = _justone[['fpath', 'param_hashid']]
                        except KeyError:
                            ...
                        else:
                            rich.print(_justone_split.to_string(index=False))

                    _justone = _justone.safe_drop(['node'], axis=1)
                    _justone = _justone.safe_drop(['fpath'], axis=1)

                # TODO: add the rich links to this too.
                # TODO: consolidate code between the only one top item  and
                # more than one top item cases.
                rich.print(_justone.to_string(index=False))
                ...
                for region_id, summary_table in region_id_to_summary.items():
                    ...
                    if concise:
                        if concise == 'split':
                            # If concise is split, we want to to print the
                            # paths, but use vertical space.
                            try:
                                _split_path_table = _summary_table[['fpath', 'param_hashid']]
                                vertical_tables.append(_split_path_table)
                            except KeyError:
                                ...
                    ...
                    vertical_tables.append(_summary_table)

                    _parts = []
                    for _table in vertical_tables:
                        _parts.append(_table.iloc[::-1].to_string(index=False))
                    text = '\n'.join(_parts)
...
class _AggregatorDeprecatedMixin:
    ...
    def params(self):
        ub.schedule_deprecation(
            modname='kwdagger', name='params', type='property',
    ...
    def hashid_to_params(self):
        ub.schedule_deprecation(
            modname='kwdagger', name='hashid_to_params', type='property',
    ...
    def type(self):
        ub.schedule_deprecation(
            modname='kwdagger', name='type', type='property',
...
class Aggregator(ub.NiceRepr, AggregatorAnalysisMixin, _AggregatorDeprecatedMixin):
    ...
    def _build_metrics_column_preferences(agg):
        ...
        for info in agg._metric_info.values():
            ...
            # TODO: objectives other than minimize / maximize should be allowed
            # perhaps "contextual" for things like threshold, where we dont
            # really want to make it bigger or smaller. I suppose that makes
            # these things not metrics anymore.
    ...
    def filterto(agg, index=None, models=None, param_hashids=None, query=None):
        ...
        def our_hacky_query(df, query):
            ...
            # TODO: also support agg.table as a variable.
            new_table = None
            if 'df[' in query:
...
def aggregate_param_cols(df, aggregator=None, hash_cols=None, allow_nonuniform=False):
    ...
    Args:
        aggregator (None | Dict[str, str]):
            mapping from column name to how it should be aggregated.

        hash_cols (None | List[str]):
            columns whos values should be hashed together.

        allow_nonuniform (bool):
            if True, drop non-uniform columns instead of raising an error.
    ...
    Example:
        ...
        >>> print(agg_row)
        node                                              demo_node
        region_id                                   macro_09_c4bfdb
        machine.demo_node.host                                  pc1
        params.demo_node.test_dataset               macro_09_6bdaa8
        metrics.demo_node.metric1                        254.236979
        specified.params.demo_node.param1                         1
        specified.params.demo_node.param2                         1
        specified.params.demo_node.param3                         1
        specified.params.demo_node.test_dataset                   1
    ...
    if len(df) == 1:
        ...
        is_safe_cols = find_uniform_columns(df_comparable)

        nonuniform_cols = list(is_safe_cols[~is_safe_cols].index)
        if allow_nonuniform:
...
def find_uniform_columns(df_comparable):
    ...
    Example:
        ...
        >>> # --- 1) Single row: trivially uniform for all columns ---
        >>> df = pd.DataFrame({'a': [1], 'b': [np.nan], 'c': ['x']})
        >>> find_uniform_columns(df).to_dict() == {'a': True, 'b': True, 'c': True}
        True

        >>> # --- 2) Numeric columns: all-same, mixed-NaN, all-NaN ---
        >>> df = pd.DataFrame({
        ...     'same_int':   [1, 1, 1, 1],
        ...     'mixed_int':  [1, 2, 1, 2],
        ...     'same_float': [3.14, 3.14, 3.14, 3.14],
        ...     'all_nan':    [np.nan, np.nan, np.nan, np.nan],
        ...     'mixed_nan':  [np.nan, 7.0, np.nan, 7.0],
        ... })
        >>> res = find_uniform_columns(df).to_dict()
        >>> res['same_int'], res['mixed_int'], res['same_float'], res['all_nan'], res['mixed_nan']
        (True, False, True, True, False)

        >>> # --- 3) Datetime columns: same timestamp, all-NaT, mixed NaT/timestamp ---
        >>> ts = datetime(2020, 1, 1, 12, 0, 0)
        >>> df = pd.DataFrame({
        ...     'same_dt':   [ts, ts, ts],
        ...     'all_nat':   [pd.NaT, pd.NaT, pd.NaT],
        ...     'mixed_dt':  [pd.NaT, ts, pd.NaT],
        ... })
        >>> res = find_uniform_columns(df).to_dict()
        >>> res['same_dt'], res['all_nat'], res['mixed_dt']
        (True, True, False)

        >>> # --- 4) Object/string columns: all-same vs mixed ---
        >>> df = pd.DataFrame({
        ...     'same_str':  ['x', 'x', 'x'],
        ...     'mixed_str': ['x', 'y', 'x'],
        ...     'same_obj':  [object(), object(), object()],  # different objects
        ... })
        >>> # Note: different object() instances are NOT equal, so 'same_obj' is False
        >>> res = find_uniform_columns(df).to_dict()
        >>> res['same_str'], res['mixed_str'], res['same_obj']
        (True, False, False)

        >>> # --- 5) Unhashables: lists/dicts (fallback path) ---
        >>> df = pd.DataFrame({
        ...     'same_list': [[1, 2], [1, 2], [1, 2]],
        ...     'mixed_list': [[1], [1, 2], [1]],
        ...     'same_dict': [{'a': 1}, {'a': 1}, {'a': 1}],
        ...     'mixed_dict': [{'a': 1}, {'a': 2}, {'a': 1}],
        ... })
        >>> res = find_uniform_columns(df).to_dict()
        >>> res['same_list'], res['mixed_list'], res['same_dict'], res['mixed_dict']
        (True, False, True, False)

        >>> # --- 5b) Lists with a NaN thrown in (should be non-uniform) ---
        >>> df = pd.DataFrame({'mix_list_nan': [[1, 2], [1, 2], np.nan]})
        >>> find_uniform_columns(df).to_dict()['mix_list_nan']
        False

        >>> # Lists with different lengths are not uniform (previous failure case)
        >>> df = pd.DataFrame({'vary_len': [[1, 2], [1], [1, 2]]})
        >>> find_uniform_columns(df).to_dict()['vary_len']
        False

        >>> # Ndarray equality with NaN inside (uses array_equal(equal_nan=True))
        >>> import numpy as np
        >>> df = pd.DataFrame({'arrs': [np.array([1.0, np.nan]), np.array([1.0, np.nan])]})
        >>> find_uniform_columns(df).to_dict()['arrs']
        True

        >>> # --- 6) Mixed dtypes together (sanity check) ---
        >>> df = pd.DataFrame({
        ...     'num_same':  [10, 10, 10],
        ...     'num_mixed': [0, 1, 0],
        ...     'str_same':  ['a', 'a', 'a'],
        ...
        ...     'nan_mixed': [np.nan, 5.0, np.nan],
        ...     'dt_same':   [ts, ts, ts],
        ...     'dt_mixed':  [pd.NaT, ts, pd.NaT],
        ...     'list_same': [[1], [1], [1]],
        ...     'list_mixed': [[1], [2], [1]],
        ... })
        >>> res = find_uniform_columns(df).to_dict()
        >>> [res[k] for k in ['num_same','num_mixed','str_same','str_mixed','all_nan','nan_mixed','dt_same','dt_mixed','list_same','list_mixed']]
    ...
    Benchmark:
        ...
        >>> # all nan column
        >>> df_comparable.loc[:, 'col_000'] = float('nan')
        >>> # mostly nan column
        >>> df_comparable.loc[0::2, 'col_001'] = float('nan')
        >>> df_comparable.loc[1::2, 'col_002'] = float('nan')
        >>> df_comparable.loc[1::2, 'col_001'] = 34
        >>> df_comparable.loc[0::2, 'col_002'] = 12
        >>> df_comparable.loc[:, 'col_003'] = 321.22
        >>> df_comparable.loc[:, 'col_004'] = 123
        >>> # Benchmark
        >>> import timerit
        >>> ti = timerit.Timerit(100, bestof=10, verbose=2, unit='ms')
        >>> for timer in ti.reset('pandas: nunique'):
        >>>     with timer:
        >>>         _is_safe_cols2 = (df_comparable.nunique(dropna=False) <= 1)
        >>> for timer in ti.reset('ubelt: allsame'):
        >>>     with timer:
        >>>         is_safe_cols1 = {
        >>>             k: ub.allsame(vs, eq=nan_eq)
        >>>             for k, vs in df_comparable.T.iterrows()}
        >>> for timer in ti.reset('GPT5: find_uniform_columns'):
        >>>     with timer:
        >>>         _is_safe_cols3 = find_uniform_columns(df_comparable)
        >>> is_safe_cols2 = _is_safe_cols2.to_dict()
        >>> is_safe_cols3 = _is_safe_cols3.to_dict()
        >>> uniform_cols1 = {k: v for k, v in is_safe_cols1.items() if v}
        >>> uniform_cols2 = {k: v for k, v in is_safe_cols2.items() if v}
        >>> uniform_cols3 = {k: v for k, v in is_safe_cols3.items() if v}
        >>> assert uniform_cols1 == uniform_cols2
        >>> assert uniform_cols1 == uniform_cols3
        Timed pandas: nunique for: 100 loops, best of 10
            time per loop: best=5.015 ms, mean=5.053 ± 0.0 ms
        Timed ubelt: allsame for: 100 loops, best of 10
            time per loop: best=1.534 ms, mean=1.556 ± 0.0 ms
        Timed GPT5: find_uniform_columns for: 100 loops, best of 10
            time per loop: best=0.521 ms, mean=0.533 ± 0.0 ms
    """
    import numpy as np
    import pandas as pd

    if df_comparable.shape[0] <= 1:
        # 0 or 1 row -> trivially uniform
        return pd.Series(True, index=df_comparable.columns)

    cols = df_comparable.columns

    # --- Fast path: numeric-like (includes ints, floats, bools) ---
    num = df_comparable.select_dtypes(include=["number", "bool"])
    mask_num = pd.Series(False, index=num.columns)
    if not num.empty:
        a = num.to_numpy(copy=False)               # (nrows, ncols)
        first = a[0:1, :]                          # broadcastable (1, ncols)
        eq = (a == first)                          # elementwise equality
        # fix NaN semantics: consider NaN==NaN
        if np.issubdtype(a.dtype, np.floating):
            eq |= (np.isnan(a) & np.isnan(first))
        # reduce over rows: uniform if all rows equal the first (after NaN fix)
        mask_num.loc[:] = eq.all(axis=0)

    # --- Fast path: datetimes ---
    dtn = df_comparable.select_dtypes(include=["datetime64[ns]", "datetimetz"])
    mask_dtn = pd.Series(False, index=dtn.columns)
    if not dtn.empty:
        a = dtn.to_numpy(copy=False)
        first = a[0:1, :]
        eq = (a == first)
        # NaT behaves like NaN; this handles it the same way:
        # numpy doesn't have isnat on object, but dt64 array comparisons propagate False on NaT==NaT,
        # so repair it with isna check via pandas:
        eq |= (pd.isna(dtn).to_numpy() & pd.isna(dtn.iloc[0]).to_numpy())
        mask_dtn.loc[:] = eq.all(axis=0)

    # --- Object & the rest: try fast factorize, then safe fallback ---
    other = df_comparable.drop(columns=list(mask_num.index) + list(mask_dtn.index), errors="ignore")
    mask_other = pd.Series(False, index=other.columns)
    for c in other.columns:
        s = other[c]
        try:
            # Fast path for hashables; NaN gets a single code via sentinel
            codes, _ = pd.factorize(s, use_na_sentinel=True)
            mask_other[c] = (len(np.unique(codes)) <= 1)
        except TypeError:
            # Unhashables (lists/dicts/sets/ndarrays) -> safe fallback.

            first_val = s.iloc[0]

            # Only treat as "scalar-NaN" when not list-like; otherwise don't call isna
            is_listlike = isinstance(first_val, (list, tuple, dict, set, np.ndarray))
            if (not is_listlike) and pd.isna(first_val):
                # All-NaN (scalar NaN) is uniform; mixed NaN/non-NaN is not.
                mask_other[c] = s.isna().all()
            else:
                ...
                def _eq_obj(a, b=first_val):
                    ...
                    if isinstance(a, np.ndarray) or isinstance(b, np.ndarray):
                        try:
                            ...
                            return False
                    # Everything else: Python equality
                    return a == b

                eq = s.map(_eq_obj)
                mask_other[c] = bool(eq.all())

    # Combine
    mask = pd.concat([mask_num, mask_dtn, mask_other]).reindex(cols, fill_value=False)
    return mask
