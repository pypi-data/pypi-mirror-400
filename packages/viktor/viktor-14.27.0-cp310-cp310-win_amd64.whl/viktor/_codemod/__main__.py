...
def _fix(root_dir: Path, upgrade_id: int, apply: bool, keep_original: bool, patched_files: list):
    ...
    python_file_paths = _python_file_paths(root_dir / 'app')
    if os.path.exists(root_dir / "app.py"):
        python_file_paths = itertools.chain([Path(root_dir) / "app.py"], python_file_paths)

    for path_ in python_file_paths:
