...
class CommandPaySavedCardEnrollment(DataTransferObject):
    """Command to pay with saved card (CIT) - Step 1: Enrollment.

    Customer Initiated Transaction - customer must be present to enter CVV.
    """
...
class ResponsePaySavedCardEnrollment(DataTransferObject):
    """Response from pay with saved card enrollment.

    If challenge_required=True, show step_up_url iframe to customer,
    then call PaySavedCardValidation with auth_transaction_id.
    """
...
@cybersource.feature(CommandPaySavedCardEnrollment)
class PaySavedCardEnrollment(Feature):
    """Pay with saved card (CIT) - Step 1: Enrollment with 3DS."""

    def execute(self, dto: CommandPaySavedCardEnrollment) -> ResponsePaySavedCardEnrollment:
        ...
        response = self.payment_adapter.pay_with_saved_card_enrollment(
        ...
        if isinstance(response, PayerAuthenticationResponse):
            return ResponsePaySavedCardEnrollment(
