...
from kodexa_document.model.objects import ContentObject, Store, Taxonomy, BaseEvent
from kodexa_document.platform.client import DocumentStoreEndpoint
import logging
...
class PipelineStep:
    """Representation of a step in a pipeline."""

    def __init__(self, step, name=None, step_type="ACTION", options=None):
        self.step = step
        self.name = name
...
class Pipeline:
    """Minimal pipeline class for assistant option handling."""

    def __init__(self):
        self.steps = []

    def add_step(self, step, name=None, step_type="ACTION", options=None):
        """Add a step to the pipeline."""
        self.steps.append(PipelineStep(step, name=name, step_type=step_type, options=options))
            ...
            if store_ref != "" and document_family_id != "undefined":
                try:
                    ...
                    if cache_key not in cache:
                        ...
                        from kodexa_document.platform.client import DocumentFamilyEndpoint
...
class AssistantContext:
    ...
    from kodexa_document.model.objects import ContentEvent
