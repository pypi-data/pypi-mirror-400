...
from ..input_file.section_lists import SECTIONS_ORDER_DEFAULT
...
class _Config:
    ...
    Attributes:
        ...
        section_separator (str): A string used to separate sections in generated input-files.
        comment_prefix (str): String how all comments begin in created input-files. default=`;;`
        comment_empty_section (str): String how all empty sections are written to the input-file. default=`;; No Data`
        init_print_epa_swmm_run (bool): if the standard commandline output should be shown when running with the EPA SWMM CLI (using the function `swmm5_run_epa`). default=False
        default_temp_run (str or function): str of built-in function (`swmm5_run_progress`, `swmm5_run_owa`, `swmm5_run_epa`, `swmm5_run`) or custom function with the first argument being the input-filename for the `swmm5_run_temporary` context manager.
    ...
    """
    section_order: list[str] = dataclasses.field(default_factory= lambda: SECTIONS_ORDER_DEFAULT.copy())
    encoding: str = 'utf-8'
    gis_decimals: int = 3
...
def main():
    ...
    from swmm_api.input_file.section_lists import SECTIONS_ORDER_MP
    CONFIG.section_order = SECTIONS_ORDER_MP
