...
def add(ctx, binary_path, box, name, yes):
    """
        ...
        The binary will be copied to the box and made executable.
        ...
        Example:
            lager binaries add ./my_tool --box my-box
    """
    # Resolve box
    if not box:
    ...
    try:
        ...
        click.secho("Ensure the box is online and reachable.", fg='yellow', err=True)
        ctx.exit(1)
...
def remove(ctx, binary_name, box, yes):
    """
        ...
        Example:
            lager binaries remove my_tool --box my-box
    """
    # Resolve box
    if not box:
        box = get_default_box(ctx)
