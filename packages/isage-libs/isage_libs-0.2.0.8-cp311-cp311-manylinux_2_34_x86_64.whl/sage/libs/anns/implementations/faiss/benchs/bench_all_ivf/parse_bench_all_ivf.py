...
def collect_results_for(db="deep1M", prefix="autotune."):
    ...
    for fname in fnames:
        if not ("db" + db in fname and fname.startswith(prefix) and fname.endswith(".stdout")):
            continue
...
def extract_pareto_optimal(allres, keys, recall_idx=0, times_idx=3):
    ...
    selected_keys = [keys[i] for i in np.unique(bigtab_sorted[0, selection].astype(int))]
...
def plot_subset(
    allres, allstats, selected_methods, recall_idx, times_idx=3, report=["overhead", "build time"]
):
    # important methods
...
def plot_tradeoffs(db, allres, allstats, code_size, recall_rank):
    ...
    selected_methods, optimal_points = extract_pareto_optimal(allres, names, recall_idx, times_idx)
    ...
    for k in not_selected:
        ...
        linestyle = ":" if "PQ" in k else "-." if "SQ4" in k else "--" if "SQ8" in k else "-"
...
if __name__ == "__main__xx":
    ...
    for db in "bigann10M", "deep10M", "bigann100M", "deep100M", "deep1B", "bigann1B":
        ...
        for cs in 8, 16, 32, 64:
            ...
            plot_tradeoffs(db, allres, allstats, code_size=cs_range, recall_rank=1)
