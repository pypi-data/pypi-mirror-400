...
class Repl:
    ...
    async def run(self):
        print("pw-repl v0.1.23 - Type 'help' for commands")
        # 自动重连上次的连接
  ...
  accounts [platform]   - List s-mgr accounts
  switch <index|id>     - Load account session
  ...
  help [cmd]            - Show help""")
    ...
    # === 会话管理 ===
    def _get_smgr(self):
        from mp_repl.smgr import SmgrClient
        url = config.get("smgr_url")
        key = config.get("smgr_key")
        if not url:
            return None
        return SmgrClient(url, key)
    
    async def cmd_accounts(self, platform: str = None):
        """列出 s-mgr 账号"""
        smgr = self._get_smgr()
        if not smgr:
            print("✗ smgr_url not configured. Use: config smgr_url <url>")
            return
        try:
            accounts = smgr.list_accounts(platform)
            if not accounts:
    ...
    async def cmd_switch(self, index_or_id: str):
        """切换账号/加载会话"""
        smgr = self._get_smgr()
        if not smgr:
        ...
        # 支持索引或 ID
        account_id = None
        if index_or_id.isdigit() and hasattr(self, '_accounts'):
            idx = int(index_or_id)
            if idx < len(self._accounts):
                account_id = self._accounts[idx].get('id')
        else:
            account_id = index_or_id
        
        if not account_id:
            print(f"✗ Invalid account: {index_or_id}")
            return
        
        try:
            session = smgr.get_session(account_id)
            if not session:
                print(f"✗ No session for account: {account_id}")
                return
            ...
            # 加载 localStorage
            storage = session.get('localStorage', {})
            if storage:
                for origin, data in storage.items():
                    for key, value in data.items():
                        await self.conn_mgr.page.evaluate(
                            f"localStorage.setItem({repr(key)}, {repr(value)})"
                        )
                print(f"✓ Loaded localStorage for {len(storage)} origins")
            
            print(f"✓ Session loaded. Reload page to apply.")
        except Exception as e:
            print(f"✗ {e}")
    ...
    async def cmd_save(self):
        ...
        if not hasattr(self, '_current_account_id'):
            print("✗ No account selected. Use 'switch' first.")
            return
        ...
        try:
            ...
            # 获取 localStorage
            storage = await self.conn_mgr.page.evaluate("""
                () => {
                    const data = {};
            ...
            origin = self.conn_mgr.page.url.split('/')[0] + '//' + self.conn_mgr.page.url.split('/')[2]
            session_data = {
                'cookies': cookies,
                'localStorage': {origin: storage}
            }
            
            if smgr.save_session(self._current_account_id, session_data):
