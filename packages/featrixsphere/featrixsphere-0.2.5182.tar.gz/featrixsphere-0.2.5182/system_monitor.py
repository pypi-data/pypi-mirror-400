    ...
    def check_backplane(self):
        ...
        try:
            ...
            try:
                ...
                # Exit code 24 = "Partial transfer due to vanished source files" - this is acceptable
                # It just means some files were deleted during transfer, but backup still succeeded
                if result.returncode == 0 or result.returncode == 24:
                    if result.returncode == 24:
                        logger.warning(f"⚠️  Rsync backup completed with warnings (some files vanished during transfer)")
                    else:
