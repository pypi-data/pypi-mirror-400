...
class RDW2D52U(AbstractConstrainedMinimisation):
    """RDW2D52U problem - A finite-element approximation to distributed optimal control.
    ...
    Attempts made: [fully vectorized implementation]
    ...
    def objective(self, y, args):
        ...
        # Compute target data V(i,j) = exp(-64*((i*h-0.5)^2 + (j*h-0.5)^2))
        i_coords = jnp.arange(N + 1) * h
        j_coords = jnp.arange(N + 1) * h
    ...
    def constraint(self, y):
        ...
        i_idx = jnp.arange(1, N)  # [1, 2, ..., N-1]
        j_idx = jnp.arange(1, N)  # [1, 2, ..., N-1]
    ...
    def bounds(self):
        ...
        # Interior F variables are unbounded (no additional bounds like others)
        ...
        # Interior U variables are unbounded
