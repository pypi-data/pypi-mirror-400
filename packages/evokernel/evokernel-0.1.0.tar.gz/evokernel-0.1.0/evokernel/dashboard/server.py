...
def get_island_data(run_path: Path) -> dict:
        ...
        if checkpoints:
            ...
            if metadata_path.exists():
                try:
                    ...
                    island_generations = metadata.get("island_generations", [0] * len(islands_data))
                    ...
                    for i, island_program_ids in enumerate(islands_data):
                        island_info = {
                            ...
                            "generation": island_generations[i] if i < len(island_generations) else 0,
                        ...
                        best_id = island_best_ids[i] if i < len(island_best_ids) else None
                        if best_id and best_id in programs:
                            prog = programs[best_id]
                            metrics = prog.get("metrics", {})
                            island_info["best_score"] = metrics.get("combined_score", 0.0)
    ...
    if log_path.exists():
        try:
            ...
            if island_info_map:
                data["islands"] = [island_info_map.get(i, {
                    "index": i, "program_count": 0, "best_score": 0.0,
                    "best_speedup": 0.0, "generation": 0, "is_current": False, "best_code": None
                }) for i in range(data["num_islands"])]
                data["total_programs"] = sum(isl.get("program_count", 0) for isl in data["islands"])
        except Exception:
            pass
...
def get_tree_data(run_path: Path) -> dict | None:
    ...
    for prog_id, prog in programs.items():
        ...
        if not parent_id or parent_id not in programs:
            if root is None or metrics.get("combined_score", 0) > nodes.get(root, {}).get("speedup", 0):
                root = prog_id
...
def get_cost_data(run_path: Path) -> dict:
    """Read cost tracking data."""
    cost_path = run_path / "costs.json"
    if cost_path.exists():
        try:
...
def create_app(run_dir: Path) -> tuple[Flask, SocketIO]:
    ...
    def api_status():
        ...
        try:
            ...
            island_data = get_island_data(run_path)
            costs = get_cost_data(run_path)
            return jsonify({**status, **island_data, "costs": costs})
        except Exception as e:
