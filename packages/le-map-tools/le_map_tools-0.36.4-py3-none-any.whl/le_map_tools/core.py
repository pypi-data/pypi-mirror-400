...
class AbstractDrawControl(object):
    ...
    def __init__(self, host_map):
        ...
        Args:
            host_map (geemap.Map): The le_map_tools.Map instance to be linked with
                the draw control.
        """
    ...
    @property
    def features(self) -> List[le.Feature]:
        ...
        if self.count:
            ...
            for i, geometry in enumerate(self.geometries):
                if i < len(self.properties):
                    ...
                    property = None
                features.append(le.Feature(geometry, property))
            return features
        else:
            return []

    @property
    def collection(self) -> le.FeatureCollection:
        """Feature collection created from features.

        Returns:
            ee.FeatureCollection: Earth Engine feature collection.
        """
        return le.FeatureCollection(self.features if self.count else [])

    @property
    def last_feature(self) -> Optional[le.Feature]:
        ...
        return le.Feature(self.last_geometry, property) if self.last_geometry else None
    ...
    def remove_geometry(self, geometry: le.Geometry) -> None:
    ...
    def get_geometry_properties(self, geometry: le.Geometry) -> Optional[dict]:
    ...
    def set_geometry_properties(self, geometry: le.Geometry, property: dict) -> None:
...
class MapDrawControl(ipyleaflet.DrawControl, AbstractDrawControl):
    ...
    def __init__(self, host_map, **kwargs: Any) -> None:
        ...
        Args:
            host_map (geemap.Map): The le_map_tools.Map object that the control will be added to.
...
class MapInterface:
    ...
    def center_object(
        self, ee_object: le.ComputedObject, zoom: Optional[int] = None
    ) -> None:
...
class Map(ipyleaflet.Map, MapInterface):
    ...
    _USER_AGENT_PREFIX = "le_map_tools-core"
    ...
    def get_bounds(self, as_geojson: bool = False) -> Sequence:
        ...
        if as_geojson:
            return le.Geometry.BBox(*coords).getInfo()
        return coords
    ...
    def _get_geometry(
        self, ee_object: le.ComputedObject, max_error: float
    ) -> le.Geometry:
        ...
        """
        if isinstance(ee_object, le.Geometry):
            return ee_object
        try:
