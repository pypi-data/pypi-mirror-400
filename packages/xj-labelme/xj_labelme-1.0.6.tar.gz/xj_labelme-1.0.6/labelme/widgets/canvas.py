...
from labelme._automation.polygon_from_mask import compute_polygon_from_mask
...
try:
    ...
    osam = None
    AI_AVAILABLE = False
    logger.warning("AI features not available (osam/onnxruntime not loaded)")
...
class Canvas(QtWidgets.QWidget):
    ...
    def mouseMoveEvent(self, a0: QtGui.QMouseEvent) -> None:
        ...
        for shape in ([self.hShape] if self.hShape else []) + [
            ...
            # 只有当形状已被选中时，才检测顶点和边缘（用于编辑）
            # 未选中的形状优先检测是否在内部（用于选中查看）
            is_shape_selected = shape in self.selectedShapes
            
            index = None
            index_edge = None
            if is_shape_selected:
                # 形状已选中，检测顶点和边缘用于编辑
    ...
    def paintEvent(self, a0: QtGui.QPaintEvent) -> None:
        ...
        # 当缩放比例 >= 200% 时，使用最近邻插值显示清晰的像素方块
        if self.scale >= 2.0:
            p.setRenderHint(QtGui.QPainter.Antialiasing, False)
            p.setRenderHint(QtGui.QPainter.SmoothPixmapTransform, False)
            
            # 手动缩放 pixmap 使用 FastTransformation（最近邻插值）
            scaled_pixmap = self.pixmap.scaled(
                int(self.pixmap.width() * self.scale),
                int(self.pixmap.height() * self.scale),
                QtCore.Qt.KeepAspectRatio,
                QtCore.Qt.FastTransformation  # 最近邻插值，像素清晰
            )
            
            # 计算偏移位置
            x_offset = int(offset.x() * self.scale)
            y_offset = int(offset.y() * self.scale)
            p.drawPixmap(x_offset, y_offset, scaled_pixmap)
            
            # 应用变换用于后续绘制形状和网格
            p.scale(self.scale, self.scale)
            p.translate(offset)
            
            # 绘制像素网格（当缩放比例 >= 800% 时显示）
            if self.scale >= 8.0:
                self._drawPixelGrid(p)
        else:
        ...
        Shape.scale = 1.0  # 坐标变换已通过 painter 处理
        for shape in self.shapes:
    ...
    def _drawPixelGrid(self, painter: QtGui.QPainter) -> None:
        """绘制像素网格，当缩放比例足够大时显示"""
        if not self.pixmap:
            return
        ...
        # 设置网格线样式
        pen = QtGui.QPen(QtGui.QColor(128, 128, 128, 100))  # 半透明灰色
        pen.setWidthF(1.0 / self.scale)  # 保持线宽为1像素
        painter.setPen(pen)
...
def _update_shape_with_sam(
    ...
    if createMode == "ai_mask":
        ...
        points = compute_polygon_from_mask(
