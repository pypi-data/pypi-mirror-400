...
def ideal_epochs(n_rows: int, batch_size: int, mode: str) -> int:
    ...
    return int(max(5, epochs))  # Removed 300 cap - allow higher epochs for large datasets
...
def ideal_epochs_predictor(n_rows: int, batch_size: int, imbalance_ratio: float = 1.0) -> int:
    ...
    # CUT EPOCHS IN HALF for faster training
    base_epochs = max(5, base_epochs // 2)
...
def ideal_single_predictor_config(
    ...
    if n_rows < 500:
        ...
        # Scale d_hidden with d_model - use 2x expansion for richer representations
        # This provides substantial capacity for the predictor head to learn complex patterns
        d_hidden = min(int(2.0 * d_model), 512)
