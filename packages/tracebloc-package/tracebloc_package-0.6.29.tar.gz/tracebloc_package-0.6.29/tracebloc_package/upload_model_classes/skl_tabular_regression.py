...
class SKLTabularRegression(Model, SklearnChecks):
    def __init__(
        self,
        model_name,
        token,
        ...
        tmp_dir_path,
        progress_bar_1,
        classes,  # For regression, this might represent output_dim
        weights_path,
        model,
        category,
        progress_bar,
    ...
    def small_training_loop(
        ...
        try:
            # Use regression dataloader instead of classification
            ...
            for inputs, targets in dataloader:
                try:
                    ...
                    self.model.fit(inputs_np, targets_np)
                except:
                    pass
    ...
    def average_weights(self):
        """
        Average two weight dictionaries for regression models.
        """
        weights = []
        ...
        if self.model_type == "linear":
            # Average coefficients and intercepts for linear regression
        ...
        elif self.model_type in ("tree", "ensemble", "xgboost", "lightgbm", "svm"):
            # Ensemble strategy: Use a VotingRegressor for tree-based and ensemble models
            prefix = "svr_" if self.model_type == "svm" else ("tree_" if self.model_type in ("tree", "ensemble") else self.model_type + "_")
            voting_regressors = [
                (prefix + str(i), weights[i]) for i in range(len(weights))
            ]
            averaged_model = VotingRegressor(estimators=voting_regressors)

        else:
            raise ValueError(
                f"model type {self.model_type} is not supported for Sklearn regression"
            )
