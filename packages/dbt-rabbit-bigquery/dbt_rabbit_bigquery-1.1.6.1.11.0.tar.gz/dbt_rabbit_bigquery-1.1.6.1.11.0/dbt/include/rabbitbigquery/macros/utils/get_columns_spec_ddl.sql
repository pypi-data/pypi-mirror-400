{% macro rabbitbigquery__format_column(column) -%}
  {% set data_type = column.data_type %}
  {% set formatted = column.column.lower() ~ " " ~ data_type %}
  {{ return({'name': column.name, 'data_type': data_type, 'formatted': formatted}) }}
{%- endmacro -%}

{% macro rabbitbigquery__get_empty_schema_sql(columns) %}
    {%- set col_err = [] -%}
...
{% macro rabbitbigquery__get_select_subquery(sql) %}
...
{% macro rabbitbigquery__get_column_names() %}
