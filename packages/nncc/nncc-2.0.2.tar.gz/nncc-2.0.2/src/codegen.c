...
static void cg_printf(CodegenContext* ctx, const char* fmt, ...) {
    ...
    va_list args;
    va_start(args, fmt);
    // Convert to simple write by formatting first?
    // Or just recurse if we had vcg_write.
    // Let's just use vsnprintf to temp stack buffer or something?
    // Actually, simplest is to implement cg_printf logic inside cg_write or similar.
    // But since we modified cg_write to handle buffer logic, we can just formatting into a buffer and call cg_write.
    // Or duplicated logic. 
    // Let's just use cg_write(ctx, "formatted string"). 
    // But that requires formatting first.
    char buf[4096];
        ...
        if (ctx->buffer) {
             if (ctx->buffer_len + len + 1 > ctx->buffer_cap) {
                 ...
                 if (new_buf) {
                     ...
                     // Alloc failure - what to do? 
                     // For now, just stop writing or set error flag?
                     // We don't have error flag in context.
                     return;
                 }
             }
             va_start(args, fmt);
...
static void emit_header(CodegenContext* ctx) {
    ...
    cg_write(ctx, " *\n");
    cg_write(ctx, " * Usage:\n");
    cg_write(ctx, " *   #define %s_IMPLEMENTATION\n", upper);
    ...
    cg_write(ctx, "#ifdef %s_IMPLEMENTATION\n\n", upper);
}
...
static void emit_footer(CodegenContext* ctx) {
    ...
    cg_write(ctx, "#endif /* %s_IMPLEMENTATION */\n", upper);
}
