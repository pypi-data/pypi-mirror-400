...
from mendevi.decode import optimal_decode
...
def encode(src: pathlib.Path, **kwargs: dict) -> tuple[pathlib.Path, str, str, dict[str]]:
    ...
    **kwargs : dict
    ...
    with cp_shm(src) as src_ram:  # copy input video into ram

        # find tempfile name
        dst = (
            src_ram.parent
            if src_ram.parent == pathlib.Path("/dev/shm") else
        ...
        prt_cmd = " ".join(
            map(
                shlex.quote,
                [{str(src_ram): "src.mp4", str(dst): "dst.mp4"}.get(c, c) for c in user_cmd or cmd],
            ),
        )
...
def get_transcode_cmd(src: pathlib.Path, dst: pathlib.Path, **kwargs: dict) -> CmdFFMPEG:
    ...
    cmd.decode = optimal_decode(cutcutcodec.get_codec_video(src), cutcutcodec.get_pix_fmt(src))
    if vid_filter:
        cmd.vid_filter = ",".join([*cmd.vid_filter.split(","), vid_filter])
    return cmd
