...
from alibabacloud_secretsmanager_client_v2.model.secret_info import convert_json_to_secret_info
from alibabacloud_secretsmanager_client_v2.utils import const, err_code_const
from alibabacloud_secretsmanager_client_v2.utils.backoff_utils import judge_need_back_off, \
    judge_need_recovery_exception
from alibabacloud_secretsmanager_client_v2.utils.byte_buffer_utils import covert_string_to_byte
...
def judge_skip_refresh_exception(e):
    return (err_code_const.FORBIDDEN_IN_DEBT_OVER_DUE == e.code) or (
            err_code_const.FORBIDDEN_IN_DEBT == e.code) or judge_need_back_off(e)
...
class SecretManagerCacheClient:
    ...
    def __get_secret_value(self, secret_name):
        ...
        get_secret_request.secret_name = secret_name
        get_secret_request.version_stage = self.stage
        get_secret_request.fetch_extended_config = True
        try:
            get_secret_resp = self.secret_client.get_secret_value(get_secret_request)
            resp_json = get_secret_resp.to_map()['body']
        except TeaException as e:
