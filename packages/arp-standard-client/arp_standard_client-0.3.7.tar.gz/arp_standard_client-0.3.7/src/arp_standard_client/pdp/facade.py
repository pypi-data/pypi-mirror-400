...
class PdpClient:
    ...
    def decide_policy(self, request: PdpDecidePolicyRequest) -> PolicyDecision:
        resp = decide_policy.sync_detailed(client=self._client, body=request.body)
        return _unwrap(resp)

    def health(self, request: PdpHealthRequest) -> Health:
        _ = request
        resp = health.sync_detailed(client=self._client)
        return _unwrap(resp)

    def version(self, request: PdpVersionRequest) -> VersionInfo:
        _ = request
