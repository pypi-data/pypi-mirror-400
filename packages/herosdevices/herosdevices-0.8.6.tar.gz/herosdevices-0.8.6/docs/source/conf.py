        ...
        if class_path in json_examples:
            for path, example in json_examples[class_path].items():
                if path not in processed_files:
                    ...
                    if any(str(val).startswith("$") for val in example.get("arguments", {}).values()):
                        content += ".. note::\n\n"
                        content += "    This example contains a variable that "
                        content += "    :external:ref:`references another HERO <json-remote-hero>` with ``$``.\n\n"
