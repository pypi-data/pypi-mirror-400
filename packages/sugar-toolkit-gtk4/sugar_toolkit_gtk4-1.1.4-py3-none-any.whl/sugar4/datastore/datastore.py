...
def _get_data_store():
    ...
    if not _data_store:
        ...
        _data_store = dbus.Interface(
            _bus.get_object(DS_DBUS_SERVICE, DS_DBUS_PATH), DS_DBUS_INTERFACE
        )
...
class DSMetadata(GObject.GObject):
    ...
    def __init__(self, properties=None):
        ...
        default_keys = ["activity", "activity_id", "mime_type", "title_set_by_user"]
...
class DSObject(object):
    ...
    def __object_updated_cb(self, object_id):
        properties = _get_data_store().get_properties(self._object_id, byte_arrays=True)
    ...
    def __del__(self):
        if not self._destroyed:
            logging.warning(
                "DSObject was deleted without cleaning up first. "
...
class RawObject(object):
    ...
    def __del__(self):
        if not self._destroyed:
            logging.warning(
                "RawObject was deleted without cleaning up. "
...
def _update_ds_entry(
    ...
    if reply_handler and error_handler:
        _get_data_store().update(
            uid,
            dbus.Dictionary(properties),
            filename,
            transfer_ownership,
        ...
        _get_data_store().update(
            uid, dbus.Dictionary(properties), filename, transfer_ownership
        )


def _create_ds_entry(properties, filename, transfer_ownership=False):
    object_id = _get_data_store().create(
        dbus.Dictionary(properties), filename, transfer_ownership
    )
    return object_id
...
def write(
    ...
    if ds_object.object_id:
        ...
        if reply_handler or error_handler:
            logging.warning(
                "datastore.write() cannot currently be called"
                "async for creates, see ticket 3071"
            )
        ds_object.object_id = _create_ds_entry(
            properties, file_path, transfer_ownership
        )
...
def find(
    ...
    if reply_handler and error_handler:
        ...
        entries, total_count = _get_data_store().find(
            query, properties, byte_arrays=True
        )
    ds_objects = []
