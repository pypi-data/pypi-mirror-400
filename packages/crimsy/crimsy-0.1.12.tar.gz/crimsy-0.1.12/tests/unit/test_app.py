...
async def test_swagger_endpoint() -> None:
    """Test that Swagger UI endpoint is available."""
    app = Crimsy()
    ...
    assert response.status_code == 200
    # Check for Swagger UI-specific identifier
    assert "swagger-ui" in response.text
    assert "SwaggerUIBundle" in response.text


async def test_swagger_endpoint_disabled() -> None:
    """Test that Swagger UI endpoint can be disabled."""
    app = Crimsy(swagger_url=None)
