...
async def get_pages_from_pagination(
    ...
    model_name = parsed_url["path"].replace("/", "").split("-")[0]
    ...
    image_tags = [tag for tag in soup.select(query) if "icon" not in tag.attrs["src"]]

    while page_content and len(image_tags) and page_number <= max_pages:
        print(f"Processing page {page_number} for model {model_name}...")

        print(f"Found {len(image_tags)} images on page {page_number}")
        final_image_tags.update(*image_tags)
