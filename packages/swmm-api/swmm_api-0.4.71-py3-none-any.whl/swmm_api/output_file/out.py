...
class SwmmOutput(SwmmOutExtract):
    ...
    def to_numpy(self, ignore_filesize=False, start=None, end=None):
        ...
        Args:
            ...
            start (datetime.datetime): start datetime for the period to read.
            end (datetime.datetime): end datetime for the period to read.
        ...
        if self._data is None:
            ...
            types = [('datetime', 'f8')] + [(self._COL_SEP.join(i), 'f4') for i in self._columns_raw]
    ...
    def get_part(self, kind=None, label=None, variable=None, slim=False, show_progress=True, ignore_filesize=False, start=None, end=None):
        ...
        if slim:
            ...
            values = self.to_numpy(ignore_filesize=ignore_filesize, start=start, end=end)[list(map(self._COL_SEP.join, columns))]
    ...
    def _to_pandas(self, data, index: pd.DatetimeIndex = None, drop_useless=False):
        ...
        df.columns = pd.MultiIndex.from_tuples([col.split(self._COL_SEP) for col in df.columns])
    ...
    def _iter_chunks(self, rows_at_a_time=1000, show_progress=True, kind=None, label=None, variable=None, slim=False):
        types = np.dtype([('datetime', 'f8')] + [(self._COL_SEP.join(i), 'f4') for i in self._columns_raw])
        self.fp.seek(self._pos_start_output, 0)

        columns = self._filter_part_columns(kind, label, variable)
        use_columns = ['datetime'] + [self._COL_SEP.join(c) for c in columns]
        ...
        for i in iterator:
            ...
            if slim:
                ...
                data = _get_selective_results(self.fp, label_list, offset_list, _pos_start_output,
                                              rows_at_a_time, self._bytes_per_period, progress_desc, show_progress=False)
    ...
    def to_hdf5_chunks_pandas(self, fn, rows_at_a_time=1000, show_progress=True, kind=None, label=None, variable=None, slim=False, complib=None, complevel=None):
        ...
        for i, df in enumerate(self._iter_chunks(rows_at_a_time=rows_at_a_time,
            ...
            for obj_kind in self.variables:
                for var_label in self.variables[obj_kind]:
                    ...
                    # replace '/' in object labels with '_' because using '/' in columns is not allowed in hdf5.
