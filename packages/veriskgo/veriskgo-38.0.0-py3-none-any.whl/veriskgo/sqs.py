...
class _VeriskGoSQS:
    ...
    def send(self, message: Optional[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            print(f"[veriskgo] Queue error → spillover: {e}")
            self._spillover_save(message)
            return False

    def send_immediate(self, message: Optional[Dict[str, Any]]) -> bool:
        """Send message immediately without batching. Use for critical messages like trace_end."""
        if not message:
            return False
        ...
        if not self.client:
            print("[veriskgo] SQS unavailable for immediate send")
            return False
        ...
        try:
            ...
            print(f"[veriskgo] Immediate send failed: {e}")
            self._spillover_save(message)
            return False
