...
def set_database_source(
    ...
    try:
        # 验证数据库类型
        if db_type.lower() not in ["mysql", "postgresql", "sqlite"]:
            error_result = {
                "message": f"不支持的数据库类型: {db_type}，支持 mysql、postgresql 和 sqlite",
                "success": False,
                "error": True,
        ...
        if db_type.lower() == "sqlite":
            ...
            new_config = {
                ...
                "charset": db_charset if db_type.lower() == "mysql" else "",
        ...
        with engine.connect() as connection:
            ...
            result_data = {
                ...
                "server_info": {"version": version, "connection_test": "成功"},
            ...
            logger.info(
                f"数据库数据源设置成功，环境变量已更新: {new_config['host']}:{new_config['port']}"
            )
            return result_data
