...
class LMSBaseClient:
    ...
    def disconnect(self: typing.Self) -> None:
...
class LMSListener(LMSBaseClient):
    """
    A listener for the LMS CLI.

    Like the synchronous request/response client, `LMSClient`, this client
    allows you to send commands to LMS. But there is no response associated to
    specific commands, as the client is listening to a stream of data from LMS,
    which _will_ include responses to commands, but also other things and
    cannot the order of things.
    """
    ...
    def read_events(
        ...
        """Read events from the LMS server."""
        while not self.gracefully_stop:
            event = self.client.read_until(b"\n", timeout=1).decode("utf-8").strip()
            if event:
                self.queue.put(parse.unquote(event))

    def listen(self: typing.Self) -> None:
        """Start listening to events from LMS."""
    ...
    def stop_listening(self: typing.Self) -> None:
        """Stop listening to events from LMS."""
        self.gracefully_stop = True
        while self.thread.is_alive():
            pass



class LMSClient(LMSBaseClient):
