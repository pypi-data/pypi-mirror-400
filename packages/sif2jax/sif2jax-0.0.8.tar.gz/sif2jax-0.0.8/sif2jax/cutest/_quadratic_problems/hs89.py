...
class HS89(AbstractConstrainedQuadraticProblem):
    """Hock and Schittkowski problem 89.
    ...
    def constraint(self, y):
        ...
        # This implements the EVAL89 function from the SIF/Fortran code
        ...
        # p[0] = x1^2 + x2^2 + x3^2, p[1] = x2^2 + x3^2, p[2] = x3^2, p[3] = 0
        p = jnp.array([x1**2 + x2**2 + x3**2, x2**2 + x3**2, x3**2, 0.0], dtype=y.dtype)
        ...
        # Add contribution from i=3 with alternating sign alpha = 2
        u = u + 2.0 * jnp.exp(-muj2 * p[2])

        # Add final constant term (alpha = -2 for i=4, p[3]=0 so exp(0)=1)
        # From line 265: u = u + 0.5 * alpha where alpha = -2 after loop
        u = u - 1.0
        ...
        # Based on HS88 experience, we need the adjustment
        eps_sqr = 0.0001
    ...
    def expected_result(self):
        ...
        # HS89SOL: X1 = 1.07337D+00, X2 = 4.56043D-01, X3 = -8.33607D-68 (essentially 0)
        return jnp.array([1.07337, 0.456043, 0.0], dtype=jnp.float64)

    @property
    def expected_objective_value(self):
        """Expected objective value from SIF file."""
        # XL HS89SOL = 1.36009D+00
        return jnp.array(1.36009)
