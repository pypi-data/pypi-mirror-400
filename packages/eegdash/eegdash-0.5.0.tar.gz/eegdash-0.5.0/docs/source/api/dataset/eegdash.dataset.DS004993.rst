..
   This documentation page is generated during the Sphinx build.
   The underlying code is manually maintained and not autogenerated.

eegdash.dataset.DS004993
========================

WIRED ICM Sample Dataset - Workshop on Intracranial Recordings in Humans, Epilepsy, DBS (OpenNeuro ``ds004993``). Access recordings and metadata through EEGDash.

.. rst-class:: sd-badges

:bdg-light:`Modality: ieeg` :bdg-light:`Tasks: 3` :bdg-light:`License: CC0` :bdg-light:`Subjects: 3` :bdg-light:`Recordings: 3` :bdg-light:`Source: openneuro`

Dataset Information
-------------------

.. list-table::
   :widths: 25 75
   :header-rows: 0

   * - Dataset ID
     - ``DS004993``
   * - Title
     - WIRED ICM Sample Dataset - Workshop on Intracranial Recordings in Humans, Epilepsy, DBS
   * - Year
     - Unknown
   * - Authors
     - Liberty S. Hamilton, Maansi Desai, Alyssa Field
   * - License
     - CC0
   * - Citation / DOI
     - `doi:10.18112/openneuro.ds004993.v1.1.2 <https://doi.org/10.18112/openneuro.ds004993.v1.1.2>`__
   * - Source links
     - `OpenNeuro <https://openneuro.org/datasets/ds004993>`__ | `NeMAR <https://nemar.org/dataexplorer/detail?dataset_id=ds004993>`__

.. dropdown:: Copy-paste BibTeX
   :class-container: sd-shadow-sm
   :class-title: sd-bg-light

   .. code-block:: bibtex

      @dataset{ds004993,
        title = {WIRED ICM Sample Dataset - Workshop on Intracranial Recordings in Humans, Epilepsy, DBS},
        author = {Liberty S. Hamilton and Maansi Desai and Alyssa Field},
        doi = {10.18112/openneuro.ds004993.v1.1.2},
        url = {https://doi.org/10.18112/openneuro.ds004993.v1.1.2},
      }

Highlights
----------

.. grid:: 1 2 3 3
   :gutter: 2

   .. grid-item-card:: Subjects & recordings
      :class-card: sd-border-1

      - Subjects: 3
      - Recordings: 3
      - Tasks: 3

   .. grid-item-card:: Channels & sampling rate
      :class-card: sd-border-1

      - Channels: 160*
      - Sampling rate (Hz): 512*
      - Duration (hours): Unknown

   .. grid-item-card:: Tasks & conditions
      :class-card: sd-border-1

      - Tasks: 3
      - Experiment type: Unknown
      - Subject type: Unknown

   .. grid-item-card:: Files & format
      :class-card: sd-border-1

      - Size on disk: 305.11 MB
      - File count: Unknown
      - Format: BIDS

   .. grid-item-card:: License & citation
      :class-card: sd-border-1

      - License: CC0
      - DOI: doi:10.18112/openneuro.ds004993.v1.1.2

   .. grid-item-card:: Provenance
      :class-card: sd-border-1

      - Source: openneuro
      - OpenNeuro: `ds004993 <https://openneuro.org/datasets/ds004993>`__
      - NeMAR: `ds004993 <https://nemar.org/dataexplorer/detail?dataset_id=ds004993>`__

Quickstart
----------

**Install**

.. code-block:: bash

   pip install eegdash

**Load a recording**

.. code-block:: python

   from eegdash.dataset import DS004993

   dataset = DS004993(cache_dir="./data")
   recording = dataset[0]
   raw = recording.load()

**Filter/query**

.. tab-set::

   .. tab-item:: Basic

      .. code-block:: python

         dataset = DS004993(cache_dir="./data", subject="01")

   .. tab-item:: Advanced

      .. code-block:: python

         dataset = DS004993(
             cache_dir="./data",
             query={"subject": {"$in": ["01", "02"]}},
         )


Quality & caveats
-----------------

- No dataset-specific caveats are listed in the available metadata.

API
---

.. currentmodule:: eegdash.dataset

.. autoclass:: eegdash.dataset.DS004993
   :members: __init__, save
   :show-inheritance:
   :member-order: bysource


See Also
--------

* :class:`eegdash.dataset.EEGDashDataset`
* :mod:`eegdash.dataset`
* `OpenNeuro dataset page <https://openneuro.org/datasets/ds004993>`__
* `NeMAR dataset page <https://nemar.org/dataexplorer/detail?dataset_id=ds004993>`__

