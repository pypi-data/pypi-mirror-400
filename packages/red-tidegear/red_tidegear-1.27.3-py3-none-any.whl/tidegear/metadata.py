...
__all__ = ["User", "Repository", "CogMetadata"]
...
class CogMetadata(BaseModel):
    ...
    minimum_tidegear_version: SemanticVersion | None = None
    """The minimum Tidegear version that this cog supports."""
    authors: list[User]
    ...
    @property
    def is_version_constraint_met(self) -> bool:
        """Returns whether or not the currently-installed version of Tidegear meets this cog's version constraints."""
        from tidegear.version import version as tidegear_version  # noqa: PLC0415

        if not self.minimum_tidegear_version:
            return True
        if tidegear_version >= self.minimum_tidegear_version:
            return True
        return False
