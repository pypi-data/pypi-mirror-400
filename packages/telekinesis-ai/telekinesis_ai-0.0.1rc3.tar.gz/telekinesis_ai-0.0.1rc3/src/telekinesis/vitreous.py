...
_VITREOUS_API_BASE_URL = "http://127.0.0.1:4100/vitreous/v1/"
...
def _send_request(endpoint: str, input_data: dict) -> dict:
    """
    Send Telekinesis datatypes via multipart/form-data,
    ...
    for name, attribute in input_data.items():
        arrow_bytes = serializer.serialize_to_pyarrow_ipc(attribute)
    ...
    body, boundary = write_multipart_request({k: v[1] for k, v in fields.items()})
    headers = {
        "X-API-Key": API_KEY,
