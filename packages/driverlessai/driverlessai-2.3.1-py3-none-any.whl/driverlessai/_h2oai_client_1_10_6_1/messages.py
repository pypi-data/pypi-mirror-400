...
class Dataset:
    ...
    """
    def __init__(self, key, name, file_path, file_size, bin_file_path, data_source, row_count, column_count, columns, original_frame, aggregated_frame, mapping_frame, uploaded, remote, missing_values, *, validate_toml=False) -> None:
        self.key = key
        self.name = name
    ...
    def clone(self) -> 'Dataset':
        return Dataset(self.key, self.name, self.file_path, self.file_size, self.bin_file_path, self.data_source, self.row_count, self.column_count, self.columns, self.original_frame, self.aggregated_frame, self.mapping_frame, self.uploaded, self.remote, self.missing_values)
