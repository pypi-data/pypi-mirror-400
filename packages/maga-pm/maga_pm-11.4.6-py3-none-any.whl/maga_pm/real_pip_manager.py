...
class RealPipManager:
    ...
    def _monitor_pip_progress(self, progress_queue: queue.Queue, stop_event: threading.Event, package_name: str):
        ...
        while not stop_event.is_set():
            try:
                ...
                # 定期更新进度条（无论是否有新消息）
                if time.time() - last_update > 0.2:  # 每0.2秒更新一次
                    if last_progress < 95:
                        # 模拟进度更新
                        progress_increment = random.randint(1, 5)  # 随机增加1-5%
                        last_progress = min(95, last_progress + progress_increment)
    ...
    def _display_progress_bar(self, progress: int, status: str = "", 
        ...
        # 每1%进度有20%概率显示连接服务器状态（更频繁）
        current_time = time.time()
        if show_server_check and random.random() < 0.2 and (current_time - last_server_check) > 0.5:
            # 测试pypi.org连接
