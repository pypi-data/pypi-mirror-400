...
from .._units.websocket_listener import WebSocketListener
...
class ChromiumBase(BasePage):
    def __init__(self, browser, target_id=None):
        ...
        if not hasattr(self, '_listener'):
            self._listener = None
        if not hasattr(self, '_ws_listener'):
    ...
    @property
    def ws_listen(self):
        """WebSocket listener for monitoring WebSocket frames"""
        if self._ws_listener is None:
            self._ws_listener = WebSocketListener(self)
