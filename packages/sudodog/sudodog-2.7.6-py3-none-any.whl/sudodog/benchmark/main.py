...
def prompt_for_publish(
    ...
    print()
    print("  Publish to get:")
    print("    + Full report with detailed insights and recommendations")
    print("    + Shareable URL and badge for your README")
    print("    + Ranking on the public leaderboard")
    print()
    print("  Privacy: Only score & metrics shared. No prompts, keys, or code.")
    print()
...
def show_results(response: Dict[str, Any], display: Display, published: Dict[str, Any] = None):
    ...
    print(f"  {score_bar}")
    print()
    print("  Publish to get full report with detailed insights.")
    print()
  ...
  sudodog-benchmark --agent-pid 12345            Monitor existing process (legacy mode)
    ...
    try:
        ...
        if args.command or (not args.agent_pid and not args.json):
            ...
            # If command provided via --command but no --type, prompt for agent type
            if command and args.type == 'general' and not args.json:
                print()
                ...
                try:
                    ...
                    type_map = {'1': 'chat', '2': 'code', '3': 'data', '4': 'task', '5': 'general', '': 'general'}
            ...
            # Show results so user sees their score
            if not args.json:
                show_results(response, display, published=None)

            # Prompt for optional publish to leaderboard (only if online)
            if not args.no_submit and not response.get("offline"):
                published = prompt_for_publish(results_data, response, machine_id, display)

                # If published, show the report URLs
                if published:
                    print()
                    display.header("YOUR LEADERBOARD ENTRY")
                    print()
                    if published.get("report_url"):
                        print(f"  Full Report:  {published.get('report_url')}")
                    if published.get("badge_url"):
                        print(f"  Get Badge:    {published.get('badge_url')}")
                    print()
                    print("  Share your score on Twitter!")
