...
def _resolve_cwd_path(cwd: str | None) -> Path:
    try:
...
def _repo_root_for_cwd(cwd: str | None) -> Path:
...
def register_project_tools(
    ...
    *,
    ...
    def _refresh_task_from_dispatch(task_state: dict[str, Any], *, job_id: str) -> None:
        ...
        # Patch-only: on completion, generate a git diff patch from the task worktree.
    ...
    def project_create(
        *,
        ...
        project_path = repo_root / ".codex" / "projects" / project_id
    ...
    def project_tick(
        *,
        ...
        # Refresh + patch generation (via status)
        status_res = project_status(project_id=project_id, options={"include_handles": False})
        ...
        patch_res = project_patch_list(project_id=project_id, options={})
        ready = blocked = applied = 0
        if patch_res.get("state") == "ok":
            for p in patch_res.get("patches", []) if isinstance(patch_res.get("patches"), list) else []:
        ...
        summary: list[str] = []
        summary.append(f"Tasks: running={counts.get('running', 0)} completed={counts.get('completed', 0)} failed={counts.get('failed', 0)} pending={counts.get('pending', 0)}")
        ...
        if ready or blocked or applied:
            summary.append(f"Patches: ready={ready} blocked={blocked} applied={applied}")
