...
class JSCoverage(object):
    ...
    async def start(self, options: dict | None = None, **kwargs: Any) -> None:
        ...
        self._eventListeners = [
            ...
            helper.addEventListener(
                self._client,
                "Debugger.paused",
                lambda e: self._client._loop.create_task(self._onDebuggerPaused()),
            ),
            helper.addEventListener(
                self._client,
    ...
    async def _onDebuggerPaused(self):
        await self._client.send("Debugger.resume")

    async def _onExecutionContextsCleared(self) -> None:
