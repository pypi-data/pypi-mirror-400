...
class FastAPIWebsocketClient:
    ...
    with support for binary, text, and mixed message types.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            is_mixed: Whether to use mixed mode (both binary and text messages).
        """
    ...
    async def _iter_mixed(self) -> typing.AsyncIterator[bytes | str]:
        """Async generator for receiving mixed binary and text WebSocket messages.

        Yields:
            Either bytes or string based on message type.
        """
        while True:
    ...
    async def _send_single(self, data: str | bytes):
        """Send a single message through the WebSocket connection.
    ...
    async def send(self, data: str | bytes | tuple):
        """Send data through the WebSocket connection.

        Args:
            data: The data to send (string, bytes, or tuple for multiple messages).
        """
        try:
            if self._can_send():
                # Handle tuple of messages (e.g., for sending command + audio together)
                if isinstance(data, tuple):
                    for item in data:
                        await self._send_single(item)
                else:
...
class FastAPIWebsocketTransport(BaseTransport):
    ...
    def __init__(
        ...
        if self._params.serializer:
            ...
            is_mixed = self._params.serializer.type == FrameSerializerType.MIXED
        self._client = FastAPIWebsocketClient(websocket, is_binary, self._callbacks, is_mixed)
