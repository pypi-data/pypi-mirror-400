...
class NetCDFOutput:
    ...
    def __init__( self, filename=None ):
        ...
        self.Zinput = self.rootgrp.createVariable( "Zinput", "f4", ("count", "X","Y",) )
        self.Zinput.units = "millimeter"
        self.Zinput.long_name = "Sparse scattered Z data before interpolation"
        self.Zinput.field = "Z, scalar, series"
    ...
    def push_Z( self, Zdata, time, workdir, image=None, Zinput=None ):
        if self.rootgrp != None:
            idx = self.count.shape[0]
            self.Z[idx,:,:] = np.expand_dims( Zdata, axis=0 )

            if not Zinput is None:
                self.Zinput[idx,:,:] = np.expand_dims( Zinput, axis=0 )
