...
if TYPE_CHECKING:
    ...
    from kodexa_document.model.objects import Taxon

logger = logging.getLogger(__name__)


def get_source(document: "Document") -> io.BytesIO:
    """
    Get the source document data as a file-like object.

    This provides compatibility with the old kodexa SDK's get_source function.
    Returns the first native document's data as a BytesIO object.

    Args:
        document: The Document to get source from

    Returns:
        BytesIO object containing the source document data

    Raises:
        ValueError: If no native document is found
    """
    logger.info("get_source starting")
    native_doc = document.get_first_native_document()
    if not native_doc:
        raise ValueError("No native document found in document")

    data = document.get_native_document_data(native_doc['id'])
    if not data:
        raise ValueError("Native document has no data")
