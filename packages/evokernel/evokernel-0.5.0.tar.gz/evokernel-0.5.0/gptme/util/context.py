...
def _find_potential_paths(content: str) -> list[str]:
    ...
    # First find @-prefixed paths (e.g., @file.py, @src/main.rs)
    for match in re.finditer(r"@([\w./\-]+)", content_no_codeblocks):
        path = match.group(1)
        paths.append(path)
    ...
    # Also remove @-prefixed paths to avoid double-processing
    content_no_backticks = re.sub(r"@[\w./\-]+", "", content_no_backticks)
