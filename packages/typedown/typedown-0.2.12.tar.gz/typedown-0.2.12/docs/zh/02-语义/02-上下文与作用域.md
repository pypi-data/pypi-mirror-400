...
隐式上下文虽然强大，但也带来了认知摩擦。为了在“脆弱”的场域中生存，开发者必须依赖工具链进行持续的**认知对齐 (Cognitive Alignment)**。
...
- **LSP Doc Lens (文档透镜)**:
  ...
  - **原则**: 也就是“看不见的代码”必须通过 Lens 变得可见。

- **`td get block query`**:
  - 当你对当前“场”产生疑惑时，运行此命令。
  - 它会模拟编译器的解析逻辑，输出当前 Block 在三重解析下的最终指向。
  - **工作流**: 编写 -> 疑惑 -> Query -> 修正 -> 提交。

### 调试哲学

在 Typedown 中，不要猜测。如果你不确定 `[[Ref]]` 指向哪里，或者不确定当前生效的 Schema 是什么，请立即使用工具查询。这种**“编写-查询-修正”**的循环是 Typedown 开发的核心体验。
