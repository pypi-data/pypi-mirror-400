...
Each coding session uses the [Ralph Wiggum technique](https://ghuntley.com/ralph/) for iterative refinement. Instead of exiting on first attempt, Claude iterates within each session until the issue is complete or max iterations (default: 10) is reached.

**How it works:**
1. Claude starts working on an issue
2. When Claude tries to stop, a Stop hook intercepts and checks completion
3. If issues remain open and iterations < max, Claude is prompted to continue
4. Loop exits when the issue is closed or max iterations reached
