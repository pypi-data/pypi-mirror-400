...
```bash
# Run complete assessment (all 136 rules)
aws-cis-assess --profile my-aws-profile --regions us-east-1,us-west-2

# Assess specific Implementation Group
...
# Enterprise multi-region assessment
aws-cis-assess --implementation-groups IG1,IG2,IG3 --regions all --output-format html
```
...
### Docker Deployment (Recommended for Enterprise)

```bash
# Pull production image
docker pull awscisassessment/aws-cis-assessment:latest

# Run with AWS credentials
docker run --rm \
  -v ~/.aws:/home/appuser/.aws:ro \
  -v $(pwd)/reports:/app/output \
  awscisassessment/aws-cis-assessment:latest \
...
- name: Run CIS Assessment
  uses: docker://awscisassessment/aws-cis-assessment:latest
  with:
    args: --implementation-groups IG1,IG2 --output-format json
  env:
...
isort aws_cis_assessment/

# Lint code
flake8 aws_cis_assessment/
pylint aws_cis_assessment/
...
- **Assessment Speed**: <5 minutes for 1000+ AWS resources
- **Memory Usage**: <2GB for enterprise-scale assessments  
- **Concurrent Regions**: Up to 16 regions processed in parallel
- **Error Rate**: <0.1% for AWS API calls with retry mechanisms
- **Scalability**: Tested with 500+ AWS accounts in AWS Organizations

### Resource Requirements by Scale
- **Small Environment** (<100 resources): 512MB RAM, 1 CPU core
- **Medium Environment** (100-1000 resources): 1GB RAM, 2 CPU cores  
- **Large Environment** (1000+ resources): 2GB+ RAM, 4+ CPU cores
- **Enterprise Environment** (Multi-account): 4GB+ RAM, 8+ CPU cores
...
- **[Maintenance Plan](COMPREHENSIVE_MAINTENANCE_PLAN.md)**: Operations and maintenance guidance  
- **[Future Roadmap](FUTURE_RULES_IMPLEMENTATION_PLAN.md)**: Enhancement and expansion plans
- **[Implementation Status](FINAL_IMPLEMENTATION_STATUS.md)**: Complete coverage analysis
- **[API Documentation](docs/)**: Detailed API and integration documentation
...
- **Security Issues**: Responsible disclosure via security@example.com
