...
ALGORITHMVERSIONID1_ANY_OF_SCHEMAS = ["int", "str"]


class AlgorithmVersionId1(BaseModel):
    """
    AlgorithmVersionId1
    """
    ...
    if TYPE_CHECKING:
        ...
        actual_instance: Any
    any_of_schemas: List[str] = Field(ALGORITHMVERSIONID1_ANY_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = AlgorithmVersionId1.construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in AlgorithmVersionId1 with anyOf schemas: int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> AlgorithmVersionId1:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> AlgorithmVersionId1:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into AlgorithmVersionId1 with anyOf schemas: int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
