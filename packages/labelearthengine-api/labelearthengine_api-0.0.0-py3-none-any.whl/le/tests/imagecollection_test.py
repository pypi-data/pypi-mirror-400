...
class ImageCollectionTest(apitestcase.ApiTestCase):
  ...
  def test_image_collection_constructors(self):
    ...
    from_id = le.ImageCollection('abcd')
    self.assertEqual(
        le.ApiFunction.lookup('ImageCollection.load'), from_id.func)
    self.assertEqual({'id': 'abcd'}, from_id.args)

    from_images = le.ImageCollection([le.Image(1), le.Image(2)])
    self.assertEqual(
        le.ApiFunction.lookup('ImageCollection.fromImages'), from_images.func)
    self.assertEqual({'images': [le.Image(1), le.Image(2)]}, from_images.args)

    self.assertEqual(
        le.ImageCollection([le.Image(1)]), le.ImageCollection(le.Image(1)))

    original = le.ImageCollection('foo')
    from_other_image_collection = le.ImageCollection(original)
    self.assertEqual(from_other_image_collection, original)

    l = le.List([le.Image(1)]).slice(0)
    from_list = le.ImageCollection(l)
    self.assertEqual({'images': l}, from_list.args)

    from_computed_object = le.ImageCollection(
        le.ComputedObject(None, {'x': 'y'}))
  ...
  def test_imperative_functions(self):
    ...
    image_collection = le.ImageCollection(le.Image(1))
  ...
  def test_filter(self):
    ...
    collection = le.ImageCollection(le.Image(1))
    noop_filter = le.Filter()
    filtered = collection.filter(noop_filter)
    self.assertIsInstance(filtered, le.ImageCollection)
    self.assertEqual(le.ApiFunction.lookup('Collection.filter'), filtered.func)
    self.assertEqual({
  ...
  def test_first_lookup(self):
    ...
    first = le.ImageCollection(le.Image(1)).first()
    self.assertIsInstance(first, le.Image)
    self.assertEqual(le.ApiFunction.lookup('Collection.first'), first.func)

  def test_prepare_for_export(self):
    """Verifies proper handling of export-related parameters."""
    with apitestcase.UsingCloudApi():
      base_collection = le.ImageCollection(le.Image(1))
  ...
  def test_aggregate_array(self):
    ...
    expression = le.ImageCollection('a').aggregate_array(property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection('a').aggregate_array(property=property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_count(self):
    ...
    expression = le.ImageCollection('a').aggregate_count(property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection('a').aggregate_count(property=property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_count_distinct(self):
    ...
    expression = le.ImageCollection('a').aggregate_count_distinct(property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection('a').aggregate_count_distinct(
  ...
  def test_aggregate_first(self):
    ...
    expression = le.ImageCollection('a').aggregate_first(property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection('a').aggregate_first(property=property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_histogram(self):
    ...
    expression = le.ImageCollection('a').aggregate_histogram(property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection('a').aggregate_histogram(
        property=property_name
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_max(self):
    ...
    expression = le.ImageCollection('a').aggregate_max(property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection('a').aggregate_max(property=property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_mean(self):
    ...
    expression = le.ImageCollection('a').aggregate_mean(property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection('a').aggregate_mean(property=property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_min(self):
    ...
    expression = le.ImageCollection('a').aggregate_min(property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection('a').aggregate_min(property=property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_product(self):
    ...
    expression = le.ImageCollection('a').aggregate_product(property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection('a').aggregate_product(
        property=property_name
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_sample_sd(self):
    ...
    expression = le.ImageCollection('a').aggregate_sample_sd(property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection('a').aggregate_sample_sd(
        property=property_name
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_sample_var(self):
    ...
    expression = le.ImageCollection('a').aggregate_sample_var(property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection('a').aggregate_sample_var(
        property=property_name
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_stats(self):
    ...
    expression = le.ImageCollection('a').aggregate_stats(property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection('a').aggregate_stats(property=property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_sum(self):
    ...
    expression = le.ImageCollection('a').aggregate_sum(property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection('a').aggregate_sum(property=property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_total_sd(self):
    ...
    expression = le.ImageCollection('a').aggregate_total_sd(property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection('a').aggregate_total_sd(
        property=property_name
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_aggregate_total_var(self):
    ...
    expression = le.ImageCollection('a').aggregate_total_var(property_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection('a').aggregate_total_var(
        property=property_name
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_bounds(self):
    ...
    collection = le.ImageCollection('a')
    max_error = 1.1
    proj = 'EPSG:4326'
    expect = make_expression_graph({
        'arguments': {
  ...
  def test_cast(self):
    ...
    expression = le.ImageCollection([le.Image(1)]).cast(band_types, band_order)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection([le.Image(1)]).cast(
  ...
  def test_combine(self):
    secondary = le.ImageCollection('b')
    overwrite = True
    expect = make_expression_graph({
        'arguments': {
            'primary': IMAGES_ONE,
    ...
    expression = le.ImageCollection([le.Image(1)]).combine(secondary, overwrite)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection([le.Image(1)]).combine(
  ...
  def test_copy_properties(self):
    ...
    expression = le.ImageCollection('a').copyProperties(
  ...
  def test_count(self):
    ...
    expression = le.ImageCollection('a').count()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

  def test_distance(self):
    # Inherited from Collection.distance.
    features = le.ImageCollection('a')
    search_radius = 1.1
    max_error = 2.2
    expect = make_expression_graph({
        'arguments': {
  ...
  def test_forma_trend(self):
    covariates = le.ImageCollection('b')
    window_size = 3
    expect = make_expression_graph({
        'arguments': {
    ...
    expression = le.ImageCollection([le.Image(1)]).formaTrend(
  ...
  def test_get_region(self):
    ...
    expression = le.ImageCollection('a').getRegion(
  ...
  def test_load(self):
    ...
    expression = le.ImageCollection.load(id_name, version)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection('a').load(id=id_name, version=version)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

  def test_merge(self):
    images_b = le.ImageCollection('b')
    expect = make_expression_graph({
        'arguments': {
    ...
    expression = le.ImageCollection('a').merge(images_b)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection('a').merge(collection2=images_b)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_mosaic(self):
    ...
    expression = le.ImageCollection('a').mosaic()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_quality_mosaic(self):
    ...
    expression = le.ImageCollection('a').qualityMosaic(quality_band)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection('a').qualityMosaic(qualityBand=quality_band)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_reduce(self):
    ...
    expression = le.ImageCollection('a').reduce(reducer, parallel_scale)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection('a').reduce(
  ...
  def test_to_array(self):
    ...
    expression = le.ImageCollection('a').toArray()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_to_array_per_band(self):
    ...
    expression = le.ImageCollection('a').toArrayPerBand(axis)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.ImageCollection('a').toArrayPerBand(axis=axis)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_to_bands(self):
    ...
    expression = le.ImageCollection('a').toBands()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
