...
REQUIRED_PROFILE_FIELDS = ("base_url",)
...
def walk_files(root: Path) -> list[Path]:
    ignore_patterns = load_aielignore(root)
    out: list[Path] = []
    for p in root.rglob("*"):
        if p.is_dir():
            continue
...
def load_aielignore(root: Path) -> List[str]:
    ignore = [".aiel/", ".aielignore"]
    ignore_path = root / ".aielignore"
...
def should_ignore(path: Path, root: Path, ignore_patterns: List[str] | None = None) -> bool:
    ignore_patterns = ignore_patterns or load_aielignore(root)
    try:
...
def profile_is_set(data: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    if not (prof.get("token_ref") or prof.get("token")):
        return False, f"Profile '{current}' missing: token (token_ref or token)"
