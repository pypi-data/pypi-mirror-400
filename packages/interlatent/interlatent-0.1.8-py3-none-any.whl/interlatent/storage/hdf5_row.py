...
class HDF5RowBackend(StorageBackend):
    ...
    def _layer_dim(self, layer: str, max_channel: int) -> int:
        existing = self._layer_dims.get(layer)
        if existing is not None:
            if max_channel >= existing:
                raise ValueError(
                    f"channel {max_channel} exceeds layer dim {existing} for '{layer}'"
                )
            return existing
        if max_channel >= self._hidden_dim:
            raise ValueError(
                f"channel {max_channel} exceeds hidden_dim {self._hidden_dim}; "
    ...
    def _get_layer_dataset(self, run_grp, layer_id: int, hidden_dim: int, layer_name: str):
        ...
        if key not in layers:
            ...
            is_latent = layer_name.startswith("latent:") or layer_name.startswith("latent_sae:")
            grp.create_dataset(
                "act",
                shape=(0, hidden_dim),
                maxshape=(None, None) if is_latent else (None, hidden_dim),
                dtype=self._act_dtype,
                chunks=(self._chunk_rows, hidden_dim),
            )
            self._layer_dims[grp.attrs["layer_name"]] = int(hidden_dim)
            return grp["act"]
        act = layers[key]["act"]
        layer_name = str(layers[key].attrs.get("layer_name", ""))
        if layer_name:
            self._layer_dims[layer_name] = int(layers[key].attrs.get("hidden_dim", act.shape[1]))
        return act

    def _ensure_act_shape(self, act, rows: int, cols: int) -> None:
        cur_rows, cur_cols = act.shape
        new_rows = max(cur_rows, rows)
        new_cols = max(cur_cols, cols)
        if new_rows == cur_rows and new_cols == cur_cols:
            return
        act.resize((new_rows, new_cols))
    ...
    def _write_activation_rows(self, run_id: str, layer_id: int, rows: Sequence[dict], hidden_dim: int, layer_name: str) -> None:
        if not rows:
            return
        run_grp = self._get_run_group(run_id)
        act = self._get_layer_dataset(run_grp, layer_id, hidden_dim, layer_name)
        rows_sorted = sorted(rows, key=lambda r: r["step"])
        max_step = rows_sorted[-1]["step"]
        self._ensure_act_shape(act, max_step + 1, hidden_dim)
    ...
    def _flush_pending(self, force: bool = False):
        ...
        for (run_id, layer, step), rec in ready:
            ...
            per_run_layer.setdefault((run_id, layer_id), []).append(
                {"step": step, "vec": rec["vec"], "hidden_dim": hidden_dim, "layer": layer}
            )
        ...
        for (run_id, layer_id), rows in per_run_layer.items():
            ...
            layer_name = rows[0]["layer"]
            for row in rows:
                row.pop("hidden_dim", None)
                row.pop("layer", None)
            t_act = time.perf_counter() if self._profile_io else None
            self._write_activation_rows(run_id, layer_id, rows, hidden_dim, layer_name)
            if self._profile_io:
    ...
    def write_events(self, events: Sequence[ActivationEvent]) -> None:
        ...
        for ev in events:
            ...
            if rec is not None:
                if ev.channel >= rec["hidden_dim"]:
                    if ev.layer.startswith("latent:") or ev.layer.startswith("latent_sae:"):
                        new_dim = ev.channel + 1
                        vec = rec["vec"]
                        new_vec = np.zeros(new_dim, dtype=vec.dtype)
                        new_vec[: vec.shape[0]] = vec
                        rec["vec"] = new_vec
                        rec["hidden_dim"] = new_dim
                    else:
                        raise ValueError(
        ...
        for (run_id, layer, step), rec in grouped.items():
            ...
            values = rec["values"]
            if layer.startswith("latent:") or layer.startswith("latent_sae:"):
                layer_dim = max(channels) + 1 if channels else 0
            else:
                layer_dim = self._layer_dim(layer, max(channels) if channels else 0)
            if len(channels) == layer_dim:
                min_ch = min(channels)
                max_ch = max(channels)
                full = min_ch == 0 and max_ch == layer_dim - 1 and len(set(channels)) == layer_dim
            else:
                full = False
            ...
            if full:
                vec = np.zeros(layer_dim, dtype=self._act_dtype)
                vec[np.asarray(channels, dtype=np.int32)] = np.asarray(values, dtype=self._act_dtype)
                layer_id = self._string_id("layers", layer)
                per_run_layer.setdefault((run_id, layer_id), []).append(
                    {"step": step, "vec": vec, "hidden_dim": layer_dim, "layer": layer}
                )
                continue
            ...
            for ch, val in zip(channels, values):
                if ch >= layer_dim:
                    raise ValueError(
                        f"channel {ch} exceeds hidden_dim {layer_dim}; "
                        "set LATENTDB_MAX_CHANNELS to the collector max_channels"
                    )
