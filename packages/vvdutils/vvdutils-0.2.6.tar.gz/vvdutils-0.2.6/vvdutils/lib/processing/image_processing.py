...
def draw_self_intersecting_polygon(mask, polygon_points, color=None):
    # 创建一个临时图像，绘制多边形边界
    temp_mask = np.zeros(mask.shape[:2], dtype=np.uint8)
    poly = np.array(polygon_points, dtype=np.int32)
    cv2.polylines(temp_mask, [poly], True, 255, 1)
    
    # 填充多边形边界内部（使用奇偶规则？）实际上，我们可以使用findContours来获取轮廓
    # 但是，findContours不会使用奇偶规则，而是使用非零环绕规则。
    # 所以，对于自交多边形，我们需要先将其转换为使用奇偶规则的二值图像。
    # 我们可以使用前面提到的扫描线算法，或者使用Shapely。
    
    # 这里，我们使用Shapely来获取正确的轮廓。
    from shapely.geometry import Polygon
    poly_shapely = Polygon(polygon_points)
    if not poly_shapely.is_valid:
        # 使多边形有效
        poly_shapely = poly_shapely.buffer(0)
    
    # 获取外环和内环
    exterior = list(poly_shapely.exterior.coords)
    interiors = [list(interior.coords) for interior in poly_shapely.interiors]
    
    # 绘制
    contours = [np.array(exterior, dtype=np.int32)] + [np.array(interior, dtype=np.int32) for interior in interiors]
    ...
    # 绘制轮廓
    mask = cv2.drawContours(mask, contours, -1, color, thickness=-1, hierarchy=hierarchy)
...
def find_adjacent_points(point, map_img):
    """找到点的相邻骨架点"""
    v, u = point
    adjacent = []
    ...
    for dy in [-1, 0, 1]:
        for dx in [-1, 0, 1]:
            ...
            nv, nu = v + dy, u + dx
            if (0 <= nv < map_img.shape[0] and 
                0 <= nu < map_img.shape[1] and 
                map_img[nv, nu]):
                adjacent.append((nv, nu))
