...
def strings_to_classifier(trees):
    ...
    ee_strings = [le.String(tree) for tree in trees]

    # pass list of ee.Strings to an ensemble decision tree classifier (i.e. RandomForest)
    classifier = le.Classifier.decisionTreeEnsemble(ee_strings)
...
def fc_to_classifier(fc):
    ...
    tree_strings = fc.aggregate_array("tree").map(
        lambda x: le.String(x).replace(
            "#", "\n", "g"
        )  # expects that # is ecoded to be a return
    )
    # pass list of ee.Strings to an ensemble decision tree classifier (i.e. RandomForest)
    classifier = le.Classifier.decisionTreeEnsemble(tree_strings)
...
def csv_to_classifier(in_csv):
    ...
    null_island = le.Geometry.Point([0, 0])
    features = [le.Feature(null_island, {"tree": line.strip()}) for line in lines]
    rf_fc = le.FeatureCollection(features)
