...
class DatabaseConf():
    def __init__(self,dbname='maxbus'):
        ...
        self._port = sc.port
        # 数据库类型，支持mysql,mssql,默认mssql
        self._dbtype = sc.get_keyvalue('%s_dbdriver' % dbname,consul=sc.consul)
        # 只有 mssql 才需要指定pylib, 可以指定为 pymssql 或 pyodbc , mysql 只能指定为 mysqldb
        self._dbpylib = os.getenv('SQL_PYLIB','pymssql') if self._dbtype == 'mssql' else os.getenv('SQL_PYLIB','mysqldb')
        # 如果mssql的pyodbc ,值为:?driver=ODBC+Driver+18+for+SQL+Server&trustServerCertificate=yes
    ...
    def sqlalchemy_database_uri(self):
        # mssql 资料库如果是 pyodbc 则需要指定 driver=SQL+Server
        return f'{self._dbtype}+{self._dbpylib}://{self._user}:{self._password}@{self._host}:{self._port}/{self._name}{self.sql_driver}'
    ...
    def is_null(self):
        ...
        '''
        return 'ifnull' if self.sqlalchemy_database_uri().startswith('mysql') else 'isnull'
...
class KongConf():
    ...
    取kong的配置
    '''
    def __init__(self,tag = 'kong'):
        '''
        取kong的配置
        '''
        sc = ServiceConf('kong', tag=tag)
        self._ip = sc.address
        self._port = sc.port
