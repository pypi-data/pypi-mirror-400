...
class SelectStatement(Selectable):
    ...
    def column_states(self) -> ColumnStateDict:
        if self._column_states is None:
            if not self.projection and not self.has_clause:
                ...
                super().column_states  # noqa: B018 # will assign value to self._column_states
    ...
    def sql_query(self) -> str:
        ...
        offset_clause = (
            f"{analyzer_utils.OFFSET}{self.offset}" if self.offset else EMPTY_STRING
        )
...
def derive_column_states_from_subquery(
    ...
    for c in cols:
        ...
        if c_name is None:
            return None
        quoted_c_name = quote_name(c_name)
