...
class Dialogue:
    ...
    curr_speaker : Entity
        The player whose self is executed on the current turn.
    counter : int
        The number of turns taken in the dialogue. One turn is when all participants finish uttering.
    next_policy_id : int
        The id of the next self to be executed. The id starts from 0 to len(self.policies)-1
    ...
    use_generator : bool
        ...
        useful not to run the goal generator if the self returns the same goal. If this field is False,
    ...
    max_safety : int
        The max_safety is used to stop the dialogue just in case some rule-based self goes into an infinite
    ...
    def save_state(self):
        ...
        return (self.dia_generator.save_state(),
                copy.copy(self.utterances),
    ...
    def recover_state(self, state):
        ...
        del self.utterances[:]
        self.utterances.extend(state[1])
        del self.policies[:]
        ...
        self.use_generator = state[12]
        self.entities_descriptions.clear()
        self.entities_descriptions.update(state[13])
        self.dia_generator.recover_state(state[0])
            ...
            if self.is_over():
                if self.feedback:
                    if self.curr_reward == 1:
                        ...
                        utter = tsentences.be(["The", "dialogue"],
                                              "is",
                                              None,
                                              "successful")
                    else:
                        utter = tsentences.be(["The", "dialogue"],
                                              ...
                                              "not",
                                              "successful")
                    self.dia_generator.execute_utters([utter])

                #random_entity =
                #utter = tsentences.be()
                #self.dia_generator.execute_utters([utter], self.random_gen)
    ...
    def step(self):
        """
        Steps into the current self and executes it.
        ...
        if isinstance(utter, list):
            utter = self.dia_generator.random_gen.choice(utter)
    ...
    def player_utters(self, utter):
        ...
        obs = self.dia_generator.execute_utters([utter])
        self.utterances += obs
        #else:
        #    obs = None
    ...
    def is_over(self):
        ...
        self.dialogue_over = (((self.counter >= self.max_episode_length if self.max_episode_length is not None else False)
                              or self.curr_reward == 1 or self.curr_reward == -1 or self.interrupt is True)
    ...
    def replace_player_policies(self, new_policies):
        """ Replaces the player policies with new ones.
            ...
            models because the agent's rule-based self is replaced with the model-based.
        """
        old_policies = []
        counter = 0
