...
from minmo.gemini_cli_wrapper import GeminiCLIWrapper, TaskPlan, ProjectAnalysis
from minmo.claude_wrapper import ClaudeCodeWrapper, TaskResult, TaskStatus

# GeminiWrapper 별칭 (하위 호환성)
GeminiWrapper = GeminiCLIWrapper
from minmo.scribe_mcp import call_log_event as log_event, call_get_state as get_state, call_update_todo as update_todo
...
def ensure_minmo_data_dir(project_path: str | Path | None = None) -> Path:
    """
    .minmo 데이터 디렉토리가 존재하도록 보장합니다.

    Args:
        project_path: 프로젝트 루트 경로 (None이면 현재 작업 디렉토리 사용)
    ...
    minmo_dir = base_path / MINMO_DATA_DIR
    minmo_dir.mkdir(parents=True, exist_ok=True)
    return minmo_dir
...
class MinmoOrchestrator:
    ...
    def __init__(
        ...
        Args:
            ...
            on_output: 출력 콜백 (로그 메시지)
            on_error: 에러 콜백
            on_status: 상태 업데이트 콜백 (인플레이스 업데이트용)
            verbose: 상세 출력 여부
        """
        self._working_directory = working_directory

        # .minmo 데이터 디렉토리 자동 생성
        self._minmo_data_dir = ensure_minmo_data_dir(working_directory)

        self.scribe = ScribeMCP()
        self.commander: GeminiCLIWrapper | None = None
    ...
    def _ensure_commander(self) -> GeminiCLIWrapper:
        """Commander(Gemini)가 초기화되었는지 확인하고 반환합니다."""
        if self.commander is None:
            self.commander = GeminiCLIWrapper(
    ...
    def start_loop(
        ...
        detailed_analysis = ""
        if isinstance(plan, TaskPlan):
            detailed_analysis = plan.detailed_analysis or ""

        return {
            "goal": user_goal,
            "clarification": clarified,
            "plan": asdict(plan) if isinstance(plan, TaskPlan) else plan,
            "detailed_analysis": detailed_analysis,  # 지휘관의 상세 분석 보고서
            "results": [
                {
                    "task_id": r.task_id,
