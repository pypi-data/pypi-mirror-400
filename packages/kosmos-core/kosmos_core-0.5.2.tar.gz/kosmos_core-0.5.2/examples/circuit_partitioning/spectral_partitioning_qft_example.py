from kosmos.partitioning.algorithms.spectral_partitioner import SpectralPartitioner
...
def spectral_partition_qft_example() -> None:
    """Run example for spectral circuit partitioning on a QFT circuit."""
    RNG.initialize(seed=1)

    output = ["\n=== Circuit Characteristics ==="]

    # Load QFT circuit from MQT Bench
    benchmark = MQTBenchCircuit(circuit_type="qft", num_qubits=64)
    ...
    partitioner = SpectralPartitioner(network=None, num_partitions=2)
    assignments = partitioner.partition(circuit)

    output.extend(
        partitioning_result_description(
            "Spectral (Fiedler Vector)", assignments, benchmark.num_gates
        )
    )
...
if __name__ == "__main__":
    spectral_partition_qft_example()
