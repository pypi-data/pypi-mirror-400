...
def mg_get_code(code):
    ...
    if len(mg_code_list) < 2:
        dde = pkg_resources.resource_filename("yinhedata", "mg_code.csv")
        df_codes = pd.read_csv(dde)
        df_codes['纯代码'] = df_codes['code'].apply(lambda x: x.split('.')[-1])
        mg_code_list = df_codes.set_index('纯代码')['code'].to_dict()
    if code in mg_code_list:
...
def mg_history_minute(code, period):
    ...
    if df.empty:
        df = wb.stock_us_hist_min_em('106.'+code)
    if df.empty:
        df = wb.stock_us_hist_min_em('107.' + code)
    if df.empty:
        return pd.DataFrame()
