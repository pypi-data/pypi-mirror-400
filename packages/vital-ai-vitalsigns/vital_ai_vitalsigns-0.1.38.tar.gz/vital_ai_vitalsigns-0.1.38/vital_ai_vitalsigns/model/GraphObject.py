...
class GraphObject(metaclass=GraphObjectMeta):
    ...
    def __get_pydantic_json_schema__(cls, core_schema, handler):
        """Pydantic v2 JSON schema generation method."""
        if not PYDANTIC_V2_AVAILABLE:
            raise ImportError("Pydantic v2 is required for this functionality. Install with: pip install pydantic>=2.0")
        
        # Return a simple object schema for GraphObject classes
        # This allows OpenAPI schema generation to work
        return {
            "type": "object",
            "title": cls.__name__,
            "description": f"VitalSigns {cls.__name__} object",
            "properties": {
                "URI": {
                    "type": "string",
