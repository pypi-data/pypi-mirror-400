...
class JoinTest(apitestcase.ApiTestCase):
  ...
  def test_join_no_args(self):
    ...
    with self.assertRaisesRegex(TypeError, message):
      le.Join()  # pytype:disable=missing-parameter
  ...
  def test_apply(self):
    ...
    expression = le.Join.inverted().apply('a', 'b', le.Filter.eq('c', 1))
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Join.inverted().apply(
        primary='a', secondary='b', condition=le.Filter.eq(name='c', value=1)
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_inner_join(self):
    first = '1st'
    second = '2nd'
    join = le.Join.inner(first, second)
    self.assertEqual({'value': 'fakeValue'}, join.getInfo())

    join_func = le.ApiFunction.lookup(INNER)
    ...
    join_cast_result = json.loads(le.Join(join).serialize())
  ...
  def test_inner(self):
    ...
    expression = le.Join.inner(primary_key, secondary_key, measure_key)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Join.inner(
  ...
  def test_save_best(self):
    ...
    expression = le.Join.saveBest(match_key, measure_key, outer)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Join.saveBest(
  ...
  def test_simple_join(self):
    ...
    join_func = le.ApiFunction.lookup(SIMPLE)
    self.assertEqual(join_func, join.func)
