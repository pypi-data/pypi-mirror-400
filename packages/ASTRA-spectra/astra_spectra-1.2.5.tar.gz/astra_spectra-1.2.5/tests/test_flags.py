from ASTRA.status.flags import KW_WARNING, VALID, Flag
...
def test_Status():
    ...
    assert stat.has_flag(VALID)
    assert stat.number_warnings == 0

    warningflag = KW_WARNING("KW foo meets the bad value")
    stat.store_warning(warningflag)
    assert stat.check_if_warning_exists(warningflag)
    warningflag = KW_WARNING("KW bar meets the bad value")
