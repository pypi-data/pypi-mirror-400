...
class ModelBenchmark:
    ...
    def plot_results(self, results, stat_results, detailed_profile=None):
        ...
        try:
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            plot_file = figures_dir / f"benchmark_dual_lb_plot_{timestamp}.png"
...
# =============================================================================
# OPENML CC-18 DATASET LOADING
# =============================================================================

class OpenMLCC18Loader:
    """Load and filter OpenML CC-18 binary classification datasets."""
    ...
    def get_binary_datasets(self, min_samples=100, max_samples=50000, 
                           exclude_image=True, limit=None):
        """
        ...
        """
        print("\n" + "="*70)
        print("LOADING OPENML CC-18 BINARY CLASSIFICATION DATASETS")
        print("="*70)
        
        # Get CC-18 suite (ID 99)
        suite = self.openml.study.get_suite(suite_id=99)
        tasks = suite.tasks
        
        datasets = []
        excluded_reasons = defaultdict(int)
        
        print(f"\nProcessing {len(tasks)} tasks from CC-18 suite...")
        ...
        for task_id in tasks:
            ...
            try:
                # Get task metadata
                task = self.openml.tasks.get_task(task_id, download_data=False)
                
                # Get dataset metadata
                ds_meta = self.openml.datasets.get_dataset(task.dataset_id, download_data=False)
                
                # Check if binary
                class_labels = task.class_labels
                if class_labels is None or len(class_labels) != 2:
                    excluded_reasons['not_binary'] += 1
                    continue
                
                # Check size constraints
                n_samples = ds_meta.qualities.get('NumberOfInstances', 0)
                if n_samples < min_samples or n_samples > max_samples:
                    excluded_reasons['size'] += 1
                    continue
                
                # Exclude image datasets
                if exclude_image and any(img_name in ds_meta.name.lower() 
                                       for img_name in ['mnist', 'fashion', 'cifar', 'image']):
                    excluded_reasons['image'] += 1
                    continue
                
                # Categorize dataset
                n_features = ds_meta.qualities.get('NumberOfFeatures', 0)
                n_numeric = ds_meta.qualities.get('NumberOfNumericFeatures', 0)
                n_nominal = ds_meta.qualities.get('NumberOfNominalFeatures', 0)
                ...
                # We'll need to download to get exact ratio, but estimate from metadata if available
                imbalance_ratio = None  # Will be calculated after download
                ...
                print(f"  ✓ {ds_meta.name}: {n_samples} samples, {n_features} features ({feature_type}), {size_category}")
                
            except Exception as e:
                excluded_reasons['error'] += 1
                continue
    ...
    def load_dataset(self, dataset_id):
        ...
        dataset = self.openml.datasets.get_dataset(dataset_id, download_data=True)
        ...
        if len(np.unique(y)) > 2:
            # Convert to binary (keep most common as 0, rest as 1)
        ...
        return X, y, categorical_cols, imbalance_ratio, dataset.name
...
if __name__ == "__main__":
    try:
        ...
        print("ERROR: openml package required for CC-18 benchmarking.")
        print("Install with: pip install openml")
        raise
    
    from sklearn.preprocessing import LabelEncoder
    
    # Initialize OpenML CC-18 loader
    loader = OpenMLCC18Loader()
    
    # Get binary datasets with diverse characteristics
    print("\n" + "="*70)
    print("  OPENML CC-18 BENCHMARK SUITE")
    print("  Filtering datasets by size, feature type, and class imbalance")
    print("="*70)
    
    datasets_meta = loader.get_binary_datasets(
        min_samples=100,
        max_samples=50000,
        exclude_image=True,
    ...
    if not datasets_meta:
        print("\nNo datasets found. Using fallback dataset...")
        ...
        benchmark = ModelBenchmark(
            X=X, y=y, categorical_cols=categorical_cols,
            n_trials=50, cv_folds=5, n_runs=10, n_jobs=4,
            hp_sample_threshold=2000, hp_sample_size=1500, include_tabpfn=False
        )
        ...
        # Categorize datasets
        datasets_by_size = {'small': [], 'medium': [], 'large': []}
        datasets_by_feature = {'numeric': [], 'categorical': [], 'mixed': []}
        datasets_by_imbalance = {'balanced': [], 'moderate': [], 'imbalanced': []}
        ...
        for ds_meta in datasets_meta:
            try:
                print(f"\n{'='*70}")
                print(f"PROCESSING: {ds_meta['name']}")
                print(f"{'='*70}")
                
                # Load dataset
                X, y, categorical_cols, imbalance_ratio, dataset_name = loader.load_dataset(ds_meta['dataset_id'])
                
                n_samples = len(X)
                n_features = len(X.columns)
                n_numeric = len([c for c in X.columns if c not in categorical_cols])
                n_categorical = len(categorical_cols)
                feature_type = ds_meta['feature_type']
                imbalance_cat = 'balanced' if imbalance_ratio >= 0.4 else ('moderate' if imbalance_ratio >= 0.2 else 'imbalanced')
                
                print(f"  Size: {ds_meta['size_category']} ({n_samples} samples)")
                ...
                datasets_by_size[ds_meta['size_category']].append(ds_meta['name'])
                datasets_by_feature[ds_meta['feature_type']].append(ds_meta['name'])
                datasets_by_imbalance[imbalance_cat].append(ds_meta['name'])
                
                # Run benchmark
                print(f"\n  Running benchmark...")
                benchmark = ModelBenchmark(
                    X=X, y=y, categorical_cols=categorical_cols,
                    n_trials=200,  # Reduced for multi-dataset run
                    cv_folds=5,
                    n_runs=30,    # Reduced for multi-dataset run
                    n_jobs=8,
                    hp_sample_threshold=50000,
                    hp_sample_size=50000,
                ...
                for model_name in results:
                    ...
                    results[model_name]['dataset_size'] = ds_meta['size_category']
                    results[model_name]['feature_type'] = ds_meta['feature_type']
                    results[model_name]['imbalance_ratio'] = imbalance_ratio
                    results[model_name]['n_samples'] = len(X)
                    results[model_name]['n_features'] = len(X.columns)
                ...
                all_results.append({
                    ...
                    'additional_results': getattr(benchmark, 'additional_results', {}),
                    'metadata': ds_meta
                })
                
                # Save individual dataset results
                benchmark.save_results(results, stat_results, 
                                     filename_prefix=f"cc18_{ds_meta['dataset_id']}_{dataset_name}")
                ...
                print(f"\n  ✗ Error processing {ds_meta['name']}: {e}")
                import traceback
                traceback.print_exc()
                continue
        ...
        results_dir.mkdir(exist_ok=True)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        summary_file = results_dir / f"cc18_aggregate_summary_{timestamp}.csv"
