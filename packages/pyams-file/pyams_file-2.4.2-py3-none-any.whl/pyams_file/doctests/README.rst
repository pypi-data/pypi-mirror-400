...
Loading images from unknown formats
-----------------------------------

In some cases, a file can be detected as an image which can't be read with PIL, so an error is raised
when setting property value:

    >>> error_content = MyContent()
    >>> locate(error_content, app)
    >>> error_img_name = os.path.join(sys.modules['pyams_file.tests'].__path__[0], 'test_image_error.dwg')
    >>> with open(error_img_name, 'rb') as file:
    ...     error_content.data = file
    Traceback (most recent call last):
    ...
    PIL.UnidentifiedImageError: cannot identify image file <...>

A solution to handle this use case is to add an argument called *accept_unknown_image_as_file* when
defining the file property, as in:

    >>> from pyams_file.property import FileProperty
    >>> class AnotherTestContentClass(MyContent):
    ...     data = FileProperty(IMyInterface['data'], accept_unknown_image_as_file=True)

    >>> from pyams_file.tests import MyErrorSupportingContent

    >>> error_content = MyErrorSupportingContent()
    >>> locate(error_content, app)
    >>> with open(error_img_name, 'rb') as file:
    ...     error_content.data = file

In this situation, the image is replaced with a simple File class:

    >>> error_content.data
    <pyams_file.file.File object at 0x...>
    >>> error_content.data.content_type
    'image/vnd.dwg'

Be warned however that in this case, interface field must be defined as a FileField and not as an ImageField, as
image fields only accept valid images:

    >>> with open(error_img_name, 'rb') as file:
    ...     error_content.img_data = file
    Traceback (most recent call last):
    ...
    zope.schema._bootstrapinterfaces.WrongType: (<pyams_file.file.File object at 0x...>, <InterfaceClass pyams_file.interfaces.IBaseImageFile>, 'img_data')
