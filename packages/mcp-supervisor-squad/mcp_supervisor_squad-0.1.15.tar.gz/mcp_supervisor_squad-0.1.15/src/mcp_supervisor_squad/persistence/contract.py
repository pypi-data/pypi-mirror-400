...
def write_repo_contract(*, layout: StorageLayout, cfg: RepoConfig, package_version: str) -> Path:
    ...
    payload: dict[str, Any] = {
        ...
        "workflow": {
            ...
            "phases": ["planner_run", "coder_run", "integrate", "done", "cleanup", "error"],
            "required_parameters": {
            ...
            "notes": [
                ...
                "squad_status enforces its wait time server-side (blocks up to 60s) when polled too quickly.",
                "Planner has a hard timeout (120s) to avoid long-running scans; rerun with a tighter query or adjust config/model if needed.",
                "Planner outputs a short requirements summary; coder starts automatically after a ~15s review window unless canceled.",
