...
class FundamentalTheoremVisualizer:
  ...
  def riemann_sum_visualization(self, func_str: str = "x**2", interval: Tuple[float, float] = (0, 2),
    ...
    for i, (x_rect, y_rect) in enumerate(zip(data['x_rects'], data['y_rects'])):
      rect = patches.Rectangle(
        (x_rect, 0), data['dx'], y_rect, linewidth=1, edgecolor='red', facecolor='lightblue', alpha=0.6
      )
      ax1.add_patch(rect)
    ...
    if data['exact_integral'] is not None:
      ax2.axhline(
        y=data['exact_integral'], color='green', linestyle='--', label=f'Exact = {data["exact_integral"]:.6f}'
      )
  ...
  def fundamental_theorem_part1_demo(self, func_str: str = "2*x", lower_bound: float = 0) -> None:
    ...
    ax2.plot(data['x_vals'], data['F_vals'], 'g-', linewidth=2, label=f'F(x) = ∫[{lower_bound} to x] f(t) dt')
    ax2.set_xlabel('x')
    ax2.set_ylabel('F(x)')
  ...
  def fundamental_theorem_part2_demo(self, func_str: str = "x**2", interval: Tuple[float, float] = (0, 2)) -> None:
    ...
    plt.fill_between(x_area, y_area, alpha=0.3, color='lightblue', label=f'Area = {data["numerical_integral"]:.4f}')
  ...
  def mean_value_theorem_demo(self, func_str: str = "x**3 - 3*x**2 + 2*x", interval: Tuple[float, float] = (0, 3)) -> None:
    '''
  ...
  def net_change_theorem_demo(self, rate_func_str: str = "2*x + 1", interval: Tuple[float, float] = (0, 3)) -> None:
    '''
...
class FundamentalTheorem:
  ...
  def mean_value_theorem_demo(
    self, func_str: str = "x**3 - 3*x**2 + 2*x", interval: Tuple[float, float] = (0, 3)
  ) -> None:
    self.visualizer.mean_value_theorem_demo(func_str, interval)

  def net_change_theorem_demo(
    self, rate_func_str: str = "2*x + 1", interval: Tuple[float, float] = (0, 3)
  ) -> None:
