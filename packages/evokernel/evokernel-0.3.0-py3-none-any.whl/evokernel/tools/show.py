...
def execute_show(
    ...
    if not best_code:
        yield Message(
            "system", f"No best program found yet in {run_dir}\nStatus: {status}"
        )
        return
    ...
    if status.get("iteration"):
        status_line.append(
            f"Iteration: {status['iteration']}/{status.get('max_iterations', '?')}"
        )
...
""",
    ...
    parameters=[
        ...
        Parameter(
            name="diff",
            type="string",
            description="Show diff (true) or full code (false)",
            required=False,
        ),
    ],
)
