...
from spacedata.models.donki import DONKIDateRangeInput, DONKIGst
...
async def _get_response(
    ...
    response = await resource.client.make_request(
        method="GET",
        url=f"{resource.client.settings.main_base_url}/DONKI/GST",
        params=params,
    )
    return [DONKIGst.model_validate(item) for item in response]
...
async def gst(
    ...
    **kwargs,
) -> SpaceDataResult[DONKIGst]:
    self.client.logger.debug("Calling gst endpoint with input_data: %s", input_data)
