...
def main():
    ...
    aa("--when_ready_dir", default=None, help="store host:port to this file when ready")
    aa("--ipv4", default=False, action="store_true", help="force ipv4")
    aa("--rank", default=0, type=int, help="rank used as index in the client table")
    ...
    rpc.run_server(
        lambda s: MyServer(s, index), args.port, report_to_file=when_ready, v6=not args.ipv4
    )
...
class SplitPerListIndex:
    ...
    def set_nprobe(self, nprobe):
        ...
        self.pool.map(lambda i: self.sub_indexes[i].set_nprobe(nprobe), range(self.ni))
    ...
    def set_prefetch_nthread(self, nt):
        ...
        self.pool.map(lambda idx: idx.set_prefetch_nthread(nt), self.sub_indexes)
    ...
    def balance_lists(self, list_nos):
        ...
        if self.verbose:
            print(
                ...
                % (bins.min(), bins.max(), bins.sum(), bins.sum() * (self.code_size + 8) / 2**20)
            )
        self.nscan = bins.sum()
    ...
    def search(self, x, k):
        ...
        def do_query(i):
            ...
            t0 = time.time()
            Di, Ii = sub_index.ivf_search_preassigned(xqo, list_nos_i, coarse_dis, k)
            # print(list_nos_i, Ii)
            if self.verbose:
