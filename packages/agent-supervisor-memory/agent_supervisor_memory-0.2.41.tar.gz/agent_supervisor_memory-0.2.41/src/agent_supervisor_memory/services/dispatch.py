...
class CodexDispatchConfig:
    ...
    prune_on_job_finish: bool = True
    prune_on_startup: bool = False
    retain_max_jobs_per_bucket: int = DEFAULT_DISPATCH_RETAIN_MAX_JOBS_PER_BUCKET
    retain_max_age_days: int = DEFAULT_DISPATCH_RETAIN_MAX_AGE_DAYS
    retain_keep_failed: bool = True
...
class CodexDispatch:
    ...
    def _prune_candidates(self, *, now: datetime) -> tuple[list[Path], list[str]]:
        """
        Return (to_delete, messages) for pruning this bucket.

        Deletes:
          - completed/canceled jobs
          - failed jobs only if retain_keep_failed is False
          - never deletes running or jobs with alive pid
        Order:
          1) Age cutoff (older than retain_max_age_days)
          2) Count cutoff (keep newest retain_max_jobs_per_bucket)
        """
        jobs_dir = self.jobs_dir
        max_age_days = max(0, int(self._config.retain_max_age_days))
        max_jobs = max(0, int(self._config.retain_max_jobs_per_bucket))
        keep_failed = bool(self._config.retain_keep_failed)
        messages: list[str] = []
        ...
        try:
            for child in jobs_dir.iterdir():
                ...
                pid = meta.get("pid")
                if status == "running" or _pid_alive(pid):
                    continue
                if status not in ("completed", "canceled", "failed"):
                    continue
                if status == "failed" and keep_failed:
                    continue
                finished_at = _parse_iso_ts(meta.get("finished_at"))
                created_at = _parse_iso_ts(meta.get("created_at"))
                ts = finished_at or created_at
                if not ts:
                    try:
            ...
            messages.append(f"prune scan error: {e}")
            return [], messages
        ...
        # Keep newest max_jobs_per_bucket; delete the rest (excluding already selected)
        if max_jobs > 0:
            remaining = [e for e in entries if e not in to_delete]
            sorted_remaining = sorted(
                remaining,
                key=lambda e: e.get("ts") or datetime.fromtimestamp(0, tz=timezone.utc),
                reverse=True,
            )
            excess = sorted_remaining[max_jobs:]
        ...
        for entry in to_delete:
            ...
            if key in seen:
                continue
            seen.add(key)
            msg_status = entry.get("status")
            msg_ts = entry.get("ts")
            if msg_ts and isinstance(msg_ts, datetime):
                ts_str = msg_ts.isoformat()
            else:
                ts_str = "unknown"
            messages.append(f"prune candidate: {job_id} status={msg_status} ts={ts_str}")
            unique_paths.append(path)
        return unique_paths, messages

    def prune_bucket(self) -> None:
        """
        Prune jobs within this bucket directory according to config.

        Uses lockfile `.prune.lock` to avoid concurrent prune.
        Deletes by renaming to `.__deleting__<ts>` then recursive removal.
        """
        if not self._config.prune_enabled:
            return
        jobs_dir = self.jobs_dir
        lock_path = jobs_dir / ".prune.lock"
        try:
        ...
        verbose = bool(self._config.prune_verbose)
        dry_run = bool(self._config.prune_dry_run)
        try:
            now = datetime.now(timezone.utc)
            candidates, messages = self._prune_candidates(now=now)
            if verbose:
                for msg in messages:
            ...
            for path in candidates:
                tmp_name = f".__deleting__{path.name}_{int(time.time() * 1000)}"
                tmp_path = path.parent / tmp_name
                try:
                    if verbose or dry_run:
                        print(f"prune delete: {path}", file=sys.stderr)
                    if dry_run:
                        continue
                    if path.exists():
                    ...
                    if verbose:
                        print(f"prune error deleting {path}: {e}", file=sys.stderr)
                    continue
        finally:
            try:
    ...
    def start(
        ...
        *,
        ...
        # Idempotency/attach: if job already exists on disk, do not start a second process.
        ...
        def run() -> None:
            try:
                ...
                if self._config.prune_enabled and self._config.prune_on_job_finish:
                    try:
                        self.prune_bucket()
                    except Exception:
                        if self._config.prune_verbose:
                            print(f"prune on finish failed for {job_id}", file=sys.stderr)
            except Exception as e:  # pragma: no cover - defensive
...
def prune_all_buckets(dispatch_base_dir: Path, *, prune_settings: DispatchPruneSettings) -> None:
    """
    Best-effort prune across all bucket directories under the dispatch base dir.
    """
    if not prune_settings.enabled or not prune_settings.prune_on_startup:
        return
    try:
