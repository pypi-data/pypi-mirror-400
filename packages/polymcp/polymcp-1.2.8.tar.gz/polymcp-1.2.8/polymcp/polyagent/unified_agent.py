...
Focus on what was accomplished, not how it was done."""
    ...
    def _extract_previous_results(self, action_history: List[Dict]) -> str:
        """Extract previous results - supports both HTTP and stdio formats."""
        ...
        for action in reversed(action_history[-5:]):
            if action['result'].get('status') == 'success':
                ...
                if isinstance(result_data, dict):
                    # Pattern 1: MCP stdio format (content array)
                    ...
                    # Pattern 2: HTTP format (flat dict with data fields)
                    else:
                        exclude_fields = {'status', 'success', '_server_url', '_server_type',
                                          '_skill_confidence', '_parameters', '_reasoning',
                                          'encoding', 'format'}
                        ...
                        for field, value in result_data.items():
                            ...
                            if value is not None:
                                if isinstance(value, str):
                                    # Skip large base64 data
                                    if len(value) > 200 and value.isalnum():
                                        content_parts.append(f"{field}: [base64 data]")
                                    elif len(value) > 500:
                                        content_parts.append(f"{field}: {value[:500]}...")
                                    else:
                                    ...
                                    if len(value) <= 5:
                                        list_str = ", ".join(str(v)[:100] for v in value)
                                        content_parts.append(f"{field}: [{list_str}]")
                                    else:
                                        list_str = ", ".join(str(v)[:100] for v in value[:5])
                                        content_parts.append(f"{field}: [{list_str}, ... +{len(value) - 5} more]")
                                elif isinstance(value, dict):
                                    nested_str = json.dumps(value, default=str)[:200]
                                    content_parts.append(f"{field}: {nested_str}")

                elif isinstance(result_data, str):
                    content_parts.append(result_data[:500])
        ...
        if results_text:
            return "PREVIOUS TOOL RESULTS (use these values):\n" + "\n---\n".join(results_text)
        else:
            return "Previous actions completed but no detailed output available."

    def _select_next_action(self, user_message: str, action_history: List[Dict], all_tools: List[Dict]) -> Optional[
        Dict[str, Any]]:
        ...
        if not action_history:
            ...
            for action in action_history[-5:]:  # Last 5 actions
                status = "[OK]" if action['result'].get('status') == 'success' else "Ã¢Å“â€”"
        ...
        for i, tool in enumerate(all_tools):
            ...
            blocked = " [BLOCKED]" if any(
                blocked_key.startswith(tool['name'] + ":") for blocked_key in blocked_actions) else ""
...
JSON only:"""
    ...
    async def _should_continue(self, user_message: str, action_history: List[Dict]) -> Tuple[bool, str]:
        ...
        for a in recent_actions:
            status = "success" if a['result'].get('status') == 'success' else "failed"
...
JSON only:"""
    ...
    def _generate_final_response(self, user_message: str, action_history: List[Dict]) -> str:
        """Generate final response - supports both HTTP and stdio formats."""
        if not action_history:
                                ...
                                try:
                                    ...
                                    if isinstance(parsed, dict):
                                        for k, v in parsed.items():
                                            if v is not None and k not in {'status', 'success'}:
                                                text_parts.append(f"{k}: {v}")
                                    else:
                                        text_parts.append(str(parsed))
                                except (json.JSONDecodeError, TypeError):
                                    text_parts.append(str(text_val)[:500])
                            elif isinstance(item, str):
                                text_parts.append(item[:500])

                        if text_parts:
                            combined = "\n    ".join(text_parts)
                            results_data.append(f"Step {step_num} ({tool_name}):\n    {combined}")
                            continue

                    # Pattern 2: HTTP format (flat dict with data fields)
                    meaningful_data = []
                    exclude_fields = {
                        'status', 'success', 'encoding', 'format',
                        '_server_url', '_server_type', '_skill_confidence',
                        '_parameters', '_reasoning'
                    }

                    for field, value in result_content.items():
                        if field in exclude_fields or value is None:
                            continue

                        if isinstance(value, str):
                            if len(value) > 200 and value.isalnum():
                                meaningful_data.append(f"{field}: [base64 data]")
                            elif len(value) > 500:
                                meaningful_data.append(f"{field}: {value[:500]}...")
                            else:
                                meaningful_data.append(f"{field}: {value}")
                        elif isinstance(value, (bool, int, float)):
                            meaningful_data.append(f"{field}: {value}")
                        elif isinstance(value, list):
                            if len(value) == 0:
                                meaningful_data.append(f"{field}: []")
                            elif len(value) <= 5:
                                meaningful_data.append(f"{field}: {value}")
                            else:
                                meaningful_data.append(f"{field}: [{len(value)} items]")
                        elif isinstance(value, dict):
                            nested_str = json.dumps(value, default=str)[:200]
                            meaningful_data.append(f"{field}: {nested_str}")

                    if meaningful_data:
                        combined = "\n    ".join(meaningful_data)
                        results_data.append(f"Step {step_num} ({tool_name}):\n    {combined}")
                    else:
                        results_data.append(f"Step {step_num} ({tool_name}): Completed")

                elif isinstance(result_content, str):
                    results_data.append(f"Step {step_num} ({tool_name}): {result_content[:500]}")
                else:
                    results_data.append(f"Step {step_num} ({tool_name}): {result_content}")

            else:
                error = result.get('error', 'Unknown error')
                results_data.append(f"Step {step_num} ({tool_name}): FAILED - {error}")

        results_text = "\n\n".join(results_data) if results_data else "Actions were executed."
...
YOUR TASK: Write a natural response that includes the specific data extracted (text, titles, counts, URLs, etc.). Be specific with the actual values found.
...
Response:"""
    ...
    async def run_async(self, user_message: str, max_steps: int = 10) -> str:
        ...
        for step in range(max_steps):
            ...
            # Log result with actual data
            if self.verbose:
                status = tool_result.get('status', 'unknown')
                if status == 'success':
                    print(f"Success")
                    # Show actual result data for debugging
                    result_data = tool_result.get('result', {})
                    if isinstance(result_data, dict):
                        preview = {k: str(v)[:100] for k, v in result_data.items()
                                   if k not in {'status', 'success'} and v is not None}
                        if preview:
