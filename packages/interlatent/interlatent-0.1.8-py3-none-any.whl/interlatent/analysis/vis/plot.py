...
def fetch_activations(
    ...
    *,
    ...
    """
    dbi = db if isinstance(db, LatentDB) else LatentDB(db)
    ...
    if prompt_index is not None:
        p_idx = meta.get("prompt_index")
        if p_idx is not None:
            mask &= np.asarray(p_idx) == prompt_index
    if prompt_like:
    ...
    if not mask.any():
        # Fallback: use the first available prompt slice for this layer.
        if "prompt_index" in meta and len(meta["prompt_index"]):
            first_idx = int(np.asarray(meta["prompt_index"])[0])
            mask = np.asarray(meta["prompt_index"]) == first_idx
        elif "prompt_id" in meta and len(meta["prompt_id"]):
            first_id = int(np.asarray(meta["prompt_id"])[0])
            mask = np.asarray(meta["prompt_id"]) == first_id
        else:
            return []
    idx = np.where(mask)[0]
    vals = x[idx, channel]
