...
class Team:
    ...
    def __init__(self):
        ...
        # Define the core team (always available)
        ...
        # Kit is optional - only add if local inference is available
        self._kit_available = self._check_kit()
        if self._kit_available:
            self.members["kit"] = IDD(
                agent_id="kit",
                name="Kit",
                role="Fast local inference (Qwen 32B) [LOCAL]",
                capabilities=["fast_inference", "validation", "routing"],
                trust_score=0.90
            )

    def _check_kit(self) -> bool:
        """Check if Kit (local Ollama) is available."""
        try:
            import httpx
            response = httpx.get(
                "http://192.168.4.85:11434/api/tags",
                timeout=2.0
            )
    ...
    def route(self, query: str) -> str:
        ...
        return self.aindex.route(query, kit_available=self._kit_available) or "root_ai"
