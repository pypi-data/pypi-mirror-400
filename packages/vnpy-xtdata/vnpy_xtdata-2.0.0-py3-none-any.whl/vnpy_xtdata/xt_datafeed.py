...
class XtDatafeed(BaseDatafeed):
    ...
    def init(self, output: Callable = print) -> bool:
        ...
        try:
            # 使用Token连接，无需启动客户端
            if not self._connect_to_existing_xtdc("127.0.0.1", 58609):
                _, port = self.init_xtdc()
                client = xtdata.connect(port=port)
                if not client.is_connected():
                    output("迅投研数据服务初始化失败，无法连接到行情服务")
                    return False
    ...
    def init_xtdc(self) -> tuple[str, int]:
        """初始化xtdc服务进程"""
        ...
        # 设置监听端口58609
        return xtdc.listen(port=58609)

    @staticmethod
    def _connect_to_existing_xtdc(ip: str, port: int = 58609, output: Callable = print) -> bool:
        """连接到已经存在的行情服务"""
        try:
            client = xtdata.connect(ip, port)
            if client.is_connected():
                output("Successfully connected to existing XTDC data center.")
                return True
            return False
        except Exception:
            return False
