...
class MemoryExpired(Event):
    """A memory has expired and been archived."""

    memory_id: UUID
    temporal_level: TemporalLevel
    expired_at: datetime
    valid_until: datetime  # When it was supposed to expire
    reason: str  # "valid_until_passed", "manual_expiration", "low_salience"

    @property
    def event_type(self) -> EventType:
        return EventType.MEMORY_EXPIRED
...
class MemoryConsolidated(Event):
    """Multiple similar memories were consolidated into one."""
    ...
    @property
    def event_type(self) -> EventType:
        return EventType.MEMORY_CONSOLIDATED

    @property
    def aggregate_id(self) -> UUID:
        return self.consolidated_memory_id
