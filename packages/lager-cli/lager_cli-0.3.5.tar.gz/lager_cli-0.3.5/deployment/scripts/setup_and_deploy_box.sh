...
# Check if sudoers file exists on box
print_info "Checking sudo configuration on box..."
if ssh "${BOX_USER}@${BOX_IP}" "test -f /etc/sudoers.d/lagerdata-udev" 2>/dev/null; then
    ...
    print_warning "Passwordless sudo not configured - will set up now"
    print_info "You may be prompted for the box sudo password once..."
    echo ""
