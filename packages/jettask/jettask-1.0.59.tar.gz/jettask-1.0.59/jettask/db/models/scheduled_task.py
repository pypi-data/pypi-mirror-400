...
class ScheduledTask(Base):
    ...
    __table_args__ = (
        ...
        Index('idx_scheduled_tasks_ns_trigger', 'namespace', 'next_trigger_time', postgresql_where=(enabled == True)),  
    )
    ...
    async def get_ready_tasks(
        ...
        logger.debug(f"get_ready_tasks: namespace={namespace}, now={now}")
        ...
        if namespace:
            conditions.append(cls.namespace == namespace)
        ...
        logger.debug(f"get_ready_tasks: found {len(tasks)} tasks")
        for task in tasks:
            logger.info(
                f"  - Task id={task.id}, scheduler_id={task.scheduler_id}, "
                f"namespace={task.namespace}, trigger={task.next_trigger_time}, run={task.next_run_time}"
            )
