...
- Python reader enforces caps for `ANNPACK_MAX_DIM`, `ANNPACK_MAX_LISTS`, and `ANNPACK_MAX_LIST_BYTES`.
- Registry validates manifests and ANNPack headers before accepting uploads.
...
- Run dependency audits periodically (see `tools/security_audit.sh`).
