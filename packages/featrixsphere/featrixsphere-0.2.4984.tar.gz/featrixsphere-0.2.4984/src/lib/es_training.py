...
from lib.meta_learning_client import mark_config_training_started, get_dataset_characteristics
...
def _train_es_with_cv(args: LightTrainingArgs, input_data_file, training_start_time=None, cv_threshold=10000):
    ...
    from featrix.neural.embedding_space_utils import write_embedding_space_pickle, get_embedding_space_pickle_filename
...
def _train_es_single(args: LightTrainingArgs, input_data_file, resume_from_es=None, training_start_time=None, cv_threshold=10000, use_cv=None, cv_decision_info=None, kv_fold_offset=0):
    ...
    try:
        ...
        # This sends all string values to the string server for batch encoding
        ...
        # Log GPU state IMMEDIATELY after batch_size selection - CRITICAL for debugging OOM
        if is_gpu_available():
            try:
                import torch
        ...
        # ============================================================================
        # Check GLOBAL GPU memory (across all processes), not just this process.
        # If another training job is using the GPU, we need to wait or fail gracefully.
        # This prevents cryptic OOM errors during synchronize_gpu() when GPU is full.
        # ============================================================================
        if is_gpu_available():
            try:
                import torch
                ...
                # Need at least 10GB free to start training (model + batch + overhead)
                MIN_FREE_GB = 10.0
        ...
        while not worked and batch_size >= 8:
            try:
                ...
                if resume_from_es is not None:
                    ...
                    if not hasattr(es, '_dataset_hash') or es._dataset_hash is None:
                        try:
        ...
        try:
            ...
            # Generate and store dataset_hash if not already set
            dataset_hash = getattr(es, '_dataset_hash', None)
            if not dataset_hash:
                try:
                    from featrix_monitor import generate_dataset_hash, create_client
                    dataset_hash = generate_dataset_hash(train_input_data.df)
