...
from vendor.tau2.domains.telecom.tasks.mms_issues import mms_issue_task_manager
from vendor.tau2.domains.telecom.tasks.mobile_data_issues import mobile_data_task_manager
from vendor.tau2.domains.telecom.tasks.service_issues import service_issues_task_manager
from vendor.tau2.domains.telecom.tasks.utils import get_persona_from_task_id
...
def create_tasks(save_tasks: bool = True, max_count_per_bin: int = 3) -> list[Task]:
    ...
    file = DATA_DIR / "domains" / "telecom" / "tasks_full.json"
    if save_tasks:
    ...
    file_small = DATA_DIR / "domains" / "telecom" / "tasks_small.json"
    ...
    file_sampled = DATA_DIR / "domains" / "telecom" / "tasks.json"
    ...
    for task in tasks_with_attrs:
        if task["num_subtasks"] < 2:  # We only keep tasks with at least 2 subtasks
            continue
        tasks_by_bins[(task["intent"], task["num_subtasks"], task["persona"])].append(task["task"])
    ...
    for (intent, num_subtasks, persona), tasks in tasks_by_bins.items():
        ...
        print(f"Sampled {num_sampled} tasks for {intent} with {num_subtasks} subtasks and persona {persona}...")
