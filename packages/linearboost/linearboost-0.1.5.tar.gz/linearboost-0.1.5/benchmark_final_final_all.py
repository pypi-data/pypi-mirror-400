...
class ModelBenchmark:
    ...
    def _optimize_linearboost(self, cv):
        def objective(trial):
            ...
            try:
                # IMPORTANT: Set n_jobs=1 here to avoid nesting. 
                # Optimization relies on fast iteration, not parallel fitting per trial.
    ...
    def _optimize_xgboost(self, cv):
        def objective(trial):
            params = {
                ...
                "eval_metric": "logloss",
                "n_jobs": self.n_jobs, # Explicitly cap threads
            }
    ...
    def _optimize_catboost(self, cv):
        def objective(trial):
            params = {
                ...
                "verbose": 0,
                "thread_count": self.n_jobs, # Explicitly cap threads
            }
    ...
    def evaluate_models_multiple_runs(self):
        ...
        # Ensure user knows threading is restricted
        ...
        for model_name, model_getter in models_config.items():
            ...
            for run_idx, seed in enumerate(self.random_seeds):
                ...
                # CRITICAL FIX FOR MACBOOK CRASHES:
                # We use n_jobs=1 for cross_validate.
                # This ensures we process folds sequentially.
                # Since the models themselves use self.n_jobs threads (e.g. 4 or 8),
                # running them sequentially prevents over-subscribing the CPU
                # and crashing the OS.
                cv_results = cross_validate(
                    model, self.X, self.y,
                    scoring=scoring,
                    cv=cv,
                    n_jobs=1,  # CHANGED FROM 10 TO 1
                    return_train_score=False,
...
if __name__ == "__main__":
    ...
    try:
        dataset = fetch_ucirepo(id=222) # Haberman's Survival
    ...
    benchmark = ModelBenchmark(
        ...
        n_trials=5,      # Low for testing, increase for real paper
        cv_folds=5,      # Standard
        n_runs=5,        # Standard
        n_jobs=4         # <--- RESOURCE CAP APPLIED HERE
    )
