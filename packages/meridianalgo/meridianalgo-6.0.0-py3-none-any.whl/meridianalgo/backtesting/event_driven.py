...
class MarketSimulator:
    ...
    def update_market_data(self, market_event: MarketDataEvent):
        ...
        if market_event.bid_price and market_event.ask_price:
            ...
            spread = self._calculate_spread(symbol, market_event.close_price, market_event.volume)
            self.bid_ask_spreads[symbol] = spread
    ...
    def calculate_market_impact(self, symbol: str, quantity: float, side: str) -> float:
        ...
        if self.impact_model == 'linear':
            ...
            impact_rate = self.impact_coefficient * volatility * np.sqrt(participation_rate)
        elif self.impact_model == 'power_law':
            impact_rate = self.impact_coefficient * volatility * (participation_rate ** 0.6)
        else:
            impact_rate = 0.0
    ...
    def get_execution_price(self, symbol: str, side: str, quantity: float, 
        ...
        for symbol in self.symbols:
            if current_date in self.data[symbol].index:
                ...
                if self.latest_data[symbol].empty:
                    ...
                    self.latest_data[symbol] = pd.concat([self.latest_data[symbol], new_bar])
...
class Strategy(ABC):
    ...
    def set_handlers(self, data_handler: DataHandler, portfolio, event_queue: EventQueue):
...
class SimpleMovingAverageStrategy(Strategy):
    ...
    def calculate_signals(self, market_event: MarketDataEvent) -> List[SignalEvent]:
        ...
        # Bullish crossover
        if prev_short_ma <= prev_long_ma and short_ma > long_ma and current_signal != 'BUY':
            signal = SignalEvent(
        ...
        # Bearish crossover
        elif prev_short_ma >= prev_long_ma and short_ma < long_ma and current_signal != 'SELL':
            signal = SignalEvent(
                timestamp=market_event.timestamp,
                symbol=symbol,
                signal_type='SELL',
...
class Portfolio:
    ...
    def update_market_value(self, market_event: MarketDataEvent):
        ...
        cash = self.current_capital - sum(
            pos * self.current_prices.get(sym, 0) 
    ...
    def execute_fill(self, fill_event: FillEvent):
        ...
        if fill.side == 'BUY':
            ...
            self.current_capital += trade_value - fill.commission - abs(fill.slippage * fill.quantity)
        ...
        logger.debug(f"Executed {fill.side} {fill.quantity} {fill.symbol} @ {fill.price}")
...
class BacktestEngine:
        ...
        if event.signal_type in ['BUY', 'SELL']:
            ...
            if current_price:
                ...
                order_event = OrderEvent(
                    timestamp=event.timestamp + self.execution_delay,
                    order=order
                )
    ...
    def _generate_results(self) -> Dict[str, Any]:
        ...
        if self.portfolio.returns:
            ...
            # Drawdown
            equity_values = [point['total_value'] for point in self.portfolio.equity_curve]
