...
def js_to_python(
    ...
    Args:
        ...
        import_geemap (bool, optional): Whether to add "import le_map_tools" to the output script. Defaults to False.
    ...
    if import_geemap:
        import_str = f"import le_map_tools\n\n{Map} = le_map_tools.Map()\n"
    ...
    if is_python:  # only update the GitHub URL if it is already a GEE Python script
        ...
        with open(in_file, encoding="utf-8") as f:
            ...
            for index, line in enumerate(lines):
                ...
                line = line.replace(
                    "Export.table.toDrive", "le_map_tools.ee_export_vector_to_drive"
                )
                line = line.replace(
                    "Export.table.toAsset", "le_map_tools.ee_export_vector_to_asset"
                )
                line = line.replace(
                    "Export.image.toAsset", "le_map_tools.ee_export_image_to_asset"
                )
                line = line.replace(
                    "Export.video.toDrive", "le_map_tools.ee_export_video_to_drive"
                )
...
def js_snippet_to_py(
    ...
    Args:
        ...
        import_geemap (bool, optional): Whether to import le_map_tools. Defaults to False.
    ...
    try:
        ...
        # if import_geemap:
        #     out_lines.append("import le_map_tools\n\n")
        #     out_lines.append(f"{Map} = le_map_tools.Map()\n")
            ...
            if "from ee_plugin import Map" in line:
                ...
                while True:
                    ...
                    if line_tmp != "":
                        ...
                        i = i + 1
            elif f"{Map} = le_map_tools.Map()" in line:
                return lines[index + 1 :]


def get_js_examples(out_dir=None):
    """Gets Earth Engine JavaScript examples from the le_map_tools package.
    ...
    if (github_username is not None) and (github_repo is not None):
        ...
        for index, line in enumerate(header):
            if index < 9:  # Change Google Colab and binder URLs
                ...
                line = line.replace("le_map_tools", github_repo)
                line = line.replace(
...
if __name__ == "__main__":
    ...
    work_dir = os.path.join(os.path.expanduser("~"), "le_map_tools")
    # Get Earth Engine JavaScript examples. There are five examples in the le_map_tools package data folder.
