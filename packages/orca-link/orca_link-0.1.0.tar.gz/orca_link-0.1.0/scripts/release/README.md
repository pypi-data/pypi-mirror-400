...
这个目录包含了 OrcaLink 发布到 PyPI 的所有脚本。
...
生成两种格式的分发包：
- `orca_link-{version}-py3-none-any.whl` - wheel 格式（推荐）
- `orca_link-{version}.tar.gz` - 源码包
...
```bash
./scripts/release/bump_version.sh 0.1.0
```
...
5. **创建 GitHub Release**
   - 访问 https://github.com/openverse-orca/OrcaLink/releases/new
   - 选择刚创建的 tag
   - 填写 Release Notes
...
```bash
./scripts/release/bump_version.sh 0.1.1
```
