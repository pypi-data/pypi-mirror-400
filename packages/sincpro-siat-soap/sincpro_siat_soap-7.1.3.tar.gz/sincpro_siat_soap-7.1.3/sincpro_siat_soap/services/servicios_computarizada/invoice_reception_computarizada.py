...
class CommandInvoiceReceptionComputarizada(DataTransferObject):
    nit: int | str
    cuis: str
    cufd: str
...
class ResponseInvoiceReceptionComputarizada(DataTransferObject):
    literal_status: SectorDocumentState
    reception_code: str
    raw_response: Any


@siat_soap_sdk.feature(CommandInvoiceReceptionComputarizada)
class InvoiceReceptionRequestSectorEducativo(Feature):

    def execute(
        self, dto: CommandInvoiceReceptionComputarizada
    ) -> ResponseInvoiceReceptionComputarizada:
        response = self.soap_client(
            SIAT_WSDL.SERVICIOS_COMPUTARIZADA
        ).service.recepcionFactura(
        ...
        return ResponseInvoiceReceptionComputarizada(
