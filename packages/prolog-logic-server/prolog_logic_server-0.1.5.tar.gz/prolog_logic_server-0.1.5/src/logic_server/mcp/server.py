...
Use this tool to validate code changes BEFORE editing. When modifying state management,
control flow, or behavioral logic, model the behavior with facts and rules:

1. Assert facts about current behavior
2. Assert rules for desired behavior
3. Query to validate the approach
4. Only edit code if validation passes

EXAMPLE - Validating predicate cleanup:
logic_query(
  facts=[
    "query1_loads_predicates([test(alice), test(bob)]).",
    "query2_has_empty_facts([]).",
  ],
  rules_program="cleanup_works :- query1_loads_predicates(Preds), retractall_called(Preds), query2_has_empty_facts(_)",
  query="cleanup_works"
)

If this returns solutions, your cleanup approach is valid. Only then edit the code.

USE THIS FOR:
- State management changes (predicate loading/unloading, cleanup sequences)
- Control flow modifications (query ordering, resource management)
- Isolation fixes (test isolation, session isolation, predicate namespacing)
- Lifecycle management (initialization, teardown, resource cleanup)
- Concurrency changes (thread-safe operations, locking patterns)
            """.strip(),
