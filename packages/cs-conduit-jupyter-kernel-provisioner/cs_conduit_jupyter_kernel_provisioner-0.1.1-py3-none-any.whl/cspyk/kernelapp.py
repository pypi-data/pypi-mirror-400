...
LEAVE_HELP = """
Launch remote kernel, leave it running, and exit this CSKernelApp.
When set to True, it provides a way to skip having to interact with the
CSKernelApp using keyboard input or signals.
"""

EXISTING = dict(
    config=True,
    help="Filename or absolute path to a provisioner info file previously saved "
    "using, e.g., `conduit-kernel --persistent ...` to connect to an existing "
    "conduit remote kernel.",
)
PERSISTENT_HELP = """
If True, the remote kernel will be left running on shutdown so that you
can reconnect to it later using, e.g., `conduit-kernel --existing ...`.
...
class CSKernelApp(JupyterApp):
    ...
    aliases = {
        ("kernel", "k"): "CSKernelApp.kernel_name",
        ("existing", "e"): "CSKernelApp.existing",
        ("persistent_file", "f"): "CSKernelApp.persistent_file",
        ("poll-interval", "i"): "CSKernelApp.poll_interval",
        "ssh-verbose": "CSKernelApp.ssh_verbose",
    }

    flags = {
        ("debug", "d"): base_flags["debug"],
        # To not have to pass `--leave=True`, but just `--leave`/`-l`
        ("leave", "l"): ({"CSKernelApp": {"leave": True}}, LEAVE_HELP),
        ("persistent", "p"): ({"CSKernelApp": {"persistent": True}}, PERSISTENT_HELP),
    }
...
main = CSKernelApp.launch_instance
