...
async def test_file_upload_with_ellipsis() -> None:
    """Test that File(...) with Ellipsis makes the parameter required."""
    app = Crimsy()
    router = Router(prefix="/files")

    @router.post("/upload-required")
    async def upload_required(file: UploadFile = File(...)) -> dict[str, Any]:
        """Upload a required file using File(...) syntax."""
        content = await file.read()
        return {"filename": file.filename, "size": len(content)}
    ...
    files = {"file": ("test.txt", b"Test content", "text/plain")}
    response = client.post("/files/upload-required", files=files)
    assert response.status_code == 200
    data = response.json()
    assert data["filename"] == "test.txt"
    assert data["size"] == 12

    # Without file - should fail with 400
    response = client.post("/files/upload-required")
    assert response.status_code == 400
    assert "error" in response.json()
