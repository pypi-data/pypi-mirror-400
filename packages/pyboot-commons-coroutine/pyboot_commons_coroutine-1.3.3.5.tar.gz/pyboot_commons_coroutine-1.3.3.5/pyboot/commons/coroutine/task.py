...
import time
from pyboot.commons.coroutine.tools import close_loop,cancel_loop_tasks  # noqa: F401

_logger = Logger('dataflow.utils.coroutine.task')
...
class CoroutineWorkGroup:
    def __init__(self, worker:int=WORKER_THREADS,name:str='CoroutineWorkGroup'):
        if worker < 0:
        ...
        self.idx = 0
        self.worker = worker
        self.name = name or 'CoroutineWorkGroup'
        self.closed = False
        self.closing = False
        ...
        for _ in range(self.worker):
            ...
            def run(loop:asyncio.AbstractEventLoop):
                ...
                try:
                    ...
                    _logger.DEBUG(f'run_forever异常:{e}')
                finally:
                    _logger.DEBUG('事件循环结束，线程退出')
            
            t = threading.Thread(name=f'{self.name}-{r_str(str(i), 2, '0') if i < 10 else i}[{_id}]', target=run, daemon=True, kwargs={"loop":w_loop})
    ...
    async def _shutdown_loop(self, loop: asyncio.AbstractEventLoop):
        # """在*本线程*里取消所有任务并等待完成"""
        ...
        # # 现在可以安全 stop
        # loop.stop()
        await close_loop(loop)
    
    # 跨线程直接 loop.stop()/close() = 强制断电 → 可能 随机 GeneratorExit / closed 异常；    
    def stop(self, abort:bool=False):  
        if self.closed:
            _logger.DEBUG('事件循环已经关闭')
            return        
        if self.closing:
            _logger.DEBUG('事件循环正在关闭中')
            return
        ...
        if abort:                      
            ...
            _logger.DEBUG('停止Work Loop')            
            def _done(f):
                _logger.DEBUG(f'****************** future={future} result={future.result()}')
                            
            for w_loop, th in zip(self.worker_loops, self.worker_threads):
                # ① 把关闭协程丢到*本循环*执行
                # asyncio.run_coroutine_threadsafe(self._shutdown_loop(w_loop), w_loop)
                # _logger.DEBUG(f'******************w_loop.is_running() = {w_loop.is_running() } w_loop.is_closed() ={w_loop.is_closed() }')
                future:Future = asyncio.run_coroutine_threadsafe(self._shutdown_loop(w_loop), w_loop)                
                # _logger.DEBUG(f'****************** future={future} result={future.result()}')
                # future.add_done_callback(_done)
                th.join()
                # w_loop.stop()
                # w_loop.close()
                # _logger.DEBUG(f'******************future.done() = {future.done()} future.cancelled()={future.cancelled()}')
                # _logger.DEBUG(f'******************w_loop.is_running() = {w_loop.is_running() } w_loop.is_closed() ={w_loop.is_closed() }')
                
                # ② 等本线程 run_forever 自然退出
                # for t in threading.enumerate():
                #     if t.name.startswith('CoroutineWorkGroup'):
                #         t.join(timeout=5)        # 给 5 秒宽限
                # # ③ 此时循环已停，再 close 就不会报错
                # w_loop.close()
        self.closed = True
            
    def submit(self, coro)->asyncio.AbstractEventLoop:
        if self.closed:
            raise ValueError('事件循环已经关闭')
        if self.closing:
            raise ValueError('事件循环正在关闭中')
