...
class Credentials(BaseCredentials):
    ...
    def __init__(
        ...
        if self.token_state == TokenState.INVALID:
            ...
            _token = self.client.device_auth_flow()
            self._set_token(_token)
    ...
    @staticmethod
    def _handle_creds_filepath(cache_dir: Optional[str] = None):
    ...
    def from_authclient_token(
        token: Token, cache_dir: Optional[str] = None, client: Optional[AuthClient] = None
    ) -> "Credentials":
    ...
    @staticmethod
    def from_creds_file(cache_dir: Optional[str] = None, client: Optional[AuthClient] = None) -> "Credentials":
        client = client or AuthClient()
        cache_dir = cache_dir or DEFAULT_CACHE_DIR
    ...
    def cache_credentials(self) -> None:
        ...
        with open(fpath, "w") as filebuffer:
            json.dump(self._token.model_dump(), filebuffer, cls=TokenEncoder)
