...
from buddy.api.schemas.user import TeamSchema, UserSchema
from buddy.api.schemas.workspace import WorkspaceSchema
...
class BuddyCliConfig:
    """The BuddyCliConfig class manages user data for the buddy cli"""
    ...
    def __init__(
        ...
        # Active ws dir - used as the default for `buddy` commands
    ...
    def clear_user_cache(self) -> None:
        ...
        BUDDY_cli_settings.ai_conversations_path.unlink(missing_ok=True)
        logger.info("Workspaces cleared. If applicable, please setup your workspace using `buddy ws setup`")
    ...
    def _add_or_update_ws_config(
        ...
        if existing_ws_config is None:
            logger.error(f"Could not find workspace at: {ws_root_str}, please run `buddy ws setup`")
            return None
    ...
    def add_new_ws_to_config(
        ...
        """Adds a newly created workspace to the BuddyCliConfig"""
    ...
    def delete_ws(self, ws_root_path: Path) -> None:
        """Handles Deleting a workspace from the BuddyCliConfig and api"""
        ...
        print_info("Note: this does not delete any data locally or from buddy.com, please delete them manually\n")
    ...
    def save_config(self):
        ...
        write_json_file(file_path=BUDDY_cli_settings.config_file_path, data=config_data)

    @classmethod
    def from_saved_config(cls) -> Optional["BuddyCliConfig"]:
        try:
            config_data = read_json_file(file_path=BUDDY_cli_settings.config_file_path)
            ...
            logger.warning(e)
            logger.warning("Please setup the workspace using `buddy ws setup`")
            return None

    ######################################################
    ## Print BuddyCliConfig
    ######################################################
    ...
    def print_to_cli(self, show_all: bool = False):
        ...
        if self.active_ws_dir:
            ...
            print_info(
                "Please create a workspace using `buddy ws create` or setup an existing workspace using `buddy ws setup`"
            )
