...
class BackgroundWorker:
    ...
    def _process(self, run_id: str, slug: str) -> None:
        ...
        if isinstance(allow_globs, list) and allow_globs:
            warning = self._try_sparse_checkout(worktree_path, allow_globs)
            if warning:
    ...
    def _try_sparse_checkout(self, worktree_path: Path, allow_globs: list[Any]) -> str | None:
        prefixes: list[str] = []
        for raw in allow_globs:
        ...
        set_res = subprocess.run(
            ["git", "sparse-checkout", "set", "--no-cone", *prefixes],
            cwd=worktree_path,
            capture_output=True,
            text=True,
        )
