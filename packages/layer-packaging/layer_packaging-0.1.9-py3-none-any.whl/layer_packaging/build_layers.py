...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    p.add_argument(
        ...
        default=[],
        action='append',
        help="Define code layer (e.g. --code-layers app=./src)"
    )
    ...
    code_layer_map: Dict[str, str] = {}
    for code_layer in args.code_layers:
        if code_layer and "=" in code_layer:
            k, v = code_layer.split("=", 1)
            code_layer_map[k.strip()] = v.strip()
    ...
    # Build code layer
    for name, path in code_layer_map.items():
        built.append(build_code_layer(pyproject, Path(path), layers_dir, name))
