...
def _apply_guardrails(df: pd.DataFrame, guard: dict) -> pd.DataFrame:
    out = df.copy()
    min_val = guard.get("min_val")
    min_tickers = guard.get("min_tickers_val")
    min_p15 = guard.get("min_p15_mfe")
    max_p85 = guard.get("max_p85_mae")
    min_wilson = guard.get("min_wilson_lb")

    if min_val is not None:
        out = out[out["n_val"] >= float(min_val)]
    if min_tickers is not None:
        out = out[out["tickers_val"] >= float(min_tickers)]
    if min_p15 is not None:
        out = out[out["p15_mfe"] >= float(min_p15)]
    if max_p85 is not None:
        out = out[out["p85_mae"] <= float(max_p85)]
    if min_wilson is not None:
        out = out[out["wilson_lb"] >= float(min_wilson)]
    return out
            ...
            for base in bases:
                ...
                for entry in entry_modes:
                    ...
                    for window in windows:
                        ...
                        stage_df = _apply_guardrails(stage_df, guard)
