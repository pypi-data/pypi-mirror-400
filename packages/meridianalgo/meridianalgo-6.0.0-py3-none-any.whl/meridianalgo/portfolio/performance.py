    ...
    def analyze(self, portfolio_returns: pd.Series,
        ...
        if len(common_dates) == 0:
            return AttributionResult(
                ...
                attribution_breakdown=pd.DataFrame(), summary_stats={},
        ...
        for date in common_dates:
            ...
            selection_effect = bw * (sr - sr)  # This would need actual security-level data
            
            # Interaction Effect: (wp - wb) * (rp - rb)
            interaction_effect = (pw - bw) * (sr - sr)  # This would need actual security-level data
        ...
        summary_stats = {
            ...
            'unexplained_return': active_return - summary_attribution['total_effect'].sum()
        }
...
class SectorStyleAttributionAnalyzer(BaseAttributionAnalyzer):
    ...
    def analyze(self, portfolio_returns: pd.Series,
        ...
        explained_return = attribution_df['total_contribution'].sum() if len(attribution_df) > 0 else 0.0
        ...
        summary_stats = {
            ...
            'largest_contributor': attribution_df['total_contribution'].idxmax() if len(attribution_df) > 0 else None,
            'largest_detractor': attribution_df['total_contribution'].idxmin() if len(attribution_df) > 0 else None
        }
...
class BenchmarkComparisonAnalyzer:
    ...
    def analyze(self, portfolio_returns: pd.Series,
        ...
        port_ann_return = (1 + port_total_return) ** (periods_per_year / len(port_ret)) - 1
        bench_ann_return = (1 + bench_total_return) ** (periods_per_year / len(bench_ret)) - 1
        ...
        port_sharpe = (port_ann_return - risk_free_rate) / port_vol if port_vol > 0 else 0
        bench_sharpe = (bench_ann_return - risk_free_rate) / bench_vol if bench_vol > 0 else 0
        ...
        if len(port_ret) > 1 and bench_ret.var() > 0:
            ...
            alpha = port_ann_return - (risk_free_rate + beta * (bench_ann_return - risk_free_rate))
        else:
            beta = 0.0
            alpha = 0.0
        ...
        if down_periods.sum() > 0 and bench_ret[down_periods].mean() != 0:
            down_capture = port_ret[down_periods].mean() / bench_ret[down_periods].mean()
        else:
            down_capture = 0.0
        ...
        cvar_95 = port_ret[port_ret <= var_95].mean() if (port_ret <= var_95).sum() > 0 else var_95
        ...
        sortino_ratio = (port_ann_return - risk_free_rate) / downside_deviation if downside_deviation > 0 else 0
...
class PerformanceAnalyzer:
    ...
    def comprehensive_analysis(self, portfolio_returns: pd.Series,
        ...
        if factor_exposures is not None:
            for factor_type, exposures in factor_exposures.items():
                if 'portfolio' in exposures and 'benchmark' in exposures and 'returns' in exposures:
                    results[f'{factor_type}_attribution'] = self.sector_style_analyzer.analyze(
                        portfolio_returns, benchmark_returns,
                        exposures['portfolio'], exposures['benchmark'], exposures['returns']
                    )
    ...
    def _rolling_analysis(self, portfolio_returns: pd.Series,
                         benchmark_returns: pd.Series,
        ...
        if len(common_dates) < window:
            return {'message': f'Insufficient data for rolling analysis (need {window} periods)'}
        ...
        # Rolling volatility
        rolling_metrics['portfolio_volatility'] = port_ret.rolling(window).std() * np.sqrt(252)
        rolling_metrics['benchmark_volatility'] = bench_ret.rolling(window).std() * np.sqrt(252)
        rolling_metrics['tracking_error'] = (port_ret - bench_ret).rolling(window).std() * np.sqrt(252)
        
        # Rolling Sharpe ratio
        rolling_metrics['portfolio_sharpe'] = (
            rolling_metrics['portfolio_return'] / rolling_metrics['portfolio_volatility']
        )
        rolling_metrics['benchmark_sharpe'] = (
            rolling_metrics['benchmark_return'] / rolling_metrics['benchmark_volatility']
        )
    ...
    def create_performance_report(self, analysis_results: Dict[str, Any],
        ...
        report = []
        report.append(f"Performance Analysis Report: {portfolio_name} vs {benchmark_name}")
        report.append("=" * 80)
        ...
        if 'benchmark_comparison' in analysis_results:
            ...
            if bc['success']:
                ...
                report.append(f"Information Ratio:         {rm['information_ratio']:8.2f}")
                report.append(f"")
        ...
        if 'brinson_attribution' in analysis_results:
            ...
            if ba.success:
                ...
                report.append("-" * 40)
                report.append(f"Total Active Return:       {ba.summary_stats['total_active_return']:8.2%}")
                report.append(f"Allocation Effect:         {ba.summary_stats['total_allocation_effect']:8.2%}")
                report.append(f"Selection Effect:          {ba.summary_stats['total_selection_effect']:8.2%}")
                report.append(f"Interaction Effect:        {ba.summary_stats['total_interaction_effect']:8.2%}")
