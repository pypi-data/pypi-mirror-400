def 编码_字符串到十六进制文本_重点突出(完整字符串, 重点字符串):
    ...
    示例:
        完整字符串 = "Hello, World!"
        重点字符串 = "Hello"
        编码_字符串到十六进制文本_重点突出(完整字符串, 重点字符串)
        # 输出: 48 65 6c 6c 6f 2c 20 57 6f 72 6c 64 21
        # 并且 "Hello" 的十六进制文本以红色输出
    """
    try:
        byte_text = 完整字符串.encode()
        highlight_bytes = 重点字符串.encode()
        hex_text_list = []
        i = 0

        while i < len(byte_text):
            if byte_text[i:i+len(highlight_bytes)] == highlight_bytes:
                # 如果匹配到重点字符串
                hex_str = ' '.join(f"{b:02x}" for b in byte_text[i:i+len(highlight_bytes)])
                hex_text_list.append(f"\033[91m{hex_str}\033[0m")
                i += len(highlight_bytes)
            else:
                # 处理单字节和多字节字符
                if byte_text[i] < 128:
                    # 单字节字符
