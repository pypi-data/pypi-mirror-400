...
class CommandSendInvoicePackage(DataTransferObject):
    nit: int | str
    cuis: str
    cufd: str
...
class ResponseSendInvoicePackage(DataTransferObject):
    literal_status: SectorDocumentState | str
    reception_code: str
    raw_response: Any


@siat_soap_sdk.feature(CommandSendInvoicePackage)
class SendInvoicePackage(Feature):

    def execute(self, dto: CommandSendInvoicePackage) -> ResponseSendInvoicePackage:
        request_dict = {
