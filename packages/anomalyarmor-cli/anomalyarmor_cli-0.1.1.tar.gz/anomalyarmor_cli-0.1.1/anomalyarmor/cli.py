...
def auth_login(
    ...
    if not api_key.startswith("aa_live_") and not api_key.startswith("aa_test_"):
        console.print(
            "[red]Error:[/red] Invalid API key format. Key should start with 'aa_live_'"
        )
        raise typer.Exit(EXIT_AUTH_ERROR)
...
def alert_destinations_list(
    active_only: bool = typer.Option(
        False, "--active", help="Only active destinations"
    ),
    limit: int = typer.Option(50, "--limit", "-l", help="Max results"),
) -> None:
...
def api_keys_list(
    include_revoked: bool = typer.Option(
        False, "--include-revoked", help="Include revoked keys"
    ),
) -> None:
    """List API keys."""
    client = get_client()
...
def intelligence_ask(
    ...
    try:
        ...
        if e.code == "INTELLIGENCE_NOT_GENERATED":
            console.print()
            console.print(
                "[yellow]Intelligence has not been generated for this asset.[/yellow]"
            )
            console.print()
...
def intelligence_generate(
    asset: str = typer.Argument(
        ..., help="Asset identifier (UUID, short UUID, or name)"
    ),
    ...
    try:
        ...
        if e.code == "SCHEMA_NOT_DISCOVERED":
            console.print()
            console.print(
                "[yellow]Schema discovery has not been run for this asset.[/yellow]"
            )
            console.print()
            console.print(
...
def tags_create(
    ...
    asset: str = typer.Option(
        ..., "--asset", "-a", help="Asset (UUID or qualified name)"
    ),
...
def tags_bulk_apply(
    ...
    assets: str = typer.Option(
        ..., "--assets", help="Comma-separated asset identifiers"
    ),
...
def badges_create(
    ...
    tags: str | None = typer.Option(
        None, "--tags", "-t", help="Comma-separated tag filters"
    ),
    ...
    no_freshness: bool = typer.Option(
        False, "--no-freshness", help="Disable freshness monitoring"
    ),
...
def badges_status(
    ...
    color = (
        ...
        else "red" if badge_status == "failing" else "yellow"
    )
