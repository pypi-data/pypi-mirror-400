          ...
          uint3 gid       : SV_GroupID)
{
    ...
    if (tid_local.x == 0) {
        ...
        if (view_data.projectionType == MADRONA_PROJECTION_PERSPECTIVE) {
            ...
            float zFar = view_data.zFar;
            float farPlaneHalfHeight = zFar / -view_data.yScale;
            float farPlaneHalfWidth = zFar / -view_data.xScale;
    ...
    for (int i = 0; i < sm.numInstancesPerThread; ++i) {
        ...
        uint current_instance_idx = sm.offset + local_idx;
        EngineInstanceData instance_data = unpackEngineInstanceData(instanceData[current_instance_idx]);
        AABB aabb = aabbs[instance_data.objectID];
        float3 pre_pmin = aabb.data[0].xyz;
        float3 pre_pmax = float3(aabb.data[0].w, aabb.data[1].xy);
        TransformedAABB txfm_aabb = applyTRS(pre_pmin, pre_pmax, instance_data);
