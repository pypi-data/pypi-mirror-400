...
class FieldInfo:
    ...
    def _get_foreign_key_metadata(self) -> dict[str, str] | None:
        """
        Get foreign key metadata including model_name and endpoint.

        Returns:
            Dictionary with 'model_name' and 'endpoint' keys, or None if not a foreign key
        """
        if not self.is_foreign_key or not self.column.foreign_keys:
            return None

        # Get the first foreign key
        fk = list(self.column.foreign_keys)[0]
        target_table = fk.column.table.name

        # Determine model_name and endpoint based on target table
        if target_table.startswith("overwatch_"):
            # Overwatch models: convert overwatch_organizations to overwatch-organizations
            model_name = target_table.replace("overwatch_", "overwatch-")
            # Replace any remaining underscores with hyphens for kebab-case
            model_name = model_name.replace("_", "-")
        else:
            # Local models: use singular form (e.g., users -> user)
            model_name = (
                target_table.rstrip("s") if target_table.endswith("s") else target_table
            )
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        fk_metadata = self._get_foreign_key_metadata()
        if fk_metadata:
            field_dict.update(fk_metadata)
