...
class Cog(commands.Cog):  # noqa: PLR0904
    ...
    async def cog_load(self) -> None:
        ...
        Raises:
            TidegearError: If this cog has a Tidegear version constraint set,
                but the installed version of Tidegear does not meet that constraint.
        """
        ...
        if self.metadata.minimum_tidegear_version and not self.metadata.is_version_constraint_met:
            msg = _(
                "The `{cog_name}` cog requires a `red-tidegear` version of at least `{required_version}`, but you have version "
                "`{current_version}` installed. Please run the following commands to update `red-tidegear`, "
                "**and then restart your bot**."
                "\n- `{prefix}load downloader`\n- `{prefix}pipinstall red-tidegear>={required_version}`\n"
                "After running these commands and **restarting your bot**, try reloading the `{cog_name}` cog. "
                "If it continues to fail to load, please contact the cog author."
            ).format(
                cog_name=self.__cog_name__,
                required_version=self.metadata.minimum_tidegear_version,
                current_version=meta.version,
                prefix=(await self.bot.get_valid_prefixes())[0],
            )

            async def send_to_owners(msg: str) -> None:
                await self.bot.wait_until_red_ready()
                await self.bot.send_to_owners(content=msg)

            task = self.bot.loop.create_task(send_to_owners(msg))
            task.add_done_callback(lambda t: t.exception())
            raise TidegearError(msg)

        await self.tidegear_create_session()
    ...
    async def tidegear_create_session(self) -> aiohttp.ClientSession:
        """Create a fresh [`ClientSession`][aiohttp.ClientSession], closing an older session if it exists.

        This also automatically assigns the created session to `self.session`.
    ...
    async def tidegear_session_headers(self) -> dict[str, str]:
        ...
        Danger:
            This method is called by [`cog_load`][tidegear.cog.Cog.cog_load] when creating an [`aiohttp.ClientSession`][].
    ...
    def tidegear_setup_file_logger(
        ...
        Warning:
            ...
            The [`cog_load`][tidegear.cog.Cog.cog_load] and  [`cog_unload`][tidegear.cog.Cog.cog_unload] methods
            ...
            You can do so using [`tidegear_close_logger`][tidegear.cog.Cog.tidegear_close_logger].
    ...
    def get_application_emoji(self, name: str) -> discord.Emoji | None:
        ...
        Raises:
            NotImplementedError: Raised if [`self.provides_application_emojis`][tidegear.cog.Cog.provides_application_emojis]
    ...
    async def add_application_emojis(self) -> dict[str, discord.Emoji]:
        ...
        This is automatically ran by [`cog_load`][tidegear.cog.Cog.cog_load]
        if your cog [ships with application emojis][tidegear.cog.Cog.provides_application_emojis],
        ...
        Returns:
            A dictionary containing the cog's emojis and their names as keys.
                Consider using [`self.get_application_emoji`][tidegear.cog.Cog.get_application_emoji]
