...
class interp3d(object):
    def __init__(self, lower_bound, upper_bound, grid_spacing, data, 
                 taylor_order=3, periodic=[False]*3, padding=[True]*3, extrapolation_distance=[0]*3):
        """
        See the documentation for interp1d
        this function is the same, except that a, b, h, p, c, and e
        should be lists or tuples of length 3 giving the values for each dimension
        ...
        if taylor_order not in [1, 3, 5, 7, 9]:
            raise Exception('k must be 1, 3, 5, 7, or 9')
        self.lower_bound = FloatList(lower_bound)
        self.upper_bound = FloatList(upper_bound)
        self.grid_spacing = FloatList(grid_spacing)
        self.data = data
        self.taylor_order = taylor_order
        self.periodic = BoolList(periodic)
        self.padding = BoolList(padding)
        self.extrapolation_distance = IntList(extrapolation_distance)
        self.data_length = IntList(data.shape)
        self.dtype = data.dtype
        self._f, _o = _extrapolate3d(data, taylor_order, periodic, padding, extrapolation_distance)
        self._o = IntList(_o)
        lb, ub = _compute_bounds(lower_bound, upper_bound, grid_spacing, 
                                 periodic, padding, extrapolation_distance, taylor_order)
        self.lb = FloatList(lb)
        self.ub = FloatList(ub)
    ...
    def __call__(self, xout, yout, zout, fout=None):
        ...
        if isinstance(xout, np.ndarray):
            if xout.size > serial_cutoffs[3]:
                func = PAR_INTERP_3D[self.taylor_order]
            else:
                func = SER_INTERP_3D[self.taylor_order]
            m = int(np.prod(xout.shape))
            copy_made = False
            if fout is None:
                _out = np.empty(m, dtype=self.dtype)
            ...
            _yout = yout.ravel()
            _zout = zout.ravel()
            func(self._f, _xout, _yout, _zout, _out, self.lower_bound, self.grid_spacing, 
                 self.data_length, self.periodic, self._o, self.lb, self.ub)
            if copy_made:
                fout[:] = _out
            return _out.reshape(xout.shape)
        else:
