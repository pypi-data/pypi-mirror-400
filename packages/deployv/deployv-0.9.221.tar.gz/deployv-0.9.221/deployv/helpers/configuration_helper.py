...
class DeployvConfig:
    ...
    def _get_psql_image(self, config):
        image = config['group_config'].get('psql_image') or ''
        return image
    ...
    def set_worker_config_sections(self, config):
        ...
        lists = ['extensions', 'listen_addresses']
        string_lists = ['ignore_timeout_deactivate', ]
            ...
            try:
                for key_value in values.keys():
                    ...
                    if key in booleans:
                        ...
                        values.update({key: [str(item)
                                             for item in config.get(section, key).split(",")]})
