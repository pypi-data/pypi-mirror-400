...
class ChromiumBaseSetter(BrowserBaseSetter):
    ...
    def proxy_auth(self, proxy_url):
        """Enable proxy authentication. Format: username:password@host:port"""
        if self._proxy_auth is None:
            self._proxy_auth = ProxyAuth(self._owner)
        return self._proxy_auth.enable(proxy_url)
    
    def block_resources(self, images=False, css=False, fonts=False, media=False, patterns=None):
        """Block resources using advanced CDP interception"""
        if self._resource_blocker is None:
            self._resource_blocker = ResourceBlocker(self._owner)
        self._resource_blocker.enable(images=images, css=css, fonts=fonts, media=media, patterns=patterns)
    
    def unblock_resources(self):
        """Disable resource blocking"""
        if self._resource_blocker:
            self._resource_blocker.disable()
