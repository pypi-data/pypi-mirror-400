...
from ..evaluators.ContextRelevance import ContextRelevanceEvalProcessor
from ..evaluators.groundedness import GroundednessEvalProcessor
...
class GroundednessProcessor(BaseProcessor):
    def pre_process(self, args) -> Any:
        ...
        eval_output_suffix = getattr(args, "eval_output_suffix", "g_relevance")
    ...
    def core_process(self, pre_data):
        ...
        processor = GroundednessEvalProcessor(
            yaml_path=yaml_path,
    ...
    def post_process(self, result: Any) -> Tuple[bool, str]:
        ...
        try:
            ...
            columns_config = getattr(args, "columns_config", {}) or {}
            col_score = columns_config.get('groundedness_score', '可信度（文档和答案）分数')
            col_score_desc = columns_config.get('groundedness_description', '可信度得分说明')
