...
def init_empty_map_plot(ax=None) -> (plt.Figure, plt.Axes):
    ...
    ax.set_axis_off()
    ax.set_aspect('equal')
    ax.margins(0.02)  # margins 2% of the map range is added (matplotlib default is 5%)
    return fig, ax
...
def add_link_map(ax: plt.Axes, inp: SwmmInput,
    ...
    for link, vertices in inp[VERTICES].items():
        ...
        if values_dict and cmap:
            ...
            line_border_color = _darken_color(kwargs.get('color', False) or style['color'])
        else:
...
def add_subcatchment_map(ax: plt.Axes, inp: SwmmInput,
    ...
    if colorbar_kwargs is not None:
        if discrete:
            ...
            if ax.legend_ is not None:
                ax.add_artist(ax.legend_)
                # {'color': get_color_from_value(value), 'marker': 's', 'lw': 0}
            _add_custom_legend(ax, {label: patches.Polygon([[0, 0]], **{**kwargs, 'facecolor': get_color_from_value(value), 'edgecolor': _darken_color(get_color_from_value(value))}) for label, value in values_convert.items()},
                               **colorbar_kwargs)
...
def add_raingauge_symbols(ax: plt.Axes, inp: SwmmInput, add_to_legend=True, **kwargs):
    """
    Plot the raingauge symbols to a map plot.

    Args:
        ax (plt.Axes): Axes of the map plot.
        inp (swmm_api.SwmmInput): SWMM input-file data.
        add_to_legend (bool): if the raingauge symbol should be added to a legend.
        **kwargs: keyword arguments for plt.plot.

    Returns:

    """
    if (SYMBOLS not in inp) or (RAINGAGES not in inp):
        # nothing to plot
        return
    ...
    kwargs.setdefault('markeredgewidth', 0.5)

    # Map short aliases to long-form equivalents if present
    for s, l in (('c', 'color'), ('mec', 'markeredgecolor'), ('ms', 'markersize'), ('lw', 'linewidth'), ('mew', 'markeredgewidth'), ):
        if s in kwargs:
            kwargs[l] = kwargs.pop(s)

    kwargs.setdefault('markeredgecolor', _darken_color(kwargs['color']))

    # ---
    coords = inp[SYMBOLS].frame
    ax.plot(coords.x, coords.y, **kwargs, label=RAINGAGES if add_to_legend else None)
...
def add_raingauge_labels(ax: plt.Axes, inp: SwmmInput, x_offset=0, y_offset=0, **kwargs):
    """
    Add the labels of the raingauges to the map plot as text.
    ...
    for name, sym in inp.SYMBOLS.items():
        ax.text(sym.x + x_offset, sym.y + y_offset, name, **kwargs)
...
class PlottingMap:
    ...
    def add_raingauge_symbols(self, add_to_legend=True, **kwargs):
        """
        Add the raingauge symbols to the map plot.

        .. Important::
            works inplace

        Args:
            add_to_legend (bool): Add symbol to the legend.
            **kwargs: keywords for plt.Axes.plot()

        Returns:
            PlottingMap: self
        """
        add_raingauge_symbols(self.ax, self.inp, add_to_legend=add_to_legend, **kwargs)
        return self

    def add_raingauge_labels(self, x_offset=0, y_offset=0, **kwargs):
        """
        Add the labels of the raingauges to the map plot as text.
        ...
        Args:
            **kwargs: keywords for plt.Axes.text()

        Returns:
            PlottingMap: self
        """
        add_raingauge_labels(self.ax, self.inp, x_offset=x_offset, y_offset=y_offset, **kwargs)
        return self

    def add_backdrop(self):
        """
        Add the backdrop image to the map plot.
    ...
    def auto_size(self):
        """
        Changes the height of the figure to have the same aspect ratio as the plotted network.
        """
        ...
        self.fig.set_size_inches(w, w * dy / dx)
