...
def create_app() -> FastAPI:
    ...
    async def health_check() -> JSONResponse:
        ...
        def _get_redis_cache_with_retry():
            """Blocking Redis operation with retry - runs in thread pool.
            Retries for up to 2 seconds if key is missing (handles transient cache refresh issues).
            """
            import redis
            import time
            
            start_time = time.time()
            max_retry_time = 2.0  # Retry for up to 2 seconds
            retry_delays = [0.05, 0.1, 0.15, 0.2, 0.3, 0.4]  # Exponential backoff delays
            attempt = 0
            ...
            while time.time() - start_time < max_retry_time:
                try:
                    ...
                    if result is not None:
                        ...
                        if attempt > 0:
                            logger.debug(f"✅ Health endpoint: Found health:cache after {attempt} retries ({time.time() - start_time:.3f}s)")
                        return result, None
                    
                    # Key doesn't exist - check if it really doesn't exist
                    exists = redis_client.exists(redis_key)
                    if exists:
                        # Key exists but GET returned None - possible decode issue
                        logger.warning(f"⚠️  Health endpoint: GET returned None but EXISTS=1 for {redis_key} (possible decode issue)")
                        return None, "Redis decode error: key exists but GET returned None"
                    
                    # Key truly doesn't exist - retry if we have time
                    elapsed = time.time() - start_time
                    if elapsed < max_retry_time:
                        # Wait before retry (use exponential backoff, but cap at remaining time)
                        delay = retry_delays[min(attempt, len(retry_delays) - 1)]
                        remaining_time = max_retry_time - elapsed
                        delay = min(delay, remaining_time - 0.01)  # Leave 10ms buffer
                        if delay > 0:
                            time.sleep(delay)
                        attempt += 1
                    else:
            ...
            # Exhausted retries - key still not found
            elapsed = time.time() - start_time
            logger.warning(f"⚠️  Health endpoint: health:cache key not found after {attempt} attempts ({elapsed:.3f}s)")
            return None, None
        
        try:
            import asyncio
            # Run blocking Redis operation in thread pool - prevents blocking event loop
            cached_data, redis_error = await asyncio.to_thread(_get_redis_cache_with_retry)
            if redis_error:
        ...
        if not cached_data:
            ...
            if redis_error:
                ...
                error_msg += " (health:cache key not found in Redis after retries)"
    ...
    async def predict_with_single_predictor(id: str, request: Request) -> JSONResponse:
        ...
        try:
            ...
            if "query_record" in request_data:
                ...
                best_metric_preference = request_data.get("best_metric_preference")  # "roc_auc", "pr_auc", or None
                predictor_index = request_data.get("predictor_index", 0)  # Fallback for old clients
                logger.info(
                    f"🔍 PREDICT REQUEST - New format: target_column='{requested_target_column}', predictor_id='{requested_predictor_id}', best_metric_preference='{best_metric_preference}', predictor_index={predictor_index}"
                )
            else:
            ...
            if single_predictors and isinstance(single_predictors, list):
                ...
                if single_predictor:
                    ...
                    # Before returning error, check if training is in progress and use checkpoints
                    logger.info(f"🔍 PREDICT REQUEST - No predictors in session file, checking for training checkpoints...")
                    
                    # Check session directory for training jobs
                    session_dir = config.output_dir / id
                    checkpoint_found = False
                    training_in_progress = False
                    training_info = None
                    
                    if session_dir.exists():
                        # Look for train_single_predictor directories (new structure: train_single_predictor_*)
                        train_sp_dirs = list(session_dir.glob("train_single_predictor_*"))
                        
                        # Also check old structure: train_single_predictor/{job_id}/
                        train_sp_subdir = session_dir / "train_single_predictor"
                        if train_sp_subdir.exists() and train_sp_subdir.is_dir():
                            old_structure_dirs = [d for d in train_sp_subdir.iterdir() if d.is_dir()]
                            train_sp_dirs.extend(old_structure_dirs)
                        
                        # Check each training directory for status and checkpoints
                        for train_dir in train_sp_dirs:
                            if not train_dir.is_dir():
                                continue
                            
                            # Look for training status files
                            status_files = list(train_dir.glob("*_training_status.json"))
                            if status_files:
                                ...
                                try:
                                    with open(latest_status_file) as f:
                                        ...
                                        if is_training:
                                            ...
                                            logger.info(
                                                f"🔄 PREDICT REQUEST - Found training in progress (epoch {current_epoch}/{total_epochs}, {progress_pct:.1f}% complete)"
                                            )
                                            
                                            # Try to use latest checkpoint from status file
                                            ...
                                            if latest_checkpoint and Path(latest_checkpoint).exists():
                                                ...
                                                logger.info(f"   ✅ Using checkpoint from status: {latest_checkpoint}")
                                                break
                                            
                                            # Fallback: find latest epoch checkpoint in training directory
                                            epoch_checkpoints = sorted(
                                                train_dir.glob("*single_predictor*_epoch_*.pickle"),
                                            ...
                                            if epoch_checkpoints:
                                                ...
                                                checkpoint_found = True
                                                logger.info(f"   ✅ Using latest epoch checkpoint: {predictor_path}")
                                                break
                                            
                                            # Also check for any *single_predictor*.pickle files (might be checkpoints)
                                            any_checkpoints = sorted(
                                                train_dir.glob("*single_predictor*.pickle"),
                                                key=lambda p: p.stat().st_mtime,
                                                reverse=True,
                                            )
                                            if any_checkpoints:
                                                predictor_path = str(any_checkpoints[0])
                                                checkpoint_found = True
                                                logger.info(f"   ✅ Using checkpoint file: {predictor_path}")
                                                break
                                except Exception as status_error:
                                    logger.warning(f"⚠️  Failed to read training status from {latest_status_file}: {status_error}")
                                    continue
                    
                    # If we found a predictor via fallback, UPDATE THE SESSION FILE to fix it
                    if checkpoint_found and predictor_path:
                        logger.info(f"🔧 PREDICT REQUEST - Session file missing predictor, auto-fixing...")
                        
                        # Try to find the BEST ROC-AUC checkpoint instead of just using what we found
                        best_predictor_path = predictor_path  # Default to what we found
                        
                        try:
                            # Look in all training directories for best ROC-AUC checkpoint
                            train_sp_dirs = list(session_dir.glob("train_single_predictor_*"))
                            train_sp_subdir = session_dir / "train_single_predictor"
                            if train_sp_subdir.exists() and train_sp_subdir.is_dir():
                                train_sp_dirs.extend([d for d in train_sp_subdir.iterdir() if d.is_dir()])
                            
                            for train_dir in train_sp_dirs:
                                if not train_dir.is_dir():
                                    continue
                                
                                # Look for best ROC-AUC checkpoint (highest priority)
                                roc_auc_checkpoints = sorted(
                                    train_dir.glob("*best_single_predictor_roc_auc*.pickle"),
                                    key=lambda p: p.stat().st_mtime,
                                    reverse=True
                                )
                                if roc_auc_checkpoints:
                                    best_predictor_path = str(roc_auc_checkpoints[0])
                                    logger.info(f"✅ Found BEST ROC-AUC checkpoint: {best_predictor_path}")
                                    break
                                
                                # Fallback: best overall checkpoint
                                best_checkpoints = sorted(
                                    train_dir.glob("*best_single_predictor.pickle"),
                                    key=lambda p: p.stat().st_mtime,
                                    reverse=True
                                )
                                if best_checkpoints:
                                    best_predictor_path = str(best_checkpoints[0])
                                    logger.info(f"✅ Found BEST checkpoint: {best_predictor_path}")
                                    break
                        except Exception as best_search_error:
                            logger.warning(f"⚠️  Could not find best checkpoint, using found checkpoint: {best_search_error}")
                        
                        try:
                            # Update session with the BEST predictor path
                            if not session.get("single_predictors"):
                                session["single_predictors"] = [best_predictor_path]
                            if not session.get("single_predictor"):
                                session["single_predictor"] = best_predictor_path
                            
                            # Save the updated session
                            from lib.session_manager import save_session
                            save_session(id, session)
                            logger.info(f"✅ Session file auto-fixed with BEST predictor path: {best_predictor_path}")
                            
                            # Update predictor_path to use the best one we found
                            predictor_path = best_predictor_path
                            
                            # Update model card JSON with the best predictor info
                            try:
                                model_card_path = session_dir / "model_card.json"
                                model_card = {}
                                
                                # Load existing model card if it exists
                                if model_card_path.exists():
                                    with open(model_card_path, 'r') as f:
                                        model_card = json.load(f)
                                
                                # Update with predictor info
                                model_card["predictor_path"] = best_predictor_path
                                model_card["auto_fixed"] = True
                                model_card["auto_fix_timestamp"] = datetime.now().isoformat()
                                
                                # Try to load training metrics to get ROC-AUC score
                                try:
                                    metrics_path = Path(best_predictor_path).parent / "training_metrics.json"
                                    if metrics_path.exists():
                                        with open(metrics_path, 'r') as f:
                                            metrics = json.load(f)
                                            quality_metrics = metrics.get("quality_metrics", {})
                                            if quality_metrics:
                                                model_card["best_roc_auc"] = quality_metrics.get("roc_auc")
                                                model_card["best_pr_auc"] = quality_metrics.get("pr_auc")
                                                model_card["quality_metrics"] = quality_metrics
                                except Exception as metrics_error:
                                    logger.warning(f"⚠️  Could not load metrics for model card: {metrics_error}")
                                
                                # Save model card
                                with open(model_card_path, 'w') as f:
                                    json.dump(model_card, f, indent=2)
                                logger.info(f"✅ Model card updated: {model_card_path}")
                                
                            except Exception as card_error:
                                logger.warning(f"⚠️  Failed to update model card: {card_error}")
                            
                        except Exception as fix_error:
                            logger.warning(f"⚠️  Failed to auto-fix session file: {fix_error}")
                            # Don't fail the prediction - we still have the predictor_path
                    
                    if not checkpoint_found:
                        # Check if training is in progress but no checkpoints available yet
                        if training_in_progress and training_info:
                            # Training is in progress but no checkpoints found - return 503
                            error_msg = (
                                f"Predictor is still training. Session exists but no trained predictors are available yet. "
                                f"Session status: running. Please wait for training to complete. "
                                f"Current progress: epoch {training_info['epoch']}/{training_info['total_epochs']} "
                                f"({training_info['progress_percent']:.1f}% complete)."
                            )
                            logger.warning(f"⚠️  PREDICT REQUEST - {error_msg}")
                            raise HTTPException(status_code=HTTPStatus.SERVICE_UNAVAILABLE, detail=error_msg)
                        
                        # No training in progress or no checkpoints found - return error
            ...
            # Check for repaired model first (prefer repaired version if available)
            original_path_for_repair = predictor_path
            predictor_path = get_best_model_path(predictor_path)
            
            # If we're using a repaired path, clear any cache entry for the original path
            if predictor_path != original_path_for_repair:
                logger.info(f"✅ Found repaired model, clearing cache for original: {original_path_for_repair}")
                predictor_cache.cache.pop(original_path_for_repair, None)
                predictor_cache.access_times.pop(original_path_for_repair, None)
            
            logger.info(f"🔍 PREDICT REQUEST - Using predictor path: {predictor_path}")
            ...
            try:
                ...
                if fsp is None:
                    ...
                    # Check model integrity and attempt repair if needed (only for original, not repaired versions)
                    original_predictor_path = predictor_path
                    if not is_training and not Path(predictor_path).name.startswith("repaired-"):
                        try:
                            repair_result = check_and_repair_model(predictor_path, force_repair=False)
                            if repair_result.repaired_path and repair_result.is_valid:
                                # Only use repaired version if it's actually valid
                                logger.info(f"✅ REPAIR SUCCESS - Using repaired model: {repair_result.repaired_path}")
                                logger.info(f"   Original path: {original_predictor_path}")
                                logger.info(f"   Repaired path: {repair_result.repaired_path}")
                                logger.info(f"   Repairs made: {repair_result.repairs_made}")
                                # Clear cache entry for original path if it exists (to avoid using broken cached version)
                                predictor_cache.cache.pop(original_predictor_path, None)
                                predictor_cache.access_times.pop(original_predictor_path, None)
                                predictor_path = repair_result.repaired_path
                            elif repair_result.repaired_path and not repair_result.is_valid:
                                # Repaired file was created but model is still broken - don't use it
                                logger.error(f"❌ REPAIR CREATED BUT INVALID - Repaired model exists but is still broken")
                                logger.error(f"   Repaired path: {repair_result.repaired_path}")
                                logger.error(f"   Issues found: {repair_result.issues_found}")
                                logger.error(f"   Repairs made: {repair_result.repairs_made}")
                                logger.error(f"   ⚠️  Using original model (repaired version is also broken)")
                                # Don't use repaired path - it's still broken
                            else:
                                logger.warning(f"⚠️  REPAIR FAILED - No repaired model created, using original: {predictor_path}")
                                if repair_result.issues_found:
                                    logger.warning(f"   Issues found: {repair_result.issues_found}")
                                if repair_result.repairs_made:
                                    logger.warning(f"   Partial repairs made: {repair_result.repairs_made}")
                        except Exception as repair_error:
                            logger.warning(f"⚠️  Model repair check failed (continuing with original): {repair_error}")
            ...
            if best_metric_preference:
                ...
                if predictor_dir.exists():
                    ...
                    if best_metric_preference.lower() in ['roc_auc', 'roc-auc', 'auc']:
                        ...
                        roc_auc_checkpoints = sorted(
                            predictor_dir.glob("best_single_predictor_roc_auc*.pickle"),
                            key=lambda p: p.stat().st_mtime,
                            reverse=True
                        )
                        if roc_auc_checkpoints:
                        ...
                        pr_auc_checkpoints = sorted(
                            predictor_dir.glob("best_single_predictor_pr_auc*.pickle"),
                            key=lambda p: p.stat().st_mtime,
                            reverse=True
                        )
    ...
    async def admin_inventory(
        ...
        try:
            ...
            for session in iterate_over_sessions():
                ...
                # Session is in Redis - no file stats needed
                all_sessions.append(
                    {
                        "session_id": session_id,
                        "session": session,
                        "session_file_size": 0,  # Sessions are in Redis, not files
                        "session_modified": None,  # Sessions are in Redis, not files
                        "created_at": created_at,
                        "status": status,
                    }
                )
