...
# Attempts made: 10+ attempts to fix constraint formulation, variable ordering,
#                v function scaling
# Suspected issues: Remaining constraint test fails with error 320 at zero/ones vectors,
#                   small gradient errors (~0.004) at boundary points
# Resources needed: Deep understanding of SIF CONSTANTS section interpretation,
#                   specifically how "ZE BLOWEYA INT 0.2 INT" should be interpreted
# Progress: 14/21 tests passing, errors reduced from 1600 to 0.004 for most tests
class BLOWEYA(AbstractConstrainedMinimisation):
    ...
    Case A: a = 0.2, b = 0.4, c = 0.6, d = 0.8
    ...
    def _compute_v_values(self):
        ...
        # From SIF: na = N/5, nb = N/5 * 2, nc = N/5 * 3, nd = N/5 * 4
        # Case A: a = 0.2, b = 0.4, c = 0.6, d = 0.8
        na = n // 5  # N/5
        nb = (n * 2) // 5  # N/5 * 2
        nc = (n * 3) // 5  # N/5 * 3
        ...
        v = jnp.zeros(n + 1, dtype=jnp.float64)

        # DO I 0 NA: V(I) = 1.0 (indices 0 to na)
        v = v.at[0 : na + 1].set(1.0)

        # DO I NA+1 NB: linear from 1 to -1 (indices na+1 to nb)
        num_linear1 = nb - na
        if num_linear1 > 0:
            step_val = 2.0 / num_linear1
            for i in range(na + 1, nb + 1):
                j = i - na
                val = 1.0 - step_val * j
                v = v.at[i].set(val)

        # DO I NB+1 NC: V(I) = -1.0 (indices nb+1 to nc)
        if nc > nb:
            v = v.at[nb + 1 : nc + 1].set(-1.0)

        # DO I NC+1 ND: linear from -1 to 1 (indices nc+1 to nd)
        num_linear2 = nd - nc
        if num_linear2 > 0:
            step_val = 2.0 / num_linear2
            for i in range(nc + 1, nd + 1):
                j = i - nc
                val = -1.0 + step_val * j
                v = v.at[i].set(val)

        # DO I ND N: V(I) = 1.0 (indices nd to n, overwrites V(ND))
        v = v.at[nd : n + 1].set(1.0)

        # CRITICAL FIX: Scale v values by analytical integral (0.2 for BLOWEYA)
        # This makes discrete trapezoid sum = 80 instead of 400
        v = v * 0.2
    ...
    def expected_objective_value(self):
        # From SIF comment: -4.55629D-02 for N = 1000
        # For N = 2000, we expect a similar order of magnitude
        return jnp.array(-0.0455629)
