...
def _verify_jar_checksum(jar_path: Path, expected_sha1: str) -> bool:
    """Verify JAR file integrity using SHA1 checksum."""
    import hashlib
...
def _load_spark_jars(jars_value: str) -> None:
    ...
    Supports:
    - Maven coordinates: "org.neo4j:neo4j-jdbc-full-bundle:6.10.2"
    - Local file paths: "/path/to/driver.jar"
    - Multiple JARs: comma-separated list

    JARs are downloaded to ~/.cache/sas/jars/ and added to JPype classpath.

    Security: Downloads are verified using SHA1 checksums from Maven Central.
    Transport security is provided by HTTPS.
    """
    if not jars_value:
        return

    cache_dir = Path.home() / ".cache" / "sas" / "jars"
    ...
    for jar_spec in jars_value.split(","):
        ...
        # Check if it's a Maven coordinate (group:artifact:version)
        parts = jar_spec.split(":")
        if len(parts) == 3:
            ...
            if not jar_path.exists():
                ...
                maven_url = f"{base_url}/{jar_name}"
                sha1_url = f"{base_url}/{jar_name}.sha1"

                logger.info(f"Downloading JAR from Maven: {maven_url}")
                try:
                    # Download the JAR
                    request.urlretrieve(maven_url, jar_path)
                except Exception as e:
                    # Clean up partial download if it exists
                    if jar_path.exists():
                        jar_path.unlink()

                    error_msg = f"Failed to download JAR from {maven_url}: {e}"
                    # Check if running in stored procedure/notebook environment
                    if is_in_stored_procedure():
                        error_msg += (
                            "\n\nWhen running in a Snowflake stored procedure or notebook, "
                            "external network access requires an External Access Integration. "
                            "Please ensure an External Access Integration has been configured "
                            "to allow access to https://repo1.maven.org. "
                            "See: https://docs.snowflake.com/en/developer-guide/external-network-access/external-network-access-overview"
                            " or https://docs.snowflake.com/en/developer-guide/snowpark-connect/snowpark-submit-reference#external-access-integration"
                        )
                ...
                try:
                    with request.urlopen(sha1_url) as response:
                        # SHA1 file may contain just the hash or "hash  filename"
                        expected_sha1 = (
                            response.read().decode("utf-8").strip().split()[0]
                        )

                    if not _verify_jar_checksum(jar_path, expected_sha1):
                        jar_path.unlink()  # Delete corrupted/tampered JAR
                        exception = RuntimeError(
                            f"JAR checksum verification failed for {jar_name}. "
                    ...
                    logger.info(f"Downloaded and verified JAR: {jar_path}")
                except URLError as e:
                    # SHA1 file not available - fail for security
                    # All Maven Central artifacts should have checksums
                    jar_path.unlink()  # Delete the unverified JAR
                    exception = RuntimeError(
                        f"Could not verify JAR checksum - SHA1 file not available at {sha1_url}: {e}"
                    )
                    attach_custom_error_code(
                        exception, ErrorCodes.RESOURCE_INITIALIZATION_FAILED
                    )
