...
app = typer.Typer(
    name="secret",
    help="Manage service secrets (fetch, show)",
...
def secrets_show(config_file: ConfigArg = "loko.yaml") -> None:
    ...
    secrets_path = runner.secrets_path
    ...
    if os.path.exists(secrets_path):
        ...
        console.print(Panel(content, title=f"Service Secrets: {config.environment.name}", border_style="green"))
    else:
        console.print("[red]Could not retrieve secrets. Try deploying services first.[/red]")
