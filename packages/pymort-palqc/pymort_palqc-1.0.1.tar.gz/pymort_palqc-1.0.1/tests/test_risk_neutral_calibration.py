...
def test_esscher_shift_normal_rw_lambda_broadcast_and_bad_length_raises():
    ...
    with pytest.raises(ValueError, match=r"lambda_esscher must have length 1 or 3"):
        rn.esscher_shift_normal_rw(
            mu_P=mu_P, sigma_P=sigma_P, lambda_esscher=[0.1, 0.2]
        )
...
def test_apply_kappa_drift_shock_broadcast_and_modes_and_errors():
    ...
    # bad mode
    with pytest.raises(
        ValueError, match=r"mode must be 'additive' or 'multiplicative'"
    ):
...
def test_price_from_scen_set_dispatches_all_kinds(monkeypatch: pytest.MonkeyPatch):
    ...
    quotes = [
        rn.MultiInstrumentQuote(
            kind="longevity_bond", spec=bond, market_price=1.0, weight=1.0
        ),
        rn.MultiInstrumentQuote(
            kind="survivor_swap", spec=swap, market_price=1.0, weight=1.0
        ),
        rn.MultiInstrumentQuote(
            kind="s_forward", spec=sf, market_price=1.0, weight=1.0
        ),
        rn.MultiInstrumentQuote(
            kind="q_forward", spec=qf, market_price=1.0, weight=1.0
        ),
        rn.MultiInstrumentQuote(
            kind="life_annuity", spec=ann, market_price=1.0, weight=1.0
        ),
    ]
    ...
    with pytest.raises(ValueError, match=r"Unknown quote.kind"):
        rn._price_from_scen_set(
            scen,
            rn.MultiInstrumentQuote(
                kind="nope", spec=bond, market_price=1.0, weight=1.0
            ),
            short_rate=0.02,
        )
...
def test_calibrate_lambda_least_squares_validations(monkeypatch: pytest.MonkeyPatch):
    ...
    q_zero_w = [
        rn.MultiInstrumentQuote(
            kind="longevity_bond", spec=bond, market_price=1.0, weight=0.0
        )
    ]
    with pytest.raises(
        ValueError, match="At least one quote must have a strictly positive weight"
    ):
        calibrate_lambda_least_squares(
            quotes=q_zero_w,
            ages=ages,
    ...
    q_bad_mkt = [
        rn.MultiInstrumentQuote(
            kind="longevity_bond", spec=bond, market_price=np.nan, weight=1.0
        )
    ]
...
def test_calibrate_market_price_of_longevity_risk_returns_lambda_star(
    ...
    monkeypatch.setattr(
        rn, "calibrate_lambda_least_squares", fake_calibrate_lambda_least_squares
    )
