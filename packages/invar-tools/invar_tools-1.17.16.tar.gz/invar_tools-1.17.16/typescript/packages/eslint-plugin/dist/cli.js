...
async function main() {
    ...
    try {
        ...
        // But use projectPath as cwd to respect project's .eslintignore and ignore patterns
        const eslint = new ESLint({
            useEslintrc: false, // Don't load .eslintrc files
            cwd: __dirname, // Use embedded directory for module resolution
            ...
            baseConfig: {
                ...
                // Explicit ignores to avoid scanning generated/cache directories
                ignorePatterns: [
                    'node_modules',
                    '.next',
                    '.git',
        ...
        try {
            ...
            if (stats.isFile()) {
                ...
                // Directory - use relative glob patterns with cwd to respect .eslintignore
                // This is much faster than absolute globs because it respects project ignore patterns
                filesToLint = [
                    '**/*.ts',
                    '**/*.tsx',
                    '**/*.js',
        ...
        // Handle case where no files matched the patterns
        // ESLint outputs "No files matching '**/*.tsx' were found" to stderr for empty results
        // We treat empty results as success (no violations to report)
        if (results.length === 0) {
            // Output empty array in JSON format
            console.log('[]');
            process.exit(0);
        }
