...
class OWNxExplorer(OWDataProjectionWidget, ConcurrentWidgetMixin):
    ...
    class Inputs:
        node_data = Input(_tr.m[65, "Node Data"], Table)
        node_subset = Input(_tr.m[66, "Node Subset"], Table)
        network = Input(_tr.m[67, "Network"], Network, default=True)
        node_distances = Input(_tr.m[68, "Node Distances"], Orange.misc.DistMatrix)

    class Outputs(OWDataProjectionWidget.Outputs):
        subgraph = Output(_tr.m[69, "Selected sub-network"], Network)
        unselected_subgraph = Output(_tr.m[70, "Remaining sub-network"], Network)
        distances = Output(_tr.m[71, "Distance matrix"], Orange.misc.DistMatrix)
