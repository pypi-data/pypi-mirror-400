...
String ssprintf_generic(const char* fmt, ...) TLX_ATTRIBUTE_FORMAT_PRINTF(1, 2);
...
String ssprintf_generic(const char* fmt, ...)
{
    ...
    int size = std::vsnprintf(const_cast<char*>(out.data()), out.size() + 1,
                              fmt, args);
    ...
    if (size >= static_cast<int>(out.size()))
    {
        ...
        out.resize(size);
        size = std::vsnprintf(const_cast<char*>(out.data()), out.size() + 1,
                              fmt, args);
    }
