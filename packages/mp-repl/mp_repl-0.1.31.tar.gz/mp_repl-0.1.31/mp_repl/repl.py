...
class Repl:
    ...
    async def run(self):
        print("pw-repl v0.1.31 - Type 'help' for commands")
        # 自动重连上次的连接
  ...
  help [cmd]            - Show help""")
    ...
    async def cmd_save(self):
        ...
        if not hasattr(self, '_current_account_id'):
            print("✗ No session selected. Use 'session' first.")
            return
        
        try:
            await helper.save_session(self.conn_mgr.page, self._current_account_id)
            print(f"✓ Session saved: {self._current_session}")
        except Exception as e:
            print(f"✗ {e}")
