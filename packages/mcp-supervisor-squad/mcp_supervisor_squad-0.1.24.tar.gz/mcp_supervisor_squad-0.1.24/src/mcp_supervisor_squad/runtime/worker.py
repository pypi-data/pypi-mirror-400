...
class Worker:
    ...
    def _requirements_brief(self, text: str, max_len: int = 1000) -> str:
        if not isinstance(text, str) or not text.strip():
            return ""
        if len(text) <= max_len:
            return text.strip()
        note = "\n\n(详见 artifacts/requirements.md)"
        head = text.strip()[: max(0, max_len - len(note))]
    ...
    def _run(self, run_id: str, repo_root_hint: str | None = None) -> None:
        ...
        update(
            ...
            event={"kind": "light_summary", "data": light_summary_text},
        )
        ...
        update(
            ...
            important_event_at=utc_now_iso(),
            event={"kind": "requirements", "data": self._requirements_brief(requirements_payload)},
        )
