...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if not m.get("app", {}).get("name") or not m.get("app", {}).get("version"):
        print(
            "ERROR: pyproject.toml missing [project].name or [project].version",
            file=sys.stderr,
        )
        return 2
    ...
    print(f"Release manifest written: {args.out}")
    return 0
