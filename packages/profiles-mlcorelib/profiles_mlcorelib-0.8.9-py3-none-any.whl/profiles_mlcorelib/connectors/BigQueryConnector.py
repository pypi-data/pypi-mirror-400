...
class BigQueryConnector(CommonWarehouseConnector):
    ...
    def get_table_as_dataframe(
        ...
        # Pre-check: Tables with JSON columns can have schema variance across rows,
        # causing ArrowInvalid errors during Arrow schema inference from batches.
        # Use manual row fetch to avoid Arrow conversion for such tables.
        schema_fields = self.fetch_table_metadata(table_name)
        has_json_columns = any(field.field_type == "JSON" for field in schema_fields)

        result = self.run_query(query, response=False)

        if has_json_columns:
            # Manually fetch rows and convert to DataFrame to avoid Arrow schema inference
            rows = list(result)
            if not rows:
                return pd.DataFrame()
        ...
        try:
            ...
            try:
                # Re-run query to get fresh iterator (iterators can only be consumed once)
                result = self.run_query(query, response=False)
