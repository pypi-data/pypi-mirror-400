...
- Schema stability: eval artifacts are `v1` and additive-only; breaking changes bump schema_version.

CLI (minimal):
```bash
python -m app.cli.main eval run evals/transcripts --preset CHAT_DEFAULT --out evals/out/latest.suite.v1.json
python -m app.cli.main eval diff evals/baselines/CHAT_DEFAULT.suite.v1.json evals/out/latest.suite.v1.json --out evals/out/diff.v1.json
```
...
```bash
pip install "mengram[embeddings]"
```

```bash
pip install "mengram[full]"
```
...
(If you’ve defined an extra in your pyproject, you can also do something like `pip install "mengram[server]"` from PyPI. The embeddings extra is `pip install "mengram[embeddings]"`.)
