...
* Added additional functions and numerical data to support setting ultralight boson constraints using black hole spin down, as described in the appendices of [Aswathi et al.][3] 

v0.2.1:

* Minor improvements to spin 0 relativistic frequency shift calculation, matches final version of [May, East, and Siemonsen][2]. 
...
**5. HybridCloudModel(CloudModel)**

A way to combine two cloud models with different regimes of validity. When initiated with `HybridCloudModel(cloud_model_1, cloud_model_2)`, `cloud_model_1` is used when valid based on azimuthal number, otherwise `cloud_model_2` is used.

**6. spindown(mu, Mbh0, abh0, tage, cloud_model, units="natural")**

*Calculate black hole mass and spin after being subject to the superradiant instability for the specified time*

Parameters:

* `mu` is the ultralight boson mass.

* `Mbh0` is the initial black hole mass (before cloud growth).

* `abh0` is the initial black hole dimensionless (before cloud growth).

* `tage` is the time the black hole is subject to potential instability.

* `cloud_model` specifies the spin of the boson and whether to perform calculations using a relativistic or non-relativistic model. 

* `units` specifies the units for input/output and is the same as in UltralightBoson.make_waveform().

Returns:

* Final black hole mass and dimensionless spin: (Mbhf,abhf). 

**7. max_spin(mu, tage, cloud_model, Mbh_lower, Mbh_upper, Nmbh, units="natural", Nspin=128, eps_spin=1.0e-3)**

*Calculate the maximum dimensionless spin of black holes subject to an ultralight boson over some range of boson masses and black hole masses* 
...
* `Mbh_lower` is the lower bound of black hole mass to consider.

* `Mbh_upper` is the upper bound of black hole mass to consider.

* `Nmbh` is the number of black hole masses to compute in the above range. This determines the resolution in black hole mass.

* `units` specifies the units for input/output and is the same as in UltralightBoson.make_waveform().

* `Nspin` is the number of points in dimensionless spin to consider, i.e. max_spin will be computed to ~1/Nspin.

* `eps_spin` computes the spin values in the range [eps_spin, a_max*(1-eps_spin)].


Returns:

* Max dimensionless black hole spin as a 2D array of size len(mu)*Nbh.

**8. Redshift dependence**

SuperRad is configured to take the luminosity distance `dObs` as input argument. All input times are given in the source frame. However, since the superradiant growth timescales, gravitational wave emission timescales, and the associated gravitational wave frequency and frequency evolution as measured in the detector frame depend on the redshift, it might be of use to obtain the correctly redshifted results. For a given time in the detector frame `tdet`, the time of the source frame `tsrc` is redshifted by `tsrc=tdet/(1+z)` for a given redshift `z`. Using a relation between the luminosity distance `dObs` and the redshift `z`, here just called `D(z)`, the correctly redshifted quantities are obtained from `superrad` as follows:
...
Currently, `superrad` does not include bosonic interactions. Some very simple functions for calculating critical couplings, below which superradiance dominates spindown, in various interacting boson models can be found in [superrad/boson_couplings.py](superrad/boson_couplings.py) with an example usage in [examples/example_coupling_constraints.py](examples/example_coupling_constraints.py). Details on these calculations can be found in [Aswathi et al.][3] and the references therein. 
...
[2]: <https://arxiv.org/abs/2410.21442>
[3]: <https://arxiv.org/abs/2507.20979>
