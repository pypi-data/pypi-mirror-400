...
def generate_summary(pis, courses, config, lang, emails, out_dir: str = 'output', draft : bool = False):
    ...
    if shutil.which("pdflatex") is None:
        ...
        return
    try:
        log_path = os.path.join(out_dir, 'pdflatex.log')
        with open(log_path, 'w', encoding='utf-8') as log:
            subprocess.run(["pdflatex", "-interaction=nonstopmode", os.path.basename(fnamepC)],
                           cwd=out_dir, check=True, stdout=log, stderr=subprocess.STDOUT)
            subprocess.run(["pdflatex", "-interaction=nonstopmode", os.path.basename(fnamepV)],
                           cwd=out_dir, check=True, stdout=log, stderr=subprocess.STDOUT)
    except subprocess.CalledProcessError as e:
        print(f"LaTeX compilation failed: {e}. See {log_path} for details.")
