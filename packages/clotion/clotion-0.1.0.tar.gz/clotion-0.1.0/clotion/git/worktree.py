...
class WorktreeManager:
    ...
    def _sanitize_name(self, identifier: str) -> str:
        """Sanitize a task identifier for use as directory/branch name.

        Args:
            identifier: Task identifier (e.g., "CLO-001")
    ...
    async def create(
        ...
        Args:
            task_identifier: Task identifier (e.g., "CLO-001")
        ...
        """
        safe_name = self._sanitize_name(task_identifier)
        branch_name = f"clotion/{safe_name}"
        ...
        logger.info(f"Creating worktree for {task_identifier} at {worktree_path}")
        ...
        try:
            ...
            if worktree_path.exists():
                await self.remove(task_identifier)
            raise
    ...
    async def remove(self, task_identifier: str, force: bool = False) -> bool:
        """Remove a worktree.
    ...
    async def get_worktree(self, task_identifier: str) -> Optional[Worktree]:
        """Get a specific worktree by task identifier.
    ...
    async def get_worktree_path(self, task_identifier: str) -> Optional[Path]:
        """Get the path for a worktree by task identifier.
    ...
    def get_branch_name(self, task_identifier: str) -> str:
