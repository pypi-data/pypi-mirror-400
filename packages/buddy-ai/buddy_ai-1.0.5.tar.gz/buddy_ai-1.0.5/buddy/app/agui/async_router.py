"""Async router handling exposing a Buddy Agent or Team in a Buddy-UI compatible format."""
...
from buddy.app.agui.utils import async_stream_BUDDY_response_as_agui_events, convert_agui_messages_to_BUDDY_messages
...
async def run_agent(agent: Agent, run_input: RunAgentInput) -> AsyncIterator[BaseEvent]:
    """Run the contextual Agent, mapping Buddy-UI input messages to Buddy format, and streaming the response in Buddy-UI format."""
    run_id = run_input.run_id or str(uuid.uuid4())

    try:
        # Preparing the input for the Agent and emitting the run started event
        messages = convert_agui_messages_to_BUDDY_messages(run_input.messages or [])
        ...
        async for event in async_stream_BUDDY_response_as_agui_events(
...
async def run_team(team: Team, input: RunAgentInput) -> AsyncIterator[BaseEvent]:
    """Run the contextual Team, mapping Buddy-UI input messages to Buddy format, and streaming the response in Buddy-UI format."""
    run_id = input.run_id or str(uuid.uuid4())
    try:
        # Extract the last user message for team execution
        messages = convert_agui_messages_to_BUDDY_messages(input.messages or [])
