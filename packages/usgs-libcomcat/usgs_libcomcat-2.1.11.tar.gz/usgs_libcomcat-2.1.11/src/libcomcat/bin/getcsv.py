...
def main():
    ...
    if isinstance(bounds, tuple) or bounds is None:
        ...
        for i, tbounds in enumerate(bounds):
            ...
            events += tevents
            print(f"Found {len(tevents)} events in bounding box {i} {tbounds}.\n")
        # there can be duplicate events here due to overlapping bounding boxes
        # so we need to remove duplicates
        non_dup_events = {}
        for event in events:
            if event.id not in non_dup_events:
                non_dup_events[event.id] = event
        events = list(non_dup_events.values())
