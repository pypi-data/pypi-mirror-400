...
def _parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Scan bullish base + addon combos and append strong finds.")
    ...
    ap.add_argument("--out-file", default="runs/bullish_combo_findings.csv")
...
WHERE b.n_fwd = ({window_max} - {window_min} + 1)
  AND b.rsi <= 30
  AND b.td_buy_count >= 10
  AND b.candle_completely_below_lower IS TRUE;
"""
...
def _build_addons(df: pd.DataFrame) -> List[Tuple[str, np.ndarray]]:
    return [
        ...
        ("stoch_k<=10", df["stoch_k"] <= 10),
        ("williams_r<=-80", df["williams_r"] <= -80),
        ("williams_r<=-90", df["williams_r"] <= -90),
        ...
        ("vwap_cross_up", df["vwap_cross_up"] == True),
        ("ema_stack_bull", df["ema_stack_bull"] == True),
        ("macd_cross_up", df["macd_cross_up"] == True),
        ("tsi_cross_up", df["tsi_cross_up"] == True),
        ("di_bull", df["di_bull"] == True),
        ("adx_strong", df["adx_strong"] == True),
        ("trend_regime", df["trend_regime"] == True),
        ("stoch_rsi_k<=20", df["stoch_rsi_k"] <= 20),
        ("stoch_rsi_k<=10", df["stoch_rsi_k"] <= 10),
        ("rsi<=20", df["rsi"] <= 20),
        ("rsi<=15", df["rsi"] <= 15),
        ("td_buy>=13", df["td_buy_count"] >= 13),
        ("td_buy>=20", df["td_buy_count"] >= 20),
        ("bullish_engulfing", df["bullish_engulfing"] == True),
        ("is_hammer", df["is_hammer"] == True),
