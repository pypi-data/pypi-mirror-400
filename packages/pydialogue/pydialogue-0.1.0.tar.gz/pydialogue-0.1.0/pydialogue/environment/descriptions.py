...
class BaseDescription:
    ...
    def filter_candidate_elements(self, cand):
        ...
        for idx, el in enumerate(elements):
            if el in self.item.prop_seen or el in self.item.attr_seen:
                ...
                if mandatory[idx] is False: #and self.random_gen.choices([True, False], weights=[0.8, 0.2], k=1)[0] is True:
                    break
                else:
                    new_el.append(el)
                    new_mand.append(mandatory[idx])

        # check if deleting some elements helps
        elements = new_el
        mandatory = new_mand
        ...
        new_mand = []
        similar_objs = self.item.world.obj_list
        for idx, el in reversed(list(enumerate(elements))):
            if mandatory[idx] is True:
                new_el.append(el)
                new_mand.append(True)
                similar_objs = kn_helpers.find_similar_objs(similar_objs, self.item, el)
                if len(similar_objs) == 1 and similar_objs[0] == self.item:
                    break
        elements = new_el
    ...
    def generate_description(self, random_pick=True):
        ...
        if 'abstract' in self.item.attributes:
            if 'the' in self.elements:
                idx = self.elements.index('the')
                del self.elements[idx]
                del self.mandatory[idx]

            if 'abstract' in self.elements:
                idx = self.elements.index('abstract')
                del self.elements[idx]
                del self.mandatory[idx]
            ...
            first_elem = None
            if self.elements[0] in self.item.attributes:
                first_elem = self.elements[0]
            elif self.elements[0] in self.item.properties:
                first_elem = self.item.properties[self.elements[0]]

            if first_elem is not None:
                if isinstance(first_elem, (tuple, list)):
                    first_elem_str = " ".join(first_elem)
                else:
                    first_elem_str = first_elem
                if first_elem_str.startswith(('a', 'e', 'i', 'o', 'u')):
                    article = "an"

            self.elements.insert(0, article)
            self.mandatory.insert(0, True)
        else:
            if 'name' in self.elements and 'player' not in self.item.attributes:
                self.elements.insert(0, 'the')
                self.mandatory.insert(0, True)
        if len(self.elements) > 0 and all([prop not in self.elements for prop in ['name', 'nickname', 'surname']]):
            if 'type' not in self.elements and 'entity' not in self.elements:
                self.elements.append('entity')
                self.mandatory.append(True)
            if 'the' not in self.elements and 'abstract' not in self.item.attributes:
                self.elements.insert(0, 'the')
                self.mandatory.insert(0, True)
