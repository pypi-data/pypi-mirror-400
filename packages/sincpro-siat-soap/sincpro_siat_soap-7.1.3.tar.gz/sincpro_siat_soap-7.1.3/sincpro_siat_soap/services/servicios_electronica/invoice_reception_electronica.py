...
from sincpro_siat_soap.domain.sector_document import (
    SectorDocumentState,
    SIATEnvironment,
    SIATModality,
)
...
class CommandInvoiceReceptionElectronica(DataTransferObject):
    nit: int | str
    cuis: str
    cufd: str
...
class ResponseInvoiceReceptionElectronica(DataTransferObject):
    literal_status: SectorDocumentState
    reception_code: str
    raw_response: Any


@siat_soap_sdk.feature(CommandInvoiceReceptionElectronica)
class InvoiceReceptionRequestSectorEducativo(Feature):

    def execute(
        self, dto: CommandInvoiceReceptionElectronica
    ) -> ResponseInvoiceReceptionElectronica:
        response = self.soap_client(SIAT_WSDL.SERVICIOS_ELECTRONICA).service.recepcionFactura(
