...
echo -e "${GREEN}开始发布g7-jira-mcp到PyPI...${NC}"
...
case $choice in
    1)
        ...
        echo -e "pip install --index-url https://test.pypi.org/simple/ g7-jira-mcp"
        ;;
    2)
        echo -e "${YELLOW}正在上传到正式PyPI...${NC}"
        python -m twine upload dist/*
        echo -e "${GREEN}上传完成！可以使用以下命令安装:${NC}"
        echo -e "pip install g7-jira-mcp"
        ;;
    3)
        echo -e "${YELLOW}正在上传到测试PyPI...${NC}"
        python -m twine upload --repository-url https://test.pypi.org/legacy/ dist/*
