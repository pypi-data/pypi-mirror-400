...
def Aroon(high: pd.Series, low: pd.Series, period: int = 25) -> Tuple[pd.Series, pd.Series]:
    ...
    """
    aroon_up = high.rolling(window=period).apply(lambda x: (period - x.argmax()) / period * 100)
    aroon_down = low.rolling(window=period).apply(lambda x: (period - x.argmin()) / period * 100)
