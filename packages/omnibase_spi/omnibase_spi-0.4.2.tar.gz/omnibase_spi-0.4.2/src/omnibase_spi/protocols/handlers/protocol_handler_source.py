...
class ProtocolHandlerSource(Protocol):
    ...
    def get_handler_descriptor(
        ...
        Performance:
            Implementations SHOULD provide O(1) lookup performance by
            maintaining an internal index (e.g., dict keyed by handler_id).
            Avoid O(n) iteration over list_handler_descriptors() for each
            lookup in performance-critical paths.
