...
순수 CLI 방식 - subprocess로 Gemini CLI 호출
API Key 불필요, OAuth 인증 사용 (gemini login)
"""
...
class TaskPlan:
    ...
    detailed_analysis: str = ""  # 지휘관의 상세 분석 보고서
...
# ============================================================
# Gemini CLI Wrapper 클래스 (순수 CLI 방식)
# ============================================================
class GeminiCLIWrapper:
    ...
    def __init__(
        ...
        Args:
            ...
            on_output: 출력 콜백 함수 (로그 메시지)
            on_error: 에러 콜백 함수
            on_status: 상태 업데이트 콜백 함수 (메시지, 경과시간) - 인플레이스 업데이트용
            verbose: 상세 출력 여부
        """
    ...
    def _log_usage(self, input_text: str, output_text: str):
        """토큰 사용량을 기록합니다."""
        try:
            from minmo.scribe_mcp import call_log_usage
    ...
    def _run_gemini(self, prompt: str, operation_name: str = "Gemini CLI") -> str:
        """
        Gemini CLI를 실행하고 결과를 반환합니다.

        Args:
            prompt: 전송할 프롬프트
            operation_name: 작업 이름 (상태 표시용)
        ...
        self._log(f"Gemini CLI 호출: {prompt[:100]}...")

        # 진행 상태 표시 (on_status 콜백이 있으면 인플레이스 업데이트)
        stop_progress = threading.Event()
        start_time = time.time()

        def show_progress():
            """별도 스레드에서 경과 시간 업데이트 (인플레이스)"""
            while not stop_progress.is_set():
                elapsed = time.time() - start_time
                if self.on_status:
                    # 인플레이스 상태 업데이트
                    self.on_status(operation_name, elapsed)
                stop_progress.wait(0.1)  # 더 빠른 업데이트 (0.1초마다)

        progress_thread = None
        if self.on_status:
            # on_status가 있으면 인플레이스 업데이트 사용
            progress_thread = threading.Thread(target=show_progress, daemon=True)
            progress_thread.start()
        elif self.verbose and self.on_output:
            # 폴백: 기존 방식 (새 줄 출력) - 한 번만 출력
            self.on_output(f"⏳ {operation_name} 처리 중...")

        try:
            # 멀티스레드 환경에서 fork 관련 경고 억제 (실제 데드락 위험 낮음)
            with warnings.catch_warnings():
                ...
                # Gemini CLI 실행
                # stdin을 통해 프롬프트 전달 (긴 프롬프트 지원, 명령줄 길이 제한 회피)
                # --output-format text: 순수 텍스트 출력
                result = subprocess.run(
                    [self.gemini_path, "--output-format", "text"],
                    input=prompt,
                    capture_output=True,
                    text=True,
                ...
                if result.returncode != 0:
                    ...
                    self._log(f"Gemini CLI 오류: {error_msg}")
                    if self.on_error:
                        self.on_error("cli_error", {"error": error_msg})
                    return ""

                self._log(f"응답 수신: {len(output)} chars ({elapsed:.1f}s)")

                # 토큰 사용량 기록
                self._log_usage(prompt, output)

                return output

        except subprocess.TimeoutExpired:
            self._log(f"Gemini CLI 타임아웃 ({self.timeout_seconds}s)")
            if self.on_error:
                self.on_error("timeout", {"timeout": self.timeout_seconds})
            return ""
        except FileNotFoundError:
            self._log("Gemini CLI를 찾을 수 없습니다. npm install -g @google/gemini-cli 를 실행하세요.")
            if self.on_error:
                self.on_error("not_found", {"path": self.gemini_path})
            return ""
        except Exception as e:
            self._log(f"Gemini CLI 실행 오류: {e}")
            if self.on_error:
                self.on_error("error", {"error": str(e)})
            return ""
        finally:
            # 진행 상태 표시 중지
            stop_progress.set()
    ...
    def _collect_project_structure(self, path: str, max_depth: int = 3) -> str:
        ...
        def _walk(current: Path, depth: int, prefix: str = ""):
            ...
            try:
                ...
                for i, item in enumerate(items[:20]):  # 각 디렉토리당 최대 20개
...
"""
        ...
        response = self._run_gemini(prompt, operation_name="프로젝트 분석")
...
"""
        ...
        response = self._run_gemini(prompt, operation_name="인터뷰 질문 생성")
        data = self._parse_json_response(response)
...
"""
        ...
        response = self._run_gemini(prompt, operation_name="기획서 생성")
        data = self._parse_json_response(response)
...
"""
        ...
        response = self._run_gemini(prompt, operation_name="태스크 분해")
        data = self._parse_json_response(response)
...
"""
        ...
        response = self._run_gemini(prompt, operation_name="요구사항 분석")
    ...
    def plan(
        ...
        # 상세 분석 보고서 생성
        detailed_analysis = self._generate_detailed_analysis(user_goal, project_analysis)
    ...
    def _generate_detailed_analysis(
        ...
        Returns:
            마크다운 형식의 상세 분석 보고서
        """
        self._log("상세 분석 보고서 생성")

        prompt = f"""{COMMANDER_CLI_CONSTITUTION}

다음 요구사항에 대해 **상세 분석 보고서**를 작성해주세요.
...
- 주요 파일: {', '.join(project_analysis.key_files[:5]) if project_analysis.key_files else '없음'}
- 컨벤션: {', '.join(project_analysis.conventions[:3]) if project_analysis.conventions else '감지되지 않음'}

---

**마크다운 형식으로 다음 내용을 포함한 분석 보고서를 작성하세요:**

1. **현황 분석**: 현재 프로젝트 상태와 관련 파일/코드 파악
2. **설계 전략**: 요구사항을 구현하기 위한 아키텍처 및 접근 방식
3. **수정 영향 범위**: 변경이 필요한 파일과 예상되는 사이드 이펙트
4. **주의사항**: 구현 시 고려해야 할 제약사항이나 위험 요소
5. **권장사항**: 최선의 구현 방법에 대한 조언

JSON이 아닌 **순수 마크다운** 형식으로 응답해주세요.
"""

        response = self._run_gemini(prompt, operation_name="상세 분석")

        # 응답이 비어있거나 JSON인 경우 기본 보고서 반환
        if not response or response.strip().startswith('{'):
            return f"""## 분석 요약

**요구사항:** {user_goal}

**프로젝트:** {project_analysis.project_path}
- 언어: {', '.join(project_analysis.languages) or '감지되지 않음'}
- 프레임워크: {', '.join(project_analysis.frameworks) or '감지되지 않음'}

*상세 분석을 수행할 수 없습니다. 기본 태스크 분해를 진행합니다.*
"""
...
"""
        ...
        response = self._run_gemini(prompt, operation_name="코드 분석")
...
"""
        ...
        response = self._run_gemini(prompt, operation_name="변경사항 리뷰")
        return self._parse_json_response(response)
