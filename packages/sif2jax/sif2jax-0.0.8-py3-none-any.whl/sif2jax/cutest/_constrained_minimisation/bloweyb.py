...
# Resources needed: Verification of integral=0 case handling,
#                    constraint formulation check
# Progress: Implemented with same structure as BLOWEYA, needs testing
class BLOWEYB(AbstractConstrainedMinimisation):
    ...
    Case B: a = 0.1, b = 0.4, c = 0.6, d = 0.9
    ...
    def _compute_v_values(self):
        ...
        # From SIF: na = N/10, nb = N/5 * 2, nc = N/5 * 3, nd = N/10 * 9
        # Case B: a = 0.1, b = 0.4, c = 0.6, d = 0.9
        na = n // 10  # N/10
        nb = (n * 2) // 5  # N/5 * 2
        nc = (n * 3) // 5  # N/5 * 3
        ...
        # Constant -1 between nb+1 and nc
        ...
        # CRITICAL FIX: Scale v values by analytical integral (0.0 for BLOWEYB)
        # This makes discrete trapezoid sum = 0 instead of 400
        v = v * 0.0
    ...
    def expected_objective_value(self):
        # From SIF comment: -5.33674D+04 for N = 1000 (much larger than BLOWEYA)
        # For N = 2000, we expect a similar order of magnitude
        return jnp.array(-533674.0)
