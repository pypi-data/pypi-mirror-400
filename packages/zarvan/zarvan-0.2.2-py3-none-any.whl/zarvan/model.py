...
class HolisticExtractor(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        context = torch.cumsum(x, dim=1) / torch.arange(1, S + 1).to(x.device).view(1, -1, 1)
        scores = self.score_net(x + context).view(B, S, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        ...
        output = torch.cumsum(weights * values, dim=2) / S # (B, num_heads, head_dim)
        
        # Reshape and project to get the final holistic output vector
...
class AssociativeExtractor(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, S, E = x.shape
        context = torch.cumsum(x, dim=1) / torch.arange(1, S + 1).to(x.device).view(1, -1, 1)
        scores = self.score_net(x + context)
