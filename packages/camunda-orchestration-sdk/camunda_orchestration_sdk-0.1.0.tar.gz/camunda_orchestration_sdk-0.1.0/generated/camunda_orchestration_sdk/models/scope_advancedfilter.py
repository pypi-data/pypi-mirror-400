...
@_attrs_define
class ScopeAdvancedfilter:
    r"""Advanced ClusterVariableScopeEnum filter.

    Attributes:
        eq (ScopeAdvancedfilterEq | Unset): Checks for equality with the provided value.
        neq (ScopeAdvancedfilterNeq | Unset): Checks for inequality with the provided value.
        exists (bool | Unset): Checks if the current property exists.
        in_ (list[ScopeAdvancedfilterInItem] | Unset): Checks if the property matches any of the provided values.
    ...
    eq: ScopeAdvancedfilterEq | Unset = UNSET
    neq: ScopeAdvancedfilterNeq | Unset = UNSET
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        _eq = d.pop("$eq", UNSET)
        eq: ScopeAdvancedfilterEq | Unset
        if isinstance(_eq, Unset):
            eq = UNSET
        else:
            eq = ScopeAdvancedfilterEq(_eq)

        _neq = d.pop("$neq", UNSET)
        neq: ScopeAdvancedfilterNeq | Unset
        if isinstance(_neq, Unset):
            neq = UNSET
        else:
            neq = ScopeAdvancedfilterNeq(_neq)
        ...
        if _in_ is not UNSET:
            ...
            for in_item_data in _in_:
                in_item = ScopeAdvancedfilterInItem(in_item_data)

                in_.append(in_item)

        like = d.pop("$like", UNSET)

        scope_advancedfilter = cls(
            eq=eq,
            neq=neq,
            exists=exists,
            in_=in_,
        ...
        scope_advancedfilter.additional_properties = d
        return scope_advancedfilter
