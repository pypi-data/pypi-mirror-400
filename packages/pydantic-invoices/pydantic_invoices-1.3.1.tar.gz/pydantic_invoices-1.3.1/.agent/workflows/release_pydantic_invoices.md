...
VERSION=$(uv run python -c "import pydantic_invoices; print(pydantic_invoices.__version__)")
echo "Waiting 30 seconds for PyPI propagation..."
sleep 30

# Check PyPI
PYPI_VERSION=$(curl -s https://pypi.org/pypi/pydantic-invoices/json | python3 -c "import sys, json; print(json.load(sys.stdin)['info']['version'])")
...
if [ "$VERSION" == "$PYPI_VERSION" ]; then
    ...
    echo "Check: https://pypi.org/project/pydantic-invoices/"
    exit 1
fi
```