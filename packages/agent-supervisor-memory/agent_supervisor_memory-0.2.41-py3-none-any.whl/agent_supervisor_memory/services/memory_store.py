...
class JsonMemoryStore:
    ...
    def upsert_singleton(
        ...
        *,
        ...
        """
        Replace all existing entries for a key with a single new entry.

        Useful for global "preferences" style memories where the latest snapshot should win.
        """
        data = self._load()
        items: list[dict[str, Any]] = data["items"]
        ...
        data["items"] = items
        self._persist(data)
        return self.put(key=key, content=content, tags=tags, source=source)
