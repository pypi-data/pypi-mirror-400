# Linux serial port detection. This works directly on top of pySerial
...
def platform_detect():
    res = []
    for p in comports():
        if p.manufacturer == ct.USB_VENDOR_STRING:
            if p.description.find(ct.USB_MGMT_PORT_NAME) != -1:
                res.append((p.serial_number, ct.USB_MGMT_PORT_NAME, p.device))
            elif p.description.find(ct.USB_CONSOLE_PORT_NAME) != -1:
                res.append((p.serial_number, ct.USB_CONSOLE_PORT_NAME, p.device))
            elif p.description.find(ct.USB_LOGS_PORT_NAME) != -1:
                res.append((p.serial_number, ct.USB_LOGS_PORT_NAME, p.device))
    return res
