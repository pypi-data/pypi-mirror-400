...
def cmd_dashboard(ctx: CommandContext) -> None:
    ...
    global _dashboard_threads, _next_port
    ...
    if arg == "list":
        ...
        for i, run in enumerate(runs):
            status = (
                "🟢 Dashboard running"
                if run.name in _dashboard_threads
                and _dashboard_threads[run.name][0].is_alive()
                else "⚪ Not running"
            )
            port = (
                _dashboard_threads[run.name][1]
                if run.name in _dashboard_threads
                else "-"
            )
    ...
    if run_path.name in _dashboard_threads:
        ...
        if thread.is_alive():
            print(
                f"Dashboard already running for {run_path.name} at http://localhost:{port}"
            )
