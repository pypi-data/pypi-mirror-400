...
def main_loop(aimaster_addr):
    ...
    global running, current_signal_info, current_signal_state, last_signal_info
        ...
        try:
            ...
            if current_signal_state == SignalState.IDLE:
                ...
                if status == "OK" and signal_type:
                    # 检查是否为重复信号
                    if (signal_type == last_signal_info["signal_type"] and 
                        diagnosis_id == last_signal_info["diagnosis_id"] and
                        diagnosis_id):  # 确保diagnosis_id不为空
                        logger.info("Duplicate signal received: {}, diagnosis_id: {}, skipping...".format(
                    ...
                    # 更新上次信号信息
                    last_signal_info["signal_type"] = signal_type
                    last_signal_info["diagnosis_id"] = diagnosis_id
