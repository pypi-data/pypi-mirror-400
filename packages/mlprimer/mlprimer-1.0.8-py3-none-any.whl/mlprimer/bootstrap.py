...
class PipelineResult:
    ...
    # Transformers for automatic encoding/scaling
    feature_scaler: Optional[Any] = None
    target_scaler: Optional[Any] = None
    categorical_encodings: Optional[Dict[str, Dict[int, str]]] = None
    ...
    def transform_features(self, X_new: pd.DataFrame) -> pd.DataFrame:
        """
        Transform new features using same encoding/scaling as training.
        
        Parameters
        ----------
        X_new : pd.DataFrame
            New feature data to transform
        ...
        if self.categorical_features and self.categorical_encodings:
            for col in self.categorical_features:
                if col in X.columns:
                    X[col] = pd.Categorical(X[col]).codes
        
        # Scale numeric columns
        if self.feature_scaler and self.numeric_features:
            X_numeric = X[self.numeric_features].copy()
            X[self.numeric_features] = self.feature_scaler.transform(X_numeric)
        
        return X
    
    def inverse_transform_predictions(self, predictions: np.ndarray) -> np.ndarray:
        """
        Reverse target scaling/transformation to original scale.
        ...
        # Handle both single predictions and arrays
        if predictions.ndim == 1:
            predictions = predictions.reshape(-1, 1)
        
        original_scale = self.target_scaler.inverse_transform(predictions)
        return original_scale.flatten()
    
    def get_categorical_mapping(self) -> pd.DataFrame:
        """
        Get mapping table for categorical encodings.
        
        Returns
        -------
        pd.DataFrame
            Mapping of feature -> (encoded_value, original_value)
        """
        if not self.categorical_encodings:
            return pd.DataFrame()
        
        rows = []
        for feature, encoding_dict in self.categorical_encodings.items():
            for code, category in sorted(encoding_dict.items()):
                rows.append({
                    "feature": feature,
                    "encoded_code": code,
...
def run_pipeline(
    ...
    *,
    ...
    task_hint : str, optional
        Override task detection. Use "ordinal" for ranked targets (e.g., 1-9 scale).
    ...
    stage_times["Step 1: Data Preparation"] = time.time() - step_start
    if verbose >= 1:
        print(f"  ✓ Completed in {stage_times['Step 1: Data Preparation']:.2f}s")
    ...
    # Override with task hint if provided
    if task_hint:
        if task_hint in ["ordinal", "binary", "multiclass", "continuous", "count"]:
            target_type = task_hint
        else:
            print(f"Warning: Unknown task_hint '{task_hint}', using auto-detected '{target_type}'")
    ...
    stage_times["Step 2: Exploratory Analysis"] = time.time() - step_start
    if verbose >= 1:
        print(f"  ✓ Completed in {stage_times['Step 2: Exploratory Analysis']:.2f}s")
    ...
    stage_times["Step 3: Preprocessing & Splitting"] = time.time() - step_start
    if verbose >= 1:
        print(f"  ✓ Completed in {stage_times['Step 3: Preprocessing & Splitting']:.2f}s")
    ...
    if verbose >= 1:
        ...
        if verbose >= 2:
            print(f"  - Training 10+ models with {cv_folds}-fold CV...")
    ...
    feature_scaler = selection_results.get("feature_scaler")
    categorical_encodings = selection_results.get("categorical_encodings", {})
    numeric_features = selection_results.get("numeric_features", [])
    categorical_features = selection_results.get("categorical_features", [])
    ...
    stage_times["Step 4: Model Selection"] = time.time() - step_start
    if verbose >= 1:
        print(f"  ✓ Completed in {stage_times['Step 4: Model Selection']:.2f}s ({stage_times['Step 4: Model Selection']/60:.1f}m)")
    ...
    if verbose >= 1:
        ...
        if verbose >= 2:
            print(f"  - Method: {tune_method.capitalize()} search")
            if tune_method != "grid":
                print(f"  - Iterations: {tune_n_iter}")
            print(f"  - This may take 5-30 minutes depending on data size...")
    ...
    for i, model_name in enumerate(top_models_to_tune, 1):
        ...
        try:
            if verbose >= 1:
                print(f"  [{i}/{len(top_models_to_tune)}] Tuning {model_name}...", end=" ", flush=True)
            ...
            if param_grid is None:
                if verbose >= 1:
                    print("⊘ No predefined grid; skipping")
                if verbose >= 2:
            ...
            if verbose >= 1:
                print(f"✓ ({model_time:.1f}s, score: {result.best_score:.4f})")
    ...
    stage_times["Step 5: Hyperparameter Tuning"] = time.time() - step_start
    if verbose >= 1:
        print(f"  ✓ Tuning completed ({stage_times['Step 5: Hyperparameter Tuning']/60:.1f}m) - {len(tuned_models)} models successfully tuned")
    ...
    if verbose >= 1:
        ...
        for stage, elapsed in stage_times.items():
            ...
            print(f"  {stage:<40} {time_str:>10}  ({pct:>5.1f}%)")
        print(f"  {'Total Pipeline Time':<40} {total_time/60:.1f}m")
        print("=" * 80)
    ...
    return PipelineResult(
        ...
        target_scaler=preprocessor_info.get("target_transformer"),
