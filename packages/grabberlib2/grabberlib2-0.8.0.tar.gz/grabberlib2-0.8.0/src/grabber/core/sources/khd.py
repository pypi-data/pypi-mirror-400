    ...
    **kwargs: Any,
    ...
    for source_url in tqdm_sources_iterable:
        ...
        for index, url in enumerate(urls):
            ...
            page_title = soup.find("title").get_text(strip=True).split("- 4KHD")[0].split("- Page")[0]
            titles.add(page_title)

            if index == 0:
                folder_name = soup.select("title")[0].get_text().split("- 4KHD")[0].split("- Page")[0]  # type: ignore
