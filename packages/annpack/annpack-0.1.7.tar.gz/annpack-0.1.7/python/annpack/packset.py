...
def _count_jsonl_rows(path: Path) -> int:
    """Count non-empty JSONL rows without loading them into memory."""
    if not path.exists():
        return 0
    count = 0
    with path.open("r", encoding="utf-8") as handle:
...
def build_packset_base(
    ...
    **kwargs: Any,
    ...
    if not base_manifest.exists():
        ...
        offline_effective = (
            offline if offline is not None else os.environ.get("ANNPACK_OFFLINE") == "1"
        )
        build_info = {
            "seed": seed,
    ...
    root_manifest: Dict[str, Any] = {
        ...
        "build": {
            ...
            "model": kwargs.get("model_name", "all-MiniLM-L6-v2"),
            "device": kwargs.get("device") or os.environ.get("ANNPACK_DEVICE") or "auto",
            "offline": offline if offline is not None else os.environ.get("ANNPACK_OFFLINE") == "1",
            "lists_requested": lists,
            "text_col": text_col,
            "id_col": id_col,
            "max_rows": kwargs.get("max_rows"),
...
def packset_status(
    packset_dir: str,
    *,
    count_meta: bool = True,
    count_tombstones: bool = True,
    unique_tombstones: bool = False,
) -> Dict[str, Any]:
    """Summarize a PackSet's base/delta state for operational checks."""
    root = Path(packset_dir).expanduser().resolve()
    manifest_path = root / "pack.manifest.json"
    data: Dict[str, Any] = json.loads(manifest_path.read_text(encoding="utf-8"))
    if data.get("schema_version") != 3:
    ...
    base_ann = root / base_info["annpack"]
    base_meta = root / base_info["meta"]
    base_header = _read_header(base_ann) if base_ann.exists() else {}

    deltas: List[Dict[str, Any]] = []
    tombstone_total = 0
    override_total = 0
    unique_ids: Optional[Set[int]] = set() if (count_tombstones and unique_tombstones) else None
    ...
    for delta in sorted(data.get("deltas") or [], key=lambda d: int(d["seq"])):
        ...
        meta_path = root / str(delta_info["meta"])
        tomb_path = root / str(delta_info["tombstones"])
        header = _read_header(ann_path) if ann_path.exists() else {}
        entry: Dict[str, Any] = {
            "seq": int(delta_info["seq"]),
            "annpack": str(ann_path),
            "meta": str(meta_path),
            "tombstones": str(tomb_path),
            "n_vectors": header.get("n_vectors"),
            "n_lists": header.get("n_lists"),
        }
        if count_meta:
            entry["overrides"] = _count_jsonl_rows(meta_path)
            override_total += int(entry["overrides"])
        if count_tombstones:
            if unique_ids is not None:
                ids = _read_tombstones(tomb_path)
                entry["tombstones"] = len(ids)
                unique_ids.update(ids)
                tombstone_total += len(ids)
            else:
                entry["tombstones"] = _count_jsonl_rows(tomb_path)
                tombstone_total += int(entry["tombstones"])
        deltas.append(entry)
    ...
    status: Dict[str, Any] = {
        ...
        "base": {
            ...
            "meta": str(base_meta),
            "n_vectors": base_header.get("n_vectors"),
            "n_lists": base_header.get("n_lists"),
            "dim": base_header.get("dim"),
        },
        "deltas": deltas,
        "delta_count": len(deltas),
    }
    if count_meta:
        status["overrides_total"] = override_total
    if count_tombstones:
        status["tombstones_total"] = tombstone_total
        if unique_ids is not None:
            status["tombstones_unique"] = len(unique_ids)
    if "build" in data:
        status["build"] = data["build"]
    return status
