...
class TestExtraDistances(unittest.TestCase):
    ...
    def test_L2(self):
        xq, yb = self.make_example()
        ref_dis = np.array([[scipy.spatial.distance.sqeuclidean(x, y) for y in yb] for x in xq])
        new_dis = faiss.pairwise_distances(xq, yb, faiss.METRIC_L2)
        self.assertTrue(np.allclose(ref_dis, new_dis))

        ref_dis = np.array([[scipy.spatial.distance.euclidean(x, y) for y in yb] for x in xq])
    ...
    def test_Lp(self):
        p = 1.5
        xq, yb = self.make_example()
        ref_dis = np.array([[scipy.spatial.distance.minkowski(x, y, p) for y in yb] for x in xq])
    ...
    def test_braycurtis(self):
        self.run_simple_dis_test(scipy.spatial.distance.braycurtis, faiss.METRIC_BrayCurtis)
    ...
    def xx_test_jensenshannon(self):
        ...
        self.run_simple_dis_test(scipy.spatial.distance.jensenshannon, faiss.METRIC_JensenShannon)
