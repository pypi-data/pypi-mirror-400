...
class CommandVerifyInvoicePackageComputarizada(DataTransferObject):
    document_type: int
    emission_code: int
    point_of_sale: int
    system_code: str
...
class ResponseVerifyInvoicePackageComputarizada(DataTransferObject):
    literal_status: SectorDocumentState | str
    reception_code: str
    raw_response: Any


@siat_soap_sdk.feature(CommandVerifyInvoicePackageComputarizada)
class VerifyInvoicePackageSiat(Feature):

    def execute(
        self, dto: CommandVerifyInvoicePackageComputarizada
        ...
        if response["transaccion"] is False or response["codigoDescripcion"] == "OBSERVADA":
            ...
            fn()
        return ResponseVerifyInvoicePackageComputarizada(
