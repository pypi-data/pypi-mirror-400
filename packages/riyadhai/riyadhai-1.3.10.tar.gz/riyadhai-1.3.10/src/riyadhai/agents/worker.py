...
class ServerOptions:
    ...
    max_retry: int = 16
    """Maximum number of times to retry connecting to the server."""
    ws_url: str | None = None
    """URL to connect to the server.

    By default it uses ``RIYADAI_URL`` from environment"""
    api_key: str | None = None
    """API key to authenticate with the server.

    By default it uses ``RIYADAI_API_KEY`` from environment"""
    api_secret: str | None = None
    """API secret to authenticate with the server.

    By default it uses ``RIYADAI_API_SECRET`` from environment"""
    ...
    """HTTP proxy used to connect to the server.
...
class AgentServer(utils.EventEmitter[EventTypes]):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self._ws_url = ws_url or os.environ.get("RIYADAI_URL") or ""
        self._api_key = api_key or os.environ.get("RIYADAI_API_KEY") or ""
        self._api_secret = api_secret or os.environ.get("RIYADAI_API_SECRET") or ""

        self._worker_token = os.environ.get("RIYADAI_WORKER_TOKEN") or ""  # hosted agents
    ...
    async def run(self, *, devmode: bool = False, unregistered: bool = False) -> None:
        ...
        async with self._lock:
            ...
            if not self._ws_url:
                raise ValueError("ws_url is required, or add RIYADAI_URL in your environment")

            if not self._api_key:
                raise ValueError("api_key is required, or add RIYADAI_API_KEY in your environment")

            if not self._api_secret:
                raise ValueError(
                    "api_secret is required, or add RIYADAI_API_SECRET in your environment"
                )
            ...
            os.environ["RIYADAI_URL"] = self._ws_url
            os.environ["RIYADAI_API_KEY"] = self._api_key
            os.environ["RIYADAI_API_SECRET"] = self._api_secret
