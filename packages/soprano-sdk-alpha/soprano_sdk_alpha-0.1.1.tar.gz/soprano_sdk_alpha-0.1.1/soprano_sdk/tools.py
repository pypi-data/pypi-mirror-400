    ...
    def execute(
        ...
        with trace_workflow_execution(
            ...
            if state.next:
                ...
                # Start with empty context if none provided (avoid None causing error)
                result = self.graph.invoke(initial_context or {}, config=config)

                # If we have user_message and workflow just interrupted, immediately resume
                # This handles the case of resuming a suspended workflow with fresh thread
                if user_message and "__interrupt__" in result and result["__interrupt__"]:
                    fresh_state = self.graph.get_state(config)
                    if fresh_state.next:
                        logger.info(f"[WorkflowTool] Immediately resuming with user message")
                        result = self.graph.invoke(
            ...
            if "__interrupt__" in result and result["__interrupt__"]:
                ...
                # Check if this is an intent change interrupt (external)
                if isinstance(interrupt_value, dict) and interrupt_value.get("type") == "intent_change":
                    span.set_attribute("workflow.status", "intent_change")
                    span.set_attribute("intent.detected", interrupt_value.get("detected_intent", ""))
                    intent_metadata = json.dumps(interrupt_value)
                    return f"{InterruptType.INTENT_CHANGE}|{thread_id}|{self.name}|{intent_metadata}"
