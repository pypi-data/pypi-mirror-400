...
try:
    ...
    print("pip install 'le_map_tools[ai]'")
...
from .le_map_tools import Map, get_env_var, js_snippet_to_py, ee_initialize, temp_file_path
...
class Genie(widgets.VBox):
    ...
    def __init__(
        ...
        def set_center(x: float, y: float, zoom: int) -> str:
            ...
            self.map_dirty = True
            return (
                "Do not call any more functions in this request to let le_map_tools bounds "
        ...
        system_prompt = f"""
        The client is running in a Python notebook with a le_map_tools Map displayed.
        ...
        le_map_tools. If you pass additional instructions to analyze_image(), do not disclose
...
def run_ee_code(code: str, ee: Any, geemap_instance: Map) -> None:
    """Executes Earth Engine Python code within the context of a le_map_tools instance.

    Args:
        code (str): The Earth Engine Python code to execute.
        ee (Any): The Earth Engine module.
        geemap_instance (Map): The le_map_tools instance.

    Raises:
        Exception: Re-raises any exception encountered during code execution.
    """
    try:
        # le_map_tools appears to have some stray print statements.
        _ = io.StringIO()
        with redirect_stdout(_):
            # Note that sometimes the le_map_tools code uses both 'Map' and 'm' to refer to a map instance.
...
class DatasetSearchInterface:
    ...
    def update_outputs(self, selected_dataset: str) -> None:
        ...
        with self.map_output:
            ...
            code = fix_ee_python_code(dataset.python_code(), le, self.geemap_instance)
...
class DatasetExplorer:
    ...
    def __init__(
        ...
        le.Authenticate()
        le.Initialize(project=project_name)
        storage_client = storage.Client(project=project_name)
        vertexai.init(project=project_name, location=vertex_ai_zone)

        # Make sure le_map_tools initialized correctly.
