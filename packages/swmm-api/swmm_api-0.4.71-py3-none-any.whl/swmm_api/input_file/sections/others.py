...
class Symbol(BaseSectionObject):
    ...
    def to_inp_line(self):
        ...
        return f'{self.gage} {self.x:0.{CONFIG.gis_decimals}f} {self.y:0.{CONFIG.gis_decimals}f}'
    ...
    def geo(self):
        ...
        Returns:
            shapely.geometry.Point: point object for the symbols.
        """
    ...
    def create_section_from_geoseries(cls, data):
        """
        create a SYMBOLS inp-file section for a geopandas.GeoSeries

        Args:
            data (geopandas.GeoSeries): geopandas.GeoSeries of symbols

        Returns:
            InpSectionGeo: SYMBOLS inp-file section
        """
        return cls.create_section(zip(data.index, data.x, data.y))

    @classmethod
    def from_shapely(cls, gage, point):
        """
        Create a Symbol object with a shapely Point

        Args:
            gage (str): label of the gage
            point (shapely.geometry.Point):

        Returns:
            Symbol: Symbol object
        """
        return cls(gage, point.x, point.y)
...
class Curve(BaseSectionObject):
    ...
    def to_inp_line(self):
        ...
        f = f'{self.name}  {self.kind} {x} {y}\n'
        empty_space = ' ' * len(self.kind)
        for x, y in points:  # [(x,y), (x,y), ...]
            f += f'{self.name}  {empty_space} {x} {y}\n'
        return f
