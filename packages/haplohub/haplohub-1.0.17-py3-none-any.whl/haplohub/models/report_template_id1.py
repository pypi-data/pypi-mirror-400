...
REPORTTEMPLATEID1_ANY_OF_SCHEMAS = ["int", "str"]


class ReportTemplateId1(BaseModel):
    """
    ReportTemplateId1
    """
    ...
    if TYPE_CHECKING:
        ...
        actual_instance: Any
    any_of_schemas: List[str] = Field(REPORTTEMPLATEID1_ANY_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ReportTemplateId1.construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in ReportTemplateId1 with anyOf schemas: int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ReportTemplateId1:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> ReportTemplateId1:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into ReportTemplateId1 with anyOf schemas: int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
