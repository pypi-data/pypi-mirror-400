#!/usr/bin/env python3
"""
Custom ViSoNorm model class for PhoBERT-based models.
...
class PhoBERTLMHead(nn.Module):
    def __init__(self, config, phobert_model_embedding_weights):
        ...
        actual_hidden_size = phobert_model_embedding_weights.size(1)
...
class PhoBERTBinaryPredictor(nn.Module):
    def __init__(self, hidden_size, dense_dim=100):
        super(PhoBERTBinaryPredictor, self).__init__()
...
class ViSoNormPhoBERTForMaskedLM(RobertaPreTrainedModel):
    ...
    def __init__(self, config: RobertaConfig):
        ...
        self.cls = PhoBERTLMHead(config, self.roberta.embeddings.word_embeddings.weight)
        
        # Additional heads for ViSoNorm functionality
        self.mask_n_predictor = PhoBERTMaskNPredictionHead(config, actual_hidden_size)
        self.nsw_detector = PhoBERTBinaryPredictor(actual_hidden_size, dense_dim=100)
    ...
    def normalize_text(self, tokenizer, text, device='cpu'):
        """
        Normalize text using the ViSoNorm PhoBERT model with proper NSW detection and masking.
...
__all__ = ["ViSoNormPhoBERTForMaskedLM"]
