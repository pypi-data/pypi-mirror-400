...
class HourlyGraphWidget(Vertical):
    ...
    def _render_temp_plot(self) -> None:
        ...
        # Always use 0-23 for x-axis for alignment
        hours = [h for h in range(24)]
        temps = [0] * 24
        for h in self._hourly_data[:24]:
            temps[h.time.hour] = h.temperature if h.temperature is not None else 0
    ...
    def _render_precip_plot(self) -> None:
        ...
        precs = [0] * 24
        for h in self._hourly_data[:24]:
            precs[h.time.hour] = h.precipitation if h.precipitation is not None else 0
        ...
        max_prec = max(precs)
        if max_prec == 0:
            y_max = 1.0  # Default range when no precipitation
        else:
            y_max = max_prec * 1.1
