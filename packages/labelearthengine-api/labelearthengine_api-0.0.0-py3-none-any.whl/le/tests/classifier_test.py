...
class ClassifierTest(apitestcase.ApiTestCase):
  ...
  def test_naive_bayes_args(self):
    classifier = le.Classifier.smileNaiveBayes(0.01)
    self.assertEqual({'value': 'fakeValue'}, classifier.getInfo())

    naive_bayes_func = le.ApiFunction.lookup('Classifier.smileNaiveBayes')
  ...
  def test_square_kwargs(self):
    kwargs = {'lambda': 0.01}
    classifier = le.Classifier.smileNaiveBayes(**kwargs)
    result = json.loads(classifier.serialize())
    self.assertEqual(_CLASSIFIER_JSON, result)

  def test_cast(self):
    classifier = le.Classifier(le.Classifier.smileNaiveBayes(0.01))
    result = json.loads(classifier.serialize())
    self.assertEqual(_CLASSIFIER_JSON, result)
  ...
  def test_no_args(self):
    ...
    with self.assertRaisesRegex(TypeError, message):
      le.Classifier()  # pytype:disable=missing-parameter
  ...
  def test_wrong_type(self):
    ...
    with self.assertRaisesRegex(TypeError, message):
      le.Classifier(1234)  # pytype:disable=wrong-arg-types
  ...
  def test_amnh_maxent(self):
    ...
    expression = le.Classifier.amnhMaxent(
        categorical_names,
        output_format,
        auto_feature,
        linear,
  ...
  def test_confusion_matrix(self):
    classifier = le.Classifier.load('a')
    expect = make_expression_graph({
  ...
  def test_decision_tree(self):
    ...
    expression = le.Classifier.decisionTree(tree_string)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Classifier.decisionTree(treeString=tree_string)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_decision_tree_ensemble(self):
    ...
    expression = le.Classifier.decisionTreeEnsemble(tree_strings)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Classifier.decisionTreeEnsemble(treeStrings=tree_strings)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_libsvm(self):
    ...
    expression = le.Classifier.libsvm(
        decision_procedure,
        svm_type,
        kernel_type,
        shrinking,
  ...
  def test_load(self):
    ...
    expression = le.Classifier.load(id)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Classifier.load(id=id)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_minimum_distance(self):
    ...
    expression = le.Classifier.minimumDistance(metric, k_nearest)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Classifier.minimumDistance(
  ...
  def test_smile_cart(self):
    ...
    expression = le.Classifier.smileCart(max_nodes, min_leaf_population)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Classifier.smileCart(
  ...
  def test_smile_gradient_tree_boost(self):
    ...
    expression = le.Classifier.smileGradientTreeBoost(
  ...
  def test_smile_knn(self):
    ...
    expression = le.Classifier.smileKNN(k, search_method, metric)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Classifier.smileKNN(
  ...
  def test_smile_naive_bayes(self):
    ...
    expression = le.Classifier.smileNaiveBayes(lambda_)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    # 'lambda' is a reserved word in Python.
    args = {'lambda': lambda_}
    expression = le.Classifier.smileNaiveBayes(**args)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_smile_naive_bayes_with_bad_kwargs(self):
    ...
    with self.assertRaisesRegex(ValueError, message):
      le.Classifier.smileNaiveBayes(bad_arg=1)
  ...
  def test_smile_random_forest(self):
    ...
    expression = le.Classifier.smileRandomForest(
        number_of_trees,
        variables_per_split,
        min_leaf_population,
        bag_fraction,
  ...
  def test_spectral_region(self):
    ...
    expression = le.Classifier.spectralRegion(coordinates, schema)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Classifier.spectralRegion(
  ...
  def test_train(self):
    classifier = le.Classifier.load('a')
    features = le.FeatureCollection('b')
    class_property = 'c'
