...
class MariaDBServiceConfig(ServiceConfig):
    """
    Configuration for MariaDB service.

    This class defines the configuration for the MariaDB service, which runs the MariaDB database. It initializes the
    service with default values for container name, image, restart policy, networks, and ports.

    Depends_on: None

    Attributes:
        container_name (str): The name of the container. Default is "mariadb".
        ...
        ports (list): The ports to expose. Default is ["127.0.0.1:3306:3306"].
    ...
    def __init__(self, **kwargs: Any):
        ...
        self._service_type = "mariadb"
        if not self.container_name:
            self.container_name = "mariadb"
        if not self.image:
            self.image = get_default("ALLOWED_SERVICE_TYPES")["mariadb"]
        if not self.restart:
            self.restart = "always"
        ...
        new_labels = (
            {
                ...
                f"traefik.http.services.{safe_container_name}.loadbalancer.server.port": "3306",
            }
