...
class TestLUTQuantization(unittest.TestCase):
    ...
    def do_test(self, LUT, bias, nprobe, alt_3d=False):
        ...
        biasq = np.zeros(bias.shape, dtype="uint16") if (bias is not None) and not alt_3d else None
...
class TestIVFImplem1(unittest.TestCase):
    ...
    def do_test(self, by_residual, metric_type=faiss.METRIC_L2, use_precomputed_table=0):
...
class TestEquivPQ(unittest.TestCase):
    def test_equiv_pq(self):
        ...
        index_pq.codes = faiss.downcast_InvertedLists(index.invlists).codes.at(0)
...
class TestAdd(unittest.TestCase):
    def do_test(self, by_residual=False, metric=faiss.METRIC_L2, d=32, bbs=32):
        ...
        for list_no in range(32):
            ...
            new_code_per_id = {
                new_ids[i]: new_codes[i // bbs, :, i % bbs] for i in range(new_ids.size)
            }
...
class TestTraining(unittest.TestCase):
    def do_test(self, by_residual=False, metric=faiss.METRIC_L2, d=32, bbs=32):
        ...
        index2 = faiss.IndexIVFPQFastScan(index.quantizer, d, 32, d // 2, 4, metric, bbs)
...
class TestIsTrained(unittest.TestCase):
    def test_issue_2019(self):
        index = faiss.index_factory(32, "PCAR16,IVF200(IVF10,PQ2x4fs,RFlat),PQ4x4fsr")
...
def add_TestIVFAQFastScan_subtest_accuracy(aq, st, by_residual, implem, metric="L2"):
    setattr(
        TestIVFAQFastScan,
