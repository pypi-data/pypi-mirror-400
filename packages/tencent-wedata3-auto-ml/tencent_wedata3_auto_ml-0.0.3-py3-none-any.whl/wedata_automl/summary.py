...
class AutoMLSummary:
    ...
    def save_notebook_to_wedata(
        ...
        # 创建请求 Wedata3
        req = models.CreateCodeFileRequest()
        req.ParentFolderId = parent_folder_id
        req.WorkspaceId = workspace_id
        req.CodeFileName = script_name
        req.ExtensionType = "ide"
        code_storage = models.CodeFileStorage()
        code_storage.StorageType = 1
        code_storage.StoragePath = ""
        # 需要将script_content 转换为 base64 编码的字符串
        code_storage.Content = base64.b64encode(script_content.encode()).decode()


        # 调用 CreateExploreScriptByOwnerUin API
        try:
            print(f"📝 Step 1: CreateCodeFileRequest...")
            print(f"   ScriptName: {script_name}")
            print(f"   ProjectId: {workspace_id}")
            print(f"   ParentFolderId: {parent_folder_id}")

            response = client.CreateCodeFile(req)
            if response.Data:
                script = response.Data
                file_id = script.CodeFileId
                # version_id = "1"
