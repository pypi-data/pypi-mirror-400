...
@click.option("--type", type=click.Choice(['code', 'gtm']), default='code', help="Type of task: 'code' (default) or 'gtm' (market research/leads)")
def delegate(prompt, local, source, branch, watch, timeout, poll_interval, type):
    ...
    if type == "gtm":
        ...
        if "gtm_ops" not in entitlements:
            console.print("[bold red]Error:[/bold red] Feature 'gtm_ops' is not enabled for your account.")
            console.print("Please upgrade your plan to access GTM Ops features.")
            return

        governance_context += "Task-Type: GTM\nPolicy: SPEC-GOV-013\n"
        # Append GTM specific instructions to prompt
        prompt += "\n\n[GTM MODE ACTIVE]\nThis is a GTM/Market Research task. " \
                  "You must treat this as a code generation task where the 'code' is the Clean Data Artifact (JSON/CSV). " \
                  "Verify all data against PII policies. Calculate Signal Scores if applicable."
