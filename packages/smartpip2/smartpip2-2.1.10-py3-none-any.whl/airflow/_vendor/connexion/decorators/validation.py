...
# Compare versions
_jsonschema_3_or_newer = version.parse(jsonschema_version) >= version.parse("3.0.0")
...
class RequestBodyValidator(object):
    ...
    def __init__(self, schema, consumes, api, is_null_value_valid=False, validator=None,
        ...
        self.validator = validatorClass(schema, format_checker=Draft4Validator.FORMAT_CHECKER)
        self.api = api
            ...
            try:
                if parameter_type == 'formdata' and param.get('type') == 'file':
                    if _jsonschema_3_or_newer:
                        ...
                        )(param, format_checker=Draft4Validator.FORMAT_CHECKER).validate(converted_value)
                    else:
                        Draft4Validator(
                            param,
