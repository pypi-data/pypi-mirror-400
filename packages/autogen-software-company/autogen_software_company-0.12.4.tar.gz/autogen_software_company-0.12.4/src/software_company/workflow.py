...
class SoftwareCompanyWorkflow:
    ...
    def revert_last_step(self):
        ...
        self.revert_from_step(step_to_revert_idx)

    def revert_from_step(self, step_idx: int):
        """Reverts workflow state starting from the given step index (inclusive)."""
        current_idx = self.state_manager.get_current_step_index()
        
        if step_idx >= current_idx:
            # We are trying to revert a step that hasn't happened yet?
            # Or maybe we are just jumping back.
            # But the outputs of steps >= step_idx should be cleared.
            pass
            
        steps_to_clear = self.steps[step_idx:]

        self.ui.print_info(f"Reverting workflow from step {step_idx + 1}...")

        for i, step in enumerate(steps_to_clear):
        ...
        # Set index back to start of revert
        self.state_manager.set_current_step_index(step_idx)
        self.state_manager.save_state()
        self.ui.print_success(f"Workflow reverted to step {step_idx + 1}. Ready to resume.")
