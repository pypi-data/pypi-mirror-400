...
这会生成以下文件到 `dist/` 目录：
- `magneto-0.1.0.tar.gz`（源码分发包）
- `magneto-0.1.0-py3-none-any.whl`（wheel 分发包）
...
# 测试安装（可选）
pip install dist/magneto-0.1.0-py3-none-any.whl
```
...
发布成功后，访问：https://pypi.org/project/torrent-to-magnet/

安装测试：
```bash
pip install torrent-to-magnet
```
...
```bash
pip install -i https://test.pypi.org/simple/ magneto
```
...
**注意**：原包名 `magneto` 在 PyPI 上已被占用，已改为 `torrent-to-magnet`。
