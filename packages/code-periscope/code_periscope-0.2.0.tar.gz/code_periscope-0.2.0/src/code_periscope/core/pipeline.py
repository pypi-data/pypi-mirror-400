...
def analyze_repository(
    *,
    ...
    into multiple report formats.

    When `debug` is False (default), intermediate CSV artifacts are deleted at the end
    of the pipeline run, leaving only the final `report_model.json` on disk.
    """
    ...
    log(f"Params: max_commits={max_commits}, k={k}, seed={seed}, top_n={top_n}, debug={debug}")
    ...
    # Pipeline produces a lot of CSVs. Keep them only in debug mode.
    ...
    report_json_path = out_dir / "report_model.json"
    ...
    if not debug:
        ...
        if forecasts_dir.exists() and forecasts_dir.is_dir():
            try:
                next(forecasts_dir.iterdir())
            except StopIteration:
                forecasts_dir.rmdir()

        if deleted:
            log(f"Cleaned up intermediate artifacts: deleted {deleted} CSV files")
...
def _write_snapshots(*, out_dir: Path, file_risk: pd.DataFrame, module_risk: pd.DataFrame, file_metrics: pd.DataFrame, module_metrics: pd.DataFrame) -> None:
    ...
    # --- Finalize: build report model and optionally clean up CSVs ---
