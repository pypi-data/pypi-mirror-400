...
import ipaddress
import stun
from utils.filter_constants import FILTER_SUBNETS
...
class Bootstrap:
    def __init__(self, token: str):
        ...
        self.config: BootstrapNetwork = BootstrapNetwork(
            bearer_token=token,
            owner="veyselkantarcilar@plotune.net",
            bootstrap_url="https://stream.plotune.net",
            connections=connections,
        )

    @property
    def peer_id(self):
        return self.config.get_peer_id()


if __name__ == "__main__":
    result = get_all_ips_with_ports()
    print(result)
