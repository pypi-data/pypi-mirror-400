...
_MessageEchoCallback: TypeAlias = Callable[["WhatsApp", MessageEcho], Any | Awaitable[Any]]
...
class MessageEchoHandler(Handler[MessageEcho]):
    """
    Handler for :class:`~pywa.types.MessageEcho` updates (Messages sent by the business, echoed back).

    - You can use the :func:`~pywa.client.WhatsApp.on_message_echo` decorator to register a handler for this type.

    Example:

        >>> from pywa import WhatsApp, filters
        >>> wa = WhatsApp(...)
        >>> print_echoed_messages = lambda _, msg: print(msg)
        >>> wa.add_handlers(MessageEchoHandler(print_echoed_messages, filters.text))

    Args:
        callback: The callback function (Takes the :class:`~pywa.client.WhatsApp` client instance and a :class:`~pywa.types.MessageEcho` as positional arguments).
        filters: The filters to apply to the callback
        priority: The priority of the handler (default: ``0``)
    """
...
class _HandlerDecorators:
    ...
    def on_message_echo(
        ...
        filters: Filter = None,
        priority: int = 0,
    ) -> Callable[[_MessageEchoCallback], _MessageEchoCallback] | _MessageEchoCallback:
        """
        Decorator to register a function as a callback for :class:`~pywa.types.MessageEcho` (messages sent by the business, echoed back).

        - Shortcut for :func:`~pywa.client.WhatsApp.add_handlers` with a :class:`~pywa.handlers.MessageEchoHandler`.

        Example:

            >>> from pywa import WhatsApp, types, filters
            >>> wa = WhatsApp(...)
            >>> @wa.on_message_echo(filters.text)
            ... def echo_handler(_: WhatsApp, msg: types.MessageEcho):
            ...     print(f"Sent message to {msg.to_phone}: {msg.text}")

        Args:
            filters: Filters to apply to the incoming message echoes.
            priority: The priority of the handler (default: ``0``).
        """
        ...
        if (
            clb := _registered_without_parentheses(
                self=self,
                handler_type=MessageEchoHandler,
                filters=filters,
                priority=priority,
            )
        ) is not None:
        ...
        def deco(callback: _MessageEchoCallback) -> _MessageEchoCallback:
            return _registered_with_parentheses(
                self=self,
                handler_type=MessageEchoHandler,
                callback=callback,
