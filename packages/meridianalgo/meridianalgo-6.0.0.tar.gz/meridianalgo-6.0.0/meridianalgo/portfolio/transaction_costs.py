...
class MarketImpactModel(ABC):
    ...
    @abstractmethod
    def calculate_impact(self, quantity: float, volume: float, 
...
class LinearImpactModel(MarketImpactModel):
    ...
    def __init__(self, temporary_impact_coeff: float = 0.1, 
    ...
    def calculate_impact(self, quantity: float, volume: float, 
        ...
        temporary_impact = self.temporary_impact_coeff * volatility * np.sqrt(participation_rate)
...
class SquareRootImpactModel(MarketImpactModel):
    ...
    def calculate_impact(self, quantity: float, volume: float, 
                        volatility: float, price: float = 100.0, **kwargs) -> float:
        """
...
class OptimalExecutionAlgorithm(ABC):
    ...
    @abstractmethod
    def optimize_execution(self, target_quantity: float, 
...
class TWAPAlgorithm(OptimalExecutionAlgorithm):
    ...
    def optimize_execution(self, target_quantity: float,
        ...
        for t in range(execution_horizon):
            ...
            slice_cost = abs(quantity_per_period) * market_data.get('price', 100.0) * (1 + impact)
...
class VWAPAlgorithm(OptimalExecutionAlgorithm):
    ...
    def optimize_execution(self, target_quantity: float,
        ...
        for t in range(execution_horizon):
            ...
            slice_cost = abs(quantity_slice) * market_data.get('price', 100.0) * (1 + impact)
...
class TaxLossHarvester:
    ...
    def optimize_tax_harvesting(self, 
                               portfolio: pd.DataFrame,
                               prices: pd.DataFrame,
                               tax_rate: float = 0.25,
                               min_loss_threshold: float = 100.0,
                               **kwargs) -> TaxLossHarvestingResult:
        """
    ...
    def _check_wash_sale_violation(self, asset: str, purchase_date: datetime,
                                  current_date: datetime, portfolio: pd.DataFrame) -> Dict[str, Any]:
        ...
        for _, pos in asset_positions.iterrows():
            ...
            if wash_sale_start <= pos_date <= wash_sale_end and pos_date != purchase_date:
                return {
                    'violation': True,
                    'asset': asset,
    ...
    def _optimize_gain_loss_pairing(self, trades: List[Dict], portfolio: pd.DataFrame,
                                   prices: pd.DataFrame, tax_rate: float) -> List[Dict]:
...
class TransactionCostOptimizer:
    ...
    def _initialize_algorithms(self):
        ...
        for model_name, model in self.impact_models.items():
            ...
            self.execution_algorithms[f'is_{model_name}'] = ImplementationShortfallAlgorithm(model)
    ...
    def optimize_execution(self, target_quantity: float,
        ...
        return algo.optimize_execution(target_quantity, market_data, execution_horizon, **kwargs)
    
    def calculate_rebalancing_costs(self, current_weights: pd.Series,
                                  target_weights: pd.Series,
                                  portfolio_value: float,
                                  market_data: Dict[str, Dict[str, Any]],
                                  execution_algorithm: str = 'is_square_root') -> Dict[str, Any]:
        """
    ...
    def optimize_tax_harvesting(self, portfolio: pd.DataFrame,
                               prices: pd.DataFrame,
                               **kwargs) -> TaxLossHarvestingResult:
