...
class FeatrixSinglePredictor:
    ...
    def _calculate_adaptive_learning_rate(self, optimizer_params=None, fine_tune=False):
        ...
        CRITICAL: OneCycleLR will scale the base LR up to 3x during warmup!
        So we must be extra conservative with the base LR to prevent instability.
        ...
        # INCREASED base LRs to prevent model collapse from too-small learning rates
        if n_samples < 500:
            # Tiny dataset: Conservative but not too small
            # max_lr will be 3x = 1.5e-4 (0.00015)
            adaptive_lr = 5e-05
        elif n_samples < 1000:
            # Very small dataset: Moderate
            # max_lr will be 3x = 2.4e-4 (0.00024)
            adaptive_lr = 8e-05
        elif n_samples < 2000:
            # Small dataset: Moderate
            # max_lr will be 3x = 3e-4 (0.0003)
            adaptive_lr = 1e-04
        elif n_samples < 5000:
            # Small-medium dataset: Moderate
            # max_lr will be 3x = 4.5e-4 (0.00045)
            adaptive_lr = 1.5e-04
        elif n_samples < 10000:
            # Medium dataset: Moderate-high
            # max_lr will be 3x = 6e-4 (0.0006)
            adaptive_lr = 2e-04
        elif n_samples < 20000:
            # Medium-large dataset: Can go higher
            # max_lr will be 3x = 7.5e-4 (0.00075)
            adaptive_lr = 2.5e-04
        else:
            # Large dataset: Can handle higher LR
            # max_lr will be 3x = 9e-4 (0.0009)
            adaptive_lr = 3e-04
        
        # Apply fine_tune scaling: reduce LR by 3x (was 5x, too aggressive)
        # Still protects pre-trained embeddings but allows meaningful learning
        if fine_tune:
            adaptive_lr = adaptive_lr / 3.0
        ...
        if optimizer_params is not None and "lr" in optimizer_params:
            ...
            if final_lr != provided_lr:
                ...
                if fine_tune:
                    logger.info(f"   Fine-tune mode: LR scaled down by 5x to protect pre-trained embeddings")
            else:
        ...
        if fine_tune:
            logger.info(f"   Fine-tune mode: LR scaled down by 5x to protect pre-trained embeddings")
        logger.info(f"   OneCycleLR peak: {final_lr * 3.0:.6e} (3x base, reached mid-training)")
        logger.info(f"   Starting LR: {final_lr * 3.0 / 25.0:.6e} (warmup from 3x/25)")
        logger.info(f"   🎯 Key: Base LR is conservative because OneCycleLR will 3x it!")
    ...
    def _create_optimizer(self, params, optimizer_params):
        """
        ...
        Args:
            ...
            optimizer_params: Dict with 'lr', 'weight_decay', etc.
        ...
        def update_best_single_predictor_symlink(checkpoint_path):
            ...
            try:
                ...
                if os.path.exists(checkpoint_path):
                    ...
                    logger.info(f"🔗 Updated best model symlink: {best_pickle_path} -> {os.path.basename(checkpoint_path)}")
                    return True
            except Exception as e:
    ...
    def _verify_training_state_assertions(self, epoch_idx, batch_idx, fine_tune):
        """
        ...
        # Encoder training state depends on fine_tune flag
        if fine_tune:
    ...
    async def train(
        ...
        if use_lr_scheduler:
            ...
            # - pct_start: Fraction of training for warmup (adaptive: 10% for long runs, 30% for short)
            #   For long training (>1000 epochs), 30% warmup is excessive (300+ epochs of low LR)
            #   Use 10% for long runs to reach peak LR sooner and avoid getting stuck in warmup
            # - div_factor: Initial LR = max_lr / div_factor (25 = very gentle warmup)
            # - final_div_factor: Final LR = max_lr / final_div_factor (1e4 = aggressive cooldown)
            # Adaptive warmup: shorter for long training runs to prevent getting stuck
            warmup_fraction = 0.1 if n_epochs > 1000 else 0.3  # 10% for long runs, 30% for short
            scheduler = OneCycleLR(
                optimizer,
                max_lr=base_lr * 3.0,  # Peak at 3x base LR (conservative multiplier)
                total_steps=total_steps,
                pct_start=warmup_fraction,  # Adaptive warmup based on training length
            ...
            logger.info(f"   Warmup: {int(total_steps * warmup_fraction)} steps ({warmup_fraction*100:.0f}%) - {'Adaptive: shorter for long training' if n_epochs > 1000 else 'Standard: 30% for stability'}")
        ...
        with PredictorTrainingContextManager(
            ...
            while restart_loop_active and restart_attempts <= max_restart_attempts:
                try:
                    ...
                    # Set models to correct training modes
                    self._set_training_modes(fine_tune=fine_tune)
                    ...
                    if use_lr_scheduler:
                        ...
                        # Use same adaptive warmup logic as initial scheduler
                        restart_warmup_fraction = 0.1 if remaining_epochs > 1000 else 0.3
                        ...
                        scheduler = OneCycleLR(
                            ...
                            pct_start=restart_warmup_fraction,
                            div_factor=25.0,
        ...
        with PredictorEvalModeContextManager(fsp=self, debugLabel="compute_classification_metrics"):
            ...
            if is_binary:
                ...
                try:
                    ...
                    if failure_detected:
                        ...
                        if "DEAD_NETWORK" in failure_label:
                            ...
                            if should_abort:
                                ...
                                # AUTOMATIC LR INCREASE: Increase LR by 5x (per ConstantProbabilityError recommendation)
                                # Allow up to 2 increases if problem persists
                                if not hasattr(self, '_lr_increased_for_constant_prob'):
                                    self._lr_increased_for_constant_prob = False
                                    self._lr_increase_epoch = None
                                    self._lr_increase_count = 0
                                ...
                                if optimizer and len(optimizer.param_groups) > 0:
                                    ...
                                    # Fallback: use original LR if available
                                    lr_value = getattr(self, '_original_lr', 1e-3)
                                
                                # Determine if we should increase LR
                                should_increase = False
                                increase_reason = ""
                                
                                if epoch_idx >= 5:
                                    if not self._lr_increased_for_constant_prob:
                                        # First increase: do it immediately
                                        should_increase = True
                                        increase_reason = "first increase (5x multiplier)"
                                    elif self._lr_increase_count == 1 and self._lr_increase_epoch is not None:
                                        # Second increase: only if problem persists 10+ epochs after first increase
                                        epochs_since_first = epoch_idx - self._lr_increase_epoch
                                        if epochs_since_first >= 10:
                                            should_increase = True
                                            increase_reason = f"second increase (problem persists {epochs_since_first} epochs after first)"
                                
                                if should_increase:
                                    # Increase LR by 5x (per ConstantProbabilityError exception recommendation)
                                    new_lr = lr_value * 5.0
                                    
                                    # CRITICAL: Cap at 0.01 to prevent gradient explosion
                                    # Also cap at 10x original LR to avoid going too far
                                    max_lr_absolute = 0.01
                                    original_lr = getattr(self, '_original_lr', lr_value)
                                    max_lr_relative = original_lr * 10.0
                                    max_lr = min(max_lr_absolute, max_lr_relative)
                                    ...
                                    logger.warning("="*80)
                                    logger.warning("🚨 AUTOMATIC LEARNING RATE INCREASE")
                                    logger.warning("="*80)
                                    logger.warning(f"   Detected CONSTANT_PROBABILITY at epoch {epoch_idx}")
                                    logger.warning(f"   Current LR: {lr_value:.6e}")
                                    logger.warning(f"   Increasing LR by 5x to: {new_lr:.6e} ({increase_reason})")
                                    logger.warning(f"   This should help the model escape the flat region")
                                    logger.warning("="*80)
                                    ...
                                    self._lr_increase_epoch = epoch_idx
                                    self._lr_increase_count += 1
                                ...
                                elif self._lr_increased_for_constant_prob and self._lr_increase_count < 2:
                                    # Already increased once, but not enough epochs have passed for second increase
                                    epochs_since_first = epoch_idx - (self._lr_increase_epoch or epoch_idx)
                                    epochs_needed = 10 - epochs_since_first
                                    if epochs_needed > 0:
                                        logger.debug(f"   ℹ️  CONSTANT_PROBABILITY still detected, but waiting {epochs_needed} more epochs before second LR increase")
                    ...
                    # CHECK FOR LOW LOGIT RANGE: If logit range is too low, increase LR
                    # Low logit range (< 0.5) indicates model outputs are compressed, similar to constant probability
                    if 'raw_logits_array' in locals() and raw_logits_array is not None:
                        ...
                        if logit_range_val < 0.5 and epoch_idx >= 5:
                            # Only increase LR if we haven't already increased for constant probability
                            # (to avoid double increases in the same epoch)
                            if not (hasattr(self, '_lr_increased_for_constant_prob') and self._lr_increased_for_constant_prob):
                                # Initialize tracking if needed
                                if not hasattr(self, '_lr_increased_for_low_logit_range'):
                                    self._lr_increased_for_low_logit_range = False
                                    self._lr_increase_for_logit_epoch = None
                                    self._lr_increase_for_logit_count = 0
                                ...
                                if not self._lr_increased_for_low_logit_range:
                                    # First increase: do it immediately
                                    should_increase_lr = True
                                    increase_reason = "first increase for low logit range (5x multiplier)"
                                elif self._lr_increase_for_logit_count == 1 and self._lr_increase_for_logit_epoch is not None:
                                    # Second increase: only if problem persists 10+ epochs after first increase
                                    epochs_since_first = epoch_idx - self._lr_increase_for_logit_epoch
                                    if epochs_since_first >= 10:
                                        should_increase_lr = True
                                        increase_reason = f"second increase (low logit range persists {epochs_since_first} epochs after first)"
                                
                                if should_increase_lr:
                                    # Increase LR by 5x (same as constant probability handling)
                                    new_lr = lr_value * 5.0
                                    ...
                                    logger.warning("="*80)
                                    logger.warning("🚨 AUTOMATIC LEARNING RATE INCREASE (LOW LOGIT RANGE)")
                                    logger.warning("="*80)
                                    logger.warning(f"   Detected low logit range at epoch {epoch_idx}: {logit_range_val:.4f} (< 0.5 threshold)")
                                    logger.warning(f"   Low logit range indicates model outputs are compressed/saturated")
                                    logger.warning(f"   Current LR: {lr_value:.6e}")
                                    logger.warning(f"   Increasing LR by 5x to: {new_lr:.6e} ({increase_reason})")
                                    logger.warning(f"   This should help the model escape the compressed region")
                                    logger.warning("="*80)
                                    ...
                                    # Set flag immediately after optimizer update
                                    self._lr_increased_for_low_logit_range = True
                                    self._lr_increase_for_logit_epoch = epoch_idx
                                    self._lr_increase_for_logit_count += 1
                                    
                                    # Update scheduler base_lrs if scheduler exists
                                    try:
                                        scheduler = self._training_scheduler
                                        if scheduler is not None:
                                            # For SequentialLR, update both schedulers
                                    ...
                                    logger.info(f"✅ Learning rate increased to {new_lr:.6e} - will take effect on next batch")
                                elif self._lr_increased_for_low_logit_range and self._lr_increase_for_logit_count < 2:
                                    # Already increased once, but not enough epochs have passed for second increase
                                    epochs_since_first = epoch_idx - (self._lr_increase_for_logit_epoch or epoch_idx)
                                    epochs_needed = 10 - epochs_since_first
                                    if epochs_needed > 0:
                                        logger.debug(f"   ℹ️  Low logit range still detected ({logit_range_val:.4f}), but waiting {epochs_needed} more epochs before second LR increase")
