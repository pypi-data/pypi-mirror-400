...
class PredictionResource(SyncAPIResource):
    ...
    def infer(
        ...
        *,
        ...
        """Uses a specified model to run inference on a given dataset.

        Can only use models
        trained for prediction. Inference can only be run with models that have
        completed training. By default, attempts to coerce the given dataset into the
        input schema required by the model (e.g., adding missing required columns and
