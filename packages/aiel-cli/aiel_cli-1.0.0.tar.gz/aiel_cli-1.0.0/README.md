...
- **Profile-safe authentication**: `aiel auth ...` manages multiple environments, keyring-backed tokens, and human-friendly panels.
...
# 4. Inspect context and manifests
aiel info workspace
aiel files ls

# 5. Initialize and sync a repo
aiel repo init --base-url https://api.aiel.test --workspace-id onboarding_team2 --project-id linkedin_onboarding
aiel repo pull
aiel repo status
```
...
1. `aiel auth login`  
   - Validates tokens via `/v1/auth/me` and stores them using keyring when available.  
...
1. `aiel repo init --base-url ... --workspace-id ... --project-id ...`  
   - Creates `.aiel/state.json` and `.aiel/index.json` with normalized metadata.
