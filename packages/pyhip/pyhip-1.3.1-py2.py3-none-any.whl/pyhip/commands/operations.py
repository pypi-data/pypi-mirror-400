...
def remove_faces(matching, cut, boundary, **kwargs):
    """ Remove every {matching, cut, or boundary} face that is interior
    """
    # The command string uses %d for integers
    command = "se fc-remove %s %s %s" % (matching, cut, boundary)

    return pyhip_cmd(command, **kwargs)

def keep_interior_faces(**kwargs):
    """ Keep all interior faces
    """
    remove_faces(0, 0, 0)
