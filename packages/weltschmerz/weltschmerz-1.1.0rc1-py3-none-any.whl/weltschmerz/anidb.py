        ...
        if self.online:
            ...
            try:
                ...
                if self.client:
                    ...
                    if self.debug:
                        print(f"DEBUG: {result.code} - {result.data[0]}")
                    if result.code == 210:  # mylist entry added
                        ...
                        self.dbs.session.merge(mylist_file)
                        self.dbs.session.commit()
                    elif result.code == 311:  # file already in mylist (edited)
                        if file_id:
                            mylist_file = self.dbs.session.query(
                                anime.MylistFile
                            ).filter(anime.MylistFile.fid == file_id).one()
                            if state:
                                mylist_file.ml_state = state
                            if viewed:
                                mylist_file.ml_viewed = viewed
