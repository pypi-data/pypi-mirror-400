                    ...
                    if "application/json" in request.headers.get("content-type"):
                        ...
                        if parse_form_data is True:
                            try:
                                ...
                                _logger.error(e)
                                record["post_data"] = safe_parse_literal(s=request.post_data_json)
                        else:
                            record["post_data"] = safe_parse_literal(s=request.post_data_json)
                    else:
                        record["post_data"] = safe_parse_literal(s=request.post_data)
    ...
    async def capture_network_by_route(
        ...
        async def intercept(route, request):
            ...
            if any(kw in request.url for kw in keywords):
                ...
                # 解析 form-urlencoded
                if (
                        parse_form_data is True
                        and request.post_data
                        and "application/x-www-form-urlencoded" in (request.headers.get("content-type") or "").lower()
                ):
                    try:
                        parsed = parse_qs(request.post_data, keep_blank_values=True)
                        flat = {k: v[0] if len(v) == 1 else v for k, v in parsed.items()}
