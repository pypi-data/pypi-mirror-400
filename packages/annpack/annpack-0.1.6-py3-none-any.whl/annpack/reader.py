...
MAX_DIM = _env_int("ANNPACK_MAX_DIM", 4096)
MAX_LISTS = _env_int("ANNPACK_MAX_LISTS", 1_000_000)
MAX_LIST_BYTES = _env_int("ANNPACK_MAX_LIST_BYTES", 128 * 1024 * 1024)
...
class ANNPackIndex:
    ...
    def __init__(self, path: str, probe: int = 8, max_list_bytes: Optional[int] = None):
        self.path = path
        self.probe = probe
        self._max_list_bytes = max_list_bytes or MAX_LIST_BYTES
    ...
    @classmethod
    def open(
        cls, path: str, probe: int = 8, max_list_bytes: Optional[int] = None
    ) -> "ANNPackIndex":
        """Open an ANNPack file and return a ready-to-search index."""
        obj = cls(path, probe=probe, max_list_bytes=max_list_bytes)
        obj._open()
        return obj
    ...
    def _open(self) -> None:
        ...
        if dim <= 0 or dim > MAX_DIM:
            raise ValueError("Invalid ANNPack header dim")
        if n_lists <= 0 or n_lists > MAX_LISTS:
            raise ValueError("Invalid ANNPack header n_lists")
        if n_vectors < 0:
            raise ValueError("Invalid ANNPack header values")
        if offset_table_pos <= 0:
            raise ValueError("Invalid offset table position")

        centroids_bytes = n_lists * dim * 4
        if header_size + centroids_bytes > size:
            raise ValueError("Centroid block exceeds file size")
        table_size = n_lists * 16
        if offset_table_pos + table_size > size:
        ...
        offsets = table[:, 0].astype(np.int64)
        lengths = table[:, 1].astype(np.int64)
        for off, length in zip(offsets.tolist(), lengths.tolist()):
            if off < 0 or length < 0:
                raise ValueError("Negative list offset or length")
            if length > self._max_list_bytes:
                raise ValueError("List length exceeds ANNPACK_MAX_LIST_BYTES")
            if off + length > size:
                raise ValueError("List offset/length exceeds file size")
        self._list_offsets = offsets
        self._list_lengths = lengths
