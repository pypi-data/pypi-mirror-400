...
from ..evaluators.context import ContextEvalProcessor
...
class ContextProcessor(BaseProcessor):
    def pre_process(self, args) -> Any:
        ...
        for file in files:
            ...
            data = handler.read()
            new_filename = f"{file.stem}_context{file.suffix}"
    ...
    def core_process(self, pre_data):
        ...
        required_columns_keys = ["question", "recall_context", "truth_context"]
        required_columns = [columns_config[k] for k in required_columns_keys if k in columns_config]

        if args.template is None:
            template_name = "context_prompt.yaml"
        else:
        ...
        processor = ContextEvalProcessor(
            yaml_path=yaml_path,
    ...
    def post_process(self, result: Any) -> Tuple[bool, str]:
        ...
        try:
            ...
            columns_config = getattr(args, "columns_config", {}) or {}
            col_score = columns_config.get('recall_and_truth_context_score', '召回和正确上下文相关性分数')
            col_score_desc = columns_config.get('recall_and_truth_context_score_description', '召回和正确上下文相关性得分说明')
