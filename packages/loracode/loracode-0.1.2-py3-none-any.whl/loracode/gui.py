...
from loracode.main import main as cli_main
from loracode.scrape import Scraper, has_playwright
...
def search(text=None):
    results = []
    for root, _, files in os.walk("loracode"):
        for file in files:
...
class GUI:
    ...
    def show_edit_info(self, edit):
        ...
        if commit_hash:
            ...
            if commit_hash == self.coder.last_loracode_commit_hash:
                show_undo = True
    ...
    def do_sidebar(self):
        with st.sidebar:
            st.markdown("### ◆ LoraCode")
            st.markdown("<p style='color: #6B6B6B; font-size: 12px;'>AI Coding Assistant</p>", unsafe_allow_html=True)
            st.markdown("---")
            ...
            st.markdown("---")
            st.markdown(
                "<p style='color: #6B6B6B; font-size: 11px;'>This browser version is experimental. "
                "<a href='https://github.com/loracode/loracode/issues' style='color: #569CD6;'>Report issues</a></p>",
    ...
    def do_recommended_actions(self):
        text = "LoraCode works best when your code is stored in a git repo.  \n"
        ...
        with st.expander("Recommended actions", expanded=True):
            ...
            with st.popover("Update your `.gitignore` file"):
                st.write("It's best to keep LoraCode's internal files out of your git repo.")
                self.button("Add `.loracode*` to `.gitignore`", key=random.random(), help="?")
    ...
    def do_add_files(self):
        fnames = st.multiselect(
            ...
            help=(
                ...
                " on. Lora Code will pull in other relevant code to provide context to the LLM."
            ),
        )
    ...
    def initialize_state(self):
        ...
        self.state.init("last_loracode_commit_hash", self.coder.last_loracode_commit_hash)
    ...
    def process_chat(self):
        ...
        with self.messages:
            edit = dict(
                role="edit",
                fnames=self.coder.loracode_edited_files,
            )
            if self.state.last_loracode_commit_hash != self.coder.last_loracode_commit_hash:
                edit["commit_hash"] = self.coder.last_loracode_commit_hash
                edit["commit_message"] = self.coder.last_loracode_commit_message
                commits = f"{self.coder.last_loracode_commit_hash}~1"
                diff = self.coder.repo.diff_commits(
                    self.coder.pretty,
                    commits,
                    self.coder.last_loracode_commit_hash,
                )
                edit["diff"] = diff
                self.state.last_loracode_commit_hash = self.coder.last_loracode_commit_hash
    ...
    def do_undo(self, commit_hash):
        ...
        if (
            self.state.last_loracode_commit_hash != commit_hash
            or self.coder.last_loracode_commit_hash != commit_hash
        ):
...
def gui_main():
    st.set_page_config(
        ...
        menu_items={
            ...
            "Report a bug": "https://github.com/loracode/loracode/issues",
