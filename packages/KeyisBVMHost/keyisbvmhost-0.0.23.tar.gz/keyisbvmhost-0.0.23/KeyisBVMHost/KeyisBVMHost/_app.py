...
def trim_old_files(dir_path: str | Path, max_files: int) -> int:
    ...
    # оптимизация: удалять нечего
    if count <= max_files:
        return 0

    # сортировка по времени изменения (старые -> новые)
...
class App():
    ...
    def runGNServerVMHostEnvironment(self):
        ...
        for i in os.listdir(servers_config_dir):
            ...
            if p.is_dir():
                ...
                start = None
                if os.path.exists(str(base / 'servers' / domain / 'server.py')):
                    start = str(base / 'servers' / domain / 'server.py')

                if os.path.exists(str(base / 'servers' / domain / 'start.sh')):
                    start = str(base / 'servers' / domain / 'start.sh')
    ...
    def addServerStartFile(self,
        ...
        if log_path is None and self._log_dir_path is not None:
            log_path = self._log_dir_path + f'/{_domain_from_web5_to_web2(domain)}/' + f'{datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}.log'
