...
async def edge_fulltext_search(
    ...
    if file_ids is not None:
        query_params['file_ids'] = file_ids
...
async def node_fulltext_search(
    ...
    if agent_ids is not None:
        query += "\nAND n.agent_id IN $agent_ids"
    if file_ids is not None:
        query += "\nAND n.file_id IN $file_ids"
    query += """
        WITH n, score
        ORDER BY score DESC
        LIMIT $limit
...
async def node_similarity_search(
    ...
    if group_ids is not None:
        ...
        query_params['group_ids'] = group_ids
    if agent_ids is not None:
        group_filter_query += ' AND n.agent_id IN $agent_ids'
        query_params['agent_ids'] = agent_ids
    if file_ids is not None:
        group_filter_query += ' AND n.file_id IN $file_ids'
        query_params['file_ids'] = file_ids
...
async def episode_fulltext_search(
    ...
    # 如果有 agent_ids 过滤，添加到参数中
    if agent_ids is not None:
        query_params['agent_ids'] = agent_ids
    # 如果有 file_ids 过滤，添加到参数中
    if file_ids is not None:
        query_params['file_ids'] = file_ids
...
async def community_similarity_search(
    ...
    if agent_ids is not None:
        if group_filter_query:
            ...
            group_filter_query = 'WHERE n.agent_id IN $agent_ids'
        query_params['agent_ids'] = agent_ids
    if file_ids is not None:
        if group_filter_query:
            group_filter_query += ' AND n.file_id IN $file_ids'
        else:
            group_filter_query = 'WHERE n.file_id IN $file_ids'
        query_params['file_ids'] = file_ids
