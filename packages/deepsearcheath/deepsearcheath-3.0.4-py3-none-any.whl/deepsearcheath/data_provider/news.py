            ...
            try:
                ...
                while page <= max_pages:
                    ...
                    for item in data:
                        ...
                        # companies 리스트에서 첫 번째 심볼 추출
                        if resp_companies and isinstance(resp_companies, list) and len(resp_companies) > 0:
                            first_comp = resp_companies[0]
                            if isinstance(first_comp, dict):
                                assigned_symbol = first_comp.get('symbol')
                        
                        # 응답에 없으면 요청했던 심볼(fallback) 사용, 그것도 없으면 None (키워드 검색인 경우)
                        if not assigned_symbol:
                            assigned_symbol = symbol
                            
                        processed_item['symbol'] = assigned_symbol
