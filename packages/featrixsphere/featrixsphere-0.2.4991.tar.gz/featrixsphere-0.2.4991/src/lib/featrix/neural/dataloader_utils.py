...
def create_dataloader_kwargs(
    ...
    if prefetch_factor == 16:  # Only adjust if using default
        ...
        elif num_columns and num_columns >= 100:
            prefetch_factor = 8  # Reduced from 16 for wide datasets
            logger.debug(f"   Reduced prefetch_factor to {prefetch_factor} for wide dataset ({num_columns} columns)")
        elif num_columns and num_columns >= 50:
            prefetch_factor = 12  # Slightly reduced for medium-wide datasets
