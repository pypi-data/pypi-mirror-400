...
def make_python_create_api(
    ...
    """Make an endpoint for creating a resource using the graph API"""
    app = target_pkg.app
    ...
    node_id = f"{collection_module}.create"

    create_controlled_props = [
        prop
        for prop in cls.properties.values()
        if prop.data.system_controlled is not None
        and prop.data.system_controlled.is_on_create_value_updated()
    ]
    if len(create_controlled_props) > 0:
        program.import_("sera.libs.api_helper.SingleAutoUSCP", True)

    # Check if any controlled property is the ID property itself
    # In this case, we have to reject the request (it doesn't make sense to create a record with a fixed ID value)
    is_id_controlled = any(prop.name == idprop.name for prop in create_controlled_props)
    if is_id_controlled:
            ...
            *(
                [
                    ...
                    lambda ast_if: ast_if.if_(
                        ...
                        # raise HTTPException(status_code=status_codes.HTTP_403_FORBIDDEN, detail="User is not authorized to perform this action")
