...
def setup_mlflow_user_id() -> str:
    ...
    从环境变量 得出ff6toikf67风格突出v 获取用户 UIN，并设置到 MLFLOW_TRACKING_USERNAME 环境变量。
...
class FLAMLTrainer:
    ...
    def _log_forecast_model(
        ...
        # pip_requirements=None 让 MLflow 自动推断依赖（与 sklearn.log_model 行为一致）
        # extra_pip_requirements 添加额外依赖（如 tencent-wedata-auto-ml）
    ...
    def _get_forecast_extra_pip_requirements(self) -> List[str]:
        """
        获取 forecast 模型的额外 pip 依赖（MLflow 无法自动推断的包）

        MLflow 会自动推断大部分依赖（pandas, numpy, scikit-learn 等），
        但以下包需要手动添加：
        - tencent-wedata-auto-ml: 本包，包含 ForecastModelWrapper 类

        Returns:
            额外 pip 依赖列表（格式：package==version）
        """
        extra_requirements = []

        # 添加本包（包含 ForecastModelWrapper，加载模型时必需）
        pkg_version = self._get_package_version("tencent-wedata-auto-ml")
        if pkg_version:
            extra_requirements.append(f"tencent-wedata-auto-ml=={pkg_version}")
        else:
            extra_requirements.append("tencent-wedata-auto-ml")
