..
   This documentation page is generated during the Sphinx build.
   The underlying code is manually maintained and not autogenerated.

eegdash.dataset.DS005345
========================

Le Petit Prince (LPP) Multi-talker: Naturalistic 7T fMRI and EEG Dataset (OpenNeuro ``ds005345``). Access recordings and metadata through EEGDash.

.. rst-class:: sd-badges

:bdg-light:`Modality: eeg` :bdg-light:`Tasks: 1` :bdg-light:`License: CC0` :bdg-light:`Subjects: 26` :bdg-light:`Recordings: 26` :bdg-light:`Source: openneuro`

Dataset Information
-------------------

.. list-table::
   :widths: 25 75
   :header-rows: 0

   * - Dataset ID
     - ``DS005345``
   * - Title
     - Le Petit Prince (LPP) Multi-talker: Naturalistic 7T fMRI and EEG Dataset
   * - Year
     - Unknown
   * - Authors
     - Zhengwu Ma, Nan Wang, Jixing Li
   * - License
     - CC0
   * - Citation / DOI
     - `doi:10.18112/openneuro.ds005345.v1.0.1 <https://doi.org/10.18112/openneuro.ds005345.v1.0.1>`__
   * - Source links
     - `OpenNeuro <https://openneuro.org/datasets/ds005345>`__ | `NeMAR <https://nemar.org/dataexplorer/detail?dataset_id=ds005345>`__

.. dropdown:: Copy-paste BibTeX
   :class-container: sd-shadow-sm
   :class-title: sd-bg-light

   .. code-block:: bibtex

      @dataset{ds005345,
        title = {Le Petit Prince (LPP) Multi-talker: Naturalistic 7T fMRI and EEG Dataset},
        author = {Zhengwu Ma and Nan Wang and Jixing Li},
        doi = {10.18112/openneuro.ds005345.v1.0.1},
        url = {https://doi.org/10.18112/openneuro.ds005345.v1.0.1},
      }

Highlights
----------

.. grid:: 1 2 3 3
   :gutter: 2

   .. grid-item-card:: Subjects & recordings
      :class-card: sd-border-1

      - Subjects: 26
      - Recordings: 26
      - Tasks: 1

   .. grid-item-card:: Channels & sampling rate
      :class-card: sd-border-1

      - Channels: 64
      - Sampling rate (Hz): 500
      - Duration (hours): 0.0

   .. grid-item-card:: Tasks & conditions
      :class-card: sd-border-1

      - Tasks: 1
      - Experiment type: Unknown
      - Subject type: Healthy

   .. grid-item-card:: Files & format
      :class-card: sd-border-1

      - Size on disk: 162.54 GB
      - File count: Unknown
      - Format: BIDS

   .. grid-item-card:: License & citation
      :class-card: sd-border-1

      - License: CC0
      - DOI: doi:10.18112/openneuro.ds005345.v1.0.1

   .. grid-item-card:: Provenance
      :class-card: sd-border-1

      - Source: openneuro
      - OpenNeuro: `ds005345 <https://openneuro.org/datasets/ds005345>`__
      - NeMAR: `ds005345 <https://nemar.org/dataexplorer/detail?dataset_id=ds005345>`__

Quickstart
----------

**Install**

.. code-block:: bash

   pip install eegdash

**Load a recording**

.. code-block:: python

   from eegdash.dataset import DS005345

   dataset = DS005345(cache_dir="./data")
   recording = dataset[0]
   raw = recording.load()

**Filter/query**

.. tab-set::

   .. tab-item:: Basic

      .. code-block:: python

         dataset = DS005345(cache_dir="./data", subject="01")

   .. tab-item:: Advanced

      .. code-block:: python

         dataset = DS005345(
             cache_dir="./data",
             query={"subject": {"$in": ["01", "02"]}},
         )


Quality & caveats
-----------------

- No dataset-specific caveats are listed in the available metadata.

API
---

.. currentmodule:: eegdash.dataset

.. autoclass:: eegdash.dataset.DS005345
   :members: __init__, save
   :show-inheritance:
   :member-order: bysource


See Also
--------

* :class:`eegdash.dataset.EEGDashDataset`
* :mod:`eegdash.dataset`
* `OpenNeuro dataset page <https://openneuro.org/datasets/ds005345>`__
* `NeMAR dataset page <https://nemar.org/dataexplorer/detail?dataset_id=ds005345>`__

