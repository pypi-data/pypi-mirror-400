...
impl PyResult {
    ...
    pub fn from_dict(py: Python<'_>, dict: &Bound<'_, PyDict>) -> PyResult2<Self> {
        ...
        if is_ok {
            ...
            let error: String = dict.get_item("error")?
                .ok_or_else(|| PyValueError::new_err("Missing 'error' key for Err result"))?
                .extract()?;
            let code: Option<String> = dict.get_item("code")?
                .and_then(|v| v.extract().ok());
            let details: Option<PyObject> = dict.get_item("details")?
    ...
    pub fn expect(&self, py: Python<'_>, message: String) -> PyResult2<PyObject> {
        self.value
            .as_ref()
    ...
    pub fn to_option(&self, py: Python<'_>) -> Option<PyObject> {
    ...
    pub fn match_(&self, py: Python<'_>, ok: Option<PyObject>, err: Option<PyObject>) -> PyResult2<PyObject> {
    ...
    #[classmethod]
    fn __class_getitem__(cls: &Bound<'_, PyType>, _item: PyObject) -> PyObject {
