...
def _create_buttons_box():
    ...
    def _edit_profiles(trigger):
        assert _find_selected_device_id() is not None
        device = _find_selected_device()
        assert device is not None
        profile_dialog.edit_device_profiles(device)

    bb._edit_profiles = _new_button(_("Edit Device Profiles"), "format-justify-fill", clicked=_edit_profiles)
    bb.add(bb._edit_profiles)
...
def _create_window_layout():
    ...
    profiles_button = _new_button(
        _("Saved Profile Editor"), "", _SMALL_BUTTON_ICON_SIZE, clicked=lambda *_trigger: profile_dialog.edit_saved_profiles()
    )
    bottom_buttons_box.add(profiles_button)
...
def _update_receiver_panel(receiver, panel, buttons, full=False):
    ...
    buttons._edit_profiles.set_visible(False)
...
def _update_device_panel(device, panel, buttons, full=False):
    ...
    buttons._edit_profiles.set_visible(False)
    buttons._edit_profiles.set_sensitive(device.profiles_new)
    buttons._edit_profiles.set_visible(True)
