...
import unittest
import le
from le import _cloud_api_utils
from le import apitestcase
from le import ee_exception
from le import serializer
...
class ImageTest(apitestcase.ApiTestCase):
  ...
  def test_constructors(self):
    ...
    from_constant = le.Image(1)
    self.assertEqual(
        le.ApiFunction.lookup('Image.constant'), from_constant.func)
    self.assertEqual({'value': 1}, from_constant.args)

    array_constant = le.Array([1, 2])
    from_array_constant = le.Image(array_constant)
    self.assertEqual(
        le.ApiFunction.lookup('Image.constant'), from_array_constant.func)
    self.assertEqual({'value': array_constant}, from_array_constant.args)

    from_id = le.Image('abcd')
    self.assertEqual(le.ApiFunction.lookup('Image.load'), from_id.func)
    self.assertEqual({'id': 'abcd'}, from_id.args)

    from_array = le.Image([1, 2])
    self.assertEqual(le.ApiFunction.lookup('Image.addBands'), from_array.func)
    self.assertEqual({
        'dstImg': le.Image(1),
        'srcImg': le.Image(2)
    }, from_array.args)

    from_computed_object = le.Image(le.ComputedObject(None, {'x': 'y'}))
    self.assertEqual({'x': 'y'}, from_computed_object.args)

    original = le.Image(1)
    from_other_image = le.Image(original)
    self.assertEqual(from_other_image, original)

    from_nothing = le.Image()
    self.assertEqual(le.ApiFunction.lookup('Image.mask'), from_nothing.func)
    self.assertEqual({
        'image': le.Image(0),
        'mask': le.Image(0)
    }, from_nothing.args)

    from_id_and_version = le.Image('abcd', 123)
    self.assertEqual(
        le.ApiFunction.lookup('Image.load'), from_id_and_version.func)
    self.assertEqual({'id': 'abcd', 'version': 123}, from_id_and_version.args)

    from_variable = le.Image(le.CustomFunction.variable(None, 'foo'))
    self.assertIsInstance(from_variable, le.Image)
    self.assertEqual({
        'type': 'ArgumentRef',
        'value': 'foo'
  ...
  def test_image_signatures(self):
    ...
    self.assertTrue(hasattr(le.Image(1), 'addBands'))
  ...
  def test_get_map_id_visualization(self):
    ...
    self.assertEqual(
        le.Image(1).visualize(min=0).serialize(),
  ...
  def test_combine(self):
    ...
    image1 = le.Image([1, 2])
    image2 = le.Image([3, 4])
    combined = le.Image.combine_([image1, image2], ['a', 'b', 'c', 'd'])

    self.assertEqual(le.ApiFunction.lookup('Image.select'), combined.func)
    self.assertEqual(le.List(['.*']), combined.args['bandSelectors'])
    self.assertEqual(le.List(['a', 'b', 'c', 'd']), combined.args['newNames'])
    self.assertEqual(
        le.ApiFunction.lookup('Image.addBands'), combined.args['input'].func)
    self.assertEqual({
        'dstImg': image1,
        'srcImg': image2
  ...
  def test_select(self):
    ...
    image = le.Image([1, 2]).select()
    self.assertEqual(le.ApiFunction.lookup('Image.select'), image.func)
    self.assertEqual(le.List([]), image.args['bandSelectors'])

  def test_rename(self):
    """Verifies image.rename varargs handling."""
    image = le.Image([1, 2]).rename('a', 'b')
    self.assertEqual(le.ApiFunction.lookup('Image.rename'), image.func)
    self.assertEqual(le.List(['a', 'b']), image.args['names'])

    image = le.Image([1, 2]).rename(['a', 'b'])
    self.assertEqual(le.ApiFunction.lookup('Image.rename'), image.func)
    self.assertEqual(le.List(['a', 'b']), image.args['names'])

    image = le.Image([1]).rename('a')
    self.assertEqual(le.ApiFunction.lookup('Image.rename'), image.func)
    self.assertEqual(le.List(['a']), image.args['names'])

  def test_expression(self):
    """Verifies the behavior of ee.Image.expression()."""
    image = le.Image([1, 2]).expression('a', {'b': 'c'})
    ...
    def dummy_encoder(x):
      if isinstance(x, le.encodable.Encodable):
        return x.encode(dummy_encoder)
      elif isinstance(x, le.encodable.EncodableFunction):
  ...
  def test_expression_in_cloud_api(self):
    ...
    image = le.Image(1).expression('a', {'b': 'c'})
...
class CloudThumbnailAndExportImageTest(apitestcase.ApiTestCase):
  ...
  def _expected_geometry(self):
    return le.Geometry(self._geo_json, geodesic=False)
  ...
  def test_thumb_with_dimensions_region_crs(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      le.data.getThumbId = self.old_get_thumb_id
      ...
      self.assertEqual(
          ...
          serializer.encode(
              self._base_image.setDefaultProjection(
                  ...
                  geometry=le.Geometry(self._geo_json, geodesic=False),
                  width=13,
                  height=42,
              )
          ),
  ...
  def test_thumb_with_dimensions_list_coords(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      ...
      _, kwargs = cloud_api_resource.projects().thumbnails().create.call_args
      expected_geometry = le.Geometry.Polygon(
  ...
  def test_thumb_with_dimensions_list_min_max(self):
    ...
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      ...
      _, kwargs = cloud_api_resource.projects().thumbnails().create.call_args
      expected_geometry = le.Geometry.Rectangle(
  ...
  def test_build_download_id_image_handles_invalid_parameters(self):
    ...
    with self.assertRaisesRegex(
      ...
      le.Image('foo')._build_download_id_image(params)
  ...
  def test_download_url(self):
    ...
    cloud_api_resource = mock.MagicMock()
    with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
      le.data.getDownloadId = self.old_get_download_id
      cloud_api_resource.projects().thumbnails().create().execute.return_value = {
          'name': 'thumbName'
      }
  ...
  def test_prepare_for_export_with_polygon(self):
    with apitestcase.UsingCloudApi():
      polygon = le.Geometry.Polygon(9, 8, 7, 6, 3, 2)
      image, params = self._base_image.prepare_for_export(
  ...
  def test_prepare_for_export_with_scale_and_region(self):
    with apitestcase.UsingCloudApi():
      ...
      expected_polygon = le.Geometry(polygon.toGeoJSON(), geodesic=False)
      self.assertImageEqual(
          self._base_image.clipToBoundsAndScale(
  ...
  def test_prepare_for_export_with_dimensions_crs_and_transform(self):
    with apitestcase.UsingCloudApi():
      ...
      expected_expression = reprojected_image.clipToBoundsAndScale(
          geometry=le.Geometry.Rectangle(
              coords=[0, 0, 3, 2],
...
class SerializeTest(apitestcase.ApiTestCase):
  ...
  def test_add(self):
    ...
    expression = le.Image('a').add('b')
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').add(image2='b')
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_add_bands(self):
    ...
    expression = le.Image('a').addBands('b', names, overwrite)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').addBands(
  ...
  def test_and(self):
    ...
    expression = le.Image('a').And('b')
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').And(image2='b')
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

  def test_array_accum(self):
    axis = 1
    reducer = le.Reducer.sum()
    expect = make_expression_graph({
        'arguments': {
            'input': IMAGE,
    ...
    expression = le.Image('a').arrayAccum(axis, reducer)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').arrayAccum(axis=axis, reducer=reducer)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_array_argmax(self):
    ...
    expression = le.Image('a').arrayArgmax()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_array_cat(self):
    ...
    expression = le.Image('a').arrayCat('b', axis)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').arrayCat(image2='b', axis=axis)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_array_dimensions(self):
    ...
    expression = le.Image('a').arrayDimensions()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_array_dot_product(self):
    ...
    expression = le.Image('a').arrayDotProduct('b')
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').arrayDotProduct(image2='b')
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_array_flatten(self):
    ...
    expression = le.Image('a').arrayFlatten(coordinate_labels, separator)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').arrayFlatten(
  ...
  def test_array_get(self):
    ...
    expression = le.Image('a').arrayGet('b')
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').arrayGet(position='b')
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_array_length(self):
    ...
    expression = le.Image('a').arrayLength(axis)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').arrayLength(axis=axis)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_array_lengths(self):
    ...
    expression = le.Image('a').arrayLengths()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_array_mask(self):
    ...
    expression = le.Image('a').arrayMask('b')
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').arrayMask(mask='b')
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_array_pad(self):
    ...
    expression = le.Image('a').arrayPad(lengths, pad)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').arrayPad(lengths=lengths, pad=pad)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_array_project(self):
    ...
    expression = le.Image('a').arrayProject(axes)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').arrayProject(axes=axes)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_array_reduce(self):
    ...
    expression = le.Image('a').arrayReduce(reducer, axes, field_axes)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').arrayReduce(
  ...
  def test_array_repeat(self):
    ...
    expression = le.Image('a').arrayRepeat(axis, copies)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').arrayRepeat(axis=axis, copies=copies)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_array_reshape(self):
    ...
    expression = le.Image('a').arrayReshape(lengths, dimensions)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').arrayReshape(
  ...
  def test_array_slice(self):
    ...
    expression = le.Image('a').arraySlice(axis, start, end, step)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').arraySlice(
  ...
  def test_array_sort(self):
    ...
    expression = le.Image('a').arraySort(keys)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_array_transpose(self):
    ...
    expression = le.Image('a').arrayTranspose(axis1, axis2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').arrayTranspose(axis1=axis1, axis2=axis2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_atan2(self):
    ...
    expression = le.Image('a').atan2(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').atan2(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_band_names(self):
    ...
    expression = le.Image('a').bandNames()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_band_types(self):
    ...
    expression = le.Image('a').bandTypes()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_bit_count(self):
    ...
    expression = le.Image('a').bitCount()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_bits_to_array_image(self):
    ...
    expression = le.Image('a').bitsToArrayImage()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_bitwise_and(self):
    ...
    expression = le.Image('a').bitwiseAnd(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').bitwiseAnd(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_bitwise_not(self):
    ...
    expression = le.Image('a').bitwiseNot()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_bitwise_or(self):
    ...
    expression = le.Image('a').bitwiseOr(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').bitwiseOr(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_bitwise_xor(self):
    ...
    expression = le.Image('a').bitwiseXor(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').bitwiseXor(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_cast(self):
    ...
    expression = le.Image('a').cast(band_types, band_order)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').cast(bandTypes=band_types, bandOrder=band_order)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_cat(self):
    ...
    expression = le.Image.cat(images)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_change_proj(self):
    ...
    expression = le.Image('a').changeProj(src_proj, dst_proj)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').changeProj(srcProj=src_proj, dstProj=dst_proj)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_clamp(self):
    ...
    expression = le.Image('a').clamp(low, high)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').clamp(low=low, high=high)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

  def test_classify(self):
    classifier = le.Classifier.decisionTree('')
    ...
    expression = le.Image('a').classify(classifier, output_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').classify(
  ...
  def test_clip(self):
    ...
    expression = le.Image('a').clip(point)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').clip(clip_geometry=point)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_clip_to_bounds_and_scale(self):
    ...
    expression = le.Image('a').clipToBoundsAndScale(
  ...
  def test_clip_to_collection(self):
    featurecollection = le.FeatureCollection(le.Feature(None))
    expect = make_expression_graph({
        'arguments': {
            'input': IMAGE,
            'collection': {
    ...
    expression = le.Image('a').clipToCollection(featurecollection)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').clipToCollection(collection=featurecollection)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

  def test_cluster(self):
    clusterer = le.Clusterer.wekaCobweb()
    output_name = 'output name'
    expect = make_expression_graph({
        'arguments': {
            'image': IMAGE,
    ...
    expression = le.Image('a').cluster(clusterer, output_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').cluster(
  ...
  def test_connected_components(self):
    connectedness = le.Kernel.square(1)
    max_size = 2
    expect = make_expression_graph({
        'arguments': {
            'image': IMAGE,
    ...
    expression = le.Image('a').connectedComponents(connectedness, max_size)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').connectedComponents(
  ...
  def test_connected_pixel_count(self):
    ...
    expression = le.Image('a').connectedPixelCount(max_size, eight_connected)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').connectedPixelCount(
  ...
  def test_constant(self):
    ...
    expression = le.Image('a').constant(value)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image.constant(value=value)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

  def test_convolve(self):
    kernel = le.Kernel.square(1)
    expect = make_expression_graph({
        'arguments': {
            'image': IMAGE,
            'kernel': {
    ...
    expression = le.Image('a').convolve(kernel)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').convolve(kernel=kernel)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_copy_properties(self):
    ...
    expression = le.Image('a').copyProperties(source, properties, exclude)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').copyProperties(
  ...
  def test_cumulative_cost(self):
    ...
    expression = le.Image('a').cumulativeCost(
  ...
  def test_derivative(self):
    ...
    expression = le.Image('a').derivative()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_directional_distance_transform(self):
    ...
    expression = le.Image('a').directionalDistanceTransform(
  ...
  def test_displace(self):
    ...
    expression = le.Image('a').displace(displacement, mode, max_offset)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').displace(
  ...
  def test_displacement(self):
    ...
    expression = le.Image('a').displacement(
  ...
  def test_distance(self):
    ...
    expression = le.Image('a').distance(kernel, skip_masked)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').distance(kernel=kernel, skipMasked=skip_masked)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_divide(self):
    ...
    expression = le.Image('a').divide(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').divide(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_entropy(self):
    ...
    expression = le.Image('a').entropy(kernel)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').entropy(kernel=kernel)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_eq(self):
    ...
    expression = le.Image('a').eq(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').eq(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_fast_distance_transform(self):
    ...
    expression = le.Image('a').fastDistanceTransform(
  ...
  def test_first(self):
    ...
    expression = le.Image('a').first(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').first(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_first_non_zero(self):
    ...
    expression = le.Image('a').firstNonZero(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').firstNonZero(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_focal_max(self):
    ...
    expression = le.Image('a').focalMax(
  ...
  def test_focal_mean(self):
    ...
    expression = le.Image('a').focalMean(
        radius, kernel_type, units, iterations, kernel
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_focal_median(self):
    ...
    expression = le.Image('a').focalMedian(
        radius, kernel_type, units, iterations, kernel
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_focal_min(self):
    ...
    expression = le.Image('a').focalMin(
        radius, kernel_type, units, iterations, kernel
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_focal_mode(self):
    ...
    expression = le.Image('a').focalMode(
        radius, kernel_type, units, iterations, kernel
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_gammainc(self):
    ...
    expression = le.Image('a').gammainc(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').gammainc(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_geometry(self):
    ...
    expression = le.Image('a').geometry(max_error, proj, geodesics)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').geometry(
  ...
  def test_glcm_texture(self):
    ...
    expression = le.Image('a').glcmTexture(size, kernel, average)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').glcmTexture(
  ...
  def test_gradient(self):
    ...
    expression = le.Image('a').gradient()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_gt(self):
    ...
    expression = le.Image('a').gt(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').gt(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_gte(self):
    ...
    expression = le.Image('a').gte(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').gte(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_hers_descriptor(self):
    ...
    expression = le.Image('a').hersDescriptor(
  ...
  def test_hers_feature(self):
    ...
    expression = le.Image('a').hersFeature(reference, peak_width_scale)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').hersFeature(
  ...
  def test_hers_image(self):
    ...
    expression = le.Image('a').hersImage(
  ...
  def test_hsv_to_rgb(self):
    ...
    expression = le.Image('a').hsvToRgb()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_hypot(self):
    ...
    expression = le.Image('a').hypot(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').hypot(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_interpolate(self):
    ...
    expression = le.Image('a').interpolate(x, y, behavior)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').interpolate(x=x, y=y, behavior=behavior)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_left_shift(self):
    ...
    expression = le.Image('a').leftShift(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').leftShift(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

  def test_link_collection(self):
    image_collection = le.ImageCollection('b')
    linked_bands = 'a band'
    ...
    expression = le.Image('a').linkCollection(
  ...
  def test_load(self):
    ...
    expression = le.Image.load(asset_id, version)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image.load(id=asset_id, version=version)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_load_geotiff(self):
    ...
    expression = le.Image.loadGeoTIFF(uri)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').loadGeoTIFF(uri=uri)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_lt(self):
    ...
    expression = le.Image('a').lt(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').lt(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_lte(self):
    ...
    expression = le.Image('a').lte(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').lte(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_mask(self):
    ...
    expression = le.Image('a').mask(mask)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').mask(mask=mask)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_matrix_cholesky_decomposition(self):
    ...
    expression = le.Image('a').matrixCholeskyDecomposition()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_matrix_determinant(self):
    ...
    expression = le.Image('a').matrixDeterminant()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_matrix_diagonal(self):
    ...
    expression = le.Image('a').matrixDiagonal()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_matrix_fnorm(self):
    ...
    expression = le.Image('a').matrixFnorm()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_matrix_identity(self):
    ...
    expression = le.Image.matrixIdentity(size)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_matrix_inverse(self):
    ...
    expression = le.Image('a').matrixInverse()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_matrix_lu_decomposition(self):
    ...
    expression = le.Image('a').matrixLUDecomposition()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_matrix_multiply(self):
    ...
    expression = le.Image('a').matrixMultiply(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').matrixMultiply(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_matrix_pseudo_inverse(self):
    ...
    expression = le.Image('a').matrixPseudoInverse()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_matrix_qr_decomposition(self):
    ...
    expression = le.Image('a').matrixQRDecomposition()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_matrix_singular_value_decomposition(self):
    ...
    expression = le.Image('a').matrixSingularValueDecomposition()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_matrix_solve(self):
    ...
    expression = le.Image('a').matrixSolve(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').matrixSolve(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_matrix_to_diag(self):
    ...
    expression = le.Image('a').matrixToDiag()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_matrix_trace(self):
    ...
    expression = le.Image('a').matrixTrace()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_matrix_transpose(self):
    ...
    expression = le.Image('a').matrixTranspose(axis1, axis2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').matrixTranspose(axis1=axis1, axis2=axis2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_max(self):
    ...
    expression = le.Image('a').max(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').max(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_medial_axis(self):
    ...
    expression = le.Image('a').medialAxis(neighborhood, units)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').medialAxis(
  ...
  def test_metadata(self):
    ...
    expression = le.Image('a').metadata(property, name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').metadata(property=property, name=name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_min(self):
    ...
    expression = le.Image('a').min(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').min(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_mod(self):
    ...
    expression = le.Image('a').mod(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').mod(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_multiply(self):
    ...
    expression = le.Image('a').multiply(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').multiply(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_neighborhood_to_array(self):
    ...
    expression = le.Image('a').neighborhoodToArray(kernel, default_value)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').neighborhoodToArray(
  ...
  def test_neighborhood_to_bands(self):
    ...
    expression = le.Image('a').neighborhoodToBands(kernel)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').neighborhoodToBands(kernel=kernel)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_neq(self):
    ...
    expression = le.Image('a').neq(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').neq(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_normalized_difference(self):
    ...
    expression = le.Image('a').normalizedDifference(band_names)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').normalizedDifference(bandNames=band_names)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_or(self):
    ...
    expression = le.Image('a').Or(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').Or(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

  def test_paint(self):
    featurecollection = le.FeatureCollection('b')
    color = 'cadetblue'
    width = 1
    expect = make_expression_graph({
        'arguments': {
    ...
    expression = le.Image('a').paint(featurecollection, color, width)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').paint(
  ...
  def test_pixel_coordinates(self):
    ...
    expression = le.Image('a').pixelCoordinates(projection)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').pixelCoordinates(projection=projection)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_polynomial(self):
    ...
    expression = le.Image('a').polynomial(coefficients)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').polynomial(coefficients=coefficients)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_pow(self):
    ...
    expression = le.Image('a').pow(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').pow(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_projection(self):
    ...
    expression = le.Image('a').projection()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_random(self):
    ...
    expression = le.Image.random(seed, distribution)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image().random(seed=seed, distribution=distribution)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_random_visualizer(self):
    ...
    expression = le.Image('a').randomVisualizer()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_reduce(self):
    ...
    expression = le.Image('a').reduce(reducer)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').reduce(reducer=reducer)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_reduce_connected_components(self):
    ...
    expression = le.Image('a').reduceConnectedComponents(
  ...
  def test_reduce_neighborhood(self):
    ...
    expression = le.Image('a').reduceNeighborhood(
  ...
  def test_reduce_region(self):
    reducer = le.Reducer.sum()
    geometry = le.Geometry.Point([1, 2])
    scale = 1
    crs = EPSG_4326
    ...
    expression = le.Image('a').reduceRegion(
        reducer,
        geometry,
        scale,
        crs,
  ...
  def test_reduce_regions(self):
    ...
    expression = le.Image('a').reduceRegions(
        featurecollection,
        reducer,
        scale,
        crs,
  ...
  def test_reduce_resolution(self):
    ...
    expression = le.Image('a').reduceResolution(
  ...
  def test_reduce_to_vectors(self):
    ...
    expression = le.Image('a').reduceToVectors(
        reducer,
        geometry,
        scale,
        geometry_type,
  ...
  def test_regexp_rename(self):
    ...
    expression = le.Image('a').regexpRename(regex, replacement, all)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').regexpRename(
  ...
  def test_register(self):
    ...
    expression = le.Image('a').register(
  ...
  def test_remap(self):
    ...
    expression = le.Image('a').remap(from_list, to, default_value, band_name)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').remap(
        to=to,
  ...
  def test_rename(self):
    ...
    expression = le.Image('a').rename(names)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_reproject(self):
    ...
    expression = le.Image('a').reproject(crs, crs_transform, scale)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').reproject(
  ...
  def test_resample(self):
    ...
    expression = le.Image('a').resample(mode)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').resample(mode=mode)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_rgb_to_hsv(self):
    ...
    expression = le.Image('a').rgbToHsv()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_right_shift(self):
    ...
    expression = le.Image('a').rightShift(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').rightShift(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_rsed_transform(self):
    ...
    expression = le.Image('a').rsedTransform(neighborhood, units)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').rsedTransform(
        neighborhood=neighborhood, units=units
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_sample_rectangle(self):
    point = [1, 2]
    region = le.Geometry.Point(point)
    properties = ['b', 'c']
    default_value = 3
    ...
    expression = le.Image('a').sampleRectangle(
  ...
  def test_sample_regions(self):
    ...
    expression = le.Image('a').sampleRegions(
  ...
  def test_select(self):
    ...
    expression = le.Image('a').select(band_selectors, new_names)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_self_mask(self):
    ...
    expression = le.Image('a').selfMask()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_set_default_projection(self):
    ...
    expression = le.Image('a').setDefaultProjection(crs, crs_transform, scale)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').setDefaultProjection(
        crs=crs, crsTransform=crs_transform, scale=scale
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_sld_style(self):
    ...
    expression = le.Image('a').sldStyle(sld_xml)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').sldStyle(sldXml=sld_xml)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_slice(self):
    ...
    expression = le.Image('a').slice(start, end)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').slice(start=start, end=end)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_spectral_dilation(self):
    ...
    expression = le.Image('a').spectralDilation(metric, kernel, use_centroid)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').spectralDilation(
  ...
  def test_spectral_distance(self):
    ...
    expression = le.Image('a').spectralDistance(image2, metric)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').spectralDistance(image2=image2, metric=metric)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_spectral_erosion(self):
    ...
    expression = le.Image('a').spectralErosion(metric, kernel, use_centroid)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').spectralErosion(
        metric=metric, kernel=kernel, useCentroid=use_centroid
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_spectral_gradient(self):
    ...
    expression = le.Image('a').spectralGradient(metric, kernel, use_centroid)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').spectralGradient(
        metric=metric, kernel=kernel, useCentroid=use_centroid
    )
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_stratified_sample(self):
    ...
    expression = le.Image('a').stratifiedSample(
        num_points,
        class_band,
        region,
        scale,
  ...
  def test_subtract(self):
    ...
    expression = le.Image('a').subtract(image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').subtract(image2=image2)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_to_array(self):
    ...
    expression = le.Image('a').toArray(axis)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').toArray(axis=axis)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_to_double(self):
    ...
    expression = le.Image('a').toDouble()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_to_uint16(self):
    ...
    expression = le.Image('a').toUint16()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_to_uint32(self):
    ...
    expression = le.Image('a').toUint32()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_translate(self):
    ...
    expression = le.Image('a').translate(x, y, units, proj)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').translate(x=x, y=y, units=units, proj=proj)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_trigamma(self):
    ...
    expression = le.Image('a').trigamma()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_unit_scale(self):
    ...
    expression = le.Image('a').unitScale(low, high)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').unitScale(low=low, high=high)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_unmask(self):
    ...
    expression = le.Image('a').unmask(value, same_footprint)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').unmask(value=value, sameFootprint=same_footprint)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_unmix(self):
    ...
    expression = le.Image('a').unmix(endmembers, sum_to_one, non_negative)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').unmix(
  ...
  def test_update_mask(self):
    ...
    expression = le.Image('a').updateMask(mask)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').updateMask(mask=mask)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_visualize(self):
    ...
    expression = le.Image('a').visualize(
        bands,
        gain,
        bias,
        min_values,
  ...
  def test_where(self):
    ...
    expression = le.Image('a').where(test, value)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)

    expression = le.Image('a').where(test=test, value=value)
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
  ...
  def test_zero_crossing(self):
    ...
    expression = le.Image('a').zeroCrossing()
    result = json.loads(expression.serialize())
    self.assertEqual(expect, result)
