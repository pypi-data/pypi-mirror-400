"""Hand-written Kotlin lexer using composable scanner mixins.
...
__all__ = ["KotlinStateMachineLexer"]
...
class KotlinStateMachineLexer(
    CStyleCommentsMixin,
    CStyleNumbersMixin,
    CStyleOperatorsMixin,
    StateMachineLexer,
):
    """Kotlin lexer using composable mixins."""
        ...
        *,
        ...
        while pos < length:
            ...
            if char == "/" and pos + 1 < length:
                ...
                if next_char == "*":
                    start = pos
                    is_kdoc = pos + 2 < length and code[pos + 2] == "*"
                    pos = scan_block_comment(code, pos + 2, "*/")
                    value = code[start:pos]
                    newlines = value.count("\n")
                    token_type = TokenType.STRING_DOC if is_kdoc else TokenType.COMMENT_MULTILINE
