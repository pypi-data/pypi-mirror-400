...
def make_params(self):
    directory_raw = self.dir_in.text().strip()
    if not directory_raw:
        raise ValueError("Directory is missing.")

    directory = os.path.realpath(os.path.expanduser(directory_raw))

    if not os.path.isdir(directory):
        raise ValueError(f"Directory is not a valid folder: {directory}")
    # strings
