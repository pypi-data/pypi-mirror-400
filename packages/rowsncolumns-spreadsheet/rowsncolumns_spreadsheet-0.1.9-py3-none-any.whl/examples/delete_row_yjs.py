"""
Demo script showing how to call `delete_row_yjs` on a y-websocket room.
"""
...
from rowsncolumns_spreadsheet import delete_row_yjs
...
async def main() -> None:
    ...
    async with websockets.connect(f"{url}/{room}") as websocket:
        ...
        if not sheets_array:
            sheets_array.insert(0, [{"sheetId": 1, "rowCount": 5, "index": 0}])

        previous_state = ydoc.get_state()
        with doc_transaction(ydoc):
            delete_row_yjs(ydoc, 1, [1])

        await client.send_doc_update(previous_state)

        sheet_data = ydoc.get("sheetDataV2")
