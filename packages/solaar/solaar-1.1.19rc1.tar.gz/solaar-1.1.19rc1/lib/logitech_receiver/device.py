...
class Device:
    ...
    def __init__(
        ...
        self._force_buttons = self._special_keys = None
        self._profiles_new = self._profiles = self._backlight = self._settings = self._cids = None
        self.registers = []
    ...
    @property
    def cids(self):  # cids are static per device so store them in the persister
        if self._cids is None:
            if self.persister and self.persister.get("_cid", None) is not None:
                self._cids = self.persister.get("_cid", None)
            elif self.online and self.protocol >= 2.0:
                self._cids = self.persister["_cid"] = _hidpp20.get_cids(self)
        return self._cids
    ...
    @property
    def profiles_new(self):
        if self._profiles_new is None:
            if self.online and self.protocol >= 2.0:
                self._profiles_new = _hidpp20.get_profiles_new(self)
        return self._profiles_new
    ...
    def special_keys(self):
        if self._special_keys is None:
            if self.online and self.protocol >= 2.0:
                self._special_keys = _hidpp20.get_special_keys(self) or ()
    ...
    def __bool__(self):
        result = (self.wpid is not None and bool(self.number)) if self.receiver else self.handle is not None
        return result
