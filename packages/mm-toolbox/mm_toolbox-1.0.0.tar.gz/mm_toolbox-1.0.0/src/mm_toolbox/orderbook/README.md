...
- Standard:
  - `consume_snapshot(bids, asks, has_ticks_and_lots=False)`
  - `consume_deltas(bids, asks, has_ticks_and_lots=False)`
  - `update_bbo(bid, ask, has_ticks_and_lots=False)`       ← method name differs
- Advanced:
  - `consume_snapshot(asks, bids, has_ticks_and_lots)`     ← order is asks, then bids
  - `consume_deltas(asks, bids, has_ticks_and_lots)`       ← order is asks, then bids
  - `consume_bbo(ask, bid, has_ticks_and_lots)`            ← method name differs and order reversed
...
Notes:
- Precision flag: both implementations use `has_ticks_and_lots` (False means auto-compute on ingest).
