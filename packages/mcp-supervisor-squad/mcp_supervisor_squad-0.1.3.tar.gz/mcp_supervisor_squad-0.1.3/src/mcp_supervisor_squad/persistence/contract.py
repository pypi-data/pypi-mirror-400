...
def write_repo_contract(*, layout: StorageLayout, cfg: RepoConfig, package_version: str) -> Path:
    ...
    payload: dict[str, Any] = {
        ...
        "principles": {
            "external_output": "squad_status returns <=3 lines; details are persisted under .codex/supervisor-squad/",
            "max_project_roots": 2,
            "no_compat": True,
        },
        "paths": {
            ...
            "kb_inbox_md": str(layout.kb_dir / "inbox" / "external_report.md"),
            "kb_inbox_json": str(layout.kb_dir / "inbox" / "external_report.json"),
            "kb_archive_dir": str(layout.kb_dir / "archive"),
        },
        "limits": {
            "external_report_max_kb": int(cfg.kb.inbox_max_kb),
        ...
        "config": {
            ...
            "defaults": {
                ...
                "supervisor": _agent_cfg(DEFAULTS.supervisor),
            ...
            "effective": {
                ...
                "supervisor": _agent_cfg(cfg.supervisor),
        ...
        "workflow": {
            ...
            "phases": ["supervisor_run", "coder_run", "integrate", "done", "cleanup", "error"],
            "notes": [
                "Pass options.cwd for squad_status/squad_apply to locate the target repo that created the run.",
                "To inject external research, write kb/inbox/external_report.md|json before calling squad_start.",
