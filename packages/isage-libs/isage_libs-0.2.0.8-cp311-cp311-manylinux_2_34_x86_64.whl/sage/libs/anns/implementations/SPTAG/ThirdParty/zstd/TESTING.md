...
Short tests run on CircleCI for new commits on every branch and pull request. They consist of the
following tests:
...
Medium tests run on every commit and pull request to `dev` branch, on TravisCI. They consist of the
following tests:
...
Long tests run on all commits to `release` branch, and once a day on the current version of `dev`
