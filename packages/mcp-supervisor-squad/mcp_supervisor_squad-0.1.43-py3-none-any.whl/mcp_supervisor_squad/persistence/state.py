...
class RunState:
    ...
    kb_compact_max_kb: int = 0
    kb_archive_keep: int = 0
    kb_archive_max_kb: int = 0
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "RunState":
        ...
        return cls(
            ...
            kb_compact_max_kb=int(payload.get("kb_compact_max_kb") or 0),
            kb_archive_keep=int(payload.get("kb_archive_keep") or 0),
            kb_archive_max_kb=int(payload.get("kb_archive_max_kb") or 0),
    ...
    def public_status(self, *, options: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if self.phase in {"planner_light", "planner_heavy"}:
            artifact_hint = "out: requirements*.md"
        elif self.phase in {"coder_run", "done"}:
            artifact_hint = "out: direct edits"
        ...
        # When无事件时避免过长等待，压到4.1s内（取整到0.1s）。
        if not has_update:
            poll_ms = min(poll_ms, 4100)
        terminal = self.phase in {"done", "canceled", "error"}
        if terminal:
            poll_ms = 0
        self.forced_wait_ms = poll_ms
        forced_wait_s = math.ceil(poll_ms / 100) / 10.0

        # Prepare display events; keep full list in state.
        if terminal:
            display_events = events_lines
        else:
            max_lines = 20
            if len(events_lines) > max_lines:
                extra = len(events_lines) - max_lines
                display_events = ["...%d earlier events" % extra] + events_lines[-max_lines:]
            else:
                display_events = events_lines

        payload = {
            "run_id": self.run_id,
            "forced_wait_s": forced_wait_s,
        }
        if display_events:
            payload["event_lines"] = display_events
        if terminal:
            payload["next_step"] = "stop polling; print event_lines once"
            # Clear events after terminal notification.
            self.events = []
        return payload
