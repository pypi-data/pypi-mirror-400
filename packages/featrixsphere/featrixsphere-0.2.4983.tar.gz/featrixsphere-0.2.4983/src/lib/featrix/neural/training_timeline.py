    ...
    **kwargs
    ...
    try:
        ...
        try:
            ...
            logger.warning("⚠️  PyTorch not available, LR schedule will be estimated")
            torch = None
            OneCycleLR = None
        
        # Pre-calculate LR schedule
        base_lr = optimizer_params.get('lr', 1e-4)
        lrs_per_epoch = []
        
        if use_lr_scheduler and torch is not None and OneCycleLR is not None:
            try:
                # Create dummy optimizer to extract schedule
                total_steps = n_epochs * batches_per_epoch
                warmup_fraction = 0.1 if n_epochs > 1000 else 0.3
                
                dummy_optimizer = torch.optim.AdamW([torch.nn.Parameter(torch.zeros(1))], lr=base_lr)
                ...
                # Extract LR for each epoch (average of batch LRs)
                for epoch_idx in range(n_epochs):
                    start_step = epoch_idx * batches_per_epoch
                    end_step = start_step + batches_per_epoch
                    epoch_lrs = []
                    for _ in range(batches_per_epoch):
                        epoch_lrs.append(scheduler.get_last_lr()[0])
                        scheduler.step()
                    avg_lr = sum(epoch_lrs) / len(epoch_lrs)
                    lrs_per_epoch.append(avg_lr)
                
                logger.info(f"   ✅ LR schedule pre-calculated: {len(lrs_per_epoch)} epochs")
            except Exception as e:
