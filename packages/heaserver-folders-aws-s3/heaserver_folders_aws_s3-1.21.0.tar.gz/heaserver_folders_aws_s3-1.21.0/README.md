...
* Don't return data for presigned URL endpoints form requests.
* Run boto3 call to get an object's latest version in parallel with other boto3 calls to retrieve file metadata.
* If an unarchive request is made and a restore is in progress, respond with a 400 status code.
* Don't compute permissions when getting the object upload form.
