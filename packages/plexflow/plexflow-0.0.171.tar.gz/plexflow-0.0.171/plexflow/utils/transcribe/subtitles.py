...
class NeuralTranslator:
    ...
    def _load_pipeline(self):
        # We check if pipe exists AND if the src_lang matches the current configuration.
        # If the detected language changed, we might need to reload or re-init the tokenizer/pipeline logic 
        # (though for NLLB pipeline, usually just passing the correct src_lang to the pipeline call is enough, 
        # but the HuggingFace pipeline object stores src_lang).
        if self.pipe is None or self.pipe.tokenizer.src_lang != self.src_lang:
            print(f"🔹 Loading Translation model ({self.model_name}) | Src: {self.src_lang} -> Tgt: {self.tgt_lang}...")
        ...
        for i, segment in enumerate(result.segments):
            ...
            if is_end_of_sentence or is_last_segment:
                ...
                if full_eng_sentence.strip():
                    try:
                        # Ensure we use the detected source language
...
class SubtitlePipeline:
    def __init__(self, 
        ...
        # NOTE: src_lang is removed from init, it will be detected
        self.transcriber = AudioTranscriber(model_size=model_size)
        
        # Initialize translator with a placeholder src_lang (e.g. 'eng_Latn')
        # This will be overwritten after transcription detects the real language
    ...
    def run(self, audio_path: str, output_srt_path: str):
        ...
        # WhisperResult stores the language code (e.g., 'en', 'fr') in result.language
        detected_whisper_lang = getattr(result, 'language', 'en') # Default to en if missing
        
        # Convert Whisper code to NLLB code
        nllb_lang = get_nllb_code(detected_whisper_lang)
        
        print(f"🌍 Detected Whisper Language: '{detected_whisper_lang}' -> NLLB Code: '{nllb_lang}'")
        
        # Update the translator instance with the detected language
...
if __name__ == "__main__":
    pipeline_runner = SubtitlePipeline(
        tgt_lang="nld_Latn", # Dutch target
        model_size="medium"
    )
