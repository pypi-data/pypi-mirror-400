...
from snorkelai.sdk.client_v3.tdm.api.annotation_tasks import (
    update_datapoint_status_annotation_tasks__annotation_task_uid__datapoint_datapoint_status_complete_patch as update_datapoint_status_to_complete,
)
from snorkelai.sdk.client_v3.tdm.api.dataset_ground_truth import (
    get_dataset_ground_truth_dataset_ground_truth_get as get_dataset_ground_truth,
)
...
# Extract field names from DatasetAnnotation model
ANNOTATION_FIELD_NAMES = {field.name for field in attrs.fields(DatasetAnnotation)}
...
class AnnotationTask(Base):
    ...
    def _get_display_data(self) -> dict:
        """Get the data dictionary for display purposes."""
        # Prepare grouped label schemas data
        grouped_schemas = []
        for group in self.annotation_form.grouped_label_schemas:
            group_data = {
                "name": group.name,
                "label_schema_uids": group.label_schema_uids,
            }
            if group.description:
                group_data["description"] = group.description
            grouped_schemas.append(group_data)
        ...
        return {
            ...
            "datapoints": {
                "count": len(self.x_uids),
                "sample": self.x_uids[:5] if self.x_uids else [],
            },
            "annotation_form": {
                "individual_label_schemas": self.annotation_form.individual_label_schemas,
                "grouped_label_schemas": grouped_schemas,
                "total_label_schemas": len(self.label_schema_uids),
            },
        }

    def __str__(self) -> str:
        """Return a compact, JSON-like string representation of the AnnotationTask object."""
        return json.dumps(self._get_display_data(), indent=2, default=str)

    def __repr__(self) -> str:
        """Return a detailed string representation of the AnnotationTask object."""
        return f"AnnotationTask(name={self.name!r}, uid={self.uid}, dataset_uid={self.dataset_uid})"
    ...
    def add_annotations(
        ...
        **Dictionaries of different label types:**
        ...
        .. code-block:: python
            ...
            single_label = [{'label': 'category_a', 'x_uid': 'doc::6', 'dataset_uid': 1003, 'label_schema_uid': 101}]

            # Multi-label classification
            multi_label = [{'label': ['tag1', 'tag2', 'tag3'], 'x_uid': 'doc::7', 'dataset_uid': 1003, 'label_schema_uid': 102}]

            # Sequence tagging (NER)
            sequence_tags = [{'label': [[0, 5, 'B-PER'], [6, 15, 'B-LOC']], 'x_uid': 'doc::8', 'dataset_uid': 1003, 'label_schema_uid': 103}]

            # Text annotation (freeform)
            text_annotation = [{'label': {}, 'freeform_text': 'User feedback here', 'x_uid': 'doc::9', 'dataset_uid': 1003, 'label_schema_uid': 104}]
    ...
    def mark_datapoints_complete(
        self, x_uids: Optional[List[str]] = None
    ) -> Dict[str, List[str]]:
        """Mark datapoints that have ground truth completed for all required label schemas.

        This method searches for datapoints that have ground truth present for all
        required label schemas and automatically advances their status to COMPLETED.

        Parameters
        ----------
        x_uids
            Optional list of datapoint UIDs to check. If None, checks all datapoints
            in the annotation task that are not already COMPLETED.

        Returns
        -------
        Dict[str, List[str]]
            A dictionary with two keys:
            - "completed": List of x_uids that were successfully marked completed
            - "skipped": List of x_uids that were skipped (missing GT for some label schemas)
        ...
        .. testcode::
            ...
            print(f"Completed: {len(result['completed'])}, Skipped: {len(result['skipped'])}")
        ...
        .. testcode::
            ...
            from snorkelai.sdk.develop import AnnotationTask
            task = AnnotationTask.get(annotation_task_uid=123)
            result = task.mark_datapoints_complete(x_uids=["doc::1", "doc::2", "doc::3"])
        ...
        """
        status_df = self.get_annotation_status(user_format=False)

        x_uids_to_check = []
        already_completed = []

        for x_uid in status_df.index:
            is_completed = status_df.loc[x_uid, "status"] == DataPointStatus.COMPLETED
            is_in_requested_xuids = x_uids is None or x_uid in x_uids

            if is_in_requested_xuids:
                if is_completed:
                    already_completed.append(x_uid)
                else:
                    x_uids_to_check.append(x_uid)

        if not x_uids_to_check and not already_completed:
            logger.info(
                f"No datapoints to check for commit in annotation task {self.name}"
            )
            return {"completed": [], "skipped": []}

        required_label_schema_uids = set(self.label_schema_uids)

        if not required_label_schema_uids:
            logger.warning(
                f"Annotation task {self.name} has no label schemas. "
                "Cannot complete any datapoints."
            )
            return {
                "completed": [],
                "skipped": list(x_uids_to_check) + already_completed,
            }

        completed = []
        skipped = already_completed.copy()
        try:
            gt_by_x_uid: Dict[str, set] = {}
            for label_schema_uid in required_label_schema_uids:
                ground_truth = get_dataset_ground_truth(
                    dataset_uid=self.dataset_uid,
                    label_schema_uids=[label_schema_uid],
                )
                for gt in ground_truth:
                    x_uid = gt.x_uid
                    if x_uid in x_uids_to_check:
                        if x_uid not in gt_by_x_uid:
                            gt_by_x_uid[x_uid] = set()
                        gt_by_x_uid[x_uid].add(label_schema_uid)

            for x_uid in x_uids_to_check:
                gt_label_schemas = gt_by_x_uid.get(x_uid, set())

                if required_label_schema_uids.issubset(gt_label_schemas):
                    try:
                        update_datapoint_status_to_complete(
                            annotation_task_uid=self.uid,
                            body=UpdateStatusOnGTCommitParams(x_uids=[x_uid]),
                        )
                        completed.append(x_uid)
                        logger.debug(
                            f"Completed datapoint {x_uid} in annotation task {self.name}"
                        )
                    except HTTPError as e:
                        raise ValueError(
                            f"Error completing datapoints: {e.response.json()['detail']}"
                        ) from e
                    except Exception as e:
                        logger.warning(
                            f"Failed to complete datapoint {x_uid}.", exc_info=True
                        )
                        skipped.append(x_uid)
                else:
                    missing_schemas = required_label_schema_uids - gt_label_schemas
                    logger.debug(
                        f"Skipping datapoint {x_uid}: missing GT for label schemas {missing_schemas}"
                    )
                    skipped.append(x_uid)

            logger.info(
                f"Completion summary for annotation task {self.name}: "
            ...
            raise ValueError("Error completing datapoints.") from e
