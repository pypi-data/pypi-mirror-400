    ...
    def request_ready_chunks(self) -> Iterator:
        for i in range(sum(len(file_request.chunks) for file_request in self.active_request.files)):
            ...
            if self.device_str == "cpu":
                logger.info(f"[Perf] Blob→CPU: {blob_time*1000:.2f}ms, chunk_size={humanize.naturalsize(chunk_buffer.size, binary=True)}")
                ...
                logger.info(f"[Perf] Blob→CPU: {blob_time*1000:.2f}ms, CPU→GPU: {gpu_time*1000:.2f}ms, chunk_size={humanize.naturalsize(chunk_buffer.size, binary=True)}")
