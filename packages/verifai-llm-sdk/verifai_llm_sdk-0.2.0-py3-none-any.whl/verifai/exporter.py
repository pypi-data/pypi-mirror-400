...
class VerifAISpanExporter(SpanExporter):
    def __init__(self, api_url: str, project_name: str = "default_project"):
        if api_url.endswith("/api/v1/traces"):
            self.api_url = api_url
        else:
            self.api_url = f"{api_url.rstrip('/')}/api/v1/traces"
    ...
    def _translate_span(self, span: ReadableSpan):
        ...
        if "span_type" in attributes:
            ...
            span_type = "tool"
        elif "gen_ai.system" in attributes or "llm.request.type" in attributes or "gen_ai.request.model" in attributes:
            span_type = "llm"

        # Map OTel GenAI attributes to input/output if not already set
        if not input_data:
            # Try standard GenAI input keys
            if "gen_ai.prompt.0.content" in attributes:
                input_data = {"prompt": attributes.get("gen_ai.prompt.0.content")}
            elif "llm.prompts" in attributes:
                input_data = {"prompt": attributes.get("llm.prompts")}
            elif "gen_ai.system" in attributes:
                 # Fallback for some instrumentations
                 input_data = {
                     "system": attributes.get("gen_ai.system"),
                     "user": attributes.get("gen_ai.prompt.0.content", "Unknown")
                 }

        if not output_data:
            # Try standard GenAI output keys
            if "gen_ai.completion.0.content" in attributes:
                output_data = {"completion": attributes.get("gen_ai.completion.0.content")}
            elif "llm.completions" in attributes:
                output_data = {"completion": attributes.get("llm.completions")}
