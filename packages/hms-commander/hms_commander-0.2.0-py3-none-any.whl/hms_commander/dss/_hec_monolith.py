"""
HEC Monolith Downloader for hms-commander

Downloads HEC Monolith JARs and native libraries from HEC Nexus repository
for DSS file operations. Based on dssrip2's approach.

Library storage: ~/.hms-commander/dss/
"""
...
class HecMonolithDownloader:
    """
    Download and manage HEC Monolith libraries for hms-commander.

    Downloads Java JARs and native libraries required for DSS file operations.
    Libraries are cached in ~/.hms-commander/dss/ for reuse across sessions.
    """
    ...
    # Required JAR files (from dssrip2's requirements.yaml)
    COMMON_JARS = [
    ...
    def __init__(self, cache_dir: Optional[Path] = None):
        ...
        Args:
            cache_dir: Directory to cache downloads.
                      Defaults to ~/.hms-commander/dss/
        """
        if cache_dir is None:
            cache_dir = Path.home() / ".hms-commander" / "dss"
    ...
    def download_file(self, url: str, dest: Path, description: str = "") -> Path:
        ...
        # Lazy import requests to avoid startup overhead
        try:
            import requests
        except ImportError:
            raise ImportError(
                "requests is required for DSS library download.\n"
    ...
    def download_file_with_tqdm(self, url: str, dest: Path, description: str = "") -> Path:
        """
        Download file with tqdm progress bar (if available).
        ...
        if use_tqdm:
            ...
            return self.download_file(url, dest, description)
    ...
    def download_jars(self):
        ...
        for artifact in self.COMMON_JARS:
            ...
            try:
                self.download_file_with_tqdm(url, dest, artifact['artifactId'])
            except Exception as e:
                logger.error(f"Failed to download {artifact['artifactId']}: {e}")
                raise
    ...
    def download_native_library(self):
        ...
        # Download ZIP
        self.download_file_with_tqdm(url, dest, f"Native library ({platform_name})")
    ...
    def install(self, force: bool = False):
        ...
        print("=" * 80)
        print("Installing HEC Monolith Libraries for hms-commander")
        print("=" * 80)
    ...
    def get_info(self) -> Dict:
        ...
        total_size = sum(f.stat().st_size for f in jars + libs if f.is_file())
        ...
        return {
            ...
            "lib_files": [l.name for l in sorted(libs) if l.is_file()],
...
if __name__ == "__main__":
    ...
    print("HEC Monolith Downloader Test (hms-commander)")
    print("=" * 80)
