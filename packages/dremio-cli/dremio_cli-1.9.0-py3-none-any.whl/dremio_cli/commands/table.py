...
@click.argument("dataset_id")
@click.pass_context
def get_table(ctx, dataset_id: str) -> None:
    """Get table by ID.
    
    Examples:
        dremio table get abc-123
    """
    try:
        # Get profile
        manager = ProfileManager()
        ...
        # Get table
        with console.status(f"[bold green]Fetching table {dataset_id}..."):
            data = client.get_catalog_item(dataset_id)
        ...
        if output_format == "json":
            ...
            format_as_table(data, title=f"Table: {dataset_id}")
...
@click.argument("path")
@click.pass_context
def get_table_by_path(ctx, path: str) -> None:
    """Get table by path.
    
    Examples:
        dremio table get-by-path space.folder.table
    """
    try:
        # Get profile
        manager = ProfileManager()
        ...
        # Get table by path
        with console.status(f"[bold green]Fetching table {path}..."):
            # Prepare path
            path_list = path.split(".")
            data = client.get_catalog_item_by_path(path_list)
