...
from .market_simulator import MarketSimulator, SlippageModel, LinearSlippageModel
    ...
    def add_fill(self, fill_event: FillEvent) -> float:
        ...
        if fill_event.side == OrderSide.BUY:
            ...
            if self.quantity >= 0:
                ...
                total_cost = (self.quantity * self.avg_cost) + (fill_event.fill_quantity * fill_event.fill_price)
            ...
            if self.quantity > 0:
                ...
                total_cost = (abs(self.quantity) * self.avg_cost) + (fill_event.fill_quantity * fill_event.fill_price)
                self.quantity -= fill_event.fill_quantity
...
class PandasDataHandler(DataHandler):
    ...
    def _prepare_data(self) -> None:
        ...
        if isinstance(self.data.index, pd.MultiIndex):
            ...
            for date in self.data.index:
                for symbol in self.symbols:
                    if f"{symbol}_Close" in self.data.columns or symbol in self.data.columns:
                        event = self._create_market_event_from_columns(date, symbol, self.data.loc[date])
                        self.market_events.append(event)
        
        # Sort by timestamp
        self.market_events.sort(key=lambda x: x.timestamp)
    
    def _create_market_event(self, date: datetime, symbol: str, row: pd.Series) -> MarketEvent:
    ...
    def _create_market_event_from_columns(self, date: datetime, symbol: str, row: pd.Series) -> MarketEvent:
        """Create market event from column-based data."""
        # Try different column naming conventions
        open_price = row.get(f"{symbol}_Open", row.get(f"Open_{symbol}", row.get("Open", 0.0)))
        high_price = row.get(f"{symbol}_High", row.get(f"High_{symbol}", row.get("High", 0.0)))
        low_price = row.get(f"{symbol}_Low", row.get(f"Low_{symbol}", row.get("Low", 0.0)))
        close_price = row.get(f"{symbol}_Close", row.get(f"Close_{symbol}", row.get(symbol, 0.0)))
        volume = int(row.get(f"{symbol}_Volume", row.get(f"Volume_{symbol}", row.get("Volume", 0))))
...
class EventDrivenBacktester:
    ...
    def run_backtest(self) -> BacktestResult:
        ...
        if not self.data_handler or not self.strategy:
            raise ValueError("Data handler and strategy must be set before running backtest")
    ...
    def _calculate_results(self, start_date: datetime, end_date: datetime, 
        ...
        if len(portfolio_df) > 1:
            ...
            # Calmar ratio
            if max_drawdown < 0:
                calmar_ratio = (total_return * 252 / len(daily_returns)) / abs(max_drawdown)
            else:
                calmar_ratio = 0.0
        else:
            daily_returns = pd.Series()
...
class BuyAndHoldStrategy(Strategy):
    ...
    def on_market_event(self, market_event: MarketEvent) -> List[SignalEvent]:
        ...
        if market_event.symbol in self.symbols and market_event.symbol not in self.bought:
            # Buy signal
