...
class DMSelectField(DMSelectFieldMixin, DMFieldMixin, wtforms.fields.SelectField):
    pass
...
class DMDateField(DMFieldMixin, wtforms.fields.Field):
    ...
    def process(self, formdata, data=unset_value, extra_filters=None):
        ...
        try:
            ...
            if not all((not field.raw_data or not field.raw_data[0]) and not field.data for field in self.form_field):
        ...
        if self.data:
            try:
                if self.form_field.year.raw_data:
                    digits = len(self.form_field.year.raw_data[0])
                else:
                    digits = len(str(self.form_field.year.data))
            except IndexError:
                digits = 0
