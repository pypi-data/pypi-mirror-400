...
def add(box, serial_port, supply_net, battery_net, solar_net, scope_net, logic_net,
    ...
    if all(v is None for v in [box, serial_port, supply_net, battery_net, solar_net,
                                ...
                                uart_net, arm_net]):
        click.echo(click.style("Error: You must specify at least one setting", fg='red'), err=True)
        raise click.Abort()
    ...
    if box is not None:
        ...
        boxes = load_boxes()
        if box not in boxes:
            click.secho(f"Error: Box '{box}' does not exist in saved boxes.", fg='red', err=True)
            click.echo("Available boxes:", err=True)
            if boxes:
                for name in boxes.keys():
                    click.echo(f"  - {name}", err=True)
            else:
                click.echo("  (No boxes saved)", err=True)
            click.echo(f"\nUse 'lager boxes add --name <name> --ip <ip>' to add a new box first.", err=True)
            raise click.Abort()
