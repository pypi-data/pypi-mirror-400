...
def PivotPoints(high: pd.Series, low: pd.Series, close: pd.Series) -> Dict[str, pd.Series]:
    """
    Pivot Points
...
def FibonacciRetracement(high: pd.Series, low: pd.Series, 
                        levels: list = None) -> Dict[str, pd.Series]:
    """
...
def SupportResistance(prices: pd.Series, window: int = 20, 
                     min_touches: int = 2) -> Dict[str, pd.Series]:
    """
