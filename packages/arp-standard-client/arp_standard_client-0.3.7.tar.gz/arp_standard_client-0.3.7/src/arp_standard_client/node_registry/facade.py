...
class NodeRegistryClient:
    ...
    def get_node_type(self, request: NodeRegistryGetNodeTypeRequest) -> NodeType:
        params = request.params
        resp = get_node_type.sync_detailed(client=self._client, node_type_id=params.node_type_id, version=_UNSET if params.version is None else params.version)
        return _unwrap(resp)

    def health(self, request: NodeRegistryHealthRequest) -> Health:
        _ = request
        resp = health.sync_detailed(client=self._client)
        return _unwrap(resp)

    def list_node_types(self, request: NodeRegistryListNodeTypesRequest) -> list[NodeType]:
        params = request.params
        resp = list_node_types.sync_detailed(client=self._client, q=_UNSET if params.q is None else params.q, kind=_UNSET if params.kind is None else params.kind)
        return _unwrap(resp)

    def publish_node_type(self, request: NodeRegistryPublishNodeTypeRequest) -> NodeType:
        resp = publish_node_type.sync_detailed(client=self._client, body=request.body)
        return _unwrap(resp)

    def version(self, request: NodeRegistryVersionRequest) -> VersionInfo:
        _ = request
