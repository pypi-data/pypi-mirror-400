...
class DecisionForestRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if self.base_model is None:
            self.base_model_ = DecisionTreeRegressor(random_state=42, max_depth=10, min_samples_split=5, min_samples_leaf=2, max_features='sqrt')
        else:
...
class DecisionSupportRegressor(BaseEstimator, RegressorMixin):
...
class DecisionKNNRegressor(BaseEstimator, RegressorMixin):
...
class DecisionLinearRegressor(BaseEstimator, RegressorMixin):
...
class DecisionXGBRegressor(BaseEstimator, RegressorMixin):
...
class DecisionCatRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if self.meta_model is None:
            self.meta_model_ = CatBoost.CatBoostRegressor(verbose=0, random_state=42, iterations=100, depth=10, learning_rate=0.1, l2_leaf_reg=3, border_count=32)
        else:
            self.meta_model_ = clone(self.meta_model)
...
class DecisionGradientRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv # Number of cross-validation folds
    ...
    def fit(self, X, y):
        ...
        if self.meta_model is None:
            self.meta_model_ = GradientBoostingRegressor(n_estimators=100, random_state=42, max_depth=10, learning_rate=0.1, subsample=0.8, max_features='sqrt')
        else:
            self.meta_model_ = clone(self.meta_model)
...
class DecisionAdaRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv # Number of cross-validation folds
...
class DecisionLightRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv # Number of cross-validation folds
