...
from cdpy import __cdpy_home__
from cdpy.browser import Browser
from cdpy.chromium_downloader import current_platform
from cdpy.connection import Connection
from cdpy.errors import BrowserError
from cdpy.helper import addEventListener, debugError, removeEventListeners
...
pyppeteer_home = Path(__cdpy_home__)
...
class Launcher(object):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.defaultViewport = options.get(
        ...
        self.temporaryUserDataDir: str | None = None

        if not any(
            arg for arg in self.chromeArguments if arg.startswith("--remote-debugging-")
        ):
            self.chromeArguments.append(f"--remote-debugging-port={self.port}")

        if not any(
            arg for arg in self.chromeArguments if arg.startswith("--user-data-dir")
        ):
