...
def _system_state_properties(
    ...
    # Ensure all values are arrays for indexing and broadcasting operations
    # This converts scalars to 1-element arrays, and leaves arrays unchanged
    radius_array = np.atleast_1d(particle_radius)
    mass_array = np.atleast_1d(particle_mass)
    friction_array = np.atleast_1d(friction_factor)
    coulomb_array = np.atleast_1d(coulomb_potential_ratio)
    diffusive_kn_array = np.atleast_1d(diffusive_knudsen)

    sum_of_radii = radius_array[:, np.newaxis] + radius_array[np.newaxis, :]
    reduced_mass = get_reduced_self_broadcast(mass_array)
    reduced_friction_factor = get_reduced_self_broadcast(friction_array)
