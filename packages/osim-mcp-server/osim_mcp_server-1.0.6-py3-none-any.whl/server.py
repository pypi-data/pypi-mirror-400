...
def filter_extra_params(func):
    """
    装饰器：过滤掉函数签名中不存在的额外参数。
    
    由于 FastMCP 不支持 **kwargs，但某些客户端可能会传入额外的参数（如 task_id、sessionId 等），
    这个装饰器会在调用函数前过滤掉这些未知参数。
    
    注意：装饰器顺序很重要，应该放在 @mcp.tool() 之后，这样 FastMCP 先注册原始函数签名，
    然后在运行时通过包装器过滤额外参数。
    """
