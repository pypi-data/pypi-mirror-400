"""DMN15333LS - Diamond Light Source powder diffraction data fitting (3-parameter LS).
...
def _load_dmn15333ls_data():
    """Load problem data from NPZ file."""
    data_file = Path(__file__).parent / "data" / "dmn15333ls.npz"
    return np.load(data_file)


# Load data once at module level
_DMN15333LS_DATA = _load_dmn15333ls_data()


class DMN15333LS(AbstractUnconstrainedMinimisation):
    """DMN15333LS Diamond Light Source powder diffraction fitting (3-parameter LS).
    ...
    DMN15333 (0.001-0.002 differences). Issue likely due to numerical precision
    ...
    def objective(self, y, args):
        ...
        """
        del args
        nvec = int(_DMN15333LS_DATA["nvec"])
        ...
        # Load data
        x_data = jnp.array(_DMN15333LS_DATA["x_data"])
        y_data = jnp.array(_DMN15333LS_DATA["y_data"])
    ...
    def y0(self):
        """Starting point."""
        if self.y0_iD == 0:
            return jnp.array(_DMN15333LS_DATA["start1"])
        else:
            return jnp.array(_DMN15333LS_DATA["start2"])
