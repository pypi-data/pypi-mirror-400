"""DMN37143 - Diamond Light Source powder diffraction data fitting (3-parameter).
...
def _load_dmn37143_data():
    """Load problem data from NPZ file."""
    data_file = Path(__file__).parent / "data" / "dmn37143.npz"
    return np.load(data_file)


# Load data once at module level
_DMN37143_DATA = _load_dmn37143_data()


class DMN37143(AbstractNonlinearEquations):
    """DMN37143 Diamond Light Source powder diffraction fitting (3-parameter).
    ...
    Jacobian precision issue: max absolute difference of 0.00134 at starting point
    ...
    def n(self):
        """Number of variables."""
        return int(_DMN37143_DATA["n"])

    @property
    def m(self):
        """Number of equations."""
        return int(_DMN37143_DATA["m"])
    ...
    def residual(self, y, args):
        ...
        # Load data
        x_data = jnp.array(_DMN37143_DATA["x_data"])
        y_data = jnp.array(_DMN37143_DATA["y_data"])
    ...
    def y0(self):
        """Starting point."""
        if self.y0_iD == 0:
            return jnp.array(_DMN37143_DATA["start1"])
        else:
            return jnp.array(_DMN37143_DATA["start2"])
