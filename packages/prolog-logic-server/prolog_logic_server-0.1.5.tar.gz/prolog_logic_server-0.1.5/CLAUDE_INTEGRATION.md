...
This installs the `logic_server_mcp` command for easy access.
...
**Alternative: Direct Script (no install needed)**

Use the repo's shebang script and point Claude directly at it:
```json
{
  "mcpServers": {
    "logic-reasoning": {
      "command": "/path/to/prolog/scripts/logic_server_mcp"
    }
  }
}
```
...
Look for the 🔌 icon in Claude Desktop. You should see "logic-reasoning" connected.
...
| `logic_query` | One-shot queries (stateless) |
...
[I call: assert_facts(abc123, [
  ...
  "upon_termination_must(acme, cease_use).",
  ...
])]
