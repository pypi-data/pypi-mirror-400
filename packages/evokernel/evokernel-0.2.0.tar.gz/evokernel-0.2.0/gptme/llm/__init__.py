...
def _render_streaming_content(content: str, agent_name: str | None, is_thinking: bool) -> Panel:
...
def _reply_stream(
    ...
    try:
        with Live(initial_panel, console=console, refresh_per_second=20, transient=False) as live:
            for char in (char for chunk in stream for char in chunk):
                ...
                if now - last_update > update_interval or char == "\n":
                    live.update(_render_streaming_content(output, agent_name, is_thinking))
                    last_update = now
