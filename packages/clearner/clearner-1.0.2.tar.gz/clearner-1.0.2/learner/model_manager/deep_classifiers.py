...
"""This module includes Learner's deep classifiers. The pattern for deep classifiers is no different than the standard
classifiers. Here, we do not directly wrap sklearn models, etc.
"""
...
class AbstractDeepClassifier:
    """The AbstractDeepClassifier class. This class does not implement any functionality, it defines the structure of
    the child classes.
    """
...
class DeepClassifierHandler(AbstractDeepClassifier):
    """The handlers class calls the appropriate method of the Classifier classes for the type defined in mdl.
    """
    @abc.abstractmethod
    def __init__(self):
        super(AbstractDeepClassifier, self).__init__()  # pragma: no cover
    ...
    def learner_predict_proba(X, mdl, device, carry_data=True, full_data=None, cols_to_carry=None):
        return getattr(sys.modules[__name__], mdl["type"]).learner_predict_proba(X, mdl, device, carry_data, full_data, cols_to_carry)
...
class DeepClassifier:
    """The DeepClassifier class to build a deep learning classification model for tabular data.
    """
    @staticmethod
    def learner_fit(tag, mdl, conf, train_loader, validation_loader):
        """First, instantiate the LearnerDeepClassifier model. Then build the loss function, the optimizer, and
        ...
        """
        model = LearnerDeepClassifier(conf=conf, mdl=mdl)
    ...
    @staticmethod
    def learner_find_learning_rate(tag, mdl, conf, train_loader):
        """First, instantiate the LearnerDeepClassifier model. Then build the loss function and the optimizer.
    ...
    def learner_predict(X, mdl, device, carry_data=True, full_data=None, cols_to_carry=None):
        ...
        with torch.no_grad():
            ...
            if isinstance(getattr(mdl["model"], "fully_connected_layers")[-1], torch.nn.LogSoftmax):
                pred = torch.exp(pred)
            # if activation is not LogSoftmax and not Softmax, we need to pass it through a softmax function to get
            # the probabilities
            if not isinstance(getattr(mdl["model"], "fully_connected_layers")[-1], torch.nn.LogSoftmax) and \
                    not isinstance(getattr(mdl["model"], "fully_connected_layers")[-1], torch.nn.Softmax):
...
class LearnerDeepClassifier(nn.Module):
    """The class to implement a deep classifier model from scratch using the parameters defined by the user. While the
    LearnerDeepClassifier model is quite flexible, it has a specific architecture. It passes all the categorical
    ...
    def __init__(self, conf, mdl):
        """Initialize a LearnerDeepClassifier object using the input arguments.

        :param conf: a conf object.
        :param mdl: an item of models_dict.
        """
        super(LearnerDeepClassifier, self).__init__()
        self._conf = conf
        self._mdl = mdl
