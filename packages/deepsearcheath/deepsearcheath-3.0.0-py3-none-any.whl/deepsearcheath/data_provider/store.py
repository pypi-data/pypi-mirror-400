...
class MarketDataCache:
    ...
    def save(self):
        ...
        if not self.news_data.empty:
            save_news_df = self.news_data.sort_values(['symbol', 'date'])
    ...
    def get_last_news_date(self, symbol: str) -> Optional[datetime.date]:
        """특정 심볼의 마지막 뉴스 날짜 반환"""
        if self.news_data.empty:
            return None
        
        if symbol not in self.news_data['symbol'].values:
            return None
            
        symbol_data = self.news_data[self.news_data['symbol'] == symbol]
        return symbol_data['date'].max().date()
    ...
    def update_news_data(self, new_news: List[Dict]):
        ...
        if self.news_data.empty:
            ...
            # 중복 제거 (symbol, date, title 기준)
            subset_cols = ['date', 'symbol']
    ...
    def get_news(
        ...
        mask = (
            (self.news_data['symbol'].isin(symbols)) & 
            (self.news_data['date'] >= date_from_dt) & 
            (self.news_data['date'] <= date_to_dt)
        )
