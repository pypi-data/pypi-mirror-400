...
class BaseEntity:
    ...
    def describe(self, elements=None, add_describer=True):
        ...
        add_describer : bool
            ...
            Please refer to :func:`phrases.description() <pydialogue.language.phrases.description>` for more information.
    ...
    def generate_description(self):
        ...
        base_desc.generate_description()
        return base_desc
...
class Entity(BaseEntity):
    ...
    def select_unique_descriptions(self, cand_descriptions=None):
        ...
        cand_descriptions : list
            A list of :class:`~pydialogue.environment.descriptions.BaseDescription` instances
    ...
    def validate_reachability(self, player, location_entity, neg_res=None):
        ...
        top_location_entity = location_entity.top_location()
        if 'player' not in player.attributes:
            log.append(tsentences.cont([neg_res, tsentences.be(player, 'is', 'not', 'player')]))

        if (('type' in self.properties and self.properties['type'] != 'door') or 'type' not in self.properties) and top_location_entity != player_top_loc:
            log.append(tsentences.cont([neg_res, tsentences.be([player, "'s", 'location'],
                                       'is', 'not', [top_location_entity.properties["location"][0], top_location_entity])]))
        ...
        if ('type' in self.properties and self.properties['type'] == 'door' and
            ...
            res1 = tsentences.be([player, "'s", 'location'],
                                 'is',
                                 'not',
                                 [self.properties["door_to"].properties["location"][0], self.properties["door_to"]])
            res2 = tsentences.be([player, "'s", 'location'],
                                 'is',
                                 'not',
                                 [self.properties["location"][0], self.properties["location"][1]])
