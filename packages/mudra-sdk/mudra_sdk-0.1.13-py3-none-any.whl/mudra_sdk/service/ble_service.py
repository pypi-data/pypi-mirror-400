...
class BleService:
                ...
                try:
                    match char_enum:
                        ...
                        case MudraCharacteristicUUID.BATTERY_CHARACTERISTIC:
                            ...
                            print(f"Reading BATTERY characteristic: {char.uuid}")
                            try:
                                value = await client.read_gatt_char(char.uuid)
                                self._battery_notification_handler(device, char.uuid, value)
                            except Exception as e:
                                print(f"Failed to read BATTERY characteristic {char.uuid}: {e}")

                        case MudraCharacteristicUUID.FIRMWARE_CHARACTERISTIC:
                            print(f"Reading FIRMWARE characteristic: {char.uuid}")
                            try:
                                value = await client.read_gatt_char(char.uuid)
                                self._firmware_notification_handler(device, char.uuid, value)
                            except Exception as e:
                                print(f"Failed to read FIRMWARE characteristic {char.uuid}: {e}")

                        case MudraCharacteristicUUID.SERIAL_RIGHT_PART_CHARACTERISTIC:
                            print(f"Reading SERIAL RIGHT PART characteristic: {char.uuid}")
                            try:
                                value = await client.read_gatt_char(char.uuid)
                                self._serial_right_notification_handler(device, char.uuid, value)
                            except Exception as e:
                                print(f"Failed to read SERIAL RIGHT PART characteristic {char.uuid}: {e}")

                        case MudraCharacteristicUUID.SERIAL_LEFT_PART_CHARACTERISTIC:
                            print(f"Reading SERIAL LEFT PART characteristic: {char.uuid}")
                            try:
                                value = await client.read_gatt_char(char.uuid)
                                self._serial_left_notification_handler(device, char.uuid, value)
                            except Exception as e:
                                print(f"Failed to read SERIAL LEFT PART characteristic {char.uuid}: {e}")
                        ...
                        case MudraCharacteristicUUID.CHARGING_STATE:
                            ...
                            print(f"Reading CHARGING STATE characteristic: {char.uuid}")
                            try:
                                value = await client.read_gatt_char(char.uuid)
                                self._charging_notification_handler(device, char.uuid, value)
                            except Exception as e:
                                print(f"Failed to read CHARGING STATE characteristic {char.uuid}: {e}")
                        case _:
                            if char_enum:
    ...
    def _firmware_notification_handler(self, device: BLEDevice, sender: int, data: bytearray):
        ...
        if self._delegate:
            self._delegate.on_firmware_version_received(device.address, data.decode('utf-8', errors='ignore'))
    ...
    def _serial_right_notification_handler(self, device: BLEDevice, sender: int, data: bytearray):
        ...
        try:
            string_serial_number = data.decode('utf-8', errors='ignore')
        except Exception:
            string_serial_number = ''.join([chr(b) for b in data])
        try:
            value = int(string_serial_number, 16)
        except Exception:
            value = 0

        if self._delegate:
            self._delegate.on_serial_number_right_part_received(device.address, value)
    ...
    def _serial_left_notification_handler(self, device: BLEDevice, sender: int, data: bytearray):
        ...
        calculated_value = value * self.bar_code_digits if value != 0 else 0

        if self._delegate:
            self._delegate.on_serial_number_left_part_received(device.address, calculated_value)
