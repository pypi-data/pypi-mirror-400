...
# *******************************************************************************#
# This module contains core features of the le_map_tools package.                      #
...
def array_to_df(
    y_values: Union[le.Array, le.List, List[List[float]]],
    x_values: Optional[Union[le.Array, le.List, List[float]]] = None,
    ...
    **kwargs: Any,
    ...
    if isinstance(y_values, le.Array) or isinstance(y_values, le.List):
        y_values = y_values.getInfo()

    if isinstance(x_values, le.Array) or isinstance(x_values, le.List):
...
class BaseChartClass:
    ...
    def __init__(
        self,
        features: Union[le.FeatureCollection, pd.DataFrame],
        ...
        **kwargs: Any,
        ...
        if isinstance(features, le.FeatureCollection):
...
def image_by_class(
    image: le.Image,
    class_band: str,
    region: Union[le.Geometry, le.FeatureCollection],
    reducer: Union[str, le.Reducer] = "MEAN",
...
def image_by_region(
    image: le.Image,
    regions: Union[le.FeatureCollection, le.Geometry],
    reducer: Union[str, le.Reducer],
    scale: int,
    x_property: str,
    **kwargs: Any,
) -> None:
...
def image_doy_series(
    image_collection: le.ImageCollection,
    region: Optional[Union[le.Geometry, le.FeatureCollection]] = None,
    region_reducer: Optional[Union[str, le.Reducer]] = None,
    scale: Optional[int] = None,
    year_reducer: Optional[Union[str, le.Reducer]] = None,
    start_day: int = 1,
    end_day: int = 366,
    chart_type: str = "LineChart",
    colors: Optional[List[str]] = None,
    ...
    **kwargs: Any,
    ...
    def group_by_doy(collection, start, end, reducer):
        ...
        doys = le.FeatureCollection(
            [le.Feature(None, {"doy": i}) for i in range(start, end + 1)]
        )

        # Group images by their day of year.
        filter = le.Filter(le.Filter.equals(leftField="doy", rightField="doy"))
        joined = le.Join.saveAll("matches").apply(
            primary=doys, secondary=collection, condition=filter
        )

        # For each DoY, reduce images across years.
        def reduce_images(doy):
            images = le.ImageCollection.fromImages(doy.get("matches"))
        ...
        return le.ImageCollection(joined.map(reduce_images))

    # Set default values and filters if parameters are not provided.
    region_reducer = region_reducer or le.Reducer.mean()
    year_reducer = year_reducer or le.Reducer.mean()
    ...
    def reduce_doy_images(image):
        ...
        return le.Feature(None, {"doy": image.get("doy")}).set(dictionary)

    reduced = le.FeatureCollection(doy_images.map(reduce_doy_images))
...
def image_doy_series_by_region(
    ...
    **kwargs: Any,
    ...
    if year_reducer is None:
        year_reducer = le.Reducer.mean()
    if region_reducer is None:
        region_reducer = le.Reducer.mean()
...
def doy_series_by_year(
    ...
    region_reducer: Optional[Union[str, le.Reducer]] = None,
    scale: Optional[int] = None,
    same_day_reducer: Optional[Union[str, le.Reducer]] = None,
    start_day: int = 1,
    end_day: int = 366,
    chart_type: str = "LineChart",
    colors: Optional[List[str]] = None,
    ...
    **kwargs: Any,
    ...
    # Set default values and filters if parameters are not provided.
    region_reducer = region_reducer or le.Reducer.mean()
    same_day_reducer = same_day_reducer or le.Reducer.mean()
    ...
    filtered_collection = filtered_collection.filter(
        le.Filter.calendarRange(start_day, end_day, "day_of_year")
    )
    ...
    filter = le.Filter.And(
        le.Filter.equals(leftField="doy", rightField="doy"),
        le.Filter.equals(leftField="year", rightField="year"),
    )
    joined = le.Join.saveAll("matches").apply(
    ...
    def reduce_features(doy_year):
        features = le.FeatureCollection(le.List(doy_year.get("matches")))
...
def image_series(
    image_collection: le.ImageCollection,
    region: Union[le.Geometry, le.FeatureCollection],
    reducer: Optional[Union[str, le.Reducer]] = None,
    scale: Optional[int] = None,
    ...
    **kwargs: Any,
    ...
    def get_stats(image):
        ...
        return le.Feature(None, results)

    # Apply the function over the image collection.
    fc = le.FeatureCollection(
        image_collection.map(get_stats).filter(le.Filter.notNull(band_names))
    )
    df = ee_to_df(fc)
...
def array_values(
    array: Union[le.Array, le.List, List[List[float]]],
    x_labels: Optional[Union[le.Array, le.List, List[float]]] = None,
    axis: int = 1,
