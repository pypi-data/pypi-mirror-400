from kosmos.partitioning.algorithms.metis_partitioner import METISPartitioner
...
def metis_partition_qft_example() -> None:
    """Run example for circuit partitioning using METIS on a QFT circuit."""
    RNG.initialize(seed=1)

    output = ["\n=== Circuit Characteristics ==="]

    # Load QFT circuit from MQT Bench
    benchmark = MQTBenchCircuit(circuit_type="qft", num_qubits=16)
    ...
    partitioner = METISPartitioner(network=None, num_partitions=3)
    assignments = partitioner.partition(circuit)

    output.extend(partitioning_result_description("METIS", assignments, benchmark.num_gates))

    print("\n".join(output))  # noqa: T201


if __name__ == "__main__":
    metis_partition_qft_example()
