...
from ..configs.config_lightgbmmt_model_inference_step import (
    LightGBMMTModelInferenceConfig,
)
from ...core.base.builder_base import StepBuilderBase
from ...core.deps.registry_manager import RegistryManager
from ...core.deps.dependency_resolver import UnifiedDependencyResolver

# Import the multi-task model inference specification
try:
    from ..specs.lightgbmmt_model_inference_spec import LIGHTGBMMT_MODEL_INFERENCE_SPEC

    SPEC_AVAILABLE = True
except ImportError:
    LIGHTGBMMT_MODEL_INFERENCE_SPEC = None
    SPEC_AVAILABLE = False

logger = logging.getLogger(__name__)


class LightGBMMTModelInferenceStepBuilder(StepBuilderBase):
    """
    Builder for a LightGBMMT Multi-Task Model Inference ProcessingStep.

    This implementation uses the specification-driven approach where dependencies, outputs,
    and script contract are defined in the multi-task model inference specification.
    ...
    def __init__(
        ...
        """
        Initializes the builder with a specific configuration for the multi-task model inference step.

        Args:
            config: A LightGBMMTModelInferenceConfig instance containing all necessary settings.
            sagemaker_session: The SageMaker session object to manage interactions with AWS.
            role: The IAM role ARN to be used by the SageMaker Processing Job.
            registry_manager: Optional registry manager for dependency injection
            dependency_resolver: Optional dependency resolver for dependency injection
        """
        if not isinstance(config, LightGBMMTModelInferenceConfig):
            raise ValueError(
                "LightGBMMTModelInferenceStepBuilder requires a LightGBMMTModelInferenceConfig instance."
            )

        # Use the multi-task model inference specification if available
        spec = LIGHTGBMMT_MODEL_INFERENCE_SPEC if SPEC_AVAILABLE else None
        ...
        self.config: LightGBMMTModelInferenceConfig = config
    ...
    def validate_configuration(self) -> None:
        ...
        """
        self.log_info("Validating LightGBMMTModelInferenceConfig...")
        ...
        for attr in required_attrs:
            if not hasattr(self.config, attr) or getattr(self.config, attr) in [
                ...
                raise ValueError(
                    f"LightGBMMTModelInferenceConfig missing required attribute: {attr}"
                )
        ...
        if (
            ...
            raise ValueError(
                "task_label_names must be a non-empty list for multi-task inference"
            )
        ...
        self.log_info("LightGBMMTModelInferenceConfig validation succeeded.")
    ...
    def _get_job_arguments(self) -> List[str]:
        ...
        # For multi-task model inference, we always return just the job_type argument
    ...
    def create_step(self, **kwargs) -> ProcessingStep:
        ...
        """
        self.log_info("Creating LightGBMMTModelInference ProcessingStep...")
        ...
        # Use FrameworkProcessor with source_dir parameter for LightGBMMT model inference
