...
from jcclang.core.jobhub.yamux.exceptions import (
    MuxedStreamEOF,
    MuxedStreamError,
    MuxedStreamReset,
    RawConnError,
...
from jcclang.core.utils.rw_lock import ReadWriteLock

# Configure logger for this module
logger = logging.getLogger("jcclang.core.jobhub.yamux")
...
class YamuxStream(IMuxedStream):
        ...
        async with self.rw_lock.write_lock():
            ...
            while sent < total_len:
                ...
                async with self.window_lock:
                    ...
                    await self.conn.conn.write(header + chunk)
                    sent += to_send
    ...
    async def send_window_update(self, increment: int, skip_lock: bool = False) -> None:
        ...
        async def _do_window_update() -> None:
            ...
            await self.conn.conn.write(header)
...
class Yamux(IMuxedConn):
    def __init__(
        ...
        self.is_initiator_value = (
            is_initiator if is_initiator is not None else conn.is_initiator
        )
    ...
    async def close2(self, error_code: int = GO_AWAY_NORMAL) -> None:
    ...
    async def close(self, error_code: int = GO_AWAY_NORMAL) -> None:
        ...
        try:
            ...
            logger.debug(f"Successfully closed secured_conn")
        except Exception as e:
            logger.debug(f"Error closing secured_conn: {e}")
        self.event_closed.set()
        if self.on_close:
            logger.debug(f"Calling on_close in Yamux.close")
    ...
    async def read_stream(self, stream_id: int, n: int = -1) -> bytes:
        logger.debug(f"Reading from stream {stream_id}, n={n}")
        if n is None:
            n = -1
        ...
        while True:
            async with self.streams_lock:
                if stream_id not in self.streams:
                    logger.debug(f"Stream {stream_id} unknown")
                    raise MuxedStreamEOF("Stream closed")
                if self.event_shutting_down.is_set():
                    logger.debug(f"Stream {stream_id}: connection shutting down")
                    raise MuxedStreamEOF("Connection shut down")
                ...
                if buffer is None:
                    logger.debug(f"Stream {stream_id}:" f"Buffer gone, assuming closed")
                    raise MuxedStreamEOF("Stream buffer closed")
                ...
                if stream.closed:
                    logger.debug(
                        f"Stream {stream_id}:" f"closed=True, raising MuxedStreamReset"
                    )
            ...
            logger.debug(f"Waiting for data on stream {stream_id}")
            try:
    ...
    async def handle_incoming(self) -> None:
        while not self.event_shutting_down.is_set():
            try:
                header = await self.conn.read(HEADER_SIZE)
                if not header or len(header) < HEADER_SIZE:
                    logger.debug(f"Connection closed orincomplete header")
                    self.event_shutting_down.set()
                ...
                if (typ == TYPE_DATA or typ == TYPE_WINDOW_UPDATE) and flags & FLAG_SYN:
                    async with self.streams_lock:
                        if stream_id not in self.streams:
                            ...
                            await self.conn.write(ack_header)
                            logger.debug(f"Sending stream {stream_id}" f"to channel")
                            ...
                            await self.conn.write(rst_header)
                elif (typ == TYPE_DATA or typ == TYPE_WINDOW_UPDATE) and flags & FLAG_RST:
                    async with self.streams_lock:
                        if stream_id in self.streams:
                            logger.debug(f"Resetting stream {stream_id}")
                    ...
                    async with self.streams_lock:
                        if stream_id in self.streams:
                            logger.debug(f"Received ACK for stream" f"{stream_id}")
                elif typ == TYPE_GO_AWAY:
                    error_code = length
                    if error_code == GO_AWAY_NORMAL:
                        logger.debug(f"Received GO_AWAY" f" Normal termination")
                    elif error_code == GO_AWAY_PROTOCOL_ERROR:
                        logger.error(f"Received GO_AWAY Protocol error")
                    elif error_code == GO_AWAY_INTERNAL_ERROR:
                        logger.error(f"Received GO_AWAY: Internal error")
                    else:
                        logger.error(
                            f"Received GO_AWAY" f"with unknown error code: {error_code}"
                        )
                    self.event_shutting_down.set()
                    await self._cleanup_on_error()
                    break
                    ...
                    if flags & FLAG_SYN:
                        logger.debug(f"Received ping request with value" f"{length}")
                        ping_header = struct.pack(
                            YAMUX_HEADER_FORMAT, 0, TYPE_PING, FLAG_ACK, 0, length
                        )
                        await self.conn.write(ping_header)
                    elif flags & FLAG_ACK:
                        logger.debug(f"Received ping response with value" f"{length}")
                elif typ == TYPE_DATA:
                    try:
                        data = await self.conn.read(length) if length > 0 else b""
                        async with self.streams_lock:
                            if stream_id in self.streams:
                ...
                if isinstance(e, IncompleteReadError):
                    ...
                    if isinstance(e, RawConnError):
                        ...
                        if not error_msg.strip():
                            logger.info(
                                f"RawConnError (empty) during cleanup "
                                f"(normal connection shutdown)"
                            )
                        else:
                            # Log non-empty RawConnError as warning
                            logger.warning(
                                f"RawConnError during connection handling "
                                f" {error_msg}"
                            )
                    else:
    ...
    async def _cleanup_on_error(self) -> None:
        ...
        try:
            ...
            logger.error(f"Error closing secured_conn: {close_error}")
        ...
        if self.on_close:
            logger.debug(f"Calling on_close")
            try:
                if inspect.iscoroutinefunction(self.on_close):
                    await self.on_close()
                else:
