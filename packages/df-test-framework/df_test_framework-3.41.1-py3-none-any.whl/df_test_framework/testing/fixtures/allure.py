...
def _auto_allure_observer(request) -> Generator[AllureObserver | None, None, None]:
    ...
    # MQ 事件 (v3.34.1 重构为 End/Error 模式)
    test_event_bus.subscribe(MessagePublishEndEvent, observer.handle_message_publish_end_event)
    test_event_bus.subscribe(MessagePublishErrorEvent, observer.handle_message_publish_error_event)
    test_event_bus.subscribe(MessageConsumeEndEvent, observer.handle_message_consume_end_event)
    test_event_bus.subscribe(MessageConsumeErrorEvent, observer.handle_message_consume_error_event)
