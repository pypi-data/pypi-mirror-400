...
Create a new task in the spec with INVEST validation.
...
| `description` | string | No | Task description (WHAT/WHERE/WHY/SCOPE/ACCEPT/REFS) |
| `acceptance_criteria` | list[string] | No | List of acceptance criteria |
| `priority` | integer | No | Priority (lower = higher, default: 100) |
| `status` | string | No | Initial status (default: "ready") |
| `task_id` | string | No | Task ID (auto-generated if omitted) |
| `depends_on` | list[string] | No | Task IDs this depends on |
| `labels` | list[string] | No | Labels for categorization |
| `locks` | list[string] | No | File glob patterns for lock coordination |
| `prd_source` | string | No | Path to PRD file |
| `prd_refs` | list[string] | No | PRD section anchors (e.g., ["#feature"]) |
| `prd_excerpt` | string | No | Frozen PRD text |
| `validate_prd` | boolean | No | Validate PRD file exists (default: true) |
...
!!! tip "Task Creation Best Practices"
    - Apply INVEST principles for 15-minute completable tasks
    - Use structured description format: WHAT/WHERE/WHY/SCOPE/ACCEPT/REFS
    - Reference PRD sections when applicable
    - Set locks to prevent concurrent edits to the same files

!!! warning "Validation"
    - Task IDs must be unique
    - Dependencies must exist
    - PRD file must exist if `validate_prd=true` (default)
    - Creates cycles are detected and rejected

#### `lodestar.task.update`

Update an existing task's properties. Cannot update status or dependencies (use specific tools for those).
...
| `add_labels` | list[string] | No | Labels to add |
| `remove_labels` | list[string] | No | Labels to remove |
| `add_locks` | list[string] | No | Lock patterns to add |
| `remove_locks` | list[string] | No | Lock patterns to remove |
| `add_acceptance_criteria` | list[string] | No | Criteria to add |
| `remove_acceptance_criteria` | list[string] | No | Criteria to remove |
...
```json
{
  ...
  "taskId": "T042",
  "title": "Updated title",
  "updatedFields": ["title", "priority", "labels"],
  "updatedAt": "2025-01-15T12:30:00Z"
}
```

!!! tip "Incremental Updates"
    Use add/remove parameters for lists (labels, locks, criteria) to avoid race conditions. This is safer than reading, modifying, and writing back the entire list.

#### `lodestar.task.delete`
...
| `cascade` | boolean | No | Delete dependents too (default: false) |
...
```json
{
  ...
  "taskId": "T042",
  "deletedCount": 3,
  "deletedTaskIds": ["T042", "T043", "T044"]
}
```

!!! warning "Cascade Behavior"
    - Without `cascade=true`: Returns error if task has dependents
    - With `cascade=true`: Deletes task and all its dependents recursively
    - Cannot delete tasks with active leases (release first)
    - Deleted tasks remain in spec (soft delete) but hidden from list by default
