...
class ModelTest(apitestcase.ApiTestCase):
  ...
  def test_serialize(self):
    model = le.Model.fromAiPlatformPredictor('some-project', 'some-model')
  ...
  def test_cast(self):
    model = le.Model(
        le.Model.fromAiPlatformPredictor('some-project', 'some-model')
    )
    result = json.loads(model.serialize())
    expect = {
        'result': '0',
  ...
  def test_from_ai_platform_predictor(self):
    ...
    input_type_override = {'c': le.PixelType.int8()}
    input_shapes = {'d': [1, 2]}
    proj = 'epsg:4326'
    fix_input_proj = True
    input_tile_size = [3, 4]
    ...
    output_bands = {'e': {'type': le.PixelType.int16(), 'dimensions': 10}}
    output_properties = {'f': {'type': le.PixelType.int32(), 'dimensions': 11}}
    ...
    expression = le.Model.fromAiPlatformPredictor(
        project_name,
        project_id,
        model_name,
        version,
  ...
  def test_from_vertex_ai(self):
    ...
    expression = le.Model.fromVertexAi(
        endpoint,
        input_properties,
        input_type_override,
        input_shapes,
  ...
  def test_predict_image(self):
    ...
    model = le.Model.fromVertexAi(end_point)
    image_name = 'an image'
    image = le.Image(image_name)
  ...
  def test_predict_properties(self):
    ...
    collection = fc = le.FeatureCollection(feature_collection_name)
    expect = make_expression_graph({
        'arguments': {
            'model': {
                'functionInvocationValue': {
