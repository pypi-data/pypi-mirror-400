...
Call stcflash with ```-h``` for usage information.

```
usage: stcflash [-h] [-e] [-a] [-A {dtr,rts}] [-r RESETCMD]
...
Call stcflash without any file to program. It will dump information
about the MCU, e.g.:
...
stcflash supports Intel HEX encoded files as well as binary files. Intel
HEX is autodetected by file extension (.hex, .ihx or .ihex).

Call stcflash just like before, but provide the path to the code image:

```
$ ./stcflash.py -P stc15 hello.hex
Waiting for MCU, please cycle power: done
Target model:
  Name: IAP15F2K61S2
  Magic: F449
...
stcflash uses a conservative baud rate of 19200 bps by
default. Programming can be sped up by choosing a faster baud rate
with the flag ```-b```.

### Device options

stcflash dumps a number of target options. These can be modified as
...
```
$ ./stcflash.py -P stc15 -o clock_source=external hello.bin
```
...
stcflash can execute a trim procedure to adjust it to a given value. This
...
If trimming fails, stcflash will abort.

### Automatic power-cycling

STC's microcontrollers require a power-on reset to invoke the bootloader,
which can be inconvenient. stcflash can use the DTR control signal of a
...
```
  $ ./stcflash.py -P stc15 -a -r "echo 1 > /sys/class/gpio/gpio666/value"
```
or

```
  $ ./stcflash.py -P stc15 -a -r "./powercycle.sh"
```

### Exit status

The exit status is 0 if no error occured while executing stcflash. Any
error, such as a protocol error or I/O error, results in an exit
status of 1. If the the user aborted stcflash by pressing CTRL-C,
that results in an exit status of 2.

### USB support

STC15W4 series have an USB-based BSL that can be optionally
used. USB support in stcflash is experimental and might change in the
