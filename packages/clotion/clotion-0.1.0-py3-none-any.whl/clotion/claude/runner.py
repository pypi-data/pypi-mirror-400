...
from clotion.claude.hooks import (
    BlockedDetection,
    CompletionDetection,
    build_prompt,
    build_resume_prompt,
...
class ClaudeRunner:
    ...
    async def run(self, additional_context: Optional[str] = None) -> SessionResult:
        ...
        """
        session_id = f"clotion-{uuid.uuid4().hex[:8]}"
        ...
        logger.info(
            f"Starting Claude session {session_id} for {self.task_identifier}"
        )
...
class ClaudeRunnerPool:
    ...
    async def resume_runner(
        self, page_id: str, user_input: str
    ) -> Optional[SessionResult]:
        """Resume a blocked runner.

        Args:
            page_id: Page ID of the runner
            user_input: User's input

        Returns:
            SessionResult or None if runner not found
        """
        runner = self._runners.get(page_id)
        if not runner:
            logger.warning(f"No runner found for page {page_id}")
            return None

        async with self._semaphore:
            return await runner.resume(user_input)

    def get_runner(self, page_id: str) -> Optional[ClaudeRunner]:
        """Get a runner by page ID.
        ...
        """
        return self._runners.get(page_id)

    async def cancel_runner(self, page_id: str) -> bool:
