...
from .constants import (
    ALL_BOOKS,
    API_APOCRYPHA_ENDPOINT,
    API_APOSTOLIC_FATHERS_ENDPOINT,
    API_BASE_URL,
    API_COMMENTARY_ENDPOINT,
...
class FHLBibleClient:
    ...
    def get_apocrypha_verse(
        ...
        """Get a specific Apocrypha verse.

        This method queries verses from the Apocrypha (Deuterocanonical books).
        The Apocrypha uses a fixed version (1933年聖公會出版).

        Args:
            book_id: Book ID (101-115 for Apocrypha)
            chapter: Chapter number
            verse: Verse number
            simplified: Use simplified Chinese (default: False)
        ...
        Raises:
            InvalidBookError: If book_id is not in Apocrypha range (101-115)
            APIConnectionError: If unable to connect to API
            APIResponseError: If API returns invalid response
        ...
        """
        # Validate that book_id is in Apocrypha range
        if book_id < 101 or book_id > 115:
            raise InvalidBookError(
                f"Book ID {book_id} is not in Apocrypha range (101-115). "
                "Use get_verse() for canonical books."
            )
        ...
        url = f"{self.base_url}{API_APOCRYPHA_ENDPOINT}?{urlencode(params)}"
    ...
    def get_apocrypha_chapter(
        self,
        book_id: int,
        chapter: int,
        simplified: bool = False,
    ) -> list[BibleQueryResponse]:
        """Get all verses from a specific Apocrypha chapter.
        ...
        if book_id < 101 or book_id > 115:
            raise InvalidBookError(
                f"Book ID {book_id} is not in Apocrypha range (101-115). "
                "Use get_chapter() for canonical books."
            )
        ...
        while verse_num <= max_attempts:
            try:
                response = self.get_apocrypha_verse(
                    book_id=book_id,
                    chapter=chapter,
                    verse=verse_num,
    ...
    def get_apostolic_fathers_verse(
        self,
        book_id: int,
        chapter: int,
        verse: int,
        ...
        """Get a specific Apostolic Fathers verse.

        This method queries verses from the Apostolic Fathers writings.
        The Apostolic Fathers uses a fixed version (黃錫木主編《使徒教父著作》).

        Args:
            book_id: Book ID (201-217 for Apostolic Fathers)
            chapter: Chapter number
            verse: Verse number
            simplified: Use simplified Chinese (default: False)
        ...
        Raises:
            InvalidBookError: If book_id is not in Apostolic Fathers range (201-217)
            APIConnectionError: If unable to connect to API
            APIResponseError: If API returns invalid response
        ...
        Example:
            ...
            >>> print(response.records[0].text)
        """
        # Validate that book_id is in Apostolic Fathers range
        if book_id < 201 or book_id > 217:
            raise InvalidBookError(
                f"Book ID {book_id} is not in Apostolic Fathers range (201-217). "
                "Use get_verse() for canonical books or get_apocrypha_verse() for Apocrypha."
            )
        ...
        url = f"{self.base_url}{API_APOSTOLIC_FATHERS_ENDPOINT}?{urlencode(params)}"
    ...
    def get_apostolic_fathers_chapter(
        self,
        book_id: int,
        chapter: int,
        simplified: bool = False,
    ) -> list[BibleQueryResponse]:
        """Get all verses from a specific Apostolic Fathers chapter.
        ...
        if book_id < 201 or book_id > 217:
            raise InvalidBookError(
                f"Book ID {book_id} is not in Apostolic Fathers range (201-217). "
                "Use get_chapter() for canonical books or get_apocrypha_chapter() for Apocrypha."
            )
        ...
        while verse_num <= max_attempts:
            try:
                response = self.get_apostolic_fathers_verse(
                    book_id=book_id,
                    chapter=chapter,
                    verse=verse_num,
                    simplified=simplified,
    ...
    def _parse_commentary_response(self, data: dict[str, Any]) -> CommentaryResponse:
        """Parse the commentary API response into a CommentaryResponse object.

        Args:
            data: Raw API response data

        Returns:
            Parsed CommentaryResponse object
        """
        status = data.get("status", "unknown")
        ...
        for record in data.get("record", []):
            commentary = Commentary(
                ...
                com_text=record.get("com_text", ""),
            )
            records.append(commentary)
        ...
        if "prev" in data and data["prev"]:
            ...
            prev_verse = BibleReference(
                bid=prev_data.get("bid", 0),
                chinese_abbr=prev_data.get("book", ""),
                english_abbr=prev_data.get("engs", ""),
                chapter=prev_data.get("chap", 0),
                verse=prev_data.get("sec", 0),
            )
        ...
        if "next" in data and data["next"]:
            ...
            next_verse = BibleReference(
                bid=next_data.get("bid", 0),
                chinese_abbr=next_data.get("book", ""),
                english_abbr=next_data.get("engs", ""),
                chapter=next_data.get("chap", 0),
                verse=next_data.get("sec", 0),
            )
    ...
    def get_commentary(
        ...
        """Get Bible commentary for a specific verse or passage.

        This method retrieves commentary from 信望愛站註釋 (FHL Commentary).
        Only supports canonical Bible books (1-66).

        Args:
            book_id: Book ID (1-66 for canonical Bible)
            chapter: Chapter number (default: 0 for all)
            verse: Verse number (default: 0 for all verses in chapter)
            simplified: Use simplified Chinese (default: False)

        Returns:
            CommentaryResponse containing commentary data

        Raises:
            InvalidBookError: If book_id is not in range 1-66
            APIConnectionError: If unable to connect to API
            APIResponseError: If API returns invalid response
        ...
        Example:
            ...
            ...     print(f"{comm.title}: {comm.com_text[:50]}...")
        """
        # Validate that book_id is in canonical Bible range
        if book_id < 1 or book_id > 66:
            raise InvalidBookError(
                f"Book ID {book_id} is not in canonical Bible range (1-66). "
                "Commentary is only available for canonical books."
            )
        ...
        url = f"{self.base_url}{API_COMMENTARY_ENDPOINT}?{urlencode(params)}"
        ...
        return self._parse_commentary_response(data)
