...
class JobContext:
    ...
    def api(self) -> api.LiveKitAPI:
        """Returns a LiveKit API client for making server API calls.
        ...
        RIYADAI_URL, RIYADAI_API_KEY, and RIYADAI_API_SECRET so this API is always
        usable inside job entrypoints.
        """
        ws_url = os.environ.get("RIYADAI_URL", "")
        api_key = os.environ.get("RIYADAI_API_KEY", "")
        api_secret = os.environ.get("RIYADAI_API_SECRET", "")
        return api.LiveKitAPI(ws_url, api_key, api_secret, session=http_context.http_session())
