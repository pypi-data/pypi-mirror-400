...
def calculate_acoustic_impedance(rhob, dtc):
    ...
    AI = rho * Vp
    where rho is density in g/cm³ and Vp is P-wave velocity in m/s.
    ...
    Returns:
        np.ndarray: Acoustic impedance [(g/cm³)*(m/s)].
    """
...
def auto_identify_layers_from_seismic_trace(
    df, method="peaks_troughs", min_distance=30
):
    """Auto-identify layers from seismic trace using peaks/troughs or zero crossings.

    Args:
        df (pd.DataFrame): DataFrame with 'TRACE' and 'DEPTH' columns.
        method (str, optional): 'peaks_troughs' or 'zero_crossings'. Defaults to 'peaks_troughs'.
        min_distance (int, optional): Minimum distance between boundaries in array indices. Defaults to 30.

    Returns:
        pd.DataFrame: The input DataFrame with an added 'LAYERS' column.
    """
    ...
    if method == "peaks_troughs":
        ...
        all_boundaries = zero_crossings[
            np.insert(np.diff(zero_crossings) >= min_distance, 0, True)
        ]
    else:
    ...
    layers = pd.Series(
        data=layer_labels,
        index=depth.iloc[all_boundaries],
        name="LAYERS",
        dtype="category",
    )
...
def calculate_reflectivity_from_layers(df):
    ...
    Returns:
        pd.DataFrame: The input DataFrame with an added 'REFLECTIVITY' column.
    """
    df = df.copy()
...
def calculate_reflectivity_each_step(df):
    ...
    Returns:
        pd.DataFrame: The input DataFrame with a 'REFLECTIVITY' column.
    """
    df = df.copy()
    df["AI"] = calculate_acoustic_impedance(df.RHOB, df.DT.rolling(30).mean())
    df["AVG_DIFF_AI"] = df["AI"].rolling(50).mean().diff()
    df["AVG_DIFF_AI_2"] = df["AVG_DIFF_AI"].shift()
    df["REFLECTIVITY"] = (df["AVG_DIFF_AI_2"] - df["AVG_DIFF_AI"]) / (
        df["AVG_DIFF_AI_2"] + df["AVG_DIFF_AI"]
    )
    ...
    df.loc[
        (df["REFLECTIVITY"] < lower_bound) | (df["REFLECTIVITY"] > upper_bound),
        "REFLECTIVITY",
    ] = np.nan
...
def convert_depth_to_time(depth, velocity):
    ...
    Returns:
        np.ndarray: Two-way-time (TWT) values [s].
...
def convert_md_to_tvd(df, well_coords):
    ...
    Args:
        df (pd.DataFrame): Input DataFrame with 'DEPTH' (as MD) and other log columns.
        well_coords (pd.DataFrame): DataFrame with well deviation survey data, including
                                    'md', 'incl', and 'azim' columns.
    Returns:
        pd.DataFrame: A new DataFrame resampled by TVD with a 0.5m interval.
    """
...
def extract_seismic_along_well(seismic_cube, well_trajectory):
    ...
    Args:
        ...
        well_trajectory (pd.DataFrame): A DataFrame defining the well path with 'x', 'y', and 'z'
                                        (depth/time) coordinates.

    Returns:
        np.ndarray: An array of seismic trace values extracted along the well path.
...
def convert_well_trajectory_to_ilxl(seismic_cube, well_trajectory):
    ...
    Args:
        ...
        well_trajectory (pd.DataFrame): A DataFrame with well deviation survey data, including
                                        'md', 'incl', 'azim', 'x', 'y', and 'z' columns.
    """
    import xarray as xr
    ...
    new_depths = np.arange(0, well_trajectory["md"].max(), 0.1)
...
def plot_well_trajectory(seismic_cube, well_coords):
    ...
    print(
        f"CDP X range: {seismic_cube.cdp_x.min().values:.1f} to {seismic_cube.cdp_x.max().values:.1f}"
    )
    print(
        f"CDP Y range: {seismic_cube.cdp_y.min().values:.1f} to {seismic_cube.cdp_y.max().values:.1f}"
    )
    ...
    ax1.scatter(
        ...
        s=1,
        alpha=0.1,
        label="Seismic coverage",
    )
    ax1.scatter(
        well_coords["x"], well_coords["y"], c="red", s=20, label="Well trajectory"
    )
    ax1.axis("equal")
    ax1.grid(True)
    ...
    ax2.scatter(
        well_coords.x.iloc[0],
        well_coords.y.iloc[0],
        color="green",
        s=100,
        marker="^",
        label="Well head",
    ...
    ax2.scatter(
        well_coords.x.iloc[-1],
        well_coords.y.iloc[-1],
        color="red",
        s=100,
        marker="v",
        label="Well bottom",
...
def plot_seismic_along_well(seismic_cube, well_trajectory):
    ...
    Args:
        seismic_cube (xarray.Dataset): A 3D seismic cube.
        well_trajectory (pd.DataFrame): A DataFrame defining the well path with 'x', 'y',
                                        and 'z' coordinates.
    """
    well_ilxl, z = convert_well_trajectory_to_ilxl(seismic_cube, well_trajectory)
    seismic_along_well = seismic_cube.interp(**well_ilxl)
    seismic_traces_along_well = extract_seismic_along_well(
        seismic_cube, well_trajectory
    )
...
def optimize_wavelet(seismic_trace, reflectivity):
    ...
    Returns:
        tuple[np.ndarray, float, np.ndarray]: A tuple containing:
    ...
    def ricker_wavelet(f0=30, phase=0.0):
        ...
        Args:
            ...
            phase (float): Phase rotation [degrees].

        Returns:
            np.ndarray: The generated Ricker wavelet.
        """
        dt = 0.001
        tmin = -0.15
        tmax = 0.15
...
def create_synthetic_seismogram(wavelet, reflectivity):
    ...
    Returns:
        np.ndarray: The resulting synthetic seismogram trace.
    """
...
def qc_synthetic_seismogram(synthetic, seismic, title="Synthetic vs Seismic"):
    ...
    Returns:
        matplotlib.figure.Figure: The generated QC figure.
    """
    # Create figure
