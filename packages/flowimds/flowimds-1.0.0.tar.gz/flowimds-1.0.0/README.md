...
Compare the legacy (v0.2.1-) and current (v1.0.0+) pipeline implementations with the bundled helper script. Running via `uv` keeps dependencies and the virtual environment consistent:
...
Stable releases are already published on PyPI (v1.0.0), and we continue to iterate toward upcoming updates. Watch the repository for new tags and changelog announcements.
