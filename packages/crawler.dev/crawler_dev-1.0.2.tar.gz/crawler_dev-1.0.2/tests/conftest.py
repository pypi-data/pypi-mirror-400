...
from api.crawler.dev_sdks import APICrawlerDevSDKs, DefaultAioHttpClient, AsyncAPICrawlerDevSDKs
from api.crawler.dev_sdks._utils import is_dict
...
logging.getLogger("api.crawler.dev_sdks").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[APICrawlerDevSDKs]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with APICrawlerDevSDKs(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncAPICrawlerDevSDKs]:
