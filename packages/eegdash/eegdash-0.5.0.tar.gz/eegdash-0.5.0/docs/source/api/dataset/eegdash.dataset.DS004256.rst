..
   This documentation page is generated during the Sphinx build.
   The underlying code is manually maintained and not autogenerated.

eegdash.dataset.DS004256
========================

Encoding of Sound Source Elevation in Human Cortex (OpenNeuro ``ds004256``). Access recordings and metadata through EEGDash.

.. rst-class:: sd-badges

:bdg-light:`Modality: eeg` :bdg-light:`Tasks: 2` :bdg-light:`License: CC0` :bdg-light:`Subjects: 53` :bdg-light:`Recordings: 53` :bdg-light:`Source: openneuro`

Dataset Information
-------------------

.. list-table::
   :widths: 25 75
   :header-rows: 0

   * - Dataset ID
     - ``DS004256``
   * - Title
     - Encoding of Sound Source Elevation in Human Cortex
   * - Year
     - Unknown
   * - Authors
     - Ole Bialas, Marc Schoenwiesner, Burkhard Maess
   * - License
     - CC0
   * - Citation / DOI
     - `doi:10.18112/openneuro.ds004256.v1.0.5 <https://doi.org/10.18112/openneuro.ds004256.v1.0.5>`__
   * - Source links
     - `OpenNeuro <https://openneuro.org/datasets/ds004256>`__ | `NeMAR <https://nemar.org/dataexplorer/detail?dataset_id=ds004256>`__

.. dropdown:: Copy-paste BibTeX
   :class-container: sd-shadow-sm
   :class-title: sd-bg-light

   .. code-block:: bibtex

      @dataset{ds004256,
        title = {Encoding of Sound Source Elevation in Human Cortex},
        author = {Ole Bialas and Marc Schoenwiesner and Burkhard Maess},
        doi = {10.18112/openneuro.ds004256.v1.0.5},
        url = {https://doi.org/10.18112/openneuro.ds004256.v1.0.5},
      }

Highlights
----------

.. grid:: 1 2 3 3
   :gutter: 2

   .. grid-item-card:: Subjects & recordings
      :class-card: sd-border-1

      - Subjects: 53
      - Recordings: 53
      - Tasks: 2

   .. grid-item-card:: Channels & sampling rate
      :class-card: sd-border-1

      - Channels: 64
      - Sampling rate (Hz): 500
      - Duration (hours): 42.337

   .. grid-item-card:: Tasks & conditions
      :class-card: sd-border-1

      - Tasks: 2
      - Experiment type: Unknown
      - Subject type: Healthy

   .. grid-item-card:: Files & format
      :class-card: sd-border-1

      - Size on disk: 18.18 GB
      - File count: Unknown
      - Format: BIDS

   .. grid-item-card:: License & citation
      :class-card: sd-border-1

      - License: CC0
      - DOI: doi:10.18112/openneuro.ds004256.v1.0.5

   .. grid-item-card:: Provenance
      :class-card: sd-border-1

      - Source: openneuro
      - OpenNeuro: `ds004256 <https://openneuro.org/datasets/ds004256>`__
      - NeMAR: `ds004256 <https://nemar.org/dataexplorer/detail?dataset_id=ds004256>`__

Quickstart
----------

**Install**

.. code-block:: bash

   pip install eegdash

**Load a recording**

.. code-block:: python

   from eegdash.dataset import DS004256

   dataset = DS004256(cache_dir="./data")
   recording = dataset[0]
   raw = recording.load()

**Filter/query**

.. tab-set::

   .. tab-item:: Basic

      .. code-block:: python

         dataset = DS004256(cache_dir="./data", subject="01")

   .. tab-item:: Advanced

      .. code-block:: python

         dataset = DS004256(
             cache_dir="./data",
             query={"subject": {"$in": ["01", "02"]}},
         )


Quality & caveats
-----------------

- No dataset-specific caveats are listed in the available metadata.

API
---

.. currentmodule:: eegdash.dataset

.. autoclass:: eegdash.dataset.DS004256
   :members: __init__, save
   :show-inheritance:
   :member-order: bysource


See Also
--------

* :class:`eegdash.dataset.EEGDashDataset`
* :mod:`eegdash.dataset`
* `OpenNeuro dataset page <https://openneuro.org/datasets/ds004256>`__
* `NeMAR dataset page <https://nemar.org/dataexplorer/detail?dataset_id=ds004256>`__

