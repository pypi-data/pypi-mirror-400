...
def test_fetch_remote_tar_and_install(tmp_path):
    ...
    import hashlib
    manifest = {'files': {"file.txt": hashlib.sha256((t / "file.txt").read_bytes()).hexdigest()}}
    ...
    engine = Engine(templates_root=tmp_path / 'templates', user_templates_root=tmp_path / 'user_templates')
    installed = engine.fetch_remote_template(tar_path.as_uri(), name='remoy', force=True)
