...
class RangeSlider:
    ...
    def __init__(
        self, motor, idx, res, calibration, present, label_pad, base_y
    ):
        import pygame
        ...
        self.max_btn = pygame.Rect(
            self.x1 + 6, self.y - BTN_H // 2, BTN_W, BTN_H
        )
...
class RangeFinderGUI:
    def __init__(
        self, bus: MotorsBus, groups: dict[str, list[str]] | None = None
    ):
        import pygame

        self.bus = bus
        self.groups = (
            groups if groups is not None else {'all': list(bus.motors)}
        )
        ...
        label_pad = max(
            self.font.size(m)[0] for ms in groups.values() for m in ms
        )
    ...
    def run(self) -> dict[str, MotorCalibration]:
        ...
        while True:
            ...
            for s in self.sliders:
                if s.drag_pos:
                    self.bus.write(
                        'Goal_Position', s.motor, s.pos_v, normalize=False
                    )

            # tick update
            for s in self.sliders:
                pos = self.bus.read(
                    'Present_Position', s.motor, normalize=False
                )
                s.set_tick(pos)
