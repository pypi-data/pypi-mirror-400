...
class CatLogisticClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        if self.base_model is None:
            self.base_model_ = CatBoost.CatBoostClassifier(iterations=100, depth=10, learning_rate=0.1, random_seed=42)
        else:
...
class CatForestClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv
...
class CatDecisionClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv
    ...
    def fit(self, X, y):
        # Initialize base model and meta model if not provided
        if self.base_model is None:
            self.base_model_ = CatBoost.CatBoostClassifier(iterations=100, depth=10, learning_rate=0.1, random_state=42)
        else:
            self.base_model_ = clone(self.base_model)
            
        if self.meta_model is None:
            self.meta_model_ = DecisionTreeClassifier(max_depth=10, random_state=42)
        else:
            self.meta_model_ = clone(self.meta_model)
        # 2. Generate "out-of-sample" predictions for the meta model
        # We use cross_val_predict ensures that for every point in X, 
...
class CatKNNClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv
    def fit(self, X, y):
        ...
        if self.meta_model is None:
            self.meta_model_ = KNeighborsClassifier(n_neighbors=5, n_jobs=-1)
        else:
            self.meta_model_ = clone(self.meta_model)
...
class CatSupportClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv
...
class CatXGBClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv
...
class CatGradientClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv
    ...
    def fit(self, X, y):
        ...
        if self.meta_model is None:
            self.meta_model_ = GradientBoostingClassifier(n_estimators=100, learning_rate=0.1, max_depth=10, random_state=42)
        else:
            self.meta_model_ = clone(self.meta_model)
...
class CatAdaClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv
...
class CatLightClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv
