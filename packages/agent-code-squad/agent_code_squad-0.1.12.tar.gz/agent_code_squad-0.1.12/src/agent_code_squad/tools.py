...
def register_tools(mcp: FastMCP, *, config: SquadConfig) -> None:
    ...
    def code_squad_apply(
        ...
        try:
            ...
            if not task:
                return {"ok": False, "error": f"Task not found in run: {slug}", "run_id": run_id, "slug": slug}

            scope = task.get("scope") if isinstance(task.get("scope"), dict) else None
            if scope and scope.get("ok") is False and not bool(opts.get("force")):
                return {"ok": False, "error": "scope violations present; re-run with options.force=true to override", "run_id": run_id, "slug": slug, "scope": scope}

            run_dir = Path(run_meta.get("run_dir") or store.run_dir(run_id))
            patch_path = run_dir / "artifacts" / "collect" / f"{slug}.patch"
            if not patch_path.exists():
                return {"ok": False, "error": "patch not found; run code_squad_collect first", "run_id": run_id, "slug": slug}
            patch_text = patch_path.read_text(encoding="utf-8", errors="replace")
            if "diff --git " not in patch_text and patch_text.strip() == "":
                return {"ok": False, "error": "empty patch", "run_id": run_id, "slug": slug}
            ...
            if debug:
                result["debug"] = {"trace_id": trace_id, "elapsed_ms": elapsed_ms, "command": cmd, "paths": {"repo_root": str(repo_root), "patch_path": str(patch_path)}}
            return result
        finally:
            if elapsed_ms is None:
                elapsed_ms = int((time.monotonic() - start) * 1000)
