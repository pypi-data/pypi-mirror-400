...
def check_and_download(resource, path):
    try:
...
# Check for 'punkt', 'wordnet', and 'averaged_perceptron_tagger'
check_and_download('wordnet', 'corpora/wordnet')
check_and_download('punkt_tab', 'tokenizers/punkt_tab')
check_and_download('averaged_perceptron_tagger_eng', 'taggers/averaged_perceptron_tagger_eng')
...
def retrieve_pos(verb):
    """ Caches the verb's (str) part-of-speech (pos) tag.
    ...
    """
    if verb not in cached_pos:
        token = nltk.pos_tag(nltk.word_tokenize(verb))[0]
        if not token[1].startswith('V'):
            lemmatizer = WordNetLemmatizer()
            verb_infinitive = lemmatizer.lemmatize(verb, pos='v')
            # compute the tag
            for key, val in pyinflect.getAllInflections(verb_infinitive).items():
                if key.startswith('V') and verb in val:
                    token = (verb, key)
                    break
        cached_pos[token[0]] = token[1]
        pos = token[1]
    else:
        pos = cached_pos[verb]
...
def retrieve_inflection(verb, tag):
    ...
    if (verb, tag) not in cached_inflections:
        ...
        cached_inflections[(verb, tag)] = pyinflect.getInflection(lemma, tag)[0]
...
class Arg:
    ...
    value : any
        ...
        value but included in the part. Please find more information about how the values are computed in our paper.
...
class Describer:
    ...
    Please check our paper to see all the modifications that we did.
