...
from riyadhai.agents import Plugin, llm
from riyadhai.agents.inference_runner import _InferenceRunner
from riyadhai.agents.ipc.inference_executor import InferenceExecutor
from riyadhai.agents.job import get_job_context
from riyadhai.agents.utils import hw
...
def _download_from_hf_hub(repo_id: str, filename: str, **kwargs: Any) -> str:
    ...
    try:
        ...
        raise RuntimeError(
            "riyadhai-plugins-turn-detector initialization failed. "
            f'Could not find file "{filename}".'
        ) from None
    return local_path
